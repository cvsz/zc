...
Full tool-use support:
  ...
  • Programmatic tool calling (real implementation, not just a bullet
    ...
    20260120, so Claude can invoke your tools from code inside the
...
Version-string / beta-header notes (checked against platform.zaicoder.com/docs
...
# Claude Fable 5, Claude Mythos 5, Opus 4.8, and Opus 4.7 only; not supported
# on ZaiCoder Code or Cowork surfaces, Messages API only.
...
def with_allowed_callers(tool_def: dict, callers: list[str] = None) -> dict:
    """Mark a custom tool definition as callable from Programmatic Tool
    Calling — Claude can then invoke it directly from Python code running
