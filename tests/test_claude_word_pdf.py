"""tests/test_claude_word_pdf.py

Covers claude_word.py / claude_pdf.py: the two Skills-only chat loops
added in v1.33.0 to close out claude_skills_api.py's PREBUILT_SKILLS set
(docx/pdf had been listed there since v1.15.0 with no CLI path to either).
Both modules are structurally identical to claude_excel.py's /
claude_powerpoint.py's --*-native path minus the hand-rolled fallback, so
these tests mirror test_claude_skills_api.py's monkeypatch style rather
...
def _no_real_network(monkeypatch):
    ...
    def _boom(*a, **k):
        raise AssertionError("unmocked network call in claude_word/claude_pdf test")
...
def test_docx_chat_no_input_file_skips_upload(monkeypatch, capsys):
    ...
    out_path = claude_word.cmd_docx_chat("k", "claude-sonnet-5")
...
def test_docx_chat_uploads_input_file_once(monkeypatch):
    ...
    claude_word.cmd_docx_chat("k", "claude-sonnet-5", input_path="draft.docx")
...
def test_docx_chat_upload_failure_exits(monkeypatch):
    ...
    with pytest.raises(SystemExit):
        claude_word.cmd_docx_chat("k", "claude-sonnet-5", input_path="missing.docx")
...
def test_docx_chat_downloads_generated_file(monkeypatch):
    ...
    claude_word.cmd_docx_chat("k", "claude-sonnet-5")
    ...
    _queue_inputs(monkeypatch, ["write a memo", "/exit"])
    claude_word.cmd_docx_chat("k", "claude-sonnet-5", output_path="memo.docx")
...
# ── claude_pdf.cmd_pdf_chat ──────────────────────────────────────────────


def test_pdf_chat_no_input_file_skips_upload(monkeypatch):
    monkeypatch.setattr(FilesAPI, "upload", lambda self, path: pytest.fail("should not upload"))
    _queue_inputs(monkeypatch, ["/exit"])
    out_path = claude_pdf.cmd_pdf_chat("k", "claude-sonnet-5")
...
def test_pdf_chat_uploads_input_file_once(monkeypatch):
    ...
    claude_pdf.cmd_pdf_chat("k", "claude-sonnet-5", input_path="form.pdf")
...
def test_pdf_chat_uses_pdf_skill(monkeypatch):
    ...
    monkeypatch.setattr(SkillsApiClient, "call_with_skills_turn", fake_call)
    _queue_inputs(monkeypatch, ["hi", "/exit"])
    claude_pdf.cmd_pdf_chat("k", "claude-sonnet-5")
