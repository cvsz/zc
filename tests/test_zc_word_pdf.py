...
def test_docx_chat_no_input_file_skips_upload(monkeypatch, capsys):
    monkeypatch.setattr(
        FilesAPI, "upload", lambda self, path: pytest.fail("should not upload")
    )
...
def test_docx_chat_uploads_input_file_once(monkeypatch):
    calls = []
    monkeypatch.setattr(
        FilesAPI, "upload", lambda self, path: calls.append(path) or {"id": "file_1"}
    )
    monkeypatch.setattr(
        SkillsApiClient, "call_with_skills_turn", lambda self, *a, **k: {"content": []}
    )
...
def test_docx_chat_upload_failure_exits(monkeypatch):
    monkeypatch.setattr(
        FilesAPI, "upload", lambda self, path: (_ for _ in ()).throw(OSError("nope"))
    )
...
def test_pdf_chat_uploads_input_file_once(monkeypatch):
    calls = []
    monkeypatch.setattr(
        FilesAPI, "upload", lambda self, path: calls.append(path) or {"id": "file_2"}
    )
    monkeypatch.setattr(
        SkillsApiClient, "call_with_skills_turn", lambda self, *a, **k: {"content": []}
    )
