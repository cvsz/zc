...
Covers the /compact fix in cmd_code_slash() (claude_code.py). Previously
/compact printed a success message and did nothing at all — see
CHANGELOG for this fix, and tests/test_claude_code_context_editing.py
...
from claude_code import CodeSession, cmd_code_slash
from claude_tools import COMPACTION_BETA
...
def test_compact_without_session_id_does_not_call_api(monkeypatch, capsys):
    called = {"n": 0}
    monkeypatch.setattr("claude_code.CodeAgent._post", lambda *a, **k: called.__setitem__("n", called["n"] + 1))
...
def test_compact_sends_compaction_beta_and_edit(monkeypatch):
    ...
    monkeypatch.setattr("claude_code.CodeAgent._post", fake_post)
