...
def test_exchange_includes_workspace_id_when_given(monkeypatch):
    ...
    monkeypatch.setattr(wif.urllib.request, "urlopen", fake_urlopen)
    wif.WIFCredentialExchanger().exchange(
        "fdrl_1", "org_1", "svac_1", "tok", workspace_id="wrkspc_1"
    )
...
def test_wif_admin_client_uses_bearer_not_api_key(monkeypatch):
    captured = {}
    _install_fake_admin_urlopen(
        monkeypatch, captured, {"id": "svac_123", "name": "test"}
    )
    ...
    assert (
        captured["headers"].get("Authorization") == "Bearer org-admin-oauth-token-xyz"
    )
...
def test_wif_admin_client_list_service_accounts_uses_get(monkeypatch):
    ...
    assert (
        captured["url"] == "https://api.anthropic.com/v1/organizations/service_accounts"
    )
...
def test_wif_admin_client_create_federation_issuer_defaults_jwks_discovery(monkeypatch):
    ...
    client.create_federation_issuer(
...
def test_wif_admin_client_create_federation_rule_omits_optional_fields(monkeypatch):
    ...
    client.create_federation_rule(
        "rule1", "fdis_1", "svac_1", {"subject_prefix": "repo:x"}
    )
...
def test_wif_admin_client_create_federation_rule_includes_optional_fields_when_given(
    monkeypatch,
):
    captured = {}
    _install_fake_admin_urlopen(monkeypatch, captured, {"id": "fdrl_1"})
