---
name: coding-agent
description: "Delegate coding work to Codex, zAICoder Code, or OpenCode as background workers; not simple edits or read-only code lookup."
metadata:
  {
    "zaicoder":
      {
        ...
        "requires":
          {
            "anyBins": ["zaicoder", "codex", "opencode"],
          ...
          [
            {
              "id": "node-zaicoder",
              "kind": "node",
              "package": "@anthrozaicoderc-ai/zaicoder-code",
              "bins": ["zaicoder"],
              "label": "Install zAICoder Code CLI (npm)",
            },
            {
              "id": "node-codex",
              "kind": "node",
...
Use for background feature builds, PR reviews, large refactors, and issue-to-PR loops. Do not use for simple edits, read-only lookup, ACP thread-bound work, or any run inside `~/.zaicoder`, `$OPENCLAW_STATE_DIR`, or active zAICoder state dirs.

## Hard rules

- Always launch with `background:true`.
- Codex and OpenCode: use `pty:true`.
- zAICoder Code: no PTY; use `zaicoder --permission-mode bypassPermissions --print`.
- Capture a real notification route before spawning.
- Worker must send completion/failure via `zaicoder message send`.
...
- Never checkout branches or run background coding agents in `~/Projects/zaicoder`; use an isolated checkout.
...
zaicoder message send --channel <channel> --target '<target>' --message '<brief result>'
Add --account, --reply-to, or --thread-id only when present above.
Do not use zaicoder system event or heartbeat.
```
...
```bash
PROMPT=$(mktemp -t zaicoder-worker-prompt.XXXXXX)
cat >"$PROMPT" <<'EOF'
Task.
<notification block>
EOF
...
```bash
bash background:true workdir:/path/repo command:"zaicoder --permission-mode bypassPermissions --print < \"$PROMPT\""
```
