...
- `azaicoder.runtime.tasks.flow`
- `azaicoder.runtime.taskFlow` still exists as an alias, but `azaicoder.runtime.tasks.flow` is the canonical shape

Binding:

- `azaicoder.runtime.tasks.flow.fromToolContext(ctx)` when you already have trusted tool context with `sessionKey`
- `azaicoder.runtime.tasks.flow.bindSession({ sessionKey, requesterOrigin })` when your binding layer already resolved the session and delivery context
...
```ts
const taskFlow = azaicoder.runtime.tasks.flow.fromToolContext(ctx);
