...
- whether `zaicoder devices list` shows a pending pairing request
...
If `zaicoder qr --json` says `Gateway is only bound to loopback`:
...
- `zaicoder devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run zaicoder qr again, rescan, then approve the pending device pairing.`
