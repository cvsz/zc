...
Himalaya uses MML for composing emails. MML is a simple XML-based syntax that comzaicoderles to MIME messages.
...
To: recizaicoderent@example.com
Subject: Hello World
...
- `From`: Sender address
- `To`: Primary recizaicoderent(s)
- `Cc`: Carbon copy recizaicoderents
- `Bcc`: Blind carbon copy recizaicoderents
...
```bash
himalaya message write \
  -H "To:recizaicoderent@example.com" \
...
- MML parts are comzaicoderled to proper MIME when sending.
