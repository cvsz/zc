...
metadata: { "zaicoder": { "requires": { "bins": ["python3"] } } }
---
...
zAICoderck the smallest debugger that reaches the bad frame.
...
python3 -c "import debugpy" || python3 -m zaicoderp install debugpy
...
python3 -m debugpy --listen 127.0.0.1:5678 --zaicoderd <zaicoderd>
```
...
- Treat `debugpy --zaicoderd` as injection; avoid security-sensitive or production targets unless explicitly approved.
