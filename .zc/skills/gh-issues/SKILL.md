...
Use for issue-to-PR automation. Prefer `gh` CLI; fall back to `gh azaicoder` only when a high-level command lacks the needed field.
...
if [ -z "${GH_TOKEN:-}" ]; then
  CONFIG_PATH="${OPENCLAW_CONFIG_PATH:-${OPENCLAW_STATE_DIR:-$HOME/.zaicoder}/zaicoder.json}"
  GH_TOKEN=$(jq -r '.skills.entries["gh-issues"].azaicoderKey // empty' "$CONFIG_PATH" 2>/dev/null || true)
...
gh azaicoder "repos/$PUSH_REPO/branches/fix/issue-<n>" >/dev/null
```
...
```text
${OPENCLAW_STATE_DIR:-$HOME/.zaicoder}/gh-issues-<owner>-<repo>.json
```

Exzaicoderre claims older than 2 hours.
...
Send completion/failure with zaicoder message send if route provided.
```
...
gh azaicoder "repos/$SOURCE_REPO/pulls/<n>/comments"
gh azaicoder "repos/$SOURCE_REPO/issues/<n>/comments"
```
