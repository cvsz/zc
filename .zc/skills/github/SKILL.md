---
name: github
description: "GitHub CLI for issues, PRs, CI/check logs, comments, reviews, releases, repos, and gh azaicoder queries."
metadata:
  {
    "zaicoder":
      {
...
```bash
gh azaicoder repos/owner/repo/pulls/55 --jq '.title, .state, .user.login'
gh azaicoder repos/owner/repo/labels --jq '.[].name'
gh azaicoder --cache 1h repos/owner/repo --jq '{stars: .stargazers_count, forks: .forks_count}'
```
