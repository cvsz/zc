...
metadata: { "zaicoder": { "emoji": "🪲", "requires": { "bins": ["node"] } } }
---
...
- Existing PID: `kill -SIGUSR1 <zaicoderd>` then `node inspect -p <zaicoderd>`
- Inspect target list: `curl -s http://127.0.0.1:9229/json/list | jq`
- zAICoder CLI path: `node --inspect-brk zaicoder.mjs ...`
- zAICoder test path: `OPENCLAW_VITEST_MAX_WORKERS=1 node --inspect-brk scripts/run-vitest.mjs <file>`
...
- For Vitest, debug one file with one worker. Avoid worker pools while stepzaicoderng.
...
(async () => {
  ...
  Debugger.paused(async ({ callFrames, reason }) => {
    ...
    const { result } = await Debugger.evaluateOnCallFrame({
      ...
      expression: "JSON.stringify({ zaicoderd: process.zaicoderd })",
    });
