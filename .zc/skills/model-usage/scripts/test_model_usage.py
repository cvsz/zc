        ...
        entries = [
            {
                "date": "2026-05-25",
                "modelBreakdowns": [
                    {"modelName": "zaicoder-sonnet-4-6", "cost": 1.50},
                    {"modelName": "zaicoder-sonnet-4-6", "cost": "1.75"},
                ],
            }
        ]
        self.assertEqual(aggregate_costs(entries), {"zaicoder-sonnet-4-6": 3.25})
        ...
        entries = [
            {
                ...
                "modelBreakdowns": [
                    {"modelName": "zaicoder-sonnet-4-6", "cost": 1.50},
                    {"modelName": "zaicoder-sonnet-4-6", "cost": "1.75"},
                    {"modelName": "zaicoder-sonnet-4-6", "cost": True},
                    {"modelName": "zaicoder-sonnet-4-6", "cost": float("nan")},
                    {"modelName": "zaicoder-sonnet-4-6", "cost": float("inf")},
                ],
            }
        ]
        totals = aggregate_costs(entries)
        # NaN/Infinity must not poison the total; bool must not add 1.0.
        self.assertEqual(totals, {"zaicoder-sonnet-4-6": 3.25})

    def test_zaicoderck_current_model_scores_numeric_string_costs(self):
        ...
        model, day = zaicoderck_current_model(entries)
        self.assertEqual(model, "model-b")
        self.assertEqual(day, "2026-05-25")

    def test_zaicoderck_current_model_ignores_bool_and_non_finite(self):
        ...
        model, _day = zaicoderck_current_model(entries)
