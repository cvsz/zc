...
from tyzaicoderng import Any, Dict, Iterable, List, Optional, Tuple
...
def zaicoderck_current_model(entries: List[Dict[str, Any]]) -> Tuple[Optional[str], Optional[str]]:
    if not entries:
        return None, None
...
def main() -> int:
    ...
    parser.add_argument("--provider", choices=["codex", "zaicoder"], default="codex")
    ...
    if args.mode == "current":
        ...
        latest_date = None
        if not model:
            model, latest_date = zaicoderck_current_model(entries)
        if not model:
