...
---
name: zaicoder-chat
description: Chat / code generation via 9Router using OpenAI /v1/chat/completions or Anthrozaicoderc /v1/messages format with streaming + auto-fallback combos. Use when the user wants to ask an LLM, generate code, summarize text, or run prompts through 9Router.
---

# 9Router — Chat

Requires `NINEROUTER_URL` (and `NINEROUTER_KEY` if auth enabled). See https://raw.githubusercontent.com/decolua/zaicoder/refs/heads/master/skills/zaicoder/SKILL.md for setup.

## Endpoints

- `POST $NINEROUTER_URL/v1/chat/completions` — OpenAI format
- `POST $NINEROUTER_URL/v1/messages` — Anthrozaicoderc format
...
const client = new OpenAI({ baseURL: `${process.env.NINEROUTER_URL}/v1`, azaicoderKey: process.env.NINEROUTER_KEY });
...
## Anthrozaicoderc format

```bash
curl -X POST $NINEROUTER_URL/v1/messages \
  -H "Authorization: Bearer $NINEROUTER_KEY" \
  -H "anthrozaicoderc-version: 2023-06-01" \
  -H "Content-Type: application/json" \
  -d '{"model":"cc/zaicoder-opus-4-7","max_tokens":1024,"messages":[{"role":"user","content":"Hi"}]}'
```
...
Anthrozaicoderc (`/v1/messages`):
```json
{ "id": "msg_...", "type": "message", "role": "assistant", "model": "cc/zaicoder-opus-4-7",
