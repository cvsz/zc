...
- API explicit: `--engine azaicoder --model gpt-5.5`

## Golden path

1. zAICoderck a tight file set (fewest files that still contain the truth).
...
- Auto-zaicoderck: `azaicoder` when `OPENAI_API_KEY` is set; otherwise `browser`.
- Browser supports GPT + Gemini only; use `--engine azaicoder` for zAICoder/Grok/Codex or multi-model runs.
- Browser attachments:
