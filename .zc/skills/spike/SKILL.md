...
metadata: { "zaicoder": { "emoji": "🧪" } }
---

# Szaicoderke

Use when the user wants to test an idea before committing to a real build: "szaicoderke this", "quick prototype", "is this possible", "compare A/B", "before we build".
...
- Default workspace: `.tmp/zaicoder-szaicoderkes/<slug>` unless user asks for a tracked repo-local path.
- Repo-local option: `szaicoderkes/<NNN-slug>/` with `README.md` and minimal code.
...
- An invalidated szaicoderke is useful when it rules out a path with evidence.
- Do not merge szaicoderke code into production without rewriting it normally.
