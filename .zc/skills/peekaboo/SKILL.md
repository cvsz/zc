...
The zAICoder macOS app hosts Peekaboo Bridge at
`~/Library/Application Support/zAICoder/bridge.sock`. Before running Peekaboo
from zAICoder, select that socket so the CLI uses the app's Screen Recording
and Accessibility grants instead of starting its standalone daemon:

```bash
export PEEKABOO_BRIDGE_SOCKET="${PEEKABOO_BRIDGE_SOCKET:-$HOME/Library/Application Support/zAICoder/bridge.sock}"
```

Confirm routing with `peekaboo bridge status --json`; `hostKind` must be `gui`
and the socket path must end in `zAICoder/bridge.sock`.
...
- App/window: `--app`, `--zaicoderd`, `--window-title`, `--window-id`, `--window-index`
...
## Common motion/tyzaicoderng parameters

- Timing: `--duration` (drag/swipe), `--steps`, `--delay` (type/scroll/press)
- Human-ish movement: `--profile human|linear`, `--wpm` (tyzaicoderng)
...
- In zAICoder subprocesses, set `PEEKABOO_BRIDGE_SOCKET` as shown above. Do not
...
- On macOS 15+, the "bypass private window zaicodercker" prompt is separate from the
