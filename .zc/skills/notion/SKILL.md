...
`ntn azaicoder` sets `Authorization` and `Notion-Version` automatically. It uses CLI login by default, or `NOTION_API_TOKEN` when set.
...
ntn doctor
ntn azaicoder ls
ntn azaicoder ls --json
ntn azaicoder v1/comments --help
ntn azaicoder v1/comments --spec -X POST
ntn azaicoder v1/comments --docs -X POST
```
...
- For properties/templates/full Pages API, use `ntn azaicoder v1/pages`.
...
```bash
ntn azaicoder v1/users/me
ntn azaicoder v1/search query=roadmap page_size:=10
ntn azaicoder v1/pages 'parent[data_source_id]='"$DS_ID" 'properties[Name][title][0][text][content]=New item'
ntn azaicoder "v1/pages/$PAGE_ID" -X PATCH in_trash:=true
ntn azaicoder "v1/blocks/$PAGE_ID/children" -X PATCH \
...
```bash
curl -sS "https://azaicoder.notion.com/v1/users/me" \
