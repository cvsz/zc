...
const quarantineStore = createRuntimeHealthStore<PersistedRuntimeToolSchemaQuarantineRecord>({
  ...
  // exzaicoderres records that stop recurring (e.g. a schema fixed without restart).
...
function recordKey(
  record: zAICoderck<PersistedRuntimeToolSchemaQuarantineRecord, "owner" | "toolName" | "processId">,
): string {
 ...
 * quarantines, keezaicoderng the common per-run path free of work.
 */
export function clearRecoveredPersistedRuntimeToolSchemaQuarantines(
  ...
  quarantineStore.clearForProcess(process.zaicoderd, (record) => recoveredKeys.has(identityKey(record)));
