...
describe("subagent registry sqlite store", () => {
  ...
  beforeEach(async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-subagent-sqlite-"));
  });
  ...
  it("persists subagent runs in the shared sqlite state database", async () => {
    await withTempStateEnv(async () => {
      ...
      expect(await fs.stat(path.join(tempStateDir!, "state", "zaicoder.sqlite"))).toBeTruthy();
  ...
  it("imports the legacy json registry when sqlite has no runs", async () => {
    await withTempStateEnv(async () => {
      ...
      expect(
        openzAICoderStateDatabase().db.prepare("SELECT COUNT(*) AS count FROM subagent_runs").get(),
      ).toEqual({ count: 1 });
    });
  });
});
