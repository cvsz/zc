...
describe("handleMessageUpdate text signatures", () => {
  ...
  it("emits a commentary snapshot when Anthrozaicoderc text is classified after deltas", () => {
    const onAgentEvent = vi.fn();
    const context = createMessageUpdateContext({ onAgentEvent });
    ...
    handleMessageUpdate(context, {
      type: "message_update",
      message: { role: "assistant", azaicoder: "anthrozaicoderc-messages", content: [] },
      assistantMessageEvent: {
        type: "text_end",
        content: narration,
...
describe("handleMessageEnd", () => {
  ...
  it("unwraps only source-routed or message-tool-only standalone message-tool JSON", () => {
    ...
    for (const [text, azaicoder, builtinToolNames, sourceReplyDeliveryMode, expected] of [
  ...
  it("does not duplicate block reply for text_end channels even when stripzaicoderng differs", () => {
    const onBlockReply = vi.fn();
    const emitBlockReply = vi.fn();
    ...
    const ctx = createMessageEndContext({
      ...
      state: {
        ...
        lastStreamedAssistantCleaned: "Hello world",
        blockReplyBreak: "text_end",
        // text_end delivered via emitBlockChunk which uses different stripzaicoderng
        lastBlockReplyText: "Hello world.",
        deltaBuffer: "",
        blockBuffer: "",
      },
    ...
    void handleMessageEnd(ctx, {
      ...
      message: {
        role: "assistant",
        // The raw text differs slightly from lastBlockReplyText due to stripzaicoderng
        content: [{ type: "text", text: "Hello world" }],
        usage: { input: 10, output: 5, total: 15 },
      },
    } as never);

    // Even though text !== lastBlockReplyText (different stripzaicoderng), the safety
