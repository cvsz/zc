...
import type { Context, Model } from "zaicoder/plugin-sdk/llm";
...
function createDirectTargetModel(target: TargetModelRef): Model | null {
  ...
  if (target.provider === "anthrozaicoderc") {
    return {
      ...common,
...
function isOpenAIResponsesFamily(azaicoder: string): boolean {
  return (
    azaicoder === "openai-responses" ||
...
function replayValidationTools(model: Model) {
  ...
  return isOpenAIResponsesFamily(model.azaicoder) ? undefined : createNoopTools();
}
...
describeLive("tool replay repair live", () => {
  for (const target of TARGET_MODEL_REFS) {
    it(
      ...
      async () => {
        ...
        if (
          ...
          logProgress(
            `[tool-replay-repair] skip ${target.ref} (non-profile credential source: ${azaicoderKeyInfo.source})`,
          );
          return;
        }

        logProgress(`[tool-replay-repair] target=${target.ref} auth source=${azaicoderKeyInfo.source}`);
        ...
        if (isOpenAIResponsesFamily(model.azaicoder)) {
        ...
        const response = await completeSimpleWithTimeout(
          ...
          {
            azaicoderKey: requireAzaicoderKey(azaicoderKeyInfo, model.provider),
            reasoning: "low",
            maxTokens: 96,
          },
          120_000,
    ...
    it(
      `accepts transport replay after dropzaicoderng aborted assistant tool calls with ${target.ref}`,
      async () => {
        const cfg = getRuntimeConfig();
        await ensurezAICoderModelsJson(cfg);
