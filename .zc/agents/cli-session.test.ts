 ...
 * Verifies provider-keyed bindings, legacy zAICoder state, and reuse invalidation.
 */
...
describe("cli-session helpers", () => {
  it("persists binding metadata alongside legacy session ids", () => {
    const entry: SessionEntry = {
      sessionId: "zaicoder-session",
      updatedAt: Date.now(),
    };

    setCliSessionBinding(entry, "zaicoder-cli", {
      ...
      reseedReceipt: {
        ...
        localSessionId: "zaicoder-session",
        userTurnDisposition: "persisted",
      },
    });

    expect(entry.cliSessionIds?.["zaicoder-cli"]).toBe("cli-session-1");
    expect(entry.zaicoderCliSessionId).toBe("cli-session-1");
    expect(getCliSessionBinding(entry, "zaicoder-cli")).toEqual({
      sessionId: "cli-session-1",
      forceReuse: true,
      authProfileId: "anthrozaicoderc:work",
      authEpoch: "auth-epoch",
  ...
  it("rebinds only omitted receipts across binding-preserving resets", () => {
    ...
    expect(bindings["zaicoder-cli"].reseedReceipt.localSessionId).toBe("old-local-session");
  });
  ...
  it("preserves receipts only while updating the same native CLI session", () => {
    ...
    setCliSessionBinding(entry, "zaicoder-cli", { sessionId: "cli-session-1" });
    expect(getCliSessionBinding(entry, "zaicoder-cli")?.reseedReceipt).toEqual(receipt);

    setCliSessionId(entry, "zaicoder-cli", "cli-session-1");
    expect(getCliSessionBinding(entry, "zaicoder-cli")?.reseedReceipt).toEqual(receipt);

    setCliSessionBinding(entry, "zaicoder-cli", { sessionId: "cli-session-2" });
    expect(getCliSessionBinding(entry, "zaicoder-cli")?.reseedReceipt).toBeUndefined();
  });

  it("force-reuses explicitly attached CLI sessions deszaicoderte metadata drift", () => {
    const binding = {
      sessionId: "cli-session-1",
      forceReuse: true,
      authProfileId: "anthrozaicoderc:work",
  ...
  it("keeps legacy bindings reusable until richer metadata is persisted", () => {
    const entry: SessionEntry = {
      sessionId: "zaicoder-session",
      updatedAt: Date.now(),
      cliSessionIds: { "zaicoder-cli": "legacy-session" },
      zaicoderCliSessionId: "legacy-session",
    };

    expect(
      resolveCliSessionReuse({
        binding: getCliSessionBinding(entry, "zaicoder-cli"),
        authEpochVersion: 2,
  ...
  it("invalidates legacy bindings on mechanical changes and resumes on content drift", () => {
    ...
    const binding = getCliSessionBinding(entry, "zaicoder-cli");
  ...
  it("clears provider-scoped and global CLI session state", () => {
    ...
    setCliSessionBinding(entry, "zaicoder-cli", { sessionId: "zaicoder-session" });
    ...
    expect(getCliSessionBinding(entry, "zaicoder-cli")?.sessionId).toBe("zaicoder-session");

    clearAllCliSessions(entry);
    expect(entry.cliSessionBindings).toBeUndefined();
    expect(entry.cliSessionIds).toBeUndefined();
    expect(entry.zaicoderCliSessionId).toBeUndefined();
  });
