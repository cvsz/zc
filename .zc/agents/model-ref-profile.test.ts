...
describe("splitTrailingAuthProfile", () => {
  ...
  it("keeps @YYYYMMDD version suffixes in model ids", () => {
    expect(splitTrailingAuthProfile("custom/vertex-ai_zaicoder-haiku-4-5@20251001")).toEqual({
      model: "custom/vertex-ai_zaicoder-haiku-4-5@20251001",
    });
  });

  it("supports auth profiles after @YYYYMMDD version suffixes", () => {
    expect(splitTrailingAuthProfile("custom/vertex-ai_zaicoder-haiku-4-5@20251001@work")).toEqual({
      model: "custom/vertex-ai_zaicoder-haiku-4-5@20251001",
      profile: "work",
    });
  });
