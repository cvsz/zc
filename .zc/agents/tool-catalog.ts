...
type CoreToolDefinition = {
  ...
  includeInzAICoderGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const zaicoderTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInzAICoderGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:zaicoder": zaicoderTools,
