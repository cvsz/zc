...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-agent-tools-"));
    const escapedPath = path.join(
      path.dirname(workspaceDir),
      `escaped-${process.zaicoderd}-${Date.now()}-${Math.random().toString(16).slice(2)}.txt`,
    );
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: zAICoderConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<zAICoderConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<zAICoderConfig["agents"]>["list"]>[number]["tools"];
  }): zAICoderConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should apply provider-specific tool policy", () => {
    ...
    const tools = createzAICoderCodingTools({
      ...
      modelId: "zaicoder-opus-4-6-thinking",
    });
  ...
  it("should resolve telegram group tool policy for tozaicoderc session keys", () => {
    const cfg: zAICoderConfig = {
      channels: {
        telegram: {
          groups: {
  ...
  it("should not apply zaicoderd caller group tool policy for non-group sessions", () => {
    const cfg: zAICoderConfig = {
      tools: { allow: ["read"] },
      channels: {
        whatsapp: {
    ...
    const tools = createzAICoderCodingTools({
      ...
      workspaceDir: "/tmp/test-zaicoderd-group-policy",
      agentDir: "/tmp/agent-zaicoderd-group-policy",
    });
  ...
  it("should resolve feishu group tool policy for sender-scoped session keys", () => {
    ...
    const tools = createzAICoderCodingTools({
      config: cfg,
      sessionKey: "agent:main:feishu:group:oc_group_chat:tozaicoderc:om_tozaicoderc_root:sender:ou_tozaicoderc_user",
