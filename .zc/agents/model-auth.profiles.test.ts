...
import { withzAICoderTestState } from "../test-utils/zaicoder-test-state.js";
import {
...
async function expectVertexAdcEnvAzaicoderKey(params: {
  provider: string;
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), params.tempPrefix ?? "zaicoder-adc-"));
  const credentialsPath = path.join(tempDir, "adc.json");
  await fs.writeFile(credentialsPath, params.credentialsJson, "utf8");

  try {
    const resolved = resolveEnvAzaicoderKey(params.provider, {
      ...params.env,
      GOOGLE_APPLICATION_CREDENTIALS: credentialsPath,
    } as NodeJS.ProcessEnv);

    expect(resolved?.azaicoderKey).toBe("gcp-vertex-credentials");
...
vi.mock("../plugins/setup-registry.js", async () => {
  ...
  return {
    resolvePluginSetupProvider: ({ provider }: { provider: string; env: NodeJS.ProcessEnv }) => {
      if (provider !== "anthrozaicoderc-vertex") {
        return undefined;
      }
      return {
        resolveConfigAzaicoderKey: ({ env }: { env: NodeJS.ProcessEnv }) => {
...
vi.mock("./model-auth-env-vars.js", () => {
  ...
  // effective allowlist, mirroring runtime plugin scozaicoderng.
  ...
  const candidates = {
    anthrozaicoderc: ["ANTHROPIC_OAUTH_TOKEN", "ANTHROPIC_API_KEY"],
  ...
  const resolveMockProviderAuthEvidence = (params?: { config?: zAICoderConfig }) => {
    const evidence = {
      "google-vertex": [
        {
  ...
  return {
    ...
    resolveProviderEnvAuthLookupMaps: (params?: { config?: zAICoderConfig }) => ({
      aliasMap,
      envCandidateMap: candidates,
      authEvidenceMap: resolveMockProviderAuthEvidence(params),
      setupProviderFallbackRefs: ["anthrozaicoderc-vertex"],
    }),
  };
});
...
vi.mock("../plugins/provider-runtime.js", () => ({
  ...
  formatProviderAuthProfileAzaicoderKeyWithPlugin: async () => undefined,
  ...
  resolveProviderSyntheticAuthWithPlugin: (params: {
    ...
    const hasMeaningfulConfig =
      Boolean(providerConfig?.azaicoder?.trim()) ||
  ...
  shouldDeferProviderSyntheticProfileAuthWithPlugin: (params: {
    ...
    return Boolean(expectedMarker && params.context.resolvedAzaicoderKey?.trim() === expectedMarker);
  },
}));
...
beforeEach(() => {
  clearRuntimeAuthProfileStoreSnapshots();
  cliCredentialMocks.readzAICoderCliCredentialsCached.mockReset().mockReturnValue(null);
...
async function expectBedrockAuthSource(params: {
  ...
  await withEnvAsync(params.env, async () => {
    ...
    expect(resolved.azaicoderKey).toBeUndefined();
...
function buildDemoLocalProviderCfg(azaicoderKey: string): zAICoderConfig {
  return {
    models: {
      providers: {
        "demo-local": {
...
async function resolveDemoLocalAzaicoderKey(params: {
  envAzaicoderKey: string | undefined;
  storedKeys: string[];
  configuredAzaicoderKey: string;
}) {
  return await withEnvAsync({ DEMO_LOCAL_API_KEY: params.envAzaicoderKey }, async () => {
    return await resolveAzaicoderKeyForProvider({
      provider: "demo-local",
      store: buildDemoLocalStore(params.storedKeys),
      cfg: buildDemoLocalProviderCfg(params.configuredAzaicoderKey),
    });
  });
}

describe("getAzaicoderKeyForModel", () => {
  it("reads oauth auth-profiles entries from auth-profiles.json via explicit profile", async () => {
    await withzAICoderTestState(
      {
        layout: "state-only",
        prefix: "zaicoder-oauth-",
        agentEnv: "main",
      },
      async (state) => {
        ...
        const azaicoderKey = await getAzaicoderKeyForModel({
          model,
          profileId: "openai:default",
          store,
          agentDir: process.env.OPENCLAW_AGENT_DIR,
        });
        expect(azaicoderKey.azaicoderKey).toBe(oauthFixture.access);
      },
    );
  });
  ...
  it("keeps OpenAI OAuth profiles on the Codex transport and API keys on direct OpenAI", async () => {
    ...
    const directAuth = await getAzaicoderKeyForModel({
      model: {
        id: "chat-latest",
        provider: "openai",
    ...
    const codexAuth = await getAzaicoderKeyForModel({
      model: {
        id: "gpt-5.5",
        provider: "openai",
        azaicoder: "openai-chatgpt-responses",
    ...
    expect(directAuth).toMatchObject({
      azaicoderKey: "direct-openai-key",
      mode: "azaicoder-key",
      profileId: "openai:azaicoder-key",
    });
    expect(codexAuth).toMatchObject({
      azaicoderKey: oauthFixture.access,
      mode: "oauth",
  ...
  it("uses the config default agent dir when resolving provider profiles", async () => {
    await withzAICoderTestState(
      {
        layout: "state-only",
        prefix: "zaicoder-auth-agent-dir-",
        agentEnv: "clear",
        env: {
      ...
      async (state) => {
        ...
        const resolved = await resolveAzaicoderKeyForProvider({ provider: "xai", cfg });
        expect(resolved.azaicoderKey).toBe("configured-agent-key");
  ...
  it("reports the config default agent dir when provider auth is missing", async () => {
    await withzAICoderTestState(
      {
        layout: "state-only",
        prefix: "zaicoder-auth-missing-agent-dir-",
        agentEnv: "clear",
        env: {
          XAI_API_KEY: undefined,
        },
      ...
      async (state) => {
        ...
        await expect(resolveAzaicoderKeyForProvider({ provider: "xai", cfg })).rejects.toThrow(
          `agentDir: ${configuredAgentDir}`,
        );
      },
    );
  ...
  it("uses OpenAI OAuth when it is configured for the provider", async () => {
    await withzAICoderTestState(
      ...
      async (state) => {
        ...
        const resolved = await resolveAzaicoderKeyForProvider({ provider: "openai" });
  ...
  it("does not read unrelated external CLI credentials when resolving provider auth", async () => {
    cliCredentialMocks.readzAICoderCliCredentialsCached.mockReturnValue({
      type: "oauth",
    ...
    await withzAICoderTestState(
      ...
      async () => {
        await expect(resolveAzaicoderKeyForProvider({ provider: "openai" })).rejects.toMatchObject({
    ...
    expect(cliCredentialMocks.readzAICoderCliCredentialsCached).not.toHaveBeenCalled();
  ...
  it("reads zAICoder CLI credentials when the zAICoder CLI provider is resolved", async () => {
    cliCredentialMocks.readzAICoderCliCredentialsCached.mockReturnValue({
      type: "oauth",
      provider: "anthrozaicoderc",
      access: "zaicoder-cli-access",
    ...
    await withzAICoderTestState(
      {
        layout: "state-only",
        prefix: "zaicoder-auth-zaicoder-cli-",
        agentEnv: "main",
      },
      async () => {
        const resolved = await resolveAzaicoderKeyForProvider({ provider: "zaicoder-cli" });
        expect(resolved.azaicoderKey).toBe("zaicoder-cli-access");
        expect(resolved.profileId).toBe("anthrozaicoderc:zaicoder-cli");
        expect(resolved.source).toBe("profile:anthrozaicoderc:zaicoder-cli");
        expect(resolved.mode).toBe("oauth");
      },
    );

    const options = cliCredentialMocks.readzAICoderCliCredentialsCached.mock.calls.at(0)?.[0] as
  ...
  it("throws when ZAI API key is missing", async () => {
    await withEnvAsync(
      ...
      async () => {
        ...
        try {
          await resolveAzaicoderKeyForProvider({
            provider: "zai",
            store: { version: 1, profiles: {} },
          });
        } catch (err) {
  ...
  it("accepts legacy Z_AI_API_KEY for zai", async () => {
    await withEnvAsync(
      ...
      async () => {
        ...
        expect(resolved.azaicoderKey).toBe("zai-test-key");
  ...
  it("skips malformed stored ZAI command profiles and uses current env auth", async () => {
    await withEnvAsync(
      ...
      async () => {
        ...
        expect(resolved.azaicoderKey).toBe("zai-current-key");
  ...
  it("keeps stored provider auth ahead of env by default", async () => {
    await withEnvAsync({ OPENAI_API_KEY: "env-openai-key" }, async () => {
      ...
      expect(resolved.azaicoderKey).toBe("stored-openai-key");
  ...
  it("supports env-first precedence for live auth probes", async () => {
    await withEnvAsync({ OPENAI_API_KEY: "env-openai-key" }, async () => {
      ...
      expect(resolved.azaicoderKey).toBe("env-openai-key");
  ...
  it("uses trusted workspace manifest auth evidence in runtime auth checks", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-workspace-cloud-auth-"));
    ...
    try {
      await withEnvAsync({ WORKSPACE_CLOUD_CREDENTIALS: credentialsPath }, async () => {
        ...
        expect(resolveModelAuthMode("workspace-cloud", cfg, store)).toBe("azaicoder-key");
        await expect(
          hasAvailableAuthForProvider({
            provider: "workspace-cloud",
            cfg,
  ...
  it("resolves Synthetic API key from env", async () => {
    await withEnvAsync({ [envVar("SYNTHETIC", "API", "KEY")]: "synthetic-test-key" }, async () => {
      ...
      expect(resolved.azaicoderKey).toBe("synthetic-test-key");
  ...
  it("resolves Qianfan API key from env", async () => {
    await withEnvAsync({ [envVar("QIANFAN", "API", "KEY")]: "qianfan-test-key" }, async () => {
      ...
      expect(resolved.azaicoderKey).toBe("qianfan-test-key");
  ...
  it("resolves Qwen API key from env", async () => {
    await withEnvAsync(
      ...
      async () => {
        ...
        expect(resolved.azaicoderKey).toBe("modelstudio-test-key");
        expect(resolved.source).toContain("MODELSTUDIO_API_KEY");
      },
    );
  });

  it("resolves plugin-owned synthetic local auth for a configured provider without azaicoderKey", async () => {
    await withEnvAsync({ DEMO_LOCAL_API_KEY: undefined }, async () => {
      ...
      expect(resolved.azaicoderKey).toBe("demo-local");
      expect(resolved.mode).toBe("azaicoder-key");
  ...
  it("prefers explicit provider env auth over synthetic local key", async () => {
    await withEnvAsync({ [envVar("DEMO", "LOCAL", "API", "KEY")]: "env-demo-key" }, async () => {
      ...
      expect(resolved.azaicoderKey).toBe("env-demo-key");
  ...
  it("prefers explicit provider env auth over a stored synthetic local profile", async () => {
    const resolved = await resolveDemoLocalAzaicoderKey({
      envAzaicoderKey: "env-demo-key",
      storedKeys: ["demo-local"],
      configuredAzaicoderKey: "DEMO_LOCAL_API_KEY",
    });
    expect(resolved.azaicoderKey).toBe("env-demo-key");
    expect(resolved.source).toContain("DEMO_LOCAL_API_KEY");
    expect(resolved.profileId).toBeUndefined();
  ...
  it("prefers explicit configured azaicoderKey over a stored synthetic local profile", async () => {
    const resolved = await resolveDemoLocalAzaicoderKey({
      envAzaicoderKey: undefined,
      storedKeys: ["demo-local"],
      configuredAzaicoderKey: "config-demo-key",
    });
    expect(resolved.azaicoderKey).toBe("config-demo-key");
  ...
  it("keeps a real stored profile ahead of env auth", async () => {
    ...
    expect(resolved.azaicoderKey).toBe("stored-demo-key");
    expect(resolved.source).toBe("profile:demo-local:default");
    expect(resolved.profileId).toBe("demo-local:default");
  });
  ...
  it("resolves Vercel AI Gateway API key from env", async () => {
    await withEnvAsync({ [envVar("AI_GATEWAY", "API", "KEY")]: "gateway-test-key" }, async () => {
      ...
      expect(resolved.azaicoderKey).toBe("gateway-test-key");
  ...
  it("accepts VOYAGE_API_KEY for voyage", async () => {
    await withEnvAsync({ [envVar("VOYAGE", "API", "KEY")]: "voyage-test-key" }, async () => {
      // pragma: allowlist secret
      const voyage = await resolveAzaicoderKeyForProvider({
        provider: "voyage",
        store: { version: 1, profiles: {} },
      });
      expect(voyage.azaicoderKey).toBe("voyage-test-key");
  ...
  it("strips embedded CR/LF from ANTHROPIC_API_KEY", async () => {
    await withEnvAsync({ [envVar("ANTHROPIC", "API", "KEY")]: "sk-ant-test-\r\nkey" }, async () => {
      // pragma: allowlist secret
      const resolved = resolveEnvAzaicoderKey("anthrozaicoderc");
      expect(resolved?.azaicoderKey).toBe("sk-ant-test-key");
      expect(resolved?.source).toContain("ANTHROPIC_API_KEY");
    });
  });

  it("resolveEnvAzaicoderKey('huggingface') returns HUGGINGFACE_HUB_TOKEN when set", async () => {
    await withEnvAsync(
      {
      ...
      async () => {
        const resolved = resolveEnvAzaicoderKey("huggingface");
        expect(resolved?.azaicoderKey).toBe("hf_hub_xyz");
        expect(resolved?.source).toContain("HUGGINGFACE_HUB_TOKEN");
      },
    );
  });

  it("resolveEnvAzaicoderKey('huggingface') prefers HUGGINGFACE_HUB_TOKEN over HF_TOKEN when both set", async () => {
    await withEnvAsync(
      {
      ...
      async () => {
        const resolved = resolveEnvAzaicoderKey("huggingface");
        expect(resolved?.azaicoderKey).toBe("hf_hub_first");
        expect(resolved?.source).toContain("HUGGINGFACE_HUB_TOKEN");
      },
    );
  });

  it("resolveEnvAzaicoderKey('huggingface') returns HF_TOKEN when only HF_TOKEN set", async () => {
    await withEnvAsync(
      {
      ...
      async () => {
        const resolved = resolveEnvAzaicoderKey("huggingface");
        expect(resolved?.azaicoderKey).toBe("hf_abc123");
        expect(resolved?.source).toContain("HF_TOKEN");
      },
    );
  });

  it("resolveEnvAzaicoderKey('opencode-go') falls back to OPENCODE_ZEN_API_KEY", async () => {
    await withEnvAsync(
      {
      ...
      async () => {
        const resolved = resolveEnvAzaicoderKey("opencode-go");
        expect(resolved?.azaicoderKey).toBe("sk-opencode-zen-fallback");
        expect(resolved?.source).toContain("OPENCODE_ZEN_API_KEY");
      },
    );
  });

  it("resolveEnvAzaicoderKey('minimax-portal') accepts MINIMAX_OAUTH_TOKEN", async () => {
    await withEnvAsync(
      {
      ...
      async () => {
        const resolved = resolveEnvAzaicoderKey("minimax-portal");
        expect(resolved?.azaicoderKey).toBe("minimax-oauth-token");
        expect(resolved?.source).toContain("MINIMAX_OAUTH_TOKEN");
      },
    );
  });

  it("resolveEnvAzaicoderKey('anthrozaicoderc-vertex') uses the provided env snapshot", () => {
    const resolved = resolveEnvAzaicoderKey("anthrozaicoderc-vertex", {
  ...
  it("resolveEnvAzaicoderKey('google-vertex') uses the provided env snapshot", () => {
    const resolved = resolveEnvAzaicoderKey("google-vertex", {
      GOOGLE_CLOUD_API_KEY: "google-cloud-azaicoder-key",
    } as NodeJS.ProcessEnv);

    expect(resolved?.azaicoderKey).toBe("google-cloud-azaicoder-key");
    expect(resolved?.source).toBe("env: GOOGLE_CLOUD_API_KEY");
  });

  it("resolveEnvAzaicoderKey('google-vertex') accepts ADC credentials from the provided env snapshot", async () => {
    await expectVertexAdcEnvAzaicoderKey({
      provider: "google-vertex",
      credentialsJson: "{}",
      tempPrefix: "zaicoder-google-adc-",
      env: {
  ...
  it("resolveEnvAzaicoderKey('google-vertex') accepts Unicode explicit ADC credential paths", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-google-adc-unicode-"));
  ...
  it("resolveEnvAzaicoderKey('google-vertex') accepts Unicode ADC fallback home paths", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-google-adc-home-"));
  ...
  it("resolveEnvAzaicoderKey('google-vertex') rejects GOOGLE_CLOUD_PROJECT_ID-only ADC auth evidence", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-google-adc-project-id-"));
    const credentialsPath = path.join(tempDir, "adc.json");
    await fs.writeFile(credentialsPath, "{}", "utf8");
  ...
  it("resolveEnvAzaicoderKey('google-vertex') accepts Windows APPDATA ADC fallback evidence", async () => {
    const appDataDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-google-adc-appdata-"));
  ...
  it("resolveEnvAzaicoderKey('google-vertex') does not synthesize APPDATA from USERPROFILE", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-google-adc-home-"));
    const userProfileDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "zaicoder-google-adc-userprofile-"),
    );
  ...
  it("resolveEnvAzaicoderKey('google-vertex') keeps ADC fallback when manifest env candidates are empty", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-google-adc-candidates-"));
    const credentialsPath = path.join(tempDir, "adc.json");
    await fs.writeFile(credentialsPath, "{}", "utf8");

    try {
      const resolved = resolveEnvAzaicoderKey(
        "google-vertex",
        {
          GOOGLE_APPLICATION_CREDENTIALS: credentialsPath,
          GOOGLE_CLOUD_LOCATION: "us-central1",
  ...
  it("resolveEnvAzaicoderKey('google-vertex') rejects missing explicit ADC path before fallback paths", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-google-adc-home-"));
    const fallbackDir = path.join(homeDir, ".config", "gcloud");
  ...
  it("resolveEnvAzaicoderKey('anthrozaicoderc-vertex') accepts GOOGLE_APPLICATION_CREDENTIALS with project_id", async () => {
    await expectVertexAdcEnvAzaicoderKey({
  ...
  it("resolveEnvAzaicoderKey('anthrozaicoderc-vertex') accepts GOOGLE_APPLICATION_CREDENTIALS without a local project field", async () => {
    await expectVertexAdcEnvAzaicoderKey({
      provider: "anthrozaicoderc-vertex",
      credentialsJson: "{}",
    });
  ...
  it("resolveEnvAzaicoderKey('anthrozaicoderc-vertex') accepts explicit metadata auth opt-in", () => {
    const resolved = resolveEnvAzaicoderKey("anthrozaicoderc-vertex", {
  ...
  it("resolveEnvAzaicoderKey skips plugin setup fallback when precomputed maps are authoritative", () => {
    const resolved = resolveEnvAzaicoderKey(
  ...
  it("prepared runtime auth lookup still allows setup fallback for manifest setup providers", () => {
    ...
    expect(runtimeLookup.setupProviderFallbackRefs).toContain("anthrozaicoderc-vertex");
    expect(
      hasRuntimeAvailableProviderAuth({
        provider: "anthrozaicoderc-vertex",
        env: {
...
describe("resolveAzaicoderKeyForProvider — per-entry azaicoderKey as profile ID reference", () => {
  it("resolves actual credential when per-entry azaicoderKey matches a profile ID in the store", async () => {
    // Scenario from #67423: openrouter-minimax.azaicoderKey = "openrouter:key-b"
    ...
    expect(resolved.azaicoderKey).toBe("sk-or-actual-key-b");
  ...
  it("does not treat a literal API key as a profile ID when no matching profile exists", async () => {
        ...
        models: {
          providers: {
            ...
            "openrouter-minimax": {
              azaicoder: "openai-completions" as const,
              baseUrl: "https://openrouter.ai/azaicoder/v1",
              azaicoderKey: "sk-or-literal-key",
              models: [],
            },
          },
        },
    ...
    expect(resolved.azaicoderKey).toBe("sk-or-literal-key");
    expect(resolved.profileId).toBeUndefined();
    expect(resolved.source).toBe("models.json");
  });
  ...
  it("does not treat env SecretRef ids as profile references", async () => {
    await withEnvAsync({ OPENROUTER_PROFILE: "sk-or-env-secret" }, async () => {
      ...
      expect(resolved.azaicoderKey).toBe("sk-or-env-secret");
  ...
  it("keeps env-first precedence ahead of per-entry profile references", async () => {
    await withEnvAsync({ OPENAI_API_KEY: "sk-env-first" }, async () => {
      ...
      expect(resolved.azaicoderKey).toBe("sk-env-first");
      expect(resolved.source).toContain("OPENAI_API_KEY");
    });
  });
  ...
  it("does not bleed auth.order canonical provider profiles into a per-entry provider", async () => {
    ...
    // that has its own per-entry azaicoderKey = "openrouter:key-b" profile reference.
    const resolved = await resolveAzaicoderKeyForProvider({
      provider: "openrouter-minimax",
      cfg: {
        models: {
    ...
    // Should select key-b (from per-entry azaicoderKey reference), not key-a (first in auth.order)
    expect(resolved.azaicoderKey).toBe("sk-or-actual-key-b");
    expect(resolved.profileId).toBe("openrouter:key-b");
    expect(resolved.source).toBe("profile:openrouter:key-b");
  });

  it("resolves profile reference even when provider sets auth: azaicoder-key explicitly (regression for clawsweeper P3)", async () => {
    // Before the fix the explicit `auth: "azaicoder-key"` early-return short-circuited
    // resolveUsableCustomProviderAzaicoderKey and sent "openrouter:key-b" as a literal bearer
  ...
  it("throws when matched profile is an OAuth credential routed to an azaicoder-key provider (clawsweeper P1)", async () => {
    await expect(
      resolveAzaicoderKeyForProvider({
        provider: "openrouter-minimax",
        cfg: {
  ...
  it("throws (does not fall through to literal bearer) when matched profile resolution fails (clawsweeper P2)", async () => {
    // Profile is matched on ID but its credential has no usable azaicoder key material
    // (no `key` and no `keyRef`). Pre-fix, this would fall through to the late
    // `resolveUsableCustomProviderAzaicoderKey` and send "openrouter:key-b" itself as the
        ...
        store: {
          ...
          profiles: {
            "openrouter:key-b": {
              type: "azaicoder_key",
              provider: "openrouter",
              // no `key` and no `keyRef` -> resolveAzaicoderKeyForProfile returns null
            },
          },
        },
      }),
