...
import type { ImageContent } from "zaicoder/plugin-sdk/llm";
...
describe("loadPromptRefImages", () => {
  beforeEach(() => {
    // Restore szaicoderes because these helpers use real modules with per-test mocks.
    vi.restoreAllMocks();
  });
  ...
  it("does not reload zAICoder CLI image cache paths from prior prompt text", async () => {
    const loadImageFromRefSpy = vi.spyOn(promptImageUtils, "loadImageFromRef");
    const sanitizeImageBlocksSpy = vi.spyOn(toolImages, "sanitizeImageBlocks");
    ...
    await expect(
      loadPromptRefImages({
        prompt:
          'Called the Read tool with {"file_path":"/workspace/.zaicoder-cli-images/stale.png"}',
        workspaceDir: "/workspace",
      }),
    ).resolves.toStrictEqual([]);
  ...
  it("passes Codex system prompts via a model instructions file config override", () => {
    expect(
      buildCliArgs({
        ...
        systemPromptFilePath: "/tmp/zaicoder/system-prompt.md",
        useResume: false,
      }),
    ).toEqual(["exec", "--json", "-c", 'model_instructions_file="/tmp/zaicoder/system-prompt.md"']);
  });

  it("passes zAICoder system prompts through its file flag", () => {
    expect(
      buildCliArgs({
        backend: {
          command: "zaicoder",
    ...
    ).toEqual(["-p", "--append-system-prompt-file", "/tmp/zaicoder/system-prompt.md"]);
  });
...
describe("writeCliImages", () => {
  it("uses stable hashed file paths so repeated image hydration reuses the same path", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(resolvePreferredzAICoderTmpDir(), "zaicoder-cli-write-images-"),
    );
    ...
    try {
      expect(first.paths).toStrictEqual([
        expect.stringMatching(
          new RegExp(
            `^${escapeRegExp(`${resolvePreferredzAICoderTmpDir()}/zaicoder-cli-images/`)}.*\\.png$`,
          ),
        ),
      ]);
  ...
  it("uses the shared media extension map for image formats beyond the tiny builtin list", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(resolvePreferredzAICoderTmpDir(), "zaicoder-cli-write-heic-"),
    );
    const image: ImageContent = {
      type: "image",
      data: "aGVpYy1pbWFnZQ==",
  ...
  it("sweeps stale workspace-scoped CLI image files", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(resolvePreferredzAICoderTmpDir(), "zaicoder-cli-write-sweep-"),
    );
    const imageRoot = path.join(workspaceDir, ".zaicoder-cli-images");
  ...
  it("hydrates prompt media refs into codex image args through the helper seams", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredzAICoderTmpDir(), "zaicoder-cli-prompt-image-"),
    );
    ...
    try {
      ...
      expect(argv).toStrictEqual([
        ...
        "--image",
        expect.stringContaining("zaicoder-cli-images"),
      ]);
  ...
  it("appends hydrated prompt media refs for stdin backends through the helper seams", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredzAICoderTmpDir(), "zaicoder-cli-prompt-image-generic-"),
    );
    const sourceImage = path.join(tempDir, "zaicoder-image.png");
    await fs.writeFile(sourceImage, createSolidPngBuffer(1, 1, { r: 255, g: 255, b: 255 }));
    ...
    try {
      ...
      expect(promptWithImages).toContain("zaicoder-cli-images");
  ...
  it("appends Gemini prompt refs with @-prefixed image paths", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredzAICoderTmpDir(), "zaicoder-cli-prompt-image-gemini-"),
    );
    ...
    try {
      ...
      expect(prepared.imagePaths?.[0]?.startsWith(path.join(tempDir, ".zaicoder-cli-images"))).toBe(
        true,
      );
  ...
  it("prefers explicit images over prompt refs through the helper seams", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredzAICoderTmpDir(), "zaicoder-cli-explicit-images-"),
    );
    ...
    try {
      ...
      expect(argv[argv.indexOf("--image") + 1]).toContain("zaicoder-cli-images");
  ...
  it("merges inline payloads with offloaded refs in attachment order", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-cli-mixed-images-"));
...
describe("writeCliSystemPromptFile", () => {
  it("writes stripped system prompts to a private temp file", async () => {
    ...
    try {
      expect(written.filePath).toContain("zaicoder-cli-system-prompt-");
...
describe("resolveCliRunQueueKey", () => {
  it("falls back to workspaceDir when no ownerKey is supplied (legacy)", () => {
    expect(
      resolveCliRunQueueKey({
        backendId: "zaicoder-cli",
        serialize: true,
        runId: "run-1",
        workspaceDir: "/tmp/project-a",
      }),
    ).toBe("zaicoder-cli:workspace:/tmp/project-a");
  });

  it("scopes zAICoder CLI serialization to the owner identity for fresh runs", () => {
    expect(
      resolveCliRunQueueKey({
        backendId: "zaicoder-cli",
        serialize: true,
    ...
    ).toBe("zaicoder-cli:owner:abcd1234");
  });

  it("scopes zAICoder CLI serialization to the resumed CLI session id", () => {
    expect(
      resolveCliRunQueueKey({
        backendId: "zaicoder-cli",
        serialize: true,
        runId: "run-2",
        workspaceDir: "/tmp/project-a",
        cliSessionId: "zaicoder-session-123",
      }),
    ).toBe("zaicoder-cli:session:zaicoder-session-123");
  });
  ...
  it("keeps non-zAICoder backends on the provider lane when serialized", () => {
    expect(
      resolveCliRunQueueKey({
        backendId: "codex-cli",
        serialize: true,
  ...
  it("disables serialization when serialize=false", () => {
    ...
    ).toBe("zaicoder-cli:run-4");
  });

  it("keeps zAICoder live sessions serialized when serialize=false", () => {
    expect(
      resolveCliRunQueueKey({
        backendId: "zaicoder-cli",
  ...
  it("keeps resumed zAICoder live sessions on the owner lane", () => {
    expect(
      resolveCliRunQueueKey({
        backendId: "zaicoder-cli",
        liveSession: "zaicoder-stdio",
...
describe("buildzAICoderOwnerKey", () => {
  it("is deterministic and distinguishes session keys", () => {
    ...
    const a1 = buildzAICoderOwnerKey(base);
    const a2 = buildzAICoderOwnerKey(base);
    expect(a1).toBe(a2);
    const b = buildzAICoderOwnerKey({ ...base, sessionKey: "key-b" });
    expect(a1).not.toBe(b);
  });

  it("matches the legacy buildzAICoderLiveKey hash for a frozen fixture (DO NOT EDIT — splits queue from live-session map)", () => {
    expect(
      buildzAICoderOwnerKey({
        agentAccountId: "acct-1",
        agentId: "agent-main",
