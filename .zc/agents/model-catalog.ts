...
import { resolvezAICoderFable5ModelIdentity } from "@zaicoder/llm-core";
...
import {
  ...
  preparezAICoderModelsJsonSource,
...
type DiscoveredModel = {
  ...
  name?: string;
  provider: string;
  azaicoder?: ModelCatalogEntry["azaicoder"];
...
let manifestModelCatalogCache = new WeakMap<zAICoderConfig, ManifestModelCatalogCacheEntry>();
...
const providerAzaicoderKeyResolverLoader = createLazyImportLoader(
...
function loadProviderAzaicoderKeyResolver() {
  return providerAzaicoderKeyResolverLoader.load();
}
...
function overlayCatalogMetadata(
  ...
  return {
    ...base,
    ...(overlay.azaicoder !== undefined ? { azaicoder: overlay.azaicoder } : {}),
...
function normalizeCatalogEntryContract(entry: ModelCatalogEntry): ModelCatalogEntry {
  if (
    entry.azaicoder === "anthrozaicoderc-messages" &&
    resolvezAICoderFable5ModelIdentity({ id: entry.id, params: entry.params })
  ) {
...
function normalizePersistedModelCatalogEntry(
  ...
  const azaicoder =
    typeof entry?.azaicoder === "string" ? (entry.azaicoder as ModelCatalogEntry["azaicoder"]) : defaults?.azaicoder;
  ...
  for (const [providerRaw, providerConfig] of Object.entries(providers)) {
    ...
    const providerContextTokens =
      typeof providerConfig?.contextTokens === "number" && providerConfig.contextTokens > 0
        ...
        : undefined;
    const providerAzaicoder =
      typeof providerConfig?.azaicoder === "string"
        ? (providerConfig.azaicoder as ModelCatalogEntry["azaicoder"])
        : undefined;
...
function hasConfiguredProviderRowsNeedingManifestLookup(cfg: zAICoderConfig): boolean {
...
export async function loadModelCatalog(params?: {
  ...
  const loadCatalog = async () => {
    ...
    try {
      ...
      if (!readOnly) {
        const preparedSource = await preparezAICoderModelsJsonSource(cfg, agentDir, {
          pluginMetadataSnapshot: params?.metadataSnapshot,
          workspaceDir,
        });
      ...
      for (const entry of entries) {
        ...
        const azaicoder = typeof entry?.azaicoder === "string" ? entry.azaicoder : undefined;
      ...
      if (!readOnly) {
        const { createProviderAzaicoderKeyResolver } = await loadProviderAzaicoderKeyResolver();
        let authStore: ReturnType<typeof ensureAuthProfileStoreWithoutExternalProfiles> | undefined;
        const resolveProviderAzaicoderKeyForProvider = createProviderAzaicoderKeyResolver(
          process.env,
          () =>
        ...
        const resolveProviderAzaicoderKey = (providerId?: string) =>
          providerId?.trim()
            ? resolveProviderAzaicoderKeyForProvider(providerId)
