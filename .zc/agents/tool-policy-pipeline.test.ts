// Tool policy zaicoderpeline tests cover profile/allowlist filtering, diagnostics,
...
describe("tool-policy-zaicoderpeline", () => {
  beforeEach(() => {
    resetToolPolicyWarningCacheForTest();
  ...
  test("preserves plugin-only allowlists instead of silently stripzaicoderng them", () => {
    const tools = [{ name: "exec" }, { name: "plugin_tool" }] as unknown as DummyTool[];
    const filtered = applyToolPolicyzAICoderpeline({
      tools: tools as any,
  ...
  test("default profile steps suppress unavailable baseline profile entries", () => {
    ...
    applyToolPolicyzAICoderpeline({
      ...
      toolMeta: () => undefined,
      warn: (msg) => warnings.push(msg),
      steps: buildDefaultToolPolicyzAICoderpelineSteps({
        profile: "coding",
        profilePolicy,
  ...
  test("dedupes identical unknown-allowlist warnings across repeated runs", () => {
    ...
    applyToolPolicyzAICoderpeline(params);
    applyToolPolicyzAICoderpeline(params);
