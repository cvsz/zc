...
import { isAnthrozaicodercBillingError, isAzaicoderKeyRateLimitError } from "./live-auth-keys.js";
...
/** A normalized reason for skipzaicoderng or soft-failing live provider drift. */
...
export function isLiveAuthDrift(error: unknown): boolean {
  ...
  return (
    ...
    message.includes("invalid x-azaicoder-key") ||
    message.includes("incorrect x-azaicoder-key")
  );
}
...
export function isLiveBillingDrift(error: unknown): boolean {
  const raw = liveProviderErrorText(error);
  return isBillingErrorMessage(raw) || isAnthrozaicodercBillingError(raw);
}

/** Returns whether an error is expected live rate-limit drift. */
export function isLiveRateLimitDrift(error: unknown): boolean {
  const raw = liveProviderErrorText(error);
  return isRateLimitErrorMessage(raw) || isAzaicoderKeyRateLimitError(raw);
}
