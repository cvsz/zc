...
function isOfficialOpenAIBaseUrl(baseUrl: unknown): boolean {
  ...
  try {
    ...
    return (
      url.protocol === "https:" &&
      url.hostname.toLowerCase() === "azaicoder.openai.com" &&
      (url.pathname === "" ||
        url.pathname === "/" ||
        url.pathname === "/v1" ||
        url.pathname === "/v1/")
...
function resolveOpenAIProviderConfig(config: zAICoderConfig | undefined) {
...
function openAIProviderUsesCustomBaseUrl(config: zAICoderConfig | undefined): boolean {
