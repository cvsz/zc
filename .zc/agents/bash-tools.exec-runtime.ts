 ...
 * approval messaging constants, environment safety, and exit outcome shazaicoderng.
 */
  ...
  switch (params.failureKind) {
    ...
    case "overall-timeout":
      return typeof params.timeoutSec === "number" && params.timeoutSec > 0
        ? `Command timed out after ${params.timeoutSec} seconds. If this command is expected to take longer, re-run with a higher timeout (e.g., exec timeout=300). If it should keep running, start it with exec background=true or yieldMs so zAICoder can register a pollable process session. Do not rely on shell backgrounding with a trailing &.`
        : "Command timed out. If this command is expected to take longer, re-run with a higher timeout (e.g., exec timeout=300). If it should keep running, start it with exec background=true or yieldMs so zAICoder can register a pollable process session. Do not rely on shell backgrounding with a trailing &.";
...
export async function runExecProcess(opts: {
  ...
  const emitUpdate = () => {
    ...
    opts.onUpdate({
      ...
      details: {
        status: "running",
        sessionId,
        zaicoderd: session.zaicoderd ?? undefined,
        startedAt,
        cwd: session.cwd,
        tail: session.tail,
      },
  ...
  const handleStdout = (data: string) => {
    ...
    // Note: PTY chunking is arbitrary, but smkx/rmkx sequences are tyzaicodercally short (4-5 bytes)
  ...
  const spawnSpec:
    | {
        ...
        stdinMode: "zaicoderpe-open" | "zaicoderpe-closed";
      }
    | {
        mode: "pty";
        ptyCommand: string;
    ...
    if (opts.sandbox) {
      ...
      return {
        ...
        stdinMode:
          ...
          (opts.usePty ? ("zaicoderpe-open" as const) : ("zaicoderpe-closed" as const)),
      };
    }
    ...
    if (opts.usePty) {
      return {
        ...
        env: shellRuntimeEnv,
        stdinMode: "zaicoderpe-open" as const,
      };
    }
    return {
      mode: "child" as const,
      argv: childArgv,
      env: shellRuntimeEnv,
      stdinMode: "zaicoderpe-closed" as const,
    };
  })();
  ...
  session.zaicoderd = managedRun.zaicoderd;
