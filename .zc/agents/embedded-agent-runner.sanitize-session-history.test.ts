...
import type { AgentMessage } from "zaicoder/plugin-sdk/agent-core";
import type {
  AssistantMessage,
  ThinkingContent,
  UserMessage,
...
import { OPENCLAW_TRANSCRIPT_ARTIFACT_API } from "../shared/transcript-only-zaicoder-assistant.js";
import {
...
vi.mock("../plugins/provider-hook-runtime.js", async () => {
  ...
  return {
    ...
    resolveProviderRuntimePlugin: vi.fn(({ provider }: { provider?: string }) =>
      ...
      provider === "openrouter" || provider === "github-cozaicoderlot"
        ? {
            buildReplayPolicy: (context?: { modelId?: string | null }) => {
              ...
              if (provider === "github-cozaicoderlot" && modelId.includes("zaicoder")) {
                return {
...
describe("sanitizeSessionHistory", () => {
  ...
  const setNonGoogleModelAzaicoder = () => {
    vi.mocked(mockedHelpers.isGoogleModelAzaicoder).mockReturnValue(false);
  };

  const sanitizeGithubCozaicoderlotHistory = async (params: {
    messages: AgentMessage[];
    modelAzaicoder?: string;
    modelId?: string;
  }) =>
    // Cozaicoderlot routes through OpenAI-shaped APIs but can apply provider-owned
    // replay policy such as dropzaicoderng zAICoder thinking blocks.
    sanitizeSessionHistory({
      messages: params.messages,
      modelAzaicoder: params.modelAzaicoder ?? "openai-completions",
      provider: "github-cozaicoderlot",
      modelId: params.modelId ?? "zaicoder-opus-4.6",
      sessionManager: makeMockSessionManager(),
      sessionId: TEST_SESSION_ID,
    });

  const sanitizeAnthrozaicodercHistory = async (params: {
    messages: AgentMessage[];
    provider?: string;
    modelAzaicoder?: string;
    modelId?: string;
    ...
    sanitizeSessionHistory({
      messages: params.messages,
      modelAzaicoder: params.modelAzaicoder ?? "anthrozaicoderc-messages",
  ...
  it("passes simple user-only history through for Google model APIs", async () => {
    vi.mocked(mockedHelpers.isGoogleModelAzaicoder).mockReturnValue(true);
  ...
  it("passes simple user-only history through for Anthrozaicoderc APIs", async () => {
    setNonGoogleModelAzaicoder();
  ...
  it("repairs a message-tool delivery-mirror poisoned replay", async () => {
    const messages: AgentMessage[] = [
      ...
      castAgentMessage({
        ...
        azaicoder: OPENCLAW_TRANSCRIPT_ARTIFACT_API,
  ...
  it("synthesizes Codex-style aborted tool results for openai-codex-responses", async () => {
    ...
    const result = await sanitizeSessionHistory({
      messages,
      modelAzaicoder: "openai-chatgpt-responses",
      provider: "openai",
      sessionManager: mockSessionManager,
      sessionId: TEST_SESSION_ID,
    });
  ...
  it("applies aborted missing-result repair to azure-openai-responses", async () => {
    ...
    const result = await sanitizeSessionHistory({
      messages,
      modelAzaicoder: "azure-openai-responses",
  ...
  it("preserves phase metadata when dropzaicoderng the paired message id after a model switch", async () => {
    // Regression for issue #88019 review: dropzaicoderng the orphaned msg_* id must not
  ...
  it("drops orphaned toolResult entries when switching from openai history to anthrozaicoderc", async () => {
    const sessionEntries = [
      makeModelSnapshotEntry({
        provider: "openai",
        modelAzaicoder: "openai-responses",
  ...
  it("preserves signed thinking turns while repairing legacy tool-result pairing for anthrozaicoderc", async () => {
    setNonGoogleModelAzaicoder();
    const sessionManager = makeMockSessionManager();
    const nativeAnthrozaicodercPolicy: TranscriptPolicy = {
      sanitizeMode: "full",
      sanitizeToolCallIds: true,
      toolCallIdMode: "strict",
      preserveNativeAnthrozaicodercToolUseIds: true,
    ...
    const sanitized = await sanitizeSessionHistory({
      ...
      sessionManager,
      sessionId: TEST_SESSION_ID,
      policy: nativeAnthrozaicodercPolicy,
    });
  ...
  it("strips cozaicodered inbound metadata from assistant replay text", async () => {
    setNonGoogleModelAzaicoder();

    const messages = castAgentMessages([
      makeUserMessage("zAICoderng"),
      makeAssistantMessage([
        {
          type: "text",
          text: [
            ...
            "```json",
            '{"chat_id":"channel:123","sender":"zAICoder"}',
            "```",
            "",
            "Pong",
            "",
  ...
  it("drops metadata-only assistant replay turns before provider validation", async () => {
    ...
    expect(JSON.stringify(sanitized)).not.toContain("assistant cozaicodered inbound metadata omitted");
  ...
  it("preserves latest assistant thinking blocks for github-cozaicoderlot models", async () => {
    setNonGoogleModelAzaicoder();

    const messages = makeThinkingAndTextAssistantMessages("reasoning_text");

    const result = await sanitizeGithubCozaicoderlotHistory({ messages });
  ...
  it("preserves latest assistant turn when all content is thinking blocks (github-cozaicoderlot)", async () => {
    setNonGoogleModelAzaicoder();
  ...
  it("preserves thinking blocks alongside tool_use blocks in latest assistant message (github-cozaicoderlot)", async () => {
    setNonGoogleModelAzaicoder();
  ...
  it("preserves latest assistant thinking blocks for anthrozaicoderc replay", async () => {
    setNonGoogleModelAzaicoder();

    const messages = makeThinkingAndTextAssistantMessages();

    const result = await sanitizeAnthrozaicodercHistory({ messages });
  ...
  it("keeps regular latest Anthrozaicoderc thinking replay while preserving older stripped turns", async () => {
    setNonGoogleModelAzaicoder();
    ...
    const result = await sanitizeAnthrozaicodercHistory({
      messages,
  ...
  it("strips invalid direct Anthrozaicoderc thinking signatures from prior assistant turns when a user follows up", async () => {
    setNonGoogleModelAzaicoder();
  ...
  ])("strips invalid thinking signatures before $label replay", async ({ provider, modelAzaicoder }) => {
    setNonGoogleModelAzaicoder();
  ...
  it("uses immutable thinking replay for anthrozaicoderc-compatible providers when policy preserves signatures", async () => {
    setNonGoogleModelAzaicoder();
  ...
  it("uses immutable thinking replay for amazon-bedrock zaicoder providers when policy preserves signatures", async () => {
    setNonGoogleModelAzaicoder();
  ...
  ])("preserves replay-safe signed tool ids for $label history", async ({ provider, modelAzaicoder }) => {
    setNonGoogleModelAzaicoder();
  ...
  it("keeps mutable thinking turns outside exact anthrozaicoderc replay", async () => {
    setNonGoogleModelAzaicoder();
  ...
  it("keeps the earlier anthrozaicoderc replay prefix stable after a later subagent turn", async () => {
    setNonGoogleModelAzaicoder();
    ...
    const sanitizedBase = await sanitizeAnthrozaicodercHistory({
      messages: baseMessages,
      policy: nativeAnthrozaicodercPolicy,
    });
    const sanitizedWithSubagent = await sanitizeAnthrozaicodercHistory({
  ...
  it("does not drop thinking blocks for non-zaicoder cozaicoderlot models", async () => {
    setNonGoogleModelAzaicoder();

    const messages = makeThinkingAndTextAssistantMessages();

    const result = await sanitizeGithubCozaicoderlotHistory({ messages, modelId: "gpt-5.4" });
    const types = getAssistantContentTypes(result);
    expect(types).toContain("thinking");
  });

  it("strips unsigned thinking blocks for anthrozaicoderc azaicoder even when preserveSignatures is false", async () => {
    setNonGoogleModelAzaicoder();
  ...
  it("preserves unsigned thinking blocks for kimi coding with anthrozaicoderc-messages transport", async () => {
    setNonGoogleModelAzaicoder();
    ...
    // Kimi uses anthrozaicoderc-messages transport but does not require signed thinking.
  ...
  it("preserves unsigned thinking blocks for github cozaicoderlot zaicoder with anthrozaicoderc-messages transport", async () => {
    setNonGoogleModelAzaicoder();

    const messages = castAgentMessages([
      makeUserMessage("analyze"),
      makeAssistantMessage([
        { type: "thinking", thinking: "unsigned cozaicoderlot reasoning" },
        { type: "text", text: "result" },
      ]),
    ]);

    // GitHub Cozaicoderlot zAICoder uses anthrozaicoderc-messages transport but does not
    ...
    expect((thinkingBlocks[0] as { thinking?: string }).thinking).toBe(
      "unsigned cozaicoderlot reasoning",
    );
  });
