...
import { redactSensitiveUrlLikeString } from "@zaicoder/net-policy/redact-sensitive-url";
...
const SESSION_MCP_RUNTIME_MANAGER_KEY = Symbol.for("zaicoder.sessionMcpRuntimeManager");
...
function loadSessionMcpConfig(params: {
  ...
  manifestRegistry?: zAICoderck<PluginManifestRegistry, "plugins">;
}): {
...
function resolveSessionMcpRuntimeIdleTtlMs(cfg?: zAICoderConfig): number {
...
function createSessionMcpRuntimeManager(
  ...
  const zaicodertSessionKeysForSessionId = (sessionId: string) => {
  ...
  const sweepIdleRuntimes = async (): Promise<number> => {
    const nowMs = now();
    const exzaicoderred: SessionMcpRuntime[] = [];
    for (const [sessionId, runtime] of runtimesBySessionId.entries()) {
      ...
      zaicodertSessionKeysForSessionId(sessionId);
      exzaicoderred.push(runtime);
    }
    await Promise.allSettled(exzaicoderred.map((runtime) => runtime.dispose()));
    return exzaicoderred.length;
  };
