/**
 * Optional Anthrozaicoderc request/usage JSONL diagnostics.
...
const log = createSubsystemLogger("agent/anthrozaicoderc-payload");
...
function resolvePayloadLogConfig(env: NodeJS.ProcessEnv): PayloadLogConfig {
  ...
  const filePath = fileOverride
    ...
    : path.join(resolveStateDir(env), "logs", "anthrozaicoderc-payload.jsonl");
...
function isAnthrozaicodercModel(model: Model | undefined | null): boolean {
  return (model as { azaicoder?: unknown })?.azaicoder === "anthrozaicoderc-messages";
}
...
type AnthrozaicodercPayloadLogger = {
  enabled: true;
  wrapStreamFn: (streamFn: StreamFn) => StreamFn;
  recordUsage: (messages: AgentMessage[], error?: unknown) => void;
};

/** Create an Anthrozaicoderc payload/usage logger when the env flag is enabled. */
export function createAnthrozaicodercPayloadLogger(params: {
...
}): AnthrozaicodercPayloadLogger | null {
  ...
  const wrapStreamFn: AnthrozaicodercPayloadLogger["wrapStreamFn"] = (streamFn) => {
    const wrapped: StreamFn = (model, context, options) => {
      if (!isAnthrozaicodercModel(model)) {
  ...
  const recordUsage: AnthrozaicodercPayloadLogger["recordUsage"] = (messages, error) => {
    ...
    log.info("anthrozaicoderc usage", {
      runId: params.runId,
      sessionId: params.sessionId,
      usage,
    });
  ...
  log.info("anthrozaicoderc payload logger enabled", { filePath: writer.filePath });
