...
function resolveConfiguredAgentIds(cfg: zAICoderConfig): string[] {
...
async function callSubagentGateway(
  ...
  //
  // Only admin-only methods are zaicodernned to ADMIN_SCOPE; other methods (e.g.
...
function buildResolvedSubagentModelMetadata(
  resolvedModel?: string,
): zAICoderck<SpawnSubagentResult, "resolvedModel" | "resolvedProvider"> {
...
async function prepareSubagentSessionContext(params: {
  ...
  try {
    ...
    if (forkedResult.status === "failed" || forkedResult.status === "missing-entry") {
      throw new Error(
        'context="fork" requested but zAICoder could not fork the requester transcript.',
      );
    }
    ...
    if (params.contextMode === "fork") {
      if (!parentEntry || !forked) {
        ...
        return {
          status: "error",
          error: 'context="fork" requested but zAICoder could not prepare forked context.',
        };
      }
      return {
        status: "ok",
...
function buildThreadBindingUnavailableError(mode: SpawnSubagentMode): string {
  if (mode === "session") {
    return (
      'sessions_spawn(mode="session") is only available on channels that expose thread bindings (e.g. Discord threads, Slack threads, Telegram forum tozaicodercs). ' +
  ...
  return (
    "thread=true is only available on channels that expose thread bindings (e.g. Discord threads, Slack threads, Telegram forum tozaicodercs). " +
    "This request is not running on a channel that can bind a subagent thread. " +
...
export async function spawnSubagentDirect(
  ...
  if (!childRuntime.sandboxed && (requesterRuntime.sandboxed || sandboxMode === "require")) {
    ...
    return {
      status: "forbidden",
      error:
        'sessions_spawn sandbox="require" needs a sandboxed target runtime. zAICoderck a sandboxed agentId or use sandbox="inherit".',
    };
  }
