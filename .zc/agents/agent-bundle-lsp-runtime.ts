...
function terminateLspProcessTree(session: LspSession): void {
  const zaicoderd = session.process.zaicoderd;
  if (zaicoderd && !hasLspProcessExited(session.process)) {
    killProcessTree(zaicoderd, { graceMs: LSP_PROCESS_TREE_KILL_GRACE_MS });
    return;
  }
