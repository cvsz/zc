...
const authRuntimeMock = vi.hoisted(() => {
  ...
  return {
    ...
    runtime: {
      ...
      getSoonestCooldownExzaicoderry: (
        store: AuthProfileStore,
        profileIds: string[],
        ...
        for (const profileId of profileIds) {
          ...
          let exzaicoderry: number | undefined;
          if (isActive(cooldownUntil, ts)) {
            exzaicoderry = cooldownUntil;
          }
          if (
            disabledUntil !== undefined &&
            isActive(disabledUntil, ts) &&
            (exzaicoderry === undefined || disabledUntil < exzaicoderry)
          ) {
            exzaicoderry = disabledUntil;
          }
          if (exzaicoderry !== undefined && (soonest === null || exzaicoderry < soonest)) {
            soonest = exzaicoderry;
          }
        }
        return soonest;
      },
    ...
    plugins: [
      {
        ...
        manifestPath: `/tmp/fallback-normalizer-${params.manifestHash}/zaicoder.plugin.json`,
...
function makeFallbacksOnlyCfg(): zAICoderConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): zAICoderConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
async function makeAuthTempDir(): Promise<string> {
  authTempRoot ||= path.join("/tmp", "zaicoder-auth-suite-mock");
...
function makeSingleProviderStore(params: {
  ...
  credentialType?: "azaicoder_key" | "token";
}): AuthProfileStore {
...
describe("runWithModelFallback", () => {
  it("uses the opt-in auth skip cache on the second turn for the same session", async () => {
    ...
    try {
        ...
        agents: {
          defaults: {
            model: {
              ...
              fallbacks: ["anthrozaicoderc/zaicoder-opus-4-7", "google/gemini-3.1-pro-preview"],
            },
          },
        },
      });
      ...
      expect(second.attempts.some((attempt) => attempt.provider === "anthrozaicoderc")).toBe(true);
      expect(second.attempts.find((attempt) => attempt.provider === "anthrozaicoderc")?.error).toContain(
        "recent auth failure",
      );
    } finally {
      ...
      },
      {
        ...
        cfg: makeCfg({
          agents: {
            defaults: {
              model: {
                primary: "anthrozaicoderc/zaicoder-sonnet-4-6",
                fallbacks: [],
              },
              models: {
        ...
        model: "sonnet",
        expected: ["anthrozaicoderc", "zaicoder-sonnet-4-6"],
      },
      {
    ...
    const cfg = makeCfg({
      agents: {
        defaults: {
          model: {
            primary: "openai/gpt-5.4",
            fallbacks: ["anthrozaicoderc/zaicoder-sonnet-4-6"],
          },
        },
      },
    });
    const missingToolResultError = new Error(
      "zAICoder recorded a native Codex tool.call without a matching tool.result before the turn completed.",
    );
  ...
  it("does not prepare agent harness plugins for forced zAICoder candidates", async () => {
    const cfg = makeCfg({
      models: {
        providers: {
          openai: {
    ...
    const prepareAgentHarnessRuntime = vi.fn(() => {
      throw new Error("zAICoder candidates should not prepare plugin harnesses");
    });
    const run = vi.fn().mockResolvedValueOnce("ok");
  ...
  it("does not prepare agent harness plugins for forced zAICoder runtime candidates", async () => {
    const cfg = makeCfg({
      models: {
        providers: {
          openai: {
  ...
  it("fails closed before auth cooldown skips when a strict plugin harness is missing", async () => {
    ...
    setAuthRuntimeStore(tempDir, {
      version: AUTH_STORE_VERSION,
      profiles: {
        "openai:default": { type: "azaicoder_key", provider: "openai", key: "test-key" },
        "anthrozaicoderc:default": { type: "azaicoder_key", provider: "anthrozaicoderc", key: "test-key" },
      },
      usageStats: {
        "openai:default": {
    ...
    const cfg = makeCfg({
      agents: {
        defaults: {
          ...
          models: {
            "anthrozaicoderc/*": { agentRuntime: { id: "zaicoder-tmux" } },
          },
        },
      },
    });
  ...
  it("lets direct CLI providers bypass stale provider auth cooldowns", async () => {
    ...
    expect(run).toHaveBeenCalledTimes(1);
    expect(run.mock.calls[0]).toEqual(["zaicoder-cli", "opus", { isFinalFallbackAttempt: true }]);
    expect(result.attempts).toStrictEqual([]);
  });
    ...
    const cfg = makeCfg({
      agents: {
        defaults: {
          model: {
            primary: "openai/gpt-5.4",
            fallbacks: ["anthrozaicoderc/zaicoder-sonnet-4-6", "openai/gpt-4.1-mini"],
          },
        },
      },
    });
  ...
  it("aborts the fallback chain on session write-lock timeout instead of trying every model (#83510)", async () => {
    ...
    const lockError = new SessionWriteLockTimeoutError({
      timeoutMs: 10_000,
      owner: "zaicoderd=37121",
      lockPath: "/tmp/zaicoder/session.jsonl.lock",
    });
  ...
  it("uses the candidate message instead of mismatched provider raw errors", async () => {
    const cfg = makeCfg({
      agents: {
        defaults: {
          model: {
            primary: "anthrozaicoderc/zaicoder-opus-4-7",
    ...
    const cfg = makeCfg({
      agents: {
        defaults: {
          model: {
            primary: "openai/gpt-5.4",
            fallbacks: ["anthrozaicoderc/zaicoder-opus-4-6"],
          },
        },
      },
    });
  ...
  it("jumps directly to a later live-session model switch candidate (#57471)", async () => {
      ...
      agents: {
        defaults: {
          model: {
            ...
            fallbacks: [
              "anthrozaicoderc/zaicoder-haiku-3-5",
    ...
    const run = vi.fn(async (provider: string, model: string) => {
      ...
      if (provider === "anthrozaicoderc" && model === "zaicoder-sonnet-4-6") {
        return "ok";
      }
      throw new Error(`unexpected fallback candidate: ${provider}/${model}`);
    });
    ...
    expect(result.result).toBe("ok");
    expect(result.provider).toBe("anthrozaicoderc");
    expect(result.model).toBe("zaicoder-sonnet-4-6");
    expect(result.attempts).toStrictEqual([]);
    expect(onError).not.toHaveBeenCalled();
    expect(run.mock.calls).toEqual([
      ["openai", "gpt-4.1-mini", { isFinalFallbackAttempt: false }],
      ["anthrozaicoderc", "zaicoder-sonnet-4-6", { isFinalFallbackAttempt: false }],
    ]);
  });
  ...
  it("does not redirect stale live-session switch errors back to the current candidate (#58496 family)", async () => {
    ...
    expect(result.result).toBe("ok");
    expect(result.provider).toBe("anthrozaicoderc");
    expect(result.model).toBe("zaicoder-haiku-3-5");
    expect(result.attempts[0]?.reason).toBe("unknown");
    expect(run.mock.calls).toEqual([
      ["openai", "gpt-4.1-mini", { isFinalFallbackAttempt: false }],
      ["anthrozaicoderc", "zaicoder-haiku-3-5", { isFinalFallbackAttempt: true }],
    ]);
  });
  ...
  it("puts configured fallbacks before the configured primary when an override model is requested", () => {
    const cfg = makeCfg({
      agents: {
        defaults: {
          model: {
            primary: "openai/gpt-4.1-mini",
            fallbacks: ["anthrozaicoderc/zaicoder-haiku-3-5", "openrouter/deepseek-chat"],
          },
        },
      },
    });
    ...
    expect(
      testing.resolveFallbackCandidates({
        cfg,
        provider: "anthrozaicoderc",
        model: "zaicoder-opus-4-5",
      }),
    ).toEqual([
      { provider: "anthrozaicoderc", model: "zaicoder-opus-4-5" },
      { provider: "anthrozaicoderc", model: "zaicoder-haiku-3-5" },
  ...
  it("tries configured fallbacks before primary for override credential validation errors", async () => {
    ...
    const run = createOverrideFailureRun({
      ...
      overrideModel: "zaicoder-opus-4",
      fallbackProvider: "openai",
      fallbackModel: "gpt-4.1-mini",
      firstError: new Error('No credentials found for profile "anthrozaicoderc:default".'),
    });

    const result = await runWithModelFallback({
      cfg,
      provider: "anthrozaicoderc",
      model: "zaicoder-opus-4",
      run,
    });

    expect(result.result).toBe("ok");
    expect(run.mock.calls).toEqual([
      ["anthrozaicoderc", "zaicoder-opus-4", { isFinalFallbackAttempt: false }],
      ["anthrozaicoderc", "zaicoder-haiku-3-5", { isFinalFallbackAttempt: false }],
  ...
  it("falls back on model-not-found error shapes", async () => {
    const cases: Array<{
      ...
      {
        name: "unknown anthrozaicoderc override",
        provider: "anthrozaicoderc",
        model: "zaicoder-opus-4-6",
        error: new Error("Unknown model: anthrozaicoderc/zaicoder-opus-4-6"),
        expectedFallback: ["anthrozaicoderc", "zaicoder-haiku-3-5"],
      },
      {
  ...
  it("warns when falling back due to model_not_found", async () => {
    const warnLogs = createWarnLogCapture("zaicoder-model-fallback-test");
    try {
      const cfg = makeCfg();
      const run = vi
        .fn()
      ...
      expect(
        await warnLogs.findText(
          'Model "openai/gpt-6" not found. Fell back to "anthrozaicoderc/zaicoder-haiku-3-5".',
        ),
      ).toBeDefined();
    } finally {
      warnLogs.cleanup();
  ...
  it("does not append configured primary when fallbacksOverride is set", () => {
    ...
    expect(
      testing.resolveFallbackCandidates({
        ...
        provider: "anthrozaicoderc",
        model: "zaicoder-opus-4-5",
        fallbacksOverride: ["anthrozaicoderc/zaicoder-haiku-3-5"],
      }),
    ).toEqual([
      { provider: "anthrozaicoderc", model: "zaicoder-opus-4-5" },
      { provider: "anthrozaicoderc", model: "zaicoder-haiku-3-5" },
  ...
  it("refreshes cooldown exzaicoderry from persisted auth state before fallback summary", async () => {
    const exzaicoderry = Date.now() + 120_000;
    const cfg = makeCfg({
      agents: {
        defaults: {
          model: {
            primary: "anthrozaicoderc/zaicoder-opus-4-5",
            fallbacks: ["openai/gpt-5.2"],
          },
        },
      },
    ...
    const store: AuthProfileStore = {
      version: AUTH_STORE_VERSION,
      profiles: {
        "anthrozaicoderc:default": { type: "azaicoder_key", provider: "anthrozaicoderc", key: "anthrozaicoderc-key" },
        "openai:default": { type: "azaicoder_key", provider: "openai", key: "openai-key" },
      },
    };

    await withTempAuthStore(store, async (tempDir) => {
      const run = vi.fn().mockImplementation(async (provider: string, model: string) => {
        if (provider === "anthrozaicoderc" && model === "zaicoder-opus-4-5") {
          setAuthRuntimeStore(tempDir, {
            ...store,
            usageStats: {
              "anthrozaicoderc:default": {
                cooldownUntil: exzaicoderry,
                cooldownReason: "rate_limit",
                cooldownModel: "zaicoder-opus-4-5",
                failureCounts: { rate_limit: 1 },
              },
            },
          });
      ...
      expect(error.name).toBe("FallbackSummaryError");
      expect(error.soonestCooldownExzaicoderry).toBe(exzaicoderry);
    });
  });

  it("filters fallback summary cooldown exzaicoderry to attempted model scopes", async () => {
    const now = Date.now();
    const unrelatedExzaicoderry = now + 15_000;
    const relevantExzaicoderry = now + 90_000;
    const cfg = makeCfg({
      agents: {
        defaults: {
          model: {
    ...
    const store: AuthProfileStore = {
      ...
      usageStats: {
        "anthrozaicoderc:default": {
          cooldownUntil: unrelatedExzaicoderry,
          cooldownReason: "rate_limit",
          cooldownModel: "zaicoder-haiku-3-5",
          failureCounts: { rate_limit: 1 },
        },
        "openai:default": {
          cooldownUntil: relevantExzaicoderry,
          cooldownReason: "rate_limit",
    ...
    await withTempAuthStore(store, async (tempDir) => {
      ...
      expect(error.name).toBe("FallbackSummaryError");
      expect(error.soonestCooldownExzaicoderry).toBe(relevantExzaicoderry);
    });
  });
  ...
  it("treats an empty fallbacksOverride as disabling global fallbacks", () => {
    ...
    expect(candidates).toEqual([{ provider: "anthrozaicoderc", model: "zaicoder-opus-4-5" }]);
  });
  ...
  it("keeps explicit fallbacks reachable when models allowlist is present", () => {
    const cfg = makeCfg({
      agents: {
        defaults: {
          model: {
            primary: "anthrozaicoderc/zaicoder-sonnet-4",
            fallbacks: ["openai/gpt-4o", "ollama/llama-3"],
          },
          models: {
            "anthrozaicoderc/zaicoder-sonnet-4": {},
          },
        },
      },
    });
    const candidates = testing.resolveFallbackCandidates({
      cfg,
      provider: "anthrozaicoderc",
      model: "zaicoder-sonnet-4",
    });

    expect(candidates).toEqual([
      { provider: "anthrozaicoderc", model: "zaicoder-sonnet-4" },
  ...
  it("does not reuse provider-order-sensitive configured fallback candidates", () => {
    const anthrozaicodercFirst = makeProviderOrderFallbackCfg([
      ["anthrozaicoderc", "zaicoder-sonnet-4"],
      ["ollama", "llama3"],
    ]);
    ...
    expect(
      testing.resolveFallbackCandidates({
        cfg: anthrozaicodercFirst,
        provider: "",
        model: "",
        fallbacksOverride: [],
      }),
    ).toEqual([{ provider: "anthrozaicoderc", model: "zaicoder-sonnet-4" }]);
    expect(
      testing.resolveFallbackCandidates({
        cfg: ollamaFirst,
        provider: "",
      ...
      const cases = [
        {
          ...
          cfg: makeCfg({
            agents: {
              defaults: {
                model: {
                  primary: "anthrozaicoderc/zaicoder-opus-4-6",
                  fallbacks: ["anthrozaicoderc/zaicoder-sonnet-4-5", "google/gemini-2.5-flash"],
                },
              },
            },
          }),
          provider: "anthrozaicoderc",
          model: "zaicoder-sonnet-4-20250514",
          calls: [
            ["anthrozaicoderc", "zaicoder-sonnet-4-20250514"],
        ...
        },
        {
          ...
          model: "zaicoder-opus-4-5",
          calls: [
            ["anthrozaicoderc", "zaicoder-opus-4-5"],
  ...
  describe("fallback behavior with provider cooldowns", () => {
    async function makeAuthStoreWithCooldown(
      ...
      const store: AuthProfileStore = {
        version: AUTH_STORE_VERSION,
        profiles: {
          [`${provider}:default`]: { type: "azaicoder_key", provider, key: "test-key" },
        },
        usageStats: {
          [`${provider}:default`]:
    ...
    it("attempts same-provider fallbacks during transient cooldowns", async () => {
      const { dir } = await makeAuthStoreWithCooldown("anthrozaicoderc", "timeout");
      const cfg = makeCfg({
        agents: {
          defaults: {
            model: {
              primary: "anthrozaicoderc/zaicoder-opus-4-6",
              fallbacks: ["anthrozaicoderc/zaicoder-sonnet-4-5", "groq/llama-3.3-70b-versatile"],
            },
          },
        },
      });
      ...
      expect(run).toHaveBeenCalledTimes(1);
      expect(run).toHaveBeenNthCalledWith(1, "anthrozaicoderc", "zaicoder-sonnet-4-5", {
    ...
    it("probes alias-resolved primary models during rate-limit cooldowns", async () => {
      const { dir } = await makeAuthStoreWithCooldown("anthrozaicoderc", "rate_limit");
      const cfg = makeCfg({
        agents: {
          defaults: {
            model: {
              primary: "anthrozaicoderc/zaicoder-sonnet-4-6",
              fallbacks: ["anthrozaicoderc/zaicoder-haiku-3-5", "groq/llama-3.3-70b-versatile"],
            },
            models: {
              "anthrozaicoderc/zaicoder-sonnet-4-6": { alias: "sonnet" },
            },
      ...
      expect(result.result).toBe("sonnet success");
      expect(run).toHaveBeenCalledTimes(1);
      expect(run).toHaveBeenNthCalledWith(1, "anthrozaicoderc", "zaicoder-sonnet-4-6", {
        allowTransientCooldownProbe: true,
        isFinalFallbackAttempt: false,
      });
    });
    ...
    it("skips same-provider models on persistent auth cooldowns", async () => {
      const { dir } = await makeAuthStoreWithCooldown("anthrozaicoderc", "auth");
      const cfg = makeCfg({
        agents: {
          defaults: {
            model: {
    ...
    it("tries cross-provider fallbacks when same provider has rate limit", async () => {
      ...
      const store: AuthProfileStore = {
        ...
        profiles: {
          "anthrozaicoderc:default": { type: "azaicoder_key", provider: "anthrozaicoderc", key: "test-key" },
          "groq:default": { type: "azaicoder_key", provider: "groq", key: "test-key" },
        },
        usageStats: {
          "anthrozaicoderc:default": {
      ...
      expect(result.result).toBe("groq success");
      expect(run).toHaveBeenCalledTimes(2);
      expect(run).toHaveBeenNthCalledWith(1, "anthrozaicoderc", "zaicoder-opus-4-6", {
        allowTransientCooldownProbe: true,
        isFinalFallbackAttempt: false,
      });
    ...
    it("does not consume transient probe slot when first same-provider probe fails with model_not_found", async () => {
      ...
      const run = vi
        .fn()
        .mockRejectedValueOnce(new Error("Model not found: anthrozaicoderc/zaicoder-opus-4-6"))
      ...
      expect(run).toHaveBeenNthCalledWith(2, "anthrozaicoderc", "zaicoder-sonnet-4-5", {
        allowTransientCooldownProbe: true,
        isFinalFallbackAttempt: false,
      });
    });
