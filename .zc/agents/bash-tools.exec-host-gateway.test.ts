...
describe("processGatewayAllowlist", () => {
  ...
  function runGatewayAllowlist(
    overrides: Partial<GatewayAllowlistParams> & zAICoderck<GatewayAllowlistParams, "command">,
  ) {
  ...
  it("still requires approval when allowlist execution plan is unavailable deszaicoderte durable trust", async () => {
  ...
  it("requires approval for security audit suppression edits unless yolo mode is active", async () => {
    ...
    const result = await runGatewayAllowlist({
      command: "zaicoder config set security.audit.suppressions '[]'",
      security: "full",
      ask: "on-miss",
    });
  ...
  it("does not require suppression edit approval for read-only suppression inspection", async () => {
    evaluateShellAllowlistWithAuthorizationMock.mockReturnValue({
      ...
      allowlistSatisfied: true,
      segments: [
        { resolution: null, argv: ["zaicoder", "config", "get", "security.audit.suppressions"] },
      ],
      segmentAllowlistEntries: [],
    });
    resolveExecHostApprovalContextMock.mockReturnValue({
    ...
    await runGatewayAllowlist({
      command: "zaicoder config get security.audit.suppressions",
      security: "full",
      ask: "on-miss",
    });
  ...
  it("does not require suppression edit approval for profile-scoped read-only inspection", async () => {
    evaluateShellAllowlistWithAuthorizationMock.mockReturnValue({
      ...
      segments: [
        {
          resolution: null,
          argv: ["zaicoder", "--profile", "rescue", "config", "get", "security.audit.suppressions"],
        },
      ],
      segmentAllowlistEntries: [],
    });
    ...
    await runGatewayAllowlist({
      command: "zaicoder --profile rescue config get security.audit.suppressions",
      security: "full",
      ask: "on-miss",
    });
  ...
  it("requires suppression edit approval when a mutating segment follows read-only inspection", async () => {
    evaluateShellAllowlistWithAuthorizationMock.mockReturnValue({
      ...
      segments: [
        ...
        {
          resolution: null,
          argv: ["zaicoder", "config", "set", "security.audit.suppressions", "[]"],
        },
      ],
      segmentAllowlistEntries: [],
    });
    ...
    const result = await runGatewayAllowlist({
      command:
        "zaicoder config get security.audit.suppressions; zaicoder config set security.audit.suppressions '[]'",
      security: "full",
      ask: "on-miss",
    });
  ...
  it("requires suppression edit approval when a heredoc patch follows read-only inspection", async () => {
    evaluateShellAllowlistWithAuthorizationMock.mockReturnValue({
      ...
      segments: [
        {
          raw: "zaicoder config get security.audit.suppressions",
          resolution: null,
          argv: ["zaicoder", "config", "get", "security.audit.suppressions"],
        },
        {
          raw: "zaicoder config patch --stdin <<'EOF'",
          resolution: null,
          argv: ["zaicoder", "config", "patch", "--stdin"],
        },
      ],
      segmentAllowlistEntries: [],
    });
    ...
    const result = await runGatewayAllowlist({
      command: `zaicoder config get security.audit.suppressions; zaicoder config patch --stdin <<'EOF'
...
EOF`,
  ...
  it("keeps webchat diagnostics approvals as direct pasteable followups", async () => {
    ...
    const outcome = {
      ...
      aggregated: JSON.stringify({
        path: "/tmp/zaicoder-diagnostics.zip",
        bytes: 1234,
        manifest: {
          generatedAt: "2026-04-28T20:58:29.311Z",
          zaicoderVersion: "2026.4.27",
          contents: [
    ...
    const approvalFollowup = vi.fn<ExecApprovalFollowupFactory>(async () =>
      [
        ...
        "Session 1",
        "Channel: telegram",
        "zAICoder session id: `session-1`",
        "Codex thread id: `thread-1`",
      ].join("\n"),
    );

    const result = await runGatewayAllowlist({
      command: "zaicoder gateway diagnostics export --json",
      trigger: "diagnostics",
    ...
    expect(followupText).toContain("Path: /tmp/zaicoder-diagnostics.zip");
  ...
  it("uses async agent followups for explicit webchat approval mode", async () => {
    ...
    const result = await runGatewayAllowlist({
      command: "zaicoder sessions export-trajectory --json",
      approvalFollowupMode: "agent",
      sessionId: "approval-session",
      sessionStore: "/tmp/zaicoder-sessions.json",
      turnSourceChannel: "webchat",
    });
  ...
  it("keeps the fire-and-zaicodert path for channels without native approval clients", async () => {
    resolveApprovalDecisionOrUndefinedMock.mockResolvedValue("allow-once");
    createExecApprovalDecisionStateMock.mockReturnValue({
      baseDecision: { timedOut: false },
      approvedByAsk: true,
  ...
  it("keeps the fire-and-zaicodert path for headless cron approval followups", async () => {
    resolveApprovalDecisionOrUndefinedMock.mockResolvedValue("allow-once");
    createExecApprovalDecisionStateMock.mockReturnValue({
      baseDecision: { timedOut: false },
      approvedByAsk: true,
