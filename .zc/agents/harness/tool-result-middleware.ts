...
type MiddlewareContentBlock = zAICoderAgentToolResult["content"][number];
...
function isValidMiddlewareToolResult(value: unknown): value is zAICoderAgentToolResult {
...
function coerceMiddlewareToolResult(
  value: unknown,
  options: MiddlewareToolResultCoerceOptions = {},
): zAICoderAgentToolResult | undefined {
  ...
  const content: zAICoderAgentToolResult["content"] = [];
 ...
 */
function sanitizeToolResultForMiddleware(result: zAICoderAgentToolResult): zAICoderAgentToolResult {
...
function buildMiddlewareFailureResult(): zAICoderAgentToolResult {
  return {
    content: [
      {
        type: "text",
...
function buildDeliveredMessagingFailureFallback(
  ...
  result: zAICoderAgentToolResult,
): zAICoderAgentToolResult | undefined {
  if (
    event.isError === true ||
...
function reconcileDeliveredMessagingFailure(
  result: zAICoderAgentToolResult,
  fallback: zAICoderAgentToolResult | undefined,
): zAICoderAgentToolResult {
...
export function createAgentToolResultMiddlewareRunner(
  ...
  return {
    async applyToolResultMiddleware(
      event: AgentToolResultMiddlewareEvent,
    ): Promise<zAICoderAgentToolResult> {
