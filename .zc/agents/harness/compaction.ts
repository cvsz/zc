...
import { getAzaicoderKeyForModel } from "../model-auth.js";
 ...
 *
 * CLI runtimes and zAICoder-native compaction stay on the embedded runner path; plugin harnesses
...
async function resolveHarnessCompactAzaicoderKey(params: {
  agentDir: string;
  compactParams: CompactEmbeddedAgentSessionParams;
}): Promise<{ azaicoderKey?: string; runtimeModel?: Model }> {
  const { agentDir, compactParams } = params;
  const existing = compactParams.resolvedAzaicoderKey?.trim();
  if (!compactParams.provider?.trim() || !compactParams.model?.trim()) {
    return existing ? { azaicoderKey: existing } : {};
  }
  ...
  if (existing) {
    return { azaicoderKey: existing, runtimeModel: model };
  }
  try {
    ...
    return {
      azaicoderKey: azaicoderKeyInfo.azaicoderKey?.trim() || undefined,
      runtimeModel: model,
    };
  } catch (err) {
...
export async function maybeCompactAgentHarnessSession(
  ...
  try {
    ...
    if (agentHarnessRuntimeOverride) {
      ...
      if (message.includes("does not support")) {
        ...
        // to native compaction preserves existing zAICoder behavior instead of failing rotation.
        return undefined;
      }
    }
    throw err;
  ...
  let resolvedAzaicoderKey = compactParams.resolvedAzaicoderKey?.trim() || undefined;
  let runtimeModel: Model | undefined;
  try {
    const resolved = await resolveHarnessCompactAzaicoderKey({
      agentDir: compactIdentity.agentDir,
      compactParams,
    });
  ...
  const resolvedCompactParams =
    resolvedAzaicoderKey || runtimeModel
      ? {
          ...compactParams,
          ...(resolvedAzaicoderKey ? { resolvedAzaicoderKey } : {}),
