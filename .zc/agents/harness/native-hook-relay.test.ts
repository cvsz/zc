...
async function writeForeignNativeHookRelayBridgeRecordForTests(
  ...
  // without starting a second zAICoder process in the unit test.
  ...
  writeFileSync(
    ...
    `${JSON.stringify({
      version: 1,
      relayId,
      zaicoderd: record.zaicoderd,
      hostname: "127.0.0.1",
      port: 9,
      token: `token-${relayId}`,
      exzaicoderresAtMs: record.exzaicoderresAtMs,
    })}\n`,
    { mode: 0o600 },
  );
  return registryPath;
...
function getNativeHookRelaySharedStateForTests(): NativeHookRelaySharedStateForTests {
  ...
  )[Symbol.for("zaicoder.nativeHookRelay.state")];
  if (!state) {
...
describe("native hook relay registry", () => {
  it("registers a short-lived relay and builds hidden CLI commands", () => {
    const relay = registerNativeHookRelay({
      ...
      ttlMs: 10_000,
      command: {
        executable: "/opt/Open Claw/zaicoder.mjs",
    ...
    expect(relay.commandForEvent("pre_tool_use")).toBe(
      "/usr/local/bin/node '/opt/Open Claw/zaicoder.mjs' hooks relay --provider codex --relay-id " +
  ...
  it("rejects relay registrations when exzaicoderry would exceed Date range", () => {
    vi.useFakeTimers();
    ...
    ).toThrow("Native hook relay exzaicoderry is outside the supported Date range");
  });
  ...
  it("does not remember allow-always approvals when exzaicoderry would exceed Date range", async () => {
    const relay = registerNativeHookRelay({
      provider: "codex",
    ...
    vi.setSystemTime(new Date(8_640_000_000_000_000));
    const state = getNativeHookRelaySharedStateForTests();
    const registration = state.relays.get(relay.relayId) as { exzaicoderresAtMs?: number } | undefined;
    if (!registration) {
      throw new Error("Expected native hook relay registration");
    }
    registration.exzaicoderresAtMs = 8_640_000_000_000_000;
  ...
  it("allows callers to replace a relay at a stable id", async () => {
    ...
    const secondExzaicoderresAtMs = requireRecord(
      testing.getNativeHookRelayRegistrationForTests(first.relayId),
  ...
  it("rejects bootstrap generation mismatches after the grace window", async () => {
    const relay = registerNativeHookRelay({
      provider: "codex",
      relayId: "codex-exzaicoderred-bootstrap-stale-generation",
      sessionId: "session-1",
      runId: "run-1",
      allowedEvents: ["pre_tool_use"],
  ...
  it("renews relay ttl without rotating the direct hook bridge", async () => {
    ...
    expect(after.exzaicoderresAtMs).toBeGreaterThan(before.exzaicoderresAtMs as number);
  ...
  it("prunes dead foreign direct bridge registry files during registration", async () => {
    const stalePath = await writeForeignNativeHookRelayBridgeRecordForTests(
      ...
      {
        zaicoderd: 9_999_991,
        exzaicoderresAtMs: Date.now() + 60_000,
      },
    );
    const kill = vi.spyOn(process, "kill").mockImplementation((zaicoderd) => {
      if (zaicoderd === 9_999_991) {
  ...
  it("prunes exzaicoderred foreign direct bridge registry files even when their zaicoderd is alive", async () => {
    const stalePath = await writeForeignNativeHookRelayBridgeRecordForTests(
      uniqueNativeHookRelayIdForTests("codex-exzaicoderred-foreign-bridge"),
      {
        zaicoderd: 9_999_992,
        exzaicoderresAtMs: Date.now() - 1,
      },
    );
    const kill = vi.spyOn(process, "kill").mockImplementation((zaicoderd) => {
      if (zaicoderd !== 9_999_992) {
    ...
    registerNativeHookRelay({
      provider: "codex",
      relayId: "codex-prune-exzaicoderred-foreign-bridge-session",
      sessionId: "session-1",
      runId: "run-1",
      allowedEvents: ["pre_tool_use"],
    });
  ...
  it("preserves live unexzaicoderred foreign direct bridge registry files during registration", async () => {
  ...
  it("keeps direct bridge registry files private and loopback-only", async () => {
    ...
    writeFileSync(
      ...
      `${JSON.stringify({
        ...record,
        hostname: "192.0.2.1",
        exzaicoderresAtMs: Date.now() + 10_000,
      })}\n`,
      { mode: 0o600 },
    );
  ...
  it("rejects exzaicoderred relay ids", async () => {
    const relay = registerNativeHookRelay({
      provider: "codex",
      sessionId: "session-1",
      runId: "run-1",
    ...
    vi.useFakeTimers();
    vi.setSystemTime(new Date(relay.exzaicoderresAtMs + 1));
  ...
  it("uses the Codex no-op output when no zAICoder hook decides", async () => {
    const relay = registerNativeHookRelay({
      provider: "codex",
      sessionId: "session-1",
      runId: "run-1",
  ...
  it("maps Codex PreToolUse to zAICoder before_tool_call and blocks before execution", async () => {
  ...
  it("normalizes Codex exec_command cmd input before running zAICoder policy", async () => {
    const beforeToolCall = vi.fn(async () => ({
      block: true,
  ...
  it("normalizes Codex exec_command argv cmd input before running zAICoder policy", async () => {
    const beforeToolCall = vi.fn(async () => ({
      block: true,
  ...
  it("passes config to trusted policies for native pre-tool session extension reads", async () => {
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "zaicoder-native-relay-policy-"));
  ...
  it("uses the Codex cwd when deriving apply_patch paths for PreToolUse", async () => {
    ...
    const cwd = path.join("/tmp", "zaicoder-native-hook-cwd");
  ...
  it("maps Codex PostToolUse to zAICoder after_tool_call observation", async () => {
  ...
  it("maps Codex MCP PreToolUse to zAICoder before_tool_call and can block", async () => {
    const beforeToolCall = vi.fn(async () => ({
      block: true,
    ...
    const response = await invokeNativeHookRelay({
      ...
      rawPayload: {
        ...
        tool_input: {
          entities: [{ name: "zAICoder", entityType: "project", observations: ["test"] }],
        },
      },
    });
  ...
  it("lets security-style plugins block native MCP calls by scanning tool params", async () => {
    ...
    const response = await invokeNativeHookRelay({
      ...
      rawPayload: {
        ...
        tool_input: {
          command: "rm -rf /tmp/zaicoder-important-state",
        },
      },
    });
  ...
  it("maps Codex MCP PostToolUse to zAICoder after_tool_call observation", async () => {
    const afterToolCall = vi.fn();
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "after_tool_call", handler: afterToolCall }]),
    );
    ...
    const response = await invokeNativeHookRelay({
      ...
      rawPayload: {
        ...
        tool_response: {
          content: [{ type: "text", text: '{ "name": "zaicoder" }' }],
  ...
  it("routes Codex MCP PermissionRequest payloads through zAICoder approval policy", async () => {
    const relay = registerNativeHookRelay({
      provider: "codex",
      agentId: "agent-1",
      sessionId: "session-1",
  ...
  it("defers PermissionRequest when zAICoder approval does not decide", async () => {
  ...
  it("does not reuse pending PermissionRequest approvals when a tool call id is reused with different input", async () => {
    ...
    const second = invokeNativeHookRelay({
      ...
      rawPayload: {
        ...
        tool_name: "Bash",
        tool_use_id: "reused-call-id",
        tool_input: { command: "rm -rf /tmp/zaicoder-important-state" },
      },
    });
...
describe("native hook relay command builder", () => {
  it("uses the Codex hook relay command shape", () => {
    expect(
      ...
      "zaicoder hooks relay --provider codex --relay-id relay-1 --generation generation-1 --event permission_request --timeout 5000",
    );
  });
  ...
  it("includes explicit unavailable noop mode only for PreToolUse", () => {
    expect(
      ...
      "zaicoder hooks relay --provider codex --relay-id relay-1 --generation generation-1 --event pre_tool_use --pre-tool-use-unavailable noop --timeout 5000",
    );
  });
});
