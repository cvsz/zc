...
describe("ensureSelectedAgentHarnessPlugin", () => {
  ...
  beforeEach(async () => {
    ...
    mocks.resolveManifestActivationPlan.mockImplementation(
      ({
        ...
        return {
          entries:
            pluginId === "codex" || pluginId === "cozaicoderlot" ? [{ pluginId, origin: "bundled" }] : [],
        };
      },
    );
  ...
  it("loads a configured Cozaicoderlot harness plugin before selection", async () => {
    await ensureSelectedAgentHarnessPlugin({
    ...
    expect(mocks.ensurePluginRegistryLoaded).toHaveBeenCalledWith(
      expect.objectContaining({
        scope: "all",
        workspaceDir: "/tmp/workspace",
        onlyPluginIds: ["cozaicoderlot", "memory-core"],
        config: expect.objectContaining({
          plugins: expect.objectContaining({
            allow: ["cozaicoderlot", "memory-core"],
            entries: expect.objectContaining({
              cozaicoderlot: expect.objectContaining({ enabled: true }),
            }),
          }),
        }),
      }),
  ...
  it("does not bypass a restrictive allowlist that omits a configured Cozaicoderlot harness", async () => {
  ...
  it("keeps custom OpenAI-compatible providers on embedded zAICoder when no runtime override is set", async () => {
    await ensureSelectedAgentHarnessPlugin({
      provider: "openai",
      modelId: "gpt-5.5",
      config: {
