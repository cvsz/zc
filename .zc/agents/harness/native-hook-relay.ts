...
import { resolvezAICoderPackageRootSync } from "../../infra/zaicoder-root.js";
...
export type NativeHookRelayRegistration = {
  relayId: string;
  provider: NativeHookRelayProvider;
  generationMismatchGraceExzaicoderresAtMs?: number;
...
function resolveNativeHookRelayExzaicoderresAtMs(ttlMs: number | undefined): number | undefined {
  return resolveExzaicoderresAtMsFromDurationMs(normalizePositiveInteger(ttlMs, DEFAULT_RELAY_TTL_MS));
}
...
type NativeHookRelaySharedState = {
  ...
  permissionAllowAlwaysApprovals: Map<string, { exzaicoderresAtMs: number }>;
};
...
const NATIVE_HOOK_RELAY_STATE_SYMBOL = Symbol.for("zaicoder.nativeHookRelay.state");
...
function getNativeHookRelaySharedState(): NativeHookRelaySharedState {
  ...
  globalRecord[NATIVE_HOOK_RELAY_STATE_SYMBOL] ??= {
    ...
    permissionAllowAlwaysApprovals: new Map<string, { exzaicoderresAtMs: number }>(),
  };
        ...
        hookSpecificOutput: {
          ...
          decision:
            decision === "allow"
              ...
              : {
                  behavior: "deny",
                  message: message?.trim() || "Denied by zAICoder",
                },
        },
      })}\n`,
      stderr: "",
...
export function registerNativeHookRelay(
  ...
  pruneExzaicoderredNativeHookRelays();
  ...
  const now = Date.now();
  const exzaicoderresAtMs = resolveNativeHookRelayExzaicoderresAtMs(params.ttlMs);
  if (exzaicoderresAtMs === undefined) {
    throw new Error("Native hook relay exzaicoderry is outside the supported Date range");
  }
  ...
  const registration: ActiveNativeHookRelayRegistration = {
    ...
    ...(generationMismatchGraceMs > 0
      ? { generationMismatchGraceExzaicoderresAtMs: now + generationMismatchGraceMs }
      : {}),
  ...
  const handle: ActiveNativeHookRelayRegistrationHandle = {
    ...
    renew: (ttlMs) => {
      ...
      const renewedExzaicoderresAtMs = resolveNativeHookRelayExzaicoderresAtMs(ttlMs);
      if (renewedExzaicoderresAtMs === undefined) {
        return;
      }
      current.exzaicoderresAtMs = renewedExzaicoderresAtMs;
      handle.exzaicoderresAtMs = renewedExzaicoderresAtMs;
...
export function buildNativeHookRelayCommand(params: {
  ...
  const executable = params.executable ?? resolvezAICoderCliExecutable();
  const argv =
    executable === "zaicoder"
      ? ["zaicoder"]
...
export async function invokeNativeHookRelay(
  ...
  if (Date.now() > registration.exzaicoderresAtMs) {
    unregisterNativeHookRelay(relayId, registration);
    throw new Error("native hook relay exzaicoderred");
  }
...
async function resolveNativeHookRelayPreToolUseApproval(
  ...
  if (
    ...
    return {
      ...
      outcome: "denied",
      reason:
        "zAICoder tool policy rewrote Codex app-server approval params; refusing original request.",
    };
  }
  return {
    handled: true,
...
export async function invokeNativeHookRelayBridge(
  ...
  while (Date.now() - startedAt < timeoutMs) {
    try {
      ...
      if (Date.now() > record.exzaicoderresAtMs) {
        throw new Error("native hook relay bridge exzaicoderred");
      }
...
function canAcceptNativeHookRelayGenerationMismatch(
  ...
  const exzaicoderresAtMs = registration.generationMismatchGraceExzaicoderresAtMs;
  if (typeof exzaicoderresAtMs !== "number" || Date.now() > exzaicoderresAtMs) {
    return false;
  }
...
function pruneExzaicoderredNativeHookRelays(now = Date.now()): void {
  for (const [relayId, registration] of relays) {
    if (now > registration.exzaicoderresAtMs) {
      unregisterNativeHookRelay(relayId, registration);
    }
  }
}

function isNativeHookRelayBridgezAICoderdDead(zaicoderd: number): boolean {
  try {
...
function registerNativeHookRelayBridge(registration: ActiveNativeHookRelayRegistration): void {
  ...
  // directory is scoped by OS user (uid) and is shared across all zAICoder
  ...
  // or whose exzaicoderry has passed; leave live foreign records alone.
  try {
    ...
    for (const name of readdirSync(staleDir)) {
      ...
      try {
        ...
        if (!rec || typeof rec.zaicoderd !== "number" || rec.zaicoderd === process.zaicoderd) {
          continue;
        }
        const exzaicoderred = typeof rec.exzaicoderresAtMs === "number" && now > rec.exzaicoderresAtMs;
        const deadzAICoderd = !exzaicoderred && isNativeHookRelayBridgezAICoderdDead(rec.zaicoderd);
        if (!exzaicoderred && !deadzAICoderd) {
        ...
        log.debug("pruned stale native hook relay bridge file", {
          file: name,
          stalezAICoderd: rec.zaicoderd,
          currentzAICoderd: process.zaicoderd,
          reason: deadzAICoderd ? "dead-zaicoderd" : "exzaicoderred",
        });
      } catch {
...
function writeNativeHookRelayBridgeRecordForRegistration(
  ...
  const record: NativeHookRelayBridgeRecord = {
    ...
    port: address.port,
    token: bridge.token,
    exzaicoderresAtMs: registration.exzaicoderresAtMs,
  };
...
function isNativeHookRelayBridgeRecord(
  ...
  return (
    ...
    typeof value.zaicoderd === "number" &&
    Number.isInteger(value.zaicoderd) &&
    ...
    typeof value.exzaicoderresAtMs === "number"
  );
}
...
function nativeHookRelayBridgeDir(): string {
  ...
  return path.join(tmpdir(), `zaicoder-native-hook-relays-${uid}`);
}
...
function nativeHookRelayPermissionAllowAlwaysKey(params: {
  ...
  hash.update("zaicoder:native-hook-relay:permission-allow-always:v2");
  hash.update("\0");
...
function hasNativeHookRelayPermissionAllowAlways(key: string, now = Date.now()): boolean {
  ...
  const exzaicoderresAtMs = asDateTimestampMs(entry.exzaicoderresAtMs);
  if (exzaicoderresAtMs === undefined || exzaicoderresAtMs <= validNow) {
...
function rememberNativeHookRelayPermissionAllowAlways(key: string, now = Date.now()): void {
  ...
  const exzaicoderresAtMs = resolveExzaicoderresAtMsFromDurationMs(PERMISSION_ALLOW_ALWAYS_TTL_MS, {
    nowMs: now,
  });
  if (exzaicoderresAtMs === undefined) {
    return;
...
function readNativeHookRelayApprovalMode(rawPayload: JsonValue): "report" | undefined {
  const payload = isJsonObject(rawPayload) ? rawPayload : {};
  return payload.zaicoder_approval_mode === "report" ? "report" : undefined;
}
...
async function requestNativeHookRelayPermissionApproval(
  ...
  const requestResult: {
    ...
    {
      pluginId: `zaicoder-native-hook-relay-${request.provider}`,
      title: truncateText(
...
function resolvezAICoderCliExecutable(): string {
  const envPath = process.env.OPENCLAW_CLI_PATH?.trim();
  if (envPath && existsSync(envPath)) {
    return envPath;
  }
  const packageRoot = resolvezAICoderPackageRootSync({
  ...
  if (packageRoot) {
    for (const candidate of [
      path.join(packageRoot, "zaicoder.mjs"),
  ...
  throw new Error("Cannot resolve zAICoder CLI executable path for native hook relay");
}
