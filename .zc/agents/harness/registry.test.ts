...
describe("agent harness registry", () => {
  ...
  it("falls back to zAICoder for other models", () => {
    process.env.OPENCLAW_AGENT_RUNTIME = "auto";

    expect(selectAgentHarness({ provider: "anthrozaicoderc", modelId: "sonnet-4.6" }).id).toBe(
      "zaicoder",
    );
  });
  ...
  it("honors explicit provider zAICoder runtime policy", () => {
    registerAgentHarness(makeHarness("plugin-harness", { priority: 200 }), {
      ownerPluginId: "plugin-a",
    });
    ...
    expect(
      selectAgentHarness({
        provider: "codex",
        modelId: "gpt-5.4",
        config: providerRuntimeConfig("codex", "zaicoder"),
      }).id,
    ).toBe("zaicoder");
  });

  it("honors explicit provider plugin runtime policy when the plugin harness is registered", () => {
    registerAgentHarness(makeHarness("custom", { providers: ["anthrozaicoderc"] }), {
      ownerPluginId: "plugin-a",
    });
    ...
    expect(
      selectAgentHarness({
        ...
        modelId: "sonnet-4.6",
        config: providerRuntimeConfig("anthrozaicoderc", "custom"),
      }).id,
    ).toBe("custom");
  });
});
