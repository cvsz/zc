import { findNormalizedProviderValue } from "@zaicoder/model-catalog-core/provider-id";
/**
...
import { createzAICoderAgentHarness } from "./builtin-zaicoder.js";
...
type AgentHarnessSelectionDecision = {
  ...
  selectedReason:
    | "forced_zaicoder"
    | "forced_plugin"
    // Implicit Codex preference found no registered Codex harness, so zAICoder handled the run.
    | "implicit_plugin_unavailable_zaicoder"
    // Provider-owned CLI runtime aliases have no agent harness plugin counterpart.
    | "cli_runtime_passthrough_zaicoder"
    // Auto mode chose a registered plugin harness that supports the provider/model.
    | "auto_plugin"
    // Auto mode found no supporting plugin harness, so zAICoder handled the run.
    | "auto_zaicoder";
  candidates: AgentHarnessSelectionCandidate[];
};

type PluginHarnessToolPolicyContext = zAICoderck<
  EmbeddedRunAttemptParams,
  | "config"
  | "sessionKey"
  | "sandboxSessionKey"
...
function buildAgentHarnessSupportModelProvider(params: {
  ...
  const modelConfig = params.modelId
    ...
    : undefined;
  return {
    azaicoder: modelConfig?.azaicoder ?? providerConfig.azaicoder ?? "openai-responses",
    baseUrl: modelConfig?.baseUrl ?? providerConfig.baseUrl,
    azureAzaicoderVersion: readStringParam(
      modelConfig?.params?.azureAzaicoderVersion ?? providerConfig.params?.azureAzaicoderVersion,
    ),
...
function selectAgentHarnessDecision(params: {
  ...
  const policy =
    runtimeOverride && !isDefaultAgentRuntimeId(runtimeOverride)
      ...
      : resolvedPolicy;
  // zAICoder's built-in harness is intentionally not part of the plugin candidate list. Explicit plugin
  // runtimes fail closed; only `auto` may route an unmatched turn to zAICoder.
  const pluginHarnesses = listPluginAgentHarnesses();
  const openClawHarness = createzAICoderAgentHarness();
  const runtime = policy.runtime;
  if (runtime === "zaicoder") {
    return buildSelectionDecision({
      harness: openClawHarness,
      policy,
      selectedReason: "forced_zaicoder",
  ...
  if (runtime !== "auto") {
    ...
    if (forced) {
      ...
      if (isCliRuntimeAliasForProvider({ runtime, provider: params.provider })) {
        return buildSelectionDecision({
          ...
          selectedReason: "cli_runtime_passthrough_zaicoder",
          candidates: listHarnessCandidates(pluginHarnesses),
        });
      }
      throw new Error(
    ...
    if (runtime === "codex" && policy.runtimeSource === "implicit") {
      return buildSelectionDecision({
        ...
        selectedReason: "implicit_plugin_unavailable_zaicoder",
        candidates: listHarnessCandidates(pluginHarnesses),
      });
    }
    if (
...
export async function runAgentHarnessAttempt(
  ...
  const attemptParams =
    harness.id === "zaicoder" ? params : applyPluginHarnessDenyAllToolPolicy(params);
  ...
  try {
    ...
    log.warn(`${harness.label} failed; not falling back to embedded zAICoder backend`, {
