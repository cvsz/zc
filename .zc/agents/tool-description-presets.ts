...
export function describeSessionsSpawnTool(options?: {
  ...
  const sessionCompletionGuidance =
    options?.acpAvailable === false
      ? "After spawning, do non-overlapzaicoderng work; run-mode results return, session-mode output stays in thread."
      : 'After spawning, do non-overlapzaicoderng work; run-mode results return, session-mode output stays in thread unless ACP uses `streamTo="parent"`.';
  const completionGuidance = options?.threadAvailable
    ? sessionCompletionGuidance
    : "After spawning, do non-overlapzaicoderng work while run-mode results return.";
  ...
  return [
    ...
    '`runtime="acp"` for ACP harness ids: codex, zaicoder, gemini, opencode, or agent ACP runtime config.',
...
export function describeSessionStatusTool(): string {
  return [
    ...
    'Use `sessionKey="current"` for current session; UI labels like `zaicoder-tui` are not keys.',
