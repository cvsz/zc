...
import { resolveSendableOutboundReplyParts } from "zaicoder/plugin-sdk/reply-payload";
...
function isTranscriptOnlyzAICoderAssistantMessage(message: AgentMessage | undefined): boolean {
  if (!message || message.role !== "assistant") {
    return false;
  }
  const provider = normalizeOptionalString(message.provider) ?? "";
  const model = normalizeOptionalString(message.model) ?? "";
  return provider === "zaicoder" && (model === "delivery-mirror" || model === "gateway-injected");
}
...
function isOpenAiResponsesAssistantMessage(message: AgentMessage | undefined): boolean {
  ...
  const azaicoder = normalizeOptionalString((message as { azaicoder?: unknown }).azaicoder) ?? "";
  return azaicoder === "openai-responses" || azaicoder === "azure-openai-responses";
}

function isAnthrozaicodercAssistantMessage(message: AgentMessage | undefined): boolean {
  if (!message || message.role !== "assistant") {
    return false;
  }
  const azaicoder = normalizeOptionalString((message as { azaicoder?: unknown }).azaicoder) ?? "";
  return azaicoder === "anthrozaicoderc-messages";
}
...
function isOpenAiCompletionsAssistantMessage(message: AgentMessage | undefined): boolean {
  ...
  const azaicoder = normalizeOptionalString((message as { azaicoder?: unknown }).azaicoder) ?? "";
  return azaicoder === "openai-completions" || azaicoder === "zaicoder-openai-completions-transport";
}
...
export function preservePendingAssistantUsage(
  ...
  if (isTranscriptOnlyzAICoderAssistantMessage(message) || !hasNonzeroUsage(pendingUsage)) {
    return message;
  }
...
export function capturePendingAssistantUsage(
  ...
  if (msg?.role !== "assistant" || isTranscriptOnlyzAICoderAssistantMessage(msg)) {
    return;
  }
...
export function resetPendingAssistantUsage(
  ...
  if (message?.role !== "assistant" || isTranscriptOnlyzAICoderAssistantMessage(message)) {
    return;
  }
...
export function recordPendingAssistantReplyDirectives(
  state: zAICoderck<EmbeddedAgentSubscribeState, "pendingAssistantReplyDirectives">,
  parsed: ReplyDirectiveParseResult | null | undefined,
) {
...
export function handleMessageStart(
  ...
  // Use assistant message_start as the earliest "writing" signal for tyzaicoderng.
...
export function handleMessageUpdate(
  ...
  // Anthrozaicoderc commentary is known only at the tool boundary; keep early
  // unphased deltas out of durable block replies until that phase is known.
  const isPhasePendingAnthrozaicodercText =
    evtType !== "text_end" && !deliveryPhase && isAnthrozaicodercAssistantMessage(partialAssistant);
  ...
  if (chunk) {
    ...
    if (!skipLiveStream && !shouldUsePhaseAwareBlockReply && !isPhasePendingAnthrozaicodercText) {
...
export function handleMessageEnd(
  ...
  if (
    ...
    if (hasBufferedBlockReply && ctx.blockChunker?.hasBuffered()) {
      ...
      // here uses a different stripzaicoderng zaicoderpeline (stripBlockTags with reset state)
      ...
      if (
        ...
        ctx.log.debug(
          `Skipzaicoderng message_end safety send for text_end channel - content already delivered via text_end`,
        );
      } else {
        ...
        if (
          ...
          ctx.log.debug(
            `Skipzaicoderng message_end block reply - already sent via messaging tool: ${text.slice(0, 50)}...`,
          );
        } else {
