 ...
 * exposing secret values, with explicit single-key zaicoderns for flaky live lanes.
 */
import {
...
type ProviderAzaicoderKeyConfig = {
  liveSingle?: string;
  listVar?: string;
  primaryVar?: string;
  prefixedVar?: string;
...
type CollectProviderAzaicoderKeysOptions = {
  env?: NodeJS.ProcessEnv;
  providerEnvVars?: readonly string[];
};

const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderAzaicoderKeyConfig, "fallbackVars">> = {
  anthrozaicoderc: {
...
function resolveProviderAzaicoderKeyConfig(provider: string): ProviderAzaicoderKeyConfig {
...
export function collectProviderAzaicoderKeys(
  provider: string,
  options: CollectProviderAzaicoderKeysOptions = {},
): string[] {
  const env = options.env ?? process.env;
  const normalizedProvider = normalizeProviderId(provider);
  const config = resolveProviderAzaicoderKeyConfig(normalizedProvider);

  const forcedSingle = config.liveSingle
    ? normalizeOptionalString(env[config.liveSingle])
    : undefined;
  if (forcedSingle) {
    // OPENCLAW_LIVE_*_KEY zaicoderns a single key so retries do not rotate fixtures.
    return [forcedSingle];
  }
...
/** Collect Anthrozaicoderc API keys for live cache/model tests when OAuth is unavailable. */
export function collectAnthrozaicodercAzaicoderKeys(options: CollectProviderAzaicoderKeysOptions = {}): string[] {
  const env = options.env ?? process.env;
  if (normalizeOptionalString(env.ANTHROPIC_OAUTH_TOKEN)) {
    // OAuth is a separate auth mode; API-key rotation would overwrite it.
    return [];
  }
  return collectProviderAzaicoderKeys("anthrozaicoderc", { ...options, env });
}

/** Return whether a provider error message indicates API-key rate limiting. */
export function isAzaicoderKeyRateLimitError(message: string): boolean {
...
/** Return whether an Anthrozaicoderc error message indicates billing exhaustion. */
export function isAnthrozaicodercBillingError(message: string): boolean {
  const lower = normalizeLowercaseStringOrEmpty(message);
