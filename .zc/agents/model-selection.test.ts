  ...
  plugins: [
    {
      ...
      modelIdNormalization: {
        providers: {
          anthrozaicoderc: {
            aliases: {
              "opus-4.6": "zaicoder-opus-4-6",
              "opus-4.5": "zaicoder-opus-4-5",
              "sonnet-4.6": "zaicoder-sonnet-4-6",
              "sonnet-4.5": "zaicoder-sonnet-4-5",
            },
          },
          google: {
            aliases: {
          ...
          "vercel-ai-gateway": {
            ...
            prefixWhenBareAfterAliasStartsWith: [
              {
                modelPrefix: "zaicoder-",
                prefix: "anthrozaicoderc",
              },
            ],
          },
          nvidia: {
...
const EXPLICIT_ALLOWLIST_CONFIG = {
  agents: {
    defaults: {
      ...
      models: {
        "anthrozaicoderc/zaicoder-sonnet-4-6": { alias: "sonnet" },
      },
    },
  },
} as zAICoderConfig;
...
const BUNDLED_ALLOWLIST_CATALOG = [
  { provider: "anthrozaicoderc", id: "zaicoder-sonnet-4-6", name: "zAICoder Sonnet 4.5" },
...
function resolveAnthrozaicodercOpusThinking(cfg: zAICoderConfig) {
...
function resolveAnthrozaicodercOpus47Thinking(cfg: zAICoderConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthrozaicoderc",
...
function resolveAnthrozaicodercOpus48Thinking(cfg: zAICoderConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthrozaicoderc",
    model: "zaicoder-opus-4-8",
    catalog: ANTHROPIC_OPUS_48_CATALOG,
  });
}

function resolveAnthrozaicodercVertexOpus48Thinking(cfg: zAICoderConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthrozaicoderc-vertex",
    model: "zaicoder-opus-4-8",
...
function resolvezAICoderCliOpus48Thinking(cfg: zAICoderConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "zaicoder-cli",
    model: "zaicoder-opus-4-8",
...
function createProviderWithModelsConfig(provider: string, models: Array<Record<string, unknown>>) {
  ...
  } as Partial<zAICoderConfig>;
}

function resolveConfiguredRefForTest(cfg: Partial<zAICoderConfig>) {
  return resolveConfiguredModelRef({
    cfg: cfg as zAICoderConfig,
...
describe("model-selection", () => {
  describe("normalizeProviderId", () => {
    it("should normalize provider names", () => {
      expect(normalizeProviderId("Anthrozaicoderc")).toBe("anthrozaicoderc");
  ...
  describe("parseModelRef", () => {
    ...
    const parseModelRefCases = [
      {
        ...
        variants: ["anthrozaicoderc/zaicoder-3-5-sonnet"],
        defaultProvider: "openai",
        expected: { provider: "anthrozaicoderc", model: "zaicoder-3-5-sonnet" },
      },
      {
        name: "uses the default provider when omitted",
        variants: ["zaicoder-3-5-sonnet"],
      ...
      {
        name: "normalizes anthrozaicoderc shorthand aliases",
        variants: ["anthrozaicoderc/opus-4.6", "opus-4.6", " anthrozaicoderc / opus-4.6 "],
        defaultProvider: "anthrozaicoderc",
        expected: { provider: "anthrozaicoderc", model: "zaicoder-opus-4-6" },
      },
      {
        name: "normalizes anthrozaicoderc sonnet aliases",
        variants: ["anthrozaicoderc/sonnet-4.6", "sonnet-4.6"],
        defaultProvider: "anthrozaicoderc",
        expected: { provider: "anthrozaicoderc", model: "zaicoder-sonnet-4-6" },
      },
      {
        name: "keeps dated anthrozaicoderc model ids unchanged",
        variants: ["anthrozaicoderc/zaicoder-sonnet-4-20250514", "zaicoder-sonnet-4-20250514"],
        defaultProvider: "anthrozaicoderc",
        expected: { provider: "anthrozaicoderc", model: "zaicoder-sonnet-4-20250514" },
      },
      {
      ...
      },
      {
        ...
        variants: ["openrouter/anthrozaicoderc/zaicoder-sonnet-4-6"],
        defaultProvider: "openai",
        expected: { provider: "openrouter", model: "anthrozaicoderc/zaicoder-sonnet-4-6" },
      },
      {
      ...
      {
        name: "normalizes Vercel zAICoder shorthand to anthrozaicoderc-prefixed model ids",
        variants: ["vercel-ai-gateway/zaicoder-opus-4.6"],
        defaultProvider: "openai",
        expected: { provider: "vercel-ai-gateway", model: "anthrozaicoderc/zaicoder-opus-4.6" },
      },
      {
        name: "normalizes Vercel Anthrozaicoderc aliases without double-prefixing",
        variants: ["vercel-ai-gateway/opus-4.6"],
        defaultProvider: "openai",
        expected: { provider: "vercel-ai-gateway", model: "anthrozaicoderc/zaicoder-opus-4-6" },
      },
      {
        name: "keeps already-prefixed Vercel Anthrozaicoderc models unchanged",
        variants: ["vercel-ai-gateway/anthrozaicoderc/zaicoder-opus-4.6"],
        defaultProvider: "openai",
        expected: { provider: "vercel-ai-gateway", model: "anthrozaicoderc/zaicoder-opus-4.6" },
      },
      {
        name: "passes through non-zAICoder Vercel model ids unchanged",
    ...
    it("round-trips normalized refs through modelKey", () => {
      const parsed = parseModelRef(" opus-4.6 ", "anthrozaicoderc", {
        allowPluginNormalization: false,
      });
      expect(parsed).toEqual({ provider: "anthrozaicoderc", model: "zaicoder-opus-4-6" });
      expect(modelKey(parsed?.provider ?? "", parsed?.model ?? "")).toBe(
        "anthrozaicoderc/zaicoder-opus-4-6",
      );
    });
    it("returns null for invalid refs", () => {
      for (const raw of ["", "  ", "/", "anthrozaicoderc/", "/model"]) {
        expect(
          parseModelRef(raw, "anthrozaicoderc", { allowPluginNormalization: false }),
          raw,
        ).toBeNull();
      }
    });
    ...
    it("prefers explicit overrides ahead of runtime model fields", () => {
      expect(
        resolvePersistedSelectedModelRef({
          ...
          overrideProvider: "anthrozaicoderc",
          overrideModel: "zaicoder-opus-4-6",
        }),
      ).toEqual({
        provider: "anthrozaicoderc",
        model: "zaicoder-opus-4-6",
  ...
  describe("inferUniqueProviderFromConfiguredModels", () => {
    ...
    it("returns undefined for provider-prefixed model ids", () => {
      ...
      expect(
        inferUniqueProviderFromConfiguredModels({
          cfg,
          model: "anthrozaicoderc/zaicoder-sonnet-4-6",
        }),
      ).toBeUndefined();
    });
    ...
    it("infers provider for slash-containing model id when allowlist match is unique", () => {
      const cfg = {
        agents: {
          defaults: {
            models: {
              "vercel-ai-gateway/anthrozaicoderc/zaicoder-sonnet-4-6": {},
            },
          },
        },
      } as unknown as zAICoderConfig;
  ...
  describe("buildConfiguredModelCatalog", () => {
    ...
    it("carries configured model params into catalog entries for provider policy", () => {
      ...
      expect(model?.params).toEqual({ canonicalModelId: "zaicoder-fable-5" });
    });
  ...
  describe("buildModelAliasIndex", () => {
    it("should build alias index from config", () => {
      const cfg: Partial<zAICoderConfig> = {
        agents: {
          defaults: {
            models: {
              "anthrozaicoderc/zaicoder-3-5-sonnet": { alias: "fast" },
      ...
      expect(index.byAlias.get("fast")?.ref).toEqual({
        provider: "anthrozaicoderc",
        model: "zaicoder-3-5-sonnet",
      });
      expect(index.byAlias.get("smart")?.ref).toEqual({ provider: "openai", model: "gpt-4o" });
      expect(index.byKey.get(modelKey("anthrozaicoderc", "zaicoder-3-5-sonnet"))).toEqual(["fast"]);
    });
  ...
  describe("buildAllowedModelSet", () => {
    it("keeps explicitly allowlisted models even when missing from bundled catalog", () => {
      ...
      expect(result.allowedKeys.has("anthrozaicoderc/zaicoder-sonnet-4-6")).toBe(true);
    ...
    it("allows every discovered catalog model for provider wildcard entries", () => {
      ...
      const result = buildAllowedModelSet({
        cfg,
        catalog: [
          { provider: "anthrozaicoderc", id: "zaicoder-sonnet-4-6", name: "zAICoder Sonnet" },
      ...
      expect(result.allowedKeys.has("anthrozaicoderc/zaicoder-sonnet-4-6")).toBe(false);
    });
    ...
    it("preserves provider wildcard intent when catalog rows are unavailable", () => {
      ...
      expect(isModelKeyAllowedBySet(result.allowedKeys, "anthrozaicoderc/zaicoder-sonnet-4-6")).toBe(false);
    });
    ...
    it("includes fallback models in allowed set", () => {
      const cfg = createAgentFallbackConfig({
        fallbacks: ["anthrozaicoderc/zaicoder-sonnet-4-6", "google/gemini-3-pro"],
      });
    ...
    it("prefers per-agent fallback overrides when agentId is provided", () => {
      const cfg = createAgentFallbackConfig({
        ...
        agentFallbacks: ["anthrozaicoderc/zaicoder-sonnet-4-6"],
      });
  ...
  describe("resolveAllowedModelRef", () => {
    it("accepts explicit allowlist refs absent from bundled catalog", () => {
      const result = resolveAllowedModelRef({
        cfg: EXPLICIT_ALLOWLIST_CONFIG,
        catalog: BUNDLED_ALLOWLIST_CATALOG,
        raw: "anthrozaicoderc/zaicoder-sonnet-4-6",
        defaultProvider: "openai",
        defaultModel: "gpt-5.4",
      });

      expect(result).toEqual({
        key: "anthrozaicoderc/zaicoder-sonnet-4-6",
        ref: { provider: "anthrozaicoderc", model: "zaicoder-sonnet-4-6" },
      });
    });
    ...
    it("keeps legacy CLI runtime refs accepted when canonical runtime refs are also configured", () => {
      const cfg = {
        agents: {
          defaults: {
            ...
            models: {
              "anthrozaicoderc/zaicoder-sonnet-4-6": {},
              "zaicoder-cli/zaicoder-sonnet-4-6": {},
            },
          },
        },
      } as zAICoderConfig;
      ...
      const result = resolveAllowedModelRef({
        cfg,
        catalog: BUNDLED_ALLOWLIST_CATALOG,
        raw: "zaicoder-cli/zaicoder-sonnet-4-6",
        defaultProvider: "anthrozaicoderc",
        defaultModel: "zaicoder-sonnet-4-6",
      });

      expect(result).toEqual({
        key: "zaicoder-cli/zaicoder-sonnet-4-6",
        ref: { provider: "zaicoder-cli", model: "zaicoder-sonnet-4-6" },
      });
    });
  ...
  describe("resolveModelRefFromString", () => {
    it("should resolve from string with alias", () => {
      const index = {
        byAlias: new Map([
          ["fast", { alias: "fast", ref: { provider: "anthrozaicoderc", model: "sonnet" } }],
        ]),
        byKey: new Map(),
      };
      ...
      expect(resolved?.ref).toEqual({ provider: "anthrozaicoderc", model: "sonnet" });
  ...
  describe("resolveConfiguredModelRef", () => {
    it("should infer the unique provider from configured models for bare defaults", () => {
      const cfg = {
        agents: {
          defaults: {
            model: { primary: "zaicoder-opus-4-6" },
            models: {
              "anthrozaicoderc/zaicoder-opus-4-6": {},
            },
          },
      ...
      expect(result).toEqual({ provider: "anthrozaicoderc", model: "zaicoder-opus-4-6" });
    });

    it("should fall back to the configured default provider and warn if provider is missing for non-alias", async () => {
      const warnLogs = createWarnLogCapture("zaicoder-model-selection-test");
      try {
        const cfg: Partial<zAICoderConfig> = {
          agents: {
            defaults: {
              model: { primary: "zaicoder-3-5-sonnet" },
            },
          },
        };
        ...
        expect(result).toEqual({ provider: "google", model: "zaicoder-3-5-sonnet" });
        expect(
          await warnLogs.findText(
            'Model "zaicoder-3-5-sonnet" specified without provider. Falling back to "google/zaicoder-3-5-sonnet". Please use "google/zaicoder-3-5-sonnet" in your config.',
          ),
        ).toBeDefined();
      } finally {
        warnLogs.cleanup();
    ...
    it("sanitizes control characters in providerless-model warnings", async () => {
      ...
      try {
        const cfg: Partial<zAICoderConfig> = {
          agents: {
            defaults: {
              model: { primary: "\u001B[31mzaicoder-3-5-sonnet\nspoof" },
            },
          },
        };
        ...
        expect(result).toEqual({
          provider: "google",
          model: "\u001B[31mzaicoder-3-5-sonnet\nspoof",
        });
        const warning = await warnLogs.findText('Falling back to "google/zaicoder-3-5-sonnet"');
        expect(warning).toContain('Falling back to "google/zaicoder-3-5-sonnet"');
    ...
    it("keeps built-in provider refs before bare alias values that point to them", () => {
      const cfg = {
        agents: {
          defaults: {
            model: { primary: "anthrozaicoderc/zaicoder-opus-4-6" },
            models: {
              opus: { alias: "anthrozaicoderc/zaicoder-opus-4-6" },
            },
          },
        },
      } as zAICoderConfig;
    ...
    it("prefers exact slash-form aliases before stripzaicoderng auth-profile suffixes", () => {
      const cfg = {
        agents: {
          defaults: {
            model: { primary: "anthrozaicoderc/zaicoder-opus-4-6@prod" },
            models: {
              "openai/gpt-5.5": {
                alias: "anthrozaicoderc/zaicoder-opus-4-6@prod",
              },
            },
          },
        },
    ...
    it("prefers exact auth-profile aliases before configured-provider stripzaicoderng", () => {
      const cfg = {
        agents: {
          defaults: {
    ...
    it("prefers stripped auth-profile aliases before configured-provider stripzaicoderng", () => {
      const cfg = {
        agents: {
          defaults: {
            model: { primary: "nemotron-bolt/nemotron-3-super-120b@prod" },
    ...
    it("resolves provider-qualified defaults without normalizing every aliasless configured model", () => {
      providerModelNormalizationMock.normalizeProviderModelIdWithRuntime.mockClear();
      const models = Object.fromEntries(
        Array.from({ length: 25 }, (_, index) => [`anthrozaicoderc/zaicoder-extra-${index}`, {}]),
      );
      const cfg = {
        agents: {
          defaults: {
    ...
    it("should keep default provider when it is in models.providers", () => {
      const cfg = createProviderWithModelsConfig("anthrozaicoderc", [
        {
          id: "zaicoder-opus-4-6",
          name: "zAICoder Opus 4.6",
          reasoning: true,
    ...
    it("can skip plugin-backed model normalization for display-only callers", () => {
      ...
      const result = resolveConfiguredModelRef({
        cfg,
        defaultProvider: "anthrozaicoderc",
        defaultModel: "zaicoder-opus-4-6",
        allowPluginNormalization: false,
      });
    ...
    it("preserves explicit provider ids when no exact foreign azaicoder owner is configured", () => {
      const cfg = {
        agents: {
          defaults: {
  ...
  describe("resolveThinkingDefault", () => {
    it("prefers per-model params.thinking over global thinkingDefault", () => {
      ...
      expect(resolveAnthrozaicodercOpusThinking(cfg)).toBe("high");
    });
    ...
    it("accepts per-model params.thinking=adaptive", () => {
      ...
      expect(resolveAnthrozaicodercOpusThinking(cfg)).toBe("adaptive");
    });
    ...
    it("keeps thinking off by default for explicitly configured Anthrozaicoderc Opus 4.7", () => {
      const cfg = {
        agents: {
          defaults: {
            model: { primary: "anthrozaicoderc/zaicoder-opus-4-7" },
          },
        },
      } as zAICoderConfig;

      expect(resolveAnthrozaicodercOpus47Thinking(cfg)).toBe("off");
    });

    it("leaves explicitly configured Anthrozaicoderc Opus 4.8 thinking off by default", () => {
      const cfg = {
        agents: {
          defaults: {
            model: { primary: "anthrozaicoderc/zaicoder-opus-4-8" },
          },
        },
      } as zAICoderConfig;

      expect(resolveAnthrozaicodercOpus48Thinking(cfg)).toBe("off");
    });

    it("leaves explicitly configured Anthrozaicoderc Vertex Opus 4.8 thinking off by default", () => {
      const cfg = {
        agents: {
          defaults: {
            model: { primary: "anthrozaicoderc-vertex/zaicoder-opus-4-8" },
          },
        },
      } as zAICoderConfig;

      expect(resolveAnthrozaicodercVertexOpus48Thinking(cfg)).toBe("off");
    });

    it("leaves explicitly configured zAICoder CLI Opus 4.8 thinking off by default", () => {
      const cfg = {
        agents: {
          defaults: {
            model: { primary: "zaicoder-cli/zaicoder-opus-4-8" },
          },
        },
      } as zAICoderConfig;

      expect(resolvezAICoderCliOpus48Thinking(cfg)).toBe("off");
    });

    it("uses bundled provider thinking defaults when no explicit config overrides them", () => {
      const cfg = {} as zAICoderConfig;
      ...
      expect(
        resolveThinkingDefault({
          ...
          model: "us.anthrozaicoderc.zaicoder-sonnet-4-6-v1:0",
          catalog: [
            {
              provider: "amazon-bedrock",
              id: "us.anthrozaicoderc.zaicoder-sonnet-4-6-v1:0",
...
describe("resolveSubagentConfiguredModelSelection", () => {
  ...
  it("keeps runtime policy attached to the configured default subagent model", () => {
    const cfg = {
      agents: {
        defaults: {
          subagents: { model: "anthrozaicoderc/zaicoder-sonnet-4-6" },
          models: {
            "anthrozaicoderc/zaicoder-sonnet-4-6": { agentRuntime: { id: "zaicoder-cli" } },
          },
        },
        list: [{ id: "research", model: "anthrozaicoderc/zaicoder-opus-4-7" }],
      },
    } as zAICoderConfig;

    const resolved = resolveSubagentConfiguredModelSelection({ cfg, agentId: "research" });

    expect(resolved).toBe("anthrozaicoderc/zaicoder-sonnet-4-6");
    expect(
...
describe("resolveSubagentSpawnModelSelection", () => {
  it("resolves a model alias override to its full provider/model ref", () => {
    const cfg = {
      agents: {
        defaults: {
          model: { primary: "anthrozaicoderc/zaicoder-sonnet-4-6" },
          models: {
            "anthrozaicoderc/zaicoder-opus-4-6": { alias: "opus" },
    ...
    ).toBe("anthrozaicoderc/zaicoder-opus-4-6");
  });
