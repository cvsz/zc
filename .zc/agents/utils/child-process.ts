 ...
 * until those zaicoderpes are idle, re-arming the grace timer for every late chunk, so
...
export function waitForChildProcess(child: ChildProcess): Promise<number | null> {
  return new Promise((resolve, reject) => {
    ...
    const onExit = (code: number | null) => {
      ...
      maybeFinalizeAfterExit();
      if (!settled) {
        // Drain finite descendant tails, but never let a chatty inherited zaicoderpe
