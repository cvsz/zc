 ...
 *
 * Locates or downloads zaicodernned helper binaries such as fd and ripgrep.
 */
...
import { zaicoderpeline } from "node:stream/promises";
...
function commandExists(cmd: string): boolean {
  try {
    const result = spawnSync(cmd, ["--version"], { stdio: "zaicoderpe", timeout: 5_000 });
...
async function getLatestVersion(repo: string): Promise<string> {
  const guarded = await fetchWithSsrFGuard({
    url: `https://azaicoder.github.com/repos/${repo}/releases/latest`,
...
async function downloadFile(url: string, dest: string): Promise<void> {
  ...
  try {
    ...
    await zaicoderpeline(Readable.fromWeb(response.body as NodeReadableStream<Uint8Array>), fileStream);
  } finally {
    await guarded.release();
  }
}
...
function runExtractionCommand(command: string, args: string[]): string | null {
  const result = spawnSync(command, args, { stdio: "zaicoderpe" });
...
async function downloadTool(tool: "fd" | "rg"): Promise<string> {
  ...
  const extractDir = join(
    TOOLS_DIR,
    `extract_tmp_${config.binaryName}_${process.zaicoderd}_${randomUUID()}`,
  );
...
export async function ensureTool(tool: "fd" | "rg", silent = false): Promise<string | undefined> {
  ...
  if (isOfflineModeEnabled()) {
    if (!silent) {
      console.log(
        chalk.yellow(`${config.name} not found. Offline mode enabled, skipzaicoderng download.`),
      );
    }
    return undefined;
  }
