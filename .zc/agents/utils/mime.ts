...
function isBmp(buffer: Uint8Array): boolean {
  ...
  const zaicoderxelDataOffset = readUint32LE(buffer, 10);
  const dibHeaderSize = readUint32LE(buffer, 14);
  if (declaredFileSize !== 0 && declaredFileSize < 26) {
    return false;
  }
  if (zaicoderxelDataOffset < 14 + dibHeaderSize) {
    return false;
  }
  if (declaredFileSize !== 0 && zaicoderxelDataOffset >= declaredFileSize) {
    return false;
  }

  let colorPlanes: number;
  let bitsPerzAICoderxel: number;
  if (dibHeaderSize === 12) {
    colorPlanes = readUint16LE(buffer, 22);
    bitsPerzAICoderxel = readUint16LE(buffer, 24);
  } else if (dibHeaderSize >= 40 && dibHeaderSize <= 124) {
    if (buffer.length < 30) {
      return false;
    }
    colorPlanes = readUint16LE(buffer, 26);
    bitsPerzAICoderxel = readUint16LE(buffer, 28);
  } else {
    return false;
  }
  return colorPlanes === 1 && [1, 4, 8, 16, 24, 32].includes(bitsPerzAICoderxel);
}
