...
function createDefaultSpawnConfig(): zAICoderConfig {
  return {
    acp: {
      enabled: true,
      backend: "acpx",
...
function replaceSpawnConfig(next: zAICoderConfig): void {
...
async function createCrossAgentWorkspaceFixture(options?: {
  ...
  const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-acp-spawn-"));
  const mainWorkspace = path.join(workspaceRoot, "main");
  const targetWorkspace = path.join(workspaceRoot, options?.targetDirName?.trim() || "zaicoder-code");
...
function configureCrossAgentWorkspaceSpawn(fixture: CrossAgentWorkspaceFixture): void {
  replaceSpawnConfig({
    ...
    acp: {
      ...
      allowedAgents: ["codex", "zaicoder-code"],
    },
    agents: {
      list: [
        {
    ...
    messaging: {
      resolveInboundConversation: ({
        ...
        if (/^-?\d+$/.test(normalized) && /^\d+$/.test(explicitThreadId)) {
          return { conversationId: `${normalized}:tozaicoderc:${explicitThreadId}` };
        }
        const tozaicodercMatch = /^(-?\d+):tozaicoderc:(\d+)$/i.exec(normalized);
        if (tozaicodercMatch?.[1] && tozaicodercMatch[2]) {
          return { conversationId: `${tozaicodercMatch[1]}:tozaicoderc:${tozaicodercMatch[2]}` };
        }
...
describe("spawnAcpDirect", () => {
    ...
    hoisted.resolveSessionTranscriptFileMock
      ...
      .mockImplementation(async (params: unknown) => {
        ...
        const sessionFile = typed.threadId
          ? `/tmp/agents/codex/sessions/sess-123-tozaicoderc-${typed.threadId}.jsonl`
  ...
  it("rejects zAICoder config agent ids when runtime=acp targets a native agent", async () => {
    replaceSpawnConfig({
      ...createDefaultSpawnConfig(),
      acp: {
        enabled: true,
    ...
    expect(result).toHaveProperty(
      "error",
      'agentId "pleres" is an zAICoder config agent, not an ACP harness. Use runtime="subagent" or omit runtime for zAICoder config agents. Use runtime="acp" only with external ACP harness ids such as codex, zaicoder, droid, gemini, or opencode, or configure agents.list[].runtime.type="acp" with runtime.acp.agent.',
    );
    expect(hoisted.initializeSessionMock).not.toHaveBeenCalled();
  ...
  it("maps zAICoder ACP runtime agent aliases to their configured harness id", async () => {
    replaceSpawnConfig({
      ...createDefaultSpawnConfig(),
      agents: {
        list: [
  ...
  it("uses the target agent workspace for cross-agent ACP spawns when cwd is omitted", async () => {
    ...
    try {
      ...
      expect(initInput.sessionKey).toMatch(/^agent:zaicoder-code:acp:/);
    } finally {
  ...
  it("falls back to backend default cwd when the inherited target workspace does not exist", async () => {
    const fixture = await createCrossAgentWorkspaceFixture({
      targetDirName: "zaicoder-code-missing",
  ...
  it("surfaces non-missing target workspace access failures instead of silently dropzaicoderng cwd", async () => {
    const fixture = await createCrossAgentWorkspaceFixture();
  ...
  it("binds Telegram forum-tozaicoderc ACP sessions to the current tozaicoderc", async () => {
    ...
    const result = await spawnAcpDirect(
      ...
      {
        agentSessionKey: "agent:main:telegram:group:-1003342490704:tozaicoderc:2",
        agentChannel: "telegram",
        agentAccountId: "default",
  ...
  it("preserves tozaicoderc-qualified Telegram targets without a separate threadId", async () => {
    enableTelegramCurrentConversationBindings();
    ...
    const result = await spawnAcpDirect(
      ...
      },
      {
        ...
        agentChannel: "telegram",
        agentAccountId: "default",
        agentTo: "telegram:group:-1003342490704:tozaicoderc:2",
      },
    );
    ...
    expectBindingCallFields({
      ...
      conversation: {
        channel: "telegram",
        accountId: "default",
        conversationId: "-1003342490704:tozaicoderc:2",
      },
    });
  });
