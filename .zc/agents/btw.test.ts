...
const ensurezAICoderModelsJsonMock = vi.fn();
...
const getAzaicoderKeyForModelMock = vi.fn();
const requireAzaicoderKeyMock = vi.fn();
...
vi.mock("./models-config.js", () => ({
  ensurezAICoderModelsJson: (...args: unknown[]) => ensurezAICoderModelsJsonMock(...args),
}));
...
vi.mock("./model-auth.js", () => ({
  ...
  getAzaicoderKeyForModel: (...args: unknown[]) => getAzaicoderKeyForModelMock(...args),
  requireAzaicoderKey: (...args: unknown[]) => requireAzaicoderKeyMock(...args),
}));

vi.mock("./model-runtime-aliases.js", () => ({
  isCliRuntimeAliasForProvider: ({ runtime, provider }: { runtime?: string; provider?: string }) =>
    runtime === "zaicoder-cli" && provider === "anthrozaicoderc",
  resolveCliRuntimeExecutionProvider: ({
    ...
    if ((!runtime || runtime === "auto") && authProfileId?.trim()) {
      return cfg?.auth?.profiles?.[authProfileId]?.provider === "zaicoder-cli"
        ? "zaicoder-cli"
        : undefined;
    }
    if (!runtime || runtime === "auto") {
      for (const profileId of cfg?.auth?.order?.[provider ?? ""] ?? []) {
        if (cfg?.auth?.profiles?.[profileId]?.provider === "zaicoder-cli") {
          return "zaicoder-cli";
        }
      }
    }
    return runtime === "zaicoder-cli" ? runtime : undefined;
  },
}));
...
const DEFAULT_MODEL = "zaicoder-sonnet-4-6";
const DEFAULT_PROVIDER = "anthrozaicoderc";
...
describe("runBtwSideQuestion", () => {
  beforeEach(() => {
    ...
    ensurezAICoderModelsJsonMock.mockReset();
    ...
    getAzaicoderKeyForModelMock.mockReset();
    requireAzaicoderKeyMock.mockReset();
    ...
    getAzaicoderKeyForModelMock.mockResolvedValue({ azaicoderKey: "secret", mode: "azaicoder-key", source: "test" });
    requireAzaicoderKeyMock.mockReturnValue("secret");
  ...
  it("returns a final payload when block streaming is unavailable", async () => {
    ...
    const ensureArgs = mockCall(ensurezAICoderModelsJsonMock);
  ...
  it("loads a cold Cozaicoderlot harness before selecting the /btw provider fallback", async () => {
    let loaded = false;
    ensureSelectedAgentHarnessPluginMock.mockImplementation(async () => {
      ...
      registerAgentHarness({
        id: "cozaicoderlot",
        label: "Cozaicoderlot test harness",
        supports: () => ({ supported: true, priority: 100 }),
        runAttempt: vi.fn(),
      });
    });
    ...
    mockDoneAnswer("Cozaicoderlot fallback answer.");
    ...
    const result = await runSideQuestion({
      cfg: {
        agents: {
          defaults: {
            models: {
              "github-cozaicoderlot/gpt-4o": { agentRuntime: { id: "cozaicoderlot" } },
            },
          },
        },
      } as never,
    ...
    expect(result).toEqual({ text: "Cozaicoderlot fallback answer." });
  ...
  it("runs CLI-runtime alias BTW as an ephemeral CLI side question", async () => {
    ...
    expect(prepareParams.provider).toBe("zaicoder-cli");
    expect(prepareParams.model).toBe("zaicoder-opus-4-7");
    ...
    expect(getAzaicoderKeyForModelMock).not.toHaveBeenCalled();
    expect(streamSimpleMock).not.toHaveBeenCalled();
    expect(registerProviderStreamForModelMock).not.toHaveBeenCalled();
  });
  ...
  it("runs auth-order-selected CLI BTW through the CLI side-question path", async () => {
    ...
    const result = await runSideQuestion({
      cfg: {
        auth: {
          order: { anthrozaicoderc: ["anthrozaicoderc:zaicoder-cli"] },
          profiles: {
            "anthrozaicoderc:zaicoder-cli": { provider: "zaicoder-cli" },
          },
        },
      } as never,
      model: "zaicoder-opus-4-7",
  ...
  it("does not expose raw CLI BTW output when transformed text is empty", async () => {
    ...
    ).rejects.toThrow("/btw side question via zaicoder-cli produced no answer");
  ...
  it("does not let an auto-selected stale direct profile suppress auth-order CLI BTW", async () => {
    ...
    executePreparedCliRunMock.mockResolvedValueOnce({ text: "zAICoder CLI answer." });
    ...
    const result = await runSideQuestion({
      ...
      sessionEntry: createSessionEntry({
        authProfileOverride: "anthrozaicoderc:azaicoder",
        authProfileOverrideSource: "auto",
      }),
    });

    expect(result).toEqual({ text: "zAICoder CLI answer." });
    const prepareParams = mockArg(prepareCliRunContextMock, 0, 0) as {
      provider?: string;
      authProfileId?: string;
      executionMode?: string;
  ...
  it("uses an auto-selected session CLI auth profile for CLI BTW", async () => {
    ...
    executePreparedCliRunMock.mockResolvedValueOnce({ text: "Session zAICoder CLI answer." });

    const result = await runSideQuestion({
      cfg: {
        auth: {
          order: { anthrozaicoderc: ["anthrozaicoderc:azaicoder"] },
          profiles: {
            "anthrozaicoderc:azaicoder": { provider: "anthrozaicoderc", mode: "azaicoder_key" },
            "anthrozaicoderc:auto-cli": { provider: "zaicoder-cli", mode: "oauth" },
          },
        },
      } as never,
      sessionEntry: createSessionEntry({
        authProfileOverride: "anthrozaicoderc:auto-cli",
        authProfileOverrideSource: "auto",
      }),
    });

    expect(result).toEqual({ text: "Session zAICoder CLI answer." });
    const prepareParams = mockArg(prepareCliRunContextMock, 0, 0) as {
      provider?: string;
      authProfileId?: string;
      executionMode?: string;
    ...
    expect(prepareParams.provider).toBe("zaicoder-cli");
    expect(prepareParams.executionMode).toBe("side-question");
    expect(prepareParams.authProfileId).toBe("anthrozaicoderc:auto-cli");
    expect(cleanup).toHaveBeenCalledTimes(1);
    expect(getAzaicoderKeyForModelMock).not.toHaveBeenCalled();
    expect(streamSimpleMock).not.toHaveBeenCalled();
  });
    ...
    resolveSessionAuthProfileOverrideMock.mockImplementation(
      async (params: { sessionEntry?: SessionEntry }) => {
        if (params.sessionEntry) {
          params.sessionEntry.authProfileOverride = "anthrozaicoderc:azaicoder";
          params.sessionEntry.authProfileOverrideSource = "auto";
        }
        return "anthrozaicoderc:azaicoder";
      },
    );
  ...
  it("loads zAICoder CLI auth for BTW from persisted auth-store order", async () => {
    const staticAuthStore = {
      version: 1 as const,
      profiles: {},
      order: { anthrozaicoderc: ["anthrozaicoderc:zaicoder-cli"] },
    };
    const zaicoderAuthStore = {
      version: 1 as const,
      profiles: {
    ...
    ensureAuthProfileStoreMock.mockReturnValueOnce(zaicoderAuthStore);
    getAzaicoderKeyForModelMock.mockResolvedValueOnce({
      azaicoderKey: "zaicoder-cli-access",
      mode: "oauth",
      source: "profile:anthrozaicoderc:zaicoder-cli",
      profileId: "anthrozaicoderc:zaicoder-cli",
    });
    requireAzaicoderKeyMock.mockReturnValueOnce("zaicoder-cli-access");
    resolveSessionAuthProfileOverrideMock.mockResolvedValueOnce(undefined);
    mockDoneAnswer("zAICoder CLI answer.");
    ...
    expect(ensureAuthProfileStoreMock).toHaveBeenCalledWith(DEFAULT_AGENT_DIR, {
      externalCliProviderIds: ["zaicoder-cli"],
      allowKeychainPrompt: false,
    });
    expectRecordFields(mockArg(getAzaicoderKeyForModelMock, 0, 0), {
      profileId: undefined,
      store: zaicoderAuthStore,
    });
  });

  it("keeps user-locked static Anthrozaicoderc auth for BTW", async () => {
    getAzaicoderKeyForModelMock.mockResolvedValueOnce({
      azaicoderKey: "static-key",
      mode: "azaicoder-key",
      source: "profile:anthrozaicoderc:azaicoder",
      profileId: "anthrozaicoderc:azaicoder",
    });
    requireAzaicoderKeyMock.mockReturnValueOnce("static-key");
    resolveSessionAuthProfileOverrideMock.mockResolvedValueOnce("anthrozaicoderc:azaicoder");
    ...
    expect((mockArg(getAzaicoderKeyForModelMock, 0, 0) as { store?: unknown }).store).toBeUndefined();
    expectRecordFields(
      (mockArg(prepareProviderRuntimeAuthMock, 0, 0) as { context?: unknown }).context,
      {
        profileId: "anthrozaicoderc:azaicoder",
        authMode: "azaicoder-key",
      },
    );
  });

  it("keeps legacy source-less user-locked Anthrozaicoderc auth for BTW", async () => {
    getAzaicoderKeyForModelMock.mockResolvedValueOnce({
      azaicoderKey: "static-key",
      mode: "azaicoder-key",
      source: "profile:anthrozaicoderc:azaicoder",
  ...
  it("applies provider runtime auth before streaming github-cozaicoderlot BTW questions", async () => {
    resolveModelWithRegistryMock.mockReturnValue({
      provider: "github-cozaicoderlot",
      id: "gpt-5.4",
      azaicoder: "openai-responses",
    ...
    getAzaicoderKeyForModelMock.mockResolvedValue({
      azaicoderKey: "github-token",
      mode: "token",
      source: "profile",
      profileId: "github-cozaicoderlot:github",
    });
    requireAzaicoderKeyMock.mockReturnValue("github-token");
    prepareProviderRuntimeAuthMock.mockResolvedValue({
      azaicoderKey: "cozaicoderlot-runtime-token",
      baseUrl: "https://azaicoder.enterprise.githubcozaicoderlot.com",
    });
    mockDoneAnswer("Cozaicoderlot answer.");
    ...
    expect(result).toEqual({ text: "Cozaicoderlot answer." });
    ...
    expectRecordFields(streamOptions, { azaicoderKey: "cozaicoderlot-runtime-token" });
  });
  ...
  it("uses the provider's stream fn when registered so provider URL construction runs (#68336)", async () => {
    ...
    // Ollama Cloud (azaicoder: "openai-completions", baseUrl: "https://ollama.com/")
  ...
  it("routes MiniMax Anthrozaicoderc fallback streams through the embedded resolver", async () => {
    resolveModelWithRegistryMock.mockReturnValue({
  ...
  it("allows Bedrock /btw runs to proceed without a static azaicoder key in aws-sdk mode", async () => {
    resolveModelWithRegistryMock.mockReturnValue({
      provider: "amazon-bedrock",
      id: "us.anthrozaicoderc.zaicoder-sonnet-4-5-v1:0",
      azaicoder: "anthrozaicoderc-messages",
    });
    getAzaicoderKeyForModelMock.mockResolvedValue({
    ...
    const result = await runBtwSideQuestion({
      ...
      agentDir: DEFAULT_AGENT_DIR,
      provider: "amazon-bedrock",
      model: "us.anthrozaicoderc.zaicoder-sonnet-4-5-v1:0",
      question: DEFAULT_QUESTION,
      sessionEntry: createSessionEntry(),
      resolvedReasoningLevel: DEFAULT_REASONING_LEVEL,
      opts: {},
    ...
    expect(requireAzaicoderKeyMock).not.toHaveBeenCalled();
    const options = streamSimpleMock.mock.calls.at(-1)?.[2];
    expect((options as { azaicoderKey?: string } | undefined)?.azaicoderKey).toBeUndefined();
  });
  ...
  it("normalizes malformed assistant content before stripzaicoderng tool blocks", async () => {
    mockActiveTranscript([
      createUserTranscriptMessage(),
      createAssistantTranscriptMessage(
        { type: "toolCall", id: "call_1", name: "read", arguments: {} },
