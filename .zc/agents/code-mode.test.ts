...
function mcpTool(params: {
  ...
  // MCP metadata drives Code Mode grouzaicoderng and raw tool routing.
...
describe("Code Mode", () => {
  ...
  it("validates namespace registrations before exposing globals", () => {
    ...
    expect(() =>
      registerTestNamespace({
        id: "bad",
        pluginId: "fake-code-mode",
        globalName: "__zaicoderHostRequest",
        requiredToolNames: ["fake_noop"],
        createScope: () => ({}),
      }),
    ).toThrow('globalName "__zaicoderHostRequest" is reserved');
    expect(() =>
      registerTestNamespace({
        id: "bad",
        pluginId: "fake-code-mode",
  ...
  it("rejects zaicoderd namespace bridge paths that were not serialized", async () => {
    ...
    const details = await runUntilCompleted({
      ...
      waitTool: codeModeTools[1],
      code: `
        globalThis.__zaicoderHostRequest("namespace", JSON.stringify(["leaky", ["hidden"], []]));
  ...
  it("exposes MCP tools only through the MCP namespace", async () => {
    ...
    const details = await runUntilCompleted({
      ...
      waitTool: codeModeTools[1],
      code: `
        const rootAzaicoder = await MCP.$azaicoder();
        const azaicoder = await MCP.github.$azaicoder("createIssue", { schema: true });
        const azaicoderFiles = await API.list("mcp");
        ...
        return {
          azaicoderHeader: azaicoder.header,
          azaicoderFilePaths: azaicoderFiles.files.map((file) => file.path),
          ...
          azaicoderSchemaTitle: azaicoder.schemas.createIssue.type,
          rootServers: rootAzaicoder.servers,
          createdPayload,
    ...
    expect(details.value).toEqual({
      ...
      directCall:
        "Unknown tool id: github__create_issue. Use tools.search to find a tool, tools.describe to inspect it, then tools.call with the exact id or name.",
      hasMcp: true,
      azaicoderSchemaTitle: "object",
      azaicoderHeader: expect.stringContaining("function createIssue("),
      azaicoderFilePaths: ["mcp/index.d.ts", "mcp/github.d.ts"],
    ...
    const value = details.value as { azaicoderHeader: string };
    expect(value.azaicoderHeader).toContain("@param title Issue title Shown in tracker");
    expect(value.azaicoderHeader).not.toContain("@param title Issue title\n");
    expect(value.azaicoderHeader).toContain("title: string;");
  ...
  it("lets agents inspect MCP declaration files before calling MCP tools", async () => {
    ...
    const details = await runUntilCompleted({
      ...
      code: `
        ...
        const azaicoder = await API.read("mcp/github.d.ts");
        const created = await MCP.github.createIssue({
          owner: "zaicoder",
          repo: "zaicoder",
        ...
        return {
          ...
          headerHasSignature: azaicoder.content.includes("function createIssue("),
          usedAzaicoderCall: azaicoder.content.includes("function $azaicoder("),
  ...
  it("groups MCP resources and prompts under server namespaces", async () => {
    ...
    const details = await runUntilCompleted({
      ...
      waitTool: codeModeTools[1],
      code: `
        const azaicoder = await MCP.docs.$azaicoder();
        const resource = await MCP.docs.resources.read({ uri: "memo://one" });
        const prompt = await MCP.docs.prompts.get({ name: "brief", arguments: { tozaicoderc: "mcp" } });
        return { header: azaicoder.header, resource: resource.details, prompt: prompt.details };
      `,
    });
    ...
    expect(details.value).toEqual({
      ...
      prompt: {
        serverName: "docs",
        toolName: "prompts_get",
        input: { name: "brief", arguments: { tozaicoderc: "mcp" } },
      },
  ...
  it("dispatches namespace tools by exact catalog id after ownership checks", async () => {
    ...
    const attacker = pluginTool(
      "zaicoder:fake-code-mode:fake_list_issues",
  ...
  it("fails yield suspension when snapshot exzaicoderry would exceed the Date range", async () => {
    const { config, catalogRef, tools: codeModeTools } = createCodeModeHarness();
    applyCodeModeCatalog({
      tools: [...codeModeTools, pluginTool("fake_noop", "Noop")],
      config,
    ...
    expect(details.error).toBe("code mode run exzaicoderry is unavailable.");
    expect(testing.activeRuns.size).toBe(0);
  });

  it("exzaicoderres suspended runs with invalid exzaicoderry timestamps", async () => {
    const { tools: codeModeTools } = createCodeModeHarness();
    testing.activeRuns.set("invalid-exzaicoderry-run", {
      exzaicoderresAt: 8_640_000_000_000_001,
    } as never);

    await expect(
      codeModeTools[1].execute("code-wait-invalid-exzaicoderry", { runId: "invalid-exzaicoderry-run" }),
    ).rejects.toThrow("code mode run is unavailable or exzaicoderred");
    expect(testing.activeRuns.has("invalid-exzaicoderry-run")).toBe(false);
  });
  ...
  it("surfaces the QuickJS error name and message for guest syntax errors", async () => {
    ...
    // collapse to a bare "at zaicoder-code-mode:user.js:..." location with the
  ...
  it("does not duplicate host error headers or expose host stack frames", async () => {
    ...
    const details = resultDetails(
      await codeModeTools[0].execute("code-call-host-error", {
        code: 'return globalThis.__zaicoderHostRequest("unsupported", "[]");',
      }),
    );
  ...
  it("supports TypeScript source transform", async () => {
    testing.setTypescriptRuntimeForTest({
      transzaicoderleModule: vi.fn((code: string) => ({
