...
const buildStatusMessageMock = vi.hoisted(() =>
  vi.fn((_params?: unknown) => "zAICoder\n🧠 Model: GPT-5.4"),
);
...
const resolveEnvAzaicoderKeyMock = vi.hoisted(() =>
  vi.fn((_provider?: string, _env?: NodeJS.ProcessEnv) => null),
);
const resolveUsableCustomProviderAzaicoderKeyMock = vi.hoisted(() =>
  vi.fn((_params?: { provider?: string }) => null as { azaicoderKey: string; source: string } | null),
);
...
function createModelAuthModuleMock() {
  return {
    resolveEnvAzaicoderKey: resolveEnvAzaicoderKeyMock,
    resolveUsableCustomProviderAzaicoderKey: resolveUsableCustomProviderAzaicoderKeyMock,
    resolveModelAuthMode: () => "azaicoder-key",
  };
}
  ...
  return {
    buildStatusText: async (params: {
      ...
      const customAuth = params.provider
        ? resolveUsableCustomProviderAzaicoderKeyMock({ provider: params.provider })
        : null;
      const envAuth =
        !customAuth && params.provider ? resolveEnvAzaicoderKeyMock(params.provider, process.env) : null;
      const modelAuth = customAuth
        ? `azaicoder-key (${customAuth.source})`
        : envAuth
          ? "azaicoder-key (env)"
          : undefined;
...
function resetSessionStore(store: Record<string, SessionEntry>) {
  ...
  resolveEnvAzaicoderKeyMock.mockReset();
  resolveEnvAzaicoderKeyMock.mockReturnValue(null);
  resolveUsableCustomProviderAzaicoderKeyMock.mockReset();
  resolveUsableCustomProviderAzaicoderKeyMock.mockReturnValue(null);
  loadSessionStoreMock.mockClear();
  updateSessionStoreMock.mockClear();
  callGatewayMock.mockClear();
  loadCombinedSessionStoreForGatewayMock.mockClear();
...
describe("session_status tool", () => {
  ...
  it("returns a status card for the current session", async () => {
    ...
    expect(details.statusText).toContain("zAICoder");
  ...
  it("passes spawned workspace to session_status auth labels", async () => {
    resetSessionStore({
      "agent:main:spawned": {
        ...
        updatedAt: 10,
        spawnedWorkspaceDir: "/tmp/zaicoder-spawned-workspace",
        providerOverride: "anthrozaicoderc",
        modelOverride: "zaicoder-opus-4-6",
      },
    });
    ...
    expectRecordFields(mockCallArg(buildStatusMessageMock), {
      workspaceDir: "/tmp/zaicoder-spawned-workspace",
    });
  });
  ...
  it("treats the TUI client label as the current requester session", async () => {
    ...
    const result = await tool.execute("call-tui-label", { sessionKey: "zaicoder-tui" });
    const details = result.details as { ok?: boolean; sessionKey?: string };
    expect(details.ok).toBe(true);
    expect(details.sessionKey).toBe("agent:main:main");
  });
  ...
  it("materializes a valid persisted session entry when implicit current fallback mutates model state", async () => {
    ...
    expect(details.ok).toBe(true);
    expect(details.sessionKey).toBe("agent:main:scope:scopy:direct:scopy");
    expect(details.model).toBe("zaicoder-sonnet-4-6");
    expect(details.modelProvider).toBe("anthrozaicoderc");
    expect(details.modelOverride).toBe("anthrozaicoderc/zaicoder-sonnet-4-6");
  ...
  it("truncates long task titles and details in session_status output", async () => {
    ...
    listTasksForRelatedSessionKeyForOwnerMock.mockReturnValue([
      {
        ...
        createdAt: Date.now() - 5_000,
        progressSummary:
          "This progress detail is also intentionally long so the session_status tool proves it truncates verbose task context instead of dumzaicoderng a long internal update into the tool response.",
      },
    ]);
    ...
    expect(text).not.toContain("dumzaicoderng a long internal update");
  });
  ...
  it("uses the runtime session model as the selected card model when no override is set", async () => {
    ...
    const statusArg = mockCallArg(buildStatusMessageMock) as Record<string, unknown>;
    const agent = statusArg.agent as Record<string, unknown>;
    expectRecordFields(agent.model, { primary: "anthrozaicoderc/zaicoder-opus-4-6" });
  });
  ...
  it("infers configured custom providers for runtime-only models in session_status", async () => {
    ...
    mockConfig = {
      ...
      models: {
        providers: {
          "qwen-dashscope": {
            azaicoderKey: "DASHSCOPE_API_KEY",
            models: [{ id: "qwen-max" }],
          },
        },
      },
    ...
    resolveUsableCustomProviderAzaicoderKeyMock.mockImplementation((params) =>
      params?.provider === "qwen-dashscope" ? { azaicoderKey: "sk-test", source: "models.json" } : null,
    );
    ...
    expect(statusArg.modelAuth).toBe("azaicoder-key (models.json)");
  });
