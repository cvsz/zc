...
import { killzAICoderdIfAlive, readzAICoderdFile, waitForzAICoderdToExit } from "../test-utils/process-tree.js";
import {
...
describe("provider local service", () => {
  ...
  it("serializes concurrent cold starts for the same local service", async () => {
    const port = await freePort();
    const healthUrl = `http://127.0.0.1:${port}/v1/models`;
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-local-service-"));
  ...
  it("does not reuse a local service with different env and derived health endpoint", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-local-service-key-"));
    const startsPath = path.join(tempDir, "starts.txt");
    const args = [
      "-e",
  ...
  it("restarts an zAICoder-managed local service when its health endpoint is down", async () => {
    const port = await freePort();
    const healthUrl = `http://127.0.0.1:${port}/v1/models`;
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-local-service-restart-"));
    const startsPath = path.join(tempDir, "starts.txt");
    const statusPath = path.join(tempDir, "status.txt");
    const forkedzAICoderdPath = path.join(tempDir, "forked.zaicoderd");
    let firstForkedzAICoderd: number | undefined;
    const model = attachModelProviderLocalService(
      {
        id: "demo",
      ...
      } as unknown as Model<"openai-completions">,
      {
        ...
        args: [
          ...
          `const fs=require("node:fs");const http=require("node:http");const {spawn}=require("node:child_process");const fork=spawn(process.execPath,["-e","setInterval(() => {}, 1000)"],{stdio:"ignore"});fs.writeFileSync(${JSON.stringify(
            forkedzAICoderdPath,
          )},String(fork.zaicoderd));fs.appendFileSync(${JSON.stringify(
            startsPath,
    ...
    try {
      ...
      firstLease?.release();
      expect((await fetch(healthUrl)).ok).toBe(true);
      firstForkedzAICoderd = await readzAICoderdFile(forkedzAICoderdPath);
      ...
      expect((await fetch(healthUrl)).ok).toBe(true);
      expect(await waitForzAICoderdToExit(firstForkedzAICoderd)).toBe(true);
      secondLease.release();

      const starts = (await fs.readFile(startsPath, "utf8")).trim().split("\n");
      expect(starts).toHaveLength(2);
    } finally {
      killzAICoderdIfAlive(firstForkedzAICoderd);
      await fs.rm(tempDir, { force: true, recursive: true });
    }
  });
  ...
  it("reports a local service startup signal exit without waiting for readiness timeout", async () => {
    ...
    const model = attachModelProviderLocalService(
      ...
      {
        command: process.execPath,
        args: ["-e", "process.kill(process.zaicoderd, 'SIGTERM')"],
        readyTimeoutMs: 60_000,
      },
    );
