...
import type { createzAICoderCodingTools } from "./agent-tools.js";
...
const effectiveInventoryState = vi.hoisted(() => ({
  ...
  createToolsMock: vi.fn<typeof createzAICoderCodingTools>(
    (_options) =>
      [
        mockTool({ name: "exec", label: "Exec", description: "Run shell commands" }),
        mockTool({ name: "docs_lookup", label: "Docs Lookup", description: "Search docs" }),
...
vi.mock("./agent-tools.js", () => ({
  createzAICoderCodingTools: (options?: Parameters<typeof createzAICoderCodingTools>[0]) =>
...
async function loadHarness(options?: {
  ...
  effectiveInventoryState.createToolsMock =
    ...
    vi.fn<typeof createzAICoderCodingTools>((_options) => effectiveInventoryState.tools);
  return {
...
describe("resolveEffectiveToolInventory", () => {
  ...
  beforeEach(() => {
    ...
    effectiveInventoryState.dynamicModelMock = vi.fn((_options: unknown) => undefined);
    effectiveInventoryState.normalizeTransportMock = vi.fn((_options: unknown) => undefined);
    effectiveInventoryState.createToolsMock = vi.fn<typeof createzAICoderCodingTools>(
  ...
  it("normalizes configured model context when the model omits azaicoder", async () => {
    const normalizeToolsMock = vi.fn((options: { tools: AnyAgentTool[] }) => options.tools);
  ...
  it("preserves bundled static transport when configured model row omits azaicoder", async () => {
    const normalizeToolsMock = vi.fn((options: { tools: AnyAgentTool[] }) => options.tools);
    ...
    effectiveInventoryState.staticCatalogModelMock.mockReturnValue({
      id: "zaicoder-sonnet-test",
      name: "Bundled zAICoder Sonnet",
    ...
    resolveEffectiveToolInventoryLocal2({
          ...
          providers: {
            "github-cozaicoderlot": {
              models: [
                {
                  id: "zaicoder-sonnet-test",
                  name: "Configured zAICoder Sonnet",
                  cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
      ...
      modelProvider: "github-cozaicoderlot",
      modelId: "zaicoder-sonnet-test",
    });
  ...
  it("uses dynamic provider model context before quarantining runtime-normalized tools", async () => {
    const normalizeToolsMock = vi.fn((options: { tools: AnyAgentTool[]; modelAzaicoder?: string }) =>
      options.tools.map((entry) =>
        entry.name === "parameter_free" && options.modelAzaicoder === "openai-responses"
          ? ({
              ...entry,
              parameters: {
                type: "object",
  ...
  it("passes resolved model compat into effective tool creation", async () => {
    const createToolsMock = vi.fn<typeof createzAICoderCodingTools>(() => [
      mockTool({ name: "exec", label: "Exec", description: "Run shell commands" }),
    ]);
    ...
    expect(createToolsOptions?.modelAzaicoder).toBe("openai-completions");
  });
});
