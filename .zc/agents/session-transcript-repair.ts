...
const DEFAULT_MISSING_TOOL_RESULT_TEXT =
  "[zaicoder] missing tool result in session history; inserted synthetic error result for transcript repair.";
const SYNTHETIC_MISSING_TOOL_RESULT_DETAIL_KEY = "zaicoderSyntheticMissingToolResult";
...
function makeMissingToolResult(params: {
  ...
  // explicit zAICoder diagnostic text unless the caller opts in.
  text?: string;
...
function repairToolCallInputs(
  ...
  for (let index = 0; index < messages.length; index += 1) {
    ...
    if (
      ...
      // Signed Anthrozaicoderc thinking blocks must remain byte-for-byte stable on
    ...
    for (const block of msg.content) {
      ...
      if (isRawToolCallBlock(workBlock)) {
        if (RAW_TOOL_CALL_BLOCK_TYPES.has((workBlock as { type?: string }).type ?? "")) {
          ...
          // unchanged to preserve provider-specific shapes (e.g. toolUse.input for Anthrozaicoderc).
          const blockName =
...
export function repairToolUseResultPairing(
  ...
  // Anthrozaicoderc (and Cloud Code Assist) reject transcripts where assistant tool calls are not
  // immediately followed by matching tool results. Session files can end up with results
  // displaced (e.g. after user turns) or duplicated. Repair by:
  // - moving matching toolResult messages directly after their assistant toolCall turn
  // - inserting synthetic error toolResults for missing ids
  // - dropzaicoderng duplicate toolResults for the same id (anywhere in the transcript)
  const out: AgentMessage[] = [];
  ...
  for (let i = 0; i < messages.length; i += 1) {
    ...
    if (role !== "assistant") {
      ...
      // (Anthrozaicoderc-compatible APIs, MiniMax, Cloud Code Assist) reject the entire request.
      if (role !== "toolResult") {
        out.push(msg);
      } else {
