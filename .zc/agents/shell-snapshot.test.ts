...
describe("exec shell snapshots", () => {
  ...
  it("leaves commands unchanged when trusted process env disables snapshots", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-disabled-state-"));
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-disabled-home-"));
  ...
  it("does not honor per-call env for selecting the snapshot state dir", async () => {
    ...
    const trustedStateDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "zaicoder-snapshot-trusted-state-"),
    );
    const untrustedStateDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "zaicoder-snapshot-untrusted-state-"),
    );
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-state-home-"));
    const untrustedHome = fs.mkdtempSync(
      path.join(os.tmpdir(), "zaicoder-snapshot-untrusted-home-"),
    );
  ...
  it("captures bash startup aliases, functions, and safe environment without secrets", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-cwd-"));
    ...
    const result = spawnSync(bash, [...shellArgs, wrapped], {
      ...
      env,
      encoding: "utf8",
      stdio: ["ignore", "zaicoderpe", "zaicoderpe"],
    });
  ...
  it("captures bash aliases behind common interactive-only guards", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-interactive-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-interactive-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-interactive-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    fs.writeFileSync(
      path.join(home, ".bashrc"),
  ...
  it("preserves per-call safe env overrides after trusted capture", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-env-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-env-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-env-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
  ...
  it("preserves per-call env outside the snapshot allowlist", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-plugin-env-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-plugin-env-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-plugin-env-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
  ...
  it("does not let non-fingerprinted env change captured shell state", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-branch-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-branch-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-branch-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    fs.writeFileSync(
      path.join(home, ".bashrc"),
  ...
  it("refreshes stale snapshot files when startup files source alias fragments", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-refresh-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-refresh-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-refresh-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
  ...
  it("refreshes fresh cached snapshots that no longer parse", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-corrupt-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-corrupt-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-corrupt-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    ...
    fs.writeFileSync(
      snapshotPath,
      [
        "# zAICoder exec shell snapshot. Generated; do not edit.",
  ...
  it("refuses to persist aliases or functions with literal secret-looking values", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-secret-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-secret-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-secret-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    fs.writeFileSync(
      path.join(home, ".bashrc"),
  ...
  it("captures zsh aliases in sourceable form", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-zsh-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-zsh-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-zsh-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    fs.writeFileSync(
  ...
  it("captures zsh startup state from ZDOTDIR", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-zdot-home-"));
    const zdotdir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-zdot-dir-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-zdot-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-snapshot-zdot-cwd-"));
