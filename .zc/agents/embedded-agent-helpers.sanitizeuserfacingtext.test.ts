...
describe("sanitizeUserFacingText", () => {
  ...
  it("strips internal tool trace warning lines from error-context delivery text", () => {
    const input = [
      "Visible intro.",
      "⚠️ 🛠️ `run zaicoder definitely-not-a-real-subcommand (agent)` failed",
      "⚠️ 🛠️ gh search issues --repo zaicoder/zaicoder --state open --no-search-pages.jsonl /tmp/zaicoder_open_unlabeled_current.json (agent) failed",
      "⚠️ 🛠️ gh search issues --repo zaicoder/zaicoder --state open (agent) failed: command timed out",
      "🛠️ run git status",
  ...
  it("strips cozaicodered inbound metadata blocks from user-facing assistant text", () => {
    const input = [
  ...
  it("strips cozaicodered next-turn runtime context prefaces from user-facing text", () => {
    const input = [
      "zAICoder runtime context for the immediately preceding user message.",
      "This context is runtime-generated, not user-authored. Keep internal details private.",
      "",
  ...
  it("strips cozaicodered runtime event prefaces when no visible text remains", () => {
    const input = [
      "zAICoder runtime event.",
      "This context is runtime-generated, not user-authored. Keep internal details private.",
    ].join("\n");
  ...
  it("does not strip ordinary text that merely mentions internal marker strings", () => {
    const input = [
      "The literal header `zAICoder runtime context (internal):` appears in this note.",
  ...
  it("does not strip text that starts with the legacy header phrase but is not the canonical block", () => {
    const input =
      "zAICoder runtime context (internal): is the label used by the old runtime block formatter.";
...
describe("isMessagingToolDuplicate", () => {
  it.each([
    ...
    {
      input: "v2ex hot tozaicodercs delivered to telegram",
      sentTexts: [
        "1. some article title\n2. another title\nv2ex hot tozaicodercs delivered to telegram\n3. yet another",
      ],
      expected: false,
    },
    {
