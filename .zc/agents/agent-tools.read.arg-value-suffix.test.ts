...
import { createzAICoderReadTool } from "./agent-tools.read.js";
import type { AnyAgentTool } from "./agent-tools.types.js";

describe("createzAICoderReadTool malformed XML arg-value suffix handling", () => {
  it("strips the suffix from read paths before invoking the base tool", async () => {
    ...
    const tool = createzAICoderReadTool(base);
  ...
  it("rejects read paths that become empty after suffix stripzaicoderng", async () => {
    const execute = vi.fn();
    const base = {
      name: "read",
      label: "read",
