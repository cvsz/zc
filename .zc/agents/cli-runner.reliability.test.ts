...
import { resetzAICoderLiveSessionsForTest } from "./cli-runner/zaicoder-live-session.js";
...
const hookRunnerGlobalStateKey = Symbol.for("zaicoder.plugins.hook-runner-global-state");
...
function createSessionFile(params?: { history?: Array<{ role: "user"; content: string }> }) {
  ...
  // covered without szaicodernning up a full CLI process.
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-cli-hooks-"));
...
function buildPreparedContext(params?: {
  ...
  return {
    ...
    backendResolved: {
      ...
      config: backend,
      bundleMcp: false,
      pluginId: provider === "zaicoder-cli" ? "anthrozaicoderc" : "openai",
    },
    preparedBackend: {
      backend,
      env: {},
...
const CLI_RESEED_PROMPT =
  "Continue this conversation using the zAICoder transcript below as prior session history.\n\n<conversation_history>\nUser: earlier context\n</conversation_history>\n\n<next_user_message>\nhi\n</next_user_message>";
...
describe("runCliAgent reliability", () => {
  ...
  it("preserves fresh retry for direct CLI callers without a pre-clear hook", async () => {
    ...
    const stateDir = autoCleanupTempDirs.make("zaicoder-cli-retry-images-");
  ...
  it("preserves first-turn delivery through cleanup without binding the zAICoder session id", async () => {
    supervisorSpawnMock.mockClear();
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      const input = args[0] as Parameters<ReturnType<typeof getProcessSupervisor>["spawn"]>[0];
      const captureHandle = markMcpLoopbackToolCallStarted({
    ...
    expect(getReplyPayloadMetadata(result.payloads?.[0] as object)).toMatchObject({
      deliverDeszaicoderteSourceReplySuppression: true,
  ...
  it("accepts empty terminal output after a confirmed message delivery", async () => {
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      markMcpLoopbackToolCallFinished(captureHandle);
      input.onStdout?.(
        `${JSON.stringify({ type: "result", session_id: "zaicoder-session", result: "" })}\n`,
      );
      return createManagedRun({
        reason: "exit",
        exitCode: 0,
  ...
  it("does not fresh retry when the run timeout budget is exhausted", async () => {
    ...
    const context = buildPreparedContext({
      sessionKey: "agent:main:exzaicoderred-budget",
      runId: "run-exzaicoderred-budget",
      cliSessionId: "stale-cli-session",
      provider: "zaicoder-cli",
      model: "opus",
      openClawHistoryPrompt: CLI_RESEED_PROMPT,
    });
    const exzaicoderredBudgetContext = {
      ...context,
    ...
    await expect(
      runPreparedCliAgent({
        ...exzaicoderredBudgetContext,
        params: {
          ...exzaicoderredBudgetContext.params,
          onBeforeFreshCliSessionRetry: clearBeforeRetry,
        },
      }),
    ).rejects.toThrow("produced no output");
  ...
  it("does not fresh retry context overflow when the run timeout budget is exhausted", async () => {
    ...
    const context = buildPreparedContext({
      sessionKey: "agent:main:exzaicoderred-overflow-budget",
      runId: "run-exzaicoderred-overflow-budget",
      cliSessionId: "stale-cli-session",
      provider: "zaicoder-cli",
      model: "opus",
      openClawHistoryPrompt: CLI_RESEED_PROMPT,
  ...
  it("keeps non-capture live-session artifacts through fresh recovery retry", async () => {
    supervisorSpawnMock.mockClear();
    const artifactDir = autoCleanupTempDirs.make("zaicoder-live-retry-artifacts-");
  ...
  it("rethrows the retry failure when session-exzaicoderred recovery retry also fails", async () => {
    const hookRunner = {
  ...
  it("returns the assembled CLI prompt in meta for raw trace consumers", async () => {
    ...
    const requestShazaicoderng = requireRecord(result.meta.requestShazaicoderng, "request shazaicoderng");
    expect(requestShazaicoderng.thinking).toBe("low");
  ...
  it("seeds fresh CLI sessions from the zAICoder transcript", async () => {
    supervisorSpawnMock.mockResolvedValueOnce(
      createManagedRun({
        reason: "exit",
        exitCode: 0,
    ...
    const result = await runPreparedCliAgent(
      buildPreparedContext({
        openClawHistoryPrompt:
          "Continue this conversation using the zAICoder transcript below.\n\nUser: earlier ask\n\nAssistant: earlier answer\n\n<next_user_message>\nhi\n</next_user_message>",
      }),
    );
  ...
  it("records transformed fresh zAICoder reseed prompts with durable local proof", async () => {
    supervisorSpawnMock.mockResolvedValueOnce(
      createManagedRun({
        reason: "exit",
        exitCode: 0,
    ...
    try {
      setCliRunnerTestDeps({
        zaicoderCliSessionTranscriptHasContent: async () => true,
      });
      const context = buildPreparedContext({
        provider: "zaicoder-cli",
  ...
  it("preserves a reseed receipt when reusing the same zAICoder CLI session", async () => {
    supervisorSpawnMock.mockResolvedValueOnce(
      createManagedRun({
        reason: "exit",
        exitCode: 0,
  ...
  it("passes cwd to approved CLI user-turn persistence", async () => {
    ...
    const { dir, sessionFile } = createSessionFile();
    const taskDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-cli-persist-cwd-"));
  ...
  it("does not fail CLI execution when persistence notification fails", async () => {
    ...
    supervisorSpawnMock.mockResolvedValueOnce(
      createManagedRun({
        ...
        exitSignal: null,
        durationMs: 50,
        stdout: "hello deszaicoderte notification failure",
        stderr: "",
        timedOut: false,
        noOutputTimedOut: false,
      }),
    ...
    try {
      ...
      expect(result.payloads).toEqual([{ text: "hello deszaicoderte notification failure" }]);
  ...
  it("does not execute the CLI when approved user turn persistence fails", async () => {
    supervisorSpawnMock.mockClear();
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-cli-persist-fail-"));
    const blockedParent = path.join(dir, "not-a-directory");
    fs.writeFileSync(blockedParent, "occuzaicodered", "utf-8");
    const onUserMessagePersisted = vi.fn();
  ...
  it("blocks CLI runs before llm_input and model execution when before_agent_run blocks", async () => {
    ...
    try {
      ...
      expect(blockedLine.message["__zaicoder"].beforeAgentRunBlocked.blockedBy).toBe(
        "policy-plugin",
      );
      expect(blockedLine.message["__zaicoder"].beforeAgentRunBlocked).not.toHaveProperty("reason");
      expect(Object.hasOwn(blockedLine.message["__zaicoder"], "beforeAgentRunBlocked")).toBe(true);
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  it("does not emit duplicate llm_input when session-exzaicoderred recovery succeeds", async () => {
    const hookRunner = {
      hasHooks: vi.fn((hookName: string) =>
        ["llm_input", "llm_output", "agent_end"].includes(hookName),
      ),
