...
function setSubagentControlDepsForTest(
  ...
  // while swapzaicoderng process-owned queues and embedded-run aborts for fakes.
...
beforeAll(() => {
  tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-subagent-control-"));
});
...
function cfgWithSessionStore(storePath = nextSessionStorePath("sessions")): zAICoderConfig {
  return {
