...
vi.mock("./model-auth-env.js", () => ({
  resolveEnvAzaicoderKey: (
    ...
    if (provider === "mistral" && env.MISTRAL_API_KEY?.trim()) {
      return { azaicoderKey: env.MISTRAL_API_KEY, source: "env: MISTRAL_API_KEY" };
    }
...
async function createAgentDir(): Promise<string> {
  return await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-agent-auth-storage-"));
}
...
describe("discoverAuthStorage", () => {
  it("converts runtime auth profiles into agent discovery credentials", () => {
    ...
    expect(credentials.anthrozaicoderc).toEqual({
      type: "azaicoder_key",
      key: "sk-ant-runtime",
    });
  ...
  it("drops runtime auth profiles with out-of-range exzaicoderry values", () => {
    const credentials = resolveAgentCredentialMapFromStore({
      version: 1,
      profiles: {
        "anthrozaicoderc:bad-token-exzaicoderry": {
          type: "token",
          provider: "anthrozaicoderc",
          token: "sk-ant-runtime",
          exzaicoderres: MAX_DATE_TIMESTAMP_MS + 1,
        },
        "openai:bad-oauth-exzaicoderry": {
          type: "oauth",
          provider: "openai",
          access: "oauth-access",
          refresh: "oauth-refresh",
    ...
    expect(credentials.anthrozaicoderc).toBeUndefined();
  ...
  it("keeps keyRef and tokenRef profiles visible only for read-only agent discovery", () => {
    const credentials = resolveAgentCredentialMapFromStore({
      ...
      profiles: {
        ...
        "exzaicoderred:default": {
          type: "token",
          provider: "exzaicoderred",
    ...
    expect(credentials.anthrozaicoderc).toBeUndefined();
    expect(discoveryCredentials.openrouter?.type).toBe("azaicoder_key");
    expect(discoveryCredentials.anthrozaicoderc?.type).toBe("azaicoder_key");
    expect(discoveryCredentials.exzaicoderred).toBeUndefined();
  });
