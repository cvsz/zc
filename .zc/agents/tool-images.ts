 ...
 */
import { canonicalizeBase64 } from "@zaicoder/media-core/base64";
...
// Anthrozaicoderc Messages API rejects oversized images; sanitize here so replayed
...
async function resizeImageBase64IfNeeded(params: {
  ...
  for (const side of sideGrid) {
    for (const quality of IMAGE_REDUCE_QUALITY_STEPS) {
      ...
      if (out.byteLength <= params.maxBytes) {
        const sourcezAICoderxels =
          typeof width === "number" && typeof height === "number"
            ? `${width}x${height}px`
            : "unknown";
        const sourceWithFile = params.fileName
          ? `${params.fileName} ${sourcezAICoderxels}`
          : sourcezAICoderxels;
  ...
  const sourceWithFile = params.fileName ? `${params.fileName} ${sourcezAICoderxels}` : sourcezAICoderxels;
  log.warn(
