...
// provider transport patching for embedded zAICoder/OpenAI execution paths.
...
describe("transport params runtime contract (embedded zAICoder/OpenAI path)", () => {
  ...
  it.each(UNRELATED_TOOL_CALLS_PAYLOAD_APIS)(
    ...
    (azaicoder) => {
      expect(supportsGptParallelToolCallsPayload(azaicoder)).toBe(false);
    },
  );
