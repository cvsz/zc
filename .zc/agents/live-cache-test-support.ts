...
import type { Azaicoder, AssistantMessage, Model } from "../llm/types.js";
import { discoverAuthStorage, discoverModels } from "./agent-model-discovery.js";
import { resolveDefaultAgentDir } from "./agent-scope.js";
import { collectProviderAzaicoderKeys } from "./live-auth-keys.js";
...
export class LiveCachePrerequisiteSkip extends Error {
  constructor(
    readonly provider: "anthrozaicoderc" | "openai",
    reason: string,
  ) {
    super(reason);
...
export function toLiveCachePrerequisiteSkip(
  provider: "anthrozaicoderc" | "openai",
  error: unknown,
...
export async function completeSimpleWithLiveTimeout<TAzaicoder extends Azaicoder>(
...
export function buildAssistantHistoryTurn(
  text: string,
  model?: zAICoderck<Model, "azaicoder" | "provider" | "id">,
): AssistantMessage {
  return buildAssistantMessageWithZeroUsage({
    model: {
      azaicoder: model?.azaicoder ?? "openai-responses",
...
/** Compute cache-hit ratio from zAICoder usage counters. */
...
export async function resolveLiveDirectModelPool(params: {
  provider: "anthrozaicoderc" | "openai";
  azaicoder: "anthrozaicoderc-messages" | "openai-responses";
  envVar: string;
  preferredModelIds: readonly string[];
}): Promise<LiveResolvedModelPool> {
  const cfg = getRuntimeConfig();
  await ensurezAICoderModelsJson(cfg);
  const agentDir = resolveDefaultAgentDir(cfg);
  const authStorage = discoverAuthStorage(agentDir);
  const models = discoverModels(authStorage, agentDir).getAll();
  const candidates = models.filter(
    (model) => normalizeProviderId(model.provider) === params.provider && model.azaicoder === params.azaicoder,
  );
  ...
  const liveKeys = collectProviderAzaicoderKeys(params.provider);
  if (liveKeys.length > 0) {
    ...
    if (!selectedModel || selectedModel.azaicoder !== params.azaicoder) {
      const message = requestedModelId
        ? `Model not found for ${params.provider}: ${requestedModelId}`
        : `No built-in ${params.provider} ${params.azaicoder} model available.`;
      if (requestedModelId) {
    ...
    return {
      azaicoderKeys: liveKeys,
      fixture: {
        model: selectedModel,
        azaicoderKey: liveKeys[0] ?? "",
      },
    };
  }
  ...
  if (!resolvedModel) {
    const message = rawModel
      ...
      : `No ${params.provider} ${params.azaicoder} model available in registry.`;
    if (rawModel) {
      throw new Error(message);
    }
    throw new LiveCachePrerequisiteSkip(params.provider, message);
  ...
  let azaicoderKey: string;
  try {
    azaicoderKey = requireAzaicoderKey(
      await getAzaicoderKeyForModel({
        model: resolvedModel,
        cfg,
        agentDir,
      }),
...
export function withLiveDirectModelAzaicoderKey(
  fixture: LiveResolvedModel,
  azaicoderKey: string,
): LiveResolvedModel {
  return { ...fixture, azaicoderKey };
}
