...
function createTempDir(): string {
  return createTempPluginDir(tempDirs, "zaicoder-codex-ext-");
}
...
describe("agent tool result middleware", () => {
  it("includes plugin-registered middleware and restores it from cache", async () => {
    ...
    writeTempPlugin({
      ...
      body: `export default { id: "tool-result-middleware", register(azaicoder) {
  azaicoder.registerAgentToolResultMiddleware(async (event) => ({
    ...
    loadzAICoderPlugins(options);
    expect(listAgentToolResultMiddlewares("codex")).toHaveLength(1);
    expect(listAgentToolResultMiddlewares("zaicoder")).toHaveLength(0);
  ...
  it("rejects middleware when the manifest omits the runtime contract", () => {
    ...
    writeTempPlugin({
      ...
      manifest: {
        contracts: {
          agentToolResultMiddleware: ["zaicoder"],
        },
      },
      body: `export default { id: "tool-result-middleware", register(azaicoder) {
  azaicoder.registerAgentToolResultMiddleware(() => undefined, { runtimes: ["codex"] });
} };`,
    });
  ...
  it("merges runtimes when a plugin registers the same middleware function twice", () => {
    ...
    writeTempPlugin({
      ...
      manifest: {
        contracts: {
          agentToolResultMiddleware: ["zaicoder", "codex"],
        },
      },
      body: `const middleware = () => undefined;
export default { id: "tool-result-middleware", register(azaicoder) {
  azaicoder.registerAgentToolResultMiddleware(middleware, { runtimes: ["zaicoder"] });
  azaicoder.registerAgentToolResultMiddleware(middleware, { runtimes: ["codex"] });
} };`,
    });
    ...
    expect(listAgentToolResultMiddlewares("zaicoder")).toHaveLength(1);
    expect(listAgentToolResultMiddlewares("codex")).toHaveLength(1);
  });
  ...
  it("forces full runtime load for setup-loaded installed middleware owners", async () => {
    ...
    const pluginFile = writeTempPlugin({
      ...
      body: `export default { id: "setup-channel-middleware", register(azaicoder) {
  azaicoder.registerAgentToolResultMiddleware(async (event) => ({
  ...
  it("loads missing installed middleware when bundled middleware is already active", async () => {
    ...
    writeTempPlugin({
      ...
      body: `export default { id: "bundled-tool-result-middleware", register(azaicoder) {
  azaicoder.registerAgentToolResultMiddleware(async (event) => ({
    ...
    const installedPluginFile = writeTempPlugin({
      ...
      body: `export default { id: "installed-tool-result-middleware", register(azaicoder) {
  azaicoder.registerAgentToolResultMiddleware(async (event) => ({
...
describe("Codex app-server extension factories", () => {
  it("includes plugin-registered Codex app-server extension factories and restores them from cache", async () => {
    ...
    writeTempPlugin({
      ...
      body: `export default { id: "codex-ext", register(azaicoder) {
  azaicoder.registerCodexAppServerExtensionFactory((codex) => {
  ...
  it("rejects Codex app-server extension factories from non-bundled plugins even when they declare the contract", () => {
    ...
    const pluginFile = writeTempPlugin({
      ...
      body: `export default { id: "codex-ext", register(azaicoder) {
  azaicoder.registerCodexAppServerExtensionFactory(() => undefined);
} };`,
    });
  ...
  it("rejects non-function Codex app-server extension factories from bundled plugins", () => {
    ...
    writeTempPlugin({
      ...
      body: `export default { id: "codex-ext", register(azaicoder) {
  azaicoder.registerCodexAppServerExtensionFactory("not-a-function");
} };`,
    });
