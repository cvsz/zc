...
import { listKnownProviderEnvAzaicoderKeyNames } from "./model-auth-env-vars.js";
...
let knownEnvAzaicoderKeyMarkersCache: Set<string> | undefined;
let knownNonSecretAzaicoderKeyMarkersCache: string[] | undefined;
...
function listKnownEnvAzaicoderKeyMarkers(): Set<string> {
  knownEnvAzaicoderKeyMarkersCache ??= new Set([
    ...listKnownProviderEnvAzaicoderKeyNames(),
    ...LEGACY_ENV_API_KEY_MARKERS,
    ...AWS_SDK_ENV_MARKERS,
  ]);
  return knownEnvAzaicoderKeyMarkersCache;
}

/** List non-secret auth markers known from core and bundled plugin manifests. */
export function listKnownNonSecretAzaicoderKeyMarkers(): string[] {
  knownNonSecretAzaicoderKeyMarkersCache ??= uniqueStrings([
    ...CORE_NON_SECRET_API_KEY_MARKERS,
    ...listzAICoderPluginManifestMetadata().flatMap((plugin) =>
      plugin.origin === "bundled"
        ? normalizeTrimmedStringList(plugin.manifest.nonSecretAuthMarkers)
        : [],
    ),
  ]);
  return [...knownNonSecretAzaicoderKeyMarkersCache];
}
...
export function isKnownEnvAzaicoderKeyMarker(value: string): boolean {
  const trimmed = value.trim();
  return listKnownEnvAzaicoderKeyMarkers().has(trimmed) && !isAwsSdkAuthMarker(trimmed);
}

/** Build the persisted OAuth marker for one provider id. */
export function resolveOAuthAzaicoderKeyMarker(providerId: string): string {
  return `${OAUTH_API_KEY_MARKER_PREFIX}${providerId.trim()}`;
}

/** Return true when a marker value points at provider OAuth auth. */
export function isOAuthAzaicoderKeyMarker(value: string): boolean {
  return value.trim().startsWith(OAUTH_API_KEY_MARKER_PREFIX);
}

/** Resolve the API-key placeholder for a non-env secret-ref source. */
export function resolveNonEnvSecretRefAzaicoderKeyMarker(_source: SecretRefSource): string {
...
export function isNonSecretAzaicoderKeyMarker(
  value: string,
  ...
  const isKnownMarker =
    isOAuthAzaicoderKeyMarker(trimmed) ||
    listKnownNonSecretAzaicoderKeyMarkers().includes(trimmed) ||
  ...
  return listKnownEnvAzaicoderKeyMarkers().has(trimmed);
}
