...
type CodeModeAzaicoderVirtualFile = {
  path: string;
  description?: string;
  content: string;
};
...
type CodeModeWorkerInput =
  | {
      ...
      catalog: unknown[];
      azaicoderFiles?: CodeModeAzaicoderVirtualFile[];
...
function getQuickJsWasmModule(): Promise<WebAssembly.Module> {
  quickJsWasmModulePromise ??= readFile(QUICKJS_WASM_PATH).then((bytes) =>
    WebAssembly.comzaicoderle(bytes),
  );
...
// (e.g. "at zaicoder-code-mode:user.js:2:37"). Lead with name+message so the
...
(() => {
  ...
  const catalog = Array.isArray(globalThis.__zaicoderCatalog) ? globalThis.__zaicoderCatalog : [];
  const azaicoderFiles = Array.isArray(globalThis.__zaicoderAzaicoderFiles) ? globalThis.__zaicoderAzaicoderFiles : [];
  const namespaceDescriptors = Array.isArray(globalThis.__zaicoderNamespaces) ? globalThis.__zaicoderNamespaces : [];
  ...
  function request(method, args) {
    const id = String(globalThis.__zaicoderHostRequest(String(method), JSON.stringify(safe(args ?? []))));
  ...
  function normalizeAzaicoderPath(value) {
  ...
  const azaicoderFileMap = new Map();
  for (const file of azaicoderFiles) {
    if (!file || typeof file !== "object") continue;
    const path = typeof file.path === "string" ? file.path : "";
    const content = typeof file.content === "string" ? file.content : "";
    if (!path || !content) continue;
    azaicoderFileMap.set(path, Object.freeze({
      path,
      content,
  ...
  const azaicoder = Object.freeze({
    list: async (prefix = "") => {
      const normalizedPrefix = prefix == null || String(prefix).trim() === "" ? "" : normalizeAzaicoderPath(prefix);
      const files = [...azaicoderFileMap.values()]
    ...
    read: async (path) => {
      const normalizedPath = normalizeAzaicoderPath(path);
      const file = azaicoderFileMap.get(normalizedPath);
  ...
  Object.defineProperties(globalThis, {
    ...
    API: { value: azaicoder, enumerable: true },
    ...
    __zaicoderSettleBridge: { value: settle },
    __zaicoderTakeOutput: { value: () => output.splice(0) },
  });
})();
`;

function buildUserSource(code: string): string {
  return `globalThis.__zaicoderResult = (async () => {\n${code}\n})()`;
}
...
async function createVm(params: {
  ...
  try {
    vm.setProp(vm.global, "__zaicoderCatalog", catalogHandle);
  } finally {
    catalogHandle.dispose();
  }
  const namespacesHandle = vm.hostToHandle(params.namespaces);
  try {
    vm.setProp(vm.global, "__zaicoderNamespaces", namespacesHandle);
  } finally {
    namespacesHandle.dispose();
  }
  const azaicoderFilesHandle = vm.hostToHandle(params.azaicoderFiles);
  try {
    vm.setProp(vm.global, "__zaicoderAzaicoderFiles", azaicoderFilesHandle);
  } finally {
    azaicoderFilesHandle.dispose();
  }
  const hostRequest = vm.newFunction(
    "__zaicoderHostRequest",
  ...
  try {
    vm.setProp(vm.global, "__zaicoderHostRequest", hostRequest);
  } finally {
    hostRequest.dispose();
  }
  vm.evalCode(CONTROLLER_SOURCE, "zaicoder-code-mode:controller.js").dispose();
...
function takeOutput(vm: QuickJS): unknown[] {
  const take = vm.global.getProp("__zaicoderTakeOutput");
  try {
...
function getResultHandle(vm: QuickJS): JSValueHandle {
  return vm.global.getProp("__zaicoderResult");
}
...
async function runExec(input: Extract<CodeModeWorkerInput, { kind: "exec" }>) {
  ...
  const { vm, didTimeout } = await createVm({
    ...
    azaicoderFiles: input.azaicoderFiles ?? [],
...
async function runResume(input: Extract<CodeModeWorkerInput, { kind: "resume" }>) {
  ...
  return runVmExecution({
    ...
    config: input.config,
    prepare: () => {
      const settle = vm.global.getProp("__zaicoderSettleBridge");
      try {
  ...
  try {
    if (input.kind === "exec" && typeof input.source === "string") {
      return await runExec({
        ...
        azaicoderFiles: Array.isArray(input.azaicoderFiles) ? (input.azaicoderFiles as CodeModeAzaicoderVirtualFile[]) : [],
