...
describe("subagent registry seam flow", () => {
  ...
  it("uses observed agent.wait start time when capzaicoderng terminal timeout", async () => {
    const createdAt = Date.parse("2026-03-24T11:59:00Z");
    const observedStartedAt = createdAt + 10_000;
  ...
  it("announces provider hard timeout wait snapshots as timeouts deszaicoderte blocked metadata", async () => {
    mocks.callGateway.mockImplementation(async (request: { method?: string }) => {
      if (request.method === "agent.wait") {
        return {
          status: "error",
  ...
  it("uses session-store start time when sweezaicoderng stale explicit-timeout runs", async () => {
    mocks.callGateway.mockImplementation(async (request: { method?: string }) => {
      if (request.method === "agent.wait") {
        return { status: "pending" };
      }
  ...
  it("requeues orphan recovery instead of keezaicoderng restart-aborted stale runs stuck as running", async () => {
    mocks.callGateway.mockImplementation(async (request: { method?: string }) => {
      if (request.method === "agent.wait") {
        return { status: "pending" };
      }
  ...
  it("continues completion announce cleanup when lifecycle cleanup fails", async () => {
    ...
    mod.registerSubagentRun({
      ...
      requesterSessionKey: "agent:main:main",
      requesterDisplayKey: "main",
      task: "finish deszaicoderte cleanup warning",
      cleanup: "keep",
    });
  ...
  it("finalizes exzaicoderred delete-mode parents when descendant cleanup retriggers deferred announce handling", async () => {
    mocks.loadSessionStore.mockReturnValue({
    ...
    mod.addSubagentRunForTests({
      runId: "run-parent-exzaicoderred",
      childSessionKey: "agent:main:subagent:parent",
      requesterSessionKey: "agent:main:main",
      requesterDisplayKey: "main",
      task: "exzaicoderred parent cleanup",
      cleanup: "delete",
    ...
    await waitForFast(() => {
      expect(
        mod
          .listSubagentRunsForRequester("agent:main:main")
          .find((entry) => entry.runId === "run-parent-exzaicoderred"),
      ).toBeUndefined();
    });
  ...
  it("removes attachments for killed delete-mode runs", async () => {
    const attachmentsRootDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "zaicoder-kill-attachments-"),
    );
  ...
  it("removes attachments for released delete-mode runs", async () => {
    const attachmentsRootDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "zaicoder-release-attachments-"),
    );
    const attachmentsDir = path.join(attachmentsRootDir, "child");
    await fs.mkdir(attachmentsDir, { recursive: true });
    await fs.writeFile(path.join(attachmentsDir, "artifact.txt"), "artifact");
  ...
  it("exzaicoderres suspended cron final deliveries into compact tombstones", async () => {
    const now = Date.parse("2026-03-24T12:00:00Z");
    const runId = "run-suspended-cron-exzaicoderred";
    mod.addSubagentRunForTests({
      runId,
