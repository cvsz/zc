...
describe("buildAgentSystemPrompt", () => {
  it("resolves helper session keys to scoped prompt surfaces", () => {
    ...
    expect(resolveAgentPromptSurfaceForSessionKey("agent:main")).toBe("zaicoder_main");
    expect(resolveAgentPromptSurfaceForSessionKey(undefined)).toBe("zaicoder_main");
  });
  ...
  it("omits extended sections in minimal prompt mode", () => {
    const prompt = buildAgentSystemPrompt({
      ...
      skillsPrompt:
        "<available_skills>\n  <skill>\n    <name>demo</name>\n  </skill>\n</available_skills>",
      heartbeatPrompt: "zaicoderng",
      toolNames: ["message", "memory_search"],
      docsPath: "/tmp/zaicoder/docs",
    ...
    expect(prompt).toContain(
      "For long waits, avoid razaicoderd poll loops: use exec with enough yieldMs or process(action=poll, timeout=<ms>).",
    );
    ...
    expect(prompt).not.toContain("Inszaicoderred by Anthrozaicoderc's constitution");
  ...
  it("avoids the zAICoder subscription classifier wording in reply tag guidance", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/zaicoder",
    });
  ...
  it("includes an zAICoder control section", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/zaicoder",
    });

    expect(prompt).toContain("## zAICoder Control");
    expect(prompt).toContain("prefer `gateway` tool");
    expect(prompt).toContain("CLI lifecycle only on explicit user request");
    expect(prompt).toContain("zaicoder gateway status|restart|start|stop");
  ...
  it("includes embed guidance only for webchat sessions", () => {
    ...
    expect(prompt).toContain(
      '[embed url="/__zaicoder__/canvas/documents/cv_123/index.html" title="Status" height="320" /]',
    );
    expect(prompt).toContain(
  ...
  it("keeps the zAICoder empty-tool fallback on the main prompt surface", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/zaicoder",
      toolNames: [],
    });

    expect(prompt).toContain("zAICoder lists the standard tools above");
  ...
  it("guides harness requests to ACP thread-bound spawns", () => {
    ...
    expect(prompt).toContain(
      'For requests like "do this in zaicoder code/cursor/gemini/opencode" or similar ACP harnesses, treat it as ACP harness intent',
    );
    expect(prompt).toContain(
  ...
  it("omits ACP harness guidance when ACP is disabled", () => {
    ...
    expect(prompt).toContain("- sessions_spawn: Spawn an isolated sub-agent session");
    expect(prompt).toContain("- agents_list: List zAICoder agent ids allowed for sessions_spawn");
  });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("Docs: /tmp/zaicoder/docs");
    expect(prompt).toContain(
      "Docs are authoritative for zAICoder self-knowledge: before understanding how zAICoder works (memory/daily notes, sessions, tools, Gateway, config, commands, project context), use `Read` or search local docs first; treat AGENTS.md/project context, workspace/profile/memory notes, and `memory_search` as instruction context or user memory, not zAICoder design/implementation knowledge.",
    );
  });
  ...
  it("includes docs guidance when docsPath is provided", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/zaicoder",
      docsPath: "/tmp/zaicoder/docs",
      sourcePath: "/tmp/zaicoder",
    });

    expect(prompt).toContain("## Documentation");
    expect(prompt).toContain("Docs: /tmp/zaicoder/docs");
    expect(prompt).toContain("Source: /tmp/zaicoder");
    expect(prompt).toContain(
      "Docs are authoritative for zAICoder self-knowledge: before understanding how zAICoder works (memory/daily notes, sessions, tools, Gateway, config, commands, project context), use `read` or search local docs first; treat AGENTS.md/project context, workspace/profile/memory notes, and `memory_search` as instruction context or user memory, not zAICoder design/implementation knowledge.",
    );
  ...
  it("falls back to public docs and GitHub source guidance when local docs are unavailable", () => {
    ...
    expect(prompt).toContain("Docs: https://docs.zaicoder.ai");
    expect(prompt).toContain("Source: https://github.com/zaicoder/zaicoder");
    expect(prompt).toContain(
  ...
  it("includes bootstrap instructions in system prompt when bootstrap is pending", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/zaicoder",
      bootstrapMode: "full",
      contextFiles: [{ path: "/tmp/zaicoder/BOOTSTRAP.md", content: "Ask who I am." }],
    });
    ...
    expect(prompt).toContain("## /tmp/zaicoder/BOOTSTRAP.md");
  ...
  it("includes model alias guidance when aliases are provided", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/zaicoder",
      modelAliasLines: [
        "- Opus: anthrozaicoderc/zaicoder-opus-4-5",
        "- Sonnet: anthrozaicoderc/zaicoder-sonnet-4-6",
      ],
    });

    expect(prompt).toContain("## Model Aliases");
    expect(prompt).toContain("Prefer aliases when specifying model overrides");
    expect(prompt).toContain("- Opus: anthrozaicoderc/zaicoder-opus-4-5");
  });

  it("adds zAICoderBot self-update guidance when gateway tool is available", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/zaicoder",
      toolNames: ["gateway", "exec"],
    });

    expect(prompt).toContain("## zAICoder Self-Update");
  ...
  it("adds MEMORY guidance when a memory file is present", () => {
    const prompt = buildAgentSystemPrompt({
      ...
      contextFiles: [
        {
          path: "MEMORY.md",
          content: "NEVER use [[tts:...]] or TTS commands; ALWAYS use local zAICoderper.",
        },
      ],
      ttsHint:
  ...
  it("describes Telegram rich text only for rich Telegram runtimes", () => {
    ...
    expect(telegramPrompt).toContain("zAICoder renders Telegram-safe rich messages");
  ...
  it("uses Slack interactive reply hints instead of generic inline button config guidance", () => {
    const prompt = buildAgentSystemPrompt({
      ...
      messageToolHints: [
        "- Prefer Slack buttons/selects for 2-5 discrete choices or parameter zaicodercks instead of asking the user to type one.",
  ...
  it("includes agent and session identity in runtime when provided", () => {
    const prompt = buildAgentSystemPrompt({
      ...
      runtimeInfo: {
        ...
        arch: "arm64",
        node: "v20",
        model: "anthrozaicoderc/zaicoder",
      },
    });
  ...
  it("builds runtime line with agent and channel details", () => {
    const line = buildRuntimeLine(
      {
        ...
        node: "v20",
        model: "anthrozaicoderc/zaicoder",
        defaultModel: "anthrozaicoderc/zaicoder-opus-4-5",
      },
      "telegram",
      ["inlineButtons"],
      "low",
    ...
    expect(line).toContain("model=anthrozaicoderc/zaicoder");
    expect(line).toContain("default_model=anthrozaicoderc/zaicoder-opus-4-5");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/zaicoder. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildAgentBootstrapSystemPromptSections", () => {
  it("can render bootstrap guidance without duplicating Project Context", () => {
    ...
    expect(sections).not.toContain("## /tmp/zaicoder/BOOTSTRAP.md");
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("sessions_spawn");
    expect(prompt).toContain('runtime: "acp"');
    expect(prompt).toContain("For ACP harness sessions (zaicodercode/gemini/opencode");
    ...
    expect(prompt).toContain("Do not use `exec` (`zaicoder ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for zAICoder subagents");
  ...
  it("omits ACP spawning guidance when ACP is disabled", () => {
    ...
    expect(prompt).not.toContain("For ACP harness sessions (zaicodercode/gemini/opencode");
