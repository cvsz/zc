...
function makeConfig(): zAICoderConfig {
  const azaicoderKeyField = ["azaicoder", "Key"].join("");
  return {
    agents: {
      defaults: {
        model: {
    ...
    models: {
      providers: {
        openai: {
          ...
          [azaicoderKeyField]: "openai-test-key", // pragma: allowlist secret
        ...
        groq: {
          azaicoder: "openai-responses",
          [azaicoderKeyField]: "groq-test-key", // pragma: allowlist secret
...
async function withAgentWorkspace<T>(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-model-fallback-"));
  ...
  saveAuthProfileStore(
    {
      version: 1,
      profiles: {
        "openai:p1": { type: "azaicoder_key", provider: "openai", key: "sk-openai" },
        "groq:p1": { type: "azaicoder_key", provider: "groq", key: "sk-groq" },
      },
      usageStats:
        usageStats ??
        ({
  ...
  saveAuthProfileStore(
    {
      version: 1,
      profiles: {
        "openai:p1": { type: "azaicoder_key", provider: "openai", key: "sk-openai-1" },
        "openai:p2": { type: "azaicoder_key", provider: "openai", key: "sk-openai-2" },
        "openai:p3": { type: "azaicoder_key", provider: "openai", key: "sk-openai-3" },
        "groq:p1": { type: "azaicoder_key", provider: "groq", key: "sk-groq" },
      },
      usageStats: {
        "openai:p1": { lastUsed: 1 },
...
async function runEmbeddedFallback(params: {
  ...
  // keezaicoderng provider/model attempts deterministic through mocks.
