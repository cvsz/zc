...
import { OPENCLAW_CLI_ENV_VALUE } from "../infra/zaicoder-exec-env.js";
...
describe("buildSandboxCreateArgs", () => {
  ...
  it("includes hardening and resource flags", () => {
    const cfg: SandboxDockerConfig = {
      ...
      apparmorProfile: "zaicoder-sandbox",
      dns: ["1.1.1.1"],
      extraHosts: ["internal.service:10.0.0.5"],
    };

    const args = buildSandboxCreateArgs({
      name: "zaicoder-sbx-test",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
      labels: { "zaicoder.sandboxBrowser": "1" },
    });

    expect(args[0]).toBe("create");
    expectFlagValues(args, "--name", ["zaicoder-sbx-test"]);
    expectFlagValues(args, "--label", [
      "zaicoder.sandbox=1",
      "zaicoder.sessionKey=main",
      "zaicoder.createdAtMs=1700000000000",
      "zaicoder.sandboxBrowser=1",
    ]);
  ...
  it("omits non-finite numeric Docker resource flags", () => {
    const cfg = createSandboxConfig({
      zaicoderdsLimit: Number.POSITIVE_INFINITY,
      cpus: Number.NaN,
      ulimits: {
        nofile: {
          soft: Number.NaN,
    ...
    const args = buildSandboxCreateArgs({
      name: "zaicoder-sbx-non-finite-limits",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
    });

    expect(args).not.toContain("--zaicoderds-limit");
  ...
  it("passes explicit configured sandbox env through even when names look sensitive", () => {
    const cfg = createSandboxConfig({
      env: {
        ANTHROPIC_ADMIN_KEY: "dummy-anthrozaicoderc-admin-key",
        GEMINI_API_KEY: "dummy-gemini-azaicoder-key",
        ...
        RESEND_API_KEY: "dummy-resend-azaicoder-key",
      },
    });

    const args = buildSandboxCreateArgs({
      name: "zaicoder-sbx-marker",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
    });
  ...
  it("blocks bind sources outside runtime allowlist roots", () => {
    ...
    expect(() =>
      buildSandboxCreateArgs({
        name: "zaicoder-sbx-outside-roots",
        cfg,
        scopeKey: "main",
        createdAtMs: 1700000000000,
  ...
  it("allows bind sources outside runtime allowlist with explicit override", () => {
    const cfg = createSandboxConfig({}, ["/opt/external:/data:rw"]);
    const args = buildSandboxCreateArgs({
      name: "zaicoder-sbx-outside-roots-override",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
      bindSourceRoots: ["/tmp/workspace", "/tmp/agent"],
  ...
  it("blocks reserved /workspace target bind mounts by default", () => {
    ...
    expectBuildToThrow("zaicoder-sbx-reserved-target", cfg, /reserved container path/);
  });

  it("allows reserved /workspace target bind mounts with explicit dangerous override", () => {
    const cfg = createSandboxConfig({}, ["/tmp/override:/workspace:rw"]);
    const args = buildSandboxCreateArgs({
      name: "zaicoder-sbx-reserved-target-override",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
  ...
  it("allows container namespace join with explicit dangerous override", () => {
    ...
    const args = buildSandboxCreateArgs({
      name: "zaicoder-sbx-container-network-override",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
    });
