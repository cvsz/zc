...
export function createProcessTool(
  ...
  const terminateSessionFallback = (session: ProcessSession) => {
    const zaicoderd = session.zaicoderd ?? session.child?.zaicoderd;
    if (typeof zaicoderd !== "number" || !Number.isFinite(zaicoderd) || zaicoderd <= 0) {
      return false;
    }
    killProcessTree(zaicoderd);
    return true;
  };
        ...
        const running = listRunningSessions()
          ...
          .map((s) => {
            ...
            return {
              sessionId: s.id,
              status: "running",
              zaicoderd: s.zaicoderd ?? undefined,
