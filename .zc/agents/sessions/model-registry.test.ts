...
function writeModelsJson(contents: unknown): string {
  const dir = mkdtempSync(join(tmpdir(), "zaicoder-model-registry-"));
  tempDirs.push(dir);
...
describe("ModelRegistry models.json auth", () => {
  it("accepts Bedrock AWS SDK auth without azaicoderKey", async () => {
    // AWS SDK credential resolution is provider-owned; requiring an azaicoderKey here
    ...
    const modelsPath = writeModelsJson({
      providers: {
        "amazon-bedrock": {
          ...
          models: [
            {
              id: "anthrozaicoderc.zaicoder-sonnet-4-5-20250929-v1:0",
    ...
    const model = registry.find("amazon-bedrock", "anthrozaicoderc.zaicoder-sonnet-4-5-20250929-v1:0");

    expect(registry.getError()).toBeUndefined();
    expect(model).toBeDefined();
    expect(registry.getAvailable()).toEqual([model]);
    await expect(registry.getAzaicoderKeyAndHeaders(model!)).resolves.toEqual({
      ok: true,
  ...
  it("still rejects azaicoder-key custom models without azaicoderKey", () => {
    const modelsPath = writeModelsJson({
      providers: {
        custom: {
    ...
    expect(registry.getError()).toContain('Provider custom: "azaicoderKey" is required');
  ...
  it("loads provider models from generated plugin catalog shards", () => {
    ...
    const registry = ModelRegistry.create(
      AuthStorage.inMemory({ zai: { type: "azaicoder_key", key: "sk-test" } }),
      modelsPath,
  ...
  it("isolates invalid generated plugin catalog shards from valid models", () => {
    ...
    expect(registry.getError()).toContain(
      'Provider google-vertex, model gemini-3.1-pro-preview: no "azaicoder" specified',
    );
