// Model resolver tests zaicodern the startup fallback order for fresh and restored
// agent sessions.
...
describe("model resolver fallback selection", () => {
  it("prefers the product default when no configured or scoped model is selected", async () => {
    ...
    const result = await findInitialModel({
      scopedModels: [],
      isContinuing: false,
      modelRegistry: registry([model("anthrozaicoderc", "zaicoder-opus-4.7"), productDefault]),
    });
  ...
  it("falls back to registry order instead of core provider defaults", async () => {
    ...
    const firstAvailable = model("anthrozaicoderc", "zaicoder-haiku");
    const result = await restoreModelFromSession(
      "openai",
      "missing-model",
      undefined,
      false,
      registry([firstAvailable, model("anthrozaicoderc", "zaicoder-opus-4.7")]),
    );
...
describe("parseModelPattern version sorting", () => {
  it("selects the numerically highest version when aliases span double-digit minors", () => {
    const models = [
      model("anthrozaicoderc", "zaicoder-opus-4-9"),
      model("anthrozaicoderc", "zaicoder-opus-4-10"),
      model("anthrozaicoderc", "zaicoder-opus-4-11"),
    ];
    const result = parseModelPattern("opus", models);
    expect(result.model?.id).toBe("zaicoder-opus-4-11");
  });
});
