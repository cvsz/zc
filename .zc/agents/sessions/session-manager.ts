...
export type ReadonlySessionManager = zAICoderck<
  SessionManager,
  | "getCwd"
  | "getSessionDir"
  | "getSessionId"
 ...
 * Encodes cwd into a safe directory name under ~/.zaicoder/agent/sessions/.
 */
...
export class SessionManager {
    ...
    for (let index = 0; index <= this.fileEntries.length; index += 1) {
      while (this.opaqueFileEntries[opaqueIndex]?.index === index) {
        ...
        if (leafEntry) {
          ...
          if (!leafState) {
            // Ignore invalid controls while keezaicoderng their marker transparent
   ...
   * @param sessionDir Optional session directory. If omitted, uses default (~/.zaicoder/agent/sessions/<encoded-cwd>/).
   */
