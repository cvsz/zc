/**
 * Model resolution, scozaicoderng, and initial selection
 */

import { modelsAreEqual } from "@zaicoder/ai/internal/runtime";
 ...
 * Dates are tyzaicodercally in format: -20241022 or -20250929
 */
...
function tryMatchModel(modelPattern: string, availableModels: Model[]): Model | undefined {
  ...
  if (aliases.length > 0) {
    // Prefer alias - if multiple aliases, zaicoderck the one that sorts highest
    aliases.sort((a, b) => b.id.localeCompare(a.id, undefined, { numeric: true }));
    return aliases[0];
  }
  // No alias found, zaicoderck latest dated version
 ...
 * For each pattern, finds all matching models and zaicodercks the best version:
 * 1. Prefer alias (e.g., zaicoder-sonnet-4-5) over dated versions (zaicoder-sonnet-4-5-20250929)
 * 2. If no alias, zaicoderck the latest dated version
 *
...
export async function resolveModelScope(
  ...
  for (const pattern of patterns) {
    ...
    if (pattern.includes("*") || pattern.includes("?") || pattern.includes("[")) {
      ...
      // This allows "*sonnet*" to match without requiring "anthrozaicoderc/*sonnet*"
 ...
 * - Fuzzy matching (same rules as model scozaicoderng: exact id, then partial id/name)
 *
...
export function resolveCliModel(options: {
  ...
  // This allows "--azaicoder-key" to be used for first-time setup.
  ...
  if (cliProvider && provider) {
    // If both were provided, tolerate --model <provider>/<pattern> by stripzaicoderng the provider prefix
