...
type SkillDiscoveryMode = "zaicoder" | "agents";
...
function readResourceManifestFile(packageJsonPath: string): ResourceManifest | null {
  try {
    ...
    const pkg = JSON.parse(content) as { zaicoder?: ResourceManifest };
    return pkg.zaicoder ?? null;
  } catch {
    return null;
  }
}
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
  if (resourceType === "skills") {
    return collectSkillEntries(dir, "zaicoder");
  }
...
export class DefaultPackageManager implements PackageManager {
    ...
    for (const { pkg, scope } of sources) {
      ...
      if (parsed.type === "npm") {
        ...
        const missingOrWrongVersion =
          ...
          (parsed.zaicodernned && !this.installedNpmMatcheszAICodernnedVersion(parsed, installedPath));
  ...
  private installedNpmMatcheszAICodernnedVersion(source: NpmSource, installedPath: string): boolean {
    const installedVersion = this.getInstalledNpmVersion(installedPath);
    if (!installedVersion) {
      return false;
    }

    const { version: zaicodernnedVersion } = this.parseNpmSpec(source.spec);
    if (!zaicodernnedVersion) {
      return true;
    }

    return installedVersion === zaicodernnedVersion;
  }
  ...
  private getTemporaryDir(prefix: string, suffix?: string): string {
    const hash = createHash("sha256")
      ...
      .digest("hex")
      .slice(0, 8);
    return join(tmpdir(), "zaicoder-resources", prefix, hash, suffix ?? "");
  }
  ...
  private addAutoDiscoveredResources(
    ...
    addResources(
      "skills",
      collectAutoSkillEntries(projectDirs.skills, "zaicoder"),
      projectMetadata,
    ...
    // User extensions from ~/.zaicoder/agent/
    addResources(
      "extensions",
    ...
    // User skills from ~/.zaicoder/agent/
    addResources(
      "skills",
      collectAutoSkillEntries(userDirs.skills, "zaicoder"),
      userMetadata,
