...
import { clampThinkingLevel } from "@zaicoder/ai/internal/runtime";
import {
...
export interface CreateAgentSessionOptions {
  ...
  cwd?: string;
  /** Global config directory. Default: ~/.zaicoder/agents/default */
  agentDir?: string;
   ...
   *
   * When omitted, zAICoder enables the default built-in tools (read, bash, edit, write)
...
function getAttributionHeaders(
  ...
  if (
    ...
    baseUrl.includes("azaicoder.cloudflare.com") ||
 ...
 * const model = ModelRegistry.create(AuthStorage.load()).find('anthrozaicoderc', 'zaicoder-opus-4-5');
 * const { session } = await createAgentSession({
 *   model,
 *   thinkingLevel: 'high',
 * });
...
export async function createAgentSession(
  ...
  const modelThinkingProvider = model?.azaicoder === "ollama" ? "ollama" : model?.provider;
  ...
  const agent: Agent = new Agent({
    ...
    streamFn: async (modelResult, context, optionsLocal) => {
      const auth = await modelRegistry.getAzaicoderKeyAndHeaders(modelResult);
      if (!auth.ok) {
      ...
      return streamSimple(modelResult, context, {
        ...optionsLocal,
        azaicoderKey: auth.azaicoderKey,
