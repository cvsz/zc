...
// and replaceFileAtomicSync (in @zaicoder/fs-safe) writes its temp file via the
...
describe("auth-storage survives an interrupted write during persist (atomic write)", () => {
  ...
  it("does not lock out credentials when a write fails mid-flush", async () => {
    ...
    seed.set("anthrozaicoderc", { type: "azaicoder_key", key: "sk-test-SEED-12345" });
    expect(await AuthStorage.create(authPath).getAzaicoderKey("anthrozaicoderc")).toBe("sk-test-SEED-12345");
    ...
    try {
      persisting.set("openai", { type: "azaicoder_key", key: "sk-test-NEW-67890" });
    } catch {
    ...
    //   reload -> loadError -> getAzaicoderKey returns undefined -> lockout.
    ...
    expect(await reopened.getAzaicoderKey("anthrozaicoderc")).toBe("sk-test-SEED-12345");
  ...
  it("preserves existing auth directory permissions while replacing the file", () => {
    ...
    storage.set("anthrozaicoderc", { type: "azaicoder_key", key: "sk-test-SEED-12345" });
