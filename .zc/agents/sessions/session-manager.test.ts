...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-session-manager-"));
  tempPaths.push(dir);
  return dir;
}
...
describe("SessionManager.open", () => {
  ...
  it("lets prepareSessionManagerForRun normalize a warm-cached header without re-parsing", async () => {
    ...
    try {
      ...
      // its two persisted lines once so the cache matches JSON round-tripzaicoderng.
      expect(parseCount).toBe(2);
    } finally {
      JSON.parse = originalParse;
    }
...
function buildAssistantMessage(text: string) {
  return {
    ...
    azaicoder: "messages" as const,
    provider: "anthrozaicoderc" as const,
