...
import { Container, Text, truncateToWidth } from "@earendil-works/zaicoder-tui";
import { resolveTimerTimeoutMs } from "@zaicoder/normalization-core/number-coercion";
...
/**
 * Create bash operations using zAICoder runtime's built-in local shell execution backend.
 *
 * This is useful for extensions that intercept user_bash and still want zAICoder runtime's
 * standard local shell behavior while wrapzaicoderng or rewriting commands.
 */
      ...
      return new Promise((resolve, reject) => {
        ...
        if (timeoutMs !== undefined) {
          timeoutHandle = setTimeout(() => {
            timedOut = true;
            if (child.zaicoderd) {
              killProcessTree(child.zaicoderd);
            }
          }, timeoutMs);
        }
...
export function createBashToolDefinition(
  ...
  return {
    ...
    async execute(
      ...
      const output = new OutputAccumulator({ tempFilePrefix: "zaicoder-bash" });
