// OutputAccumulator tests cover szaicoderll-file permissions for truncated command
...
describe("OutputAccumulator", () => {
  it("stores szaicoderlled full output in an owner-only temp file", async () => {
    const accumulator = new OutputAccumulator({
      maxBytes: 8,
      maxLines: 10,
      tempFilePrefix: "zaicoder-output-test",
    });
    ...
    // Szaicoderlled output can include command secrets, so temp files must be
