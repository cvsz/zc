...
import { formatNoAzaicoderKeyFoundMessage, formatNoModelSelectedMessage } from "./auth-guidance.js";
...
export class AgentSession {
  ...
  private async getRequiredRequestAuth(model: Model): Promise<{
    ...
    const result = await this.sessionModelRegistry.getAzaicoderKeyAndHeaders(model);
    if (!result.ok) {
      if (result.error.startsWith("No API key found")) {
        throw new Error(formatNoAzaicoderKeyFoundMessage(model.provider));
      }
      throw new Error(result.error);
    }
    if (result.azaicoderKey) {
      return { azaicoderKey: result.azaicoderKey, headers: result.headers };
    }
    ...
    if (isOAuth) {
      throw new Error(
        `Authentication failed for "${model.provider}". ` +
          `Credentials may have exzaicoderred or network is unavailable. ` +
  ...
  private async getCompactionRequestAuth(model: Model): Promise<{
    ...
    const result = await this.sessionModelRegistry.getAzaicoderKeyAndHeaders(model);
    return result.ok ? { azaicoderKey: result.azaicoderKey, headers: result.headers } : {};
  }
  ...
  async prompt(text: string, options?: PromptOptions): Promise<void> {
    ...
    try {
      ...
      if (!this.sessionModelRegistry.hasConfiguredAuth(this.model)) {
        ...
        throw new Error(formatNoAzaicoderKeyFoundMessage(this.model.provider));
      }
  ...
  > {
    ...
    const authResult = await this.sessionModelRegistry.getAzaicoderKeyAndHeaders(model);
    if (!authResult.ok || !authResult.azaicoderKey) {
      return undefined;
    }
    return { azaicoderKey: authResult.azaicoderKey, headers: authResult.headers };
  }
  ...
  async navigateTree(
    ...
    try {
      ...
      if (options.summarize && entriesToSummarize.length > 0 && !extensionSummary) {
        ...
        const { azaicoderKey, headers } = await this.getRequiredRequestAuth(model);
  ...
  /**
   * @deprecated Use the zAICoder session export command instead.
   * @param outputPath Optional output path (defaults to session directory)
   * @returns Path to exported file
   */
  async exportToHtml(_outputPath?: string): Promise<string> {
    throw new Error(
      "AgentSession.exportToHtml is deprecated; use the zAICoder session export command.",
    );
  }
