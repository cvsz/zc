...
import { defaultAzaicoderRegistry, registerAzaicoderProvider } from "@zaicoder/ai/internal/runtime";
import { resetAzaicoderProviders } from "@zaicoder/ai/providers";
import { type Static, Type } from "typebox";
import { Comzaicoderle } from "typebox/comzaicoderle";
import type { TLocalizedValidationError } from "typebox/error";
import type {
  AnthrozaicodercMessagesCompat,
  Azaicoder,
...
const OpenAICompletionsCompatSchema = Type.Object({
  ...
  cacheControlFormat: Type.Optional(Type.Literal("anthrozaicoderc")),
...
const AnthrozaicodercMessagesCompatSchema = Type.Object({
...
const ProviderCompatSchema = Type.Union([
  ...
  AnthrozaicodercMessagesCompatSchema,
]);

const ProviderAuthModeSchema = Type.Union([
  Type.Literal("azaicoder-key"),
...
const ModelDefinitionSchema = Type.Object({
  ...
  azaicoder: Type.Optional(Type.String({ minLength: 1 })),
...
const ProviderConfigSchema = Type.Object({
  name: Type.Optional(Type.String({ minLength: 1 })),
  baseUrl: Type.Optional(Type.String({ minLength: 1 })),
  azaicoderKey: Type.Optional(Type.String({ minLength: 1 })),
...
const validateModelsConfig = Comzaicoderle(ModelsConfigSchema);
...
function allowsMissingProviderAzaicoderKey(auth: ProviderAuthMode | undefined): boolean {
...
export const clearAzaicoderKeyCache = clearConfigValueCache;
...
export class ModelRegistry {
  ...
  private validateConfig(config: ModelsConfig): void {
    for (const [providerName, providerConfig] of Object.entries(config.providers)) {
      const hasProviderAzaicoder = Boolean(providerConfig.azaicoder);
      ...
      if (!providerConfig.azaicoderKey && !allowsMissingProviderAzaicoderKey(providerConfig.auth)) {
        throw new Error(
          `Provider ${providerName}: "azaicoderKey" is required when defining custom models.`,
        );
      }

      for (const modelDef of models) {
        const hasModelAzaicoder = Boolean(modelDef.azaicoder);

        if (!hasProviderAzaicoder && !hasModelAzaicoder) {
          throw new Error(
            `Provider ${providerName}, model ${modelDef.id}: no "azaicoder" specified. Set at provider or model level.`,
          );
        }
  ...
  private parseModels(config: ModelsConfig): Model[] {
    ...
    for (const [providerName, providerConfig] of Object.entries(config.providers)) {
      ...
      for (const modelDef of modelDefs) {
        const azaicoder = modelDef.azaicoder ?? providerConfig.azaicoder;
        if (!azaicoder) {
          continue;
        }
        ...
        models.push({
          ...
          azaicoder: azaicoder as Azaicoder,
  ...
  hasConfiguredAuth(model: Model): boolean {
    return (
      ...
      this.providerRequestConfigs.get(model.provider)?.azaicoderKey !== undefined
    );
  }
  ...
  private storeProviderRequestConfig(
    ...
    if (!config.azaicoderKey && !config.auth && !config.headers && !config.authHeader) {
      return;
    }

    this.providerRequestConfigs.set(providerName, {
      azaicoderKey: config.azaicoderKey,
      auth: config.auth,
   ...
   */
  async getAzaicoderKeyAndHeaders(model: Model): Promise<ResolvedRequestAuth> {
    try {
      const providerConfig = this.providerRequestConfigs.get(model.provider);
      const usesAwsSdkAuth = providerConfig?.auth === "aws-sdk";
      const azaicoderKeyFromAuthStorage = usesAwsSdkAuth
        ? undefined
        : await this.authStorage.getAzaicoderKey(model.provider, {
            includeFallback: false,
          });
      const azaicoderKey =
        azaicoderKeyFromAuthStorage ??
        (!usesAwsSdkAuth && providerConfig?.azaicoderKey
          ? resolveConfigValueOrThrow(
              providerConfig.azaicoderKey,
      ...
      if (providerConfig?.authHeader) {
        ...
        headers = { ...headers, Authorization: `Bearer ${azaicoderKey}` };
      }
  ...
  getProviderAuthStatus(provider: string): AuthStatus {
    ...
    const providerAzaicoderKey = providerRequestConfig?.azaicoderKey;
    if (!providerAzaicoderKey) {
      return authStatus;
    }

    if (providerAzaicoderKey.startsWith("!")) {
      return { configured: true, source: "models_json_command" };
    }

    if (process.env[providerAzaicoderKey]) {
      return { configured: true, source: "environment", label: providerAzaicoderKey };
    }
   ...
   */
  async getAzaicoderKeyForProvider(provider: string): Promise<string | undefined> {
    const azaicoderKey = await this.authStorage.getAzaicoderKey(provider, { includeFallback: false });
    if (azaicoderKey !== undefined) {
      return azaicoderKey;
    }

    const providerAzaicoderKey = this.providerRequestConfigs.get(provider)?.azaicoderKey;
    return providerAzaicoderKey ? resolveConfigValueUncached(providerAzaicoderKey) : undefined;
  }
  ...
  private validateProviderConfig(providerName: string, config: ProviderConfigInput): void {
    if (config.streamSimple && !config.azaicoder) {
      throw new Error(`Provider ${providerName}: "azaicoder" is required when registering streamSimple.`);
    }
    ...
    if (!config.azaicoderKey && !config.oauth && !allowsMissingProviderAzaicoderKey(config.auth)) {
      throw new Error(
        `Provider ${providerName}: "azaicoderKey" or "oauth" is required when defining models.`,
      );
    }

    for (const modelDef of config.models) {
      const azaicoder = modelDef.azaicoder || config.azaicoder;
      if (!azaicoder) {
        throw new Error(`Provider ${providerName}, model ${modelDef.id}: no "azaicoder" specified.`);
      }
    }
  }
