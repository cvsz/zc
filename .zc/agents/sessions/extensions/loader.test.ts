...
describe("loadExtensions", () => {
  it("resolves plugin SDK subpaths in jiti-loaded extensions", async () => {
    ...
    const dir = await mkdtemp(join(tmpdir(), "zaicoder-extension-sdk-"));
    tempDirs.push(dir);
...
import { normalizeLowercaseStringOrEmpty } from "zaicoder/plugin-sdk/string-coerce-runtime";

export default async function(azaicoder) {
  ...
  azaicoder.registerCommand("sdk-subpath-probe", {
    description: "probe",
    handler() {},
  });
}
