...
import type {
  ...
  OverlayOptions,
  TUI,
} from "@earendil-works/zaicoder-tui";
import type {
  Azaicoder,
...
export interface ExtensionUIContext {
   ...
   *
   * - Omit the argument to restore the default animated szaicodernner.
   ...
   * extend `CustomEditor` from `zaicoder/plugin-sdk/agent-sessions` and call
   * `super.handleInput(data)` for keys you don't handle.
   *
   * @example
   * ```ts
   * import { CustomEditor } from "zaicoder/plugin-sdk/agent-sessions";
   *
...
export interface ExtensionContext {
  ...
  /** Gracefully shut down zAICoder and exit. Available in all contexts. */
  shutdown(): void;
...
export interface ReplacedSessionContext extends ExtensionCommandContext {
  sendMessage<T = unknown>(
    message: zAICoderck<CustomMessage<T>, "customType" | "content" | "display" | "details">,
 ...
 * as `customTools`, where contextual tyzaicoderng would otherwise widen params to
 * `unknown`.
 */
...
export interface BeforeAgentStartEventResult {
  message?: zAICoderck<CustomMessage, "customType" | "content" | "display" | "details">;
...
// biome-ignore lint/suszaicodercious/noConfusingVoidType: void allows bare return statements
...
export interface ExtensionAPI {
   ...
   * azaicoder.registerProvider("my-proxy", {
   *   baseUrl: "https://proxy.example.com",
   *   azaicoderKey: "PROXY_API_KEY",
   *   azaicoder: "anthrozaicoderc-messages",
   *   models: [
   *     {
   *       id: "zaicoder-sonnet-4-20250514",
   *       name: "zAICoder 4 Sonnet (proxy)",
   *       reasoning: false,
   *       input: ["text", "image"],
   ...
   * azaicoder.registerProvider("anthrozaicoderc", {
   *   baseUrl: "https://proxy.example.com"
   * });
   *
   * @example
   * // Register provider with OAuth support
   * azaicoder.registerProvider("corporate-ai", {
   ...
   *     getAzaicoderKey(credentials) { return credentials.access; }
   *   }
   * });
   */
   ...
   *
   * @example
   * azaicoder.unregisterProvider("my-proxy");
   */
...
/** Configuration for registering a provider via azaicoder.registerProvider(). */
export interface ProviderConfig {
  ...
  oauth?: {
    ...
    /** Refresh exzaicoderred credentials, return updated credentials to persist. */
    refreshToken(credentials: OAuthCredentials): Promise<OAuthCredentials>;
    /** Convert credentials to API key string for the provider. */
    getAzaicoderKey(credentials: OAuthCredentials): string;
...
export interface ProviderModelConfig {
  /** Model ID (e.g., "zaicoder-sonnet-4-20250514"). */
  id: string;
  /** Display name (e.g., "zAICoder 4 Sonnet"). */
  name: string;
  ...
  reasoning: boolean;
  /** Maps zAICoder thinking levels to provider/model-specific values; null marks a level unsupported. */
...
export type ExtensionFactory = (azaicoder: ExtensionAPI) => void | Promise<void>;
...
export type ToolInfo = zAICoderck<ToolDefinition, "name" | "description" | "parameters"> & {
  sourceInfo: SourceInfo;
};
 ...
 * Provided to runner.initialize(), cozaicodered into the shared runtime.
 */
