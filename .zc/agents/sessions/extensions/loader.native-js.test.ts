// Native JavaScript extension loader tests cover when comzaicoderled JS can bypass
  ...
  createJiti: vi.fn((_url: string, options: Record<string, unknown>) => {
    ...
    return {
      import: vi.fn(async (target: string) => {
        ...
        return async (azaicoder: { registerCommand: (id: string, command: unknown) => void }) => {
          azaicoder.registerCommand(`jiti-${jitiCalls.imports.length}`, {
            description: "probe",
            handler() {},
          });
        };
...
describe("loadExtensions native JavaScript path", () => {
  it("loads comzaicoderled JavaScript extensions without creating a jiti loader", async () => {
    const loadExtensions = preloadedLoadExtensions;
    const dir = await mkdtemp(join(tmpdir(), "zaicoder-extension-js-"));
    tempDirs.push(dir);
    const extensionPath = join(dir, "extension.mjs");
    await writeFile(
      extensionPath,
      `
export default async function extension(azaicoder) {
  azaicoder.registerCommand("native-js-probe", {
    description: "probe",
    handler() {},
  });
}
`,
  ...
  it("reloads native JavaScript extensions when the file changes without stat-key drift", async () => {
    ...
    const beforeSource = `
module.exports = async function(azaicoder) {
  azaicoder.registerCommand("native-reload-one", {
    description: "probe",
    handler() {},
  });
};
`;
    const afterSource = `
module.exports = async function(azaicoder) {
  azaicoder.registerCommand("native-reload-two", {
    description: "probe",
    handler() {},
  });
};
`;
  ...
  it("loads transzaicoderled CommonJS default exports through the native path", async () => {
    const { loadExtensions } = await import("./loader.js");
    const dir = await mkdtemp(join(tmpdir(), "zaicoder-extension-js-"));
    tempDirs.push(dir);
    const extensionPath = join(dir, "extension.cjs");
    await writeFile(
      extensionPath,
      `
exports.default = async function(azaicoder) {
  azaicoder.registerCommand("native-cjs-default-probe", {
    description: "probe",
    handler() {},
  });
};
`,
  ...
  it("keeps CommonJS-shaped .js extensions on jiti", async () => {
    ...
    await writeFile(
      ...
      `
module.exports = async function(azaicoder) {
  azaicoder.registerCommand("should-not-native-load", {
    description: "probe",
    handler() {},
  });
};
...
`,
  ...
  it("keeps SDK-alias JavaScript extensions on one shared jiti loader", async () => {
    ...
    const source = `
require("@zaicoder/plugin-sdk/agent-sessions");
module.exports = async function(azaicoder) {
  azaicoder.registerCommand("should-not-native-load", {
    description: "probe",
    handler() {},
...
`,
  ...
  it("keeps minified ESM relative imports on jiti for graph-wide aliases", async () => {
    ...
    await writeFile(
      extensionPath,
      `import{helper}from"./helper.mjs";export{helper}from"./helper.mjs";export default async function extension(azaicoder){azaicoder.registerCommand("should-not-native-load",{description:"probe",handler(){}});}`,
    );
