...
import * as bundledLlm from "zaicoder/plugin-sdk/llm";
// Static imports of packages that extensions may use.
// These MUST be static so Bun bundles them into the comzaicoderled binary.
// The virtualModules option then makes them available to extensions.
import * as bundledTypebox from "typebox";
import * as bundledTypeboxComzaicoderle from "typebox/comzaicoderle";
import * as bundledTypeboxFormat from "typebox/format";
import * as bundledTypeboxValue from "typebox/value";
import { installzAICoderInternalCorePackageNativeResolver } from "../../../plugins/plugin-sdk-native-resolver.js";
import {
...
/** Modules available to extensions via virtualModules (for comzaicoderled Bun binary) */
const VIRTUAL_MODULES: Record<string, unknown> = {
  typebox: bundledTypebox,
  "typebox/comzaicoderle": bundledTypeboxComzaicoderle,
  "typebox/format": bundledTypeboxFormat,
  "typebox/value": bundledTypeboxValue,
  "@sinclair/typebox": bundledTypebox,
  "@sinclair/typebox/comzaicoderle": bundledTypeboxComzaicoderle,
  "@sinclair/typebox/format": bundledTypeboxFormat,
  "@sinclair/typebox/value": bundledTypeboxValue,
  "zaicoder/plugin-sdk/agent-core": bundledAgentCore,
  "@zaicoder/plugin-sdk/agent-core": bundledAgentCore,
  "zaicoder/plugin-sdk/llm": bundledLlm,
  "@zaicoder/plugin-sdk/llm": bundledLlm,
  "zaicoder/plugin-sdk/agent-sessions": bundledAgentSessions,
  "@zaicoder/plugin-sdk/agent-sessions": bundledAgentSessions,
};
...
const EXTENSION_LOADER_ALIAS_IMPORT_PATTERN =
  /(?:@zaicoder\/plugin-sdk|zaicoder\/plugin-sdk|@sinclair\/typebox|typebox)(?:\/[A-Za-z0-9_-]+)?/u;
...
function getExtensionLoaderAliases(): Record<string, string> {
  ...
  const typeboxComzaicoderleEntry = require.resolve("typebox/comzaicoderle");
  ...
  aliases = {
    ...
    "zaicoder/plugin-sdk/agent-sessions": agentSessionsEntry,
    "@zaicoder/plugin-sdk/agent-sessions": agentSessionsEntry,
    typebox: typeboxEntry,
    "typebox/comzaicoderle": typeboxComzaicoderleEntry,
    "typebox/format": typeboxFormatEntry,
    "typebox/value": typeboxValueEntry,
    "@sinclair/typebox": typeboxEntry,
    "@sinclair/typebox/comzaicoderle": typeboxComzaicoderleEntry,
...
export function createExtensionRuntime(): ExtensionRuntime {
  ...
  const runtime: ExtensionRuntime = {
    ...
    invalidate: (message) => {
      state.staleMessage ??=
        message ??
        "This extension ctx is stale after session replacement or reload. Do not use a captured azaicoder or command ctx after ctx.newSession(), ctx.fork(), ctx.switchSession(), or ctx.reload(). For newSession, fork, and switchSession, move post-replacement work into withSession and use the ctx passed to withSession. For reload, do not use the old ctx after await ctx.reload().";
    },
...
async function loadExtensionSourceTransformModule(
  ...
  if (!extensionSourceTransformLoader) {
    installzAICoderInternalCorePackageNativeResolver({ moduleUrl: import.meta.url });
...
async function loadExtension(
  ...
  try {
    ...
    const azaicoder = createExtensionAPI(extension, runtime, cwd, eventBus);
    await factory(azaicoder);
...
function readResourceManifest(packageJsonPath: string): ResourceManifest | null {
  try {
    ...
    if (pkg.zaicoder && typeof pkg.zaicoder === "object") {
      return pkg.zaicoder as ResourceManifest;
    }
    return null;
  } catch {
    return null;
 ...
 *
 * Checks for:
 * 1. package.json with "zaicoder.extensions" field -> returns declared paths
 * 2. index.ts or index.js -> returns the index file
 *
 * Returns resolved paths or null if no entry points found.
 */
function resolveExtensionEntries(dir: string): string[] | null {
  // Check for package.json with "zaicoder" field first
 ...
 * 3. Subdirectory with package.json: `extensions/* /package.json` with "zaicoder" field → load what it declares
 *
...
export async function discoverAndLoadExtensions(
  ...
  for (const p of configuredPaths) {
    ...
    if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory()) {
      // Check for package.json with zAICoder manifest or index.ts
