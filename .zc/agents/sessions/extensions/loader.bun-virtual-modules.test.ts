...
vi.mock("jiti/static", () => ({
  createJiti: vi.fn((_url: string, options: Record<string, unknown>) => {
    ...
    return {
      import: vi.fn(
        async () => async (azaicoder: { registerCommand: (id: string, command: unknown) => void }) => {
          azaicoder.registerCommand("bun-virtual-module-probe", {
            description: "probe",
            handler() {},
          });
        },
  ...
  it("virtualizes scoped and unscoped SDK module ids", async () => {
    ...
    expect(virtualModulesCase.virtualModuleIds).toEqual(
      expect.arrayContaining([
        "zaicoder/plugin-sdk/agent-core",
        "@zaicoder/plugin-sdk/agent-core",
        "zaicoder/plugin-sdk/llm",
        "@zaicoder/plugin-sdk/llm",
