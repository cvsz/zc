...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-tool-result-replay-"));
  tempPaths.push(dir);
  return dir;
}
...
function makeAnthrozaicodercModel(): Model<"anthrozaicoderc-messages"> {
  return {
...
describe("SessionManager tool-result replay", () => {
  ...
  it("replays string assistant JSONL content as Anthrozaicoderc assistant text", async () => {
    const dir = await makeTempDir();
    const sessionFile = path.join(dir, "session.jsonl");
    ...
    const stream = streamAnthrozaicoderc(makeAnthrozaicodercModel(), toLlmContext(context), {
      azaicoderKey: "sk-ant-provider",
      onPayload: (payload) => {
  ...
  it("replays string tool-result JSONL content as Anthrozaicoderc tool text", async () => {
    const dir = await makeTempDir();
    const sessionFile = path.join(dir, "session.jsonl");
    await writeSessionWithToolResultContent(sessionFile, "lookup result text");
    const context = SessionManager.open(
  ...
  it("replays object tool-result JSONL content as structured Anthrozaicoderc tool text", async () => {
    const dir = await makeTempDir();
    const sessionFile = path.join(dir, "session.jsonl");
