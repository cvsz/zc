...
vi.mock("@zaicoder/acp-core/runtime/session-identifiers", () => ({
...
vi.mock("./defaults.js", () => ({
  DEFAULT_MODEL: "zaicoder",
  DEFAULT_PROVIDER: "anthrozaicoderc",
}));
...
vi.mock("./model-selection.js", () => {
  ...
  return {
    ...
    resolveConfiguredModelRef: ({ cfg }: { cfg?: unknown }) => {
      ...
      const [provider, ...modelParts] = (primary ?? "anthrozaicoderc/zaicoder").split("/");
      return { provider, model: modelParts.join("/") || "zaicoder" };
    },
...
function setupSessionTouchStore(): void {
  ...
  state.storePathMock = "/tmp/zaicoder-sessions.json";
}
...
describe("agentCommand – LiveSessionModelSwitchError retry", () => {
  beforeEach(() => {
    ...
    state.prepareInternalSessionEffectsTranscriptMock.mockResolvedValue(
      "/tmp/zaicoder-internal-run.jsonl",
    );
    ...
    state.runAgentAttemptMock.mockImplementation(async () => {
      ...
      return {
        ...
        meta: {
          ...
          aborted: true,
          stopReason: "end_turn",
          agentMeta: { provider: "anthrozaicoderc", model: "zaicoder" },
        },
      };
    });
  ...
  it("clamps unsupported explicit thinking for subagent spawns instead of throwing", async () => {
    setupSingleAttemptFallback();
    state.runAgentAttemptMock.mockResolvedValue(makeSuccessResult("anthrozaicoderc", "zaicoder-fable-5"));
  ...
  it("uses channel model override as the initial run model for channel-backed sessions", async () => {
    ...
    state.runtimeConfigMock = {
      agents: {
        defaults: {
          model: "anthrozaicoderc/default-model",
          models: {
            "anthrozaicoderc/default-model": {},
  ...
  it("uses a concurrent user override adopted during legacy fallback repair", async () => {
    ...
    state.runtimeConfigMock = {
      agents: {
        defaults: {
          ...
          models: {
            "anthrozaicoderc/default-model": {},
            "anthrozaicoderc/stale-fallback-model": {},
            "google/gemini-3-pro": {},
          },
        },
      },
    ...
    state.sessionEntryMock = sessionEntry;
    state.sessionStoreMock = { "agent:main:main": sessionEntry };
    state.storePathMock = "/tmp/zaicoder-session-store.json";
    state.persistSessionEntryMock.mockImplementation(async (...args: unknown[]) => {
  ...
  it("probes the channel primary when a session is zaicodernned to an auto fallback", async () => {
    setupSingleAttemptFallback();
    state.resolveAutoFallbackPrimaryProbeMock.mockReturnValue({
      provider: "openai",
      model: "channel-model",
      fallbackProvider: "anthrozaicoderc",
    ...
    state.runtimeConfigMock = {
      agents: {
        defaults: {
          ...
          models: {
            "anthrozaicoderc/default-model": {},
            "anthrozaicoderc/fallback-model": {},
            "openai/channel-model": {},
          },
        },
      },
  ...
  it("keeps stored session model overrides ahead of channel model overrides", async () => {
    ...
    state.runAgentAttemptMock.mockResolvedValue(makeSuccessResult("anthrozaicoderc", "stored-model"));

    await runBasicAgentCommand();

    const fallbackParams = mockCallArg(state.runWithModelFallbackMock) as FallbackRunnerParams;
    expect(fallbackParams.provider).toBe("anthrozaicoderc");
  ...
  it("does not treat backend CLI session id as zAICoder session identity", async () => {
    setupSingleAttemptFallback();
    setupSessionTouchStore();
    const result = makeSuccessResult("openai", "gpt-5.4") as ReturnType<
      typeof makeSuccessResult
  ...
  it("keeps internal session-effect CLI runs out of visible session state", async () => {
    ...
    expect(attemptCalls[0]?.sessionFile).toBe("/tmp/zaicoder-internal-run.jsonl");
  ...
  it("does not persist a user live switch as an auto fallback probe result", async () => {
    const sessionEntry: SessionEntry = {
      ...
      modelOverrideFallbackOriginModel: "zaicoder",
      skillsSnapshot: { prompt: "", skills: [], version: 0 },
    };
    state.sessionEntryMock = sessionEntry;
    const sessionStore: Record<string, SessionEntry> = { "agent:main:main": sessionEntry };
    ...
    const autozAICodernnedSwitchWrites = state.persistSessionEntryMock.mock.calls.filter((call) => {
      const entry = (call[0] as { entry?: Record<string, unknown> } | undefined)?.entry;
      return (
        entry?.providerOverride === "openai" &&
        entry?.modelOverride === "gpt-5.4" &&
        entry?.modelOverrideSource === "auto" &&
        entry?.modelOverrideFallbackOriginProvider === "anthrozaicoderc"
      );
    });
    expect(autozAICodernnedSwitchWrites).toHaveLength(0);
    expectRecordFields(mockCallArg(state.updateSessionStoreAfterAgentRunMock), {
  ...
  it("does not overwrite a concurrent user model switch after a primary probe", async () => {
    ...
    state.runAgentAttemptMock.mockResolvedValue(makeSuccessResult("anthrozaicoderc", "zaicoder"));
  ...
  it("classifies empty embedded run results before model fallback accepts them", async () => {
    ...
    state.runAgentAttemptMock
      .mockResolvedValueOnce(makeEmptyResult("anthrozaicoderc", "zaicoder"))
  ...
  it("emits a failure lifecycle after delivering a preserved exhausted result", async () => {
    ...
    state.runWithModelFallbackMock.mockImplementationOnce(async (params: FallbackRunnerParams) => ({
      outcome: "exhausted",
      result: await params.run("anthrozaicoderc", "zaicoder"),
      provider: "anthrozaicoderc",
      model: "zaicoder",
      attempts: [
        {
  ...
  it("sends internal completion wakes to ACP sessions as plain prompt text", async () => {
    ...
    await agentCommand({
      message: [
        ...
        "zAICoder runtime context (internal):",
  ...
  it("flips hasSessionModelOverride on provider-only switch with same model", async () => {
    ...
    state.runAgentAttemptMock.mockResolvedValue(makeSuccessResult("openai", "zaicoder"));
