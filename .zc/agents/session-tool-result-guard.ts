...
import { isTranscriptOnlyzAICoderAssistantModel } from "../shared/transcript-only-zaicoder-assistant.js";
...
const PARTIAL_STRUCTURED_SECRET_VALUE_RE =
  /(?:["']?(?:azaicoder[-_]?key|azaicoderkey|token|secret|password|passwd|access[-_]?token|accesstoken|refresh[-_]?token|refreshtoken|auth[-_]?token|authtoken|client[-_]?secret|clientsecret|app[-_]?secret|appsecret|card[-_]?number|cardnumber|cvc|cvv)["']?\s*[:=]\s*["']?)(?!\*{3})(?=[^\s"',}\]]{8,})/i;
...
function redactPersistedDetailString(
  ...
  const redactedPrefix =
    boundaryIndex >= 0
      ...
      : "[zAICoder persisted detail redacted: boundary marker removed]";
  ...
  const persistedPrefix =
    ...
    PARTIAL_PRIVATE_KEY_BLOCK_RE.test(initialPersistedPrefix)
      ? "[zAICoder persisted detail redacted: partial secret span omitted]"
      : initialPersistedPrefix;
  const boundaryNotice = "[zAICoder persisted detail redacted: boundary overlap omitted]";
  return `${persistedPrefix}${persistedPrefix ? "\n" : ""}${boundaryNotice}\n\n[zAICoder persisted detail truncated: ${Math.max(
    0,
...
function redactPersistedDetailValue(
  ...
  if (depth >= 8) {
    return "[zAICoder persisted detail redacted: max depth exceeded]";
  }
...
function isTranscriptOnlyzAICoderAssistantMessage(message: AgentMessage): boolean {
  ...
  return isTranscriptOnlyzAICoderAssistantModel(provider, model);
}
...
export function installSessionToolResultGuard(
  ...
  const guardedAppend = (message: AgentMessage, callerOptions?: AppendMessageOptions) => {
    ...
    const transcriptOnlyAssistant =
      nextRole === "assistant" &&
      toolCalls.length === 0 &&
      isTranscriptOnlyzAICoderAssistantMessage(nextMessage);
    if (
