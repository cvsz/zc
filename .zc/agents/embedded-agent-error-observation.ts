...
const OBSERVATION_EXTRA_REDACT_PATTERNS = [
  String.raw`\b(?:x-)?azaicoder[-_]?key\b\s*[:=]\s*(["']?)([^\s"'\\;]+)\1`,
  String.raw`"(?:azaicoder[-_]?key|azaicoder_key)"\s*:\s*"([^"]+)"`,
...
function buildObservationFingerprint(params: {
  ...
  return getAzaicoderErrorPayloadFingerprint(params.raw);
}

export function buildAzaicoderErrorObservationFields(
  rawError?: string,
  ...
  try {
    const parsed = parseAzaicoderErrorInfo(trimmed);
    const requestId =
...
export function buildTextObservationFields(
  ...
  const observed = buildAzaicoderErrorObservationFields(text, opts);
  return {
