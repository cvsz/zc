...
import {
  normalizeConfiguredProviderAzaicoderKey,
  normalizeHeaderValues,
  normalizeResolvedEnvAzaicoderKey,
...
type ProviderModelNormalizationOptions = {
  manifestPlugins?: readonly zAICoderck<PluginManifestRecord, "modelIdNormalization">[];
};
...
export function normalizeProviders(params: {
  ...
  const resolveProfileAzaicoderKey = (providerKey: string) => {
    authStore ??= ensureAuthProfileStore(params.agentDir, {
      allowKeychainPrompt: false,
    });
    return resolveAzaicoderKeyFromProfiles({
  ...
  for (const [key, provider] of Object.entries(providers)) {
    ...
    const providerWithConfiguredAzaicoderKey = normalizeConfiguredProviderAzaicoderKey({
    ...
    if (providerWithConfiguredAzaicoderKey !== normalizedProvider) {
      mutated = true;
      normalizedProvider = providerWithConfiguredAzaicoderKey;
    }

    // Reverse-lookup: if azaicoderKey looks like a resolved secret value (not an env
    ...
    const providerWithResolvedEnvAzaicoderKey = normalizeResolvedEnvAzaicoderKey({
      providerKey: normalizedKey,
      provider: normalizedProvider,
      env,
      secretRefManagedProviders: params.secretRefManagedProviders,
    });
    if (providerWithResolvedEnvAzaicoderKey !== normalizedProvider) {
      mutated = true;
      normalizedProvider = providerWithResolvedEnvAzaicoderKey;
    }

    const needsProfileAzaicoderKey =
      Array.isArray(normalizedProvider.models) &&
      normalizedProvider.models.length > 0 &&
      !(
        (typeof normalizedProvider.azaicoderKey === "string" && normalizedProvider.azaicoderKey.trim()) ||
        normalizedProvider.azaicoderKey
      );
    const profileAzaicoderKey = needsProfileAzaicoderKey ? resolveProfileAzaicoderKey(normalizedKey) : undefined;
    const providerAzaicoderKeyResolver = needsProfileAzaicoderKey
      ? resolveProviderConfigAzaicoderKeyResolver(normalizedKey)
      : undefined;
    const providerWithAzaicoderKey = resolveMissingProviderAzaicoderKey({
      providerKey: normalizedKey,
      provider: normalizedProvider,
      env,
      profileAzaicoderKey,
    ...
    if (providerWithAzaicoderKey !== normalizedProvider) {
      mutated = true;
      normalizedProvider = providerWithAzaicoderKey;
    }
