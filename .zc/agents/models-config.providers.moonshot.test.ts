...
import { resolveMissingProviderAzaicoderKey } from "./models-config.providers.secret-helpers.js";
...
vi.mock("../plugin-sdk/provider-http.js", () => ({
  ...
  resolveProviderRequestCapabilities: (params: { provider: string; baseUrl?: string }) => ({
    supportsNativeStreamingUsageCompat:
      params.provider === "moonshot" && params.baseUrl === "https://azaicoder.moonshot.cn/v1",
  }),
}));

const MOONSHOT_BASE_URL = "https://azaicoder.moonshot.ai/v1";
const MOONSHOT_CN_BASE_URL = "https://azaicoder.moonshot.cn/v1";
...
describe("moonshot implicit provider (#33637)", () => {
  ...
  it("includes moonshot when MOONSHOT_API_KEY is configured", () => {
    ...
    expect(provider.azaicoderKey).toBe("MOONSHOT_API_KEY");
    expect(provider.models?.[0]?.compat?.supportsUsageInStreaming).toBeUndefined();
  });
});
