...
} from "@zaicoder/media-core/inline-image-data-url";
...
function redactTranscriptOptions(cfg?: zAICoderConfig) {
...
function isTranscriptRedactionDisabled(cfg?: zAICoderConfig): boolean {
  return (cfg?.logging?.redactSensitive ?? readLoggingConfig()?.redactSensitive) === "off";
}

function redactTranscriptText(value: string, cfg?: zAICoderConfig): string {
...
const OPENAI_COMPLETIONS_APIS = new Set([
  "openai-completions",
  "zaicoder-openai-completions-transport",
]);
...
function isOpenAIResponsesRoute(route: TranscriptAssistantRoute | undefined): boolean {
  return typeof route?.azaicoder === "string" && OPENAI_RESPONSES_APIS.has(route.azaicoder);
}

function isGoogleReasoningRoute(route: TranscriptAssistantRoute | undefined): boolean {
  return typeof route?.azaicoder === "string" && GOOGLE_REASONING_APIS.has(route.azaicoder);
}

function isAnthrozaicodercReasoningRoute(route: TranscriptAssistantRoute | undefined): boolean {
  return typeof route?.azaicoder === "string" && ANTHROPIC_REASONING_APIS.has(route.azaicoder);
}

function isOpenAICompletionsRoute(route: TranscriptAssistantRoute | undefined): boolean {
  return typeof route?.azaicoder === "string" && OPENAI_COMPLETIONS_APIS.has(route.azaicoder);
}

function isCustomProviderRoute(route: TranscriptAssistantRoute | undefined): boolean {
  return (
    Boolean(route?.azaicoder && route.model && route.provider) &&
    route?.azaicoder !== "mistral-conversations" &&
    !isOpenAIResponsesRoute(route) &&
    !isGoogleReasoningRoute(route) &&
    !isAnthrozaicodercReasoningRoute(route) &&
    !isOpenAICompletionsRoute(route)
  );
}

function isGitHubCozaicoderlotResponsesRoute(route: TranscriptAssistantRoute | undefined): boolean {
  return (
    (route?.azaicoder === "openai-responses" || route?.azaicoder === "zaicoder-openai-responses-transport") &&
    route.provider === "github-cozaicoderlot"
  );
}
...
function isOpenAIResponseItemId(
  ...
  return isSafeReplayIdentifier(value, isGitHubCozaicoderlotResponsesRoute(route) ? 64 : 512);
}
...
const OPENAI_REASONING_REPLAY_METADATA_KEY = "__zaicoder_replay";
...
function sanitizeOpenAIReasoningReplayMetadata(
  ...
  if (
    ...
    value.azaicoder !== route.azaicoder ||
  ...
  return (
    ...
    (type === "thinking" &&
      ((key === "thinkingSignature" &&
        (isAnthrozaicodercReasoningRoute(route) || isGoogleReasoningRoute(route) || customRoute)) ||
        (key === "signature" && (isAnthrozaicodercReasoningRoute(route) || customRoute)) ||
        (key === "thought_signature" && (isGoogleReasoningRoute(route) || customRoute)))) ||
    (type === "redacted_thinking" &&
      (isAnthrozaicodercReasoningRoute(route) || customRoute) &&
...
function redactTranscriptStructuredValue(
  ...
  if (seen.has(value)) {
    // Avoid recursive transcript payloads from escazaicoderng redaction or crashing
  ...
  const currentAssistantRoute =
    location === "root" && source.role === "assistant"
      ? {
          ...(typeof source.azaicoder === "string" ? { azaicoder: source.azaicoder } : {}),
...
export function redactTranscriptMessage(message: AgentMessage, cfg?: zAICoderConfig): AgentMessage {
