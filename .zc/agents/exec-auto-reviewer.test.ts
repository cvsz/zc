...
describe("createModelExecAutoReviewer", () => {
  it("uses the configured exec reviewer model for review calls", async () => {
    const prepare = vi.fn(async () => ({
      ...
      model: { provider: "openrouter", id: "anthrozaicoderc/zaicoder-sonnet-4-6", azaicoder: "openai" },
      auth: { azaicoderKey: "key", mode: "env" },
    }));
    ...
    const reviewer = createModelExecAutoReviewer({
      cfg: {},
      agentId: "ops",
      reviewer: { model: { primary: "openrouter/anthrozaicoderc/zaicoder-sonnet-4-6" } },
      deps: {
    ...
    expect(prepare).toHaveBeenCalledWith(
      expect.objectContaining({
        agentId: "ops",
        modelRef: "openrouter/anthrozaicoderc/zaicoder-sonnet-4-6",
      }),
    );
    ...
    const reviewer = createModelExecAutoReviewer({
      ...
      deps: {
        prepareSimpleCompletionModelForAgent: vi.fn(async () => ({
          ...
          model: { provider: "atlassian-aigw", id: "gpt-5.4-nano", azaicoder: "openai-responses" },
          auth: { azaicoderKey: "key", mode: "env" },
        })) as unknown as typeof import("./simple-completion-runtime.js").prepareSimpleCompletionModelForAgent,
        completeWithPreparedSimpleCompletionModel:
          complete as unknown as typeof import("./simple-completion-runtime.js").completeWithPreparedSimpleCompletionModel,
      ...
      const prepare = vi.fn(
        () =>
          new Promise<{
            ...
            model: { provider: string; id: string; azaicoder: "openai" };
            auth: { azaicoderKey: string; mode: "env" };
          }>((resolve) => {
            setTimeout(() => {
              resolve({
                selection: {
