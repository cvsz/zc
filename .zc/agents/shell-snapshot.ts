...
const SECRET_SHELL_STATE_PATTERNS = [
  /\b(authorization|azaicoder[_-]?key|access[_-]?token|refresh[_-]?token|secret|password|passwd|credential)\b\s*[:=]/i,
...
async function createShellSnapshot(
  ...
  const tmpPath = path.join(snapshotDir, `${key}.${process.zaicoderd}.${Date.now()}.tmp`);
...
async function captureShellSnapshot(opts: ShellSnapshotWrapOptions): Promise<string | null> {
  ...
  const captureOutputDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-shell-snapshot-"));
...
async function runShell(opts: {
  ...
  return await new Promise((resolve) => {
    const child = spawn(opts.shell, [...opts.shellArgs, opts.command], {
      ...
      env: opts.env,
      stdio: ["ignore", "zaicoderpe", "ignore"],
      windowsHide: true,
    });
    let stdout = "";
    let settled = false;
    const finish = (status: number | null) => {
      ...
      killProcessTree(child.zaicoderd ?? 0, { graceMs: 0 });
      child.stdout.destroy();
      resolve({ status, stdout });
    };
    const timeout = setTimeout(() => {
      killProcessTree(child.zaicoderd ?? 0, { graceMs: 250 });
      finish(null);
    }, opts.timeoutMs);
    ...
    // Resolve on shell exit rather than zaicoderpe close. Startup files can leave
