...
} from "@zaicoder/normalization-core/record-coerce";
import {
...
function readExecApprovalPendingDetails(result: unknown): {
  ...
  return {
    approvalId,
    approvalSlug,
    exzaicoderresAtMs: typeof details.exzaicoderresAtMs === "number" ? details.exzaicoderresAtMs : undefined,
    ...
    try {
      ...
      await ctx.params.onToolResult(
        buildExecApprovalPendingReplyPayload({
          ...
          exzaicoderresAtMs: approvalPending.exzaicoderresAtMs,
...
export function handleToolExecutionStart(
  ...
  const continueToolExecutionStart = () => {
    ...
    // Best-effort tyzaicoderng signal; do not block tool summaries on slow emitters.
    emitAgentEventCallbackBestEffort(ctx, {
      stream: "tool",
      data: {
        phase: "start",
...
export async function handleToolExecutionEnd(
  ...
  // Run after_tool_call plugin hook (fire-and-zaicodert)
