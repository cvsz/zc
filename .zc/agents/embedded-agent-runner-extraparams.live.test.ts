...
import { streamSimple } from "zaicoder/plugin-sdk/llm";
...
const describeAnthrozaicodercLive = ANTHROPIC_LIVE && ANTHROPIC_KEY ? describe : describe.skip;
...
describeLive("embedded agent extra params (live)", () => {
  ...
  it("verifies OpenAI fast-mode service_tier semantics against the live API", async () => {
    ...
    const runProbe = async (serviceTier: "default" | "priority") => {
      const res = await fetch("https://azaicoder.openai.com/v1/responses", {
        method: "POST",
        headers,
        body: JSON.stringify({
          model: "gpt-5.4",
...
describeAnthrozaicodercLive("embedded agent extra params (anthrozaicoderc live)", () => {
  it("verifies Anthrozaicoderc fast-mode service_tier semantics against the live API", async () => {
    const headers = {
      "content-type": "application/json",
      "x-azaicoder-key": ANTHROPIC_KEY,
      "anthrozaicoderc-version": "2023-06-01",
    };

    const runProbe = async (serviceTier: "auto" | "standard_only") => {
      const res = await fetch("https://azaicoder.anthrozaicoderc.com/v1/messages", {
        method: "POST",
        headers,
        body: JSON.stringify({
      ...
      if (!res.ok) {
        if (isLiveBillingDrift(errorMessage)) {
          console.warn(`[anthrozaicoderc:live] skip service_tier ${serviceTier}: billing drift`);
          return null;
        }
        if (isLiveAuthDrift(errorMessage)) {
          console.warn(`[anthrozaicoderc:live] skip service_tier ${serviceTier}: auth drift`);
          return null;
        }
      }
