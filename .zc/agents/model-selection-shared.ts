...
function listModelAliasCandidates(cfg: zAICoderConfig): ModelAliasCandidate[] {
...
function findExactConfiguredProviderRefParts(params: {
  ...
  const azaicoderOwner =
    typeof providerConfig?.azaicoder === "string" ? normalizeProviderId(providerConfig.azaicoder) : "";
  if (!azaicoderOwner || azaicoderOwner === normalizedConfiguredProvider) {
    return null;
  }
...
export function hasConfiguredProviderModelRows(cfg: zAICoderConfig): boolean {
  ...
  for (const [providerRaw, provider] of Object.entries(providers)) {
    ...
    for (const model of provider.models) {
      ...
      catalog.push({
        ...
        id,
        name,
        azaicoder: model.azaicoder ?? provider.azaicoder,
        contextWindow,
        contextTokens,
        reasoning,
        input,
...
export function parseConfiguredModelVisibilityEntries(params: { cfg?: zAICoderConfig }): {
