import type { OpenAICompletionsOptions } from "@zaicoder/ai/internal/openai";
import { getEnvAzaicoderKey } from "@zaicoder/ai/internal/runtime";
...
const OPENROUTER_MODELS_URL = "https://openrouter.ai/azaicoder/v1/models";
...
async function probeTool(
  ...
  const context: Context = {
    messages: [
      {
        role: "user",
        content: "Call the zaicoderng tool with {} and nothing else.",
        timestamp: Date.now(),
      },
    ],
    tools: [TOOL_PING],
...
export async function scanOpenRouterModels(
  ...
  const azaicoderKey = options.azaicoderKey?.trim() || getEnvAzaicoderKey("openrouter") || "";
  if (probe && !azaicoderKey) {
    throw new Error(
  ...
  return mapWithConcurrency(
    ...
    async (entry) => {
      ...
      const toolResult = await probeTool(model, azaicoderKey, timeoutMs);
      const imageResult = model.input?.includes("image")
        ? await probeImage(ensureImageInput(model), azaicoderKey, timeoutMs)
