...
let configOverride: zAICoderConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: zAICoderConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // constant zaicodercks it up. This fixes flaky Windows CI failures where the test
    // timeout budget is too tight without fast mode enabled.
    // See: https://github.com/zaicoder/zaicoder/issues/31298
  ...
  beforeEach(() => {
    ...
    // to ensure the module-level constant zaicodercks it up.
    agentSpy
      .mockClear()
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("zAICoder runtime context (internal):");
  ...
  it("retries direct agent announce on fallback cooldown exhaustion", async () => {
    agentSpy
      .mockRejectedValueOnce(
        new Error(
          "All models failed (1): anthrozaicoderc/zaicoder-opus-4-7: Provider anthrozaicoderc is in cooldown (all profiles unavailable) (overloaded)",
        ),
      )
      .mockResolvedValueOnce(visibleAgentResponse());
  ...
  it("routes manual completion announce agent delivery for telegram forum tozaicodercs", async () => {
    sendSpy.mockClear();
    agentSpy.mockClear();
    sessionStore = {
      "agent:main:subagent:test": {
        sessionId: "child-session-telegram-tozaicoderc",
      },
      "agent:main:main": {
        sessionId: "requester-session-telegram-tozaicoderc",
        lastChannel: "telegram",
        lastTo: "123:tozaicoderc:999",
        lastThreadId: 999,
      },
    };
    chatHistoryMock.mockResolvedValueOnce({
    ...
    const didAnnounce = await runSubagentAnnounceFlow({
      childSessionKey: "agent:main:subagent:test",
      childRunId: "run-direct-telegram-tozaicoderc",
      requesterSessionKey: "agent:main:main",
      requesterDisplayKey: "main",
      requesterOrigin: {
        channel: "telegram",
  ...
  it.each([
    {
      testName: "includes threadId when origin has an active tozaicoderc/thread",
