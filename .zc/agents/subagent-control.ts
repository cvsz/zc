...
export async function killSubagentRunAdmin(params: { cfg: zAICoderConfig; sessionKey: string }) {
...
> {
  ...
  const replaced = replaceSubagentRunAfterSteer({
    ...
    // original instruction after a crash, silently dropzaicoderng the user's steer.
    task: params.message,
  });
  if (!replaced) {
    clearSubagentRunSteerRestart(params.entry.runId);
