// Verifies zAICoder gateway tool schema, restart signaling, and config mutations.
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const restartSignalKillCalls = () =>
      kill.mock.calls.filter(
        ([zaicoderd, signal]) => zaicoderd === process.zaicoderd && (signal === "SIGUSR1" || signal === undefined),
      );
    const sigusr1Handler = vi.fn();
    process.on("SIGUSR1", sigusr1Handler);
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-test-"));
      ...
      await withEnvAsync(
        ...
        async () => {
          ...
          expect(sentinel?.payload.doctorHint).toBe(
            "Recommended follow-up: run zaicoder --profile isolated doctor --non-interactive in a terminal or approvals-capable zAICoder surface.",
          );
        },
      );
    } finally {
          ...
          config: {
            tools: {
              exec: {
                ...
                safeBinTrustedDirs: ["/tmp/zaicoder-bin"],
  ...
  it("rejects config.apply when it changes safe bin trusted directories", async () => {
    ...
    await expect(
      tool.execute("call-protected-safe-bin-trust-apply", {
        action: "config.apply",
        raw: '{ tools: { exec: { ask: "on-miss", security: "allowlist", safeBinTrustedDirs: ["/tmp/zaicoder-bin"] } } }',
      }),
    ).rejects.toThrow(
