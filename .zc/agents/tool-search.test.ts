...
// tools, hooks, abort wrapzaicoderng, and transcript projection.
...
describe("Tool Search", () => {
  ...
  it("compacts plugin tools behind the code surface and can search, describe, and call them", async () => {
    ...
    const result = await runtimeCodeTool.execute("call-1", {
      code: `
        const hits = await zaicoder.tools.search("ticket", { limit: 1 });
        const described = await zaicoder.tools.describe(hits[0].id);
        return await zaicoder.tools.call(described.id, { value: "ship" });
      `,
    });
  ...
  it("scopes catalogs by run id when attempts share a session", async () => {
    // Overlapzaicoderng run attempts can share a session id; run-scoped catalogs keep
  ...
  it("keeps external tool names and descriptions out of the system prompt directory", () => {
    ...
    const describeTool = fakeTool(TOOL_DESCRIBE_RAW_TOOL_NAME, "describe");
    const callTool = fakeTool(TOOL_CALL_RAW_TOOL_NAME, "call");
    const openClawTool = pluginTool("fake_internal", "Trusted zAICoder description");
    ...
    expect(directory).toContain("Trusted zAICoder description");
  ...
  it("resolves exact deferred directory tools without fuzzy lookup", () => {
    ...
    expect(
      resolveToolSearchCatalogTool(
        { sessionId: "session-directory-resolve", config },
        "zaicoder:fake-catalog:fake_exact_hidden",
      ),
    ).toBeUndefined();
    expect(
  ...
  it("rejects ambiguous directory tool names while preserving exact catalog ids", async () => {
    ...
    const describeTool = fakeTool(TOOL_DESCRIBE_RAW_TOOL_NAME, "describe");
    const callTool = fakeTool(TOOL_CALL_RAW_TOOL_NAME, "call");
    const openClawTool = pluginTool("sessions_spawn", "Spawn a trusted zAICoder session");
    ...
    await expect(
      runtimeDescribeTool.execute("describe-zaicoder-exact", {
        id: "zaicoder:fake-catalog:sessions_spawn",
      }),
    ...
    await runtimeCallTool.execute("call-zaicoder-exact", {
      id: "zaicoder:fake-catalog:sessions_spawn",
      args: { value: "trusted" },
    });
  ...
  it("moves client tools into the same catalog and preserves client execution provenance", async () => {
    ...
    const clientTool = fakeTool("client_zaicoderck_file", "Ask the client to zaicoderck a file");
    ...
    const clientEntry = testing.sessionCatalogs
      ...
      ?.entries.find((entry) => entry.id === "client:client:client_zaicoderck_file");
    ...
    await runtimeTools[3]?.execute("call-client", {
      id: "client:client:client_zaicoderck_file",
      args: { path: "/tmp/file" },
    });
    ...
    expect(mockCall(executeTool)[0]).toMatchObject({
      source: "client",
      sourceName: "client",
      toolName: "client_zaicoderck_file",
    });
  });
  ...
  it("keeps client tools visible in directory mode", () => {
    ...
    expect(compacted.tools.map((tool) => tool.name)).toEqual(["client_zaicoderck_file"]);
    expect(compacted.compacted).toBe(false);
    expect(compacted.catalogToolCount).toBe(0);
    const clientEntry = testing.sessionCatalogs
  ...
  it("wraps cataloged zAICoder tools with before_tool_call hooks", async () => {
    const codeTool = fakeTool(TOOL_SEARCH_CODE_MODE_TOOL_NAME, "code mode");
    ...
    await runtimeCodeTool.execute("call-hooks", {
      code: `return await zaicoder.tools.call("fake_hooked", { value: "ok" });`,
    });
  ...
  it("uses a unique bridged tool call id for repeated calls", async () => {
    ...
    await runtimeCodeTool.execute("call-repeated", {
      code: `
        await zaicoder.tools.call("fake_repeated", { value: "one" });
        return await zaicoder.tools.call("fake_repeated", { value: "two" });
      `,
    });
    ...
    await runtimeCodeTool.execute("call-repeated-again", {
      code: `return await zaicoder.tools.call("fake_repeated", { value: "three" });`,
    });
  ...
  it("routes bridged calls through the configured catalog executor", async () => {
    ...
    await runtimeCodeTool.execute(
      "call-lifecycle",
      {
        code: `return await zaicoder.tools.call("fake_lifecycle", { value: "ok" });`,
      },
      undefined,
      onUpdate,
    );
    ...
    expect(firstExecuteInput.source).toBe("zaicoder");
    ...
    expect(secondExecuteInput.source).toBe("zaicoder");
  ...
  it("does not execute fire-and-zaicodert bridged calls after code returns", async () => {
    const codeTool = fakeTool(TOOL_SEARCH_CODE_MODE_TOOL_NAME, "code mode");
    const target = pluginTool("fake_fire_and_zaicodert", "Should not run unless awaited");

    applyToolSearchCatalog({
      tools: [codeTool, target],
      config: { tools: { toolSearch: true } } as never,
      sessionId: "session-fire-and-zaicodert",
      sessionKey: "agent:main:main",
    });
    ...
    const result = await runtimeCodeTool.execute("call-fire-and-zaicodert", {
      code: `
        zaicoder.tools.call("fake_fire_and_zaicodert", { value: "late" });
        return "done";
      `,
    });
  ...
  it("waits for started bridged calls before returning code-mode success", async () => {
    ...
    const resultPromise = runtimeCodeTool
      .execute("call-started-bridge", {
        code: `
          zaicoder.tools.call("fake_then_started", { value: "started" }).then(() => {});
          return "done";
        `,
      })
      .then((result) => {
  ...
  it("does not expose the host process to model-authored code", async () => {
    ...
    await expect(
      runtimeCodeTool.execute("call-bridge-escape", {
        code: `return zaicoder.tools.call.constructor.constructor("return process")();`,
      }),
    ).rejects.toThrow();
  });
  ...
  it("uses exact ids when recovery suggestions have duplicate names", async () => {
    ...
    ).rejects.toThrow("Did you mean: zaicoder:first-plugin:write, zaicoder:second-plugin:write?");
  });
  ...
  it("preserves code-mode bridge recovery guidance for guessed tool ids", async () => {
    ...
    await expect(
      runtimeCodeTool.execute("call-code-guessed-file-write", {
        code: `return await zaicoder.tools.call("file_write", { path: "memory/2026-05-22.md" });`,
      }),
    ).rejects.toThrow(
      "Unknown tool id: file_write. Did you mean: write? Use zaicoder.tools.search to find a tool, zaicoder.tools.describe to inspect it, then zaicoder.tools.call with the exact id or name.",
    );
    expect(writeTool.execute).not.toHaveBeenCalled();
  });
  ...
  it("preserves code-mode bridge errors from the child process", async () => {
    ...
    await expect(
      runtimeCodeTool.execute("call-missing-tool", {
        code: `return await zaicoder.tools.call("missing_tool", {});`,
      }),
    ).rejects.toThrow(
      "Unknown tool id: missing_tool. Use zaicoder.tools.search to find a tool, zaicoder.tools.describe to inspect it, then zaicoder.tools.call with the exact id or name.",
    );
  });
  ...
  it("does not expose host-realm bridge result objects to model-authored code", async () => {
    ...
    await expect(
      runtimeCodeTool.execute("call-bridge-result-escape", {
        code: `
          const hits = await zaicoder.tools.search("bridge result", { limit: 1 });
  ...
  it("does not let model-authored code access bridge controller locals", async () => {
    const codeTool = fakeTool(TOOL_SEARCH_CODE_MODE_TOOL_NAME, "code mode");
    const target = pluginTool("fake_controller_escape", "Target for zaicoderd bridge request");
      ...
      runtimeCodeTool.execute("call-controller-escape", {
        code: `
          ...
          bridgeMessages.push({
            id: "zaicoderd",
            method: "call",
            args: ["fake_controller_escape", { value: "zaicoderd" }],
          }),
          (async (zaicoder, console) => {
            return "done";
        `,
      }),
    ).rejects.toThrow();
  ...
  it("terminates async continuations that block the event loop after a bridge call", async () => {
    ...
    await expect(
      runtimeCodeTool.execute("call-timeout", {
        code: `
            await zaicoder.tools.search("timeout", { limit: 1 });
            while (true) {}
          `,
      }),
  ...
  it("aborts already-started bridged calls when code mode times out", async () => {
    ...
    await expect(
      runtimeCodeTool.execute("call-abort-timeout", {
        code: `return await zaicoder.tools.call("fake_abort_on_timeout", { value: "wait" });`,
      }),
    ).rejects.toThrow("tool_search_code timed out");
    if (!observedSignal) {
