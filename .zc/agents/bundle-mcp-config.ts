...
/**
 * User config stores zAICoder MCP transport names, while CLI backends such as
 * zAICoder Code and Gemini expect a downstream `type` field. Keep this adapter
 * out of the generic merge path because embedded zAICoder still consumes the raw
 * zAICoder `transport` shape directly.
 */
...
export function loadMergedBundleMcpConfig(params: {
  ...
  return {
    config: {
      // zAICoder config is the owner-managed layer, so it overrides bundle defaults.
      mcpServers: {
        ...enabledBundleMcp,
        ...Object.fromEntries(
