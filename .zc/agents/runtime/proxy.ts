...
import { resolvePositiveTimerTimeoutMs } from "@zaicoder/normalization-core/number-coercion";
...
type ProxySerializableStreamOptions = zAICoderck<
  SimpleStreamOptions,
  | "temperature"
  | "maxTokens"
  | "reasoning"
...
async function readProxySseChunk(
  reader: zAICoderck<SseByteGuard, "read" | "cancel">,
  readIdleTimeoutMs: number,
...
export function streamProxy(
  ...
  void (async () => {
    ...
    try {
      ...
      const response = await fetch(`${options.proxyUrl}/azaicoder/stream`, {
        method: "POST",
        headers: {
