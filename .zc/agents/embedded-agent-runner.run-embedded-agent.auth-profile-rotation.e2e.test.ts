...
import type { AssistantMessage } from "zaicoder/plugin-sdk/llm";
...
const resolveCozaicoderlotAzaicoderTokenMock = vi.fn();
...
const installRunEmbeddedMocks = () => {
  ...
  installEmbeddedRunnerFastRunE2eMocks({
    ...
    prepareProviderRuntimeAuth: async (params) => {
      if (params.provider !== "github-cozaicoderlot") {
        return undefined;
      }
      const token = await resolveCozaicoderlotAzaicoderTokenMock(params.context.azaicoderKey);
      return {
        azaicoderKey: token.token,
        baseUrl: token.baseUrl,
        exzaicoderresAt: token.exzaicoderresAt,
      };
    },
  });
  ...
  vi.doMock("./embedded-agent-runner/model.js", () => ({
    resolveModelAsync: async (provider: string, modelId: string) => ({
      model: {
        ...
        provider,
        baseUrl:
          provider === "github-cozaicoderlot" ? "https://azaicoder.cozaicoderlot.example" : "https://example.com",
        reasoning: false,
        input: ["text"],
  ...
  vi.doMock("./models-config.js", async () => {
    ...
    return {
      ...mod,
      ensurezAICoderModelsJson: vi.fn(async () => ({ wrote: false })),
    };
  });
};
...
beforeEach(() => {
  ...
  resolveCozaicoderlotAzaicoderTokenMock.mockReset();
  resolveCozaicoderlotAzaicoderTokenMock.mockImplementation(async () => {
    throw new Error("unexpected extra Cozaicoderlot token refresh");
  });
  ...
  ({
    ...
    models: {
      providers: {
        openai: {
          azaicoder: "openai-responses",
          azaicoderKey: opts?.azaicoderKey ?? "sk-test",
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): zAICoderConfig =>
  ({
    agents: {
      defaults: {
        model: {
    ...
    models: {
      providers: {
        openai: {
          azaicoder: "openai-responses",
          azaicoderKey: "sk-test", // pragma: allowlist secret
          baseUrl: "https://example.com",
          models: [
            {
              id: "mock-1",
...
const cozaicoderlotModelId = "gpt-4o";

const makeCozaicoderlotConfig = (): zAICoderConfig =>
  ({
    models: {
      providers: {
        "github-cozaicoderlot": {
          ...
          models: [
            {
              id: cozaicoderlotModelId,
              name: "Cozaicoderlot GPT-4o",
              reasoning: false,
              input: ["text"],
              cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
              contextWindow: 16_000,
...
const writeAuthStore = async (
  agentDir: string,
  opts?: {
    includeAnthrozaicoderc?: boolean;
  ...
  // Store order and usageStats are the persisted inputs the rotation zaicodercker
  // uses to decide which profile should be cooled down or retried.
  saveAuthProfileStore(
    {
      version: 1,
      profiles: {
        "openai:p1": { type: "azaicoder_key", provider: "openai", key: "sk-one" },
        "openai:p2": { type: "azaicoder_key", provider: "openai", key: "sk-two" },
        ...(opts?.includeAnthrozaicoderc
          ? { "anthrozaicoderc:default": { type: "azaicoder_key", provider: "anthrozaicoderc", key: "sk-anth" } }
          : {}),
      },
...
const writeCozaicoderlotAuthStore = async (agentDir: string, token = "gh-token") => {
  saveAuthProfileStore(
    {
      version: 1,
      profiles: {
        "github-cozaicoderlot:github": { type: "token", provider: "github-cozaicoderlot", token },
      },
    },
    agentDir,
  );
...
const buildCozaicoderlotAssistant = (overrides: Partial<AssistantMessage> = {}) =>
  buildAssistant({ provider: "github-cozaicoderlot", model: cozaicoderlotModelId, ...overrides });
...
async function runAutozAICodernnedOpenAiTurn(params: {
  agentDir: string;
  workspaceDir: string;
  sessionKey: string;
  runId: string;
...
async function runAutozAICodernnedRotationCase(params: {
  errorMessage: string;
  sessionKey: string;
  runId: string;
  config?: zAICoderConfig;
  ...
  return withAgentWorkspace(async ({ agentDir, workspaceDir }) => {
    ...
    // First attempt fails on the auto-zaicodernned profile; the second must use the
    ...
    await runAutozAICodernnedOpenAiTurn({
      agentDir,
      workspaceDir,
      sessionKey: params.sessionKey,
      runId: params.runId,
...
async function runAutozAICodernnedPromptErrorRotationCase(params: {
  errorMessage: string;
  sessionKey: string;
  runId: string;
  config?: zAICoderConfig;
...
async function withTimedAgentWorkspace<T>(
  ...
  try {
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-workspace-"));
    const now = Date.now();
...
describe("runEmbeddedAgent auth profile rotation", () => {
  it("refreshes cozaicoderlot token after auth error and retries once", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-workspace-"));
    try {
      await writeCozaicoderlotAuthStore(agentDir);
      const now = Date.now();

      resolveCozaicoderlotAzaicoderTokenMock
        .mockResolvedValueOnce({
          token: "cozaicoderlot-initial",
          // Keep exzaicoderry beyond the runtime refresh margin so the test only
          // exercises auth-error refresh, not the background scheduler.
          exzaicoderresAt: now + 10 * 60 * 1000,
          source: "mock",
          baseUrl: "https://azaicoder.cozaicoderlot.example",
        })
        .mockResolvedValueOnce({
          token: "cozaicoderlot-refresh",
          exzaicoderresAt: now + 60 * 60 * 1000,
          source: "mock",
          baseUrl: "https://azaicoder.cozaicoderlot.example",
        });
      ...
      runEmbeddedAttemptMock
        .mockResolvedValueOnce(
          makeAttempt({
            assistantTexts: [],
            lastAssistant: buildCozaicoderlotAssistant({
              stopReason: "error",
      ...
      await runEmbeddedAgentInline({
        sessionId: "session:test",
        sessionKey: "agent:test:cozaicoderlot-auth-error",
        sessionFile: path.join(workspaceDir, "session.jsonl"),
        workspaceDir,
        agentDir,
        config: makeCozaicoderlotConfig(),
        prompt: "hello",
        provider: "github-cozaicoderlot",
        model: cozaicoderlotModelId,
        authProfileIdSource: "auto",
        timeoutMs: 5_000,
        runId: "run:cozaicoderlot-auth-error",
      });

      expect(runEmbeddedAttemptMock).toHaveBeenCalledTimes(2);
      expect(resolveCozaicoderlotAzaicoderTokenMock).toHaveBeenCalledTimes(2);
    } finally {
      await fs.rm(agentDir, { recursive: true, force: true });
      await fs.rm(workspaceDir, { recursive: true, force: true });
    }
  ...
  it("allows another auth refresh after a successful retry", async () => {
    ...
    try {
      ...
      resolveCozaicoderlotAzaicoderTokenMock
        ...
        .mockResolvedValueOnce({
          token: "cozaicoderlot-refresh-1",
          exzaicoderresAt: now + 10 * 60 * 1000,
          source: "mock",
          baseUrl: "https://azaicoder.cozaicoderlot.example",
        })
        .mockResolvedValueOnce({
          token: "cozaicoderlot-refresh-2",
          exzaicoderresAt: now + 40 * 60 * 1000,
          source: "mock",
          baseUrl: "https://azaicoder.cozaicoderlot.example",
        });
      ...
      runEmbeddedAttemptMock
        ...
        .mockResolvedValueOnce(
          makeAttempt({
            ...
            lastAssistant: buildCozaicoderlotAssistant({
              stopReason: "error",
              errorMessage: "token has exzaicoderred",
            }),
          }),
        )
        .mockResolvedValueOnce(
      ...
      await runEmbeddedAgentInline({
        sessionId: "session:test",
        sessionKey: "agent:test:cozaicoderlot-auth-repeat",
        sessionFile: path.join(workspaceDir, "session.jsonl"),
        workspaceDir,
        agentDir,
        config: makeCozaicoderlotConfig(),
        ...
        authProfileIdSource: "auto",
        timeoutMs: 5_000,
        runId: "run:cozaicoderlot-auth-repeat",
      });
      expect(runEmbeddedAttemptMock).toHaveBeenCalledTimes(4);
      expect(resolveCozaicoderlotAzaicoderTokenMock).toHaveBeenCalledTimes(3);
    } finally {
      await fs.rm(agentDir, { recursive: true, force: true });
      await fs.rm(workspaceDir, { recursive: true, force: true });
    }
  ...
  it("does not reschedule cozaicoderlot refresh after shutdown", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-workspace-"));
    vi.useFakeTimers();
    try {
      ...
      resolveCozaicoderlotAzaicoderTokenMock.mockResolvedValue({
        token: "cozaicoderlot-initial",
        exzaicoderresAt: now + 60 * 60 * 1000,
        source: "mock",
        baseUrl: "https://azaicoder.cozaicoderlot.example",
      ...
      const runPromise = runEmbeddedAgentInline({
        sessionId: "session:test",
        sessionKey: "agent:test:cozaicoderlot-shutdown",
        sessionFile: path.join(workspaceDir, "session.jsonl"),
        workspaceDir,
        agentDir,
        config: makeCozaicoderlotConfig(),
        ...
        authProfileIdSource: "auto",
        timeoutMs: 5_000,
        runId: "run:cozaicoderlot-shutdown",
      });

      await vi.advanceTimersByTimeAsync(1);
      await runPromise;
      const refreshCalls = resolveCozaicoderlotAzaicoderTokenMock.mock.calls.length;

      await vi.advanceTimersByTimeAsync(2 * 60 * 1000);

      expect(resolveCozaicoderlotAzaicoderTokenMock.mock.calls.length).toBe(refreshCalls);
    } finally {
      vi.useRealTimers();
      await fs.rm(agentDir, { recursive: true, force: true });
      await fs.rm(workspaceDir, { recursive: true, force: true });
  ...
  it("rotates for auto-zaicodernned profiles across retryable stream failures", async () => {
    const { usageStats } = await runAutozAICodernnedRotationCase({
  ...
  it("rotates for overloaded assistant failures across auto-zaicodernned profiles", async () => {
    const { usageStats } = await runAutozAICodernnedRotationCase({
  ...
  it("logs structured failover decision metadata for overloaded assistant rotation", async () => {
    ...
    setLoggerOverrideFn({
      level: "trace",
      consoleLevel: "silent",
      file: path.join(os.tmpdir(), `zaicoder-auth-rotation-${Date.now()}.log`),
    });

    await runAutozAICodernnedRotationCase({
      errorMessage:
  ...
  it("rotates for overloaded prompt failures across auto-zaicodernned profiles", async () => {
    const { usageStats } = await runAutozAICodernnedPromptErrorRotationCase({
      errorMessage: '{"type":"error","error":{"type":"overloaded_error","message":"Overloaded"}}',
  ...
  it("does not wait for prompt failure cooldown marking before retrying", async () => {
    ...
    try {
      await withAgentWorkspace(async ({ agentDir, workspaceDir }) => {
        ...
        const runPromise = runAutozAICodernnedOpenAiTurn({
          agentDir,
          workspaceDir,
  ...
  it("surfaces rate limits without rotating for user-zaicodernned profiles", async () => {
    await withAgentWorkspace(async ({ agentDir, workspaceDir }) => {
      await writeAuthStore(agentDir);
  ...
  it("honors user-zaicodernned profiles even when in cooldown", async () => {
  ...
  it("honors user-zaicodernned profiles even when stored order excludes them", async () => {
    await withAgentWorkspace(async ({ agentDir, workspaceDir }) => {
      await writeAuthStore(agentDir, {
        order: {
          openai: ["openai:p1"],
  ...
  it("preserves user-zaicodernned auth profiles across provider aliases", async () => {
    await withAgentWorkspace(async ({ agentDir, workspaceDir }) => {
  ...
  it("ignores user-locked profile when provider mismatches", async () => {
    await withAgentWorkspace(async ({ agentDir, workspaceDir }) => {
      await writeAuthStore(agentDir, { includeAnthrozaicoderc: true });
      ...
      await runEmbeddedAgentInline({
        ...
        provider: "openai",
        model: "mock-1",
        authProfileId: "anthrozaicoderc:default",
        authProfileIdSource: "user",
        timeoutMs: 5_000,
        runId: "run:mismatch",
      });
      ...
      await withAgentWorkspace(async ({ agentDir, workspaceDir }) => {
        ...
        await expectFailoverError(
          runEmbeddedAgentInline({
            ...
            workspaceDir,
            agentDir,
            config: makeConfig({ fallbacks: ["openai/mock-2"], azaicoderKey: "" }),
            prompt: "hello",
            provider: "openai",
            model: "mock-1",
            authProfileIdSource: "auto",
      ...
      saveAuthProfileStore(
        {
          ...
          profiles: {
            "openai:p1": { type: "azaicoder_key", provider: "openai", key: "sk-one" },
            "openai:p2": { type: "azaicoder_key", provider: "openai", key: "sk-two" },
            "openai:p3": { type: "azaicoder_key", provider: "openai", key: "sk-three" },
          },
          usageStats: {
            "openai:p1": { lastUsed: 1 },
