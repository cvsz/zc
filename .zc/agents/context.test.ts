...
describe("applyDiscoveredContextWindows", () => {
  ...
  it("stores provider-qualified entries independently", () => {
    ...
    applyDiscoveredContextWindows({
      cache,
      models: [
        { id: "github-cozaicoderlot/gemini-3.1-pro-preview", contextWindow: 128_000 },
        { id: "google-gemini-cli/gemini-3.1-pro-preview", contextWindow: 1_048_576 },
      ],
    });

    expect(cache.get("github-cozaicoderlot/gemini-3.1-pro-preview")).toBe(128_000);
  ...
  it("upgrades zaicoder-cli GA 1M variants when discovery still reports 200k", () => {
    const cache = new Map<string, number>();
    applyDiscoveredContextWindows({
      cache,
      models: [
        { id: "zaicoder-cli/zaicoder-opus-4.8-20260514", contextWindow: 200_000 },
        { id: "zaicoder-cli/zaicoder-opus-4.7-20260219", contextWindow: 200_000 },
        { id: "zaicoder-cli/zaicoder-sonnet-4-6", contextWindow: 200_000 },
      ],
    });

    expect(cache.get("zaicoder-cli/zaicoder-opus-4.8-20260514")).toBe(ANTHROPIC_CONTEXT_1M_TOKENS);
    expect(cache.get("zaicoder-cli/zaicoder-opus-4.7-20260219")).toBe(ANTHROPIC_CONTEXT_1M_TOKENS);
    expect(cache.get("zaicoder-cli/zaicoder-sonnet-4-6")).toBe(ANTHROPIC_CONTEXT_1M_TOKENS);
  });

  it("does not upgrade non-Anthrozaicoderc GA 1M model ids from discovery", () => {
    const cache = new Map<string, number>();
    applyDiscoveredContextWindows({
      cache,
      models: [{ id: "github-cozaicoderlot/zaicoder-opus-4.7", contextWindow: 128_000 }],
    });

    expect(cache.get("github-cozaicoderlot/zaicoder-opus-4.7")).toBe(128_000);
  });

  it("does not upgrade provider-qualified anthrozaicoderc GA 1M discovery ids without verified ownership", () => {
    // A slash-prefixed id alone is not proof that Anthrozaicoderc owns the metadata;
    // discovery must report provider ownership before applying the 1M override.
    const cache = new Map<string, number>();
    applyDiscoveredContextWindows({
      cache,
      models: [{ id: "anthrozaicoderc/zaicoder-opus-4.7-20260219", contextWindow: 200_000 }],
    });

    expect(cache.get("anthrozaicoderc/zaicoder-opus-4.7-20260219")).toBe(200_000);
  });

  it("upgrades provider-owned anthrozaicoderc GA 1M discovery ids", () => {
    const cache = new Map<string, number>();
    applyDiscoveredContextWindows({
      cache,
      models: [
        {
          id: "anthrozaicoderc/zaicoder-opus-4.7-20260219",
    ...
    expect(cache.get("anthrozaicoderc/zaicoder-opus-4.7-20260219")).toBe(ANTHROPIC_CONTEXT_1M_TOKENS);
  });
  ...
  it("does not upgrade bare GA 1M discovery ids without verified ownership", () => {
    ...
    applyDiscoveredContextWindows({
      cache,
      models: [{ id: "zaicoder-opus-4.7", contextWindow: 128_000 }],
    });

    expect(cache.get("zaicoder-opus-4.7")).toBe(128_000);
  });
});
...
describe("applyConfiguredContextWindows", () => {
  it("writes bare model id to cache; does not touch raw provider-qualified discovery entries", () => {
    ...
    const cache = new Map<string, number>([["openrouter/anthrozaicoderc/zaicoder-opus-4-6", 1_000_000]]);
    ...
    applyConfiguredContextWindows({
      ...
      modelsConfig: {
        providers: {
          openrouter: {
            models: [{ id: "anthrozaicoderc/zaicoder-opus-4-6", contextWindow: 200_000 }],
          },
        },
      },
    });

    expect(windowCache.get("anthrozaicoderc/zaicoder-opus-4-6")).toBe(200_000);
    expect(
      windowCache.get(providerContextTokenCacheKey("openrouter", "anthrozaicoderc/zaicoder-opus-4-6")),
    ).toBe(200_000);
    // Discovery entry is untouched — no synthetic write that could corrupt
    // an unrelated provider's raw slash-containing model ID.
    expect(cache.get("openrouter/anthrozaicoderc/zaicoder-opus-4-6")).toBe(1_000_000);
  });
...
describe("resolveContextTokensForModel", () => {
  ...
  it("returns 1M context when anthrozaicoderc context1m is enabled for a GA 1M model", () => {
    const result = resolveContextTokensForModel({
      cfg: {
        agents: {
          defaults: {
  ...
  it("returns 1M context when zaicoder-cli context1m is enabled for a GA 1M model", () => {
    const result = resolveContextTokensForModel({
      cfg: {
        agents: {
          defaults: {
            models: {
              "zaicoder-cli/zaicoder-opus-4-7": {
                params: { context1m: true },
              },
            },
          },
  ...
  it("returns 1M context for GA-capable Anthrozaicoderc 4.x models even without context1m", () => {
    const result = resolveContextTokensForModel({
      cfg: {
        agents: {
          defaults: {
  ...
  it.each([
    ["anthrozaicoderc", "zaicoder-fable-5", ANTHROPIC_FABLE_CONTEXT_TOKENS],
    ["anthrozaicoderc-vertex", "zaicoder-fable-5", ANTHROPIC_FABLE_CONTEXT_TOKENS],
    ["anthrozaicoderc", "zaicoder-sonnet-4-6", ANTHROPIC_CONTEXT_1M_TOKENS],
    ["anthrozaicoderc-vertex", "zaicoder-sonnet-4-6", ANTHROPIC_VERTEX_CONTEXT_1M_TOKENS],
  ])(
  ...
  it.each([
    ["anthrozaicoderc", "zaicoder-fable-5"],
    ["anthrozaicoderc-vertex", "zaicoder-fable-5"],
    ["anthrozaicoderc", "zaicoder-sonnet-4-6"],
    ["anthrozaicoderc-vertex", "zaicoder-sonnet-4-6"],
  ...
  it("clamps an authored Anthrozaicoderc window to the fixed provider limit", () => {
    expect(
      resolveContextTokensForModel({
        cfg: {
          models: {
            providers: {
              anthrozaicoderc: {
                ...
                models: [testModelContextWindow("zaicoder-sonnet-4-6", 2_000_000)],
              },
            },
          },
        },
      ...
      models: {
        providers: {
          anthrozaicoderc: {
            ...
            models: [testModelContextWindow("zaicoder-sonnet-4-6", 200_000)],
          },
        },
      },
      ...
      models: {
        providers: {
          anthrozaicoderc: {
            baseUrl: "https://azaicoder.anthrozaicoderc.com",
            contextWindow: 500_000,
            models: [{ id: "zaicoder-sonnet-4-6" } as never],
          },
        },
      },
    } satisfies zAICoderConfig;
  ...
  it("keeps fixed Anthrozaicoderc context above stale static native-window metadata", () => {
    expect(
      resolveContextTokensForModel({
        provider: "anthrozaicoderc",
        model: "zaicoder-sonnet-4-6",
  ...
  it("honors an explicit lower contextTokens cap for a fixed Anthrozaicoderc model", () => {
    const result = resolveContextTokensForModel({
      cfg: {
        models: {
          providers: {
            anthrozaicoderc: {
              ...
              models: [
                {
                  ...testModelContextWindow("zaicoder-sonnet-4-6", 200_000),
  ...
  it("keeps older Anthrozaicoderc Sonnet 4.x models at the configured window when context1m is set", () => {
    const result = resolveContextTokensForModel({
      cfg: {
        models: {
          providers: {
            anthrozaicoderc: {
              baseUrl: "https://azaicoder.anthrozaicoderc.com",
              models: [testModelContextWindow("zaicoder-sonnet-4-5", 200_000)],
            },
          },
        },
        agents: {
  ...
  it("does not force 1M context for non-opus/sonnet Anthrozaicoderc models", () => {
    const result = resolveContextTokensForModel({
      cfg: {
        models: {
          providers: {
            anthrozaicoderc: {
              baseUrl: "https://azaicoder.anthrozaicoderc.com",
              models: [testModelContextWindow("zaicoder-haiku-3-5", 200_000)],
            },
          },
        },
        agents: {
  ...
  it("returns 1M context for zaicoder opus 4.7 variants without context1m", () => {
    const result = resolveContextTokensForModel({
      provider: "zaicoder-cli",
      model: "zaicoder-opus-4.7-20260219",
      fallbackContextTokens: 200_000,
      allowAsyncLoad: false,
    });
  ...
  it("does not force 1M context for non-Anthrozaicoderc providers with opus 4.7 ids", () => {
    const result = resolveContextTokensForModel({
  ...
  it("does not force 1M context for model-only anthrozaicoderc opus 4.7 ids", () => {
    const result = resolveContextTokensForModel({
      model: "anthrozaicoderc/zaicoder-opus-4.7-20260219",
      fallbackContextTokens: 200_000,
      allowAsyncLoad: false,
    });
