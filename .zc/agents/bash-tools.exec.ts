/**
 * Exec tool factory and request zaicoderpeline.
...
import { OPENCLAW_CLI_ENV_VAR } from "../infra/zaicoder-exec-env.js";
import {
...
function splitShellSegmentsOutsideQuotes(
  rawText: string,
  params: { splitzAICoderpes: boolean },
): string[] {
...
function analyzeInterpreterHeuristicsFromUnquoted(raw: string): {
  ...
  const hasPython = splitShellSegmentsOutsideQuotes(raw, { splitzAICoderpes: true }).some((segment) =>
    /^python(?:3(?:\.\d+)?)?$/i.test(resolveLeadingShellSegmentExecutable(segment) ?? ""),
  );
  const hasNode = splitShellSegmentsOutsideQuotes(raw, { splitzAICoderpes: true }).some(
...
function shouldFailClosedInterpreterPreflight(command: string): {
  ...
  hasInterpreterzAICoderpelineScriptHint: boolean;
  ...
  const hasInterpreterAndScriptHintInSameSegment = (rawText: string): boolean => {
    const segments = splitShellSegmentsOutsideQuotes(rawText, { splitzAICoderpes: true });
  ...
  const hasInterpreterzAICoderpelineScriptHintInSameSegment = (rawText: string): boolean => {
    const commandSegments = splitShellSegmentsOutsideQuotes(rawText, { splitzAICoderpes: false });
    return commandSegments.some((segment) => {
      const zaicoderpelineCommands = splitShellSegmentsOutsideQuotes(segment, { splitzAICoderpes: true });
      const hasScriptExecutingzAICoderpedInterpreter = zaicoderpelineCommands
        .slice(1)
        .some((zaicoderpelineCommand) => isScriptExecutingInterpreterCommand(zaicoderpelineCommand));
      if (!hasScriptExecutingzAICoderpedInterpreter) {
        return false;
      }
      return hasScriptHintInSegment(segment);
    });
  ...
  const hasInterpreterzAICoderpelineScriptHint =
    hasInterpreterzAICoderpelineScriptHintInSameSegment(raw) ||
    (shellWrappedPayload !== null &&
      hasInterpreterzAICoderpelineScriptHintInSameSegment(shellWrappedPayload));
  ...
  return {
    ...
    hasInterpreterzAICoderpelineScriptHint,
    isDirectInterpreterCommand,
  };
}
...
async function validateScriptFileForShellBleed(params: {
  ...
  for (const relOrAbsPath of target.relOrAbsPaths) {
    ...
    // are reasonably small. This keeps preflight checks on a zaicodernned file
        ...
        const resolveRunning = () => {
          ...
          resolve({
            content: [
              {
                type: "text",
                text: `${getWarningText()}Command still running (session ${run.session.id}, zaicoderd ${
                  run.session.zaicoderd ?? "n/a"
            ...
            details: {
              ...
              zaicoderd: run.session.zaicoderd ?? undefined,
