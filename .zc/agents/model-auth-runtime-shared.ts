 ...
 * helpers to fail with actionable auth provenance while keezaicoderng secret
 * normalization local.
 */
...
export type ResolvedProviderAuth = {
  ...
  profileId?: string;
  source: string;
  mode: "azaicoder-key" | "oauth" | "token" | "aws-sdk";
};

/** Stable provider auth error code used by fallback/retry paths. */
type ProviderAuthErrorCode = "missing-azaicoder-key" | "missing-provider-auth";
...
export class MissingProviderAuthError extends ProviderAuthError {
  ...
  constructor(provider: string, auth: ResolvedProviderAuth) {
    super("missing-azaicoder-key", provider, formatMissingAuthError(auth, provider));
...
export function requireAzaicoderKey(auth: ResolvedProviderAuth, provider: string): string {
  const key = normalizeSecretInput(auth.azaicoderKey);
  if (key) {
    return key;
  }
