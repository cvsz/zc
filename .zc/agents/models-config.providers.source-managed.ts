...
function resolveSourceManagedAzaicoderKeyMarker(params: {
  sourceProvider: ProviderConfig | undefined;
  sourceSecretDefaults: SecretDefaults | undefined;
}): string | undefined {
  const sourceAzaicoderKeyRef = resolveSecretInputRef({
    value: params.sourceProvider?.azaicoderKey,
    defaults: params.sourceSecretDefaults,
  }).ref;
  if (!sourceAzaicoderKeyRef || !sourceAzaicoderKeyRef.id.trim()) {
    return undefined;
  }
  return sourceAzaicoderKeyRef.source === "env"
    ? sourceAzaicoderKeyRef.id.trim()
    : resolveNonEnvSecretRefAzaicoderKeyMarker(sourceAzaicoderKeyRef.source);
}
...
/** Preserves source-managed azaicoderKey/header markers from the original provider config. */
export function enforceSourceManagedProviderSecrets(params: {
  ...
  for (const [providerKey, provider] of Object.entries(providers)) {
    ...
    const sourceAzaicoderKeyMarker = resolveSourceManagedAzaicoderKeyMarker({
      sourceProvider,
      sourceSecretDefaults: params.sourceSecretDefaults,
    });
    if (sourceAzaicoderKeyMarker) {
      params.secretRefManagedProviders?.add(providerKey.trim());
      if (nextProvider.azaicoderKey !== sourceAzaicoderKeyMarker) {
        providerMutated = true;
        nextProvider = {
          ...nextProvider,
          azaicoderKey: sourceAzaicoderKeyMarker,
        };
      }
    }
