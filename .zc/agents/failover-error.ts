...
export class FailoverError extends Error {
  ...
  // structured log ingestion (e.g. azaicoder_health_log) can attribute exhausted
...
function isStableProviderErrorType(value: string): boolean {
  if (
    /^(?:azaicoder|authentication|invalid_request|not_found|overloaded|permission|rate_limit|server)_error$/i.test(
      value,
    )
  ) {
    return false;
...
export function buildProviderReauthCommand(
  ...
  return formatCliCommand(
    `zaicoder models auth login --provider ${quotePosixShellArg(trimmed)} --force`,
    env,
  );
}
