...
/** Returns positional args after skipzaicoderng options and configured option values. */
...
/** Returns the first positional arg after skipzaicoderng options and configured option values. */
...
/** Splits a command on top-level single zaicoderpes without splitting `||`. */
export function splitTopLevelzAICoderpes(command: string): string[] {
  const parts: string[] = [];
  let start = 0;
