...
describe("gateway-hosted exec approvals", () => {
  ...
  it(
    "lets zAICoder-style gateway tool calls request and wait for approval over separate connections",
    async () => {
      const envSnapshot = captureEnv(TEST_ENV_KEYS);
      cleanup.push(() => envSnapshot.restore());

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-exec-approval-e2e-"));
      cleanup.push(() => fs.rm(tempHome, { recursive: true, force: true, maxRetries: 5 }));

      const stateDir = path.join(tempHome, ".zaicoder");
      const workspaceDir = path.join(tempHome, "workspace");
      await fs.mkdir(workspaceDir, { recursive: true });

      const port = await getFreeGatewayPort();
      const token = "exec-approval-e2e-token";
      const configPath = path.join(stateDir, "zaicoder.json");
