...
describe("subagent registry steer restarts", () => {
  ...
  const registerCompletionModeRun = (
    ...
    task: string,
    options: Partial<zAICoderck<RegisterSubagentRunInput, "spawnMode">> = {},
  ): void => {
    registerRun({
      runId,
      childSessionKey,
  ...
  const registerRun = (
    params: {
      ...
      zAICoderck<RegisterSubagentRunInput, "spawnMode" | "requesterOrigin" | "expectsCompletionMessage">
    >,
  ): void => {
  ...
  it("removes orphaned private transcript when steer replaces an internally resumed run", async () => {
    {
      ...
      previous.execution = {
        ...
        transcriptFile: "/tmp/zaicoder-state/internal-agent-runs/run-old.jsonl",
      };
      ...
      expect(removeInternalSessionEffectsTranscriptMock).toHaveBeenCalledWith(
        "/tmp/zaicoder-state/internal-agent-runs/run-old.jsonl",
      );
    }
  });
  ...
  it("keeps completion cleanup pending while descendants are still active", async () => {
    ...
    registerCompletionModeRun(
      "run-parent-exzaicoderry",
      "agent:main:subagent:parent-exzaicoderry",
      "parent completion exzaicoderry",
    );
    registerRun({
      runId: "run-child-active",
      childSessionKey: "agent:main:subagent:parent-exzaicoderry:subagent:child-active",
      requesterSessionKey: "agent:main:subagent:parent-exzaicoderry",
      requesterDisplayKey: "parent-exzaicoderry",
      task: "child still running",
    });

    emitLifecycleEnd("run-parent-exzaicoderry", {
    ...
    const parentHookCall = runSubagentEndedHookMock.mock.calls.find((call) => {
      ...
      return event.runId === "run-parent-exzaicoderry" && event.reason === "subagent-complete";
    });
    expect(parentHookCall).toBeUndefined();
    const parent = mod
      .listSubagentRunsForRequester(MAIN_REQUESTER_SESSION_KEY)
      .find((entry) => entry.runId === "run-parent-exzaicoderry");
