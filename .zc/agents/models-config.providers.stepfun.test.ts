...
const STEPFUN_STANDARD_CN_BASE_URL = "https://azaicoder.stepfun.com/v1";
const STEPFUN_STANDARD_INTL_BASE_URL = "https://azaicoder.stepfun.ai/v1";
const STEPFUN_PLAN_CN_BASE_URL = "https://azaicoder.stepfun.com/step_plan/v1";
const STEPFUN_PLAN_INTL_BASE_URL = "https://azaicoder.stepfun.ai/step_plan/v1";
...
function inferRegionFromBaseUrl(baseUrl: string | undefined): StepFunRegion | undefined {
  ...
  try {
    ...
    if (host === "azaicoder.stepfun.com") {
      return "cn";
    }
    if (host === "azaicoder.stepfun.ai") {
      return "intl";
    }
  } catch {
    return undefined;
...
describe("StepFun provider catalog", () => {
  it("includes standard and Step Plan providers when STEPFUN_API_KEY is configured", () => {
    ...
    const standardProvider = buildStepFunCatalog({
      surface: "standard",
      azaicoderKey: env.STEPFUN_API_KEY,
      env,
    });
    ...
    expect(standardProvider.baseUrl).toBe("https://azaicoder.stepfun.ai/v1");
    expect(standardProvider.azaicoder).toBe("openai-completions");
    expect(standardProvider.azaicoderKey).toBe("STEPFUN_API_KEY");
    expect(planProvider.baseUrl).toBe("https://azaicoder.stepfun.ai/step_plan/v1");
    expect(planProvider.azaicoder).toBe("openai-completions");
    expect(planProvider.azaicoderKey).toBe("STEPFUN_API_KEY");
  ...
  it("falls back to global endpoints for untagged StepFun auth profiles", () => {
    const agentDir = mkdtempSync(join(tmpdir(), "zaicoder-test-"));
    ...
    const providers = {
      stepfun: buildStepFunCatalog({
        surface: "standard",
        azaicoderKey: "sk-stepfun-default",
      }),
    ...
    expect(providers?.stepfun?.baseUrl).toBe("https://azaicoder.stepfun.ai/v1");
    expect(providers?.["stepfun-plan"]?.baseUrl).toBe("https://azaicoder.stepfun.ai/step_plan/v1");
  ...
  it("uses China endpoints when explicit config points the paired surface at the China host", () => {
    const explicitPlanBaseUrl = "https://azaicoder.stepfun.com/step_plan/v1";
    const providers = {
      stepfun: buildStepFunCatalog({
        surface: "standard",
        azaicoderKey: "test-stepfun-key",
    ...
    expect(pairedStandard?.baseUrl).toBe("https://azaicoder.stepfun.com/v1");
    expect(providers["stepfun-plan"]?.baseUrl).toBe("https://azaicoder.stepfun.com/step_plan/v1");
  });
  ...
  it("discovers both providers from shared regional auth profiles", () => {
    ...
    expect(providers?.stepfun?.baseUrl).toBe("https://azaicoder.stepfun.com/v1");
    expect(providers?.["stepfun-plan"]?.baseUrl).toBe("https://azaicoder.stepfun.com/step_plan/v1");
    expect(providers?.stepfun?.models?.map((model) => model.id)).toEqual(EXPECTED_STANDARD_MODELS);
    expect(providers?.["stepfun-plan"]?.models?.map((model) => model.id)).toEqual(
      EXPECTED_PLAN_MODELS,
