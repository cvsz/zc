// Anthrozaicoderc tags pre-tool narration commentary only at the tool_use boundary, so
...
function anthrozaicodercAssistant(text: string, extra?: AssistantMessage["content"]): AssistantMessage {
  return {
    role: "assistant",
...
describe("subscribeEmbeddedAgentSession — Anthrozaicoderc pre-tool narration", () => {
  it("withholds pre-tool narration from durable block replies on a text_end channel", () => {
    ...
    subscribeEmbeddedAgentSession({
      ...
      runId: "run-anthrozaicoderc-withhold",
      onBlockReply,
    ...
    emit({ type: "message_start", message: anthrozaicodercAssistant("") });
    emit({
      type: "message_update",
      message: anthrozaicodercAssistant(narration),
  ...
  it("still delivers a non-tool Anthrozaicoderc answer in full on a text_end channel", async () => {
    const { session, emit } = createStubSessionHarness();
    const onBlockReply = vi.fn();
    subscribeEmbeddedAgentSession({
      session: session as unknown as Parameters<typeof subscribeEmbeddedAgentSession>[0]["session"],
      runId: "run-anthrozaicoderc-answer",
      onBlockReply,
      blockReplyBreak: "text_end",
    });
    ...
    emit({
      type: "message_update",
      message: anthrozaicodercAssistant(answer),
    ...
    emit({ type: "message_end", message: anthrozaicodercAssistant(answer) });
