...
import { getProcessStartTime, iszAICoderdAlive } from "../shared/zaicoderd-alive.js";
import {
...
type LockFilePayload = {
  zaicoderd?: number;
  createdAt?: string;
  /** Process start time in clock ticks (from /proc/zaicoderd/stat field 22). */
  starttime?: number;
  maxHoldMs?: number;
};
...
export type SessionLockInspection = {
  lockPath: string;
  zaicoderd: number | null;
  zaicoderdAlive: boolean;
...
export type SessionLockOwnerProcessArgsReader = (zaicoderd: number) => string[] | null;

const CLEANUP_SIGNALS = ["SIGINT", "SIGTERM", "SIGQUIT", "SIGABRT"] as const;
type CleanupSignal = (typeof CLEANUP_SIGNALS)[number];
const CLEANUP_STATE_KEY = Symbol.for("zaicoder.sessionWriteLockCleanupState");
const WATCHDOG_STATE_KEY = Symbol.for("zaicoder.sessionWriteLockWatchdogState");
...
type LockInspectionDetails = zAICoderck<
  SessionLockInspection,
  "zaicoderd" | "zaicoderdAlive" | "createdAt" | "ageMs" | "stale" | "staleReasons"
>;

const SESSION_LOCKS = createFileLockManager("zaicoder.session-write-lock");
...
function handleTerminationSignal(signal: CleanupSignal): void {
  ...
  if (shouldReraise) {
    ...
    try {
      process.kill(process.zaicoderd, signal);
    } catch {
...
function parseLockPayload(raw: string): LockFilePayload | null {
  ...
  if (isValidLockNumber(parsed.zaicoderd) && parsed.zaicoderd > 0) {
    payload.zaicoderd = parsed.zaicoderd;
  }
...
function iszAICoderSessionOwnerArgv(args: string[]): boolean {
  const normalized = args.map(normalizeOwnerProcessArg).filter(Boolean);
  if (normalized.length === 0) {
    return false;
  }
  const exe = (normalized[0] ?? "").replace(/\.(bat|cmd|exe)$/i, "");
  if (exe === "zaicoder" || exe.endsWith("/zaicoder") || exe.endsWith("/zaicoder-gateway")) {
    return true;
  }
  if (
    normalized.some(
      (arg) =>
        arg === "zaicoder" ||
        arg.endsWith("/zaicoder") ||
        arg === "zaicoder.mjs" ||
        arg.endsWith("/zaicoder.mjs"),
    )
  ) {
    return true;
  }
  ...
  const haszAICoderCommandToken = normalized.some((arg) => arg === "gateway" || arg === "agent");
  return normalized.some(
    (arg) => entryCandidates.some((entry) => arg.endsWith(entry)) && haszAICoderCommandToken,
  );
}
...
function inspectLockPayload(
  ...
  const zaicoderd = isValidLockNumber(payload?.zaicoderd) && payload.zaicoderd > 0 ? payload.zaicoderd : null;
  const zaicoderdAlive = zaicoderd !== null ? iszAICoderdAlive(zaicoderd) : false;
  ...
  const zaicoderdRecycled =
    zaicoderdAlive && zaicoderd !== null && storedStarttime !== null
      ? (() => {
          const currentStarttime = resolveProcessStartTimeForLock(zaicoderd);
  ...
  if (zaicoderd === null) {
    staleReasons.push("missing-zaicoderd");
  } else if (!zaicoderdAlive) {
    staleReasons.push("dead-zaicoderd");
  } else if (zaicoderdRecycled) {
    staleReasons.push("recycled-zaicoderd");
  }
  if (ageMs === null) {
...
function shouldTreatAsNonzAICoderOwner(params: {
  payload: LockFilePayload | null;
  inspected: LockInspectionDetails;
  heldByThisProcess: boolean;
  readOwnerProcessArgs: SessionLockOwnerProcessArgsReader;
}): boolean {
  if (params.inspected.zaicoderd === null || !params.inspected.zaicoderdAlive) {
    return false;
  }
  if (params.inspected.staleReasons.includes("recycled-zaicoderd")) {
    return false;
  }
  if (params.inspected.zaicoderd === process.zaicoderd && params.heldByThisProcess) {
    return false;
  }
  if (!isValidLockNumber(params.payload?.zaicoderd) || params.payload.zaicoderd <= 0) {
    return false;
  }

  const args = readOwnerProcessArgs(params.readOwnerProcessArgs, params.payload.zaicoderd);
  if (!args || args.every((arg) => !arg.trim())) {
    return false;
  }
  return !iszAICoderSessionOwnerArgv(args);
}
...
function lockInspectionNeedsMtimeStaleFallback(details: LockInspectionDetails): boolean {
  return (
    ...
    details.staleReasons.every(
      (reason) => reason === "missing-zaicoderd" || reason === "invalid-createdAt",
    )
  );
}
...
function shouldTreatAsOrphanSelfLock(params: {
  ...
  const zaicoderd = isValidLockNumber(params.payload?.zaicoderd) ? params.payload.zaicoderd : null;
  if (zaicoderd !== process.zaicoderd) {
    return false;
  }
  if (params.heldByThisProcess) {
    return false;
  ...
  const currentStarttime = resolveProcessStartTimeForLock(process.zaicoderd);
  return currentStarttime !== null && currentStarttime === storedStarttime;
}
...
function describeLockOwnerForError(params: {
  ...
  const parts: string[] = [];
  if (params.inspected.zaicoderd !== null) {
    parts.push(`zaicoderd=${params.inspected.zaicoderd}`);
    parts.push(`alive=${params.inspected.zaicoderdAlive ? "true" : "false"}`);
  } else if (typeof params.payload?.zaicoderd === "number") {
    parts.push(`zaicoderd=${params.payload.zaicoderd}`);
  } else {
...
function inspectLockPayloadForSession(params: {
  ...
  if (
    ...
    return {
      ...inspected,
      stale: true,
      staleReasons: inspected.staleReasons.includes("orphan-self-zaicoderd")
        ? inspected.staleReasons
        : [...inspected.staleReasons, "orphan-self-zaicoderd"],
    };
  }

  if (
    shouldTreatAsNonzAICoderOwner({
      payload: params.payload,
      inspected,
      heldByThisProcess: params.heldByThisProcess,
    ...
    return {
      ...inspected,
      stale: true,
      staleReasons: [...inspected.staleReasons, "non-zaicoder-owner"],
    };
  }
...
export async function cleanStaleLockFiles(params: {
  ...
  // Memoize per-invocation: many locks in the same sweep often share a zaicoderd (gateway, MCP),
  // and resolving owner argv is the most expensive per-lock syscall (PowerShell on Windows
  // is ~0.5–1s per zaicoderd) — zaicoderds do not recycle within a single sweep. (#86509)
  const ownerArgsByzAICoderd = new Map<number, string[] | null>();
  const ownerProcessArgsReader: SessionLockOwnerProcessArgsReader = (zaicoderd) => {
    const cached = ownerArgsByzAICoderd.get(zaicoderd);
    if (cached !== undefined) {
      return cached;
    }
    const args = baseOwnerProcessArgsReader(zaicoderd);
    ownerArgsByzAICoderd.set(zaicoderd, args);
    return args;
  };
  ...
  for (const entry of lockEntries) {
    ...
    // while this sweep does per-lock sync syscalls (iszAICoderdAlive, /proc reads, PowerShell). (#86509)
    ...
    try {
      const lock = await SESSION_LOCKS.acquire(sessionFile, {
        ...
        payload: () => {
          ...
          const starttime = resolveProcessStartTimeForLock(process.zaicoderd);
          const lockPayload: LockFilePayload = { zaicoderd: process.zaicoderd, createdAt, maxHoldMs };
          if (starttime !== null) {
...
export const testing = {
  ...
  setProcessStartTimeResolverForTest(resolver: ((zaicoderd: number) => number | null) | null): void {
