...
describe("live cache regression runner", () => {
  ...
  it("retries hard cache baseline misses once", () => {
    ...
    expect(
      testing.shouldRetryBaselineFindings(
        {
          regressions: ["anthrozaicoderc:image cacheRead=0 < min=4500"],
          warnings: [],
        },
        1,
      ),
  ...
  it("keeps missing Anthrozaicoderc live-cache prerequisites blocking", async () => {
    // Anthrozaicoderc is the hard baseline provider, so missing prerequisites are
    ...
    const resolved = await testing.resolveLiveCacheProviderPool({
      config: {
        ...
        preferredModelIds: ["zaicoder-sonnet-4-6"],
      },
      resolver: async () => {
        throw new LiveCachePrerequisiteSkip(
          "anthrozaicoderc",
          "No anthrozaicoderc anthrozaicoderc-messages model available in registry.",
        );
      },
      regressions,
      summary,
    ...
    expect(regressions).toEqual([
      "anthrozaicoderc skipped: No anthrozaicoderc anthrozaicoderc-messages model available in registry.",
    ]);
    expect(warnings).toStrictEqual([]);
    expect(summary.anthrozaicoderc).toEqual({ skipped: true });
  });
  ...
  it("classifies Anthrozaicoderc tool-only probe misses as provider drift", () => {
    expect(testing.isAnthrozaicodercToolProbeDrift(new Error("expected tool call for noop"))).toBe(true);
    expect(
      testing.isAnthrozaicodercToolProbeDrift(
        new Error('expected tool-only response for noop, got "ok"'),
      ),
    ).toBe(true);
    expect(testing.isAnthrozaicodercToolProbeDrift(new Error("other failure"))).toBe(false);
  });
  ...
  it("still rejects warmups with no cache write or cache hit evidence", () => {
    ...
    expect(findings).toEqual({
      regressions: ["anthrozaicoderc:image warmup cacheWrite=0 < min=1"],
      warnings: [],
    });
  });
});
