...
function progressModeConfig(acp?: zAICoderConfig["acp"]): zAICoderConfig {
  return {
    ...(acp ? { acp } : {}),
    channels: {
      forum: {
...
describe("startAcpSpawnParentStreamRelay", () => {
  ...
  it("resolves ACP spawn stream log path from session metadata", () => {
    readAcpSessionEntryMock.mockReturnValue({
      storePath: "/tmp/zaicoder/agents/codex/sessions/sessions.json",
      entry: {
        sessionId: "sess-123",
        sessionFile: "/tmp/zaicoder/agents/codex/sessions/sess-123.jsonl",
      },
    });
    resolveSessionFilePathMock.mockReturnValue(
      "/tmp/zaicoder/agents/codex/sessions/sess-123.jsonl",
    );
    ...
    expect(resolved).toBe("/tmp/zaicoder/agents/codex/sessions/sess-123.acp-stream.jsonl");
    ...
    expect(options.storePath).toBe("/tmp/zaicoder/agents/codex/sessions/sessions.json");
  });
});
