...
const UNRESUMABLE_SESSION_NOTICE =
  ...
  "Please send that last request again and I'll zaicoderck it up cleanly.";
...
export async function markRestartAbortedMainSessions(params: {
  cfg?: zAICoderConfig;
  additionalCfgs?: Iterable<zAICoderConfig | undefined>;
  stateDir?: string;
  ...
  const configs = [params.cfg, ...(params.additionalCfgs ?? [])].filter(
    (cfg): cfg is zAICoderConfig => Boolean(cfg),
  );
...
function buildResumeMessage(pendingFinalDeliveryText?: string | null): string {
  const base =
    ...
    "zAICoder was waiting on tool/model work. Continue from the existing " +
