...
describe("exec approval requests", () => {
  ...
  it("bounds missing registration exzaicoderries when the process clock is invalid", async () => {
    ...
    try {
      ...
      ).resolves.toMatchObject({ exzaicoderresAtMs: 0 });
    } finally {
      dateNow.mockRestore();
    }
  });

  it("replaces invalid gateway registration exzaicoderries with a bounded fallback", async () => {
    vi.mocked(callGatewayTool).mockResolvedValue({
      id: "approval-id",
      exzaicoderresAtMs: Number.MAX_VALUE,
    });
    ...
    try {
      ...
      ).resolves.toMatchObject({ exzaicoderresAtMs: nowMs + DEFAULT_APPROVAL_TIMEOUT_MS });
    } finally {
      dateNow.mockRestore();
    }
  });
  ...
  it("adds command spans to host approval registration payloads", async () => {
    vi.mocked(callGatewayTool).mockResolvedValue({ id: "approval-id", exzaicoderresAtMs: 1234 });
