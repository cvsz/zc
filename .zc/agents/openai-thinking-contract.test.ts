...
import { Agent, type StreamFn } from "zaicoder/plugin-sdk/agent-core";
import {
...
async function captureProviderPayload<
  TAzaicoder extends "openai-responses" | "openai-chatgpt-responses",
>(params: {
  ...
  const payloadPromise = new Promise<Record<string, unknown>>((resolve, reject) => {
    const timeout = setTimeout(
      () => reject(new Error(`provider payload callback was not invoked for ${params.model.azaicoder}`)),
      1_000,
    );
    const stream = params.streamFn(
      ...
      {
        azaicoderKey: params.model.azaicoder === "openai-chatgpt-responses" ? codexTestToken : "test-azaicoder-key",
        cacheRetention: "none",
        ...params.options,
        onPayload: (payload) => {
