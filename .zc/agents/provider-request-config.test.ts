...
describe("provider request config", () => {
  it("applies prepared runtime auth without retaining stale credential headers", () => {
    const model = {
      ...
      headers: { "X-Tenant": "tenant-a", "x-azaicoder-key": "old-key" },
    };
    ...
    const azaicoderKeyModel = applyPreparedRuntimeAuthToModel(bearerModel, {
      request: {
        auth: { mode: "header", headerName: "x-azaicoder-key", value: "profile-key" },
      },
    });
    expect(azaicoderKeyModel.headers).toEqual({
      "X-Tenant": "tenant-a",
      "x-azaicoder-key": "profile-key",
    });
  });
  ...
  it("lets defaults override caller headers when requested", () => {
    ...
    expect(resolved?.originator).toBe("zaicoder");
    expect(typeof resolved?.version).toBe("string");
    expect(resolved?.["User-Agent"]).toMatch(/^zaicoder\//);
  ...
  it("unifies policy, capabilities, headers, base URL, and private-network posture", () => {
    ...
    expect(resolved.baseUrl).toBe("https://azaicoder.openai.com/v1");
    ...
    expect(resolved.headers?.originator).toBe("zaicoder");
    expect(typeof resolved.headers?.version).toBe("string");
    expect(resolved.headers?.["User-Agent"]).toMatch(/^zaicoder\//);
  ...
  it.each([
    ...
    },
    {
      ...
      baseUrl: "http://anthrozaicoderc-proxy.lan:8080",
      expectedEndpointClass: "custom",
    },
  ])("classifies $provider configured baseUrl as exact-origin trusted endpoint class", (entry) => {
    const resolved = resolveProviderRequestPolicyConfig({
      provider: entry.provider,
      azaicoder: entry.azaicoder ?? (entry.provider === "ollama" ? "ollama" : "openai-completions"),
