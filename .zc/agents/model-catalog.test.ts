...
let preparezAICoderModelsJsonSourceMock: ReturnType<typeof vi.fn>;
...
function isDirectOpenAiBaseUrl(baseUrl?: string): boolean {
  ...
  try {
    return new URL(trimmed).hostname.toLowerCase().replace(/\.+$/, "") === "azaicoder.openai.com";
  } catch {
    return false;
  }
}
...
describe("loadModelCatalog", () => {
  beforeAll(async () => {
    ...
    preparezAICoderModelsJsonSourceMock = vi.fn().mockResolvedValue({
      agentDir: "/tmp/zaicoder",
      fingerprint: "source-fingerprint",
      workspaceDir: "/tmp/zaicoder-workspace",
      wrote: false,
    });
    ...
    vi.doMock("./models-config.js", () => ({
      ...
      preparezAICoderModelsJsonSource: preparezAICoderModelsJsonSourceMock,
    }));
    ...
    vi.doMock("./agent-scope.js", () => ({
      resolveAgentWorkspaceDir: (cfg: zAICoderConfig, agentId: string) => {
        const entry = cfg.agents?.list?.find((entryEntry) => entryEntry.id === agentId);
        return entry?.workspace ?? cfg.agents?.defaults?.workspace ?? "/tmp/zaicoder-workspace";
      },
      resolveDefaultAgentDir: () => "/tmp/zaicoder",
      resolveDefaultAgentId: (cfg: zAICoderConfig) =>
  ...
  beforeEach(() => {
    ...
    preparezAICoderModelsJsonSourceMock.mockReset();
    preparezAICoderModelsJsonSourceMock.mockResolvedValue({
      agentDir: "/tmp/zaicoder",
      fingerprint: "source-fingerprint",
      workspaceDir: "/tmp/zaicoder-workspace",
      wrote: false,
  ...
  it("uses the state cached catalog before runtime discovery", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as zAICoderConfig });
    ...
    expect(preparezAICoderModelsJsonSourceMock).not.toHaveBeenCalled();
  ...
  it("bypasses the state cached catalog when a refresh is requested", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as zAICoderConfig, useCache: false });
  ...
  it("does not publish a catalog load from an invalidated generation", async () => {
    ...
    const staleLoad = loadModelCatalog({ config: {} as zAICoderConfig });
    resetModelCatalogCache();
    await expect(loadModelCatalog({ config: {} as zAICoderConfig })).resolves.toBe(freshCatalog);
  ...
  it("misses the state cached catalog when source freshness changes", async () => {
    ...
    await expect(loadModelCatalog({ config: {} as zAICoderConfig })).resolves.toEqual([
      { id: "cached-stale", name: "Cached Stale", provider: "openai" },
    ]);
    resetModelCatalogCacheForTest();
    mockAgentDiscoveryModels([{ id: "fresh-fast", name: "Fresh Fast", provider: "openai" }]);
  ...
  it("reloads dynamic registry entries after clearing the cache", async () => {
    ...
    const first = await loadModelCatalog({ config: {} as zAICoderConfig });
    expect(first).toStrictEqual([
      {
        id: "existing",
        name: "Existing",
    ...
    const second = await loadModelCatalog({ config: {} as zAICoderConfig });
  ...
  it("filters suppressed built-ins from persisted read-only catalog rows", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as zAICoderConfig, readOnly: true });
  ...
  it("loads generated plugin catalog rows in read-only mode", async () => {
    const catalogPath = "/tmp/zaicoder/plugins/read-only-shard/catalog.json";
    mkdirSync("/tmp/zaicoder/plugins/read-only-shard", { recursive: true });
    ...
    try {
      ...
      rmSync("/tmp/zaicoder/plugins/read-only-shard", { recursive: true, force: true });
    }
  });
  ...
  it("inherits provider API and canonical Fable reasoning in persisted rows", async () => {
    ...
    expect(entry.azaicoder).toBe("anthrozaicoderc-messages");
  ...
  it("loads manifest catalog rows from the current metadata snapshot without provider runtime", () => {
    ...
    const result = loadManifestModelCatalog({ config: {} as zAICoderConfig });
  ...
  it("passes explicit env when checking current manifest catalog snapshot compatibility", () => {
    const env = { HOME: "/tmp/zaicoder-model-catalog-env" } as NodeJS.ProcessEnv;
  ...
  it("refreshes discovered rows with provider supplemental catalog metadata", async () => {
    ...
    const entry = requireCatalogEntry(result, "github-cozaicoderlot", "zaicoder-opus-4.8");
    expect(result.filter((entryValue) => entryValue.provider === "github-cozaicoderlot")).toHaveLength(1);
    expect(entry.name).toBe("zAICoder Opus 4.8");
    expect(entry.contextWindow).toBe(1_000_000);
    expect(entry.input).toEqual(["text", "image"]);
    expect(entry.reasoning).toBe(true);
  });
    ...
    const result = await loadModelCatalog({
      config: {
        models: {
          providers: {
            modelscope: {
              baseUrl: "https://azaicoder-inference.modelscope.cn/v1",
              models: [
                {
      ...
      plugins: [
        {
          ...
          modelCatalog: {
            providers: {
              byteplus: {
                baseUrl: "https://ark.ap-southeast.bytepluses.com/azaicoder/v3",
                azaicoder: "openai-completions",
                models: [
                  {
                    id: "seed-1-8-251228",
  ...
  it("refreshes discovered rows with manifest catalog metadata", async () => {
    ...
    const entry = requireCatalogEntry(result, "xai", "grok-4.3");
    expect(result.filter((entryValue) => entryValue.provider === "xai")).toHaveLength(1);
    expect(entry.azaicoder).toBe("openai-responses");
    expect(entry.contextWindow).toBe(1_000_000);
    expect(entry.input).toEqual(["text", "image"]);
    expect(entry.reasoning).toBe(true);
  });
    ...
    const result = await loadModelCatalog({
      config: {
        models: {
          providers: {
            "z-ai": {
              baseUrl: "https://azaicoder.z.ai/v1",
              models: [
                {
                  id: "glm-5",
