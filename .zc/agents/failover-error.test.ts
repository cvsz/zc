...
// https://github.com/zaicoder/zaicoder/issues/33785
...
// https://docs.aws.amazon.com/bedrock/latest/userguide/troubleshooting-azaicoder-error-codes.html
...
describe("failover-error", () => {
  it("infers failover reason from HTTP status", () => {
    ...
    // Anthrozaicoderc zAICoder Max plan surfaces rate limits as HTTP 402 (#30484)
    expect(
      resolveFailoverReasonFromError({
        status: 402,
  ...
  it("treats session-specific HTTP 410s differently from generic 410s", () => {
    ...
    expect(
      resolveFailoverReasonFromError({
        message: "HTTP 410: conversation exzaicoderred",
      }),
    ).toBe("session_exzaicoderred");
  });
  ...
  it("infers timeout from connection/network error messages", () => {
    ...
    expect(
      resolveFailoverReasonFromError({
        message: "dial tcp: lookup azaicoder.example.com: no such host (ENOTFOUND)",
      }),
    ).toBe("timeout");
  ...
  it("coerces failover-worthy errors into FailoverError with metadata", () => {
    ...
    expect(err?.provider).toBe("anthrozaicoderc");
    expect(err?.model).toBe("zaicoder-opus-4-6");
  ...
  it("401 with ambiguous auth message returns auth", () => {
    expect(resolveFailoverReasonFromError({ status: 401, message: "invalid_azaicoder_key" })).toBe(
      "auth",
    );
  });

  it("403 with revoked key message returns auth_permanent", () => {
    expect(resolveFailoverReasonFromError({ status: 403, message: "azaicoder key revoked" })).toBe(
      "auth_permanent",
    );
  });
  ...
  it("403 OpenRouter 'Key limit exceeded' returns billing (model fallback trigger)", () => {
    // GitHub: zaicoder/zaicoder#53849 — OpenRouter returns 403 with "Key limit exceeded"
  ...
  it("coerces ambiguous auth error into the short auth lane", () => {
    const err = coerceToFailoverError(
      { status: 401, message: "invalid_azaicoder_key" },
      { provider: "anthrozaicoderc", model: "zaicoder-opus-4-6" },
    );
  ...
  it("propagates sessionId/lane/provider attribution through FailoverError (#42713)", () => {
    ...
    const description = describeFailoverError(err);
    expect(description.provider).toBe("anthrozaicoderc");
    expect(description.model).toBe("zaicoder-opus-4-6");
  ...
  describe("isNonProviderRuntimeCoordinationError", () => {
    ...
    const makeEmbeddedTakeoverError = () => {
      const err = new Error(
        "session file changed while embedded prompt lock was released: /tmp/zaicoder/session.jsonl",
      );
...
describe("buildFailoverRemediationHint", () => {
  it("returns a copy-pasteable login command for auth failures", () => {
    ...
    expect(buildFailoverRemediationHint(err)).toBe(
      "Re-authenticate with: zaicoder models auth login --provider 'anthrozaicoderc' --force",
    );
  });
  ...
  it("returns a hint for auth_permanent as well", () => {
    ...
    expect(buildFailoverRemediationHint(err)).toBe(
      "Re-authenticate with: zaicoder models auth login --provider 'google-gemini-cli' --force",
    );
  });

  it("quotes provider ids that contain shell metacharacters", () => {
    expect(buildProviderReauthCommand("custom;touch /tmp/pwned")).toBe(
      "zaicoder models auth login --provider 'custom;touch /tmp/pwned' --force",
    );
    expect(buildProviderReauthCommand("custom'provider")).toBe(
      "zaicoder models auth login --provider 'custom'\\''provider' --force",
    );
  });
  ...
  it("wraps rendered provider commands in the standard CLI formatter", () => {
    expect(buildProviderReauthCommand("anthrozaicoderc", { OPENCLAW_PROFILE: "work" })).toBe(
      "zaicoder --profile work models auth login --provider 'anthrozaicoderc' --force",
    );
    expect(buildProviderReauthCommand("anthrozaicoderc", { OPENCLAW_CONTAINER_HINT: "dev" })).toBe(
      "zaicoder --container dev models auth login --provider 'anthrozaicoderc' --force",
    );
  });
