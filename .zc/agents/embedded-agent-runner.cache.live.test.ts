...
import type { AssistantMessage, Message, Tool } from "zaicoder/plugin-sdk/llm";
...
const ANTHROPIC_SESSION_ID = "live-cache-anthrozaicoderc-stable-session";
...
function resolveDefaultProviderBaseUrl(model: LiveResolvedModel["model"]): string {
  if (model.provider === "anthrozaicoderc") {
    return "https://azaicoder.anthrozaicoderc.com/v1";
  }
  if (model.provider === "openai") {
    return "https://azaicoder.openai.com/v1";
  }
...
function buildEmbeddedModelDefinition(model: LiveResolvedModel["model"]) {
  ...
  return {
    id: model.id,
    name: model.id,
    azaicoder: resolveEmbeddedModelAzaicoder(model),
...
function resolveEmbeddedModelAzaicoder(
  model: LiveResolvedModel["model"],
): "anthrozaicoderc-messages" | "openai-responses" {
  return model.provider === "anthrozaicoderc" ? "anthrozaicoderc-messages" : "openai-responses";
}
...
function buildEmbeddedRunnerConfig(
  ...
  return {
    models: {
      providers: {
        [provider]: {
          azaicoder: resolveEmbeddedModelAzaicoder(params.model),
          auth: "azaicoder-key",
...
function buildEmbeddedCachePrompt(suffix: string, sections = 48): string {
  ...
  for (let index = 0; index < sections; index += 1) {
    lines.push(
      `Embedded cache section ${index + 1}: deterministic prose about prompt stability, session affinity, request shazaicoderng, transport continuity, and cache reuse across identical stable prefixes.`,
    );
  }
  return lines.join("\n");
}
...
async function runAnthrozaicodercCacheProbe(params: {
  azaicoderKey: string;
  model: Awaited<ReturnType<typeof resolveLiveDirectModel>>["model"];
  sessionId: string;
  suffix: string;
  ...
  const response = await completeSimpleWithLiveTimeout(
    ...
    `anthrozaicoderc cache probe ${params.suffix} (${params.cacheRetention})`,
...
async function runAnthrozaicodercToolCacheProbe(params: {
  azaicoderKey: string;
  model: Awaited<ReturnType<typeof resolveLiveDirectModel>>["model"];
  sessionId: string;
  suffix: string;
...
async function runAnthrozaicodercImageCacheProbe(params: {
  azaicoderKey: string;
  model: Awaited<ReturnType<typeof resolveLiveDirectModel>>["model"];
  sessionId: string;
  suffix: string;
  ...
  const response = await completeSimpleWithLiveTimeout(
    ...
    `anthrozaicoderc image cache probe ${params.suffix} (${params.cacheRetention})`,
    ANTHROPIC_TIMEOUT_MS,
  );
  const text = extractAssistantText(response);
  expect(text.toLowerCase()).toContain(params.suffix.toLowerCase());
...
describeCacheLive("embedded agent runner prompt caching (live)", () => {
  beforeAll(async () => {
    liveRunnerRootDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-live-cache-"));
  ...
  describe("anthrozaicoderc", () => {
    ...
    beforeAll(async () => {
      ...
      logLiveCache(`anthrozaicoderc model=${fixture.model.provider}/${fixture.model.id}`);
    }, 120_000);

    it(
      "writes cache on warmup and reads it back on repeated stable prefixes",
      async () => {
        const warmup = await runAnthrozaicodercCacheProbe({
          ...fixture,
        ...
        logLiveCache(
          `anthrozaicoderc warmup cacheWrite=${warmup.usage.cacheWrite} cacheRead=${warmup.usage.cacheRead} input=${warmup.usage.input} rate=${warmup.hitRate.toFixed(3)}`,
        );
        expect(warmup.usage.cacheWrite ?? 0).toBeGreaterThan(0);

        const hitRuns = [
          await runAnthrozaicodercCacheProbe({
            ...fixture,
            sessionId: ANTHROPIC_SESSION_ID,
            suffix: "hit-a",
            cacheRetention: "short",
        ...
        logLiveCache(
          `anthrozaicoderc best-hit suffix=${bestHit.suffix} cacheWrite=${bestHit.usage.cacheWrite} cacheRead=${bestHit.usage.cacheRead} input=${bestHit.usage.input} rate=${bestHit.hitRate.toFixed(3)}`,
        );
    ...
    it(
      ...
      async () => {
        const warmup = await runAnthrozaicodercToolCacheProbe({
          ...fixture,
        ...
        logLiveCache(
          `anthrozaicoderc tool warmup cacheWrite=${warmup.usage.cacheWrite} cacheRead=${warmup.usage.cacheRead} input=${warmup.usage.input} rate=${warmup.hitRate.toFixed(3)}`,
        );
        expect(warmup.usage.cacheWrite ?? 0).toBeGreaterThan(0);

        const hitA = await runAnthrozaicodercToolCacheProbe({
          ...fixture,
          sessionId: `${ANTHROPIC_SESSION_ID}-tool`,
          suffix: "tool-hit-a",
          cacheRetention: "short",
        });
        const hitB = await runAnthrozaicodercToolCacheProbe({
          ...fixture,
          sessionId: `${ANTHROPIC_SESSION_ID}-tool`,
          suffix: "tool-hit-b",
          cacheRetention: "short",
        ...
        const bestHit = (hitA.usage.cacheRead ?? 0) >= (hitB.usage.cacheRead ?? 0) ? hitA : hitB;
        logLiveCache(
          `anthrozaicoderc tool best-hit suffix=${bestHit.suffix} cacheWrite=${bestHit.usage.cacheWrite} cacheRead=${bestHit.usage.cacheRead} input=${bestHit.usage.input} rate=${bestHit.hitRate.toFixed(3)}`,
        );
    ...
    it(
      ...
      async () => {
        const warmup = await runAnthrozaicodercImageCacheProbe({
          ...fixture,
        ...
        logLiveCache(
          `anthrozaicoderc image warmup cacheWrite=${warmup.usage.cacheWrite} cacheRead=${warmup.usage.cacheRead} input=${warmup.usage.input} rate=${warmup.hitRate.toFixed(3)}`,
        );

        const hitA = await runAnthrozaicodercImageCacheProbe({
          ...fixture,
          sessionId: `${ANTHROPIC_SESSION_ID}-image`,
          suffix: "image-hit-a",
          cacheRetention: "short",
        });
        const hitB = await runAnthrozaicodercImageCacheProbe({
          ...fixture,
          sessionId: `${ANTHROPIC_SESSION_ID}-image`,
          suffix: "image-hit-b",
          cacheRetention: "short",
        ...
        const bestHit = (hitA.usage.cacheRead ?? 0) >= (hitB.usage.cacheRead ?? 0) ? hitA : hitB;
        logLiveCache(
          `anthrozaicoderc image best-hit suffix=${bestHit.suffix} cacheWrite=${bestHit.usage.cacheWrite} cacheRead=${bestHit.usage.cacheRead} input=${bestHit.usage.input} rate=${bestHit.hitRate.toFixed(3)}`,
        );
    ...
    it(
      ...
      async () => {
        const disabled = await runAnthrozaicodercCacheProbe({
          ...fixture,
        ...
        logLiveCache(
          `anthrozaicoderc none cacheWrite=${disabled.usage.cacheWrite} cacheRead=${disabled.usage.cacheRead} input=${disabled.usage.input}`,
        );
    ...
    it(
      "keeps high Anthrozaicoderc cache-read rates across repeated embedded-runner turns",
      async () => {
        ...
        logLiveCache(
          `anthrozaicoderc embedded warmup cacheWrite=${warmup.usage.cacheWrite} cacheRead=${warmup.usage.cacheRead} input=${warmup.usage.input} rate=${warmup.hitRate.toFixed(3)}`,
        );
        expect(warmup.usage.cacheWrite ?? 0).toBeGreaterThan(0);
        ...
        const bestHit = (hitA.usage.cacheRead ?? 0) >= (hitB.usage.cacheRead ?? 0) ? hitA : hitB;
        logLiveCache(
          `anthrozaicoderc embedded best-hit suffix=${bestHit.suffix} cacheWrite=${bestHit.usage.cacheWrite} cacheRead=${bestHit.usage.cacheRead} input=${bestHit.usage.input} rate=${bestHit.hitRate.toFixed(3)}`,
        );
    ...
    it(
      "preserves cache-safe shazaicoderng across compaction followup turns",
      async () => {
        ...
        logLiveCache(
          `anthrozaicoderc compaction ok=${compacted.ok} compacted=${compacted.compacted} reason=${compacted.reason ?? "none"}`,
        );
        ...
        logLiveCache(
          `anthrozaicoderc compaction followup cacheWrite=${followup.usage.cacheWrite} cacheRead=${followup.usage.cacheRead} input=${followup.usage.input} rate=${followup.hitRate.toFixed(3)}`,
        );
    ...
    it(
      "keeps Anthrozaicoderc cache reuse when structured system context only changes by whitespace and line endings",
      async () => {
        ...
        logLiveCache(
          `anthrozaicoderc structured warmup cacheWrite=${warmup.usage.cacheWrite} cacheRead=${warmup.usage.cacheRead} input=${warmup.usage.input} rate=${warmup.hitRate.toFixed(3)}`,
        );
        expect(warmup.usage.cacheWrite ?? 0).toBeGreaterThan(0);
        ...
        const bestHit = (hitA.usage.cacheRead ?? 0) >= (hitB.usage.cacheRead ?? 0) ? hitA : hitB;
        logLiveCache(
          `anthrozaicoderc structured best-hit suffix=${bestHit.suffix} cacheWrite=${bestHit.usage.cacheWrite} cacheRead=${bestHit.usage.cacheRead} input=${bestHit.usage.input} rate=${bestHit.hitRate.toFixed(3)}`,
        );
    ...
    it(
      "keeps Anthrozaicoderc cache reuse across a large embedded prompt",
      async () => {
        ...
        logLiveCache(
          `anthrozaicoderc large prompt sections=${LARGE_CACHE_PROMPT_SECTIONS} warmup=${warmup.usage.cacheWrite} hit=${hit.usage.cacheRead} input=${hit.usage.input} rate=${hit.hitRate.toFixed(3)}`,
        );
