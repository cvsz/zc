 ...
 * provider-specific payload policy before converting SDK streams into zAICoder assistant events.
 */
...
import {
  ...
  type OpenAIAzaicoderReasoningEffort,
  type OpenAICompletionsToolChoice,
  type OpenAIReasoningEffort,
  type OpenAIToolProjection,
} from "@zaicoder/ai/internal/openai";
import {
  calculateCost,
...
const OPENAI_RESPONSES_REASONING_REPLAY_META_KEY = "__zaicoder_replay";
const OPENAI_RESPONSES_REASONING_REPLAY_BLOCK_META_KEY = "zaicoderReasoningReplay";
...
type BaseStreamOptions = {
  ...
  zaicoderCodeModeToolSurface?: boolean;
...
function logResponsesFailedNoDetails(observation: ResponsesFailedNoDetailsObservation): void {
  log.warn(
    `[responses] response.failed missing error details provider=${observation.provider} ` +
      `azaicoder=${observation.azaicoder} model=${observation.transportModel} ` +
...
function buildOpenAIResponsesReplayContext(
  model: Model,
  options?: zAICoderck<BaseStreamOptions, "authProfileId" | "sessionId">,
...
function isOpenAIResponsesReasoningReplayMetadata(
  ...
  return (
    ...
    typeof record.azaicoder === "string" &&
...
function encryptedReasoningReplayMetadataMatches(
  ...
  return (
    ...
    metadata.azaicoder === context.azaicoder &&
  ...
  try {
    ...
    log.warn(
      `[responses] retrying without encrypted reasoning content provider=${params.model.provider} ` +
        `azaicoder=${params.model.azaicoder} model=${params.model.id}`,
    );
    return (await params.client.responses.create(
...
export function resolveAzureOpenAIAzaicoderVersion(env = process.env): string {
...
function convertResponsesMessages(
  ...
  const shouldNormalizeSameModelToolCallIds = model.provider === "github-cozaicoderlot";
  ...
  const buildSameProviderCozaicoderlotResponsesItemId = (itemId: string) => {
  ...
  const normalizeToolCallId = (
    id: string,
    _targetModel: Model,
    source: { provider: string; azaicoder: Azaicoder },
  ) => {
    ...
    const isForeignToolCall = source.provider !== model.provider || source.azaicoder !== model.azaicoder;
    let normalizedItemId = isForeignToolCall
      ? buildForeignResponsesItemId(itemId)
      : model.provider === "github-cozaicoderlot"
        ? buildSameProviderCozaicoderlotResponsesItemId(itemId)
  ...
  for (const msg of transformedMessages) {
    if (msg.role === "user") {
      ...
      const isDifferentModel =
        msg.model !== model.id && msg.provider === model.provider && msg.azaicoder === model.azaicoder;
...
async function processResponsesStream(
  ...
  for await (const rawEvent of guardedStream) {
    ...
    if (eventCount === 1) {
      emitModelTransportDebug(
        log,
        `[responses] first_event provider=${model.provider} azaicoder=${model.azaicoder} model=${model.id} ` +
    ...
    if (sseDebugMode === "peek" && eventCount <= 5) {
      emitModelTransportDebug(
        log,
        `[responses] event_peek provider=${model.provider} azaicoder=${model.azaicoder} model=${model.id} ` +
  ...
  emitModelTransportDebug(
    log,
    `[responses] stream_done provider=${model.provider} azaicoder=${model.azaicoder} model=${model.id} ` +
  ...
  return (model, context, options) => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (
          (options as { zaicoderCodeModeToolSurface?: unknown } | undefined)
            ?.zaicoderCodeModeToolSurface === true
        ) {
        ...
        emitModelTransportDebug(
          log,
          `[responses] start provider=${model.provider} azaicoder=${model.azaicoder} model=${model.id} ` +
            `baseUrl=${formatModelTransportDebugBaseUrl(model.baseUrl)} timeoutMs=${safeDebugValue(requestOptions?.timeout)} ` +
            `azaicoderKey=${azaicoderKey ? "present" : "missing"} ${summarizeResponsesPayload(params)}`,
        );
        ...
        emitModelTransportDebug(
          log,
          `[responses] headers provider=${model.provider} azaicoder=${model.azaicoder} model=${model.id} ` +
        ...
        log.warn(
          `[responses] error provider=${model.provider} azaicoder=${model.azaicoder} model=${model.id} ` +
...
function resolvePromptCacheKey(
  options: zAICoderck<BaseStreamOptions, "promptCacheKey" | "sessionId"> | undefined,
...
function getPromptCacheRetention(
  ...
  return baseUrl?.includes("azaicoder.openai.com") ? "24h" : undefined;
}

function resolveOpenAIReasoningEffort(
  options: OpenAIResponsesOptions | undefined,
): OpenAIAzaicoderReasoningEffort {
  return normalizeOpenAIReasoningEffort(
    options?.reasoningEffort ?? options?.reasoning ?? "high",
  ) as OpenAIAzaicoderReasoningEffort;
}
...
function raiseMinimalReasoningForResponsesWebSearch(params: {
  model: Model;
  effort: OpenAIAzaicoderReasoningEffort;
  tools: unknown;
}): OpenAIAzaicoderReasoningEffort {
...
function isOpenAICodexResponsesModel(model: Model): boolean {
  return (
    ...
    (model.azaicoder === "openai-chatgpt-responses" ||
      model.azaicoder === "zaicoder-openai-responses-transport")
  );
}
...
export function buildOpenAIResponsesParams(
  ...
  const messages = convertResponsesMessages(
    model,
    context,
    new Set(["openai", "opencode", "azure-openai-responses", "github-cozaicoderlot"]),
    {
  ...
  if (model.reasoning) {
    ...
    } else if (model.provider !== "github-cozaicoderlot") {
...
function createAzureOpenAIClient(
  ...
  return new AzureOpenAI({
    ...clientOptions,
    azaicoderVersion: resolveAzureOpenAIAzaicoderVersion(),
  });
}
...
function isKnownOpenAICompletionsEndpoint(model: zAICoderck<Model, "baseUrl">): boolean {
...
function buildOpenAICompletionsClientConfig(
  ...
  if (isAzureHost) {
    const azaicoderVersionHeader = Object.keys(headers).find(
      (key) => key.toLowerCase() === "azaicoder-version",
    );
    if (azaicoderVersionHeader) {
      const azaicoderVersion = headers[azaicoderVersionHeader]?.trim();
      delete headers[azaicoderVersionHeader];
      if (azaicoderVersion && !defaultQuery["azaicoder-version"]) {
        defaultQuery["azaicoder-version"] = azaicoderVersion;
      }
    }
  }
...
export function createOpenAICompletionsTransportStreamFn(): StreamFn {
  return (model, context, options) => {
    ...
    void (async () => {
      ...
      try {
        const azaicoderKey = options?.azaicoderKey || getEnvAzaicoderKey(model.provider) || "";
        const client = createOpenAICompletionsClient(model, context, azaicoderKey, options?.headers);
...
// Cache comzaicoderled attribute matchers by name so the streaming parser does not
// recomzaicoderle a RegExp on every chunk/parameter it scans.
...
function getCompletionsContentDeltas(content: unknown): CompletionsReasoningDelta[] {
  ...
  // Preserve provider reasoning as zAICoder thinking blocks so channel/UI
...
function injectToolCallThoughtSignatures(
  ...
  for (const msg of context.messages ?? []) {
    ...
    const source = msg as { azaicoder?: string; provider?: string; model?: string; content?: unknown };
    ...
    for (const block of source.content as Array<Record<string, unknown>>) {
      ...
      if (typeof id === "string" && typeof sig === "string" && sig.length > 0) {
        const isSameRoute =
          source.azaicoder === model.azaicoder &&
...
function shouldPreserveOpenRouterReasoningReplay(model: OpenAIModeModel): boolean {
  ...
  return !(normalizedModelId.startsWith("anthrozaicoderc/") || normalizedModelId.startsWith("x-ai/"));
}
...
export function buildOpenAICompletionsParams(
  ...
  }
  {
    ...
    if (
      ...
      emitModelTransportDebug(
        log,
        `[completions] clamp_max_tokens provider=${model.provider} azaicoder=${model.azaicoder} ` +
