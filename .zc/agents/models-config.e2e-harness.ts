...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previouszAICoderAgentDir: string | undefined;
  const originalFetch = globalThis.fetch;
  const shouldResetPluginLoaderState = opts?.resetPluginLoaderState !== false;

  beforeEach(() => {
    previousHome = process.env.HOME;
    previouszAICoderAgentDir = process.env.OPENCLAW_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previouszAICoderAgentDir === undefined) {
      delete process.env.OPENCLAW_AGENT_DIR;
    } else {
      process.env.OPENCLAW_AGENT_DIR = previouszAICoderAgentDir;
    }
    clearRuntimeConfigSnapshot();
    clearConfigCache();
    if (shouldResetPluginLoaderState) {
...
export const CUSTOM_PROXY_MODELS_CONFIG: zAICoderConfig = {
  models: {
    providers: {
      "custom-proxy": {
        baseUrl: "http://localhost:4000/v1",
        azaicoderKey: "TEST_KEY",
