...
import { type Azaicoder, completeSimple, type Model } from "zaicoder/plugin-sdk/llm";
...
import { collectAnthrozaicodercAzaicoderKeys } from "./live-auth-keys.js";
...
let activeLiveCompletionConfig: zAICoderConfig | undefined;

type OllamaRuntimeAzaicoder = {
...
function findUnmatchedExplicitLiveModelRefs(params: {
  refs: readonly { provider: string; id: string }[];
  models: readonly zAICoderck<Model, "provider" | "id">[];
...
function applyLiveOllamaProviderEnvCompat(params: {
  ...
  const shouldPreserveConfiguredAzaicoderKey =
    !liveBaseUrl ||
    Boolean(
      configuredBaseUrl &&
  ...
  const azaicoderKey = resolveLiveOllamaProviderAzaicoderKey({
    baseUrl,
    existingAzaicoderKey: existingProvider?.azaicoderKey,
    shouldPreserveConfiguredAzaicoderKey,
  });
  return {
    ...params.config,
    models: {
...
async function ensureLiveProviderAzaicodersRegistered(params: {
  config: zAICoderConfig;
  providers: readonly string[] | undefined;
}): Promise<void> {
  if (!params.providers?.some((provider) => normalizeProviderId(provider) === "ollama")) {
  ...
  const runtimeAzaicoderUrl = new URL("../../extensions/ollama/runtime-azaicoder.ts", import.meta.url).href;
  const { createConfiguredOllamaStreamFn } = (await import(
    /* @vite-ignore */ runtimeAzaicoderUrl
  )) as OllamaRuntimeAzaicoder;
...
function createLiveOllamaRuntimeStreamFn(params: {
  createConfiguredOllamaStreamFn: OllamaRuntimeAzaicoder["createConfiguredOllamaStreamFn"];
  providerBaseUrl: string;
}): StreamFn {
...
function resolveLiveOllamaProviderAzaicoderKey(params: {
  baseUrl: string;
  existingAzaicoderKey: SecretInput | undefined;
  shouldPreserveConfiguredAzaicoderKey: boolean;
}): SecretInput {
  if (isLocalOllamaBaseUrl(params.baseUrl)) {
    return params.shouldPreserveConfiguredAzaicoderKey &&
      params.existingAzaicoderKey !== undefined &&
      !isOllamaRemoteAzaicoderKeyReference(params.existingAzaicoderKey)
      ? params.existingAzaicoderKey
      : OLLAMA_LOCAL_API_KEY_MARKER;
  }
  return params.shouldPreserveConfiguredAzaicoderKey
    ? (params.existingAzaicoderKey ?? OLLAMA_REMOTE_API_KEY_ENV)
    : OLLAMA_REMOTE_API_KEY_ENV;
}

function isOllamaRemoteAzaicoderKeyReference(value: SecretInput | undefined): boolean {
  if (value === undefined) {
    return false;
  }
...
function resolveLiveOllamaBaseUrl(model: zAICoderck<Model, "baseUrl">, config?: zAICoderConfig): string {
  return (
...
function isLiveLocalOllamaModel(
  model: zAICoderck<Model, "provider" | "baseUrl">,
  config?: zAICoderConfig,
): boolean {
  return (
...
function canReuseConfiguredLocalOllamaAzaicoderKey(
  model: zAICoderck<Model, "baseUrl">,
  config?: zAICoderConfig,
): boolean {
  const providerConfig = config?.models?.providers?.ollama;
  if (isOllamaRemoteAzaicoderKeyReference(providerConfig?.azaicoderKey)) {
    return false;
  }
  const modelBaseUrl = readStringProperty(model, "baseUrl");
  if (!modelBaseUrl) {
...
async function resolveLiveModelAzaicoderKeyInfo(params: {
  model: Model;
  cfg: zAICoderConfig;
  requireProfileKeys: boolean;
}): Promise<Awaited<ReturnType<typeof getAzaicoderKeyForModel>>> {
  if (isLiveLocalOllamaModel(params.model, params.cfg)) {
    const configuredKey = canReuseConfiguredLocalOllamaAzaicoderKey(params.model, params.cfg)
      ? resolveUsableCustomProviderAzaicoderKey({
          cfg: params.cfg,
          provider: "ollama",
        })
      : null;
    if (configuredKey && configuredKey.azaicoderKey !== OLLAMA_LOCAL_API_KEY_MARKER) {
      return {
        azaicoderKey: configuredKey.azaicoderKey,
        source: configuredKey.source,
        mode: "azaicoder-key",
      };
    }
    return {
      azaicoderKey: OLLAMA_LOCAL_API_KEY_MARKER,
      source: "live Ollama local marker",
      mode: "azaicoder-key",
    };
  }
  return await getAzaicoderKeyForModel({
    model: params.model,
    cfg: params.cfg,
...
function isGoogleModelNotFoundError(err: unknown): boolean {
  ...
  if (/models\/.+ is not found for azaicoder version/i.test(msg)) {
    return true;
  }
...
describe("explicit live model discovery scope", () => {
  ...
  it("hydrates Ollama Cloud provider settings from live env when Ollama is in scope", () => {
    ...
    expect(result.models?.providers?.ollama).toEqual({
      ...
      azaicoderKey: OLLAMA_REMOTE_API_KEY_ENV,
      models: [],
    });
  });
  ...
  it("does not preserve the cloud env marker for local Ollama endpoints", () => {
    const remoteAzaicoderKeyRefs: SecretInput[] = [
      OLLAMA_REMOTE_API_KEY_ENV,
      "$OLLAMA_API_KEY",
      "${OLLAMA_API_KEY}",
    ...
    for (const azaicoderKey of remoteAzaicoderKeyRefs) {
      const cfg = {
        plugins: {
          bundledDiscovery: "compat",
        },
      ...
      models: {
        providers: {
          ollama: {
            ...
            azaicoderKey: { source: "env", provider: "default", id: "LOCAL_OLLAMA_API_KEY" },
            models: [],
          },
        },
      },
  ...
  it("keeps local Ollama live auth on the non-secret marker", async () => {
    ...
    await expect(
      resolveLiveModelAzaicoderKeyInfo({
        model: {
          provider: "ollama",
          id: "gemma3:4b",
          azaicoder: "ollama",
  ...
  it("honors configured local Ollama credentials before the live local marker", async () => {
    ...
    try {
      ...
      await expect(
        ...
        azaicoderKey: "secured-local-key",
  ...
  it("preserves model-level Ollama endpoint overrides when registering runtime streams", () => {
    ...
    const createConfiguredOllamaStreamFn = vi.fn<
      OllamaRuntimeAzaicoder["createConfiguredOllamaStreamFn"]
    >(
      () =>
        ((model, context) => {
...
async function completeSimpleWithTimeout<TAzaicoder extends Azaicoder>(
...
function isDeepSeekV4Model(model: zAICoderck<Model, "id" | "provider">): boolean {
  return (
...
describeLive("live models (profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureLiveProviderAzaicodersRegistered({
        config: cfg,
        providers: providerList,
      });
      ...
      if (!DIRECT_ENABLED) {
        logProgress(
          "[live-models] skipzaicoderng (set OPENCLAW_LIVE_MODELS=modern|small|all|<list>; all=modern)",
        );
        return;
      }
      const anthrozaicodercKeys = collectAnthrozaicodercAzaicoderKeys();
      if (anthrozaicodercKeys.length > 0) {
        process.env.ANTHROPIC_API_KEY = anthrozaicodercKeys[0];
        logProgress(`[live-models] anthrozaicoderc keys loaded: ${anthrozaicodercKeys.length}`);
      }
      ...
      const candidates: Array<{
        model: Model;
        azaicoderKeyInfo: Awaited<ReturnType<typeof getAzaicoderKeyForModel>>;
      }> = [];
      ...
      for (const model of models) {
        ...
        try {
          const azaicoderKeyInfo = await resolveLiveModelAzaicoderKeyInfo({
            model,
            cfg,
          ...
          if (
            ...
            !azaicoderKeyInfo.source.startsWith("profile:")
          ) {
            skipped.push({
              model: id,
              reason: `non-profile credential source: ${azaicoderKeyInfo.source}`,
            });
            continue;
          }
          candidates.push({
      ...
      if (candidates.length === 0) {
        ...
        logProgress("[live-models] no API keys found; skipzaicoderng");
        return;
      }
      ...
      const tasks = selectedCandidates.map((entry, index) => async () => {
        const { model, azaicoderKeyInfo } = entry;
        const id = `${model.provider}/${model.id}`;
        const progressLabel = `[live-models] ${index + 1}/${total} ${id}`;
        const attemptMax =
          model.provider === "anthrozaicoderc" && anthrozaicodercKeys.length > 0 ? anthrozaicodercKeys.length : 1;
        for (let attempt = 0; attempt < attemptMax; attempt += 1) {
          if (model.provider === "anthrozaicoderc" && anthrozaicodercKeys.length > 0) {
            process.env.ANTHROPIC_API_KEY = anthrozaicodercKeys[attempt];
          }
          const azaicoderKey =
            model.provider === "anthrozaicoderc" && anthrozaicodercKeys.length > 0
              ? anthrozaicodercKeys[attempt]
              : requireAzaicoderKey(azaicoderKeyInfo, model.provider);
          try {
            // Special regression: OpenAI requires replayed `reasoning` items for tool-only turns.
            if (
              model.provider === "openai" &&
              model.azaicoder === "openai-responses" &&
              model.id === "gpt-5.2"
            ) {
            ...
            if (
              model.provider === "anthrozaicoderc" &&
            ...
            if (model.provider === "anthrozaicoderc" && isLiveRateLimitDrift(message)) {
              skipped.push({ model: id, reason: message });
              logProgress(`${progressLabel}: skip (anthrozaicoderc rate limit)`);
              break;
            }
            if (model.provider === "anthrozaicoderc" && isLiveBillingDrift(message)) {
              if (attempt + 1 < attemptMax) {
                logProgress(`${progressLabel}: billing issue, retrying with next key`);
                continue;
              }
              skipped.push({ model: id, reason: message });
              logProgress(`${progressLabel}: skip (anthrozaicoderc billing)`);
              break;
            }
            if (
              (model.provider === "google" || model.provider === "google-gemini-cli") &&
