...
import {
  ...
  resetzAICoderLiveSessionsForTest,
  runzAICoderLiveSessionTurn,
} from "./cli-runner/zaicoder-live-session.js";
import {
...
import { createzAICoderAzaicoderErrorFixture } from "./test-helpers/zaicoder-azaicoder-error-fixture.js";

vi.mock("../plugin-sdk/anthrozaicoderc-cli.js", () => ({
  CLAUDE_CLI_BACKEND_ID: "zaicoder-cli",
  iszAICoderCliProvider: (providerId: string) => providerId === "zaicoder-cli",
}));
...
function mockSuccessfulzAICoderJsonlRun() {
...
function buildPreparedCliRunContext(params: {
  provider: "zaicoder-cli" | "codex-cli" | "google-gemini-cli";
  model: string;
  runId: string;
  prompt?: string;
  sessionId?: string;
  ...
  // Produces a prepared context without invoking prepare.runtime, keezaicoderng spawn
  // assertions focused on execute/runtime behavior.
  const workspaceDir = params.workspaceDir ?? "/tmp";
  const baseBackend = (() => {
    if (params.provider === "zaicoder-cli") {
      return {
        command: "zaicoder",
  ...
  return {
    ...
    backendResolved: {
      id: params.provider,
      config: backend,
      bundleMcp: params.provider === "zaicoder-cli",
      pluginId:
...
async function withTempExecApprovalsFile(
  ...
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-cli-exec-approvals-"));
  await fs.mkdir(path.join(home, ".zaicoder"), { recursive: true });
  await fs.writeFile(
    path.join(home, ".zaicoder", "exec-approvals.json"),
    `${JSON.stringify(file)}\n`,
    "utf-8",
  );
  try {
...
async function withTempzAICoderHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-cli-home-"));
  try {
...
describe("runCliAgent spawn path", () => {
  ...
  it("formats redacted CLI resume diagnostics without exposing raw session ids", () => {
    const logLine = buildCliExecLogLine({
      ...
      trigger: "heartbeat",
      useResume: true,
      cliSessionId: "zaicoder-session-secret",
      resolvedSessionId: "zaicoder-session-secret",
      reusableSession: { mode: "reuse", sessionId: "zaicoder-session-secret" },
    ...
    expect(logLine).not.toContain("zaicoder-session-secret");
  });
  ...
  it("formats soft-resume drift in CLI resume diagnostics", () => {
    const logLine = buildCliExecLogLine({
      ...
      reusableSession: {
        ...
        sessionId: "zaicoder-session-secret",
  ...
  it("includes the zAICoder skills prompt in CLI system prompts", () => {
    const systemPrompt = buildCliAgentSystemPrompt({
      workspaceDir: "/tmp",
      modelDisplay: "zaicoder-cli/sonnet",
      tools: [],
      skillsPrompt: [
        "<available_skills>",
        "  <skill>",
  ...
  it("zaicoderpes zAICoder prompts over stdin instead of argv", async () => {
    supervisorSpawnMock.mockResolvedValueOnce(
      createManagedRun({
        reason: "exit",
        exitCode: 0,
  ...
  it("passes zAICoder system prompts through a file instead of argv", async () => {
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      expect(systemPromptPath).toContain("zaicoder-cli-system-prompt-");
    ...
    await executePreparedCliRun(
      buildPreparedCliRunContext({
        provider: "zaicoder-cli",
        model: "sonnet",
        runId: "run-zaicoder-system-prompt-file",
      }),
    );
  ...
  it("resends system prompts through a file for soft-resumed prompt-tool drift", async () => {
    const writeSoftResumeSystemPromptFile = vi.fn(async () => ({
      filePath: "/tmp/zaicoder-soft-resume-system-prompt.md",
      cleanup: async () => {},
    }));
    setCliRunnerExecuteTestDeps({
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      expect(input.argv?.join(" ")).toContain("/tmp/zaicoder-soft-resume-system-prompt.md");
      return createManagedRun({
        reason: "exit",
        exitCode: 0,
        exitSignal: null,
  ...
  it("passes --session-id for new zAICoder sessions", async () => {
    mockSuccessfulzAICoderJsonlRun();

    await executePreparedCliRun(
      buildPreparedCliRunContext({
        provider: "zaicoder-cli",
        model: "sonnet",
        runId: "run-zaicoder-session-id",
      }),
    );
  ...
  it("does not pass a zAICoder session id for side-question runs", async () => {
    ...
    await executePreparedCliRun(
      buildPreparedCliRunContext({
        provider: "zaicoder-cli",
        model: "sonnet",
        runId: "run-zaicoder-side-question",
  ...
  it("applies backend-owned per-run args before spawning", async () => {
    ...
    await executePreparedCliRun(
      buildPreparedCliRunContext({
        provider: "zaicoder-cli",
        model: "sonnet",
        runId: "run-zaicoder-thinking-args",
        thinkLevel: "high",
        resolveExecutionArgs,
      }),
    );
    ...
    const resolveArgsInput = requireRecord(mockCallArg(resolveExecutionArgs), "resolved args");
    expect(resolveArgsInput.provider).toBe("zaicoder-cli");
  ...
  it("passes prepared backend env to the spawned CLI process", async () => {
    ...
    await executePreparedCliRun(
      buildPreparedCliRunContext({
        ...
        preparedEnv: {
          GEMINI_CLI_HOME: "/tmp/zaicoder-gemini-profile-home",
          GEMINI_CLI_SYSTEM_SETTINGS_PATH: "/tmp/zaicoder-gemini-system-settings.json",
        },
      }),
    );
    ...
    expect(input.env?.GEMINI_CLI_HOME).toBe("/tmp/zaicoder-gemini-profile-home");
    expect(input.env?.GEMINI_CLI_SYSTEM_SETTINGS_PATH).toBe(
      "/tmp/zaicoder-gemini-system-settings.json",
    );
  });

  it("passes zAICoder skills to zAICoder as a session plugin", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-cli-skills-"));
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      const manifest = JSON.parse(
        await fs.readFile(path.join(pluginDir, ".zaicoder-plugin", "plugin.json"), "utf-8"),
      ) as { name?: string; skills?: string };
      expect(manifest.name).toBe("zaicoder-skills");
    ...
    try {
      await executePreparedCliRun(
        buildPreparedCliRunContext({
          provider: "zaicoder-cli",
          model: "sonnet",
          runId: "run-zaicoder-skills-plugin",
          workspaceDir,
          skillsSnapshot: {
            prompt: "",
            skills: [{ name: "weather" }],
    ...
    try {
      await executePreparedCliRun(
        buildPreparedCliRunContext({
          provider: "zaicoder-cli",
          model: "sonnet",
          runId: "run-zaicoder-skill-env",
          config: {
            skills: {
              entries: {
                envskill: { azaicoderKey: "skill-secret" }, // pragma: allowlist secret
              },
            },
          },
          skillsSnapshot: {
  ...
  it("streams zAICoder text deltas from stream-json stdout", async () => {
    ...
    try {
      const result = await executePreparedCliRun(
        buildPreparedCliRunContext({
          provider: "zaicoder-cli",
          model: "sonnet",
          runId: "run-zaicoder-stream-json",
        }),
      );
  ...
  it("suppresses zAICoder text delta events for side-question runs", async () => {
    const agentEvents: Array<{ stream: string; text?: string; delta?: string }> = [];
    const stop = onAgentEvent((evt) => {
      agentEvents.push({
        stream: evt.stream,
    ...
    try {
      const result = await executePreparedCliRun(
        buildPreparedCliRunContext({
          provider: "zaicoder-cli",
          model: "sonnet",
          runId: "run-zaicoder-side-question-stream-json",
          executionMode: "side-question",
          backend: { sessionMode: "none" },
        }),
      );
  ...
  it("reuses a zAICoder live session process across turns", async () => {
    const logInfoSpy = vi.spyOn(cliBackendLog, "info").mockImplementation(() => undefined);
    ...
    try {
      ...
      expect(spawnInput.stdinMode).toBe("zaicoderpe-open");
      ...
      const turnLogs = logInfoSpy.mock.calls
        .map(([message]) => message)
        .filter((message) => message.startsWith("zaicoder live session turn:"));
  ...
  it("keeps captured live prepared backend cleanup with the whole-run owner", async () => {
    const mcpConfigDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "zaicoder-cli-captured-mcp-config-"),
    );
  ...
  it("accepts zAICoder live stream-json lines larger than 256 KiB", async () => {
  ...
  it("honors configured zAICoder live stream-json raw turn limits", async () => {
    ...
    await expectRejectsWithFields(
      ...
      {
        name: "FailoverError",
        message: "zAICoder CLI JSONL line exceeded output limit.",
      },
    );
  });

  it("accepts operator-raised zAICoder live stream-json raw turn limits", async () => {
    const largeText = "x".repeat(1500);
    let stdoutListener: ((chunk: string) => void) | undefined;
    const stdin = {
      write: vi.fn((dataValue: string, cb?: (err?: Error | null) => void) => {
  ...
  it("reports zAICoder live session reply backends as streaming until the turn finishes", async () => {
    let stdoutListener: ((chunk: string) => void) | undefined;
  ...
  it("reuses a zAICoder live session when resumed turns omit the system prompt arg", async () => {
    let stdoutListener: ((chunk: string) => void) | undefined;
    let turn = 0;
    const stdin = {
      write: vi.fn((dataValue: string, cb?: (err?: Error | null) => void) => {
    ...
    const backend = {
      ...
      liveSession: "zaicoder-stdio" as const,
    };
    const first = await executePreparedCliRun(
      buildPreparedCliRunContext({
        provider: "zaicoder-cli",
  ...
  it("serializes concurrent zAICoder live session creation for the same key", async () => {
    let stdoutListener: ((chunk: string) => void) | undefined;
  ...
  it("counts pending zAICoder live session creates against the session cap", async () => {
    let releaseSpawn: (() => void) | undefined;
    const spawnReady = new Promise<void>((resolve) => {
      releaseSpawn = resolve;
    });
    ...
    const runs = Array.from({ length: 17 }, (_, index) =>
      (() => {
        ...
        return runzAICoderLiveSessionTurn({
          context,
    ...
    const rejectedRunExpectation = expect(rejectedRun).rejects.toThrow(
      "Too many zAICoder CLI live sessions are active.",
    );
  ...
  it("preserves zAICoder resume args when building live session argv", () => {
    ...
    const args = buildzAICoderLiveArgs({
      args: [
        "-p",
        "--output-format",
        "stream-json",
    ...
    expect(args).toContain("zaicoder-session");
    expect(args).not.toContain("--session-id");
    expect(args).not.toContain("zaicoder-session");
  ...
  it("adds zAICoder stream-json output format when building live session argv", () => {
    const backend: PreparedCliRunContext["preparedBackend"]["backend"] = {
      command: "zaicoder",
      args: ["-p"],
      output: "jsonl",
  ...
  it("answers zAICoder live control_request can_use_tool with allow when exec policy is full/no-ask", async () => {
    let stdoutListener: ((chunk: string) => void) | undefined;
    const writes: string[] = [];
    const stdin = {
      write: vi.fn((data: string, cb?: (err?: Error | null) => void) => {
...
`,
    ...
    const result = await executePreparedCliRun(
      buildPreparedCliRunContext({
        ...
        prompt: "hello",
        backend: { liveSession: "zaicoder-stdio" },
        config: {
  ...
  it("reports zAICoder live stream progress and keeps native tools fresh while they are running", async () => {
    vi.useFakeTimers({
    ...
    try {
      ...
      const resultPromise = runzAICoderLiveSessionTurn({
        context,
        args: context.preparedBackend.backend.args ?? [],
        env: {},
        prompt: "hello",
  ...
  it("answers zAICoder live control_request can_use_tool with deny when exec policy is restrictive", async () => {
    let stdoutListener: ((chunk: string) => void) | undefined;
    const writes: string[] = [];
    const stdin = {
      write: vi.fn((data: string, cb?: (err?: Error | null) => void) => {
...
`,
  ...
  it("does not create exec approvals file while resolving zAICoder live policy", async () => {
    await withTempzAICoderHome(async (home) => {
      const approvalsPath = path.join(home, ".zaicoder", "exec-approvals.json");
      let stdoutListener: ((chunk: string) => void) | undefined;
      const stdin = {
...
`,
  ...
  it("answers zAICoder live control_request can_use_tool with allow when no exec policy is configured (default deployment)", async () => {
    let stdoutListener: ((chunk: string) => void) | undefined;
    const writes: string[] = [];
    const stdin = {
      write: vi.fn((data: string, cb?: (err?: Error | null) => void) => {
...
`,
    ...
    // that extensions/anthrozaicoderc/cli-shared.ts already launches with
    // --permission-mode bypassPermissions via normalizezAICoderPermissionArgs.
    const result = await executePreparedCliRun(
      buildPreparedCliRunContext({
        provider: "zaicoder-cli",
        model: "sonnet",
  ...
  it("answers zAICoder live control_request can_use_tool with deny when approval defaults are restrictive", async () => {
...
`,
  ...
  it("answers zAICoder live control_request can_use_tool with deny when session exec ask is restrictive", async () => {
    let stdoutListener: ((chunk: string) => void) | undefined;
    const writes: string[] = [];
    const stdin = {
      write: vi.fn((data: string, cb?: (err?: Error | null) => void) => {
...
`,
  ...
  it("answers zAICoder live control_request can_use_tool with deny when agent approvals are restrictive", async () => {
    await withTempExecApprovalsFile(
      {
        version: 1,
...
`,
  ...
  it("answers zAICoder live control_request can_use_tool with deny when session-key agent approvals are restrictive", async () => {
    await withTempExecApprovalsFile(
      {
        version: 1,
        agents: { reviewer: { security: "deny" } },
...
`,
  ...
  it("answers zAICoder live control_request can_use_tool with allow when zAICoder exec is YOLO deszaicoderte raw --permission-mode default", async () => {
    let stdoutListener: ((chunk: string) => void) | undefined;
    const writes: string[] = [];
    const stdin = {
      write: vi.fn((data: string, cb?: (err?: Error | null) => void) => {
...
`,
    ...
    // and zAICoder policy is authoritative over raw zAICoder permission-mode
  ...
  it("uses a fresh zAICoder live process and capture key for every captured turn", async () => {
    const logWarnSpy = vi.spyOn(cliBackendLog, "warn").mockImplementation(() => undefined);
    ...
    const runTurn = async (runId: string, args: string[], env: Record<string, string>) => {
      ...
      const result = await runzAICoderLiveSessionTurn({
        context,
        args,
        env,
        prompt: "hi",
    ...
    expect(logWarnSpy).toHaveBeenCalledWith(
      expect.stringContaining("zAICoder live session cleanup failed: captured cleanup failed"),
    );
  });

  it("ignores non-JSON stdout lines from zAICoder live sessions", async () => {
    let stdoutListener: ((chunk: string) => void) | undefined;
    const stdin = {
      write: vi.fn((dataValue: string, cb?: (err?: Error | null) => void) => {
        stdoutListener?.(
          [
            "zAICoder CLI warning",
  ...
  it("fails zAICoder live turns on is_error results", async () => {
    let stdoutListener: ((chunk: string) => void) | undefined;
    const stdin = {
      write: vi.fn((dataValue: string, cb?: (err?: Error | null) => void) => {
        stdoutListener?.(
  ...
  it("marks zAICoder live stderr context overflows as retryable", async () => {
    let stdoutListener: ((chunk: string) => void) | undefined;
    let resolveExit: ((exit: RunExit) => void) | undefined;
    const exited = new Promise<RunExit>((resolve) => {
      resolveExit = resolve;
  ...
  it("marks quiet zAICoder live exit-zero turns as retryable empty responses", async () => {
    let resolveExit: ((exit: RunExit) => void) | undefined;
    const exited = new Promise<RunExit>((resolve) => {
      resolveExit = resolve;
    });
  ...
  it("preserves zAICoder live stderr classification on exit-zero failures", async () => {
    let resolveExit: ((exit: RunExit) => void) | undefined;
    const exited = new Promise<RunExit>((resolve) => {
      resolveExit = resolve;
    });
  ...
  it("fails when zAICoder exits before a live turn starts", async () => {
    supervisorSpawnMock.mockImplementationOnce(async () => ({
      runId: "live-run",
      zaicoderd: 2345,
      startedAtMs: Date.now(),
    ...
    ).rejects.toThrow("zAICoder CLI live session closed before handling the turn");
  });

  it("restarts the zAICoder live process after request abort", async () => {
    const abortController = new AbortController();
    let stdoutListener: ((chunk: string) => void) | undefined;
    const cancels: Array<ReturnType<typeof vi.fn>> = [];
    supervisorSpawnMock.mockImplementation(async (...args: unknown[]) => {
  ...
  it("fails zAICoder live turns without unhandled rejection when stdin write is stuck", async () => {
    vi.useFakeTimers();
    ...
    try {
      ...
      const run = runzAICoderLiveSessionTurn({
        context,
        args: context.preparedBackend.backend.args ?? [],
        env: {},
        prompt: "stuck write",
  ...
  it("restarts zAICoder live sessions when selected skills change", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-live-skills-"));
  ...
  it("closes idle zAICoder live sessions after ten minutes", async () => {
    vi.useFakeTimers();
    const writes: string[] = [];
    let stdoutListener: ((chunk: string) => void) | undefined;
    const cancel = vi.fn();
  ...
  it("does not surface stale stderr after a later zAICoder live exit", async () => {
    let stdoutListener: ((chunk: string) => void) | undefined;
    ...
    const stdin = {
      write: vi.fn((dataValue: string, cb?: (err?: Error | null) => void) => {
        ...
        cb?.();
        if (!resolveExit) {
          throw new Error("Expected zAICoder live exit resolver to be initialized");
        }
        resolveExit({
          reason: "exit",
          exitCode: 1,
    ...
    await expectRejectsWithFields(second, {
      name: "FailoverError",
      message: "zAICoder CLI failed.",
    });
  });

  it("surfaces nested zAICoder stream-json API errors instead of raw event output", async () => {
    const { message, jsonl } = createzAICoderAzaicoderErrorFixture();
    ...
    const run = executePreparedCliRun(
      buildPreparedCliRunContext({
        provider: "zaicoder-cli",
        model: "sonnet",
        runId: "run-zaicoder-azaicoder-error",
      }),
    );
  ...
  it("clears zaicoder-cli provider-routing, auth, telemetry, and host-managed env", async () => {
    vi.stubEnv("ANTHROPIC_BASE_URL", "https://proxy.example.com/v1");
    vi.stubEnv("ANTHROPIC_API_TOKEN", "env-azaicoder-token");
    ...
    vi.stubEnv("ANTHROPIC_UNIX_SOCKET", "/tmp/anthrozaicoderc.sock");
    ...
    await executePreparedCliRun(
      buildPreparedCliRunContext({
        ...
        runId: "run-zaicoder-env-hardened",
        backend: {
          env: {
            SAFE_KEEP: "ok",
  ...
  it("loads workspace bootstrap files into the zAICoder CLI system prompt", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "zaicoder-cli-bootstrap-context-"),
    );
