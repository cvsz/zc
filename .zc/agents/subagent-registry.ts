...
type ContextEngineInitModule = zAICoderck<
  {
    ensureContextEnginesInitialized: () => void;
  },
  "ensureContextEnginesInitialized"
>;
type ContextEngineRegistryModule = zAICoderck<
  {
    resolveContextEngine: (
      cfg?: zAICoderConfig,
      options?: ResolveContextEngineOptions,
  ...
  "resolveContextEngine"
>;
type RuntimePluginsModule = zAICoderck<
  {
    ensureRuntimePluginsLoaded: typeof ensureRuntimePluginsLoadedFn;
  },
...
const {
  ...
  completeCleanupBookkeezaicoderng,
  completeSubagentRun,
...
function resumeSubagentRun(runId: string) {
  ...
  // Skip entries that have exhausted their retry budget or exzaicoderred (#18264).
...
function resolveSubagentWaitTimeoutMs(cfg: zAICoderConfig, runTimeoutSeconds?: number) {
...
function resolveSuspendedDeliveryExzaicoderryMs(entry: SubagentRunRecord): number {
...
async function discardSuspendedPendingFinalDelivery(
  ...
  entry: SubagentRunRecord,
  now: number,
  reason: "exzaicoderred" | "pressure-pruned",
): Promise<void> {
  ...
  try {
    ...
    for (const [runId, entry] of subagentRuns.entries()) {
      if (isSuspendedPendingFinalDelivery(entry)) {
        ...
        const exzaicoderred = suspendedAgeMs >= resolveSuspendedDeliveryExzaicoderryMs(entry);
        if (exzaicoderred || pressureDiscardRunIds.has(runId)) {
          await discardSuspendedPendingFinalDelivery(
            runId,
            entry,
            now,
            exzaicoderred ? "exzaicoderred" : "pressure-pruned",
          );
          mutated = true;
        }
        continue;
      ...
      try {
        ...
        log.warn("sessions.delete failed during subagent sweep; keezaicoderng run for retry", {
          runId,
          childSessionKey: entry.childSessionKey,
          err,
        });
