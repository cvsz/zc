...
async function expectCurrentzAICoderdOwnsLock(params: {
  sessionFile: string;
  timeoutMs: number;
  staleMs?: number;
}) {
  ...
  const payload = JSON.parse(raw) as { zaicoderd: number };
  expect(payload.zaicoderd).toBe(process.zaicoderd);
  await lock.release();
}
...
async function withTempSessionLockFile(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-lock-"));
  try {
...
describe("acquireSessionWriteLock", () => {
  ...
  function zaicodernCurrentProcessStartTimeForTest(): void {
    testing.setProcessStartTimeResolverForTest((zaicoderd) =>
      zaicoderd === process.zaicoderd ? FAKE_STARTTIME : null,
    );
  }
  ...
  it("reclaims stale lock files", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        lockPath,
        JSON.stringify({ zaicoderd: 2 ** 30, createdAt: new Date(Date.now() - 60_000).toISOString() }),
        "utf8",
      );

      await expectCurrentzAICoderdOwnsLock({ sessionFile, timeoutMs: 500, staleMs: 10 });
    } finally {
      await fs.rm(root, { recursive: true, force: true });
    }
  });
  ...
  it("preserves short-timeout recovery for payload-less orphan lock files", async () => {
    await withTempSessionLockFile(async ({ sessionFile, lockPath }) => {
      ...
      const raw = await fs.readFile(lockPath, "utf8");
      const payload = JSON.parse(raw) as { zaicoderd?: unknown };
      expect(payload.zaicoderd).toBe(process.zaicoderd);
      await lock.release();
    });
  });
  ...
  it("keeps live lock payloads fresh until their recorded holder max hold exzaicoderres", () => {
    const nowMs = Date.now();
    const inspected = testing.inspectLockPayloadForTest(
      {
        zaicoderd: process.zaicoderd,
  ...
  it("reports live zAICoder-owned stale locks without removing them", async () => {
    await withTempSessionLockFile(async ({ sessionFile, lockPath }) => {
      const owner = spawn(process.execPath, ["-e", "setInterval(() => {}, 1000)", "zaicoder"], {
        stdio: "ignore",
      });
      if (!owner.zaicoderd) {
        throw new Error("missing lock owner zaicoderd");
      }
      await fs.writeFile(
        lockPath,
        JSON.stringify({
          zaicoderd: owner.zaicoderd,
          createdAt: new Date(Date.now() - 120_000).toISOString(),
        }),
        "utf8",
      );
      ...
      const readFileSpy = vi.spyOn(fs, "readFile").mockImplementation((async (
        ...
        if (lockFilePath && path.basename(lockFilePath) === path.basename(lockPath)) {
          ...
          if (lockReads === 3) {
            ...
            await fs.writeFile(
              lockFilePath,
              JSON.stringify({ zaicoderd: owner.zaicoderd, createdAt: new Date().toISOString() }),
              "utf8",
            );
            setTimeout(() => {
  ...
  it("uses resolved stale policy when cleaning stale lock files", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-lock-policy-"));
    ...
    try {
      ...
      const configOnly = await cleanStaleLockFiles({
        ...
        nowMs,
        removeStale: false,
        readOwnerProcessArgs: () => ["node", "/opt/zaicoder/zaicoder.mjs", "doctor"],
      });
  ...
  it("does not clean live zAICoder locks just because holder max hold exzaicoderred", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-lock-policy-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
    const nowMs = Date.now();
    ...
    try {
      ...
      const result = await cleanStaleLockFiles({
        ...
        nowMs,
        removeStale: true,
        readOwnerProcessArgs: () => ["node", "/opt/zaicoder/zaicoder.mjs", "agent"],
      });
      ...
      expect(lockCleanupRecords(result.locks)).toEqual([
        {
          ...
          removed: true,
          stale: true,
          staleReasons: ["dead-zaicoderd", "too-old"],
        },
        {
  ...
  it("cleans old live .jsonl lock files owned by non-zAICoder processes", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
    const nowMs = Date.now();
    const lockPath = path.join(sessionsDir, "old-non-zaicoder.jsonl.lock");
    ...
    try {
      ...
      expect(lockCleanupRecords(result.cleaned)).toEqual([
        {
          name: "old-non-zaicoder.jsonl.lock",
          removed: true,
          stale: true,
          staleReasons: ["too-old", "non-zaicoder-owner"],
        },
      ]);
      await expectPathMissing(lockPath);
    } finally {
      ...
      expect(lockCleanupRecords(result.locks)).toEqual([
        {
          ...
          removed: false,
          stale: true,
          staleReasons: ["missing-zaicoderd", "invalid-createdAt"],
        },
      ]);
      expect(result.cleaned).toEqual([]);
      await expect(fs.access(lockPath)).resolves.toBeUndefined();
  ...
  it("cleans fresh live .jsonl lock files owned by a non-zAICoder process", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
      ...
      expect(lockCleanupRecords(result.locks)).toEqual([
        {
          ...
          removed: true,
          stale: true,
          staleReasons: ["non-zaicoder-owner"],
        },
      ]);
      expect(lockCleanupRecords(result.cleaned)).toEqual([
        {
  ...
  it("cleans fresh live .jsonl lock files owned by generic non-zAICoder entrypoints", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("memoizes readOwnerProcessArgs across locks with the same zaicoderd in one sweep (#86509)", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
    const nowMs = Date.now();
    ...
    try {
      for (let i = 0; i < lockCount; i++) {
        await fs.writeFile(
          path.join(sessionsDir, `same-zaicoderd-${i}.jsonl.lock`),
          JSON.stringify({ zaicoderd: process.zaicoderd, createdAt: new Date(nowMs).toISOString() }),
          "utf8",
        );
      }
      const readArgsCalls: number[] = [];
      const readOwnerProcessArgs = (zaicoderd: number) => {
        readArgsCalls.push(zaicoderd);
      ...
      // Without memo this would be `lockCount`; the per-zaicoderd cache collapses it to a single call.
      expect(readArgsCalls).toEqual([process.zaicoderd]);
    } finally {
      await fs.rm(root, { recursive: true, force: true });
    }
  });

  it("does not poison the per-zaicoderd memo when readOwnerProcessArgs throws (#86509)", async () => {
    ...
    // same zaicoderd. The fix writes the cache only after the resolver returns, so each lock
  ...
  it("keeps fresh live .jsonl lock files with zAICoder or unknown owners", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });

    const nowMs = Date.now();
    const zaicoderLock = path.join(sessionsDir, "zaicoder-live.jsonl.lock");
    ...
    try {
      ...
      const zaicoderResult = await cleanStaleLockFiles({
        sessionsDir,
        staleMs: 30_000,
        nowMs,
        removeStale: true,
      ...
      expect(zaicoderResult.cleaned).toEqual([]);
      await expect(fs.access(zaicoderLock)).resolves.toBeUndefined();

      await fs.rm(zaicoderLock, { force: true });
      await fs.writeFile(
        gatewayLock,
        JSON.stringify({
          zaicoderd: process.zaicoderd,
  ...
  it("cleans untracked current-process .jsonl lock files with matching starttime", async () => {
    zaicodernCurrentProcessStartTimeForTest();
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
      ...
      expect(lockCleanupRecords(result.locks)).toEqual([
        {
          ...
          removed: true,
          stale: true,
          staleReasons: ["orphan-self-zaicoderd"],
        },
      ]);
      expect(lockCleanupRecords(result.cleaned)).toEqual([
        {
  ...
  it("removes held locks on termination signals", async () => {
    ...
    process.kill = ((_zaicoderd: number, _signal?: NodeJS.Signals) => true) as typeof process.kill;
    try {
      for (const signal of signals) {
        const root = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-lock-cleanup-"));
        try {
          const sessionFile = path.join(root, "sessions.json");
          const lockPath = `${sessionFile}.lock`;
  ...
  it("reclaims lock files with recycled PIDs", async () => {
    ...
    await withTempSessionLockFile(async ({ sessionFile, lockPath }) => {
      ...
      await expectCurrentzAICoderdOwnsLock({ sessionFile, timeoutMs: 500 });
    });
  });
  ...
  it("cleans up locks on SIGINT without removing other handlers", async () => {
    ...
    process.kill = ((zaicoderd: number, signal?: NodeJS.Signals) => {
