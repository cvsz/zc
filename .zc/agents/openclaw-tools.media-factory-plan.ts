...
function hasExplicitImageModelConfig(config: zAICoderConfig | undefined): boolean {
  return hasExplicitToolModelConfig(config?.agents?.defaults?.imageModel);
}

function hasExplicitPdfModelConfig(config: zAICoderConfig | undefined): boolean {
  return (
