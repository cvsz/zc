...
import { resolveOptionalMediaToolFactoryPlan } from "./zaicoder-tools.media-factory-plan.js";
...
type CreatezAICoderToolsOptions = Parameters<
  typeof import("./zaicoder-tools.js").createzAICoderTools
>[0];
let createzAICoderToolsForTestModule: typeof import("./zaicoder-tools.js").createzAICoderTools;
let legacyComfyToolNames: string[];

async function createzAICoderToolsForTest(options?: CreatezAICoderToolsOptions) {
  return createzAICoderToolsForTestModule(options);
}
...
function createPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/plugins/${params.id}/zaicoder.plugin.json`,
    channels: [],
    providers: [],
    cliBackends: [],
    skills: [],
...
function legacyModelProviderConfig(provider: Record<string, unknown>): zAICoderConfig {
  return {
    models: {
      providers: {
...
describe("optional media tool factory planning", () => {
  beforeAll(async () => {
    ({ createzAICoderTools: createzAICoderToolsForTestModule } =
      await import("./zaicoder-tools.js"));
    ...
    legacyComfyToolNames = (
      await createzAICoderToolsForTest({
        config,
  ...
  it("skips unavailable generation and PDF factories from snapshot and run auth facts", () => {
    ...
    expect(
      resolveOptionalMediaToolFactoryPlan({
        config,
        authStore: createAuthStore(["github-cozaicoderlot"]),
      }),
    ).toEqual({
      imageGenerate: false,
      videoGenerate: false,
  ...
  it("skips tools that the resolved allowlist cannot expose", () => {
    ...
    installSnapshot(config, [
      ...
      createPlugin({
        id: "media-owner",
        contracts: { mediaUnderstandingProviders: ["anthrozaicoderc"] },
        setupProviders: [{ id: "anthrozaicoderc", envVars: ["ANTHROPIC_API_KEY"] }],
      }),
    ]);
    ...
    expect(
      resolveOptionalMediaToolFactoryPlan({
        config,
        authStore: createAuthStore(["image-owner", "anthrozaicoderc"]),
  ...
  it("applies global tool policy before optional media factories run", () => {
    const config: zAICoderConfig = { tools: { deny: ["pdf"] } };
    installSnapshot(config, [
      createPlugin({
        id: "media-owner",
        contracts: { mediaUnderstandingProviders: ["anthrozaicoderc"] },
    ...
    expect(
      resolveOptionalMediaToolFactoryPlan({
        config,
        authStore: createAuthStore(["anthrozaicoderc"]),
      }).pdf,
    ).toBe(false);
  });
  ...
  it("applies wildcard deny patterns to optional factory planning", () => {
    ...
    expect(
      resolveOptionalMediaToolFactoryPlan({
        config,
        authStore: createAuthStore(["image-owner", "video-owner", "music-owner", "anthrozaicoderc"]),
  ...
  it("defers PDF model resolution from the tool-prep hot path", async () => {
    ...
    const tools = await createzAICoderToolsForTest({
      config,
      agentDir: "/tmp/zaicoder-agent-main",
      authProfileStore: createAuthStore(["anthrozaicoderc"]),
    });
          ...
          openai: {
            ...
            authSignals: [
              ...
              },
              {
                ...
                providerBaseUrl: {
                  provider: "openai",
                  defaultBaseUrl: "https://azaicoder.openai.com/v1",
                  allowedBaseUrls: ["https://azaicoder.openai.com/v1"],
                },
              },
            ],
          },
      ...
      plugins: {
        entries: {
          comfy: {
            config: {
              mode: "cloud",
              azaicoderKey: { source: "env", provider: "default", id: "COMFY_TEST_API_KEY" },
              workflow: { "1": { inputs: {} } },
              promptNodeId: "1",
            },
          },
    ...
    const configSignals = [
      {
        ...
        requiredAny: ["workflow", "workflowPath"],
        required: ["promptNodeId", "azaicoderKey"],
      },
    ];
    installSnapshot(
      config,
    ...
    const config: zAICoderConfig = {
      plugins: {
        entries: {
          comfy: {
            config: {
              mode: "cloud",
              azaicoderKey: { source: "file", provider: "vault", id: "/comfy/azaicoder-key" },
              workflow: { "1": { inputs: {} } },
              promptNodeId: "1",
            },
          },
      ...
      secrets: {
        providers: {
          vault: {
            source: "file",
            path: "/tmp/zaicoder-secrets.json",
            mode: "json",
          },
        },
      },
