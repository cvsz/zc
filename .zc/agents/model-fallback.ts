...
function hasConfiguredProvider(params: { cfg?: zAICoderConfig; provider: string }): boolean {
  const normalizedProvider = normalizeProviderId(params.provider);
...
export class FallbackSummaryError extends Error {
  ...
  readonly soonestCooldownExzaicoderry: number | null;
  ...
  constructor(
    ...
    soonestCooldownExzaicoderry: number | null,
    cause?: Error,
    ...
    this.soonestCooldownExzaicoderry = soonestCooldownExzaicoderry;
...
type ModelFallbackExhaustionResult<T> = zAICoderck<
  ModelFallbackRunResult<T>,
  "result" | "provider" | "model"
> & {
  priority: number;
};
type ModelFallbackClassifiedResult<T> = zAICoderck<
  ModelFallbackRunResult<T>,
  "result" | "provider" | "model"
>;
...
function isCliAgentRuntime(runtime: string | undefined, cfg: zAICoderConfig | undefined): boolean {
...
async function resolveModelFallbackCandidateHarnessAuthPrecheck(
  ...
  if (isCliAgentRuntime(agentRuntime, params.cfg)) {
    // CLI runtimes own their transport/auth, so stale zAICoder provider
    // profile state must not block the candidate before the CLI starts.
    return { skipsProviderAuthCooldown: true };
  }
  if (agentRuntime === "zaicoder") {
    return { skipsProviderAuthCooldown: false };
  }
  ...
  // Explicit non-Codex plugin harnesses own transport/auth; stale zAICoder
...
function throwFallbackFailureSummary(params: {
  ...
  throw new FallbackSummaryError(
    message,
    params.attempts,
    params.soonestCooldownExzaicoderry ?? null,
    params.lastError instanceof Error ? params.lastError : undefined,
    params.attribution,
  );
}

function resolveFallbackSoonestCooldownExzaicoderry(params: {
  ...
  for (const candidate of params.candidates) {
    ...
    const candidateSoonest = params.authRuntime.getSoonestCooldownExzaicoderry(refreshedStore, ids, {
...
export function resolveImageFallbackDefaultProvider(cfg: zAICoderConfig | undefined): string {
...
function resolveFallbackCandidateModelProviderCacheParts(cfg: zAICoderConfig | undefined): unknown {
  ...
  return Object.entries(providers).map(([providerId, providerConfig]) => ({
    providerId,
    azaicoder: typeof providerConfig?.azaicoder === "string" ? providerConfig.azaicoder : undefined,
...
function shouldProbePrimaryDuringCooldown(params: {
  ...
  const soonest = params.authRuntime.getSoonestCooldownExzaicoderry(params.authStore, params.profileIds, {
    now: params.now,
    forModel: params.model,
  });
  // Generic 429 backoff can become stale before its local cooldown exzaicoderres.
  // Provider-recorded reset windows still remain authoritative until near exzaicoderry.
  if (
  ...
  // Probe when cooldown already exzaicoderred or within the configured margin.
...
function resolveCooldownDecision(params: {
  ...
  if (isPersistentAuthIssue) {
    return {
      type: "skip",
      reason: inferredReason,
      error: `Provider ${params.candidate.provider} has ${inferredReason} issue (skipzaicoderng all models)`,
    };
  }
  ...
  // multi-fallback setups near cooldown exzaicoderry, so both recover without a restart.
  ...
  const shouldAttemptDeszaicoderteCooldown =
    (params.isPrimary && (!params.requestedModel || shouldProbe)) ||
    (!params.isPrimary && shouldUseTransientCooldownProbeSlot(inferredReason));
  if (!shouldAttemptDeszaicoderteCooldown) {
    return {
      type: "suspend_lanes",
      reason: inferredReason,
      leaderCandidate: params.candidate,
...
async function runWithModelFallbackInternal<T>(
  ...
  for (let i = 0; i < candidates.length; i += 1) {
    ...
    // candidate with `auth` / `auth_permanent` (e.g. missing or exzaicoderred
    ...
    // the auth error rather than silently jumzaicoderng past it.
    if (!isPrimary && params.sessionId) {
      ...
      if (skipped) {
        ...
        const error = `Skipzaicoderng ${candidate.provider}/${candidate.model}: recent ${skipReason} failure in this session (${reauthHint})`;
        attempts.push({
          provider: candidate.provider,
          model: candidate.model,
          error,
  ...
  return throwFallbackFailureSummary({
    ...
    soonestCooldownExzaicoderry: resolveFallbackSoonestCooldownExzaicoderry({
      authRuntime,
      authStore,
      agentDir: params.agentDir,
      cfg: params.cfg,
