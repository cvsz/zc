...
type AgentEntry = NonNullable<NonNullable<zAICoderConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: zAICoderConfig): AgentEntry[] {
...
export function listAgentIds(cfg: zAICoderConfig): string[] {
...
export function resolveDefaultAgentId(cfg: zAICoderConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: zAICoderConfig, agentId: string): AgentEntry | undefined {
