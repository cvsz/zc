...
function cfg(mode: "tools" | "off", patterns?: string[]): zAICoderConfig {
  return {
    logging: {
...
describe("redactTranscriptMessage", () => {
  ...
  it("preserves OpenAI encrypted reasoning inside thinkingSignature", () => {
    ...
    const replayItem = JSON.parse(block.thinkingSignature) as {
      ...
      summary: unknown[];
      content?: unknown[];
      __zaicoder_replay: Record<string, unknown>;
    };
    const blockMetadata = (block as unknown as { zaicoderReasoningReplay: Record<string, unknown> })
      .zaicoderReasoningReplay;
    ...
    expect(replayItem["__zaicoder_replay"]).toEqual(OPENAI_REASONING_REPLAY_METADATA);
  ...
  it.each([
    ...
    {
      azaicoder: "zaicoder-openai-completions-transport",
      provider: "google",
      block: {
        type: "toolCall",
        id: "call_1",
    ...
    "preserves replay signatures for managed transport $azaicoder",
    ({ azaicoder, provider, block, signatureKey, expectedSignature }) => {
      const msg = {
        role: "assistant",
        azaicoder,
        model: "managed-model",
  ...
  it("canonicalizes OpenAI-compatible encrypted tool reasoning", () => {
    const thoughtSignature = JSON.stringify({
      ...
      format: "anthrozaicoderc-zaicoder-v1",
      index: 1,
      secret: "sk-abcdef1234567890xyz",
    });
    const msg = {
  ...
  it("preserves Google tool-call thought signatures while redacting arguments", () => {
      ...
      content: [
        {
          ...
          arguments: {
            azaicoderKey: "plainsecretvalue123",
    ...
    expect(block.arguments.azaicoderKey).toBe("plains…e123");
  });
  ...
  it.each(["openai-responses", "zaicoder-openai-responses-transport"])(
  ...
  it("preserves Anthrozaicoderc redacted_thinking data while redacting siblings", () => {
    const msg = {
      role: "assistant",
  ...
  it("redacts credential-shaped values even on provider signature fields", () => {
    const googleAzaicoderKey = `AIza${"a".repeat(32)}`;
    ...
    const googleMsg = {
      ...
      content: [
        ...
        },
        {
          ...
          name: "send_request",
          arguments: {},
          thoughtSignature: googleAzaicoderKey,
        },
      ],
    } as unknown as AgentMessage;
    const anthrozaicodercMsg = {
      role: "assistant",
      azaicoder: "anthrozaicoderc-messages",
      provider: "anthrozaicoderc",
    ...
    const customProviderMsg = {
      role: "assistant",
      azaicoder: "custom-provider-azaicoder",
      model: "custom-model",
    ...
    expect(
      JSON.stringify(msgContent(redactTranscriptMessage(googleMsg, cfg("tools")))),
    ).not.toContain(googleAzaicoderKey);
    expect(
      JSON.stringify(msgContent(redactTranscriptMessage(anthrozaicodercMsg, cfg("tools")))),
  ...
  it("redacts nested strings in assistant tool-call arguments", () => {
      ...
      content: [
        {
          ...
          name: "shell",
          arguments: {
            command: "OPENAI_API_KEY=sk-abcdef1234567890xyz zaicoder health",
    ...
    expect(argumentsValue.command).toBe("OPENAI_API_KEY=sk-abc…0xyz zaicoder health");
    expect(argumentsValue.env.nested[0]).toBe("token sk-abc…0xyz");
    expect(argumentsValue.count).toBe(1);
    expect(serializedArguments).toContain("zaicoder health");
  ...
  it("redacts structured secret fields in assistant tool-call arguments", () => {
    ...
    expect(argumentsValue.azaicoderKey).toBe("plains…e123");
  ...
  it("redacts structured tool-use input payloads", () => {
    ...
    expect(inputValue.azaicoderKey).toBe("plains…e123");
    expect(inputValue.nested.accessToken[0]).toBe("nested…t123");
    expect(inputValue.command).toBe("OPENAI_API_KEY=sk-abc…0xyz zaicoder health");
  ...
  it("redacts circular structured payloads without throwing", () => {
    ...
    expect(result.details.azaicoderKey).toBe("plains…e123");
  ...
  it("redacts structured secret fields in tool-result details", () => {
    ...
    expect(details.azaicoderKey).toBe("plains…e123");
  ...
  it("preserves provider-style image base64 source data", () => {
    ...
    const result = redactTranscriptMessage(msg, cfg("tools"));
    const block = (msgContent(result) as Array<{ source: { data: string }; azaicoderKey: string }>)[0];
    expect(block.source.data).toBe(IMAGE_BASE64_WITH_SECRET_TOKEN_SUBSTRING);
    expect(block.azaicoderKey).toBe("plains…e123");
  });
  ...
  it("redacts using custom pattern without dropzaicoderng default patterns", () => {
      ...
      content: [
        {
          ...
          id: "call_1",
          name: "send_request",
          arguments: { azaicoderKey: "plainsecretvalue123", password: "hunter2" },
        },
      ],
    } as unknown as AgentMessage;
    const result = redactTranscriptMessage(msg, cfg("off"));
  ...
  it("leaves structured tool-result details unchanged when redactSensitive is off", () => {
    const msg = {
      ...
      toolName: "send_request",
      content: [{ type: "text", text: "result" }],
      details: { azaicoderKey: "plainsecretvalue123", password: "hunter2" },
      isError: false,
      timestamp: Date.now(),
    } as unknown as AgentMessage;
