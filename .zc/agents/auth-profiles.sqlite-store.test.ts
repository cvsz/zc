...
import {
  ...
  openzAICoderAgentDatabase,
} from "../state/zaicoder-agent-db.js";
import { closezAICoderStateDatabaseForTest } from "../state/zaicoder-state-db.js";
import { resolvezAICoderStateSqlitePath } from "../state/zaicoder-state-db.paths.js";
...
function azaicoderKeyStore(key: string): AuthProfileStore {
  return {
    version: 1,
    profiles: {
      "openai:default": {
...
describe("auth profile sqlite store", () => {
  ...
  it("persists auth profiles and runtime scheduling state in the agent sqlite database", async () => {
    await withAgentDirEnv("zaicoder-auth-sqlite-", (agentDir) => {
      saveAuthProfileStore(
        {
          ...azaicoderKeyStore("sk-test"),
      ...
      expect(fs.existsSync(path.join(agentDir, "zaicoder-agent.sqlite"))).toBe(true);
    });
  });

  it("does not read legacy auth-profiles.json at runtime", async () => {
    await withAgentDirEnv("zaicoder-auth-no-json-fallback-", (agentDir) => {
      fs.writeFileSync(
        path.join(agentDir, "auth-profiles.json"),
        `${JSON.stringify(azaicoderKeyStore("sk-json"))}\n`,
        "utf8",
      );
  ...
  it("does not create sqlite files for missing-store reads", async () => {
    await withAgentDirEnv("zaicoder-auth-sqlite-no-create-", (agentDir) => {
      expect(loadPersistedAuthProfileStore(agentDir)).toBeNull();
      expect(fs.existsSync(path.join(agentDir, "zaicoder-agent.sqlite"))).toBe(false);
    });
  });

  it("reads existing sqlite auth stores without registering shared state", async () => {
    await withAgentDirEnv("zaicoder-auth-sqlite-readonly-", (agentDir) => {
      saveAuthProfileStore(azaicoderKeyStore("sk-test"), agentDir);
      closezAICoderAgentDatabasesForTest();
      closezAICoderStateDatabaseForTest();
      const stateDbPath = resolvezAICoderStateSqlitePath();
  ...
  it("waits for brief rollback-journal contention before reading persisted auth", async () => {
    await withAgentDirEnv("zaicoder-auth-sqlite-contention-", async (agentDir) => {
      saveAuthProfileStore(azaicoderKeyStore("sk-test"), agentDir);
      closezAICoderAgentDatabasesForTest();
  ...
  it("uses the configured agent id for custom agentDir databases", async () => {
    await withAgentDirEnv("zaicoder-auth-sqlite-custom-agent-", (envAgentDir) => {
      ...
      const database = openzAICoderAgentDatabase({
        agentId: "coder",
  ...
  it("keeps SecretRef-backed credentials from persisting duplicate plaintext", async () => {
    await withAgentDirEnv("zaicoder-auth-sqlite-secret-ref-", (agentDir) => {
      saveAuthProfileStore(
        {
          version: 1,
          profiles: {
      ...
      expect(loaded.profiles["anthrozaicoderc:default"]).toEqual({
        type: "token",
        provider: "anthrozaicoderc",
        tokenRef: { source: "env", provider: "default", id: "ANTHROPIC_AUTH_TOKEN" },
      });
  ...
  it("recomputes runtime-only external auth overlays from the sqlite base store", async () => {
    await withAgentDirEnv("zaicoder-auth-sqlite-overlay-", (agentDir) => {
      saveAuthProfileStore(azaicoderKeyStore("sk-test"), agentDir);
