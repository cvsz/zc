 ...
 * Covers container path mapzaicoderng, malformed suffix cleanup, and normalized path
...
describe("wrapToolWorkspaceRootGuardWithOptions", () => {
  ...
  it("adds a workspace-safe temp hint when rejecting paths outside the workspace", async () => {
    ...
    await expect(
      ...
      /Path escapes sandbox root .* Use a relative path under `.zaicoder\/tmp\/` inside the workspace/,
    );
  ...
  it("rejects custom path params that become empty after suffix stripzaicoderng", async () => {
    const { execute, tool } = createToolHarness();
    const wrapped = wrapToolWorkspaceRootGuardWithOptions(tool, root, {
      containerWorkdir: "/workspace",
      pathParamKeys: ["outPath"],
