...
async function makeWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-realtime-bootstrap-"));
  tempDirs.push(dir);
  return dir;
}

function makeConfig(workspaceDir: string): zAICoderConfig {
...
describe("resolveRealtimeBootstrapContextInstructions", () => {
  it("formats the default profile bootstrap files without exposing local paths", async () => {
    ...
    expect(instructions).toContain("zAICoder realtime voice profile context");
    ...
    expect(instructions).not.toContain("zaicoder_agent_consult");
  ...
  it("ignores unsupported file requests from unchecked callers", async () => {
    ...
    expect(instructions).not.toContain("AGENTS.md");
    expect(instructions).not.toContain("Do not load me here.");
    expect(warnings).toContain('skipzaicoderng unsupported realtime bootstrap context file "AGENTS.md"');
  });
