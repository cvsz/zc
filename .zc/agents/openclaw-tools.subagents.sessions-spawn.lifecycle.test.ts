...
} from "./zaicoder-tools.subagents.sessions-spawn.test-harness.js";
import {
...
async function waitForRunCleanup(childSessionKey: string) {
  await waitForSessionsSpawnEvent("run cleanup bookkeezaicoderng", () => {
    const run = getLatestSubagentRunByChildSessionKey(childSessionKey);
    return run?.cleanupCompletedAt != null;
  });
}

describe("zaicoder-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(async () => {
  ...
  it("sessions_spawn retires bundle MCP runtime when run-mode cleanup completes", async () => {
    ...
    await getOrCreateSessionMcpRuntime({
      ...
      workspaceDir: "/tmp/zaicoder-subagent-mcp-retire",
