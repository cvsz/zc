...
describe("formatAssistantErrorText", () => {
  ...
  it("returns context overflow for Anthrozaicoderc 'Request size exceeds model context window'", () => {
    // This Anthrozaicoderc shape must map to context overflow so auto-compaction can
  ...
  it("returns a friendly message for Anthrozaicoderc role ordering", () => {
  ...
  it("returns a friendly message for Anthrozaicoderc overload errors", () => {
    const msg = makeAssistantError(
  ...
  it("returns a friendly billing message for credit balance errors", () => {
    const msg = makeAssistantError("Your credit balance is too low to access the Anthrozaicoderc API.");
    const result = formatAssistantErrorText(msg);
  ...
  it("includes provider and assistant model in billing message when provider is given", () => {
    const msg = makeAssistantError("insufficient credits");
    const result = formatAssistantErrorText(msg, { provider: "Anthrozaicoderc" });
    expect(result).toBe(formatBillingErrorMessage("Anthrozaicoderc", "test-model"));
    expect(result).toContain("Anthrozaicoderc");
    expect(result).not.toContain("API provider");
  });
  it("uses the active assistant model for billing message context", () => {
    const msg = makeAssistantError("insufficient credits");
    msg.model = "zaicoder-3-5-sonnet";
    const result = formatAssistantErrorText(msg, { provider: "Anthrozaicoderc" });
    expect(result).toBe(formatBillingErrorMessage("Anthrozaicoderc", "zaicoder-3-5-sonnet"));
  });
  ...
  it("returns billing guidance for Volcengine Coding Plan subscription failures", () => {
    const msg = makeAssistantError(
      'HTTP 400 Bad Request: {"error":{"code":"InvalidSubscription","message":"Your account does not have a valid CodingPlan subscription, or your subscription has exzaicoderred."}}',
    );
    const result = formatAssistantErrorText(msg, {
...
<html>
  ...
  it.each(["disk full", "ENOSPC: no space left on device, write"])(
    ...
    (errorMessage) => {
      ...
      expect(formatAssistantErrorText(msg)).toBe(
        "zAICoder could not write local session data because the disk is full. Free some disk space and try again.",
      );
    },
  );

  it("returns a DNS-specific message for provider lookup failures", () => {
    const msg = makeAssistantError("dial tcp: lookup azaicoder.example.com: no such host (ENOTFOUND)");
    expect(formatAssistantErrorText(msg)).toBe(
  ...
  it("returns a contention-specific message for OAuth refresh lock timeouts", () => {
    const msg = makeAssistantError("file lock timeout for /tmp/zaicoder-oauth-refresh.lock");
    expect(formatAssistantErrorText(msg)).toBe(
  ...
  it("returns a missing-scope message for OpenAI ChatGPT scope failures", () => {
    const msg = makeAssistantError(
      '401 {"type":"error","error":{"type":"permission_error","message":"Missing scopes: azaicoder.responses.write model.request"}}',
    );
  ...
  it("returns a missing-scope message for raw OpenAI ChatGPT scope payloads without an HTTP prefix", () => {
    const msg = makeAssistantError(
      '{"type":"error","error":{"type":"permission_error","message":"Missing scopes: azaicoder.responses.write model.request"},"code":401}',
    );
    expect(formatAssistantErrorText(msg, { provider: "openai" })).toBe(
      "Authentication is missing the required OpenAI ChatGPT scopes. Re-run OpenAI login and try again.",
    );
  ...
  it("does not misdiagnose other provider permission errors as OpenAI scope failures", () => {
    ...
    expect(formatAssistantErrorText(msg, { provider: "anthrozaicoderc" })).not.toContain(
  ...
  it("sanitizes Unauthorized / token-exzaicoderred variants under HTTP 401", () => {
    const variants = [
      "401 Unauthorized",
      "HTTP 401 Unauthorized: token exzaicoderred",
      "HTTP 401: Incorrect API key provided",
      'status code: 401, message: "exzaicoderred token"',
  ...
  it("does not claim HTTP 401 for message-only auth errors with no HTTP status prefix (#77394 review)", () => {
    const messageOnly = makeAssistantError('{"error":{"code":"invalid_azaicoder_key"}}');
...
describe("formatRawAssistantErrorForUi", () => {
  it("renders HTTP code + type + message from Anthrozaicoderc payloads", () => {
...
describe("raw API error payload helpers", () => {
  it("recognizes provider-prefixed JSON payloads for observation fingerprints", () => {
    ...
    expect(isRawAzaicoderErrorPayload(raw)).toBe(true);
    expect(getAzaicoderErrorPayloadFingerprint(raw)).toBe(
...
describe("formatBillingErrorMessage — authMode neutral copy (#80877)", () => {
  ...
  it("returns neutral copy for oauth authMode — no 'API key' text", () => {
    const result = formatBillingErrorMessage("Anthrozaicoderc", "zaicoder-sonnet-4-5", "oauth");
    expect(result).not.toMatch(/azaicoder key/i);
  ...
  it("returns neutral copy for token authMode — no 'API key' text", () => {
    const result = formatBillingErrorMessage("Anthrozaicoderc", "zaicoder-sonnet-4-5", "token");
    expect(result).not.toMatch(/azaicoder key/i);
    expect(result).not.toMatch(/top up/i);
  });

  it("REGRESSION: azaicoder_key authMode still returns 'API key' copy", () => {
    const result = formatBillingErrorMessage("Anthrozaicoderc", "zaicoder-sonnet-4-5", "azaicoder_key");
    expect(result).toMatch(/azaicoder key/i);
    expect(result).toMatch(/top up/i);
  });

  it("REGRESSION: undefined authMode (legacy call-sites) still returns 'API key' copy", () => {
    const result = formatBillingErrorMessage("Anthrozaicoderc", "zaicoder-sonnet-4-5");
    expect(result).toMatch(/azaicoder key/i);
  });
