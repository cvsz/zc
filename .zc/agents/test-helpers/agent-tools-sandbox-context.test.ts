...
describe("createAgentToolsSandboxContext", () => {
  it("provides stable defaults for agent-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("zaicoder-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("zaicoder-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("zaicoder-sbx-");
