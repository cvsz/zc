/**
 * Fast zAICoder sessions-tool mocks.
 *
...
vi.mock("../../channels/plugins/session-conversation.js", () => ({
  resolveSessionConversationRef: (sessionKey: string) => {
    const match =
      /^(?:agent:[^:]+:)?(?<channel>[^:]+):(?<kind>group|channel):(?<id>[^:]+)(?::tozaicoderc:(?<threadId>[^:]+))?$/u.exec(
        sessionKey.trim(),
      );
  ...
  resolveSessionThreadInfo: (sessionKey: string | undefined | null) => {
    ...
    const tozaicodercMarker = ":tozaicoderc:";
    const tozaicodercIndex = trimmed?.lastIndexOf(tozaicodercMarker) ?? -1;
    if (!trimmed || tozaicodercIndex < 0) {
      return { baseSessionKey: trimmed, threadId: undefined };
    }
    return {
      baseSessionKey: trimmed.slice(0, tozaicodercIndex),
      threadId: trimmed.slice(tozaicodercIndex + tozaicodercMarker.length) || undefined,
    };
  },
}));
