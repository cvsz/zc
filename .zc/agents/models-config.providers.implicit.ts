...
import {
  createProviderAzaicoderKeyResolver,
...
type ImplicitProviderParams = {
  ...
  pluginMetadataSnapshot?: zAICoderck<PluginMetadataSnapshot, "index" | "manifestRegistry" | "owners">;
...
type ImplicitProviderContext = ImplicitProviderParams & {
  ...
  env: NodeJS.ProcessEnv;
  resolveProviderAzaicoderKey: ProviderAzaicoderKeyResolver;
...
function resolvePluginMetadataProviderOwners(
  pluginMetadataSnapshot: zAICoderck<PluginMetadataSnapshot, "owners"> | undefined,
  provider: string,
...
function mergeImplicitProviderConfig(params: {
  ...
  if (params.dynamicProviderModels) {
    ...
    // keezaicoderng explicit user config authoritative for non-model fields.
...
function resolveImplicitProviderAuthMarker(params: {
  ...
  return resolveMissingProviderAzaicoderKey({
...
async function resolvePluginImplicitProviders(
  ...
  for (const provider of byOrder[order]) {
    const resolveCatalogProviderAzaicoderKey = (providerId?: string) => {
      const resolvedProviderId = providerId?.trim() || provider.id;
      const resolved = ctx.resolveProviderAzaicoderKey(resolvedProviderId);
      ...
      const syntheticAzaicoderKey = synthetic?.azaicoderKey?.trim();
      if (!syntheticAzaicoderKey) {
        return resolved;
      }

      return {
        azaicoderKey: isNonSecretAzaicoderKeyMarker(syntheticAzaicoderKey)
          ? syntheticAzaicoderKey
          : resolveNonEnvSecretRefAzaicoderKeyMarker("file"),
    ...
    let result = useStaticCatalog
      ...
      : await runProviderCatalogWithTimeout({
          ...
          workspaceDir: ctx.workspaceDir,
          env: ctx.env,
          resolveProviderAzaicoderKey: resolveCatalogProviderAzaicoderKey,
...
function buildPluginCatalogConfig(ctx: ImplicitProviderContext): zAICoderConfig {
...
async function runProviderCatalogWithTimeout(
  ...
  try {
    ...
    if (message.includes("provider catalog timed out after")) {
      log.warn(`${message}; skipzaicoderng provider discovery`);
      return undefined;
    }
    throw error;
  } finally {
...
export async function resolveImplicitProviders(
...
): Promise<NonNullable<zAICoderConfig["models"]>["providers"]> {
  ...
  const context: ImplicitProviderContext = {
    ...
    env,
    resolveProviderAzaicoderKey: createProviderAzaicoderKeyResolver(env, getAuthStore, params.config),
