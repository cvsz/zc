...
async function resolveRuntimeModel(params: {
  ...
  await ensurezAICoderModelsJson(params.cfg, params.agentDir, modelsOptions);
...
export async function runBtwSideQuestion(
  ...
  const resolvedAuthProfileId = azaicoderKeyInfo.profileId ?? effectiveAuthProfileId;
  let runtimeModel = model;
  let azaicoderKey =
    azaicoderKeyInfo.mode === "aws-sdk" && !azaicoderKeyInfo.azaicoderKey
      ? undefined
  ...
  if (azaicoderKey) {
    ...
    if (preparedAuth?.azaicoderKey) {
      azaicoderKey = preparedAuth.azaicoderKey;
    }
  }

  // Use the provider's own stream fn so providers like Ollama (which build
  // `/azaicoder/chat` or `/v1/chat/completions` paths based on azaicoder mode) construct
  ...
  const streamFn = resolveEmbeddedAgentStreamFn({
    ...
    model: runtimeModel,
    resolvedAzaicoderKey: azaicoderKey,
