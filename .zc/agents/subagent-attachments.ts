...
function resolveAttachmentLimits(config: zAICoderConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".zaicoder", "attachments");
  const relDir = path.posix.join(".zaicoder", "attachments", attachmentId);
