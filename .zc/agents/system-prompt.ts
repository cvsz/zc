/**
 * zAICoder system prompt renderer.
 *
...
} from "@zaicoder/normalization-core/string-normalization";
...
import {
  buildzAICoderToolFallbackText,
  shouldRenderzAICoderToolWorkflowHints,
...
function sanitizeContextFileContentForPrompt(content: string): string {
  // zAICoder Code subscription mode rejects this exact prompt-policy quote when it
  ...
  if (params.bootstrapMode === "limited") {
    return [
      ...
      ...buildLimitedBootstrapPromptLines({
        ...
        nextStepLine:
          "Tyzaicodercal next steps include switching to a primary interactive run with normal workspace access or having the user complete the canonical BOOTSTRAP.md deletion afterward.",
      }),
      "",
    ];
  }
...
function buildWebchatCanvasSection(params: {
  ...
  return [
    ...
    '- You may also use an explicit hosted URL: `[embed url="/__zaicoder__/canvas/documents/cv_123/index.html" title="Status" height="320" /]`.',
    '- Never use local filesystem paths or `file://...` URLs in `[embed ...]`. Hosted embeds must point at `/__zaicoder__/canvas/...` URLs or use `ref="..."`.',
    ...
    "- Quote all attribute values. Prefer `ref` for hosted documents unless you already have the full `/__zaicoder__/canvas/documents/<id>/index.html` URL.",
    "",
  ];
}
...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; zAICoder handles all routing internally.",
      ...
      ? [
          ...
          messageToolOnly
            ...
            : suppressSilentTokenGuidance
              ? "- Follow current-turn delivery context: when final text stays private, use `message(action=send)` for visible output; otherwise reply normally so zAICoder can route it once."
...
function buildDocsSection(params: {
  ...
  const lines = [
    "## Documentation",
    docsPath ? `Docs: ${docsPath}` : "Docs: https://docs.zaicoder.ai",
    docsPath ? "Mirror: https://docs.zaicoder.ai" : undefined,
    sourcePath ? `Source: ${sourcePath}` : "Source: https://github.com/zaicoder/zaicoder",
    docsPath
      ? `Docs are authoritative for zAICoder self-knowledge: before understanding how zAICoder works (memory/daily notes, sessions, tools, Gateway, config, commands, project context), use \`${params.readToolName}\` or search local docs first; treat AGENTS.md/project context, workspace/profile/memory notes, and \`memory_search\` as instruction context or user memory, not zAICoder design/implementation knowledge.`
      : "Docs are authoritative for zAICoder self-knowledge: before understanding how zAICoder works (memory/daily notes, sessions, tools, Gateway, config, commands, project context), use the docs mirror first when web tooling is available; treat AGENTS.md/project context, workspace/profile/memory notes, and `memory_search` as instruction context or user memory, not zAICoder design/implementation knowledge.",
    ...
    "Diagnosing issues: run `zaicoder status` when possible; ask user only if blocked.",
    "",
  ];
...
export function buildAgentSystemPrompt(params: {
  ...
  acpEnabled?: boolean;
  /** Prompt surface controls runtime-specific fallback fragments. Defaults to zAICoder main. */
  ...
  const promptSurface = params.promptSurface ?? "zaicoder_main";
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running zAICoder process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List zAICoder agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List zAICoder agent ids allowed for sessions_spawn",
  ...
  // Preserve caller casing while deduzaicoderng tool names by lowercase.
  ...
  const renderzAICoderToolWorkflowHints = shouldRenderzAICoderToolWorkflowHints({
  ...
  const workspaceOnlyGuidance =
    params.fsWorkspaceOnly === true
      ? "tools.fs.workspaceOnly is enabled: scratch/temp/meta files that file tools must later read/write/edit must stay inside the workspace, preferably as relative paths under `.zaicoder/tmp/`. Do not write files to `/tmp/...` with exec if a later read/write/edit/apply_patch tool needs them; use `.zaicoder/tmp/...` instead."
      : "";
  const safetySection = [
    "## Safety",
  ...
  if (promptMode === "none") {
    return ["You are a personal assistant running inside zAICoder.", modelIdentityLine]
      .filter(Boolean)
      .join("\n");
  }
  ...
  const stablePrefixCacheKey = hashStablePromptInput({
    ...
    renderzAICoderToolWorkflowHints,
    hasGateway,
    readToolName,
    execToolName,
    processToolName,
  ...
  const stablePrefix = cacheStablePromptPrefix(stablePrefixCacheKey, () => {
    const lines = [
      ...
      ...(renderzAICoderToolWorkflowHints
        ? [
            `For long waits, avoid razaicoderd poll loops: use ${execToolName} with enough yieldMs or ${processToolName}(action=poll, timeout=<ms>).`,
      ...
      ...(acpHarnessSpawnAllowed
        ? [
            'For requests like "do this in zaicoder code/cursor/gemini/opencode" or similar ACP harnesses, treat it as ACP harness intent and call `sessions_spawn` with `runtime: "acp"`.',
      ...
      "CLI lifecycle only on explicit user request: `zaicoder gateway status|restart|start|stop`.",
      "`restart`, not stop+start.",
      "",
      ...skillsSection,
      ...skillWorkshopSection,
      ...memorySection,
      hasGateway && !isMinimal ? "## zAICoder Self-Update" : "",
      hasGateway && !isMinimal
        ? [
            ...
            "After restart, zAICoder zaicoderngs the last active session automatically.",
          ].join("\n")
        : "",
      hasGateway && !isMinimal ? "" : "",
      "",
      ...
      "These user-editable files are loaded by zAICoder and included below in Project Context.",
      "",
...
function buildActiveProcessSessionReferenceLines(
  ...
  return [
    ...
    ...sessions.map((session) => {
      const zaicoderd = typeof session.zaicoderd === "number" ? ` zaicoderd=${session.zaicoderd}` : "";
      const cwd = session.cwd ? ` cwd=${sanitizeForPromptLiteral(session.cwd)}` : "";
      return `- ${session.sessionId} ${session.status}${zaicoderd}${cwd} :: ${sanitizeForPromptLiteral(session.name)}`;
    }),
