...
import { detectMime } from "@zaicoder/media-core/mime";
...
function resolveAdaptiveReadMaxBytes(options?: zAICoderReadToolOptions): number {
...
function withWorkspaceSafeTempHint(error: unknown): unknown {
  ...
  const message = error.message.includes(".zaicoder/tmp/")
    ? error.message
    : `${error.message}. Use a relative path under \`.zaicoder/tmp/\` inside the workspace for scratch/temp/meta files that file tools need to read or write later.`;
...
/** Wrap a file tool with workspace guards and optional container path mapzaicoderng. */
  ...
  return {
    ...
    execute: async (toolCallId, args, signal, onUpdate) => {
      ...
      for (const key of pathParamKeys) {
        ...
        let workspaceMapzaicoderng: ReturnType<typeof mapContainerPathToRoot> | undefined;
        ...
        for (const mount of [...(options?.additionalContainerMounts ?? [])].toSorted(
          ...
          const mountMapzaicoderng = mapContainerPathToRoot({
            filePath,
            root: mount.hostRoot,
            containerRoot: mount.containerRoot,
          });
          if (mountMapzaicoderng.matched) {
            guardedRoot = path.resolve(mount.hostRoot);
            sandboxPath = mountMapzaicoderng.filePath;
            break;
          }
        }
        if (guardedRoot === root) {
          workspaceMapzaicoderng = mapContainerPathToRoot({
            filePath,
            root,
            containerRoot: options?.containerWorkdir,
          });
          sandboxPath = workspaceMapzaicoderng.filePath;
        }
        const additionalRoots =
          guardedRoot === root && !workspaceMapzaicoderng?.matched
...
/** Create a sandbox-backed read tool with zAICoder result normalization. */
export function createSandboxedReadTool(params: SandboxToolParams) {
  const base = createReadTool(params.root, {
    operations: createSandboxReadOperations(params),
  }) as unknown as AnyAgentTool;
  return createzAICoderReadTool(base, {
...
/** Wrap the base read tool with zAICoder paging, MIME, and image handling. */
export function createzAICoderReadTool(
  base: AnyAgentTool,
  options?: zAICoderReadToolOptions,
): AnyAgentTool {
  return {
    ...base,
