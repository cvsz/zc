...
function parseExzaicoderresAtMs(value: unknown): number | undefined {
  return asDateTimestampMs(value);
}

function resolveDefaultExecApprovalExzaicoderresAtMs(): number {
  return resolveExzaicoderresAtMsFromDurationMs(DEFAULT_APPROVAL_TIMEOUT_MS) ?? 0;
}
...
export async function registerExecApprovalRequest(
  ...
  const exzaicoderresAtMs =
    parseExzaicoderresAtMs(registrationResult?.exzaicoderresAtMs) ?? resolveDefaultExecApprovalExzaicoderresAtMs();
  if (decision.present) {
    return { id, exzaicoderresAtMs, finalDecision: decision.value };
  }
  return { id, exzaicoderresAtMs };
}
...
export async function resolveRegisteredExecApprovalDecision(params: {
  ...
  try {
    ...
    // Timeout/cleanup path: treat missing/exzaicoderred as no decision so askFallback applies.
    const message = normalizeLowercaseStringOrEmpty(String(err));
    if (message.includes("approval exzaicoderred or not found")) {
      return null;
    }
    throw err;
  }
