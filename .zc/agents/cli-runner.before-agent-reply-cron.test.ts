...
const {
  ...
  closezAICoderLiveSessionForContextMock,
  ...
  closezAICoderLiveSessionForContextMock: vi.fn(),
...
vi.mock("./cli-runner/zaicoder-live-session.js", () => ({
  closezAICoderLiveSessionForContext: closezAICoderLiveSessionForContextMock,
}));
...
const baseRunParams = {
  ...
  prompt: "__zaicoder_memory_core_short_term_promotion_dream__",
  provider: "codex-cli",
  model: "gpt-5.5",
  timeoutMs: 30_000,
  runId: "test-run-id",
...
beforeEach(() => {
  ...
  closezAICoderLiveSessionForContextMock.mockReset();
...
describe("runCliAgent cron before_agent_reply seam", () => {
  ...
  it("can close temporary CLI live sessions after a run", async () => {
    ...
    expect(executePreparedCliRunMock).toHaveBeenCalledTimes(1);
    expect(closezAICoderLiveSessionForContextMock).toHaveBeenCalledTimes(1);
    expect(closezAICoderLiveSessionForContextMock).toHaveBeenCalledWith(
