...
import type { UserMessage } from "zaicoder/plugin-sdk/llm";
...
vi.mock("zaicoder/plugin-sdk/agent-sessions", async () => {
  const actual = await vi.importActual<typeof import("zaicoder/plugin-sdk/agent-sessions")>(
    "zaicoder/plugin-sdk/agent-sessions",
  );
  return {
    ...actual,
...
describe("summarizeWithFallback", () => {
  ...
  it("does not duplicate summarization when no messages were oversized", async () => {
    ...
    const result = await summarizeWithFallback({
      messages,
      model: testModel,
      azaicoderKey: "test-key", // pragma: allowlist secret
