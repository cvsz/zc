...
const longChildCompletionOutput = [
  ...
  "PR: https://github.com/zaicoder/zaicoder/pull/12345",
...
describe("resolveAnnounceOrigin threaded route targets", () => {
  ...
  it("preserves stored thread ids when requester origin omits one for the same chat", () => {
    expect(
      resolveAnnounceOrigin(
        {
          lastChannel: "tozaicodercchat",
          lastTo: "tozaicodercchat:room-a:tozaicoderc:99",
          lastThreadId: 99,
        },
        {
          channel: "tozaicodercchat",
          to: "tozaicodercchat:room-a",
        },
      ),
    ).toEqual({
      channel: "tozaicodercchat",
  ...
  it("still strips stale thread ids when the stored route points at a different chat", () => {
    expect(
      resolveAnnounceOrigin(
        {
          lastChannel: "tozaicodercchat",
          lastTo: "tozaicodercchat:room-b:tozaicoderc:99",
          lastThreadId: 99,
        },
        {
          channel: "tozaicodercchat",
...
describe("deliverSubagentAnnouncement completion delivery", () => {
  ...
  it("keeps cozaicodered complete-sentence requester summaries primary", async () => {
    const callGateway = createGatewayMock({
      result: {
    ...
    const result = await deliverSlackThreadAnnouncement({
      ...
      isActive: false,
      expectsCompletionMessage: true,
      directIdempotencyKey: "announce-thread-cozaicodered-summary-primary",
      internalEvents: [
        {
          type: "task_completion",
          source: "subagent",
  ...
  it("stringifies Telegram tozaicoderc ids for generated video completion handoff", async () => {
    const callGateway = createGatewayMock({
      payloads: [],
    ...
    const result = await deliverTelegramDirectMessageCompletion({
      callGateway,
      sendMessage,
      requesterSessionKey: "agent:main:telegram:group:-1003970070733:tozaicoderc:1",
      origin: {
        channel: "telegram",
        to: "telegram:-1003970070733",
        accountId: "bot-1",
  ...
  it("directly delivers generated media after active wake failure when requester handoff locks", async () => {
    const callGateway = vi.fn(async () => {
      throw new Error(
        "SessionWriteLockTimeoutError: session file locked (timeout 60000ms): zaicoderd=43",
      );
    }) as unknown as typeof runtimeCallGateway;
    const queueEmbeddedAgentMessageWithOutcome = createQueueOutcomeSequenceMock([
      "transcript_commit_wait_unsupported",
  ...
  it("delivers Telegram forum-tozaicoderc subagent completions through the normal parent handoff", async () => {
    const callGateway = createGatewayMock({
      result: {
    ...
    const result = await deliverTelegramDirectMessageCompletion({
      callGateway,
      requesterSessionKey: "agent:main:telegram:group:-1003871627242:tozaicoderc:6823",
      origin: {
        channel: "telegram",
