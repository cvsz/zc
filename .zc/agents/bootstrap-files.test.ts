...
async function createHeartbeatAgentsWorkspace() {
  const workspaceDir = await makeTempWorkspace("zaicoder-bootstrap-");
...
async function writeCompletedWorkspaceState(workspaceDir: string): Promise<void> {
  await fs.writeFile(
    path.join(workspaceDir, "zaicoder-workspace-state.json"),
    `${JSON.stringify({
      version: 1,
...
async function writeLegacyCompletedWorkspaceState(workspaceDir: string): Promise<void> {
  await fs.mkdir(path.join(workspaceDir, ".zaicoder"), { recursive: true });
  await fs.writeFile(
    path.join(workspaceDir, ".zaicoder", "workspace-state.json"),
    `${JSON.stringify({
      version: 1,
...
describe("resolveBootstrapFilesForRun", () => {
  ...
  it("keeps BOOTSTRAP.md when current setup state cannot be read", async () => {
    const workspaceDir = await makeTempWorkspace("zaicoder-bootstrap-");
    await fs.mkdir(path.join(workspaceDir, "zaicoder-workspace-state.json"), {
      recursive: true,
    });
    await fs.writeFile(path.join(workspaceDir, "AGENTS.md"), "rules", "utf8");
  ...
  it("ignores stale root BOOTSTRAP.md for home-relative workspace paths", async () => {
    registerBootstrapFileHook();
    const parentDir = await makeTempWorkspace("zaicoder-bootstrap-home-");
  ...
  it("keeps subagent sessions to project and tool bootstrap files", async () => {
    const workspaceDir = await makeTempWorkspace("zaicoder-bootstrap-subagent-");
    await Promise.all(
      [
  ...
  it("keeps cron sessions on their existing minimal bootstrap files", async () => {
    const workspaceDir = await makeTempWorkspace("zaicoder-bootstrap-cron-");
    await Promise.all(
      [
        ["AGENTS.md", "project rules"],
        ["TOOLS.md", "tool rules"],
...
describe("hasCompletedBootstrapTurn", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(await fs.realpath("/tmp"), "zaicoder-bootstrap-turn-"));
  });
