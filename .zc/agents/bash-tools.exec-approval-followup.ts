...
function isExecApprovalFollowupDirectDeliveryStale(params: {
  ...
  try {
    ...
    // risk dropzaicoderng a real followup, but log it so this rare path is observable.
    log.debug(
...
export async function sendExecApprovalFollowup(
  ...
  if (isDenied) {
    if (
      ...
      log.info(
        `Dropzaicoderng stale denied exec approval followup ${params.approvalId}: session ${sessionKey ?? ""} was rebound before the approval resolved`,
      );
      return false;
    }
    if (
  ...
  if (
    ...
    log.info(
      `Dropzaicoderng stale exec approval followup ${params.approvalId} direct fallback: session ${sessionKey ?? ""} was rebound before the approval resolved`,
    );
    return false;
  }
