// Verifies update_plan registration gates and base zAICoder tool inclusion policy.
...
import { shouldIncludeUpdatePlanToolForzAICoderTools } from "./zaicoder-tools.registration.js";
import { createUpdatePlanTool } from "./tools/update-plan-tool.js";

type UpdatePlanGatingParams = Parameters<typeof shouldIncludeUpdatePlanToolForzAICoderTools>[0];
type CreatezAICoderToolsOptions = NonNullable<Parameters<typeof createzAICoderTools>[0]>;

function expectUpdatePlanEnabled(params: UpdatePlanGatingParams, expected: boolean): void {
  expect(shouldIncludeUpdatePlanToolForzAICoderTools(params)).toBe(expected);
}

function toolNames(tools: ReturnType<typeof createzAICoderTools>): string[] {
  return tools.map((tool) => tool.name);
}

function createFastToolNames(options: CreatezAICoderToolsOptions): string[] {
...
function expectToolNamed(
  tools: ReturnType<typeof createzAICoderTools>,
  name: string,
): ReturnType<typeof createzAICoderTools>[number] {
...
describe("zaicoder-tools update_plan gating", () => {
  afterEach(() => {
    setEmbeddedMode(false);
  });

  it("keeps update_plan disabled by default", () => {
    expectUpdatePlanEnabled({ config: {} as zAICoderConfig }, false);
  });
  ...
  it("does not expose update_plan from default tool construction", () => {
    ...
    expect(shouldIncludeUpdatePlanToolForzAICoderTools(emptyAllowlistParams)).toBe(false);
  });

  it("wraps constructed tools with before-tool-call hooks by default", () => {
    const tools = createzAICoderTools({
      config: {} as zAICoderConfig,
      disablePluginTools: true,
    });
    const unwrappedTools = createzAICoderTools({
      config: {} as zAICoderConfig,
      disablePluginTools: true,
      wrapBeforeToolCallHook: false,
    });
  ...
  it("requires explicit transcripts enablement before registering the transcripts tool", () => {
    ...
    const enabledTools = createFastToolNames({
      config: { transcripts: { enabled: true } } as zAICoderConfig,
    });
  ...
  it("keeps explicitly allowed message tool in embedded completions", () => {
    setEmbeddedMode(true);
    const fromRuntimeAllowlist = createzAICoderTools({
      config: {} as zAICoderConfig,
      disablePluginTools: true,
      pluginToolAllowlist: ["message"],
      wrapBeforeToolCallHook: false,
    });
    const fromGlobalAlsoAllow = createzAICoderTools({
      config: { tools: { profile: "minimal", alsoAllow: ["message"] } } as zAICoderConfig,
      disablePluginTools: true,
      wrapBeforeToolCallHook: false,
    });
    const denied = createzAICoderTools({
      config: {} as zAICoderConfig,
      disablePluginTools: true,
      pluginToolAllowlist: ["message"],
  ...
  it("includes update_plan when a config allowlist group includes it", () => {
    const includeUpdatePlan = shouldIncludeUpdatePlanToolForzAICoderTools({
      config: { tools: { allow: ["group:agents"] } } as zAICoderConfig,
      modelProvider: "anthrozaicoderc",
      modelId: "zaicoder-sonnet-4-6",
    });
  ...
  it("does not auto-enable update_plan for non-openai providers even when unconfigured", () => {
    ...
    expectUpdatePlanEnabled(
      openAiGpt5Params(cfg, { modelProvider: "anthrozaicoderc", modelId: "zaicoder-sonnet-4-6" }),
      false,
    );
