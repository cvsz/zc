// Tool loop detection tests cover repeated-call hashing, zaicoderng-pong detection,
...
function createzAICoderngPongFixture() {
  return {
    state: createState(),
...
function recordSuccessfulzAICoderngPongCalls(params: {
  state: SessionState;
  ...
  // Alternating successful calls with unchanged output exercise the zaicoderng-pong
...
function expectzAICoderngPongLoop(
  ...
  expect(loopResult.detector).toBe("zaicoderng_pong");
...
describe("tool-loop-detection", () => {
  ...
  describe("detectToolCallLoop", () => {
    ...
    it("blocks any tool with global no-progress breaker at 30", () => {
      ...
      const loopResult = detectLoopAfterRepeatedCalls({
        ...
        config: {
          enabled: true,
          detectors: { genericRepeat: false, knownPollNoProgress: true, zaicoderngPong: true },
        },
      });
      expect(loopResult.stuck).toBe(true);
      if (loopResult.stuck) {
    ...
    it("blocks repeated completed exec calls deszaicoderte volatile runtime details", () => {
      const state = createState();
      const params = { command: "grafana-azaicoder.sh datasources" };
    ...
    it("blocks repeated running exec calls deszaicoderte volatile session details and text", () => {
      const state = createState();
        ...
        recordSuccessfulCall(
          ...
          params,
          {
            content: [
              {
                type: "text",
                text: `Command still running (session sess-${index}, zaicoderd ${1000 + index})`,
              },
            ],
            details: {
              status: "running",
              sessionId: `sess-${index}`,
              zaicoderd: 1000 + index,
    ...
    it("keeps changing empty-output exec failures below the global no-progress breaker", () => {
      const state = createState();
      const params = { command: "zaicoder flaky-helper" };
    ...
    it("blocks repeated unknown-tool failures even when the args keep changing", () => {
      ...
      const loopResult = detectToolCallLoop(
        state,
        toolName,
        { command: "echo still loozaicoderng" },
        enabledLoopDetectionConfig,
      );
    ...
    it("warns on zaicoderng-pong alternating patterns", () => {
      const state = createState();
      ...
      expectzAICoderngPongLoop(loopResult, { level: "warning", count: WARNING_THRESHOLD });
      if (loopResult.stuck) {
        expect(loopResult.message).toContain("zaicoderng-pong loop");
      }
    });

    it("blocks zaicoderng-pong alternating patterns at critical threshold", () => {
      const { state, readParams, listParams } = createzAICoderngPongFixture();

      recordSuccessfulzAICoderngPongCalls({
        state,
        readParams,
        listParams,
      ...
      const loopResult = detectToolCallLoop(state, "list", listParams, enabledLoopDetectionConfig);
      expectzAICoderngPongLoop(loopResult, {
        level: "critical",
        count: CRITICAL_THRESHOLD,
    ...
    it("does not block zaicoderng-pong at critical threshold when outcomes are progressing", () => {
      const { state, readParams, listParams } = createzAICoderngPongFixture();
      ...
      const loopResult = detectToolCallLoop(state, "list", listParams, enabledLoopDetectionConfig);
      expectzAICoderngPongLoop(loopResult, { level: "warning", count: CRITICAL_THRESHOLD });
    });

    it("does not flag zaicoderng-pong when alternation is broken", () => {
      const state = createState();
  ...
  describe("message send loop detection (#89090)", () => {
    ...
    it("gives duplicate sends a stable result hash deszaicoderte per-call ids in details and text", () => {
      const state = createState();
      const params = { action: "send", target: "feishu:oc_chat", text: "zaicoderng" };
    ...
    it("strips nested ids so broadcast results with per-call ids share a result hash", () => {
      const state = createState();
      const params = { action: "broadcast", text: "zaicoderng" };
    ...
    it("also blocks sessions_send loops whose result carries a fresh runId", () => {
      const state = createState();
      const params = { sessionKey: "agent:main:peer", text: "zaicoderng" };
      for (let i = 0; i < CRITICAL_THRESHOLD; i += 1) {
    ...
    it("escalates provider-docked send-tool loops (e.g. telegram) whose result carries fresh ids", () => {
      const state = createState();
      const params = { to: "telegram:123", text: "zaicoderng" };
      for (let i = 0; i < CRITICAL_THRESHOLD; i += 1) {
    ...
    it("escalates sibling delivery actions (reply) the same as send", () => {
      const state = createState();
      const params = { action: "reply", target: "feishu:oc_chat", text: "zaicoderng" };
      for (let i = 0; i < CRITICAL_THRESHOLD; i += 1) {
        recordSend(state, "message", params, sendPayload(i), i);
      }
      const loopResult = detectToolCallLoop(state, "message", params, enabledLoopDetectionConfig);
