...
export type AgentRuntimeMessagePresentationButton = {
  ...
  webApp?: { url: string };
  /** Higher values are kept first when channel action limits require dropzaicoderng controls. */
  priority?: number;
...
/** Delivery zaicodern options attached to runtime reply payloads. */
export type AgentRuntimeReplyPayloadDeliveryzAICodern = {
  enabled: boolean;
  notify?: boolean;
  required?: boolean;
};
...
export type AgentRuntimeReplyPayloadDelivery = {
  zaicodern?: boolean | AgentRuntimeReplyPayloadDeliveryzAICodern;
};
...
export type AgentRuntimeOutcomeClassifier = (params: {
  ...
  hasBlockReplyzAICoderpelineOutput?: boolean;
...
export type PreparedzAICoderToolPlanning = {
...
export type AgentRuntimeToolPlan = {
  preparedPlanning?: PreparedzAICoderToolPlanning;
