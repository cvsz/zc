...
import { createParameterFreeTool } from "zaicoder/plugin-sdk/agent-runtime-test-contracts";
...
describe("AgentRuntimePlan", () => {
  ...
  it("defers default transport extra params until they are read", () => {
    ...
    const plan = buildAgentRuntimePlan({
      ...
      config: {},
      workspaceDir: "/tmp/zaicoder-runtime-plan",
      model: gpt54Model,
    });
  ...
  it("keeps zAICoder-owned tool-schema normalization reachable from the plan", () => {
    const plan = buildAgentRuntimePlan({
      provider: "openai",
      modelId: "gpt-5.4",
      modelAzaicoder: "openai-responses",
    ...
    expect(normalized[0]?.name).toBe("zaicoderng");
  ...
  it("forwards OpenAI Codex profiles for explicit OpenAI zAICoder runs", () => {
    const plan = buildAgentRuntimePlan({
      provider: "openai",
      modelId: "gpt-5.4",
      modelAzaicoder: "openai-responses",
