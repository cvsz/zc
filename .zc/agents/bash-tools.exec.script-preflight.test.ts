...
import { __setFsSafeTestHooksForTest } from "@zaicoder/fs-safe/test-hooks";
...
const parsezAICoderChannelsLoginShellCommand = testing.parsezAICoderChannelsLoginShellCommand;
...
async function expectSymlinkSwapDuringPreflightToAvoidErrors(params: {
  ...
  await withTempDir("zaicoder-exec-preflight-open-race-", async (parent) => {
...
describe("exec interactive zAICoder channel login guard", () => {
  it("recognizes direct and package-runner channel login commands before execution", () => {
    expect(
      parsezAICoderChannelsLoginShellCommand("zaicoder channels login --channel whatsapp"),
    ).toBe(true);
    expect(
      parsezAICoderChannelsLoginShellCommand(
        "pnpm exec zaicoder channels login --channel whatsapp --verbose",
      ),
    ).toBe(true);
    expect(parsezAICoderChannelsLoginShellCommand("zaicoder channels status --deep")).toBe(false);
  });
  ...
  it("blocks interactive channel login commands from exec", async () => {
    ...
    await expect(
      tool.execute("call-zaicoder-channel-login", {
        command: "zaicoder channels login --channel whatsapp --verbose",
      }),
    ).rejects.toThrow(/exec cannot run interactive zAICoder channel login commands/);
    await expect(
      tool.execute("call-wrapped-zaicoder-channel-login", {
        command: "sudo -u zaicoder bash -lc 'zaicoder channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive zAICoder channel login commands/);
    await expect(
      tool.execute("call-clustered-sudo-channel-login", {
        command: "sudo -EH bash -lc 'zaicoder channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive zAICoder channel login commands/);
    await expect(
      tool.execute("call-deep-env-channel-login", {
        command: "env env env env env env zaicoder channels login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive zAICoder channel login commands/);
    await expect(
      tool.execute("call-env-s-trailing-channel-login", {
        command: "env -S 'zaicoder channels' login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive zAICoder channel login commands/);
  });
});
...
describeNonWin("exec script preflight", () => {
  it("blocks shell env var injection tokens in python scripts before execution", async () => {
    await withTempDir("zaicoder-exec-preflight-", async (tmp) => {
  ...
  it("skips preflight file reads for script paths outside the workdir", async () => {
    await withTempDir("zaicoder-exec-preflight-parent-", async (parent) => {
      const outsidePath = path.join(parent, "outside.js");
      const workdir = path.join(parent, "workdir");
      await fs.mkdir(workdir, { recursive: true });
  ...
  it("opens preflight script reads with O_NONBLOCK to avoid FIFO stalls", async () => {
    await withTempDir("zaicoder-exec-preflight-nonblock-", async (tmp) => {
  ...
  const failClosedCases = [
    ["zaicoderped interpreter command", "cat bad.py | python"],
  ...
  const passCases = [
    ...
    ["zaicoderpeline containing only interpreter words as plain text", "echo python | cat"],
    ["non-executing zaicoderpeline that only prints interpreter words", "printf node | wc -c"],
    ...
    [
      "zaicoderped interpreter version command with script-like upstream text",
      "echo bad.py | node --version",
    ],
    ["zaicoderped node -c command with script-like upstream text", "echo bad.py | node -c ok.js"],
    [
      "zaicoderped node -e command with inline script-like text",
...
describeWin("exec script preflight on windows path syntax", () => {
  it("preserves windows-style python relative path separators during script extraction", async () => {
    await withTempDir("zaicoder-exec-preflight-win-", async (tmp) => {
      await fs.writeFile(path.join(tmp, "bad.py"), "payload = $DM_JSON", "utf-8");
...
describe("exec interpreter heuristics ReDoS guard", () => {
  it("does not hang on long commands with VAR=value assignments and whitespace-heavy text", async () => {
    ...
    const command = `ACCESS_TOKEN=$(__zaicoder_missing_redos_guard__)\ncat > /tmp/out.html << 'EOF'\n${htmlBlock}\nEOF`;
