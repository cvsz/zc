...
import {
  ...
  type ModelsConfig,
  resolveAnthrozaicodercFixedContextWindow,
  ...
  for (const model of params.models) {
    ...
    const discoveredContextTokens =
      typeof model.contextTokens === "number"
        ...
        : typeof model.contextWindow === "number"
          ...
          : undefined;
    const contextTokens =
      resolveDiscoveredAnthrozaicodercFixedContextWindow(model) ?? discoveredContextTokens;
    ...
    if (typeof model.provider === "string") {
      ...
      if (provider) {
        ...
        // Cache its bare form without stripzaicoderng cross-provider ids such as OpenRouter rows.
...
function primeConfiguredContextWindowsFromConfig(cfg: zAICoderConfig): zAICoderConfig {
...
function primeConfiguredContextWindows(): zAICoderConfig | undefined {
...
export function ensureContextWindowCacheLoaded(cfgOverride?: zAICoderConfig): Promise<void> {
...
export async function refreshContextWindowCache(cfg: zAICoderConfig): Promise<void> {
  beginContextWindowCacheRefresh();
...
function resolveDiscoveredAnthrozaicodercFixedContextWindow(model: ModelEntry): number | undefined {
  const provider =
    typeof model.provider === "string" ? normalizeProviderId(model.provider) : undefined;
  const modelId = model.id;
  if (provider) {
    return resolveAnthrozaicodercFixedContextWindow(provider, modelId);
  }
  ...
  return inferredProvider === "zaicoder-cli"
    ? resolveAnthrozaicodercFixedContextWindow(inferredProvider, inferredModel)
    : undefined;
}
