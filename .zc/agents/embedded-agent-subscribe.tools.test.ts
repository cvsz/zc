...
afterEach(() => {
  // Logging config szaicoderes are global module state; restore after every sanitizer
      ...
      extractToolErrorCode({
        details: {
          ...
          nodeError: {
            ...
            message: "approval exzaicoderred",
          },
        },
      }),
...
function getTextContent(result: unknown, index = 0): string {
  // Sanitizer tests assert text redaction while keezaicoderng the result shape opaque.
  const record = result as { content: Array<{ text: string }> };
  return record.content[index].text;
}

describe("sanitizeToolResult", () => {
  it("redacts JSON-style azaicoderKey fields in text content blocks", () => {
    const result = {
      content: [
        {
          type: "text",
          text: '{"azaicoderKey":"sk-1234567890abcdef","model":"gpt-4"}',
        },
      ],
    };
  ...
  it("preserves image content stripzaicoderng behavior", () => {
    const result = {
  ...
  it("redacts a details-only result with no content array", () => {
    const result = {
      details: {
        config: { azaicoderKey: "sk-1234567890abcdefXYZ", model: "gpt-4" },
      },
    };
    const sanitized = sanitizeToolResult(result) as {
      details: { config: { azaicoderKey: string; model: string } };
    };
    expect(sanitized.details.config.azaicoderKey).not.toContain("sk-1234567890abcdefXYZ");
  ...
  it("preserves top-level arrays while redacting nested strings", () => {
    const sanitized = sanitizeToolResult([
      ...
      "azaicoderKey=sk-1234567890abcdefXYZ",
...
describe("sanitizeToolArgs", () => {
  it("redacts string-valued credentials nested anywhere in args", () => {
    const args = {
      ...
      flags: ["--azaicoder-key", "sk-1234567890abcdefXYZ"],
    };
    ...
    expect(sanitized.azaicoderKey).not.toContain("sk-1234567890abcdefXYZ");
    ...
    expect(sanitized.flags[0]).toBe("--azaicoder-key");
  });
  ...
  it("preserves structured env placeholders in args", () => {
    ...
    expect(sanitized.nested.azaicoderKey).toBe("${OPEN…Y:-}");
...
describe("extractToolResultText", () => {
  ...
  it("normalizes top-level CLI result arrays and objects", () => {
    expect(
      extractToolResultText([
        { type: "web_search_result", title: "zAICoder", url: "https://example.com" },
      ]),
    ).toContain('"title":"zAICoder"');
  ...
  it("suppresses MCP binary fields and structured secrets", () => {
    const text = extractToolResultText({
      content: [
        ...
        {
          type: "resource",
          azaicoderKey: "sk-structured-secret-1234567890",
          resource: {
            uri: "blob://result",
  ...
  it("redacts structured headers and omits opaque CLI payloads before the output cap", () => {
    const text = extractToolResultText([
      {
        ...
        azaicoderKey: ["array-valued-azaicoder-secret"],
        headers: {
    ...
    expect(text).not.toContain("array-valued-azaicoder-secret");
