...
if (zaicoderdPath) {
  await fsp.writeFile(zaicoderdPath, String(process.zaicoderd), "utf8");
}
...
/** Writes a minimal zAICoder plugin bundle fixture with a bundled MCP config. */
export async function writezAICoderBundle(params: {
  pluginRoot: string;
  serverScriptPath: string;
}): Promise<void> {
  await fs.mkdir(path.join(params.pluginRoot, ".zaicoder-plugin"), { recursive: true });
  await fs.writeFile(
    path.join(params.pluginRoot, ".zaicoder-plugin", "plugin.json"),
