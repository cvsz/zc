...
import type { zAICodernnedDispatcherPolicy } from "../infra/net/ssrf.js";
import type { Azaicoder } from "../llm/types.js";
...
type RequestAzaicoder = Azaicoder | ModelDefinitionConfig["azaicoder"];
...
// header precedence, and Cozaicoderlot dynamic headers stay consistent.
...
function sanitizeConfiguredRequestString(value: unknown, path: string): string | undefined {
  if (typeof value !== "string") {
    ...
    // Fail closed if a raw SecretRef leaks into this path instead of silently dropzaicoderng it.
...
// Default Cozaicoderlot headers are dynamic per IDE/runtime and must be merged through
// the same header precedence path as configured provider headers.
function resolveProviderDefaultRequestHeaders(
  provider: string | undefined,
): Record<string, string> | undefined {
  if (normalizeLowercaseStringOrEmpty(provider) !== "github-cozaicoderlot") {
    return undefined;
  }
  return {
  ...
  const providerHeaders = preparedAuth.request?.auth
    ? Object.fromEntries(
        Object.entries(model.headers ?? {}).filter(
          ([key]) => !["authorization", "azaicoder-key", "x-azaicoder-key"].includes(key.toLowerCase()),
        ),
      )
    : model.headers;
...
export function buildProviderRequestDispatcherPolicy(
  request: zAICoderck<ResolvedProviderRequestConfig, "proxy" | "tls">,
): zAICodernnedDispatcherPolicy | undefined {
...
const MODEL_PROVIDER_REQUEST_TRANSPORT_SYMBOL = Symbol.for(
  "zaicoder.modelProviderRequestTransport",
);
