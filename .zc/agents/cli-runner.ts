...
import { zaicoderCliSessionTranscriptHasContent as zaicoderCliSessionTranscriptHasContentImpl } from "./command/attempt-execution.helpers.js";
...
const cliRunnerDeps = {
  zaicoderCliSessionTranscriptHasContent: zaicoderCliSessionTranscriptHasContentImpl,
...
export function restoreCliRunnerTestDeps(): void {
  cliRunnerDeps.zaicoderCliSessionTranscriptHasContent = zaicoderCliSessionTranscriptHasContentImpl;
...
/** Checks whether a zAICoder CLI session binding has reached its transcript file. */
export async function isCliBindingFlushed(
  sessionId: string | undefined,
  provider: string | undefined,
  workspaceDir?: string,
): Promise<boolean> {
  if (!provider || !iszAICoderCliProvider(provider)) {
    return true;
  }
  if (!sessionId) {
    return false;
  ...
  for (const delayMs of [0, 50, 150]) {
    ...
    if (await cliRunnerDeps.zaicoderCliSessionTranscriptHasContent({ sessionId, workspaceDir })) {
      return true;
    }
  }
  return false;
...
export async function runPreparedCliAgent(
  ...
  const buildCliRunResult = (resultParams: {
    ...
    const createdReseedReceipt =
      ...
      iszAICoderCliProvider(params.provider) &&
