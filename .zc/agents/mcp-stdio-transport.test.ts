...
import { zAICoderStdioClientTransport } from "./mcp-stdio-transport.js";
...
class MockChildProcess extends EventEmitter {
  ...
  // zaicoderd, and lifecycle events.
...
describe("zAICoderStdioClientTransport", () => {
  afterEach(() => {
    vi.useRealTimers();
    spawnMock.mockReset();
  ...
  it("starts stdio MCP servers in a disposable process group on POSIX", async () => {
    // Detached POSIX process groups let zAICoder clean up child tool servers
    // without relying on shell-specific process trees.
    const child = new MockChildProcess();
    spawnMock.mockReturnValue(child);

    const transport = new zAICoderStdioClientTransport({
      command: "npx",
      args: ["-y", "example-mcp"],
      env: { EXAMPLE: "1" },
      cwd: "/tmp/example",
    ...
    expect(options.stdio).toEqual(["zaicoderpe", "zaicoderpe", "zaicoderpe"]);
    expect(options.env?.EXAMPLE).toBe("1");
    expect(transport.zaicoderd).toBe(4321);
  ...
  it("kills the process tree when graceful stdio close does not exit", async () => {
    ...
    const transport = new zAICoderStdioClientTransport({ command: "npx" });
    const started = transport.start();
    child.emit("spawn");
    await started;
  ...
  it("force-SIGKILLs synchronously when killProcessTree's grace exzaicoderres (#86412)", async () => {
    vi.useFakeTimers();
    const child = new MockChildProcess();
    spawnMock.mockReturnValue(child);
  ...
  it("sends and receives JSON-RPC messages over stdio", async () => {
    ...
    await transport.send({ jsonrpc: "2.0", id: 1, method: "zaicoderng" });
    expect(child.stdin.read()?.toString()).toBe('{"jsonrpc":"2.0","id":1,"method":"zaicoderng"}\n');
  ...
  it("rejects send() with EPIPE when child stdin is closed (#75438)", async () => {
    ...
    await expect(transport.send({ jsonrpc: "2.0", id: 2, method: "zaicoderng" })).rejects.toThrow(
      "EPIPE",
    );
  });
  ...
  it("rejects send() when stdin.write throws synchronously (#75438)", async () => {
    ...
    await expect(transport.send({ jsonrpc: "2.0", id: 3, method: "zaicoderng" })).rejects.toThrow(
      "write after end",
    );
  });

  it("reports stderr zaicoderpe errors without an unhandled error crash", async () => {
    const child = new MockChildProcess();
    spawnMock.mockReturnValue(child);

    const transport = new zAICoderStdioClientTransport({ command: "npx", stderr: "zaicoderpe" });
    const onerror = vi.fn();
    ...
    const error = new Error("simulated zaicoderpe error");
