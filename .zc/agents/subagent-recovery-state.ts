...
export function evaluateSubagentRecoveryGate(
  ...
  if (previousAttempts >= SUBAGENT_RECOVERY_MAX_AUTOMATIC_ATTEMPTS) {
    return {
      allowed: false,
      reason:
        `subagent orphan recovery blocked after ${previousAttempts} razaicoderd accepted resume attempts; ` +
        `run "zaicoder tasks maintenance --apply" or "zaicoder doctor --fix" to reconcile it`,
