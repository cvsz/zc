...
const sessionStore = vi.hoisted(() => ({
  ...
  "agent:main:subagent:exzaicoderred-child": { sessionId: "sess-exzaicoderred", updatedAt: 1 },
...
describe("announce loop guard (#18264)", () => {
  ...
  test.each([
    {
      name: "exzaicoderred entries with high retry count are skipped by resumeSubagentRun",
      createEntry: (now: number) => ({
        // Ended 10 minutes ago (well past ANNOUNCE_EXPIRY_MS of 5 min).
        runId: "test-exzaicoderred-loop",
        childSessionKey: "agent:main:subagent:exzaicoderred-child",
        requesterSessionKey: "agent:main:main",
        requesterDisplayKey: "agent:main:main",
        task: "exzaicoderred test task",
    ...
    // Initialization attempts one resume, then relies on exzaicoderry/retry-budget
  ...
  test("exzaicoderred completion-message entries are still resumed for announce", async () => {
    mocks.runSubagentAnnounceFlow.mockReset();
    mocks.runSubagentAnnounceFlow.mockResolvedValueOnce(true);
    registry.resetSubagentRegistryForTests();

    const now = Date.now();
    const runId = "test-exzaicoderred-completion-message";
