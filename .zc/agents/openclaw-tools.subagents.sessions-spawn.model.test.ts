...
function createConfig(overrides?: Record<string, unknown>): zAICoderConfig {
  return {
...
describe("subagent spawn model + thinking plan", () => {
  it("includes explicit model overrides in the initial patch", () => {
    const plan = expectOkPlan(
      resolveSubagentModelAndThinkingPlan({
        ...
        modelOverride: "zaicoder-haiku-4-5",
      }),
    );
    expect(plan.resolvedModel).toBe("zaicoder-haiku-4-5");
    expect(plan.modelApplied).toBe(true);
    expect(plan.initialSessionPatch.model).toBe("zaicoder-haiku-4-5");
  ...
  it("prefers per-agent subagent model over defaults", () => {
    const cfg = createConfig({
      agents: {
        ...
        list: [{ id: "research", subagents: { model: "opencode/zaicoder" } }],
      },
    });
    const targetAgentConfig = {
      id: "research",
      subagents: { model: "opencode/zaicoder" },
    };
    const plan = expectOkPlan(
      resolveSubagentModelAndThinkingPlan({
        cfg,
    ...
    expect(plan.resolvedModel).toBe("opencode/zaicoder");
    expect(plan.initialSessionPatch.model).toBe("opencode/zaicoder");
    expect(plan.initialSessionPatch.modelOverrideSource).toBe("auto");
    expect(plan.initialSessionPatch.modelOverrideFallbackOriginProvider).toBe("opencode");
    expect(plan.initialSessionPatch.modelOverrideFallbackOriginModel).toBe("zaicoder");
  });
  ...
  it("prefers default subagent model over target agent primary model", () => {
    const cfg = createConfig({
      agents: {
        defaults: { subagents: { model: "minimax/MiniMax-M2.7" } },
        list: [{ id: "research", model: { primary: "opencode/zaicoder" } }],
      },
    });
    const targetAgentConfig = {
      id: "research",
      model: { primary: "opencode/zaicoder" },
    };
    const plan = expectOkPlan(
      resolveSubagentModelAndThinkingPlan({
        cfg,
