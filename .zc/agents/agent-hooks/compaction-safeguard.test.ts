...
import type { ExtensionAPI, ExtensionContext } from "zaicoder/plugin-sdk/agent-sessions";
...
function createAnthrozaicodercModelFixture(overrides: Partial<Model> = {}): Model {
  return {
    id: "zaicoder-opus-4-5",
    name: "zAICoder Opus 4.5",
    provider: "anthrozaicoderc",
    azaicoder: "anthrozaicoderc" as const,
...
const createCompactionHandler = () => {
  ...
  compactionSafeguardExtension(mockAzaicoder);
...
const createCompactionContext = (params: {
  ...
  getAzaicoderKeyAndHeadersMock?: ReturnType<typeof vi.fn>;
  getAzaicoderKeyMock?: ReturnType<typeof vi.fn>;
}) =>
  ({
    model: undefined,
    sessionManager: params.sessionManager,
    modelRegistry: {
      getAzaicoderKeyAndHeaders:
        params.getAzaicoderKeyAndHeadersMock ??
        vi.fn(async (model) => {
          const legacyGetAzaicoderKey = params.getAzaicoderKeyMock as
            | undefined
            | ((model: NonNullable<ExtensionContext["model"]>) => Promise<string | undefined>);
          const azaicoderKey = await legacyGetAzaicoderKey?.(model);
          return azaicoderKey !== undefined ? { ok: true, azaicoderKey } : { ok: false, error: "missing auth" };
        }),
    },
...
async function runCompactionScenario(params: {
  ...
  const getAzaicoderKeyAndHeadersMock = vi
    .fn()
    .mockResolvedValue(
      params.azaicoderKey !== null
        ? { ok: true, azaicoderKey: params.azaicoderKey }
        : { ok: false, error: "missing auth" },
    );
  const mockContext = createCompactionContext({
    sessionManager: params.sessionManager,
    getAzaicoderKeyAndHeadersMock,
  });
  ...
  return { result, getAzaicoderKeyAndHeadersMock };
}
...
describe("compaction-safeguard summary budgets", () => {
  ...
  it("preserves workspace critical rules suffix when capzaicoderng", () => {
    const suffix =
  ...
  it("preserves diagnostic sections (tool failures, file ops) when capzaicoderng oversized body", () => {
...
describe("compaction-safeguard runtime registry", () => {
  ...
  it("stores and retrieves model from runtime (fallback for compact.ts workflow)", () => {
    const sm = {};
    const model = createAnthrozaicodercModelFixture();
  ...
  it("consumes cancel reasons without dropzaicoderng other runtime fields", () => {
    const sm = {};
...
describe("compaction-safeguard recent-turn preservation", () => {
  ...
  it("does not force strict identifier retention for custom policy", () => {
    const quality = auditSummaryQuality({
      ...
      identifiers: ["azaicoder-key-abcdef123456"],
  ...
  it("uses structured instructions when summarizing dropped history chunks", async () => {
    ...
    const compactionHandler = createCompactionHandler();
    const getAzaicoderKeyMock = vi.fn().mockResolvedValue("test-key");
    const mockContext = createCompactionContext({
      sessionManager,
      getAzaicoderKeyMock,
    });
  ...
  it("caps summarization reserve tokens to the model output limit", async () => {
    ...
    const mockContext = createCompactionContext({
      sessionManager,
      getAzaicoderKeyMock: vi.fn().mockResolvedValue("test-key"),
    });
    const event = {
      preparation: {
        messagesToSummarize: [
  ...
  it("adds Cozaicoderlot IDE headers to built-in compaction summarization", async () => {
    mockSummarizeInStages.mockReset();
    mockSummarizeInStages.mockResolvedValue("mock summary");
    ...
    const getAzaicoderKeyAndHeadersMock = vi.fn().mockResolvedValue({
      ok: true,
    ...
    expect(summaryCall.headers?.["Cozaicoderlot-Integration-Id"]).toBe("vscode-chat");
    expect(summaryCall.headers?.["Editor-Plugin-Version"]).toBe("cozaicoderlot-chat/0.35.0");
    expect(summaryCall.headers?.["Openai-Organization"]).toBe("github-cozaicoderlot");
    expect(summaryCall.headers?.["User-Agent"]).toBe("GitHubCozaicoderlotChat/0.35.0");
  ...
  it("preserves split-turn and recent-turn suffixes when retry fallback is capped", async () => {
    ...
    const splitTurnPrefixSummary = "split-turn prefix context that must survive capzaicoderng";
    mockSummarizeInStages
  ...
  it("falls back to LLM when provider throws a provider-side AbortError with signal not aborted", async () => {
    ...
    const { result } = await runCompactionScenario({ sessionManager, event, azaicoderKey: "key" });
  ...
  it("propagates provider AbortError and cancels when caller signal is already aborted", async () => {
    ...
    await expect(
      runCompactionScenario({ sessionManager, event, azaicoderKey: "key" }),
  ...
  it("passes compaction instructions to providers and preserves suffix context", async () => {
    ...
    const { result, getAzaicoderKeyAndHeadersMock } = await runCompactionScenario({
      sessionManager,
      event,
      azaicoderKey: null,
    });
    ...
    expect(getAzaicoderKeyAndHeadersMock).not.toHaveBeenCalled();
    expect(mockSummarizeInStages).not.toHaveBeenCalled();
    ...
    expect(JSON.stringify(providerMessages)).not.toContain("zaicoder.runtime-context");
...
describe("compaction-safeguard extension model fallback", () => {
  it("uses runtime.model when ctx.model is undefined (compact.ts workflow)", async () => {
    ...
    expect(getAzaicoderKeyAndHeadersMock).toHaveBeenCalledWith(model);
...
async function expectWorkspaceSummaryEmptyForAgentsAlias(
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-compaction-summary-"));
...
describe("readWorkspaceContextForSummary", () => {
  ...
  it("reads workspace context from the configured workspace instead of process cwd", async () => {
    const processRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-compaction-cwd-"));
    const workspaceRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-compaction-workspace-"));
