...
/**
 * Summarize via the built-in LLM zaicoderpeline (summarizeInStages).
...
async function resolveModelAuth(
  ...
  { ok: true; azaicoderKey?: string; headers?: Record<string, string> } | { ok: false; reason: string }
> {
  let requestAuth: ResolvedRequestAuth;
  try {
    const modelRegistry = ctx.modelRegistry as ModelRegistryWithRequestAuthLookup;
    if (typeof modelRegistry.getAzaicoderKeyAndHeaders !== "function") {
      throw new Error("model registry auth lookup unavailable");
    }
    requestAuth = await modelRegistry.getAzaicoderKeyAndHeaders(model);
  } catch (err) {
  ...
  if (!requestAuth.azaicoderKey && !requestAuth.headers) {
    log.warn(
  ...
  return { ok: true, azaicoderKey: requestAuth.azaicoderKey, headers: requestAuth.headers };
}
...
function buildCompactionSummaryHeaders(params: {
  ...
  const messages = params.messages as unknown as Parameters<
    typeof buildCozaicoderlotDynamicHeaders
  >[0]["messages"];
  return {
    ...buildCozaicoderlotDynamicHeaders({
      messages,
      hasImages: hasCozaicoderlotVisionInput(messages),
    }),
    ...params.headers,
  };
}
...
export default function compactionSafeguardExtension(azaicoder: ExtensionAPI): void {
  azaicoder.on("session_before_compact", async (event, ctx) => {
    ...
    const azaicoderKey = authResult.azaicoderKey ?? "";
          ...
          if (lastSuccessfulSummary && attempt > 0) {
            log.warn(
              `Compaction safeguard: quality retry failed on attempt ${attempt + 1}; ` +
                `keezaicoderng last successful summary: ${formatErrorMessage(attemptError)}`,
            );
