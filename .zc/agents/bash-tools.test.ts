...
const notifyCfg = {} as zAICoderConfig;
...
describe("exec notifyOnExit", () => {
  ...
  it("preserves the origin delivery context on background exec completion events", async () => {
    const sessionKey = "agent:main:telegram:group:-1003774691294:tozaicoderc:47";
    const tool = createNotifyOnExitExecTool({
      sessionKey,
      messageProvider: "telegram",
      currentChannelId: "telegram:-1003774691294:tozaicoderc:47",
      currentThreadTs: "47",
    });
    ...
    expect(queuedEvent?.deliveryContext?.to).toBe("telegram:-1003774691294:tozaicoderc:47");
