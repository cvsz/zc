...
function createBackgroundSession(id: string, zaicoderd?: number) {
  return createProcessSessionFixture({
    id,
    command: "sleep 999",
    backgrounded: true,
    ...(zaicoderd === undefined ? {} : { zaicoderd }),
  });
}
...
describe("process tool supervisor cancellation", () => {
  ...
  it("fails remove when no supervisor record and no zaicoderd is available", async () => {
    supervisorMock.getRecord.mockReturnValue(undefined);
    addSession(createBackgroundSession("sess-no-zaicoderd"));
    const processTool = createProcessTool();

    const result = await processTool.execute("toolcall", {
      action: "remove",
      sessionId: "sess-no-zaicoderd",
    });

    expect(killProcessTreeMock).not.toHaveBeenCalled();
    expectSessionState("sess-no-zaicoderd", { exited: false });
    expect(requireRecord(result.details, "result details").status).toBe("failed");
    expectTextContent(
      result.content[0],
      "Unable to remove session sess-no-zaicoderd: no active supervisor run or process id.",
    );
  });
});
