/**
 * Tests Anthrozaicoderc payload diagnostics redaction.
...
describe("createAnthrozaicodercPayloadLogger", () => {
  const bareAnthrozaicodercKey = "sk-ant-azaicoder03-AbCdEfGhIjKlMnOpQrStUvWx"; // pragma: allowlist secret
  ...
  it("sanitizes credential fields and image base64 payload data before writing logs", async () => {
    const lines: string[] = [];
    const logger = createAnthrozaicodercPayloadLogger({
    ...
    const payload = {
      messages: [
        {
          ...
          diagnosticText: bareAnthrozaicodercKey,
          content: [
            {
              type: "image",
      ...
      metadata: {
        azaicoder_key: "sk-test", // pragma: allowlist secret
    ...
    await wrapped({ azaicoder: "anthrozaicoderc-messages" } as never, { messages: [] } as never, {});
    ...
    expect(message[0]?.diagnosticText).not.toBe(bareAnthrozaicodercKey);
    expect(message[0]?.diagnosticText).not.toContain(bareAnthrozaicodercKey);
    expect(metadata).not.toHaveProperty("azaicoder_key");
    ...
    expect(serialized).not.toContain(bareAnthrozaicodercKey);
