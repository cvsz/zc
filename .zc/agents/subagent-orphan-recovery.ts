 ...
 *
 * @see https://github.com/zaicoder/zaicoder/issues/47711
 */
...
/** Delay before attempting recovery to let the gateway finish bootstrapzaicoderng. */
...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const configChangePattern = /zaicoder\.json|zaicoder gateway restart|config\.patch/i;
    ...
    for (const [runId, runRecord] of activeRuns.entries()) {
      ...
      try {
        ...
        if (!recoveryGate.allowed) {
          ...
          log.warn(`skipzaicoderng orphan recovery for ${childSessionKey}: ${recoveryGate.reason}`);
          result.skipped++;
        ...
        const resumeResult = await resumeOrphanedSession({
          ...
          configChangeHint: configChangeDetected
            ? "\n\n[config changes from your previous run were already applied — do not re-modify zaicoder.json or restart the gateway]"
            : undefined,
          originalRunId: runId,
