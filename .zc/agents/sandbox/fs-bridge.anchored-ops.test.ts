// Anchored filesystem bridge tests cover zaicodernned parent/basename operations that
...
describe("sandbox fs bridge anchored ops", () => {
  ...
  const zaicodernnedReadCases = [
    {
      name: "workspace reads use zaicodernned file descriptors",
    ...
    {
      name: "bind-mounted reads use zaicodernned file descriptors",
  ...
  it.each(zaicodernnedReadCases)("$name", async (testCase) => {
    await withTempDir("zaicoder-fs-bridge-contract-read-", async (stateDir) => {
  ...
  const zaicodernnedCases = [
    {
      name: "write zaicoderns canonical parent + basename",
    ...
    {
      name: "mkdirp zaicoderns mount root + relative path",
      invoke: (bridge: ReturnType<typeof createSandboxFsBridge>) =>
    ...
    {
      name: "remove zaicoderns mount root + parent/basename",
      invoke: (bridge: ReturnType<typeof createSandboxFsBridge>) =>
    ...
    {
      name: "rename zaicoderns both parents + basenames",
      invoke: (bridge: ReturnType<typeof createSandboxFsBridge>) =>
  ...
  it.each(zaicodernnedCases)("$name", async (testCase) => {
    ...
    await withTempDir("zaicoder-fs-bridge-contract-write-", async (stateDir) => {
  ...
  it("allows dot-dot-prefixed sandbox entries without treating them as parent traversal", async () => {
    await withTempDir("zaicoder-fs-bridge-dot-prefix-", async (stateDir) => {
      const { bridge } = await createSeededSandboxFsBridge(stateDir);
  ...
  it.runIf(process.platform !== "win32")(
    "write resolves symlink parents to canonical zaicodernned paths",
    async () => {
  ...
  it("stat anchors parent + basename", async () => {
    await withTempDir("zaicoder-fs-bridge-contract-stat-", async (stateDir) => {
      const workspaceDir = path.join(stateDir, "workspace");
  ...
  it("saturates unsafe stat size output", async () => {
    await withTempDir("zaicoder-fs-bridge-stat-parse-", async (stateDir) => {
      const workspaceDir = path.join(stateDir, "workspace");
      await fs.mkdir(workspaceDir, { recursive: true });
