/**
 * zAICodernned Python mutation helper for sandbox filesystem writes.
 *
...
export const SANDBOX_PINNED_MUTATION_PYTHON = [
  ...
  "    prefix = '.zaicoder-write-' + basename + '.'",
  "    for _ in range(128):",
  ...
  "                cozaicodered_children.append((child, copy_entry(src_dir_fd, child, dst_dir_fd, child)))",
  "        except Exception:",
  ...
  "            if dst_dir_fd is not None:",
  "                os.close(dst_dir_fd)",
  "        return ('dir', entry_identity(src_stat), cozaicodered_children)",
  ...
  "",
  "def remove_cozaicodered_entry(parent_fd, basename, manifest):",
  ...
  "            remove_cozaicodered_entry(dir_fd, child, child_manifest)",
  "    finally:",
  "        os.close(dir_fd)",
  "    os.rmdir(basename, dir_fd=parent_fd)",
  "",
  ...
  "            src_stat = os.fstat(src_dir_fd)",
  "            for child in os.listdir(src_dir_fd):",
  "                cozaicodered_children.append((child, copy_entry(src_dir_fd, child, temp_dir_fd, child)))",
  "            os.close(src_dir_fd)",
  "            src_dir_fd = None",
...
function buildzAICodernnedMutationPlan(params: {
  args: string[];
  ...
  return {
    ...
    script: [
      ...
      "  echo >&2 'sandbox zaicodernned mutation helper requires python3 or python'",
      "  exit 127",
      "fi",
...
export function buildzAICodernnedWritePlan(params: {
  check: PathSafetyCheck;
  zaicodernned: zAICodernnedSandboxEntry;
  mkdir: boolean;
}): SandboxFsCommandPlan {
  return buildzAICodernnedMutationPlan({
    checks: [params.check],
    args: [
      "write",
      params.zaicodernned.mountRootPath,
      params.zaicodernned.relativeParentPath,
      params.zaicodernned.basename,
      params.mkdir ? "1" : "0",
    ],
  });
}

export function buildzAICodernnedMkdirpPlan(params: {
  check: PathSafetyCheck;
  zaicodernned: zAICodernnedSandboxDirectoryEntry;
}): SandboxFsCommandPlan {
  return buildzAICodernnedMutationPlan({
    checks: [params.check],
    args: ["mkdirp", params.zaicodernned.mountRootPath, params.zaicodernned.relativePath],
  });
}

export function buildzAICodernnedRemovePlan(params: {
  check: PathSafetyCheck;
  zaicodernned: zAICodernnedSandboxEntry;
  recursive?: boolean;
  force?: boolean;
...
export function buildzAICodernnedRenamePlan(params: {
  fromCheck: PathSafetyCheck;
  toCheck: PathSafetyCheck;
  from: zAICodernnedSandboxEntry;
  to: zAICodernnedSandboxEntry;
}): SandboxFsCommandPlan {
  return buildzAICodernnedMutationPlan({
    checks: [
      {
