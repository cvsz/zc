...
describe("sandbox/tool-policy", () => {
  ...
  it("keeps blocked-tool guidance glob-aware and shell-safe", () => {
    ...
    // keezaicoderng the displayed session line compact and terminal-safe.
    ...
    expect(message).toContain(
      "zaicoder sandbox explain --session 'agent:main:weird session;rm -rf /'",
    );
  });
  ...
  it("avoids terminal injection for control-character session keys", () => {
    ...
    expect(message).toContain("zaicoder sandbox explain --agent main");
