...
// canonicalization, bind reads, and zaicodernned mutation helpers.
...
describe("sandbox fs bridge shell compatibility", () => {
  ...
  it("uses POSIX-safe shell prologue in all bridge commands", async () => {
    await withTempDir("zaicoder-fs-bridge-shell-", async (stateDir) => {
      ...
      expectNoScriptsContaining(scripts, "zaicoderpefail");
    });
  });
  ...
  it("reads inbound media-style filenames with triple-dash ids", async () => {
    await withTempDir("zaicoder-fs-bridge-read-", async (stateDir) => {
      const workspaceDir = path.join(stateDir, "workspace");
  ...
  it("resolves bind-mounted absolute container paths for reads", async () => {
    await withTempDir("zaicoder-fs-bridge-bind-read-", async (stateDir) => {
      const workspaceDir = path.join(stateDir, "workspace");
  ...
  it("routes mkdirp, remove, and rename through the zaicodernned mutation helper", async () => {
    await withTempDir("zaicoder-fs-bridge-shell-write-", async (stateDir) => {
  ...
  it("re-validates target before the zaicodernned write helper runs", async () => {
    mockedOpenRootFile
