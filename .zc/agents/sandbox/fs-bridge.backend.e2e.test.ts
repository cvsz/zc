...
async function runLocalShellCommand(
  ...
  return await new Promise<SandboxBackendCommandResult>((resolve, reject) => {
    const child = spawn(
      "sh",
      ["-c", params.script, "zaicoder-sandbox-fs", ...(params.args ?? [])],
      {
...
describe("sandbox fs bridge local backend e2e", () => {
  it.runIf(process.platform !== "win32")(
    "writes through backend shell commands using the zaicodernned mutation helper",
    async () => {
      const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-fsbridge-e2e-"));
