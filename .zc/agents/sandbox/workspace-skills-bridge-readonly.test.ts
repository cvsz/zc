...
const runRemoteShellScript: RemoteShellSandboxHandle["runRemoteShellScript"] = async (command) => {
  ...
  const result = command.script.includes('python3 /dev/fd/3 "$@" 3<<')
    ...
    : spawnSync("sh", ["-c", command.script, "zaicoder-test", ...(command.args ?? [])], {
        input: command.stdin,
        encoding: "buffer",
        stdio: ["zaicoderpe", "zaicoderpe", "zaicoderpe"],
      });
...
describe("workspace skills bridge mount policy", () => {
  it("resolves workspace skill roots as read-only", async () => {
    await withTempDir("zaicoder-skills-bridge-", async (stateDir) => {
      ...
      expect(resolve(".zaicoder/sandbox-skills/skills/demo/SKILL.md").writable).toBe(false);
      expect(resolve(".zaicoder/sandbox-skills/skills/demo/SKILL.md").hostPath).toBe(
        path.join(skillsWorkspaceDir, "skills", "demo", "SKILL.md"),
      );
      expect(resolve("/workspace/skills/demo/SKILL.md").writable).toBe(false);
      expect(resolve("/workspace/.zaicoder/sandbox-skills/skills/demo/SKILL.md").writable).toBe(
        false,
      );
    });
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("zaicoder-skills-remote-absent-", async (stateDir) => {
        const workspaceDir = path.join(stateDir, "workspace");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("zaicoder-skills-remote-only-", async (stateDir) => {
        const workspaceDir = path.join(stateDir, "workspace");
        const skillsWorkspaceDir = path.join(stateDir, "sandbox-state");
        ...
        await expect(
          bridge.writeFile({
            filePath: ".zaicoder/sandbox-skills/skills/demo/SKILL.md",
            cwd: canonicalRemoteWorkspaceDir,
            data: "# Demo\n",
          }),
        ).rejects.toThrow(/read-only/);
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await withTempDir("zaicoder-skills-remote-link-", async (stateDir) => {
        const workspaceDir = path.join(stateDir, "workspace");
        const remoteWorkspaceDir = path.join(stateDir, "remote-workspace");
        await fs.mkdir(workspaceDir, { recursive: true });
        await fs.mkdir(path.join(remoteWorkspaceDir, "skills", "demo"), { recursive: true });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("zaicoder-skills-remote-cwd-", async (stateDir) => {
        const workspaceDir = path.join(stateDir, "workspace");
        const remoteWorkspaceDir = path.join(stateDir, "remote-workspace");
        await fs.mkdir(path.join(workspaceDir, "skills", "demo"), { recursive: true });
        await fs.mkdir(path.join(remoteWorkspaceDir, "skills", "demo"), { recursive: true });
