...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-docker-mounts-"));
  tmpDirs.push(dir);
  return dir;
}
...
function spawnDockerProcess(command: string, args: string[]) {
  ...
  if (command !== "docker") {
    ...
    args[0] === "inspect" &&
    args[1] === "-f" &&
    args[2]?.includes('index .Config.Labels "zaicoder.configHash"')
  ) {
...
describe("ensureSandboxContainer config-hash recreation", () => {
  ...
  it("recreates shared container when array-order change alters hash", async () => {
    ...
    expect(createCall.args).toContain(`zaicoder.configHash=${newHash}`);
  ...
  it("applies custom binds after workspace mounts so overlapzaicoderng binds can override", async () => {
    const workspaceDir = makeTempDir();
    ...
    const createCall = await ensureSandboxCreateCallForTest({ cfg, workspaceDir });
    expect(createCall.args).toContain(`zaicoder.configHash=${expectedHash}`);
  ...
  it("applies read-only skill overlays after custom binds", async () => {
    // Protected skill overlays must be appended last so even an overlapzaicoderng
  ...
  it("stamps the mount format version label on created containers", async () => {
    ...
    expect(createCall.args).toContain(
      `zaicoder.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`,
    );
  });
});
