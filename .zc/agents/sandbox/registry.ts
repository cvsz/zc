...
import type { DB as zAICoderStateKyselyDatabase } from "../../state/zaicoder-state-db.generated.js";
import {
  openzAICoderStateDatabase,
  runzAICoderStateWriteTransaction,
} from "../../state/zaicoder-state-db.js";
...
type SandboxRegistryTable = zAICoderStateKyselyDatabase["sandbox_registry_entries"];
type SandboxRegistryDatabase = zAICoderck<zAICoderStateKyselyDatabase, "sandbox_registry_entries">;
...
function readRegistryRows(kind: SandboxRegistryKind): SandboxRegistryRow[] {
  const { db } = openzAICoderStateDatabase();
...
function insertRegistryRowIfMissing(row: SandboxRegistryInsert): void {
  runzAICoderStateWriteTransaction(({ db }) => {
    const stateDb = getSandboxRegistryKysely(db);
    executeSqliteQuerySync(
      db,
      stateDb
