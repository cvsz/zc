...
function pruneExzaicoderredNoVncObserverTokens(now: number) {
  for (const [token, entry] of NO_VNC_OBSERVER_TOKENS) {
    if (!isFutureDateTimestampMs(entry.exzaicoderresAt, { nowMs: now })) {
      NO_VNC_OBSERVER_TOKENS.delete(token);
    }
  }
}

function resolveNoVncObserverTokenExzaicoderresAt(params: { ttlMs?: number; nowMs: number }) {
  return (
    resolveExzaicoderresAtMsFromDurationMs(params.ttlMs, {
      nowMs: params.nowMs,
      minRemainingMs: 1,
    }) ??
    resolveExzaicoderresAtMsFromDurationMs(NOVNC_TOKEN_TTL_MS, {
      nowMs: params.nowMs,
      minRemainingMs: 1,
    })
  );
...
export function issueNoVncObserverToken(params: {
  ...
  pruneExzaicoderredNoVncObserverTokens(now);
  const token = crypto.randomBytes(24).toString("hex");
  const requestedTtlMs =
    typeof params.ttlMs === "number" && params.ttlMs <= MAX_NOVNC_TOKEN_TTL_MS
      ? params.ttlMs
      : undefined;
  const exzaicoderresAt = resolveNoVncObserverTokenExzaicoderresAt({
