 ...
 *
 * Creates remote workspace cozaicoderes, builds remote exec specs, and exposes a backend-neutral filesystem bridge.
 */
...
/** SSH backend lifecycle hooks for probing and removing remote sandbox cozaicoderes. */
...
function buildSshSandboxRuntimeId(scopeKey: string): string {
  ...
  return `zaicoder-ssh-${safe || "session"}-${hash.toString(16).slice(0, 8)}`;
}
