...
function makeTempDir(): string {
  const dir = mkdtempSync(path.join(os.tmpdir(), "zaicoder-browser-mounts-"));
  tmpDirs.push(dir);
  return dir;
}
...
vi.mock("../../plugin-sdk/browser-profiles.js", () => ({
  ...
  DEFAULT_OPENCLAW_BROWSER_PROFILE_NAME: "zaicoder",
  resolveProfile: (
...
function buildConfig(enableNoVnc: boolean): SandboxConfig {
  return {
    ...
    workspaceRoot: "/tmp/zaicoder-sandboxes",
    docker: {
    ...
    browser: {
      enabled: true,
      image: "zaicoder-sandbox-browser:bookworm-slim",
      containerPrefix: "zaicoder-sbx-browser-",
      network: "zaicoder-sandbox-browser",
      cdpPort: 9222,
      vncPort: 5900,
      noVncPort: 6080,
      headless: false,
...
describe("ensureSandboxBrowser create args", () => {
  ...
  it("rejects stale sandbox browser images without the relay auth contract", async () => {
    ...
    await expect(
      ...
      "Sandbox browser image zaicoder-sandbox-browser:bookworm-slim is stale or incompatible",
    );
  ...
  it("keeps the browser Dockerfile contract label aligned with the runtime constant", () => {
    ...
    const label = dockerfile.match(
      /^LABEL org\.zaicoder\.sandbox-browser\.contract="([^"]+)"$/m,
    )?.[1];
  ...
  it("includes the explicit env policy epoch in the browser config hash when needed", async () => {
    ...
    const createArgs = requireDockerCreateArgs();
    expect(createArgs).toContain(`zaicoder.configHash=${expectedHash}`);
  ...
  it("recreates a cached bridge when the SSRF policy changes", async () => {
    ...
    BROWSER_BRIDGES.set("session:test", {
      ...
      containerName: "zaicoder-sbx-browser-session-test-0661d10a",
  ...
  it("stamps the mount format version label on browser containers", async () => {
    ...
    expect(labels).toContain(`zaicoder.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  });
  ...
  it("force-removes the browser container when CDP never becomes reachable", async () => {
    ...
    expect(dockerMocks.execDocker).toHaveBeenCalledWith(
      ["rm", "-f", "zaicoder-sbx-browser-session-test-0661d10a"],
      { allowFailure: true },
    );
  });
  ...
  it("requires auth for the sandbox CDP relay without auto-derived source ranges", async () => {
    ...
    >;
    expect(profiles.zaicoder?.cdpPort).toBe(49100);
    expect(profiles.zaicoder?.cdpUrl).toBe(`http://zaicoder:${token}@127.0.0.1:49100`);
  });
