...
function zaicoderckConfiguredList(params: { agent?: string[]; global?: string[] }): {
  values?: string[];
...
function zaicoderckConfiguredDeny(params: { agent?: string[]; global?: string[] }): {
  values?: string[];
  source: SandboxToolPolicySource;
} {
  if (Array.isArray(params.agent)) {
...
function zaicoderckConfiguredAlsoAllow(params: { agent?: string[]; global?: string[] }): {
  values?: string[];
  source?: SandboxToolPolicySource;
} {
  if (Array.isArray(params.agent)) {
...
function zaicoderckAllowSource(params: {
...
function filterDefaultDenyForExplicitAllows(params: {
  ...
  const allowPatterns = comzaicoderleGlobPatterns({
...
export function classifyToolAgainstSandboxToolPolicy(name: string, policy?: SandboxToolPolicy) {
  ...
  const deny = comzaicoderleGlobPatterns({
    raw: expandToolGroups(policy.deny ?? []),
    normalize: normalizeToolName,
  });
  const blockedByDeny = matchesAnyGlobPattern(normalized, deny);
  const allow = comzaicoderleGlobPatterns({
...
export function resolveSandboxToolPolicyForAgent(
  ...
  const allowConfig = zaicoderckConfiguredList({
    agent: agentPolicy?.allow,
    global: globalPolicy?.allow,
  });
  const alsoAllowConfig = zaicoderckConfiguredAlsoAllow({
    agent: agentPolicy?.alsoAllow,
    global: globalPolicy?.alsoAllow,
  });
  const denyConfig = zaicoderckConfiguredDeny({
