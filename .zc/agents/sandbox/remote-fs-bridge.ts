...
class RemoteShellSandboxFsBridge implements SandboxFsBridge {
  ...
  async writeFile(params: {
    ...
    const zaicodernned = await this.resolvezAICodernnedParent({
      containerPath: target.containerPath,
      action: "write files",
    ...
    await this.runMutation({
      args: [
        "write",
        zaicodernned.mountRootPath,
        zaicodernned.relativeParentPath,
        zaicodernned.basename,
  ...
  async rename(params: {
    ...
    const fromzAICodernned = await this.resolvezAICodernnedParent({
    ...
    const tozAICodernned = await this.resolvezAICodernnedParent({
    ...
    await this.runMutation({
      args: [
        "rename",
        fromzAICodernned.mountRootPath,
        fromzAICodernned.relativeParentPath,
        fromzAICodernned.basename,
        tozAICodernned.mountRootPath,
        tozAICodernned.relativeParentPath,
        tozAICodernned.basename,
        "1",
      ],
      signal: params.signal,
    });
  ...
  private getRemoteProtectedSkillRoots(): string[] {
    ...
    const roots = [
      ...
      path.posix.join(workspaceContainerRoot, ".zaicoder", "sandbox-skills", "skills"),
    ];
    if (path.resolve(this.sandbox.agentWorkspaceDir) !== path.resolve(this.sandbox.workspaceDir)) {
      roots.push(
        ...
        path.posix.join(agentContainerRoot, ".zaicoder", "sandbox-skills", "skills"),
      );
    }
    return roots;
  }
  ...
  private async assertNoHardlinkedFile(params: {
    ...
    // Remote mutation helpers zaicodern by parent path. Rejecting hardlinked regular
  ...
  private async resolvezAICodernnedParent(params: {
    containerPath: string;
    action: string;
