...
describe("sandbox ssh helpers", () => {
  it("materializes inline ssh auth data into a temp config", async () => {
    ...
    expect(config).toContain("Host zaicoder-sandbox");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const realParent = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-ssh-real-"));
      ...
      const target = path.join(root, "workspace", ".zaicoder", "sandbox-skills");
      ...
      await expect(
        fs.stat(path.join(realParent, "runtime", "workspace", ".zaicoder", "sandbox-skills")),
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const realParent = makeTempDir(tempDirs, "zaicoder-ssh-real-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = makeTempDir(tempDirs, "zaicoder-ssh-workdir-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = makeTempDir(tempDirs, "zaicoder-ssh-root-");
  ...
  it("builds remote workdir validation commands with quoted literal paths", () => {
    ...
    expect(command).toContain("zaicoder-validate-workdir");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await fs.symlink(realParent, path.join(root, "workspace", ".zaicoder"));
      ...
      await expect(
        execFileAsync("/bin/sh", [
          ...
          ENSURE_REMOTE_REAL_DIRECTORY_SCRIPT,
          "zaicoder-remote-dir",
          path.join(root, "workspace", ".zaicoder", "sandbox-skills"),
          root,
        ]),
      ).rejects.toThrow(/unsafe remote directory symlink/);
    },
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const localDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-ssh-upload-"));
      ...
      await expect(
        uploadDirectoryToSshTarget({
          session: {
            command: "ssh",
            configPath: "/tmp/zaicoder-test-ssh-config",
            host: "zaicoder-sandbox",
          },
          localDir,
          remoteDir: "/remote/workspace",
        }),
      ).rejects.toThrow(/refuses symlink escazaicoderng the workspace: escape/i);
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const localDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-ssh-upload-safe-"));
      tempDirs.push(localDir);
