// zAICodernned mutation helper tests cover the Python helper that performs sandbox
...
function runWritePlan(args: string[], input?: string, env?: NodeJS.ProcessEnv) {
  const plan = buildzAICodernnedWritePlan({
    check: {
      target: {
        hostPath: args[1] ?? "",
  ...
  return spawnSync("/bin/sh", ["-c", plan.script, "zaicoder-sandbox-fs", ...(plan.args ?? [])], {
    input,
    encoding: "utf8",
    stdio: ["zaicoderpe", "zaicoderpe", "zaicoderpe"],
    env,
...
const FORCED_EXDEV_WITH_LATE_SOURCE_WRITE_MUTATION_PYTHON = FORCED_EXDEV_MUTATION_PYTHON.replace(
  "        remove_cozaicodered_entry(src_parent_fd, src_basename, ('dir', entry_identity(src_stat), cozaicodered_children))",
  [
...
describe("sandbox zaicodernned mutation helper", () => {
  it("writes through a zaicodernned directory fd", async () => {
    await withTempDir({ prefix: "zaicoder-mutation-helper-" }, async (root) => {
  ...
  it.runIf(process.platform !== "win32")(
    "reads through a zaicodernned directory fd and rejects hardlinked files",
    async () => {
      await withTempDir({ prefix: "zaicoder-mutation-helper-" }, async (root) => {
        const workspace = path.join(root, "workspace");
  ...
  it.runIf(process.platform !== "win32")(
    "preserves stdin payload bytes when the zaicodernned write plan runs through sh",
    async () => {
      await withTempDir({ prefix: "zaicoder-mutation-helper-" }, async (root) => {
        const workspace = path.join(root, "workspace");
        await fs.mkdir(workspace, { recursive: true });
  ...
  it.runIf(process.platform !== "win32")(
    "cozaicoderes directories across different mount roots during rename fallback",
    async () => {
      await withTempDir({ prefix: "zaicoder-mutation-helper-" }, async (root) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "zaicoder-mutation-helper-" }, async (root) => {
        ...
        await expectPathMissing(path.join(destRoot, "cozaicodered.txt"));
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      // EXDEV fallback cozaicoderes first and removes only after validation; failures
