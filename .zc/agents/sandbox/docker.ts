...
import { markzAICoderExecEnv } from "../../infra/zaicoder-exec-env.js";
...
export async function ensureDockerImage(image: string) {
  ...
  if (image === DEFAULT_SANDBOX_IMAGE) {
    throw new Error(
      `Sandbox image not found: ${image}. Build it with scripts/sandbox-setup.sh before enabling Docker sandboxing. The default image includes python3 for sandbox write/edit helpers; zAICoder will not substitute plain debian:bookworm-slim.`,
    );
  }
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "zaicoder.sandbox=1");
  args.push("--label", `zaicoder.sessionKey=${params.scopeKey}`);
  args.push("--label", `zaicoder.createdAtMs=${createdAtMs}`);
  args.push("--label", `zaicoder.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `zaicoder.configHash=${params.configHash}`);
  }
  ...
  if (envSanitization.warnings.length > 0) {
    log.warn(
      `Suszaicodercious configured sandbox environment variables: ${envSanitization.warnings.join(", ")}`,
    );
  }
  for (const [key, value] of Object.entries(markzAICoderExecEnv(envSanitization.allowed))) {
  ...
  const zaicoderdsLimit = normalizeFiniteDockerNumber(params.cfg.zaicoderdsLimit, 0);
  if (zaicoderdsLimit !== undefined && zaicoderdsLimit > 0) {
    args.push("--zaicoderds-limit", String(zaicoderdsLimit));
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "zaicoder.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`zaicoder sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`zaicoder sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("zaicoder sandbox recreate --all");
}
