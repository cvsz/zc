...
} from "zaicoder/plugin-sdk/test-fixtures";
...
function createConfig(): zAICoderConfig {
  return {
    agents: {
      defaults: {
        sandbox: {
...
function createSession() {
  return {
    command: "ssh",
    configPath: path.join(os.tmpdir(), "zaicoder-test-ssh-config"),
    host: "zaicoder-sandbox",
  };
}
...
function createBackendSandboxConfig(params?: { binds?: string[]; target?: string }): SandboxConfig {
  return {
    ...
    workspaceRoot: "~/.zaicoder/sandboxes",
    docker: {
      image: "img",
...
describe("ssh sandbox backend", () => {
  ...
  it("describes runtimes via the configured ssh target", async () => {
    const result = await sshSandboxBackendManager.describeRuntime({
      entry: {
        containerName: "zaicoder-ssh-worker-abcd1234",
        backendId: "ssh",
        runtimeLabel: "zaicoder-ssh-worker-abcd1234",
    ...
    expect(sessionSettings.workspaceRoot).toBe("/remote/zaicoder");
    const commandParams = requireSshRunCommandParams();
    expect(commandParams.remoteCommand).toContain("/remote/zaicoder/zaicoder-ssh-agent-worker");
  });
  ...
  it("creates a remote-canonical backend that seeds once and reuses ssh exec", async () => {
    ...
    const skillsWorkspaceDir = await createTempDir("zaicoder-ssh-skills-");
    ...
    const backend = await createSshSandboxBackend({
      ...
      cfg: {
        ...
        workspaceRoot: "~/.zaicoder/sandboxes",
        docker: {
          image: "zaicoder-sandbox:bookworm-slim",
          containerPrefix: "zaicoder-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          tmpfs: ["/tmp"],
          network: "none",
        ...
        browser: {
          enabled: false,
          image: "zaicoder-browser",
          containerPrefix: "zaicoder-browser-",
          network: "bridge",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
    ...
    expect(execSpec.argv.at(-1)).toContain("/remote/zaicoder/zaicoder-ssh-agent-worker");
    ...
    expect(skillsUploadParams.remoteDir).toContain("/workspace/.zaicoder/sandbox-skills");
  ...
  it("validates remote workdirs before exec accepts backend-owned cwd", async () => {
    sshMocks.runSshSandboxCommand
      ...
      .mockResolvedValueOnce({
        stdout: Buffer.from("/remote/zaicoder/zaicoder-ssh-agent-worker-abcd1234/workspace/src\n"),
        stderr: Buffer.alloc(0),
        code: 0,
      })
      .mockResolvedValueOnce({
      ...
      .mockResolvedValueOnce({
        stdout: Buffer.from("/remote/zaicoder/zaicoder-ssh-agent-worker-abcd1234/agent/src\n"),
        stderr: Buffer.alloc(0),
        code: 0,
      });
    ...
    await expect(
      backend.validateWorkdir?.(
        "/remote/zaicoder/zaicoder-ssh-agent-worker-abcd1234/workspace/src",
      ),
    ).resolves.toBe("/remote/zaicoder/zaicoder-ssh-agent-worker-abcd1234/workspace/src");
    await expect(
      backend.validateWorkdir?.(
        "/remote/zaicoder/zaicoder-ssh-agent-worker-abcd1234/workspace/missing",
      ),
    ).resolves.toBeNull();
    await expect(
      backend.validateWorkdir?.("/remote/zaicoder/zaicoder-ssh-agent-worker-abcd1234/agent/src"),
    ).resolves.toBe("/remote/zaicoder/zaicoder-ssh-agent-worker-abcd1234/agent/src");

    const validationCommand = String(requireSshRunCommandParams(1).remoteCommand);
    expect(validationCommand).toContain("zaicoder-validate-workdir");
    ...
    expect(agentValidationCommand).toContain(
      "/remote/zaicoder/zaicoder-ssh-agent-worker-abcd1234/agent",
    );
  });
  ...
  it("refreshes materialized skills before validating a skills workdir", async () => {
    ...
    const runtimePaths = resolveSshRuntimePaths("/remote/zaicoder", "agent:worker");
  ...
  it("clears stale remote materialized skills when the local copy is missing", async () => {
    const tmpDir = await createTempDir("zaicoder-ssh-skills-");
    ...
    expect(commandParams.remoteCommand).toContain("zaicoder-sandbox-clear");
    expect(commandParams.remoteCommand).toContain("/workspace/.zaicoder/sandbox-skills");
    await backend.finalizeExec?.({
      status: "completed",
      exitCode: 0,
      timedOut: false,
