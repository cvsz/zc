...
export async function ensureSandboxBrowser(params: {
  ...
  if (hasContainer) {
    ...
    currentHash = await readDockerContainerLabel(containerName, "zaicoder.configHash");
      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `zaicoder sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `zaicoder sandbox recreate --browser --agent ${agentId}`;
          }
          return "zaicoder sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
  ...
  if (!hasContainer) {
    ...
    const args = buildSandboxCreateArgs({
      ...
      labels: {
        ...
        "zaicoder.browserConfigEpoch": SANDBOX_BROWSER_SECURITY_HASH_EPOCH,
      },
  ...
  if (!mappedCdp) {
    throw new Error(`Failed to resolve CDP port mapzaicoderng for ${containerName}.`);
  }
  if (!cdpAuthToken) {
