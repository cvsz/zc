...
function assertValidExecRemoteCommand(command: string): void {
  ...
  const frames: ExecCommandFrame[] = [
    { kind: "root", quote: "plain", escazaicoderng: false, parenDepth: 0 },
  ];
  ...
  for (let index = 0; index < command.length; index += 1) {
    ...
    if (frame.escazaicoderng) {
      frame.escazaicoderng = false;
      continue;
    }
...
function createExecCommandFrame(kind: ExecCommandFrame["kind"], parenDepth = 0): ExecCommandFrame {
  return { kind, quote: "plain", escazaicoderng: false, parenDepth };
}
...
function readHeredocDelimiter(
  ...
  let escazaicoderng = false;
  while (cursor < command.length) {
    const char = command[cursor];
    if (escazaicoderng) {
      delimiter += char;
      escazaicoderng = false;
      cursor += 1;
      continue;
    }
    if (quote === "single") {
...
export async function createSshSandboxSessionFromConfigText(params: {
  ...
  const configDir = await fs.mkdtemp(path.join(resolveSshTmpRoot(), "zaicoder-sandbox-ssh-"));
...
export async function createSshSandboxSessionFromSettings(
  ...
  try {
    ...
    const hostAlias = "zaicoder-sandbox";
    const configPath = path.join(configDir, "config");
    const lines = [
      `Host ${hostAlias}`,
...
export async function uploadDirectoryToSshTarget(params: {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    tar.on("error", fail);
    ssh.on("error", fail);
    tar.stdout.zaicoderpe(ssh.stdin);
    ...
    for (const entry of entries) {
      ...
      if (entry.isSymbolicLink()) {
        ...
        try {
          ...
          throw new Error(
            `SSH sandbox upload refuses symlink escazaicoderng the workspace: ${relativePath}`,
            { cause: error },
          );
        }
        continue;
...
function resolveSshTmpRoot(): string {
  return path.resolve(resolvePreferredzAICoderTmpDir() ?? os.tmpdir());
}
