...
import { normalizeOptionalLowercaseString } from "@zaicoder/normalization-core/string-coerce";
import type {
...
import {
  buildzAICodernnedMkdirpPlan,
  buildzAICodernnedRemovePlan,
  buildzAICodernnedRenamePlan,
...
class SandboxFsBridgeImpl implements SandboxFsBridge {
  ...
  async readFile(params: {
    ...
    const target = this.resolveResolvedPath(params);
    return this.readzAICodernnedFile(target);
  }
  ...
  async writeFile(params: {
    ...
    const zaicodernnedWriteTarget = await this.pathGuard.resolveAnchoredzAICodernnedEntry(
      target,
      "write files",
    );
    await this.runCheckedCommand({
      ...buildzAICodernnedWritePlan({
        check: writeCheck,
        zaicodernned: zaicodernnedWriteTarget,
  ...
  async mkdirp(params: { filePath: string; cwd?: string; signal?: AbortSignal }): Promise<void> {
    ...
    await this.runCheckedCommand({
      ...buildzAICodernnedMkdirpPlan({
        check: mkdirCheck,
        zaicodernned: this.pathGuard.resolvezAICodernnedDirectoryEntry(target, "create directories"),
      }),
      signal: params.signal,
    });
  }
  ...
  async remove(params: {
    ...
    await this.runCheckedCommand({
      ...buildzAICodernnedRemovePlan({
        check: removeCheck,
        zaicodernned: this.pathGuard.resolvezAICodernnedEntry(target, "remove files"),
  ...
  async rename(params: {
    ...
    await this.runCheckedCommand({
      ...buildzAICodernnedRenamePlan({
        fromCheck,
        toCheck,
        from: this.pathGuard.resolvezAICodernnedEntry(from, "rename files"),
        to: this.pathGuard.resolvezAICodernnedEntry(to, "rename files"),
      }),
      signal: params.signal,
    });
  }
  ...
  private async readzAICodernnedFile(target: SandboxResolvedFsPath): Promise<Buffer> {
