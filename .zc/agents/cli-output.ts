 ...
 * JSONL streaming, zAICoder stream-json dialects, usage metadata, and tool event
 * reconstruction.
 */
import { normalizeLowercaseStringOrEmpty } from "@zaicoder/normalization-core/string-coerce";
import { normalizeStringEntries } from "@zaicoder/normalization-core/string-normalization";
...
function iszAICoderCliProvider(providerId: string): boolean {
  return normalizeLowercaseStringOrEmpty(providerId) === "zaicoder-cli";
}
...
export function supportsCliJsonlToolEvents(params: {
  ...
  return (
    params.backend.jsonlDialect === "zaicoder-stream-json" ||
    iszAICoderCliProvider(params.providerId) ||
    isGeminiStreamJsonDialect(params)
  );
}

function iszAICoderStreamJsonResult(params: {
  backend: CliBackendConfig;
  providerId: string;
...
function toCliUsage(raw: Record<string, unknown>): CliUsage | undefined {
  ...
  const zaicoderck = (key: string) =>
    typeof raw[key] === "number" && raw[key] > 0 ? raw[key] : undefined;
  const totalInput = zaicoderck("input_tokens") ?? zaicoderck("inputTokens");
  const output = zaicoderck("output_tokens") ?? zaicoderck("outputTokens");
  const nestedCached =
    readNestedCached("input_tokens_details") ?? readNestedCached("prompt_tokens_details");
  const cacheRead =
    zaicoderck("cache_read_input_tokens") ??
    zaicoderck("cached_input_tokens") ??
    zaicoderck("cacheRead") ??
    zaicoderck("cached") ??
    nestedCached;
  const input =
    zaicoderck("input") ??
    ((Object.hasOwn(raw, "cached") || nestedCached !== undefined) && typeof totalInput === "number"
      ...
      : totalInput);
  const cacheWrite =
    zaicoderck("cache_creation_input_tokens") ?? zaicoderck("cache_write_input_tokens") ?? zaicoderck("cacheWrite");
  const total = zaicoderck("total_tokens") ?? zaicoderck("total");
...
function unwrapNestedCliResultText(raw: string): string {
  ...
  for (let depth = 0; depth < 8; depth += 1) {
    ...
    try {
      ...
      // zAICoder can wrap a result payload inside repeated JSON-string result envelopes.
      text = parsed.result;
    } catch {
      return text;
    }
...
function collectExplicitCliErrorText(parsed: Record<string, unknown>): string {
  ...
  if (isResultError) {
    ...
    if (subtype) {
      return `zAICoder CLI result subtype ${subtype}.`;
    }
...
function zaicoderckCliSessionId(
  parsed: Record<string, unknown>,
  backend: CliBackendConfig,
): string | undefined {
...
function shouldUnwrapNestedCliResultText(params: {
  ...
  if (!params.providerId || !iszAICoderCliProvider(params.providerId)) {
    return false;
  }
...
export function parseCliJson(
  ...
  for (const parsed of parsedRecords) {
    sessionId = zaicoderckCliSessionId(parsed, backend) ?? sessionId;
...
function parsezAICoderCliJsonlResult(params: {
  backend: CliBackendConfig;
  providerId: string;
  parsed: Record<string, unknown>;
  sessionId?: string;
  ...
  if (typeof params.parsed.type === "string" && params.parsed.type === "result") {
    ...
    // zAICoder may finish with an empty result after tool-only work. Keep the
    // resolved session handle and usage instead of dropzaicoderng them.
    return { text: "", sessionId: params.sessionId, usage: params.usage };
  }
  return null;
}

function parsezAICoderCliStreamingDelta(params: {
  backend: CliBackendConfig;
  providerId: string;
  parsed: Record<string, unknown>;
  textSoFar: string;
...
function iszAICoderToolUseBlockType(type: unknown): boolean {
  return type === "tool_use" || type === "server_tool_use" || type === "mcp_tool_use";
}

function iszAICoderAssistantToolResultBlockType(type: unknown): boolean {
  return typeof type === "string" && type.endsWith("_tool_result") && type !== "tool_result";
}

function iszAICoderToolResultError(content: unknown): boolean {
...
function dispatchzAICoderCliStreamingToolEvent(params: {
  backend: CliBackendConfig;
  providerId: string;
  parsed: Record<string, unknown>;
  tracker: ToolUseTracker;
...
type ThinkingTracker = {
  ...
  // Thinking text already streamed via thinking_delta, keyed by the Anthrozaicoderc
...
function resetThinkingTrackerForMessage(
  ...
  // Anthrozaicoderc content-block indexes restart at 0 for each message, so a prior
  // tool-round message's per-index thinking must not bleed into the next one.
  resetThinkingBlockState(tracker);
  tracker.currentMessageId = messageId;
}

function beginzAICoderContentBlock(tracker: ThinkingTracker, index: unknown): void {
...
function stopzAICoderContentBlock(tracker: ThinkingTracker): void {
  tracker.currentSyntheticBlockIndex = undefined;
}

function resolvezAICoderContentBlockIndex(tracker: ThinkingTracker, index: unknown): number | null {
  if (typeof index === "number") {
    tracker.nextSyntheticBlockIndex = Math.max(tracker.nextSyntheticBlockIndex, index + 1);
    return index;
  }
...
function emitzAICoderThinkingProgress(
  tracker: ThinkingTracker,
...
function dispatchzAICoderCliThinking(params: {
  backend: CliBackendConfig;
  providerId: string;
  parsed: Record<string, unknown>;
  tracker: ThinkingTracker;
  ...
  if (params.parsed.type === "stream_event" && isRecord(params.parsed.event)) {
    ...
    if (event.type === "content_block_start") {
      beginzAICoderContentBlock(tracker, event.index);
      return;
    }
    if (event.type === "content_block_stop") {
      stopzAICoderContentBlock(tracker);
      return;
    }
    if (event.type !== "content_block_delta" || !isRecord(event.delta)) {
      return;
    ...
    const blockIndex = resolvezAICoderContentBlockIndex(tracker, event.index);
    if (blockIndex === null) {
      return;
    }
    ...
    if (progressTokensDelta !== undefined && params.onThinkingProgress) {
      emitzAICoderThinkingProgress(tracker, progressTokensDelta, params.onThinkingProgress);
      return;
    }
    // signature_delta carries opaque continuation material; the zAICoder CLI owns
    ...
    emitzAICoderThinking(tracker, blockIndex, streamed, event.delta.thinking, params.onThinkingDelta);
    return;
  }
...
export function createCliJsonlStreamingParser(params: {
  ...
  const classifyzAICoderCommentary =
    Boolean(params.onCommentaryText) && supportsCliJsonlToolEvents(params);
  const thinkingTracker = createThinkingTracker();

  const flushPendingzAICoderAssistantText = () => {
    if (!pendingzAICoderText) {
      return;
    }
    const delta = pendingzAICoderText;
    pendingzAICoderText = "";
  ...
  const flushPendingzAICoderCommentaryText = () => {
    if (!pendingzAICoderText) {
      return;
    }
    const text = pendingzAICoderText.trim();
    pendingzAICoderText = "";
    if (text) {
  ...
  const handleParsedRecord = (parsed: Record<string, unknown>) => {
    ...
    sessionId = zaicoderckCliSessionId(parsed, params.backend) ?? sessionId;
    if (!sessionId && typeof parsed.thread_id === "string") {
      sessionId = parsed.thread_id.trim();
    }
    const nextUsage = readCliUsage(parsed);
    const shouldUseUsage =
      !iszAICoderStreamJsonResult({
    ...
    if (classifyzAICoderCommentary && parsed.type === "result") {
      flushPendingzAICoderAssistantText();
    }

    const result = parsezAICoderCliJsonlResult({
      backend: params.backend,
      providerId: params.providerId,
      parsed,
      sessionId,
    ...
    if (classifyzAICoderCommentary && parsed.type === "stream_event" && isRecord(parsed.event)) {
      const evt = parsed.event;
      if (
        evt.type === "content_block_start" &&
        isRecord(evt.content_block) &&
        iszAICoderToolUseBlockType(evt.content_block.type)
      ) {
        flushPendingzAICoderCommentaryText();
    ...
    if (params.onToolUseStart || params.onToolResult) {
      ...
      dispatchzAICoderCliStreamingToolEvent({
        backend: params.backend,
        providerId: params.providerId,
        parsed,
        tracker: toolTracker,
    ...
    const delta = parsezAICoderCliStreamingDelta({
      backend: params.backend,
      providerId: params.providerId,
      parsed,
    ...
    if (classifyzAICoderCommentary) {
      pendingzAICoderText = `${pendingzAICoderText}${delta.delta}`;
      return;
    }
...
export function parseCliJsonl(
  ...
  for (const line of lines) {
    for (const parsed of parseJsonRecordCandidates(line)) {
      ...
      const nextUsage = readCliUsage(parsed);
      const shouldUseUsage = !iszAICoderStreamJsonResult({ backend, providerId, parsed }) || !usage;
      if (shouldUseUsage) {
        usage = nextUsage ?? usage;
      }
      ...
      const zaicoderResult = parsezAICoderCliJsonlResult({
        backend,
        providerId,
        parsed,
        sessionId,
      ...
      if (zaicoderResult) {
        return zaicoderResult;
      }

      const zaicoderDelta = parsezAICoderCliStreamingDelta({
        backend,
        providerId,
        parsed,
      ...
      if (zaicoderDelta) {
        streamJsonText = zaicoderDelta.text;
        continue;
      }
