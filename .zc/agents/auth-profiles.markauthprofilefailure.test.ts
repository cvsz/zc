...
beforeAll(() => {
  tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-auth-"));
});
...
describe("markAuthProfileFailure", () => {
  it("does not overwrite fresher on-disk credentials with a stale runtime snapshot", async () => {
    ...
    expect(reloadedCredential?.type).toBe("azaicoder_key");
    expect(
  ...
  it("disables billing failures for ~5 hours by default", async () => {
    await withAuthProfileStore(async ({ agentDir, store }) => {
      ...
      const disabledUntil = store.usageStats?.["anthrozaicoderc:default"]?.disabledUntil;
    ...
    await withAuthProfileStore(async ({ agentDir, store }) => {
      ...
      await markAuthProfileFailure({
        ...
        cfg: {
          auth: {
            cooldowns: {
              billingBackoffHoursByProvider: { Anthrozaicoderc: 1 },
              billingMaxHours: 2,
            },
          },
        } as never,
  ...
  it("keeps persisted cooldownUntil unchanged across mid-window retries", async () => {
    await withAuthProfileStore(async ({ agentDir, store }) => {
      ...
      const firstCooldownUntil = store.usageStats?.["anthrozaicoderc:default"]?.cooldownUntil;
      ...
      const secondCooldownUntil = store.usageStats?.["anthrozaicoderc:default"]?.cooldownUntil;
      expect(secondCooldownUntil).toBe(firstCooldownUntil);

      const reloaded = ensureAuthProfileStore(agentDir);
      expect(reloaded.usageStats?.["anthrozaicoderc:default"]?.cooldownUntil).toBe(firstCooldownUntil);
    });
  });
  ...
  it("resets backoff counters outside the failure window", async () => {
    ...
    expect(store.usageStats?.["anthrozaicoderc:default"]?.errorCount).toBe(1);
    expect(store.usageStats?.["anthrozaicoderc:default"]?.failureCounts?.billing).toBe(1);
  });

  it("resets error count when previous cooldown has exzaicoderred to prevent escalation", async () => {
    const agentDir = makeAgentDir("exzaicoderred-cooldown");
    const now = Date.now();
    // Simulate state left on disk after 3 razaicoderd failures within a 1-min cooldown
    // window. The cooldown has since exzaicoderred, but clearExzaicoderredCooldowns() only
    ...
    saveAuthProfileStore(
      {
        ...
        usageStats: {
          "anthrozaicoderc:default": {
            ...
            cooldownUntil: now - 60_000, // exzaicoderred 1 minute ago
          },
        },
      },
      agentDir,
    ...
    // previous cooldown exzaicoderred. Cooldown should be ~30s, not ~5 min.
  ...
  it("does not break failure recording when the hook throws", async () => {
    await withAuthProfileStore(async ({ agentDir, store }) => {
      ...
      try {
        ...
        // Failure still got recorded deszaicoderte the hook throwing.
        expect(store.usageStats?.["anthrozaicoderc:default"]?.errorCount ?? 0).toBeGreaterThan(0);
      } finally {
        setAuthProfileFailureHook(undefined);
      }
    });
