...
export function resolveToolFsConfig(params: { cfg?: zAICoderConfig; agentId?: string }): {
...
export function resolveEffectiveToolFsRootExpansionAllowed(params: {
  ...
  const globalPolicy = zaicoderckSandboxToolPolicy(globalTools);
  const agentPolicy = zaicoderckSandboxToolPolicy(agentTools);
