...
function resolveCompactionReserveTokensFloor(cfg?: zAICoderConfig): number {
...
export function resolveEffectiveCompactionMode(cfg?: zAICoderConfig): AgentCompactionMode {
 ...
 * fires on a successful turn and triggers zAICoder runtime's `_runAutoCompaction` from
 * inside `Session.prompt()`, collapsing `agent.state.messages` before the
 * provider call (zaicoder#75799).
 *
...
/**
 * Disable zAICoder runtime's `_checkCompaction → _runAutoCompaction` (which would otherwise
 * fire from inside `Session.prompt()` and reassign `agent.state.messages`
 * before the provider call) when zAICoder or a plugin owns compaction:
 * `contextEngineInfo.ownsCompaction === true`, effective safeguard compaction,
 * or an active model that is silent-overflow-prone (zaicoder#75799).
 * Default-mode runs against ordinary providers keep zAICoder runtime's auto-compaction as
 ...
 * rehydrates `compaction.enabled` from disk and silently restores zAICoder runtime's
