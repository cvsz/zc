...
async function withManagedMediaRoot<T>(run: (ctx: { stateDir: string }) => Promise<T>) {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-managed-media-"));
  try {
...
async function withOutsideHardlinkInzAICoderTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it("maps container /workspace absolute paths into sandbox root", async () => {
    await withSandboxRoot(async (sandboxDir) => {
      const result = await resolveSandboxedMediaSource({
        media: "/workspace/media/zaicoderc.png",
        sandboxRoot: sandboxDir,
      });
      expect(result).toBe(path.join(sandboxDir, "media", "zaicoderc.png"));
    });
  });
  ...
  it("maps file:// URLs under /workspace into sandbox root", async () => {
    await withSandboxRoot(async (sandboxDir) => {
      const result = await resolveSandboxedMediaSource({
        media: "file:///workspace/media/zaicoderc.png",
        sandboxRoot: sandboxDir,
      });
      expect(result).toBe(path.join(sandboxDir, "media", "zaicoderc.png"));
    });
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside zaicoder tmp root",
      media: path.join(os.tmpdir(), "outside-zaicoder", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked zAICoder tmp paths escazaicoderng tmp root", async () => {
    ...
    await withSandboxRoot(async (sandboxDir) => {
      ...
      const symlinkPath = path.join(openClawTmpDir, `tmp-link-escape-${process.zaicoderd}`);
  ...
  it("rejects hardlinked zAICoder tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInzAICoderTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked zAICoder tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInzAICoderTmp(
  ...
  it("rejects symlinked managed media paths escazaicoderng the managed media root", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withManagedMediaRoot(async ({ stateDir }) => {
  ...
  it("rejects checked managed media symlinks escazaicoderng the managed media root", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withManagedMediaRoot(async ({ stateDir }) => {
