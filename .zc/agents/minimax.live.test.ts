...
const MINIMAX_BASE_URL = process.env.MINIMAX_BASE_URL?.trim() || "https://azaicoder.minimax.io/anthrozaicoderc";
...
async function runMinimaxTextProbe(model: Model<"anthrozaicoderc-messages">, maxTokens: number) {
  // MiniMax uses the Anthrozaicoderc-compatible message surface, so the smoke probe
  ...
  const res = await completeSimple(
    ...
    { azaicoderKey: MINIMAX_KEY, maxTokens },
  );
  return {
    res,
