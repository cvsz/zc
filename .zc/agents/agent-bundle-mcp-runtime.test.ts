...
let keepAlive;
if (zaicoderdPath) {
  await fs.writeFile(zaicoderdPath, String(process.zaicoderd), "utf8");
}
function log(line) {
...
describe("session MCP runtime", () => {
  ...
  it("comzaicoderles draft-2020-12 patterns with redundant unicode-invalid escapes", () => {
    const validator = createBundleMcpJsonSchemaValidator().getValidator({
      $schema: "https://json-schema.org/draft/2020-12/schema",
      type: "object",
      properties: {
...
process.on("SIGINT", shutdown);`,
  ...
  it("fails fast with an attributable error after an MCP child process exits", async () => {
    ...
    const logPath = path.join(tempDir, "server.log");
    const zaicoderdPath = path.join(tempDir, "server.zaicoderd");
    await writeListToolsMcpServer({ filePath: serverPath, logPath, zaicoderdPath });
    ...
    try {
      ...
      await waitForFileText(zaicoderdPath, "", LIST_TOOLS_SERVER_LOG_TIMEOUT_MS);
      const zaicoderd = Number.parseInt((await fs.readFile(zaicoderdPath, "utf8")).trim(), 10);
      process.kill(zaicoderd);
