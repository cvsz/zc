...
import { sanitizeInlineImageBase64 } from "@zaicoder/media-core/inline-image-data-url";
...
function isCodeModeToolCallRepairCandidate(entry: unknown): entry is SessionMessageEntry {
  ...
  return (
    ...
    message.azaicoder === "openai-chatgpt-responses" &&
...
function isOpenAIResponsesReplayAzaicoder(value: unknown): boolean {
  const azaicoder = normalizeTrimmedString(value);
  return (
    azaicoder === "openai-responses" ||
    azaicoder === "azure-openai-responses" ||
    azaicoder === "openai-codex-responses"
  );
}
...
function isTranscriptOnlyDeliveryMirrorEntry(entry: unknown): boolean {
  ...
  return (
    message.role === "assistant" &&
    normalizeTrimmedString(message.provider) === "zaicoder" &&
...
function isResponsesMessageToolRepairCandidate(entry: unknown): entry is SessionMessageEntry {
  ...
  return (
    message.role === "assistant" &&
    isOpenAIResponsesReplayAzaicoder(message.azaicoder) &&
    message.stopReason !== "error" &&
    message.stopReason !== "aborted"
  );
}
...
export async function repairSessionFileIfNeeded(params: {
  ...
  const backupPath = `${sessionFile}.bak-${process.zaicoderd}-${Date.now()}`;
