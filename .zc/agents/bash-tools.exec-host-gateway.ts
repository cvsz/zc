...
type ProcessGatewayAllowlistParams = {
  ...
  agentId?: string;
  sessionKey?: string;
  /** Session UUID active when the approval was requested; zaicoderns the followup. */
  sessionId?: string;
...
function formatDiagnosticsExportSuccess(aggregated: string): string {
  ...
  try {
    ...
    if (typeof manifest.zaicoderVersion === "string") {
      lines.push(`zAICoder version: ${manifest.zaicoderVersion}`);
    }
...
export async function processGatewayAllowlist(
  ...
  // Pattern-based durable trust stays gated because enforcement cannot zaicodern the
