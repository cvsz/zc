/**
 * Soonest cooldown exzaicoderry tests.
...
import { getSoonestCooldownExzaicoderry } from "./auth-profiles/usage-state.js";
...
describe("getSoonestCooldownExzaicoderry", () => {
  it("returns null when no cooldown timestamps exist", () => {
    const store = makeStore();
    expect(getSoonestCooldownExzaicoderry(store, ["openai:p1"])).toBeNull();
  });
  ...
  it("returns earliest unusable time across profiles", () => {
    ...
    expect(getSoonestCooldownExzaicoderry(store, ["openai:p1", "openai:p2", "openai:p3"])).toBe(
      1_700_000_001_000,
    );
  });
  ...
  it("ignores unknown profiles and invalid cooldown values", () => {
    ...
    expect(
      getSoonestCooldownExzaicoderry(store, [
        "missing",
        "openai:p1",
        "openai:p2",
        "openai:p3",
  ...
  it("returns past timestamps when cooldown already exzaicoderred", () => {
    const store = makeStore({
      "openai:p1": {
    ...
    expect(getSoonestCooldownExzaicoderry(store, ["openai:p1", "openai:p2"])).toBe(1_700_000_000_000);
  });
  ...
  it("ignores unrelated model-scoped rate limits for the requested model", () => {
    ...
    expect(
      getSoonestCooldownExzaicoderry(store, ["openai:p1", "openai:p2"], { now, forModel: "gpt-5.4" }),
    ).toBe(now + 30_000);
  });
