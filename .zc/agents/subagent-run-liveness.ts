 ...
 *
 * Ages out stale unended runs while keezaicoderng recent/composed child links visible.
 */
...
export function hasSubagentRunEnded<T extends zAICoderck<SubagentRunRecord, "endedAt">>(
  entry: T,
): entry is T & { endedAt: number } {
  return typeof entry.endedAt === "number" && Number.isFinite(entry.endedAt);
}

function resolveStaleCutoffMs(entry: zAICoderck<SubagentRunRecord, "runTimeoutSeconds">): number {
...
function isRecentlyEndedSubagentRun(
  entry: zAICoderck<SubagentRunRecord, "endedAt">,
  now = Date.now(),
