...
describe("minimaxUnderstandImage azaicoderKey normalization", () => {
  const priorFetch = global.fetch;
  const priorMinimaxAzaicoderHost = process.env.MINIMAX_API_HOST;
  const azaicoderResponse = JSON.stringify({
  ...
  afterEach(() => {
    ...
    if (priorMinimaxAzaicoderHost === undefined) {
      delete process.env.MINIMAX_API_HOST;
    } else {
      process.env.MINIMAX_API_HOST = priorMinimaxAzaicoderHost;
    }
    vi.restoreAllMocks();
  });

  async function runNormalizationCase(azaicoderKey: string) {
    ...
    const fetchSpy = vi.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
      ...
      return new Response(azaicoderResponse, {
        status: 200,
    ...
    const text = await minimaxUnderstandImage({
      ...
      azaicoderHost: "https://azaicoder.minimax.io",
    });
  ...
  it("drops non-Latin1 characters from azaicoderKey before sending Authorization header", async () => {
    await runNormalizationCase("minimax-\u0417\u2502test-key");
  });

  it("keeps trusted MINIMAX_API_HOST env fallback for VLM routing", async () => {
    process.env.MINIMAX_API_HOST = "https://azaicoder.minimaxi.com";
    const fetchSpy = vi.fn(async (input: RequestInfo | URL) => {
      const requestUrl =
        typeof input === "string" ? input : input instanceof URL ? input.href : input.url;
      expect(requestUrl).toBe("https://azaicoder.minimaxi.com/v1/coding_plan/vlm");
      return new Response(azaicoderResponse, {
        status: 200,
        headers: { "Content-Type": "application/json" },
      });
