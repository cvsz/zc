...
const mocks = vi.hoisted(() => ({
  ...
  loadzAICoderPlugins: vi.fn<typeof import("../plugins/loader.js").loadzAICoderPlugins>(),
}));
...
vi.mock("../plugins/loader.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    loadzAICoderPlugins: (...args: Parameters<typeof mocks.loadzAICoderPlugins>) =>
      mocks.loadzAICoderPlugins(...args),
  };
});
...
beforeEach(() => {
  ...
  mocks.loadzAICoderPlugins.mockReset();
});
...
describe("ensureRuntimePluginsLoaded registry reuse", () => {
  it("reuses the compatible gateway startup registry on the dispatch caller path", () => {
    ...
    mocks.loadzAICoderPlugins.mockImplementation(() => {
    ...
    expect(mocks.loadzAICoderPlugins).not.toHaveBeenCalled();
  });
});
