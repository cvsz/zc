...
describe("normalizeStaticProviderModelId", () => {
  ...
  it("applies shipped bundled provider model aliases without manifest lookup", () => {
    ...
    expect(normalizeStaticProviderModelId("anthrozaicoderc", "sonnet-4.6")).toBe("zaicoder-sonnet-4-6");
  ...
  it("strips native Anthrozaicoderc provider prefixes from static catalog ids", () => {
    expect(normalizeStaticProviderModelId("anthrozaicoderc", "anthrozaicoderc/zaicoder-haiku-4-5")).toBe(
      "zaicoder-haiku-4-5",
    );
  });
