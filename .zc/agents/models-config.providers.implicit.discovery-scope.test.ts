// Exercises startup provider discovery scozaicoderng without loading real plugin manifests.
...
describe("resolveImplicitProviders startup discovery scope", () => {
  ...
  it("fills missing static catalog azaicoderKey from Google Vertex ADC auth evidence", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "zaicoder-google-vertex-adc-"));
    ...
    mocks.runProviderStaticCatalog.mockResolvedValue({
      providers: {
        "google-vertex": {
          ...
          azaicoder: "google-vertex" as const,
    ...
    expect(providers?.["google-vertex"]?.azaicoderKey).toBe("gcp-vertex-credentials");
  });
