...
const CODE_MODE_NAMESPACE_TOOL_CALL = Symbol.for("zaicoder.codeMode.namespaceToolCall");
...
const CODE_MODE_NAMESPACE_REGISTRY_KEY = Symbol.for("zaicoder.codeMode.namespaces");
...
function normalizeRegistration(
  ...
  if (RESERVED_NAMESPACE_GLOBALS.has(globalName) || globalName.startsWith("__zaicoder")) {
...
function renderDocComment(
  ...
  params: readonly McpAzaicoderParamDoc[],
): string[] {
  const lines: string[] = [];
...
type McpAzaicoderToolDoc = {
  ...
  description?: string;
  parameters: unknown;
  params: McpAzaicoderParamDoc[];
};

type McpAzaicoderServerDoc = {
  identifier: string;
  serverName: string;
  tools: McpAzaicoderToolDoc[];
};

/** Virtual TypeScript-style API file exposed to code mode. */
export type CodeModeAzaicoderVirtualFile = {
  path: string;
  description?: string;
  content: string;
};

function buildMcpParamDocs(schema: unknown): McpAzaicoderParamDoc[] {
  const required = new Set(readRequiredKeys(schema));
  return orderedSchemaKeys(schema).map((key) => {
    const descriptor = readSchemaProperties(schema)[key];
    const doc: McpAzaicoderParamDoc = {
      name: key,
...
function renderMcpInputType(params: readonly McpAzaicoderParamDoc[]): string[] {
  if (params.length === 0) {
...
function renderMcpServerHeader(server: McpAzaicoderServerDoc, tools: readonly McpAzaicoderToolDoc[]): string {
  const lines = [
    "type McpAzaicoderHeader = { header: string; tools?: unknown[]; schemas?: Record<string, unknown> };",
    "",
    "type McpToolResult = {",
    "  content?: unknown[];",
    ...
    "  function $azaicoder(toolName?: string, options?: { schema?: boolean }): Promise<McpAzaicoderHeader>;",
  ];
  ...
  const nestedGroups = new Map<string, McpAzaicoderToolDoc[]>();
...
function renderMcpRootHeader(servers: readonly McpAzaicoderServerDoc[]): string {
  return [
    "type McpAzaicoderHeader = { header: string; servers?: unknown[] };",
    "",
    "declare const MCP: {",
    "  /** List visible MCP servers and request server-specific headers. */",
    "  $azaicoder(): Promise<McpAzaicoderHeader>;",
    ...servers.map((server) => `  readonly ${server.identifier}: typeof MCP.${server.identifier};`),
    "};",
  ].join("\n");
}

function renderMcpRootFile(servers: readonly McpAzaicoderServerDoc[]): string {
...
function buildMcpAzaicoderResponse(params: {
  servers: readonly McpAzaicoderServerDoc[];
  server?: McpAzaicoderServerDoc;
  args: unknown[];
}) {
  ...
  if (!params.server) {
    return {
      ...
      note: "Call MCP.<server>.$azaicoder() for a TypeScript-style header, then call tools with one object argument matching the shown input type.",
    };
  }
  const selected =
...
function toolIdentifiersForServer(
  ...
  const created = new Set<string>(["$azaicoder", "resources", "prompts"]);
...
function createMcpNamespaceModel(
  ...
  const serverDocs = new Map<string, McpAzaicoderServerDoc>();
  ...
  root.$azaicoder = createCodeModeNamespaceLocalFunction("$azaicoder", (args) =>
    buildMcpAzaicoderResponse({ servers: docs, args }),
  );
  for (const server of docs) {
    const serverScope = scopeAtPath(root, [server.identifier]);
    serverScope.$azaicoder = createCodeModeNamespaceLocalFunction("$azaicoder", (args) =>
      buildMcpAzaicoderResponse({ servers: docs, server, args }),
    );
  }
  return { root, docs };
}
...
export function createCodeModeAzaicoderVirtualFiles(
  catalog: readonly CodeModeNamespaceCatalogEntry[] = [],
): CodeModeAzaicoderVirtualFile[] {
  const model = createMcpNamespaceModel(catalog);
  if (!model) {
    return [];
  }
  const files: CodeModeAzaicoderVirtualFile[] = [
    {
      path: "mcp/index.d.ts",
