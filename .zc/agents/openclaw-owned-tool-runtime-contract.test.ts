// Verifies zAICoder-owned tool hooks preserve adjusted params and telemetry.
import type { AgentTool } from "zaicoder/plugin-sdk/agent-core";
import {
  installzAICoderOwnedToolHooks,
  resetzAICoderOwnedToolHooks,
  textToolResult,
} from "zaicoder/plugin-sdk/agent-runtime-test-contracts";
import type { ExtensionContext } from "zaicoder/plugin-sdk/agent-sessions";
...
describe("zAICoder-owned tool runtime contract - embedded agent adapter", () => {
  afterEach(() => {
    resetzAICoderOwnedToolHooks();
  });
  ...
  it("preserves partially adjusted before_tool_call params through execution and after_tool_call", async () => {
    ...
    const hooks = installzAICoderOwnedToolHooks({ adjustedParams });
  ...
  it("commits successful embedded agent messaging text, media, and target telemetry", async () => {
    const hooks = installzAICoderOwnedToolHooks();
    ...
    const originalParams = {
      ...
      mediaUrl: "/tmp/zaicoder-reply.png",
      provider: "telegram",
      to: "chat-1",
    };
    ...
    expect(ctx.state.messagingToolSentMediaUrls).toEqual(["/tmp/zaicoder-reply.png"]);
    expect(
    ...
    ).toEqual([
      {
        ...
        mediaUrls: ["/tmp/zaicoder-reply.png"],
      },
    ]);
    const [afterPayload, afterContext] = await waitForAfterToolCall(hooks);
  ...
  it("fails closed when before_tool_call blocks an embedded agent dynamic tool", async () => {
    const hooks = installzAICoderOwnedToolHooks({ blockReason: "blocked by policy" });
