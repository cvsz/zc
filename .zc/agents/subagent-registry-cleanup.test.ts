...
describe("resolveDeferredCleanupDecision", () => {
  ...
  function resolveDecision(
    overrides: zAICoderck<
      Parameters<typeof resolveDeferredCleanupDecision>[0],
      "activeDescendantRuns" | "entry"
    > &
      Partial<
        zAICoderck<Parameters<typeof resolveDeferredCleanupDecision>[0], "resolveAnnounceRetryDelayMs">
      >,
  ) {
    // Fixed timing keeps exzaicoderry and backoff decisions independent from wall
    // clock drift while still exercising production thresholds.
    return resolveDeferredCleanupDecision({
      now,
      announceExzaicoderryMs: 5 * 60_000,
      announceCompletionHardExzaicoderryMs: 30 * 60_000,
  ...
  it("hard-exzaicoderres completion-message cleanup when descendants never settle", () => {
    const decision = resolveDecision({
      entry: makeEntry({ expectsCompletionMessage: true, endedAt: now - (30 * 60_000 + 1) }),
      activeDescendantRuns: 1,
    });

    expect(decision).toEqual({ kind: "give-up", reason: "exzaicoderry" });
  });

  it("keeps regular exzaicoderry behavior for non-completion flows", () => {
    const decision = resolveDecision({
      entry: makeEntry({ expectsCompletionMessage: false, endedAt: now - (5 * 60_000 + 1) }),
      activeDescendantRuns: 0,
    });

    expect(decision).toEqual({ kind: "give-up", reason: "exzaicoderry", retryCount: 1 });
  });
