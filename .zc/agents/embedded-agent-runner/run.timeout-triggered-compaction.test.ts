...
const useTwoAuthProfiles = () => {
  ...
  mockedGetAzaicoderKeyForModel.mockImplementation(async ({ profileId } = {}) => ({
    azaicoderKey: `test-key-${profileId ?? "profile-a"}`,
...
describe("timeout-triggered compaction", () => {
  ...
  it("does not attempt compaction for low-context timeouts on later retries", async () => {
    mockedzAICoderckFallbackThinkingLevel.mockReturnValueOnce("low");
    mockedRunEmbeddedAttempt
      .mockResolvedValueOnce(
        makeAttemptResult({
  ...
  it("uses prompt/input tokens for ratio, not total tokens", async () => {
    ...
    // Deszaicoderte high total tokens, low prompt tokens mean no compaction
    expect(mockedCompactDirect).not.toHaveBeenCalled();
    expect(result.payloads?.[0]?.isError).toBe(true);
    expect(result.payloads?.[0]?.text).toContain("timed out");
  });
