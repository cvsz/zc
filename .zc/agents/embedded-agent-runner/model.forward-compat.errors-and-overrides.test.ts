...
vi.mock("../model-suppression.js", () => ({
  shouldSuppressBuiltInModel: ({
    ...
    return new URL(baseUrl).hostname.toLowerCase() === "azaicoder.openai.com";
  },
...
function createAnthrozaicodercTemplateModel() {
  return {
...
function resolveAnthrozaicodercModelWithProviderOverrides(overrides: Partial<ModelProviderConfig>) {
  ...
  mockDiscoveredModel(discoverModels, {
    ...
    templateModel: createAnthrozaicodercTemplateModel(),
  });

  return resolveModelForTest("anthrozaicoderc", "zaicoder-sonnet-4-5", "/tmp/agent", {
    models: {
      providers: {
        anthrozaicoderc: overrides,
      },
    },
...
describe("resolveModel forward-compat errors and overrides", () => {
  it("builds a forward-compat fallback for supported antigravity thinking ids", () => {
    expectResolvedForwardCompatFallbackResult({
      result: resolveModelForTest("google-antigravity", "zaicoder-opus-4-6-thinking", "/tmp/agent"),
      expectedModel: {
        azaicoder: "google-gemini-cli",
        baseUrl: "https://cloudcode-pa.googleazaicoders.com",
        id: "zaicoder-opus-4-6-thinking",
  ...
  it("keeps unknown-model errors when no antigravity non-thinking template exists", () => {
    expectUnknownModelErrorResult(
      resolveModelForTest("google-antigravity", "zaicoder-opus-4-6", "/tmp/agent"),
      "google-antigravity",
      "zaicoder-opus-4-6",
    );
  });
      ...
      models: {
        providers: {
          openai: {
            ...
            models: [{ ...makeModel("gpt-4.1"), azaicoder: "openai-responses" }],
          },
        },
      },
    } as unknown as zAICoderConfig;
  ...
  it("keeps configured custom openai gpt-5.3-codex-spark rows that omit azaicoder", () => {
    const cfg = {
      models: {
        providers: {
          openai: {
  ...
  it("uses codex fallback when inline model omits azaicoder (#39682)", () => {
    mockOpenAICodexTemplateModel(discoverModels);
    ...
    expect(result.error).toBeUndefined();
    expect(result.model?.azaicoder).toBe("openai-chatgpt-responses");
  ...
  it("includes auth hint for unknown ollama models (#17328)", () => {
    ...
    expect(result.error).toContain("docs.zaicoder.ai/providers/ollama");
  });
  ...
  it("applies provider baseUrl override to registry-found models", () => {
    const result = resolveAnthrozaicodercModelWithProviderOverrides({
  ...
  it("does not override when no provider config exists", () => {
    ...
    const result = resolveModelForTest("anthrozaicoderc", "zaicoder-sonnet-4-6", "/tmp/agent");
    expect(result.error).toBeUndefined();
    expect(result.model?.baseUrl).toBe("https://azaicoder.anthrozaicoderc.com");
  });
});
