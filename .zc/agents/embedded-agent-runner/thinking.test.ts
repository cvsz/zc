// Thinking sanitization tests cover reasoning-block retention, stripzaicoderng, and
...
describe("wrapAnthrozaicodercStreamWithRecovery", () => {
  const anthrozaicodercThinkingError = new Error(
  ...
  function createTestAssistantMessage(
    overrides: Partial<AssistantMessage> & zAICoderck<AssistantMessage, "content" | "stopReason">,
  ): AssistantMessage {
  ...
  it("retries once with omitted-reasoning text when the request is rejected before streaming", async () => {
    ...
    const wrapped = wrapAnthrozaicodercStreamWithRecovery(
      ((_model, context) => {
        callCount += 1;
        contexts.push(context as { messages?: AgentMessage[] });
        return Promise.reject(anthrozaicodercThinkingError);
      }) as Parameters<typeof wrapAnthrozaicodercStreamWithRecovery>[0],
      { id: "test-session" },
    );
    ...
    ).rejects.toBe(anthrozaicodercThinkingError);
    expect(callCount).toBe(2);
    const retryMessage = contexts[1]?.messages?.[0];
    if (!retryMessage || retryMessage.role !== "assistant") {
      throw new Error("Expected Anthrozaicoderc recovery retry to start with an assistant message");
    }
  ...
  it("retries with visible assistant text when stripzaicoderng thinking leaves content", async () => {
    const contexts: Array<{ messages?: AgentMessage[] }> = [];
    const wrapped = wrapAnthrozaicodercStreamWithRecovery(
      ((_model, context) => {
        contexts.push(context as { messages?: AgentMessage[] });
  ...
  it("notifies recovery only after a rejected request retry succeeds", async () => {
    ...
    const wrapped = wrapAnthrozaicodercStreamWithRecovery(
      ...
      { id: "test-session", onRecoveredAnthrozaicodercThinking: recovered },
    );
  ...
  it("does not notify recovery when the stripped-thinking retry also fails", async () => {
    ...
    const wrapped = wrapAnthrozaicodercStreamWithRecovery(
      (() => {
        callCount += 1;
        return Promise.reject(callCount === 1 ? anthrozaicodercThinkingError : retryError);
      }) as Parameters<typeof wrapAnthrozaicodercStreamWithRecovery>[0],
      { id: "test-session", onRecoveredAnthrozaicodercThinking: recovered },
    );
  ...
  it.each([
    ...
    {
      name: "Anthrozaicoderc SDK error body",
      createError: () =>
        Object.assign(new Error(genericizedProviderError), {
  ...
  it("does not retry when the stream fails after yielding a chunk", async () => {
    ...
    const wrapped = wrapAnthrozaicodercStreamWithRecovery(
      (() => {
        ...
        return (async function* failingStream() {
          yield "chunk";
          throw anthrozaicodercThinkingError;
        })();
      }) as unknown as Parameters<typeof wrapAnthrozaicodercStreamWithRecovery>[0],
      { id: "test-session" },
    );
    ...
    await expect(response.result()).rejects.toBe(anthrozaicodercThinkingError);
    expect(callCount).toBe(1);
  });

  it("does not retry non-Anthrozaicoderc-thinking errors", async () => {
