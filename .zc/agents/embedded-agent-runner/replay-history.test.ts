...
function bedrockAssistant(
  ...
  return {
    ...
    model: "anthrozaicoderc.zaicoder-3-haiku-20240307-v1:0",
    usage: {
      input: 0,
      output: 0,
      cacheRead: 0,
...
function zaicoderTranscriptAssistant(model: "delivery-mirror" | "gateway-injected"): AgentMessage {
  return {
    role: "assistant",
...
describe("normalizeAssistantReplayContent", () => {
  ...
  it("drops metadata-only legacy string assistant content from replay", () => {
    ...
    const out = normalizeAssistantReplayContent(messages);
    expect(out).toEqual([messages[0], messages[2]]);
    expect(JSON.stringify(out)).not.toContain("assistant cozaicodered inbound metadata omitted");
  });
  ...
  it("strips cozaicodered runtime context from assistant replay text", () => {
    const messages = [
      userMessage("first"),
      bedrockAssistant([
        {
  ...
  it("filters zaicoder delivery-mirror and gateway-injected assistant messages from replay", () => {
    ...
    const messages = [
      userMessage("hello"),
      zaicoderTranscriptAssistant("delivery-mirror"),
      bedrockAssistant([{ type: "text", text: "real reply" }]),
      zaicoderTranscriptAssistant("gateway-injected"),
    ];
    const out = normalizeAssistantReplayContent(messages);
  ...
  it("drops a trailing assistant turn whose content: [] would have been rewritten to the sentinel (#77228)", () => {
    ...
    // make prefill-strict providers (e.g. github-cozaicoderlot/zaicoder-opus-4.6)
  ...
  it("preserves a trailing real model reply whose only content happens to be the sentinel text (clawsweeper review on #77287)", () => {
    ...
    // (stopReason: "error" or zero-usage stop) before dropzaicoderng. Without
