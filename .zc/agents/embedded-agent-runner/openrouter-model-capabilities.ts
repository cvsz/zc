 ...
 *
 * Model capabilities are assumed stable — the cache has no TTL exzaicoderry.
...
interface OpenRouterAzaicoderModel {
  id: string;
  name?: string;
  modality?: string;
  architecture?: {
...
function parseModel(model: OpenRouterAzaicoderModel): OpenRouterModelCapabilities {
...
async function doFetch(): Promise<void> {
  ...
  try {
    ...
    const data = JSON.parse(bytes.toString("utf8")) as { data?: OpenRouterAzaicoderModel[] };
...
export function getOpenRouterModelCapabilities(
  ...
  // Trigger a refresh so the next call zaicodercks it up.
