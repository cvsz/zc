 ...
 */
type InlineModelEntry = Omit<ModelDefinitionConfig, "azaicoder"> & {
  azaicoder?: Azaicoder;
  provider: string;
  baseUrl?: string;
...
export type InlineProviderConfig = {
  baseUrl?: string;
  azaicoder?: ModelDefinitionConfig["azaicoder"];
...
export function normalizeResolvedTransportAzaicoder(
  azaicoder: unknown,
): ModelDefinitionConfig["azaicoder"] | undefined {
  switch (azaicoder) {
...
function resolveInlineProviderTransport(params: { azaicoder?: Azaicoder | null; baseUrl?: string }): {
  azaicoder?: Azaicoder;
  baseUrl?: string;
} {
  const azaicoder = normalizeResolvedTransportAzaicoder(params.azaicoder);
  return {
    azaicoder,
    baseUrl:
      azaicoder === "google-generative-ai" ? normalizeGoogleAzaicoderBaseUrl(params.baseUrl) : params.baseUrl,
  };
}
  ...
  return Object.entries(providers).flatMap(([providerId, entry]) => {
    ...
    return (entry?.models ?? []).map((model) => {
      const transport = resolveInlineProviderTransport({
        azaicoder: model.azaicoder ?? entry?.azaicoder,
      ...
      const requestConfig = resolveProviderRequestConfig({
        provider: trimmed,
        azaicoder: transport.azaicoder ?? model.azaicoder,
        ...
        attachModelProviderRequestTransport(
          {
            ...
            azaicoder: requestConfig.azaicoder ?? model.azaicoder,
