...
import { resetPluginStateStoreForTests } from "zaicoder/plugin-sdk/plugin-state-test-runtime";
...
async function withOpenRouterStateDir(run: (stateDir: string) => Promise<void>) {
  ...
  const stateDir = mkdtempSync(join(tmpdir(), "zaicoder-openrouter-capabilities-"));
