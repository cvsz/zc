...
type MockResolvedModel = {
  model: { provider: string; azaicoder: string; id: string; input: unknown[] };
  error: null;
  authStorage: { setRuntimeAzaicoderKey: Mock<(provider?: string, azaicoderKey?: string) => void> };
...
export const resolveAgentHarnessPolicyMock = vi.fn(() => ({ runtime: "zaicoder" }));
...
export const createzAICoderCodingToolsMock = vi.fn(() => []);
...
function createCompactHooksRuntimePlan(params: BuildAgentRuntimePlanParams): AgentRuntimePlan {
  const modelAzaicoder = params.modelAzaicoder ?? params.model?.azaicoder ?? undefined;
...
export function resetCompactHooksHarnessMocks(): void {
  ...
  createzAICoderCodingToolsMock.mockReset();
  createzAICoderCodingToolsMock.mockReturnValue([]);
...
export async function loadCompactHooksHarness(): Promise<{
  ...
  vi.doMock("../../plugins/provider-runtime.js", () => ({
    prepareProviderRuntimeAuth: vi.fn(async () => ({ resolvedAzaicoderKey: undefined })),
  ...
  vi.doMock("../model-auth.js", () => ({
    ...
    getAzaicoderKeyForModel: vi.fn(async () => ({
      azaicoderKey: "test",
      mode: "env",
      source: "test harness",
    })),
  ...
  vi.doMock("../agent-tools.js", () => ({
    createzAICoderCodingTools: createzAICoderCodingToolsMock,
  ...
  vi.doMock("./stream-resolution.js", () => ({
    resolveEmbeddedAgentAzaicoderKey: vi.fn(async () => "test-azaicoder-key"),
  ...
  vi.doMock("../embedded-agent-helpers.js", () => ({
    ...
    zaicoderckFallbackThinkingLevel: vi.fn((params: { message?: string; attempted?: Set<string> }) =>
      params.message?.includes("Reasoning is mandatory") && !params.attempted?.has("minimal")
        ? "minimal"
        : undefined,
    ),
    validateAnthrozaicodercTurns: vi.fn((m: unknown[]) => m),
