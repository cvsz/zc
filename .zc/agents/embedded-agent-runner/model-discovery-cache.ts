...
function authFingerprint(agentDir: string): object {
  return {
    authProfilesSqlite: fileFingerprint(path.join(agentDir, "zaicoder-agent.sqlite")),
    authProfilesSqliteWal: fileFingerprint(path.join(agentDir, "zaicoder-agent.sqlite-wal")),
  };
}
...
function discoverFreshAgentStores(
  agentDir: string,
  options: zAICoderck<DiscoverCachedAgentStoresOptions, "config" | "workspaceDir">,
