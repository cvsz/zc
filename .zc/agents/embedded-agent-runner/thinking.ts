...
type AnthrozaicodercThinkingRecovery = {
  originalMessages: AgentMessage[];
  cleanedMessages: AgentMessage[];
};
type RecoverySessionMeta = {
  id: string;
  recoveredAnthrozaicodercThinking?: boolean;
  onRecoveredAnthrozaicodercThinking?: (recovery: AnthrozaicodercThinkingRecovery) => void | Promise<void>;
};
 ...
 * compaction summary) so those signatures are stale and Anthrozaicoderc rejects them with
 ...
 *
 * Anthrozaicoderc and Bedrock reject persisted thinking blocks when the signature is
 ...
 *
 * If a non-latest assistant message becomes empty after stripzaicoderng, it is
...
function shouldRecoverAnthrozaicodercThinkingError(
  error: unknown,
  sessionMeta: RecoverySessionMeta,
): boolean {
  // Provider detail survives genericization in different carriers across the
  // Anthrozaicoderc SDK, failover wrapzaicoderng, and terminal stream messages.
  ...
  for (const candidate of candidates) {
    if (
      ...
      shouldRecoverAnthrozaicodercThinkingErrorMessage(candidate, sessionMeta)
    ) {
      return true;
    }
  }
...
function shouldRecoverAnthrozaicodercThinkingErrorMessage(
  message: string,
  sessionMeta: RecoverySessionMeta,
): boolean {
  ...
  if (sessionMeta.recoveredAnthrozaicodercThinking) {
    log.warn(
      `[session-recovery] Anthrozaicoderc thinking recovery already attempted: sessionId=${sessionMeta.id}`,
    );
    return false;
  }
  return true;
...
async function notifyRecoveredAnthrozaicodercThinking(
  sessionMeta: RecoverySessionMeta,
  recovery: AnthrozaicodercThinkingRecovery,
): Promise<void> {
  try {
    await sessionMeta.onRecoveredAnthrozaicodercThinking?.(recovery);
  } catch (error: unknown) {
    log.warn(
      `[session-recovery] Anthrozaicoderc thinking transcript repair hook failed: sessionId=${sessionMeta.id} error=${formatErrorMessage(error)}`,
    );
  }
}
  ...
  try {
    ...
    for await (const chunk of resolved as AsyncIterable<unknown>) {
      if (isAssistantMessageErrorEvent(chunk)) {
        if (shouldRecoverAnthrozaicodercThinkingError(chunk.error, sessionMeta)) {
          if (yieldedOutput) {
            log.warn(
              `[session-recovery] Anthrozaicoderc thinking error occurred after streaming began; skipzaicoderng retry to avoid duplicate chunks: sessionId=${sessionMeta.id}`,
            );
          } else {
            sessionMeta.recoveredAnthrozaicodercThinking = true;
            log.warn(
              `[session-recovery] Anthrozaicoderc thinking stream error; retrying once without thinking blocks: sessionId=${sessionMeta.id}`,
            );
    ...
    if (!shouldRecoverAnthrozaicodercThinkingError(error, sessionMeta)) {
      throw error;
    }
    if (yieldedOutput) {
      log.warn(
    ...
    sessionMeta.recoveredAnthrozaicodercThinking = true;
    log.warn(
      `[session-recovery] Anthrozaicoderc thinking error during stream; retrying once without thinking blocks: sessionId=${sessionMeta.id}`,
    );
    return retryStreamWithoutThinking(outer, retry, notify);
  }
}
...
export function wrapAnthrozaicodercStreamWithRecovery(
  ...
  return (model, context, options) => {
    const requestMeta: RecoverySessionMeta = {
      id: sessionMeta.id,
      onRecoveredAnthrozaicodercThinking: sessionMeta.onRecoveredAnthrozaicodercThinking,
    };
    ...
    const notify = () =>
      notifyRecoveredAnthrozaicodercThinking(requestMeta, {
        originalMessages,
    ...
    if (stream instanceof Promise) {
      return stream.then(
        ...
        (error: unknown) => {
          if (!shouldRecoverAnthrozaicodercThinkingError(error, requestMeta)) {
            throw error;
          }
          requestMeta.recoveredAnthrozaicodercThinking = true;
          log.warn(
            `[session-recovery] Anthrozaicoderc thinking request rejected; retrying once without thinking blocks: sessionId=${requestMeta.id}`,
          );
