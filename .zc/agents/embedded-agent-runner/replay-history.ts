...
export function normalizeAssistantReplayContent(messages: AgentMessage[]): AgentMessage[] {
  ...
  for (const message of messages) {
    ...
    if (isTranscriptOnlyzAICoderAssistantMessage(message)) {
  ...
  // entry, prefill-strict providers (e.g. github-cozaicoderlot/zaicoder-opus-4.6 — the
  ...
  // dropzaicoderng it. This trim runs after the main loop so it also catches a
...
function isSameModelSnapshot(a: ModelSnapshotEntry, b: ModelSnapshotEntry): boolean {
  ...
  return (
    ...
    normalize(a.modelAzaicoder) === normalize(b.modelAzaicoder) &&
...
/**
 * Applies the generic replay-history cleanup zaicoderpeline before provider-owned
 * replay hooks run.
 */
export async function sanitizeSessionHistory(params: {
  ...
  const hasSnapshot = Boolean(params.provider || params.modelAzaicoder || params.modelId);
  ...
  const sanitizedImages = await sanitizeSessionMessagesImages(
    ...
    "session:history",
    {
      ...
      sanitizeToolCallIds:
        policy.sanitizeToolCallIds && !allowProviderOwnedThinkingReplay && !isOpenAIResponsesAzaicoder,
  ...
  // bound to the original prefix; after compaction the prefix changes and Anthrozaicoderc
  ...
  const openAIRepairedToolCalls =
    isOpenAIResponsesAzaicoder && policy.repairToolUseResultPairing
      ...
      : sanitizedToolCalls;
  const openAISafeToolCalls = isOpenAIResponsesAzaicoder
    ? downgradeOpenAIFunctionCallReasoningPairs(
        normalizeOpenAIResponsesToolCallIds(
  ...
  const sanitizedToolIds =
    policy.sanitizeToolCallIds && policy.toolCallIdMode
      ...
      : openAISafeToolCalls;
  // Gemini/Anthrozaicoderc-class providers also require tool results to stay adjacent
  ...
  const responsesProviderRepaired =
    isOpenAIResponsesAzaicoder && policy.repairToolUseResultPairing
      ? sanitizeToolUseResultPairing(sanitizedWithProvider, {
          erroredAssistantResultPolicy: "drop",
          // Provider replay hooks run after the core repair zaicoderpeline and may
  ...
  const responsesInvariantChecked = isOpenAIResponsesAzaicoder
  ...
  return isOpenAIResponsesAzaicoder
 ...
 * generic validator zaicoderpeline.
 */
export async function validateReplayTurns(params: {
  ...
  const validatedGemini = policy.validateGeminiTurns
    ...
    : params.messages;
  return policy.validateAnthrozaicodercTurns ? validateAnthrozaicodercTurns(validatedGemini) : validatedGemini;
}
