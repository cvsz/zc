...
describe("runEmbeddedAgent incomplete-turn safety", () => {
  ...
  it("retries Kimi Anthrozaicoderc reasoning-only turns with a visible-answer continuation instruction", async () => {
    mockedClassifyFailoverReason.mockReturnValue(null);
    ...
    await runEmbeddedAgent({
      ...
      provider: "kimi",
      model: "kimi-for-coding",
      runId: "run-kimi-anthrozaicoderc-reasoning-only-continuation",
    });
  ...
  it("retries zero-token empty zAICoder stop turns with a visible-answer continuation instruction", async () => {
    mockedClassifyFailoverReason.mockReturnValue(null);
    mockedRunEmbeddedAttempt.mockResolvedValueOnce(
      makeAttemptResult({
        assistantTexts: [],
    ...
    mockedRunEmbeddedAttempt.mockResolvedValueOnce(
      makeAttemptResult({
        assistantTexts: ["Visible zAICoder answer."],
        lastAssistant: {
          role: "assistant",
          stopReason: "stop",
          provider: "anthrozaicoderc",
          model: "zaicoder-opus-4.7",
          content: [{ type: "text", text: "Visible zAICoder answer." }],
          usage: {
            input: 100,
            output: 5,
            cacheRead: 0,
    ...
    await runEmbeddedAgent({
      ...
      provider: "anthrozaicoderc",
      model: "zaicoder-opus-4.7",
      runId: "run-empty-zero-usage-zaicoder-continuation",
    });
  ...
  it("retries empty Anthrozaicoderc-compatible stop turns even when the provider is not Kimi", async () => {
    mockedClassifyFailoverReason.mockReturnValue(null);
    mockedResolveModelAsync.mockResolvedValue({
      model: {
        id: "zaicoder-opus-4-7",
        provider: "sub2azaicoder",
        contextWindow: 200000,
        azaicoder: "anthrozaicoderc-messages",
      },
      error: null,
    ...
    mockedRunEmbeddedAttempt.mockResolvedValueOnce(
      makeAttemptResult({
        assistantTexts: ["Visible Anthrozaicoderc-compatible answer."],
        lastAssistant: {
          role: "assistant",
          azaicoder: "anthrozaicoderc-messages",
          stopReason: "stop",
          provider: "sub2azaicoder",
          model: "zaicoder-opus-4-7",
          content: [{ type: "text", text: "Visible Anthrozaicoderc-compatible answer." }],
          usage: {
            input: 2300,
            output: 8,
            cacheRead: 0,
    ...
    await runEmbeddedAgent({
      ...
      provider: "sub2azaicoder",
      model: "zaicoder-opus-4-7",
      runId: "run-empty-anthrozaicoderc-compatible-stop-continuation",
    });
    ...
    const retryInstruction = resolveEmptyResponseRetryInstruction({
      ...
      attempt: makeAttemptResult({
        ...
        acceptedSessionSpawns: [
          {
            runId: "run-child",
            childSessionKey: "agent:zaicoder:subagent:child",
          },
        ],
        lastAssistant: {
          role: "assistant",
  ...
  it("treats post-tool exact NO_REPLY assistant turns as intentional silence", () => {
    const attempt = makeAttemptResult({
      assistantTexts: ["NO_REPLY"],
      toolMetas: [{ toolName: "process.poll", meta: "zaicoderd=123", replaySafe: true }],
      lastAssistant: {
        role: "assistant",
        stopReason: "stop",
        provider: "openai",
  ...
  it("does not classify visible assistant prose for retry", async () => {
    ...
    const result = await runEmbeddedAgent({
      ...overflowBaseRunParams,
      prompt:
        "made a bunch of improvements to the student's source code (zaicoder) this weekend, along with a few other maintainers. hopefully he will be more proactive now",
      provider: "openai",
      model: "gpt-5.4",
