...
async function makeTempFile(): Promise<string> {
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-session-manager-init-"));
  tempPaths.push(dir);
...
describe("prepareSessionManagerForRun", () => {
  ...
  it("rewrites forked transcript headers with cozaicodered assistant messages to the runtime cwd", async () => {
    // Forked sessions keep cozaicodered assistant context but rewrite the session
    ...
    await fs.writeFile(
      ...
      [
        ...
        JSON.stringify({
          ...
          parentId: null,
          timestamp: "2026-05-27T00:00:01.000Z",
          message: { role: "assistant", content: "cozaicodered context" },
        }),
      ].join("\n") + "\n",
      "utf-8",
    );
