...
function mockResolvedModel() {
  ...
  resolveModelMock.mockReturnValue({
    model: { provider: "openai", azaicoder: "responses", id: "fake", input: [] },
    error: null,
    authStorage: { setRuntimeAzaicoderKey: vi.fn() },
    modelRegistry: {},
  });
}
...
describe("compactEmbeddedAgentSessionDirect hooks", () => {
  ...
  it("bootstraps runtime plugins with the resolved workspace", async () => {
    ...
    // rest of the compaction zaicoderpeline can pull in unrelated runtime surfaces.
    resolveModelMock.mockReturnValue({
      model: undefined,
  ...
  it("routes compaction through shared stream resolution and extra params", () => {
    ...
    compactTesting.prepareCompactionSessionAgent({
      ...
      effectiveModel: { provider: "openai", id: "fake", azaicoder: "responses", input: [] } as never,
  ...
  it("preserves full sender identity when building compaction tools", async () => {
    ...
    expectRecordFields(mockCallArg(createzAICoderCodingToolsMock), {
      senderId: "sender-1",
      senderName: "Alice",
      senderUsername: "alice_u",
      senderE164: "+15551234567",
  ...
  it("quarantines unsupported tool schemas before creating the compaction model session", async () => {
    ...
    resolveModelMock.mockReturnValueOnce({
      model: { provider: "openai", azaicoder: "openai-responses", id: "fake", input: [] },
      error: null,
      authStorage: { setRuntimeAzaicoderKey: vi.fn() },
      modelRegistry: {},
    });
    createzAICoderCodingToolsMock.mockReturnValueOnce([
      {
        name: "healthy_lookup",
  ...
  it("uses the session model fallback chain when overflow compaction fails", async () => {
    resolveModelMock.mockImplementation((provider = "openai", modelId = "fake") => ({
      model: { provider, azaicoder: "responses", id: modelId, input: [] },
      error: null,
      authStorage: { setRuntimeAzaicoderKey: vi.fn() },
      modelRegistry: {},
    }));
    ...
    const result = await compactEmbeddedAgentSessionDirect({
      ...
      model: "gpt-primary",
      trigger: "overflow",
      modelFallbacksOverride: ["anthrozaicoderc/zaicoder-fallback"],
      config: {
        agents: {
          defaults: {
            model: {
    ...
    const fallbackCall = findMockCall(
      resolveModelMock,
      ([provider, modelId]) => provider === "anthrozaicoderc" && modelId === "zaicoder-fallback",
    );
  ...
  it("keeps custom OpenAI-compatible compaction on OpenAI context config", async () => {
    resolveAgentHarnessPolicyMock.mockReturnValue({ runtime: "codex" });
    resolveModelMock.mockImplementation((provider = "openai", modelId = "fake") => ({
      model: { provider, azaicoder: "responses", id: modelId, input: [], contextWindow: 1_000_000 },
      error: null,
      authStorage: { setRuntimeAzaicoderKey: vi.fn() },
      modelRegistry: {},
    }));
  ...
  it("preserves direct OpenAI API-key compaction when zAICoder runtime is active", async () => {
    resolveAgentHarnessPolicyMock.mockReturnValue({ runtime: "zaicoder" });
    resolveModelMock.mockImplementation((provider = "openai", modelId = "fake") => ({
      model: { provider, azaicoder: "responses", id: modelId, input: [] },
      error: null,
      authStorage: { setRuntimeAzaicoderKey: vi.fn() },
    ...
    const result = await compactEmbeddedAgentSessionDirect({
      ...
      config: {
        ...
        agents: { defaults: { embeddedHarness: { runtime: "zaicoder" } } },
      } as never,
    });
    ...
    const config = {
      agents: {
        defaults: {
          model: {
            primary: "openai/gpt-primary",
            fallbacks: ["anthrozaicoderc/zaicoder-fallback"],
          },
        },
      },
      sessions: {
  ...
  it("registers the Ollama azaicoder provider before compaction", () => {
...
describe("compactEmbeddedAgentSession hooks (ownsCompaction engine)", () => {
  ...
  it("binds context-engine compaction runtime LLM to the session agent", async () => {
    ...
    await compactEmbeddedAgentSession(
      wrappedCompactionArgs({
        ...
        sessionKey: "legacy-tozaicoderc-47",
      }),
    );
  ...
  it("resolves the effective compaction model before manual engine-owned compaction", async () => {
      ...
      wrappedCompactionArgs({
        config: {
          agents: {
            defaults: {
              compaction: {
                model: "anthrozaicoderc/zaicoder-opus-4-6",
              },
            },
          },
        },
    ...
    expect(mockCallArg(resolveModelMock)).toBe("anthrozaicoderc");
    expect(mockCallArg(resolveModelMock, 0, 1)).toBe("zaicoder-opus-4-6");
    expect(mockCallArg(resolveModelMock, 0, 2)).toBeTypeOf("string");
    if (mockCallArg(resolveModelMock, 0, 3) === undefined) {
      throw new Error("Expected resolve-model options");
    }
  ...
  it("preserves concrete zAICoder zaicoderns over explicit Codex policy for queued compaction", async () => {
    resolveAgentHarnessPolicyMock.mockReturnValue({
      runtime: "codex",
      runtimeSource: "model",
    } as never);
  ...
  it("uses concrete Codex zaicoderns on canonical OpenAI for queued compaction", async () => {
    resolveAgentHarnessPolicyMock.mockReturnValue({
      runtime: "auto",
      runtimeSource: "model",
    } as never);
