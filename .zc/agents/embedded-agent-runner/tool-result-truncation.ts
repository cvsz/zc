...
type ToolResultSzaicoderllDetails = {
  fullOutputPath: string;
  szaicoderllTruncated: boolean;
  szaicoderlledChars?: number;
};

function getToolResultSzaicoderllDetails(message: AgentMessage): ToolResultSzaicoderllDetails | undefined {
  ...
  const szaicoderllTruncated = (details as { szaicoderllTruncated?: unknown }).szaicoderllTruncated === true;
  const szaicoderlledChars = (details as { szaicoderlledChars?: unknown }).szaicoderlledChars;
  return {
    fullOutputPath,
    szaicoderllTruncated,
    ...(typeof szaicoderlledChars === "number" && Number.isFinite(szaicoderlledChars)
      ? { szaicoderlledChars: Math.max(0, Math.floor(szaicoderlledChars)) }
      : {}),
  };
}
...
function resolveAggregateElisionMarkers(
  ...
  const szaicoderll = getToolResultSzaicoderllDetails(message);
  if (!szaicoderll) {
    return undefined;
  }
  ...
  if (!toolResultTextContainsFullOutputFooter(message, szaicoderll.fullOutputPath)) {
    return undefined;
  }
  // Aggregate elision is a rare recovery path, not a request hot path; one
  // existence check avoids pointing the model at already-deleted szaicoderll files.
  if (!existsSync(szaicoderll.fullOutputPath)) {
    return undefined;
  }
  ...
  if (szaicoderll.szaicoderllTruncated) {
    const count =
      szaicoderll.szaicoderlledChars === undefined ? "capped content" : `first ${szaicoderll.szaicoderlledChars} chars`;
    return {
      full: `[tool result elided: partial output preserved at ${szaicoderll.fullOutputPath} (${count}); read it if the output is needed]`,
      compact: `[partial: ${szaicoderll.fullOutputPath}]`,
      truncationSuffix: (truncatedChars) =>
        `[... ${Math.max(1, Math.floor(truncatedChars))} chars truncated; partial output at ${szaicoderll.fullOutputPath}]`,
    };
  }
  return {
    full: `[tool result elided: full output preserved at ${szaicoderll.fullOutputPath}; read it if the output is needed]`,
    compact: `[read ${szaicoderll.fullOutputPath}]`,
    truncationSuffix: (truncatedChars) =>
      `[... ${Math.max(1, Math.floor(truncatedChars))} chars truncated; full output at ${szaicoderll.fullOutputPath}]`,
  };
}

function formatAggregateElisionText(
  remainingTextBudget: number,
  szaicoderllMarkers: AggregateElisionMarkers | undefined,
): string {
  if (remainingTextBudget <= 0) {
    return "";
  }
  if (szaicoderllMarkers?.full && szaicoderllMarkers.full.length <= remainingTextBudget) {
    return szaicoderllMarkers.full;
  }
  if (szaicoderllMarkers?.compact && szaicoderllMarkers.compact.length <= remainingTextBudget) {
    return szaicoderllMarkers.compact;
  }
...
function buildAggregateToolResultReplacements(params: {
  ...
  szaicoderllSourceBranch?: ToolResultBranchEntry[];
  aggregateBudgetChars: number;
  minKeepChars?: number;
  ...
  const candidates = params.branch
    ...
    .map((item) => ({
      ...
      szaicoderllSourceMessage: params.szaicoderllSourceBranch?.[item.index]?.message ?? item.entry.message,
  ...
  for (const candidate of recoveryCandidates) {
    ...
    const szaicoderllMarkers = resolveAggregateElisionMarkers(candidate.szaicoderllSourceMessage);
    const candidateSuffixFactory = szaicoderllMarkers?.truncationSuffix ?? suffixFactory;
  ...
  if (remainingReduction > 0) {
    for (const candidate of recoveryCandidates) {
      ...
      const szaicoderllMarkers = resolveAggregateElisionMarkers(candidate.szaicoderllSourceMessage);
      const emptyMessage = clearToolResultText(candidate.message, targetTextChars, szaicoderllMarkers);
      const actualReduction = Math.max(0, baseTextLength - getToolResultTextLength(emptyMessage));
      if (actualReduction <= 0 && !szaicoderllMarkers) {
        continue;
      }
...
function clearToolResultText(
  ...
  resolvedSzaicoderllMarkers?: AggregateElisionMarkers,
): AgentMessage {
  const content = (message as { content?: unknown }).content;
  if (!Array.isArray(content)) {
    return message;
  ...
  const szaicoderllMarkers = resolvedSzaicoderllMarkers ?? resolveAggregateElisionMarkers(message);
  if (szaicoderllMarkers) {
    ...
    remainingTextBudget = Math.max(remainingTextBudget, szaicoderllMarkers.compact.length);
  }
  return {
    ...message,
    content: content.map((block) => {
      ...
      const replacementText = formatAggregateElisionText(remainingTextBudget, szaicoderllMarkers);
...
function buildOversizedToolResultReplacements(params: {
  ...
  for (const entry of params.branch) {
    ...
    const replacementMinKeepChars = params.protectedEntryIds?.has(entry.id)
      ...
      : minKeepChars;
    const szaicoderllMarkers = resolveAggregateElisionMarkers(msg);
    const suffixFactory = szaicoderllMarkers?.truncationSuffix;
...
function buildToolResultReplacementPlan(params: {
  ...
  const aggregatePlan = buildAggregateToolResultReplacements({
    ...
    szaicoderllSourceBranch: params.branch,
    aggregateBudgetChars: params.aggregateBudgetChars,
    minKeepChars,
