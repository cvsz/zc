...
function buildAgentToolResultMiddlewareFactory(
  ...
  const runner = createAgentToolResultMiddlewareRunner({ runtime: "zaicoder" });
  return (agent) => {
    agent.on("tool_result", async (rawEvent: unknown, ctx: { cwd?: string }) => {
      ...
      const eventToolCallId =
        typeof event.toolCallId === "string" && event.toolCallId.trim()
          ? event.toolCallId
          : undefined;
      const toolCallId = eventToolCallId ?? `zaicoder-${randomUUID()}`;
...
function buildContextPruningFactory(params: {
  ...
  if (!isCacheTtlEligibleProvider(params.provider, params.modelId, params.model?.azaicoder)) {
    return undefined;
  }
