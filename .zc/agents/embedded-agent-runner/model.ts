 ...
 */
import { finiteSecondsToTimerSafeMilliseconds } from "@zaicoder/normalization-core/number-coercion";
...
import {
  ...
  normalizeResolvedTransportAzaicoder,
...
function applyResolvedTransportFallback(params: {
  ...
  }) as { azaicoder?: Azaicoder | null; baseUrl?: string } | undefined;
  if (!normalized) {
    return undefined;
  }
  const nextAzaicoder = normalizeResolvedTransportAzaicoder(normalized.azaicoder) ?? params.model.azaicoder;
  const nextBaseUrl = normalized.baseUrl ?? params.model.baseUrl;
  if (nextAzaicoder === params.model.azaicoder && nextBaseUrl === params.model.baseUrl) {
    return undefined;
  }
  return {
    ...params.model,
...
function resolveProviderTransport(params: {
  ...
  const normalized = runtimeHooks.normalizeProviderTransportWithPlugin({
    ...
    context: {
      ...
      provider: params.provider,
      ...(params.modelId ? { modelId: params.modelId } : {}),
      azaicoder: params.azaicoder,
      baseUrl: params.baseUrl,
    },
  }) as { azaicoder?: Azaicoder | null; baseUrl?: string } | undefined;

  return {
    azaicoder: normalizeResolvedTransportAzaicoder(normalized?.azaicoder ?? params.azaicoder),
    baseUrl: normalized?.baseUrl ?? params.baseUrl,
  };
}

function resolveConfiguredProviderDefaultAzaicoder(params: {
  provider: string;
  ...
  const explicit = normalizeResolvedTransportAzaicoder(providerConfig?.azaicoder);
  if (explicit) {
    return explicit;
  }
  ...
  return normalized.azaicoder ?? "openai-completions";
}
...
function mergeStaticCatalogInlineModel(
  ...
  return {
    ...staticCatalogModel,
    ...inlineModel,
    azaicoder: inlineModel.azaicoder ?? staticCatalogModel.azaicoder,
    baseUrl:
...
function applyConfiguredProviderOverrides(params: {
  ...
  if (!providerConfig) {
    ...
    const requestConfig = resolveProviderRequestConfig({
      provider: params.provider,
      azaicoder: discoveredModel.azaicoder,
  ...
  const providerDefaultAzaicoder = resolveConfiguredProviderDefaultAzaicoder({
    provider: params.provider,
    providerConfig,
    cfg: params.cfg,
    workspaceDir: params.workspaceDir,
  ...
  const resolvedTransportAzaicoder = params.preferDiscoveredTransport
    ? (discoveredModel.azaicoder ??
      metadataOverrideModel?.azaicoder ??
      providerConfig.azaicoder ??
      configuredStaticCatalogModel?.azaicoder ??
      providerDefaultAzaicoder)
    : (metadataOverrideModel?.azaicoder ??
      providerConfig.azaicoder ??
      discoveredModel.azaicoder ??
      configuredStaticCatalogModel?.azaicoder ??
  ...
  const resolvedTransport = resolveProviderTransport({
    ...
    azaicoder: resolvedTransportAzaicoder,
  ...
  const requestConfig = resolveProviderRequestConfig({
    provider: params.provider,
    azaicoder:
      resolvedTransport.azaicoder ??
      normalizeResolvedTransportAzaicoder(configuredStaticCatalogModel?.azaicoder) ??
      normalizeResolvedTransportAzaicoder(discoveredModel.azaicoder) ??
      providerDefaultAzaicoder ??
      "openai-responses",
    baseUrl:
  ...
  return attachModelProviderLocalService(
    attachModelProviderRequestTransport(
      {
        ...discoveredModel,
        azaicoder: requestConfig.azaicoder ?? "openai-responses",
...
function resolveExplicitModelWithRegistry(params: {
  ...
  if (inlineMatch?.azaicoder) {
    const transport = resolveProviderTransport({
      provider,
      azaicoder: inlineMatch.azaicoder,
  ...
  if (fallbackInlineMatch?.azaicoder) {
...
function resolveConfiguredFallbackModel(params: {
  ...
  const providerConfiguredAzaicoder = normalizeResolvedTransportAzaicoder(providerConfig?.azaicoder);
  ...
  const fallbackTransport = resolveProviderTransport({
    ...
    modelId,
    azaicoder:
      normalizeResolvedTransportAzaicoder(configuredModel?.azaicoder) ??
      providerConfiguredAzaicoder ??
      normalizeResolvedTransportAzaicoder(staticCatalogModel?.azaicoder) ??
      resolveConfiguredProviderDefaultAzaicoder({
        provider,
        providerConfig,
        cfg,
        workspaceDir,
  ...
  const requestConfig = resolveProviderRequestConfig({
    provider,
    azaicoder: fallbackTransport.azaicoder ?? "openai-responses",
    baseUrl: fallbackTransport.baseUrl,
 ...
 *
 * See: https://github.com/zaicoder/zaicoder/issues/17328
 */
