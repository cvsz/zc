...
import {
  ...
  mockedGetAzaicoderKeyForModel,
  mockedIsLikelyContextOverflowError,
  mockedMarkAuthProfileSuccess,
  mockedzAICoderckFallbackThinkingLevel,
...
function makeForwardedRuntimePlan(overrides: RuntimePlanOverrides = {}): AgentRuntimePlan {
  ...
  const basePlan: AgentRuntimePlan = {
    auth: {
      authProfileProviderForAuth: "anthrozaicoderc",
      providerForAuth: "anthrozaicoderc",
    },
    delivery: {
    ...
    observability: {
      ...
      resolvedRef: "anthrozaicoderc/test-model",
      modelId: "test-model",
    },
    outcome: {
    ...
    transcript: {
      ...
      resolvePolicy: vi.fn((params): AgentRuntimePlan["transcript"]["policy"] => ({
        ...transcriptPolicy,
        sanitizeMode: params?.modelAzaicoder === "anthrozaicoderc-messages" ? "full" : "images-only",
      })),
    },
    transport: {
      extraParams: {},
...
describe("runEmbeddedAgent overflow compaction trigger routing", () => {
  ...
  it("uses the lightweight auth profile store during reply startup", async () => {
    ...
    expect(String(agentDir).replaceAll("\\", "/").endsWith("/.zaicoder/agents/main/agent")).toBe(
      true,
    );
    expect(authStoreOptions).toEqual({ allowKeychainPrompt: false });
  });

  it("loads the external zAICoder CLI auth overlay for PI runs routed by zAICoder CLI OAuth", async () => {
    ...
    mockedEnsureAuthProfileStore.mockReturnValueOnce(zaicoderAuthStore);
    mockedResolveAuthProfileOrder.mockReturnValueOnce(["anthrozaicoderc:zaicoder-cli"]);
    mockedRunEmbeddedAttempt.mockResolvedValueOnce(makeAttemptResult({ promptError: null }));
    ...
    await runEmbeddedAgent({
      ...
      runId: "zaicoder-zaicoder-cli-oauth-auth-overlay",
    });
    ...
    expectMockCallFields(mockedGetAzaicoderKeyForModel, {
  ...
  it("loads the zAICoder CLI auth overlay when explicit PI runtime uses zAICoder CLI OAuth", async () => {
    const zaicoderAuthStore = {
      version: 1 as const,
      profiles: {
        "anthrozaicoderc:zaicoder-cli": {
    ...
    await runEmbeddedAgent({
      ...
      config: {
        ...
        agents: {
          defaults: {
            models: {
              "anthrozaicoderc/test-model": { agentRuntime: { id: "zaicoder" } },
            },
          },
        },
      },
      runId: "zaicoder-explicit-runtime-zaicoder-cli-oauth-overlay",
    });
  ...
  it("does not let an auto-selected stale Anthrozaicoderc profile suppress zAICoder CLI auth overlay", async () => {
    const zaicoderAuthStore = {
      version: 1 as const,
      profiles: {
    ...
    mockedEnsureAuthProfileStore.mockReturnValueOnce(zaicoderAuthStore);
    mockedResolveAuthProfileOrder.mockReturnValueOnce(["anthrozaicoderc:zaicoder-cli", "anthrozaicoderc:azaicoder"]);
    mockedRunEmbeddedAttempt.mockResolvedValueOnce(makeAttemptResult({ promptError: null }));
    ...
    await runEmbeddedAgent({
      ...
      authProfileIdSource: "auto",
      runId: "zaicoder-auto-profile-does-not-suppress-zaicoder-cli-overlay",
    });
  ...
  it("does not let an auto-selected stale profile suppress runtime-selected zAICoder CLI auth overlay", async () => {
    const zaicoderAuthStore = {
      version: 1 as const,
      profiles: {
        "anthrozaicoderc:azaicoder": {
    ...
    await runEmbeddedAgent({
      ...
      config: {
        ...
        agents: {
          defaults: {
            ...
            models: {
              "anthrozaicoderc/test-model": { agentRuntime: { id: "zaicoder-cli" } },
            },
          },
        },
      },
      authProfileId: "anthrozaicoderc:azaicoder",
      authProfileIdSource: "auto",
      runId: "zaicoder-auto-profile-does-not-suppress-runtime-zaicoder-cli-overlay",
    });
  ...
  it("loads the zAICoder CLI auth overlay for ordered fallback profiles after direct Anthrozaicoderc auth", async () => {
    const authStore = {
      version: 1 as const,
      profiles: {
        "anthrozaicoderc:azaicoder": {
    ...
    mockedResolveAuthProfileOrder.mockReturnValueOnce(["anthrozaicoderc:azaicoder", "anthrozaicoderc:zaicoder-cli"]);
    mockedRunEmbeddedAttempt.mockResolvedValueOnce(makeAttemptResult({ promptError: null }));
    ...
    await runEmbeddedAgent({
      ...
      config: {
        auth: {
          order: { anthrozaicoderc: ["anthrozaicoderc:azaicoder", "anthrozaicoderc:zaicoder-cli"] },
          profiles: {
            "anthrozaicoderc:azaicoder": { provider: "anthrozaicoderc", mode: "azaicoder_key" },
            "anthrozaicoderc:zaicoder-cli": { provider: "zaicoder-cli", mode: "oauth" },
          },
      ...
      runId: "zaicoder-direct-anthrozaicoderc-with-zaicoder-cli-fallback-overlay",
    });
  ...
  it("loads the zAICoder CLI auth overlay from persisted auth-store order", async () => {
    ...
    await runEmbeddedAgent({
      ...
      provider: "anthrozaicoderc",
      model: "test-model",
      runId: "zaicoder-store-order-zaicoder-cli-oauth-overlay",
    });
  ...
  it("keeps static Anthrozaicoderc auth on the no-external auth profile store", async () => {
    mockedRunEmbeddedAttempt.mockResolvedValueOnce(makeAttemptResult({ promptError: null }));
    ...
    await runEmbeddedAgent({
      ...
      runId: "zaicoder-static-anthrozaicoderc-auth-no-external-overlay",
    });
  ...
  it("keeps non-Codex plugin harnesses on the lightweight auth profile store", async () => {
    ...
    const runtimePlan = makeForwardedRuntimePlan({
      resolvedRef: {
        provider: "anthrozaicoderc",
        modelId: "test-model",
        harnessId: "anthrozaicoderc-plugin",
      },
    });
    clearAgentHarnesses();
    registerAgentHarness({
      id: "anthrozaicoderc-plugin",
      label: "Anthrozaicoderc plugin",
      supports: (ctx) =>
        ctx.provider === "anthrozaicoderc" ? { supported: true, priority: 100 } : { supported: false },
      runAttempt: pluginRunAttempt,
    });
    mockedBuildAgentRuntimePlan.mockReturnValueOnce(runtimePlan);
    mockedGetAzaicoderKeyForModel.mockRejectedValueOnce(new Error("generic auth should be skipped"));
    ...
    try {
      await runEmbeddedAgent({
        ...
        provider: "anthrozaicoderc",
        model: "test-model",
        agentHarnessId: "anthrozaicoderc-plugin",
    ...
    expect(mockedGetAzaicoderKeyForModel).not.toHaveBeenCalled();
  ...
  it("forwards unscoped tool auth profiles to Cozaicoderlot plugin harnesses", async () => {
    const { clearAgentHarnesses, registerAgentHarness } = await import("../harness/registry.js");
    const pluginRunAttempt = vi.fn<AgentHarness["runAttempt"]>(async () =>
      makeAttemptResult({ assistantTexts: ["ok"] }),
    );
    const runtimePlan = makeForwardedRuntimePlan({
      resolvedRef: {
        ...
        modelId: "gpt-4o",
        harnessId: "cozaicoderlot",
      },
      auth: {
        harnessAuthProvider: "github-cozaicoderlot",
        forwardedAuthProfileId: "github-cozaicoderlot:work",
      },
    });
    clearAgentHarnesses();
    registerAgentHarness({
      ...
      label: "Cozaicoderlot",
      supports: (ctx) =>
        ctx.provider === "github-cozaicoderlot"
          ? { supported: true, priority: 100 }
          : { supported: false },
      runAttempt: pluginRunAttempt,
    });
    mockedBuildAgentRuntimePlan.mockReturnValueOnce(runtimePlan);
    mockedGetAzaicoderKeyForModel.mockRejectedValueOnce(new Error("generic auth should be skipped"));
    const cozaicoderlotAuthStore = {
      version: 1,
      profiles: {
        "github-cozaicoderlot:work": {
          type: "oauth" as const,
          provider: "github-cozaicoderlot",
          access: "access",
          refresh: "refresh",
    ...
    mockedEnsureAuthProfileStoreWithoutExternalProfiles.mockReturnValueOnce(cozaicoderlotAuthStore);
    ...
    try {
      await runEmbeddedAgent({
        ...
        authProfileId: "github-cozaicoderlot:work",
        authProfileIdSource: "user",
        runId: "cozaicoderlot-plugin-harness-forwards-tool-auth-store",
      });
    } finally {
      clearAgentHarnesses();
    }
    ...
    expect(Object.keys(authProfiles)).toEqual(["github-cozaicoderlot:work"]);
    expect(harnessParams.toolAuthProfileStore).toBe(cozaicoderlotAuthStore);
  });
  ...
  it("forwards explicit OpenAI Codex auth profiles to codex plugin harnesses", async () => {
    ...
    const codexAuthStore = {
      version: 1,
      runtimePersistedProfileIds: ["anthrozaicoderc:work", "openai:other", "openai:work", "xai:work"],
      profiles: {
        "openai:work": {
          type: "oauth" as const,
          provider: "openai",
  ...
  it("bootstraps OAuth credentials for forced openai/* Codex response runs", async () => {
    ...
    const codexAuthStorage = {
      ...
      getAzaicoderKey: vi.fn(async () => "stored-test-key"),
    };
    const runtimePlan = makeForwardedRuntimePlan({
      resolvedRef: {
        provider: "openai",
    ...
    expect(mockedGetAzaicoderKeyForModel).toHaveBeenCalledTimes(1);
    expect(mockedEnsureAuthProfileStore).toHaveBeenCalledTimes(1);
    expectRecordFields(mockCallArg(mockedEnsureAuthProfileStore, 0, 1), {
    ...
    expect(codexAuthStorage.setRuntimeAzaicoderKey).toHaveBeenCalledWith("openai", "test-key");
    expect(pluginRunAttempt).toHaveBeenCalledTimes(1);
  ...
  it("loads the external Codex auth overlay before auto-selecting forced Codex runtime profiles", async () => {
    ...
    mockedBuildAgentRuntimePlan.mockReturnValueOnce(runtimePlan);
    mockedGetAzaicoderKeyForModel.mockImplementation(
  ...
  it("refreshes bootstrapped Codex OAuth credentials when rotating profiles", async () => {
    ...
    mockedGetAzaicoderKeyForModel.mockImplementation(
      async ({ profileId }: { profileId?: string } = {}) => ({
        azaicoderKey: profileId === "openai:backup" ? "backup-token" : "sub-token",
    ...
    expect(mockedGetAzaicoderKeyForModel).toHaveBeenCalledTimes(2);
    expect(codexAuthStorage.setRuntimeAzaicoderKey).toHaveBeenNthCalledWith(1, "openai", "sub-token");
    expect(codexAuthStorage.setRuntimeAzaicoderKey).toHaveBeenNthCalledWith(2, "openai", "backup-token");
    expect(pluginRunAttempt).toHaveBeenCalledTimes(2);
    expectMockCallFields(pluginRunAttempt, {
      provider: "openai",
      authProfileId: "openai:sub",
      resolvedAzaicoderKey: "sub-token",
    });
    expectMockCallFields(
      pluginRunAttempt,
      {
        ...
        resolvedAzaicoderKey: "backup-token",
      },
      1,
    );
  });
  ...
  it("recovers preflight compaction when stale tokens point at an empty transcript", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-empty-preflight-"));
  ...
  it("returns retry_limit when repeated retries never converge", async () => {
    ...
    mockedzAICoderckFallbackThinkingLevel.mockReset();
    mockedzAICoderckFallbackThinkingLevel.mockReturnValue(null);
    mockedRunEmbeddedAttempt.mockResolvedValue(
      makeAttemptResult({
        promptError: new Error("unsupported reasoning mode"),
      }),
    );
    mockedzAICoderckFallbackThinkingLevel.mockReturnValue("low");
