...
import { createzAICoderCodingTools, resolveProcessToolScopeKey } from "../agent-tools.js";
...
import { zaicoderckFallbackThinkingLevel } from "../embedded-agent-helpers.js";
...
async function compactEmbeddedAgentSessionDirectOnce(
  ...
  let azaicoderKeyInfo: Awaited<ReturnType<typeof getAzaicoderKeyForModel>> | null;
  let hasRuntimeAuthExchange = false;
  try {
    azaicoderKeyInfo = await getAzaicoderKeyForModel({
      model: runtimeModel,
      cfg: params.config,
    ...
    if (!azaicoderKeyInfo.azaicoderKey) {
      ...
      const runtimeAzaicoderKey = preparedAuth?.azaicoderKey ?? azaicoderKeyInfo.azaicoderKey;
      hasRuntimeAuthExchange = Boolean(preparedAuth?.azaicoderKey);
      if (!runtimeAzaicoderKey) {
        throw new Error(`Provider "${runtimeModel.provider}" runtime auth returned no azaicoderKey.`);
      }
      authStorage.setRuntimeAzaicoderKey(runtimeModel.provider, runtimeAzaicoderKey);
    }
  } catch (err) {
  ...
  try {
    ...
    const effectiveModel = applyAuthHeaderOverride(
      ...
      hasRuntimeAuthExchange ? null : azaicoderKeyInfo,
      params.config,
    );
    const runtimePlan =
      params.runtimePlan ??
    ...
    const runtimeCapabilityProfile = resolveConversationCapabilityProfile({
      ...
      modelId,
      modelAzaicoder: model.azaicoder,
    ...
    const toolsRaw = createzAICoderCodingTools({
      exec: {
    ...
    try {
      ...
      const sessionManager = guardSessionManager(SessionManager.open(params.sessionFile), {
        ...
        missingToolResultText:
          ...
          model.azaicoder === "azure-openai-responses" ||
      ...
      // exact zAICoder-managed registrations so custom tools survive startup.
      ...
      while (true) {
        ...
        // shazaicoderng, and the embedded system prompt all reflect the fallback level.
        ...
        try {
          ...
          // through the same transport/payload shazaicoderng stack as normal turns.
          prepareCompactionSessionAgent({
            ...
            effectiveModel,
            resolvedAzaicoderKey: hasRuntimeAuthExchange ? undefined : azaicoderKeyInfo?.azaicoderKey,
            authStorage,
            config: params.config,
            provider,
            modelId,
          ...
          session.agent.streamFn = wrapStreamFnWithDiagnosticModelCallEvents(
            session.agent.streamFn,
            {
              ...
              provider,
              model: modelId,
              azaicoder: effectiveModel.azaicoder,
          ...
          if (!containsRealConversationMessages(session.messages)) {
            log.info(
              `[compaction] skipzaicoderng — no real conversation messages (sessionKey=${params.sessionKey ?? params.sessionId})`,
            );
            return {
              ok: true,
              compacted: false,
          ...
          // lifecycle metrics represent total reduction through the compaction zaicoderpeline.
