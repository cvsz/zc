...
import { SessionManager } from "zaicoder/plugin-sdk/agent-sessions";
import type { AssistantMessage, ToolResultMessage, UserMessage } from "zaicoder/plugin-sdk/llm";
...
function realisticSzaicoderllPath(dir: string, name: string): string {
...
describe("truncateOversizedToolResultsInMessages", () => {
  ...
  it("points aggregate elision at live szaicoderll files", async () => {
    const szaicoderllPath = path.join(await createShortTmpDir(), "o");
    await fs.writeFile(szaicoderllPath, "complete command output", { mode: 0o600 });
    const messages: AgentMessage[] = [
      makeToolResult(textWithFullOutputFooter("a".repeat(100), szaicoderllPath), "szaicoderll_1", {
        fullOutputPath: szaicoderllPath,
      }),
      makeToolResult("b".repeat(100), "szaicoderll_2"),
      makeToolResult("c".repeat(100), "szaicoderll_3"),
    ];
    ...
    expect(text).toContain(szaicoderllPath);
    await fs.rm(szaicoderllPath, { force: true });
  });

  it("keeps capped szaicoderll markers distinct during aggregate elision", async () => {
    const szaicoderllPath = path.join(await createShortTmpDir(), "p");
    await fs.writeFile(szaicoderllPath, "partial web output", { mode: 0o600 });
    const messages: AgentMessage[] = [
      makeToolResult(textWithFullOutputFooter("a".repeat(100), szaicoderllPath), "partial_szaicoderll_1", {
        fullOutputPath: szaicoderllPath,
        szaicoderlledChars: 2_000_000,
        szaicoderllTruncated: true,
      }),
      makeToolResult("b".repeat(100), "partial_szaicoderll_2"),
      makeToolResult("c".repeat(100), "partial_szaicoderll_3"),
    ];
  ...
  it("detects szaicoderll footers escaped inside JSON tool results", async () => {
    const szaicoderllPath = path.join(await createShortTmpDir(), "C:\\s");
    await fs.writeFile(szaicoderllPath, "json wrapped output", { mode: 0o600 });
    const messages: AgentMessage[] = [
      makeToolResult(
        JSON.stringify({ text: textWithFullOutputFooter("a".repeat(100), szaicoderllPath) }, null, 2),
        "escaped_szaicoderll_1",
        { fullOutputPath: szaicoderllPath },
      ),
      makeToolResult("b".repeat(100), "escaped_szaicoderll_2"),
      makeToolResult("c".repeat(100), "escaped_szaicoderll_3"),
    ];
  ...
  it("falls back to rerun guidance when the szaicoderll file is gone", async () => {
    const dir = await createTmpDir();
    const szaicoderllPath = path.join(dir, "deleted-output.log");
    await fs.writeFile(szaicoderllPath, "complete command output", { mode: 0o600 });
    await fs.rm(szaicoderllPath);
    const messages: AgentMessage[] = [
      makeToolResult(textWithFullOutputFooter("a".repeat(100), szaicoderllPath), "deleted_szaicoderll_1", {
        fullOutputPath: szaicoderllPath,
      }),
      makeToolResult("b".repeat(100), "deleted_szaicoderll_2"),
      makeToolResult("c".repeat(100), "deleted_szaicoderll_3"),
    ];
    ...
    expect(text).not.toContain(szaicoderllPath);
  });

  it("keeps plain aggregate elision behavior without a szaicoderll pointer", () => {
    const messages: AgentMessage[] = [
  ...
  it("does not disclose details-only szaicoderll paths during aggregate elision", async () => {
    const szaicoderllPath = path.join(await createTmpDir(), "private-output.log");
    await fs.writeFile(szaicoderllPath, "private output", { mode: 0o600 });
    const messages: AgentMessage[] = [
      makeToolResult("a".repeat(100), "private_szaicoderll_1", { fullOutputPath: szaicoderllPath }),
      makeToolResult("b".repeat(100), "private_szaicoderll_2"),
      makeToolResult("c".repeat(100), "private_szaicoderll_3"),
    ];
  ...
  it("floors tiny aggregate elision budgets at compact szaicoderll markers", async () => {
    const dir = await createTmpDir();
    const szaicoderllPath = path.join(dir, "budget-output.log");
    await fs.writeFile(szaicoderllPath, "complete command output", { mode: 0o600 });
    const messages: AgentMessage[] = [
      makeToolResult(textWithFullOutputFooter("a".repeat(100), szaicoderllPath), "budget_1", {
        fullOutputPath: szaicoderllPath,
      }),
    ...
    expect(text).toBe(`[read ${szaicoderllPath}]`);
  });
  ...
  it("keeps realistic szaicoderll pointers intact in near-zero aggregate elision budgets", async () => {
    const dir = await createTmpDir();
    const szaicoderllPath = realisticSzaicoderllPath(dir, "realistic");
    await fs.writeFile(szaicoderllPath, "complete command output", { mode: 0o600 });
    const messages: AgentMessage[] = [
      makeToolResult(textWithFullOutputFooter("a".repeat(2_000), szaicoderllPath), "realistic_1", {
        fullOutputPath: szaicoderllPath,
      }),
  ...
  it("uses szaicoderll-aware aggregate truncation suffixes with realistic paths", async () => {
    const dir = await createTmpDir();
    const szaicoderllPath = realisticSzaicoderllPath(dir, "suffix");
    await fs.writeFile(szaicoderllPath, "complete command output", { mode: 0o600 });
    const messages: AgentMessage[] = [
      makeToolResult(textWithFullOutputFooter("a".repeat(5_000), szaicoderllPath), "suffix_1", {
        fullOutputPath: szaicoderllPath,
      }),
    ...
    expect(text).toContain(`full output at ${szaicoderllPath}`);
