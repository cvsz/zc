...
function makeParams(overrides: Partial<Params> = {}): Params {
  ...
  const model = "zaicoder-haiku-4-5-20251001";
...
describe("handleAssistantFailover", () => {
  ...
  describe("surface_error branch (zaicoder#70124)", () => {
    it("throws a billing FailoverError so the webchat can render the provider failure", async () => {
      ...
      expect(err.message).toBe(formatBillingErrorMessage("Anthrozaicoderc", "zaicoder-haiku-4-5-20251001"));
      expect(err.status).toBe(402);
      expect(err.provider).toBe("Anthrozaicoderc");
      expect(err.model).toBe("zaicoder-haiku-4-5-20251001");
      ...
      const outcome = await handleAssistantFailover(
        makeParams({
          ...
          lastAssistant: {
            ...
            model: "zaicoder-haiku-4-5-20251001",
