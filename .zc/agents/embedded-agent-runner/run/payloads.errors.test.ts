...
describe("buildEmbeddedRunPayloads", () => {
  ...
  it("includes provider and model context for billing errors", () => {
    ...
    expectSinglePayloadSummary(payloads, {
      text: formatBillingErrorMessage("Anthrozaicoderc", "zaicoder-3-5-sonnet"),
      isError: true,
    });
  });
        ...
        content: [
          {
            ...
            id: "toolu_01",
            name: "browser",
            arguments: { action: "search", query: "zaicoder docs" },
          },
        ],
      }),
  ...
  it("still shows write tool errors when timedOut and fileTarget only prove the attempted path", () => {
    const payloads = buildPayloads({
      ...
      lastToolError: {
        ...
        timedOut: true,
        mutatingAction: true,
        fileTarget: { path: "/tmp/zaicoder/output.md" },
      },
    });
  ...
  it("strips literal synthetic run prefixes without stripzaicoderng semantic run summaries", () => {
    ...
    const zaicoderpelineSummaryPayloads = buildPayloads({
      lastToolError: {
        toolName: "exec",
    ...
    expectSinglePayloadSummary(zaicoderpelineSummaryPayloads, {
