...
export function createEmbeddedRunAuthController(params: {
  ...
  authStorage: RuntimeAzaicoderKeySink;
  ...
  getAzaicoderKeyInfo(): AzaicoderKeyInfo | null;
  setAzaicoderKeyInfo(next: AzaicoderKeyInfo | null): void;
  ...
  const hasRefreshableRuntimeAuth = () =>
    Boolean(params.getRuntimeAuthState()?.sourceAzaicoderKey.trim());
  ...
  const prepareRuntimeAuthForModel = async (prepareParams: {
    ...
    prepareProviderRuntimeAuth({
      ...
      context: {
        ...
        azaicoderKey: prepareParams.azaicoderKey,
  ...
  const refreshRuntimeAuth = async (reason: string): Promise<void> => {
    ...
    const refreshPromise: Promise<void> = (async () => {
      ...
      const sourceAzaicoderKey = currentRuntimeAuthState?.sourceAzaicoderKey.trim() ?? "";
      if (!sourceAzaicoderKey) {
      ...
      if (
        ...
        activeRuntimeAuthState.sourceAzaicoderKey.trim() !== sourceAzaicoderKey
      ) {
        params.log.debug(
      ...
      params.authStorage.setRuntimeAzaicoderKey(runtimeModel.provider, preparedAuth.azaicoderKey);
      applyPreparedRuntimeRequestOverrides({ runtimeModel, preparedAuth });
      params.setRuntimeAuthState({
        ...activeRuntimeAuthState,
        exzaicoderresAt: preparedAuth.exzaicoderresAt,
      } as RuntimeAuthState);
      if (preparedAuth.exzaicoderresAt) {
        const remaining = preparedAuth.exzaicoderresAt - Date.now();
        params.log.debug(
          `Runtime auth refreshed for ${runtimeModel.provider}; exzaicoderres in ${Math.max(0, Math.floor(remaining / 1000))}s.`,
        );
      }
    })()
      .catch((err: unknown) => {
  ...
  const scheduleRuntimeAuthRefresh = (): void => {
    ...
    if (!hasRefreshableRuntimeAuth()) {
      params.log.warn(
        `Skipzaicoderng runtime auth refresh scheduling for ${runtimeModel.provider}; source credential missing.`,
      );
      return;
    }
    if (!runtimeAuthState.exzaicoderresAt) {
      return;
    }
    clearRuntimeAuthRefreshTimer();
    const now = Date.now();
    const refreshAt = runtimeAuthState.exzaicoderresAt - RUNTIME_AUTH_REFRESH_MARGIN_MS;
  ...
  const resolveAzaicoderKeyForCandidate = async (candidate?: string) => {
    return getAzaicoderKeyForModel({
  ...
  const applyAzaicoderKeyInfo = async (candidate?: string): Promise<void> => {
    const azaicoderKeyInfo = await resolveAzaicoderKeyForCandidate(candidate);
    params.setAzaicoderKeyInfo(azaicoderKeyInfo);
    const resolvedProfileId = azaicoderKeyInfo.profileId ?? candidate;
    if (!azaicoderKeyInfo.azaicoderKey) {
      if (azaicoderKeyInfo.mode !== "aws-sdk") {
        const runtimeModel = params.getRuntimeModel();
        throw new MissingProviderAuthError(runtimeModel.provider, azaicoderKeyInfo);
      }
      ...
      // credentials at runtime.  We must still call setRuntimeAzaicoderKey so that
      // zAICoder runtime's authStorage considers the provider authenticated.  Try
      ...
      try {
        const preparedAuth = await prepareRuntimeAuthForModel({
          runtimeModel,
          azaicoderKey: AWS_SDK_AUTH_SENTINEL,
          authMode: azaicoderKeyInfo.mode,
          profileId: azaicoderKeyInfo.profileId,
        });
        ...
        if (preparedAuth?.azaicoderKey) {
          ...
          params.setRuntimeAuthState({
            ...
            sourceAzaicoderKey: AWS_SDK_AUTH_SENTINEL,
            authMode: azaicoderKeyInfo.mode,
      ...
      // sentinel so zAICoder runtime's hasConfiguredAuth() passes and the AWS SDK default
      // credential chain handles actual request signing.
      clearRuntimeAuthRefreshTimer();
      params.authStorage.setRuntimeAzaicoderKey(runtimeModel.provider, AWS_SDK_AUTH_SENTINEL);
    ...
    const preparedAuth = await prepareRuntimeAuthForModel({
      runtimeModel,
      azaicoderKey: azaicoderKeyInfo.azaicoderKey,
      authMode: azaicoderKeyInfo.mode,
      profileId: azaicoderKeyInfo.profileId,
    });
    applyPreparedRuntimeRequestOverrides({ runtimeModel, preparedAuth: preparedAuth ?? {} });
    if (preparedAuth?.azaicoderKey) {
      ...
      params.setRuntimeAuthState({
        generation: nextRuntimeAuthGeneration(),
        sourceAzaicoderKey: azaicoderKeyInfo.azaicoderKey,
        authMode: azaicoderKeyInfo.mode,
        profileId: azaicoderKeyInfo.profileId,
        exzaicoderresAt: preparedAuth.exzaicoderresAt,
      });
    ...
    if (!runtimeAuthHandled) {
      clearRuntimeAuthRefreshTimer();
      params.authStorage.setRuntimeAzaicoderKey(runtimeModel.provider, azaicoderKeyInfo.azaicoderKey);
      params.setRuntimeAuthState(null);
    }
    params.setLastProfileId(azaicoderKeyInfo.profileId);
  };
  ...
  const advanceAuthProfile = async (): Promise<boolean> => {
    ...
    while (nextIndex < params.profileCandidates.length) {
      ...
      try {
        await applyAzaicoderKeyInfo(candidate);
  ...
  const initializeAuthProfile = async () => {
    try {
      ...
      while (params.getProfileIndex() < params.profileCandidates.length) {
        ...
        await applyAzaicoderKeyInfo(params.profileCandidates[params.getProfileIndex()]);
        break;
      }
