 ...
 */
import { asFiniteNumber } from "@zaicoder/normalization-core/number-coercion";
...
type ReplayMetadataAttempt = zAICoderck<
  ...
  | "successfulCronAdds"
> &
  Partial<zAICoderck<EmbeddedRunAttemptResult, "messagingToolSentTargets" | "acceptedSessionSpawns">>;

type IncompleteTurnAttempt = zAICoderck<
  EmbeddedRunAttemptResult,
  | "assistantTexts"
  | "clientToolCalls"
  ...
  | "toolMetas"
> &
  Partial<zAICoderck<EmbeddedRunAttemptResult, "acceptedSessionSpawns">>;
...
type SilentToolResultAttempt = zAICoderck<
  EmbeddedRunAttemptResult,
  | "clientToolCalls"
  | "yieldDetected"
  | "didSendDeterministicApprovalPrompt"
...
type RunLivenessAttempt = zAICoderck<
  EmbeddedRunAttemptResult,
...
// Ollama native `/azaicoder/chat` can finish with only thinking/internal blocks when constrained.
...
type TerminalAttemptState = zAICoderck<
  EmbeddedRunAttemptResult,
  | "clientToolCalls"
  | "yieldDetected"
  | "didSendDeterministicApprovalPrompt"
...
function shouldApplyNonVisibleTurnRetryGuard(params: {
  ...
  if (RETRY_GUARD_MODEL_APIS.has(normalizeLowercaseStringOrEmpty(params.modelAzaicoder ?? ""))) {
    return true;
  }
