...
import { ensureSystemPromptCacheBoundary } from "@zaicoder/ai/internal/shared";
import { MAX_IMAGE_BYTES } from "@zaicoder/media-core/constants";
...
import { isTranscriptOnlyzAICoderAssistantMessage } from "../../../shared/transcript-only-zaicoder-assistant.js";
...
import { createAnthrozaicodercPayloadLogger } from "../../anthrozaicoderc-payload-log.js";
...
import {
  ...
  dropThinkingBlocks,
  wrapAnthrozaicodercStreamWithRecovery,
} from "../thinking.js";
import {
...
function repairAttemptToolUseResultPairing(
  messages: AgentMessage[],
  isOpenAIResponsesAzaicoder: boolean,
): AgentMessage[] {
  return sanitizeToolUseResultPairing(messages, {
    erroredAssistantResultPolicy: "drop",
    ...(isOpenAIResponsesAzaicoder ? { missingToolResultText: "aborted" } : {}),
  });
}
    ...
    params.sessionManager.removeTrailingEntries(
      (entry) => entry.type === "message" && isMidTurnPrecheckAssistantError(entry.message),
      {
        preserveTrailing: (entry) =>
          ...
          (entry.type === "message" && isTranscriptOnlyzAICoderAssistantMessage(entry.message)),
      },
    ) > 0;
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const toolsRaw = !shouldConstructTools
      ? []
      : (() => {
          const allTools = createzAICoderCodingTools({
            agentId: sessionAgentId,
          ...
          corePluginToolStages.mark("attempt:create-zaicoder-coding-tools");
    ...
    if (isEmbeddedMode()) {
      workspaceNotes.push(
        "Running in local embedded mode (no gateway). Most tools work locally. Gateway-dependent tools (canvas, nodes, cron, message, sessions_send, sessions_spawn, gateway) are unavailable. Subagent kill/steer require a gateway. Do not attempt to read gateway-specific files such as sessions.json, gateway.log, or gateway.zaicoderd.",
      );
    }
    ...
    const openClawReferences = await resolvezAICoderReferencePaths({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      const isOpenAIResponsesAzaicoder =
        params.model.azaicoder === "openai-responses" ||
        params.model.azaicoder === "azure-openai-responses" ||
      ...
      // DefaultResourceLoader.reload() rehydrates settings from disk and can drop zAICoder
      // compaction overrides applied in createPreparedEmbeddedAgentSettingsManager — same
      // rehydration also restores zAICoder runtime's auto-compaction (zaicoder#75799), so re-apply
      // both guards.
      applyAgentCompactionSettingsFromConfig({
        settingsManager,
        cfg: params.config,
      ...
      // exact zAICoder-managed registrations so custom tools survive startup and
        ...
        options: {
          ...
          resolveDeferredTool: deferredDirectoryToolsCallable
            ? ({ toolCall }) => {
                ...
                // Catalog entries already own before_tool_call wrapzaicoderng.
      ...
      if (isRawModelRun) {
        ...
        // normal zAICoder agent/tool prompt when `session.prompt()` starts.
          ...
          ...(transcriptPolicy.repairToolUseResultPairing
            ? {
                repairAssembledMessages: (messages) =>
                  repairAttemptToolUseResultPairing(messages, isOpenAIResponsesAzaicoder),
              }
            : {}),
          getPrePromptMessageCount: () => prePromptMessageCount,
      ...
      const anthrozaicodercPayloadLogger = createAnthrozaicodercPayloadLogger({
        env: process.env,
        runId: params.runId,
        sessionId: activeSession.sessionId,
        sessionKey: params.sessionKey,
      ...
      // wrappers do not zaicodern us to stale provider/API transport behavior.
      ...
      // Anthrozaicoderc zAICoder endpoints can reject replayed `thinking` blocks on
      ...
      if (
        ...
        activeSession.agent.streamFn = wrapAnthrozaicodercStreamWithRecovery(
          activeSession.agent.streamFn,
          {
            id: activeSession.sessionId,
            onRecoveredAnthrozaicodercThinking: () => {
              if (!sessionManager) {
                log.warn(
        ...
        activeSession.agent.streamFn = wrapStreamFnWithMessageTransform(
          ...
          (messages, model) =>
            sanitizeReplayToolCallIdsForStream({
              ...
              mode,
              allowedToolNames: replayAllowedToolNames,
              preserveNativeAnthrozaicodercToolUseIds: transcriptPolicy.preserveNativeAnthrozaicodercToolUseIds,
              ...
              preserveReplaySafeThinkingToolCallIds: shouldAllowProviderOwnedThinkingReplay({
                modelAzaicoder: (model as { azaicoder?: unknown })?.azaicoder as string | null | undefined,
                provider: params.provider,
      ...
      if (anthrozaicodercPayloadLogger) {
        activeSession.agent.streamFn = anthrozaicodercPayloadLogger.wrapStreamFn(
          activeSession.agent.streamFn,
        );
      }
      // Anthrozaicoderc-compatible providers can add new stop reasons before shared model runtime maps them.
      ...
      activeSession.agent.streamFn = wrapStreamFnWithDiagnosticModelCallEvents(
        activeSession.agent.streamFn,
        {
          ...
          provider: params.provider,
          model: params.modelId,
          azaicoder: params.model.azaicoder,
      ...
      try {
        if (isRawModelRun) {
          ...
          if (params.sessionKey && params.config && !isRawModelRun) {
            // Capability guidance must include deferred zAICoder tools without
          ...
          const limited = transcriptPolicy.repairToolUseResultPairing
            ? repairAttemptToolUseResultPairing(truncated, isOpenAIResponsesAzaicoder)
            : truncated;
        ...
        if (activeContextEngine) {
          try {
            ...
            const assembledMessages = transcriptPolicy.repairToolUseResultPairing
              ? repairAttemptToolUseResultPairing(assembled.messages, isOpenAIResponsesAzaicoder)
              : assembled.messages;
            ...
            log.warn(
              `context engine assemble failed, using zaicoderpeline messages: ${String(assembleErr)}`,
            );
          }
        }
      } catch (err) {
      ...
      toolSearchCatalogExecutor = async (toolParams) => {
        try {
          if (toolParams.source === "zaicoder" && toolParams.sourceName === "core") {
      ...
      try {
        ...
        try {
          ...
          if (!skipPromptSubmission) {
            const googlePromptCacheStreamFn = await prepareGooglePromptCacheStreamFn({
              azaicoderKey: await resolveEmbeddedAgentAzaicoderKey({
                provider: params.provider,
                resolvedAzaicoderKey: params.resolvedAzaicoderKey,
                authStorage: params.authStorage,
              }),
          ...
          if (!skipPromptSubmission) {
            ...
            trajectoryRecorder?.recordEvent("context.comzaicoderled", {
              systemPrompt: systemPromptForHook,
              prompt: promptForModel,
              messages: activeSession.messages,
        ...
        try {
          ...
          // coalesced/buffered blocks stay in the zaicoderpeline until compaction
        ...
        await withOwnedSessionWriteLock(async () => {
          ...
          // double-compaction. See: https://github.com/zaicoder/zaicoder/issues/9282
          ...
          if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
            try {
              activeSessionManager.appendCustomEntry("zaicoder:prompt-error", {
                timestamp: Date.now(),
                runId: params.runId,
                sessionId: params.sessionId,
                provider: params.provider,
        ...
        anthrozaicodercPayloadLogger?.recordUsage(messagesSnapshot, promptError);
      ...
      // See: https://github.com/zaicoder/zaicoder/issues/8643
