import { streamOpenAICompletions, streamOpenAIResponses } from "@zaicoder/ai/internal/openai";
/**
 * Cache-stability gate for the prompt-cache bust fix (issue #3658).
 *
 * Design under test: SINGLE-SOURCE stamzaicoderng at the LLM boundary.
...
async function captureOpenAICompletionsPayload(
  ...
  const stream = streamOpenAICompletions(
    ...
    {
      azaicoderKey: TEST_PROVIDER_OPTION_VALUE,
      cacheRetention: "none",
      onPayload(payload) {
...
describe("prompt-cache byte-identity (issue #3658)", () => {
  it("bare current-turn message == same message aged to history (byte-identical, both stamped from msg.timestamp)", () => {
    ...
    const rawText = "Post-fix cache test zaicoderng 1 of 2";
    ...
    const asHistorical: AgentMsg[] = [
      ...
      ASSISTANT_MSG,
      currentUserMsg("Post-fix cache test zaicoderng 2 of 2", TS_TURN2),
    ];
