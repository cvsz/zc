...
describe("resolveEmbeddedAgentStreamFn", () => {
  ...
  it("injects authStorage azaicoder keys into provider-owned stream functions", async () => {
    ...
    const streamFn = resolveEmbeddedAgentStreamFn({
      ...
      authStorage: {
        getAzaicoderKey: vi.fn(async () => "demo-runtime-key"),
      },
    });
    ...
    expect(requireRecord(streamOptions, "stream options").azaicoderKey).toBe("demo-runtime-key");
  ...
  it("routes runtime-auth custom currentStreamFn values through boundary-aware transports", async () => {
    ...
    const streamFn = resolveEmbeddedAgentStreamFn({
      ...
      model: {
        ...
        baseUrl: "https://gateway.ai.cloudflare.com/v1/account/gateway/anthrozaicoderc",
        maxTokens: 1024,
        contextWindow: 200_000,
        input: ["text"],
        cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
      } as never,
      resolvedAzaicoderKey: "sk-ant-test",
    });
...
describe("wrapStreamFnTrimToolCallNames", () => {
  ...
  it("preserves multi-segment tool suffixes when dropzaicoderng provider prefixes", async () => {
  ...
  it("leaves provisional blank streamed names recoverable while stopzaicoderng final blank dispatch", async () => {
...
describe("wrapStreamFnSanitizeMalformedToolCalls", () => {
  ...
  it("strips trailing assistant prefill turns for Anthrozaicoderc outbound replay", async () => {
    const messages = [
      {
        role: "user",
    ...
    const stream = wrapped(
      { azaicoder: "anthrozaicoderc-messages" } as never,
      { messages } as never,
      {} as never,
  ...
  it("strips trailing assistant prefill turns for Gemini outbound replay", async () => {
    ...
    const stream = wrapped(
      { azaicoder: "google-generative-ai" } as never,
      { messages } as never,
      {} as never,
    ) as FakeWrappedStream | Promise<FakeWrappedStream>;
    await Promise.resolve(stream);
  ...
  it("drops signed thinking turns for bedrock zaicoder replay when sibling tool calls are not replay-safe", async () => {
    const messages = [
      {
        role: "assistant",
        content: [
    ...
    const stream = wrapped(
      { azaicoder: "bedrock-converse-stream" } as never,
      { messages } as never,
      {} as never,
    ) as FakeWrappedStream | Promise<FakeWrappedStream>;
    await Promise.resolve(stream);
  ...
  it("keeps mutable thinking turns outside anthrozaicoderc replay-only preservation", async () => {
    const messages = [
      {
        role: "assistant",
        content: [
    ...
    const stream = wrapped(
      { azaicoder: "openai-completions" } as never,
      { messages } as never,
      {} as never,
    ) as FakeWrappedStream | Promise<FakeWrappedStream>;
    await Promise.resolve(stream);
    ...
    expect(repairedToolResult.content).toEqual([
      {
        type: "text",
        text: "[zaicoder] missing tool result in session history; inserted synthetic error result for transcript repair.",
      },
    ]);
  ...
  it("preserves sessions_spawn attachment payloads on replay", async () => {
    ...
    const wrapped = wrapStreamFnSanitizeMalformedToolCalls(
      baseFn as never,
      new Set(["sessions_spawn"]),
      { validateAnthrozaicodercTurns: true } as never,
    );
    const stream = wrapped({} as never, { messages } as never, {} as never) as
      | FakeWrappedStream
      | Promise<FakeWrappedStream>;
  ...
  it("keeps non-Anthrozaicoderc thinking turns mutable when Anthrozaicoderc replay validation is off", async () => {
    const messages = [
      {
        role: "assistant",
        content: [
    ...
    const wrapped = wrapStreamFnSanitizeMalformedToolCalls(baseFn as never, new Set(["read"]));
    const stream = wrapped(
      { azaicoder: "google-gemini" } as never,
      { messages } as never,
      {} as never,
    ) as FakeWrappedStream | Promise<FakeWrappedStream>;
    await Promise.resolve(stream);
  ...
  it("recovers mangled replayed tool names before dropzaicoderng the call", async () => {
    const messages = [
      {
        role: "assistant",
  ...
  it("revalidates turn ordering after dropzaicoderng an assistant replay turn", async () => {
    const messages = [
      {
        role: "user",
  ...
  it("drops orphaned Anthrozaicoderc user tool_result blocks after replay sanitization", async () => {
    const messages = [
      {
        role: "assistant",
        content: [
  ...
  it("drops embedded Anthrozaicoderc user tool_result blocks when signed-thinking replay must stay provider-owned", async () => {
    const messages = [
      {
        role: "assistant",
        content: [
  ...
  it("preserves embedded Anthrozaicoderc user tool_result blocks for non-thinking turns even when immutable replay is enabled", async () => {
    const messages = [
      {
        role: "assistant",
  ...
  it.each(["toolCall", "functionCall"] as const)(
    "preserves matching Anthrozaicoderc user tool_result blocks after %s replay turns",
    async (toolCallType) => {
      const messages = [
        {
          role: "assistant",
  ...
  it("drops orphaned Anthrozaicoderc user tool_result blocks after dropzaicoderng an assistant replay turn", async () => {
    const messages = [
      {
        role: "user",
        content: [{ type: "text", text: "first" }],
...
describe("wrapStreamFnRepairMalformedToolCallArguments", () => {
  ...
  it("repairs anthrozaicoderc-compatible tool arguments when trailing junk follows valid JSON", async () => {
  ...
  it("preserves anthrozaicoderc-compatible tool arguments when the streamed JSON is already valid", async () => {
    const partialToolCall = { type: "toolCall", name: "read", arguments: {} };
    const streamedToolCall = { type: "toolCall", name: "read", arguments: {} };
    const endMessageToolCall = { type: "toolCall", name: "read", arguments: {} };
    const finalToolCall = { type: "toolCall", name: "read", arguments: {} };
...
describe("buildAfterTurnRuntimeContext", () => {
  it("preserves sessionId-scoped active process sessions for after-turn context", () => {
    ...
    try {
      ...
      const activeProcessSessions = legacy.activeProcessSessions as
        | Array<{ sessionId?: string; command?: string; zaicoderd?: number }>
        | undefined;
      ...
      expect(activeSession.zaicoderd).toBe(1234);
  ...
  it("resolves compaction.model override in runtime context so all context engines use the correct model", () => {
      ...
      attempt: {
        ...
        config: {
          agents: {
            defaults: {
              models: {
                "openrouter/anthrozaicoderc/zaicoder-sonnet-4-5": {
                  alias: "summary",
                },
              },
              compaction: {
    ...
    expect(legacy.model).toBe("anthrozaicoderc/zaicoder-sonnet-4-5");
  ...
  it("includes resolved auth profile fields for context-engine afterTurn compaction", () => {
    ...
    const legacy = buildAfterTurnRuntimeContext({
      attempt: {
        ...
        agentAccountId: "acct-1",
        authProfileId: "openai:p1",
        config: { plugins: { slots: { contextEngine: "lossless-claw" } } } as zAICoderConfig,
        skillsSnapshot: undefined,
        provider: "openai",
        modelId: "gpt-5.4",
        thinkLevel: "off",
