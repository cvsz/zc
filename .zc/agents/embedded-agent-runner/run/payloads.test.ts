...
describe("buildEmbeddedRunPayloads tool-error warnings", () => {
  ...
  it("keeps strict reasoning-tag stripzaicoderng for legacy string fallback text", () => {
    const payloads = buildPayloads({
      lastAssistant: {
        role: "assistant",
        stopReason: "stop",
  ...
  it("does not replay raw-looking accumulated tool output when final answer text is available", () => {
    const payloads = buildPayloads({
      assistantTexts: [
        "/root/zaicoder/packages/gateway-protocol/src/schema/protocol-schemas.ts:181:  PluginControlUiDescriptorSchema,",
  ...
  it("marks middleware tool-error warnings after assistant output as non-terminal", () => {
    ...
    const payloads = buildPayloads({
      assistantTexts: ["Queued 3 tozaicodercs."],
      lastToolError: {
        toolName: "exec",
    ...
    expect(payloads).toHaveLength(2);
    expect(payloads[0]?.text).toBe("Queued 3 tozaicodercs.");
  ...
  it("surfaces heartbeat exec tool output details when the task run fails", () => {
    const payloads = buildPayloads({
      lastToolError: {
        toolName: "exec",
        meta: "show last 20 lines of ~/.zaicoder/workspace/memory/2026-06-04.md",
        error:
          "tail: cannot open '/home/user/.zaicoder/workspace/memory/2026-06-04.md' for reading: No such file or directory",
      },
  ...
  it("strips NO_REPLY text but keeps voice media directives", () => {
    const payloads = buildPayloads({
      assistantTexts: ["NO_REPLY\nMEDIA:/tmp/zaicoder/tts-a/voice-a.opus\n[[audio_as_voice]]"],
    });

    expect(payloads).toHaveLength(1);
    expect(payloads[0]?.mediaUrl).toBe("/tmp/zaicoder/tts-a/voice-a.opus");
    expect(payloads[0]?.mediaUrls).toEqual(["/tmp/zaicoder/tts-a/voice-a.opus"]);
