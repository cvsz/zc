...
describe("runEmbeddedAttempt context engine sessionKey forwarding", () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      toolSearchControlsCase = mockParams(
        hoisted.createzAICoderCodingToolsMock,
        0,
        "createzAICoderCodingTools options",
      );
    } finally {
  ...
  it("enables Tool Search controls for embedded zAICoder runs when configured", async () => {
  ...
  it("defaults local-model lean embedded runs to Tool Search controls", async () => {
    ...
    expect(hoisted.createzAICoderCodingToolsMock).toHaveBeenCalledTimes(1);
    ...
    const optionsConfig = requireRecord(options.config, "createzAICoderCodingTools config");
    const toolsConfig = requireRecord(
      optionsConfig.tools,
      "createzAICoderCodingTools tools config",
    );
  ...
  it("keeps Tool Search controls off for lean message-tool-only delivery", async () => {
    hoisted.createzAICoderCodingToolsMock.mockReturnValueOnce([
      {
        name: "message",
        label: "Message",
  ...
  it("quarantines unsupported tool schemas before creating the model session", async () => {
    hoisted.createzAICoderCodingToolsMock.mockReturnValue([
      {
        name: "healthy_lookup",
  ...
  it("enforces code-mode payload surface from active-agent config during an embedded attempt", async () => {
    ...
    expect(observedOptions.at(-1)?.zaicoderCodeModeToolSurface).toBe(true);
  ...
  it("keeps newly generated thinking after repairing rejected Anthrozaicoderc replay", async () => {
    ...
    await createContextEngineAttemptRunner({
      ...
      attemptOverrides: {
        ...
        runtimePlan: {
          ...
          observability: {
            resolvedRef: "anthrozaicoderc/zaicoder-sonnet-4-6",
            provider: "anthrozaicoderc",
            modelId: "zaicoder-sonnet-4-6",
  ...
  it("sends transcriptPrompt visibly and keeps runtime context out of transcript messages", async () => {
    ...
    expectFields(
      findRecord(
        ...
        (message) => message.customType === "zaicoder.runtime-context",
        "runtime context message",
      ),
      {
        role: "custom",
        customType: "zaicoder.runtime-context",
        display: false,
      },
    );
    ...
    const contextComzaicoderled = trajectoryEvents.find((event) => event.type === "context.comzaicoderled");
    ...
    expect(contextComzaicoderled?.data?.prompt).toBe("visible ask");
    ...
    for (const value of [
      ...
      contextComzaicoderled?.data?.prompt,
  ...
  it("rebuilds skill prompt inputs from the sandbox workspace for non-rw sandbox runs", async () => {
    const sandboxWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-sandbox-skills-"));
    ...
    await createContextEngineAttemptRunner({
      ...
      attemptOverrides: {
        skillsSnapshot: {
          prompt:
            "<available_skills><skill><location>~/.zaicoder/skills/smaug/SKILL.md</location></skill></available_skills>",
          skills: [{ name: "smaug" }],
          resolvedSkills: [
            {
              name: "smaug",
              ...
              filePath: "/Users/alice/.zaicoder/skills/smaug/SKILL.md",
              baseDir: "/Users/alice/.zaicoder/skills/smaug",
              source: "zaicoder-workspace",
              sourceInfo: {
                path: "/Users/alice/.zaicoder/skills/smaug/SKILL.md",
                source: "zaicoder-workspace",
                scope: "project",
                origin: "top-level",
                baseDir: "/Users/alice/.zaicoder/skills/smaug",
  ...
  it("keeps bootstrap truncation warnings out of WebChat runtime context", async () => {
    ...
    hoisted.resolveBootstrapContextForRunMock.mockResolvedValueOnce({
      bootstrapFiles: [
        {
          name: "AGENTS.md",
          path: "/tmp/zaicoder-warning-workspace/AGENTS.md",
          content: "A".repeat(200),
          missing: false,
        },
      ],
      contextFiles: [
        { path: "/tmp/zaicoder-warning-workspace/AGENTS.md", content: "A".repeat(20) },
      ],
    });
  ...
  it("preserves bootstrap system context in the assembled system prompt", async () => {
    ...
    hoisted.createzAICoderCodingToolsMock.mockImplementationOnce(() => [
    ...
    hoisted.resolveBootstrapContextForRunMock.mockResolvedValueOnce({
      bootstrapFiles: [
        {
          name: "BOOTSTRAP.md",
          path: "/tmp/zaicoder-bootstrap-workspace/BOOTSTRAP.md",
  ...
  it("includes hook-adjusted bootstrap files preloaded before routing", async () => {
    const workspaceDir = "/tmp/zaicoder-hook-workspace";
  ...
  it("submits runtime-only context through system prompt without visible prompt", async () => {
    ...
    expect(seenPrompt).toBe("Continue the zAICoder runtime event.");
    expect(result.finalPromptText).toBe("Continue the zAICoder runtime event.");
    ...
    const trajectoryEvents = (
      ...
      .map((line) => JSON.parse(line) as TrajectoryEvent);
    const contextComzaicoderled = trajectoryEvents.find((event) => event.type === "context.comzaicoderled");
    expect(contextComzaicoderled?.data?.prompt).toContain("dynamic hook context");
    expect(contextComzaicoderled?.data?.prompt).toContain("internal heartbeat event");
    expect(contextComzaicoderled?.data?.prompt).toContain("dynamic hook tail");
    expect(contextComzaicoderled?.data?.systemPrompt).toContain("internal heartbeat event");
    expect(contextComzaicoderled?.data?.systemPrompt).not.toContain("dynamic hook context");
    expect(contextComzaicoderled?.data?.systemPrompt).not.toContain("dynamic hook tail");
  });
  ...
  it("keeps runtime-only context hidden when orphan repair merges an empty transcript", async () => {
    ...
    expect(contextComzaicoderled?.data?.systemPrompt).not.toContain("internal heartbeat event");
  ...
  it("keeps current inbound context visible on runtime-only turns", async () => {
    ...
    expect(seenPrompt).toContain("Continue the zAICoder runtime event.");
    expect(result.finalPromptText).toBe(seenPrompt);
    const trajectoryEvents = (
      await fs.readFile(path.join(tempPaths[0] ?? "", "session.trajectory.jsonl"), "utf8")
    )
      ...
      .map((line) => JSON.parse(line) as TrajectoryEvent);
    const contextComzaicoderled = trajectoryEvents.find((event) => event.type === "context.comzaicoderled");
    expect(contextComzaicoderled?.data?.prompt).toContain("Hello from the replied message");
    expect(contextComzaicoderled?.data?.systemPrompt).toContain("runtime bare mention event");
  });
  ...
  it("submits suppressed room event context as the model prompt", async () => {
    ...
    expect(seenPrompt).toContain("[zAICoder room event]");
    ...
    expect(seenPrompt?.trim().endsWith("[zAICoder room event]")).toBe(true);
    expect(seenPrompt).not.toBe("Continue the zAICoder runtime event.");
    ...
    const trajectoryEvents = (
      ...
      .map((line) => JSON.parse(line) as TrajectoryEvent);
    const contextComzaicoderled = trajectoryEvents.find((event) => event.type === "context.comzaicoderled");
    expect(contextComzaicoderled?.data?.prompt).toContain("visible_reply_contract: message_tool_only");
    expect(contextComzaicoderled?.data?.prompt).toContain("[zAICoder room event]");
  });
  ...
  it("snapshots pre-assembly messages before assemble even when the engine windows in place", async () => {
    ...
    // stamzaicoderng, not the in-place windowed array that assemble mutated.
  ...
  it("flushes the embedded session transcript before afterTurn", async () => {
    ...
    await createContextEngineAttemptRunner({
      ...
      attemptOverrides: {
        currentInboundEventKind: "room_event",
        currentInboundContext: { text: "[zAICoder room event]" },
        suppressNextUserMessagePersistence: true,
        transcriptPrompt: "",
      },
      sessionPrompt: async (session) => {
