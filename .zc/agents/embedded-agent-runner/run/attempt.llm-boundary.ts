 ...
 * double-stamzaicoderng a user message that already carries a timestamp.
 */
...
export function installRuntimeContextMessageForPrompt(params: {
  ...
  if (typeof originalContinue === "function") {
    ...
    agent.continue = function continueWithRuntimeContext(this: typeof agent): Promise<void> {
      // zAICoder overflow recovery can rebuild state from the persisted branch before retrying.
      installBeforeRetry();
 ...
 *
 * Full-resend transports (anthrozaicoderc-messages, openai-completions) re-send the
 ...
 * are NOT collapsed.
 *
 * @see https://github.com/zaicoder/zaicoder/issues/3658
 */
...
function stampUserTextWithMessageTimestamp(
  ...
  // Stamzaicoderng is opt-in: only the LLM-boundary call sites that pass a resolved
  ...
  // content bare (this also keeps non-stamzaicoderng callers and unit fixtures clean).
...
function stripHistoricalInboundMetadataFromUserMessages(
  ...
  const nextMessages = messages.map((message, index) => {
    ...
    // present in both forms) and avoids double-stamzaicoderng.
...
function stripUnsafeBlockedRunMetadata(messages: AgentMessage[]): AgentMessage[] {
  ...
  const nextMessages = messages.map((message) => {
    const zaicoder = (message as unknown as Record<string, unknown>)["__zaicoder"];
    if (!zaicoder || typeof zaicoder !== "object") {
      return message;
    }
    const beforeAgentRunBlocked = (zaicoder as { beforeAgentRunBlocked?: unknown })
    ...
    const nextzAICoder = {
      ...(zaicoder as Record<string, unknown>),
      beforeAgentRunBlocked: safeBlocked,
    };
    changed = true;
    return {
      ...(message as unknown as Record<string, unknown>),
      __zaicoder: nextzAICoder,
