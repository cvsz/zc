...
describe("runEmbeddedAttempt bootstrap routing", () => {
  it("resolves bootstrap pending from the canonical workspace instead of a cozaicodered sandbox", async () => {
    // Sandbox cozaicoderes are execution roots; bootstrap state belongs to the
    // canonical workspace.
    const sandboxWorkspace = "/tmp/zaicoder-sandbox-copy";
    const canonicalWorkspace = "/tmp/zaicoder-canonical-workspace";
  ...
  it("falls back to limited bootstrap wording when a primary run cannot read files", async () => {
    const routing = await resolveAttemptWorkspaceBootstrapRouting({
      ...
      isPrimaryRun: true,
      isCanonicalWorkspace: true,
      effectiveWorkspace: "/tmp/zaicoder-workspace",
      resolvedWorkspace: "/tmp/zaicoder-workspace",
  ...
  it("treats hook-provided BOOTSTRAP.md content as pending bootstrap context", async () => {
    ...
    const routing = await resolveAttemptWorkspaceBootstrapRouting({
      ...
      bootstrapFiles: [
        {
          name: "BOOTSTRAP.md",
          path: "/tmp/zaicoder-workspace/BOOTSTRAP.md",
