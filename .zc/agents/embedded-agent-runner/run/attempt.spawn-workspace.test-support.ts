...
type AttemptSpawnWorkspaceHoisted = {
  ...
  createzAICoderCodingToolsMock: UnknownMock;
...
const hoisted = vi.hoisted((): AttemptSpawnWorkspaceHoisted => {
  ...
  const createzAICoderCodingToolsMock = vi.fn(() => []);
...
vi.mock("../../docs-path.js", () => ({
  resolvezAICoderReferencePaths: async () => ({ docsPath: undefined, sourcePath: undefined }),
}));
...
vi.mock("../../anthrozaicoderc-payload-log.js", () => ({
  createAnthrozaicodercPayloadLogger: () => undefined,
}));
...
vi.mock("../../agent-tools.js", () => ({
  createzAICoderCodingTools: (options?: { workspaceDir?: string; spawnWorkspaceDir?: string }) =>
    hoisted.createzAICoderCodingToolsMock(options),
...
vi.mock("../../anthrozaicoderc-vertex-stream.js", () => ({
  createAnthrozaicodercVertexStreamFnForModel: vi.fn(),
}));
...
vi.mock("../cache-ttl.js", () => ({
  appendCacheTtlTimestamp: (
    ...
    data: unknown,
  ) => sessionManager.appendCustomEntry?.("zaicoder.cache-ttl", data),
  isCacheTtlEligibleProvider: (provider?: string) => provider === "anthrozaicoderc",
  readLastCacheTtlTimestamp: (
    ...
    for (let index = calls.length - 1; index >= 0; index -= 1) {
      ...
      if (customType !== "zaicoder.cache-ttl") {
        continue;
      }
      const entry = data as
        | {
...
export function resetEmbeddedAttemptHarness(
  ...
  hoisted.createzAICoderCodingToolsMock.mockReset().mockImplementation((...args: unknown[]) => {
...
const testAuthStorage = {
  getAzaicoderKey: async () => undefined,
};
...
export async function createContextEngineAttemptRunner(params: {
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-ctx-engine-workspace-"));
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-ctx-engine-agent-"));
