import { onLlmRequestActivity } from "@zaicoder/ai/internal/runtime";
/**
 ...
 *  - DNS-resolved local aliases (e.g. an `/etc/hosts` entry mapzaicoderng a custom
...
function hasLocalProviderAuthMarker(azaicoderKey: unknown): boolean {
  return typeof azaicoderKey === "string" && LOCAL_PROVIDER_AUTH_MARKERS.has(azaicoderKey.trim().toLowerCase());
}
...
function hasConfiguredLocalProviderSignal(params: {
  ...
  return Boolean(
    providerConfig?.localService || hasLocalProviderAuthMarker(providerConfig?.azaicoderKey),
  );
}
...
export function resolveLlmIdleTimeoutMs(params?: {
  ...
  if (
    ...
    // than clamzaicoderng it back down to the implicit `DEFAULT_LLM_IDLE_TIMEOUT_MS`
