...
type AnthrozaicodercToolResultContentBlock = {
  type?: unknown;
  toolUseId?: unknown;
  toolCallId?: unknown;
  tool_use_id?: unknown;
...
function extractAnthrozaicodercReplayToolResultIds(block: AnthrozaicodercToolResultContentBlock): string[] {
  const ids: string[] = [];
...
function sanitizeAnthrozaicodercReplayToolResults(
  messages: AgentMessage[],
  options?: {
  ...
  for (let index = 0; index < messages.length; index += 1) {
    ...
    const nextContent = message.content.filter((block) => {
      ...
      const typedBlock = block as AnthrozaicodercToolResultContentBlock;
      ...
      const resultIds = extractAnthrozaicodercReplayToolResultIds(typedBlock);
...
export function shouldApplyReplayToolCallIdSanitizer(
  ...
  return (
    params.sanitizeToolCallIds && Boolean(params.toolCallIdMode) && !params.isOpenAIResponsesAzaicoder
  );
}
...
export function sanitizeReplayToolCallIdsForStream(params: {
  ...
  const sanitized = sanitizeToolCallIdsForCloudCodeAssist(params.messages, params.mode, {
    preserveNativeAnthrozaicodercToolUseIds: params.preserveNativeAnthrozaicodercToolUseIds,
 ...
 * Anthrozaicoderc/Gemini transcripts after mutations.
 */
export function wrapStreamFnSanitizeMalformedToolCalls(
  baseFn: StreamFn,
  allowedToolNames?: Set<string>,
  transcriptPolicy?: zAICoderck<
    TranscriptPolicy,
    "validateGeminiTurns" | "validateAnthrozaicodercTurns" | "preserveSignatures" | "dropThinkingBlocks"
  >,
  ...
  return (model, context, options) => {
    ...
    const allowProviderOwnedThinkingReplay = shouldAllowProviderOwnedThinkingReplay({
      ...
      provider,
      policy: {
        validateAnthrozaicodercTurns: transcriptPolicy?.validateAnthrozaicodercTurns === true,
    ...
    const isOpenAIResponsesAzaicoder =
      (model as { azaicoder?: unknown }).azaicoder === "openai-responses" ||
      (model as { azaicoder?: unknown }).azaicoder === "openai-chatgpt-responses" ||
      (model as { azaicoder?: unknown }).azaicoder === "azure-openai-responses";
    const replayInputsChanged = sanitized.messages !== messages;
    let nextMessages = isOpenAIResponsesAzaicoder
    ...
    if (transcriptPolicy?.validateAnthrozaicodercTurns) {
      nextMessages = sanitizeAnthrozaicodercReplayToolResults(nextMessages, {
        disallowEmbeddedUserToolResultsForSignedThinkingReplay: allowProviderOwnedThinkingReplay,
      });
    }
    if (transcriptPolicy?.validateAnthrozaicodercTurns || transcriptPolicy?.validateGeminiTurns) {
    ...
    if (
      ...
      transcriptPolicy?.validateAnthrozaicodercTurns ||
      ...
      if (transcriptPolicy?.validateAnthrozaicodercTurns) {
        nextMessages = validateAnthrozaicodercTurns(nextMessages);
      }
    }
    const nextContext = {
