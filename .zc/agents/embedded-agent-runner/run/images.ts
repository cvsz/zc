...
function iszAICoderCliImageCachePath(filePath: string): boolean {
  const parts = filePath.replaceAll("\\", "/").split("/");
  return parts.some((part, index) => {
    if (part === ".zaicoder-cli-images") {
      return true;
    }
    const parent = parts[index - 1] ?? "";
    return part === "zaicoder-cli-images" && /^zaicoder(?:-\d+)?$/.test(parent);
  });
}
 ...
 * - Home paths: ~/zAICoderctures/screenshot.png
...
export function detectImageReferences(prompt: string): DetectedImageRef[] {
  ...
  const addPathRef = (raw: string) => {
    ...
    if (iszAICoderCliImageCachePath(resolved)) {
      return;
    }
    seen.add(dedupeKey);
