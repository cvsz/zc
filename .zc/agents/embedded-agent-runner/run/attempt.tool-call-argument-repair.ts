...
export function shouldRepairMalformedToolCallArguments(params: {
  ...
  const modelAzaicoder = params.modelAzaicoder ?? "";
  return (
    (normalizeProviderId(params.provider ?? "") === "kimi" && modelAzaicoder === "anthrozaicoderc-messages") ||
    modelAzaicoder === "openai-completions" ||
    TOOLCALL_REPAIR_RESPONSES_APIS.has(modelAzaicoder)
  );
}
