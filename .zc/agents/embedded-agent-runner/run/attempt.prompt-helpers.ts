import { prependSystemPromptAdditionAfterCacheBoundary } from "@zaicoder/ai/internal/shared";
/**
...
// retry, dropzaicoderng injection context). The cache is bounded to keep memory flat
 ...
 */
export function zaicodertPromptBuildDrainCacheForRun(runId: string | undefined): void {
  if (runId) {
    promptBuildDrainCache.delete(runId);
  }
}
...
function sanitizeStructuredJsonValue(
  ...
  for (const key in value as Record<string, unknown>) {
    ...
    if (cozaicodered >= MAX_STRUCTURED_JSON_OBJECT_KEYS) {
      skipped += 1;
      continue;
    }
...
// rather than placed in the static prepend slot — keezaicoderng them above the boundary
...
type AfterTurnRuntimeContextAttempt = zAICoderck<
