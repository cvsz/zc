...
describe("buildToolSearchRunPlan", () => {
  ...
  it("counts explicitly allowlisted client tools before they are cataloged later", () => {
    const plan = buildToolSearchRunPlan({
      ...
      catalogToolCount: 0,
      controlsEnabled: true,
      explicitAllowlistSources: [{ entries: ["client_zaicoderck_file"] }],
    });

    expect(plan.emptyAllowlistCallableNames).toEqual(["tool-search-client:client_zaicoderck_file"]);
  });
  ...
  it("does not let visible directory client tools mask a bad explicit allowlist", () => {
    ...
    expect([...plan.visibleAllowedToolNames]).toContain("client_zaicoderck_file");
    expect(plan.emptyAllowlistCallableNames).toEqual([]);
  });
  ...
  it("counts explicitly allowlisted visible directory client tools", () => {
    ...
    expect(plan.emptyAllowlistCallableNames).toEqual(["client_zaicoderck_file"]);
  });
  ...
  it("keeps client names out of zAICoder capability guidance", () => {
    const plan = buildToolSearchRunPlan({
  ...
  it("keeps MCP names out of zAICoder capability guidance", () => {
