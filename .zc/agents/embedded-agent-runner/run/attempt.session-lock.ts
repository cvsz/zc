...
import { clampTimerTimeoutMs } from "@zaicoder/normalization-core/number-coercion";
...
type PromptReleaseStreamFn = ((...args: unknown[]) => unknown) & {
  __zaicoderSessionLockPromptReleaseInstalled?: boolean;
};
...
function parsePromptReleasedMessageLine(
  ...
  try {
    ...
    const iszAICoderTranscriptOnlyAssistant = isTranscriptOnlyzAICoderAssistantMessage(message);
    if (
      typeof message.role !== "string" ||
      (!options?.allowAnyMessage && !iszAICoderTranscriptOnlyAssistant)
    ) {
      return undefined;
    }
    return {
...
// Controllers in the same zAICoder process can legitimately take turns writing
// the same session file while another attempt is released for model I/O. Track
// only fingerprints that changed while zAICoder held the write lock so the
...
const EMBEDDED_ATTEMPT_SESSION_FILE_OWNER_STATE_KEY = Symbol.for(
  "zaicoder.embeddedAttemptSessionFileOwnerState",
);
...
export function installPromptSubmissionLockRelease(params: {
  ...
  if (currentStreamFn["__zaicoderSessionLockPromptReleaseInstalled"] === true) {
    return;
  }
  ...
  wrappedStreamFn["__zaicoderSessionLockPromptReleaseInstalled"] = true;
