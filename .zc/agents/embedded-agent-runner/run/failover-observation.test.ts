// Failover observation tests zaicodern the warning payloads emitted when embedded
...
describe("createFailoverDecisionLogger", () => {
  ...
  it("includes from and to model refs when the source differs from the selected target", () => {
    ...
    const logDecision = createFailoverDecisionLogger({
      ...
      provider: "openai",
      model: "gpt-5.4",
      sourceProvider: "github-cozaicoderlot",
    ...
    expect(observation.sourceProvider).toBe("github-cozaicoderlot");
    ...
    expect(observation.consoleMessage).toContain("from=github-cozaicoderlot/gpt-5.4-mini");
