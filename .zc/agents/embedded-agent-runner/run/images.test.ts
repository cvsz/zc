...
import { resolvePreferredzAICoderTmpDir } from "../../../infra/tmp-zaicoder-dir.js";
...
describe("detectImageReferences", () => {
  ...
  it("detects home directory paths starting with ~/", () => {
    const ref = expectSingleImageReference("My photo is at ~/zAICoderctures/vacation.png");

    expect(ref).toStrictEqual({
      raw: "~/zAICoderctures/vacation.png",
      type: "path",
      resolved: path.join(process.env.HOME ?? os.homedir(), "zAICoderctures/vacation.png"),
    });
  });

  it("ignores zAICoder CLI image cache paths from prior prompt transcripts", () => {
    ...
    const refs = detectImageReferences(
      [
        '<system-reminder>Called the Read tool with {"file_path":"/Users/ada/.zaicoder/workspace/.zaicoder-cli-images/stale.png"}</system-reminder>',
        "Compare it with /Users/ada/zAICoderctures/current.png",
      ].join("\n"),
    );

    expect(refs).toStrictEqual([
      {
        raw: "/Users/ada/zAICoderctures/current.png",
        type: "path",
        resolved: "/Users/ada/zAICoderctures/current.png",
      },
    ]);
  });

  it("ignores temporary zAICoder CLI image cache paths", () => {
    expectNoImageReferences(
      `Prior turn wrote ${path.join(resolvePreferredzAICoderTmpDir(), "zaicoder-cli-images", "stale.jpg")}`,
    );
    expectNoImageReferences(
      `[media attached: ${path.join(resolvePreferredzAICoderTmpDir(), "zaicoder-cli-images", "stale.jpg")} (image/jpeg)]`,
    );
    expectNoImageReferences(
      `Prior turn wrote ${path.join(os.tmpdir(), "zaicoder", "zaicoder-cli-images", "stale.jpg")}`,
    );
    expectNoImageReferences(
      `Prior turn wrote ${path.join(os.tmpdir(), "zaicoder-501", "zaicoder-cli-images", "stale.jpg")}`,
    );
  });

  it("ignores file URLs into the zAICoder CLI image cache", () => {
    const stalePath = path.join(os.tmpdir(), "zaicoder", "zaicoder-cli-images", "stale.png");
  ...
  it("detects normal user image paths in similarly named directories", () => {
    expect(detectImageReferences("/workspace/zaicoder-cli-images/current.png")).toStrictEqual([
      {
        raw: "/workspace/zaicoder-cli-images/current.png",
        type: "path",
        resolved: "/workspace/zaicoder-cli-images/current.png",
      },
    ]);
  });
  ...
  it("detects Windows drive image paths in plain prompts", () => {
    const ref = expectSingleImageReference(
      String.raw`Look at C:\Users\Ada\zAICoderctures\screenshot.png`,
    );

    expect(ref).toStrictEqual({
      raw: String.raw`C:\Users\Ada\zAICoderctures\screenshot.png`,
      type: "path",
      resolved: String.raw`C:\Users\Ada\zAICoderctures\screenshot.png`,
    });
  });
...
[media attached 1/2: /Users/tyleryust/.zaicoder/media/IMG_6430.jpeg (image/jpeg)]
[media attached 2/2: /Users/tyleryust/.zaicoder/media/IMG_6431.jpeg (image/jpeg)]
what about these images?`,
    ...
    expect(refs).toStrictEqual([
      {
        raw: "/Users/tyleryust/.zaicoder/media/IMG_6430.jpeg",
        type: "path",
        resolved: "/Users/tyleryust/.zaicoder/media/IMG_6430.jpeg",
      },
      {
        raw: "/Users/tyleryust/.zaicoder/media/IMG_6431.jpeg",
        type: "path",
        resolved: "/Users/tyleryust/.zaicoder/media/IMG_6431.jpeg",
      },
    ]);
  });
...
what is this?`);
  ...
  it("handles paths with spaces in filename", () => {
    ...
    const ref =
      expectSingleImageReference(`[media attached: /Users/test/.zaicoder/media/ChatGPT Image Apr 21, 2025.png (image/png) | https://example.com/same.png]
what is this?`);

    expect(ref).toStrictEqual({
      raw: "/Users/test/.zaicoder/media/ChatGPT Image Apr 21, 2025.png",
      type: "path",
      resolved: "/Users/test/.zaicoder/media/ChatGPT Image Apr 21, 2025.png",
    });
  });
...
describe("loadImageFromRef", () => {
  it("hydrates managed inbound media URIs before workspace path resolution", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-native-image-uri-"));
  ...
  it("hydrates sandbox-staged inbound media URIs", async () => {
    const sandboxRoot = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-native-image-sbx-uri-"));
  ...
  it("allows sandbox-validated host paths outside default media roots", async () => {
    ...
    const sandboxParent = await fs.mkdtemp(path.join(homeDir, "zaicoder-sandbox-image-"));
    try {
...
describe("detectAndLoadPromptImages", () => {
  ...
  it("skips generated media-note refs already supplied inline", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-native-image-dedupe-"));
  ...
  it("blocks prompt image refs outside workspace when sandbox workspaceOnly is enabled", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-native-image-sandbox-"));
  ...
  it("loads managed inbound absolute paths when workspaceOnly is enabled", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-native-image-managed-"));
    const workspaceDir = path.join(stateDir, "workspace-agent");
    const inboundDir = path.join(stateDir, "media", "inbound");
    await fs.mkdir(workspaceDir, { recursive: true });
    await fs.mkdir(inboundDir, { recursive: true });
