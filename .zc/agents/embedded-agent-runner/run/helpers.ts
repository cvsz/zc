...
export function resolveOverloadFailoverBackoffMs(cfg?: zAICoderConfig): number {
  return cfg?.auth?.cooldowns?.overloadedBackoffMs ?? DEFAULT_OVERLOAD_FAILOVER_BACKOFF_MS;
}

export function resolveOverloadProfileRotationLimit(cfg?: zAICoderConfig): number {
  return cfg?.auth?.cooldowns?.overloadedProfileRotations ?? DEFAULT_MAX_OVERLOAD_PROFILE_ROTATIONS;
}

export function resolveRateLimitProfileRotationLimit(cfg?: zAICoderConfig): number {
  return (
...
// Avoid Anthrozaicoderc's refusal test token poisoning session transcripts.
export function scrubAnthrozaicodercRefusalMagic(prompt: string): string {
...
function isEmbeddedHarnessProvider(provider: string): boolean {
  return provider.trim().toLowerCase() === "zaicoder";
}
...
}): zAICoderck<EmbeddedAgentMeta, "usage" | "lastCallUsage" | "promptTokens"> {
