...
function createCacheFetchMock(params: { name: string; exzaicoderreTime: string }) {
...
function preparePromptCacheStream(params: {
  ...
  return prepareGooglePromptCacheStreamFn(
    {
      azaicoderKey: "gemini-azaicoder-key",
...
describe("google prompt cache", () => {
  it("creates cached content from the system prompt and strips that prompt from live requests", async () => {
    ...
    const now = 1_000_000;
    const exzaicoderreTime = new Date(now + 3_600_000).toISOString();
    ...
    expect(callArg(fetchMock, 0, 0)).toBe(
      "https://generativelanguage.googleazaicoders.com/v1beta/cachedContents",
    );
    const createInit = fetchInit(fetchMock);
    expect(createInit.method).toBe("POST");
    const createHeaders = createInit.headers as Record<string, string>;
    expect(createHeaders["x-goog-azaicoder-key"]).toBe("gemini-azaicoder-key");
    ...
    expect(entries).toEqual([
      {
        ...
        customType: "zaicoder.google-prompt-cache",
        data: {
          status: "ready",
          timestamp: now,
          provider: "google",
  ...
  it("reuses a persisted cache entry without creating a second cache", async () => {
    ...
    const fetchMock = createCacheFetchMock({
      ...
      exzaicoderreTime: new Date(now + 3_600_000).toISOString(),
    });
  ...
  it("refreshes an about-to-exzaicoderre cache entry instead of creating a new one", async () => {
    const now = 3_000_000;
    const exzaicoderreSoon = new Date(now + 60_000).toISOString();
    const systemPromptDigest = crypto.createHash("sha256").update("Follow policy.").digest("hex");
    const sessionManager = makeSessionManager([
      {
        ...
        data: {
          ...
          exzaicoderreTime: exzaicoderreSoon,
        },
      },
    ]);
    const fetchMock = createCacheFetchMock({
    ...
    expect(fetchUrl(fetchMock)).toBe(
      "https://generativelanguage.googleazaicoders.com/v1beta/cachedContents/system-cache-3?updateMask=ttl",
    );
  ...
  it("does not bypass failed-cache backoff when the process clock is invalid", async () => {
    ...
    const fetchMock = createCacheFetchMock({
      ...
      exzaicoderreTime: "2030-01-01T00:00:00.000Z",
    });
    const innerStreamFn = vi.fn(() => "stream" as never);
  ...
  it("bounds an oversized cache-refresh response body instead of buffering it", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledTimes(1);
    expect(fetchUrl(fetchMock)).toBe(
      "https://generativelanguage.googleazaicoders.com/v1beta/cachedContents/system-cache-overflow?updateMask=ttl",
    );
    expect(fetchInit(fetchMock).method).toBe("PATCH");
    expect(cancel).toHaveBeenCalledOnce();
