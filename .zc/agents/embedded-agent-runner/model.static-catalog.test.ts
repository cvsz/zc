...
const manifestMocks = vi.hoisted(() => ({
  listzAICoderPluginManifestMetadata: vi.fn(),
...
vi.mock("../../plugins/manifest-metadata-scan.js", () => ({
  listzAICoderPluginManifestMetadata: manifestMocks.listzAICoderPluginManifestMetadata,
}));
...
function setManifestPlugins(plugins: unknown[]) {
  ...
  manifestMocks.listzAICoderPluginManifestMetadata.mockReturnValue(
  ...
  manifestMocks.loadPluginManifest.mockImplementation((pluginDir: string) => {
    ...
    return plugin
      ...
      : { ok: false, error: "missing manifest", manifestPath: `${pluginDir}/zaicoder.plugin.json` };
  });
}
...
beforeEach(() => {
  manifestMocks.listzAICoderPluginManifestMetadata.mockReset();
...
describe("resolveBundledStaticCatalogModel", () => {
  it("reuses one manifest scan across prepared lookups", () => {
    ...
    expect(manifestMocks.listzAICoderPluginManifestMetadata).toHaveBeenCalledTimes(1);
  });
  ...
  it("requires an exact provider and model match", async () => {
    ...
    providerMocks.normalizePluginDiscoveryResult.mockReturnValue({
      ...
      "google-vertex": {
        ...
        baseUrl: "https://aiplatform.googleazaicoders.com/v1",
        models: [{ id: "gemini-3.1-pro-preview", name: "Gemini 3.1 Pro Preview" }],
      },
    });
