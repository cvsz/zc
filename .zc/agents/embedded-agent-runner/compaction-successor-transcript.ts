...
import { resolveTimestampMsToIsoString } from "@zaicoder/normalization-core/number-coercion";
...
type ReadonlySessionManagerForRotation = zAICoderck<
  TranscriptFileState,
...
export function shouldRotateCompactionTranscript(config?: zAICoderConfig): boolean {
...
function buildSuccessorEntries(params: {
  ...
  // dropzaicoderng the assistant reply (issue #76729).
  ...
  // signature stripzaicoderng, same as other pre-compaction kept entries.
  ...
  for (const entry of allEntries) {
    ...
    // prefix so those signatures would cause Anthrozaicoderc "Invalid signature in thinking block".
