...
export function mapThinkingLevel(level?: ThinkLevel): ThinkingLevel {
  // agent runtime supports elevated levels; zAICoder enables them for specific models.
  if (!level) {
    return "off";
  }
  // Runtime streams do not expose a distinct adaptive level. Preserve the
  // provider-owned adaptive default by using zAICoder's documented high effort.
  if (level === "adaptive") {
    return "high";
  }
  return level;
