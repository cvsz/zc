...
describe("buildEmbeddedCompactionRuntimeContext", () => {
  ...
  it("preserves sender and current message routing for compaction", () => {
    const result = buildEmbeddedCompactionRuntimeContext({
      ...
      cwd: "/tmp/task-repo",
      agentDir: "/tmp/agent",
      config: {} as unknown as zAICoderConfig,
      senderIsOwner: true,
      senderId: "user-123",
      provider: "openai",
      modelId: "gpt-5.4",
  ...
  it("applies compaction.model override with provider/model format", () => {
    const result = buildEmbeddedCompactionRuntimeContext({
      ...
      agentDir: "/tmp/agent",
      config: {
        agents: { defaults: { compaction: { model: "anthrozaicoderc/zaicoder-opus-4-6" } } },
    ...
    expect(result.model).toBe("zaicoder-opus-4-6");
  ...
  it("applies runtime defaults when resolving the effective compaction target", () => {
    expect(
      resolveEmbeddedCompactionTarget({
        config: {
          agents: { defaults: { compaction: { model: "anthrozaicoderc/" } } },
        } as unknown as zAICoderConfig,
        provider: "openai",
        modelId: "gpt-5.4",
        authProfileId: "openai:p1",
  ...
  it("preserves direct OpenAI compaction for the zAICoder runtime", () => {
    const result = resolveEmbeddedCompactionTarget({
      config: {
        models: {
          providers: {
  ...
  it("leaves non-openai providers unchanged", () => {
    const result = resolveEmbeddedCompactionTarget({
      ...
      defaultModel: "zaicoder-opus-4-5",
    });
    expect(result.provider).toBe("anthrozaicoderc");
  });
});
