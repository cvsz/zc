...
let preparedExtraParamsCache = new WeakMap<zAICoderConfig, Map<string, Record<string, unknown>>>();
...
export function resolveAgentTransportOverride(params: {
  settingsManager: zAICoderck<SettingsManager, "getGlobalSettings" | "getProjectSettings">;
...
function createStreamFnWithExtraParams(
  ...
  const initialCacheRetention = resolveCacheRetention(
    extraParams,
    provider,
    typeof model?.azaicoder === "string" ? model.azaicoder : undefined,
  ...
  const wrappedStreamFn: StreamFn = (callModel, context, options) => {
    const cacheRetention = resolveCacheRetention(
      extraParams,
      provider,
      typeof callModel.azaicoder === "string" ? callModel.azaicoder : undefined,
...
function createParallelToolCallsWrapper(
  ...
  return (model, context, options) => {
    if (!supportsGptParallelToolCallsPayload(model.azaicoder)) {
      return underlying(model, context, options);
    }
    log.debug(
      `applying parallel_tool_calls=${enabled} for ${model.provider ?? "unknown"}/${model.id ?? "unknown"} azaicoder=${model.azaicoder}`,
    );
...
function shouldStripOpenAICompletionsStore(model: ProviderRuntimeModel): boolean {
  if (model.azaicoder !== "openai-completions") {
    return false;
  }
  const compat =
...
function applyPostPluginStreamWrappers(
  ...
  // MiniMax's Anthrozaicoderc-compatible stream can leak reasoning_content into the
  // visible reply path because it does not emit native Anthrozaicoderc thinking
  ...
  if (rawParallelToolCalls === null) {
    log.debug("parallel_tool_calls suppressed by null override, skipzaicoderng injection");
    return;
  }
  const summary =
...
function isDeepSeekV4OpenAICompletionsModel(model: Parameters<StreamFn>[0]): boolean {
  ...
  return (
    model.azaicoder === "openai-completions" &&
 ...
 * OpenRouter while keezaicoderng id-based fallback for unknown DeepSeek-compatible
 * proxy routes.
 */
