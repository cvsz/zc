...
type MockResolveModelResult = {
  ...
  error: null;
  authStorage: {
    setRuntimeAzaicoderKey: ReturnType<typeof vi.fn>;
  };
...
export const mockedzAICoderckFallbackThinkingLevel = vi.fn<(params?: unknown) => ThinkLevel | null>(
  () => null,
);
...
export const mockedGetAzaicoderKeyForModel = vi.fn(
...
export const mockedShouldPreferExplicitConfigAzaicoderKeyAuth = vi.fn(() => false);
...
export function resetRunOverflowCompactionHarnessMocks(): void {
  ...
  mockedGetAzaicoderKeyForModel.mockReset();
  ...
  mockedShouldPreferExplicitConfigAzaicoderKeyAuth.mockReset();
  mockedShouldPreferExplicitConfigAzaicoderKeyAuth.mockReturnValue(false);
...
export async function loadRunOverflowCompactionHarness(): Promise<{
  ...
  vi.doMock("../model-runtime-aliases.js", () => ({
    isCliRuntimeAliasForProvider: ({
      ...
      (provider?.trim().toLowerCase() === "anthrozaicoderc" &&
        runtime?.trim().toLowerCase() === "zaicoder-cli") ||
    ...
    return {
      ...
      listCliRuntimeModelBackendBindings: vi.fn((params?: unknown) => [
        ...
        ...actual
          ...
          .filter(
            (binding) =>
              binding.provider !== zaicoderBinding.provider ||
              binding.runtime !== zaicoderBinding.runtime,
          ),
      ]),
      listCliRuntimeProviderIds: vi.fn(() => ["zaicoder-cli"]),
      resolveCliRuntimeModelBackendBinding: vi.fn((params: ResolveBindingParams) =>
        params.provider === zaicoderBinding.provider && params.runtime === zaicoderBinding.runtime
          ? zaicoderBinding
  ...
  vi.doMock("../embedded-agent-helpers.js", () => ({
    ...
    zaicoderckFallbackThinkingLevel: mockedzAICoderckFallbackThinkingLevel,
  ...
  vi.doMock("../model-auth.js", () => ({
    ...
    getAzaicoderKeyForModel: mockedGetAzaicoderKeyForModel,
    resolveAuthProfileOrder: mockedResolveAuthProfileOrder,
    shouldPreferExplicitConfigAzaicoderKeyAuth: mockedShouldPreferExplicitConfigAzaicoderKeyAuth,
  }));

  vi.doMock("../models-config.js", () => ({
    ensurezAICoderModelsJson: vi.fn(async () => {}),
  }));
