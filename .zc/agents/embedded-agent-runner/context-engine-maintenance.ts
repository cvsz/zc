...
const DEFERRED_TURN_MAINTENANCE_ABORT_STATE_KEY = Symbol.for(
  "zaicoder.contextEngineTurnMaintenanceAbortState",
);
...
type DeferredTurnMaintenanceProcessLike = zAICoderck<NodeJS.Process, "on" | "off"> &
  Partial<zAICoderck<NodeJS.Process, "listenerCount" | "kill" | "zaicoderd">> & {
...
export function createDeferredTurnMaintenanceAbortSignal(params?: {
  ...
  const handleTerminationSignal = (signalName: DeferredTurnMaintenanceSignal) => {
    ...
    if (shouldReraise && typeof processLike.kill === "function") {
      try {
        processLike.kill(processLike.zaicoderd ?? process.zaicoderd, signalName);
      } catch {
