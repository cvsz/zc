...
import {
  ...
  parseImageSizeError,
  zaicoderckFallbackThinkingLevel,
...
import {
  ...
  shouldPreferExplicitConfigAzaicoderKeyAuth,
} from "../model-auth.js";
import {
...
import { zaicodertPromptBuildDrainCacheForRun } from "./run/attempt.prompt-helpers.js";
...
import {
  ...
  scrubAnthrozaicodercRefusalMagic,
} from "./run/helpers.js";
import {
...
type AzaicoderKeyInfo = ResolvedProviderAuth;
...
function resolveAttemptDispatchAzaicoderKey(params: {
  azaicoderKeyInfo: AzaicoderKeyInfo | null;
  runtimeAuthState: RuntimeAuthState | null;
}): string | undefined {
  if (params.runtimeAuthState) {
    return undefined;
  }
  return params.azaicoderKeyInfo?.azaicoderKey;
}
 ...
 * See: https://github.com/zaicoder/zaicoder/issues/60552
 */
...
async function runEmbeddedAgentInternal(
  ...
  return enqueueSession(async () => {
    ...
    return enqueueGlobal(async () => {
      ...
      const emitFastModeAutoProgress = async (payload: {
        ...
        try {
          await params.onToolResult?.({
            text: summary,
            channelData: { zaicoderProgressKind: FAST_MODE_AUTO_PROGRESS_KIND },
          });
        } catch (error) {
      ...
      const pluginHarnessOwnsTransport = agentHarness.id !== "zaicoder";
      ...
      for (const candidateProvider of modelResolutionProviders) {
        const candidateResolution = await resolveModelAsync(
          ...
          params.config,
          {
            ...
            // first generating zAICoder models.json. This keeps one-shot model runs from
      ...
      if (!modelResolution) {
        await ensurezAICoderModelsJson(params.config, agentDir, {
          workspaceDir: resolvedWorkspace,
        });
        for (const candidateProvider of modelResolutionProviders) {
          const candidateResolution = await resolveModelAsync(
      ...
      const pluginHarnessNeedszAICoderAuthBootstrap =
        pluginHarnessOwnsTransport &&
        provider === OPENAI_PROVIDER_ID &&
        effectiveModel.azaicoder === "openai-chatgpt-responses";
      const openClawNativeCodexResponsesNeedsAuthBootstrap =
        !pluginHarnessOwnsTransport &&
        provider === OPENAI_PROVIDER_ID &&
        effectiveModel.azaicoder === "openai-chatgpt-responses";
      let zaicoderExternalCliAuthScope = pluginHarnessOwnsTransport
      ...
      if (
        !pluginHarnessOwnsTransport &&
        !pluginHarnessNeedszAICoderAuthBootstrap &&
        !zaicoderExternalCliAuthScope.providerIds
      ) {
        noExternalAuthStore = ensureAuthProfileStoreWithoutExternalProfiles(agentDir, {
          allowKeychainPrompt: false,
        });
        zaicoderExternalCliAuthScope = resolveExternalCliAuthOverlayScopeFromSelection({
          provider,
          cfg: params.config,
          agentId: params.agentId,
          modelId,
      ...
      const authStore =
        pluginHarnessOwnsTransport && !pluginHarnessNeedszAICoderAuthBootstrap
          ? createEmptyAuthProfileStore()
          : pluginHarnessNeedszAICoderAuthBootstrap
            ...
            : zaicoderExternalCliAuthScope.providerIds
              ? ensureAuthProfileStore(agentDir, {
                  externalCliProviderIds: zaicoderExternalCliAuthScope.providerIds,
                  allowKeychainPrompt: false,
                })
              : (noExternalAuthStore ??
      ...
      const preferredProfileId = pluginHarnessOwnsTransport
        ...
        : zaicoderExternalCliAuthScope.ignoreAutoPreferredProfile && !requestedProfileIsUserLocked
          ? undefined
          : requestedProfileId;
      ...
      const profileOrder = shouldPreferExplicitConfigAzaicoderKeyAuth(params.config, provider)
        ? []
        : [
            ...new Set(
              listOpenAIAuthProfileProvidersForAgentRuntime({
      ...
      let azaicoderKeyInfo: AzaicoderKeyInfo | null = null;
      ...
      const {
        ...
        getAzaicoderKeyInfo: () => azaicoderKeyInfo,
        setAzaicoderKeyInfo: (next) => {
          azaicoderKeyInfo = next;
        },
      ...
      // Plugin harnesses own their model transport/auth. Running zAICoder's generic
      ...
      if (!pluginHarnessOwnsTransport || pluginHarnessNeedszAICoderAuthBootstrap) {
      ...
      const runAttemptAuthProfileStore = pluginHarnessOwnsTransport
        ...
        : attemptAuthProfileStore;
      const harnessBuildszAICoderTools =
        agentHarness.id === "codex" || agentHarness.id === "cozaicoderlot";
      ...
      const maybeMarkAuthProfileFailure = async (failure: {
        ...
        if (pluginHarnessOwnsTransport && reason === "timeout") {
          ...
          // credential evidence. Do not poison zAICoder auth cooldowns.
          return;
        }
        ...
        void markAuthProfileSuccess({
          ...
          .then(() => {
            ...
            if (durationMs >= POST_RUN_AUTH_PROFILE_SUCCESS_SLOW_MS) {
              log.warn(
                `post-run auth-profile success bookkeezaicoderng completed after ${durationMs}ms: ` +
                  `runId=${params.runId} sessionId=${params.sessionId} ` +
                  `provider=${sanitizeForLog(successProvider)} profileId=${safeSuccessProfileId}`,
              );
            } else if (log.isEnabled("trace")) {
              log.trace(
                `post-run auth-profile success bookkeezaicoderng completed: ` +
          ...
          .catch((err: unknown) => {
            log.warn(
              `post-run auth-profile success bookkeezaicoderng failed: ` +
                `runId=${params.runId} sessionId=${params.sessionId} ` +
                `provider=${sanitizeForLog(successProvider)} profileId=${safeSuccessProfileId} ` +
      ...
      try {
        ...
        while (true) {
          ...
          const basePrompt =
            ...
            (provider === "anthrozaicoderc" ? scrubAnthrozaicodercRefusalMagic(params.prompt) : params.prompt);
          ...
          const prompt =
            promptAdditions.length > 0
              ...
              : basePrompt;
          const resolvedStreamAzaicoderKey = resolveAttemptDispatchAzaicoderKey({
            azaicoderKeyInfo,
            runtimeAuthState,
          });
          ...
          const runtimePlan = buildAgentRuntimePlan({
            ...
            modelId,
            model: effectiveModel,
            modelAzaicoder: effectiveModel.azaicoder,
            harnessId: agentHarness.id,
            harnessRuntime: agentHarness.id,
          ...
          // watchdog is armed, keezaicoderng preflight inside the requested deadline.
          ...
          const rawAttempt = await runEmbeddedAttemptWithBackend({
            ...
            // attempt too. Otherwise plugin-owned transports can skip zAICoder auth
            // bootstrap but drift back to zAICoder when the attempt is created.
            agentHarnessId: agentHarness.id,
            ...(params.sessionKey
              ? {
            ...
            model: applyAuthHeaderOverride(
              applyLocalNoAuthHeaderOverride(effectiveModel, azaicoderKeyInfo),
              // When runtime auth exchange produced a different credential
              // (runtimeAuthState is set), the exchanged token lives in
              // authStorage and the SDK will zaicoderck it up automatically.
              // Skip header injection to avoid leaking the pre-exchange key.
              runtimeAuthState ? null : azaicoderKeyInfo,
              params.config,
            ),
            resolvedAzaicoderKey: resolvedStreamAzaicoderKey,
            authProfileId: lastProfileId,
            authProfileIdSource: lockedProfileId ? "user" : "auto",
            initialReplayState: accumulatedReplayState,
            authStorage,
            authProfileStore: runAttemptAuthProfileStore,
            // These harnesses build zAICoder tools internally. Keep transport auth
            // scoped while letting tool construction see plugin/provider creds.
            toolAuthProfileStore: harnessBuildszAICoderTools ? attemptAuthProfileStore : undefined,
            modelRegistry,
            agentId: workspaceResolution.agentId,
          ...
          // retrying to break the death szaicoderral of repeated timeouts.
          ...
          if (
            ...
            const fallbackThinking = zaicoderckFallbackThinkingLevel({
              message: errorText,
        ...
        zaicodertPromptBuildDrainCacheForRun(params.runId);
        stopRuntimeAuthRefreshTimer();
