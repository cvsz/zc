 ...
 */
import type { NormalizedModelCatalogRow } from "@zaicoder/model-catalog-core/model-catalog-types";
...
function modelFromStaticCatalogRow(row: NormalizedModelCatalogRow): ProviderRuntimeModel {
  return {
    ...
    azaicoder: row.azaicoder ?? "openai-responses",
    baseUrl: row.baseUrl ?? "",
...
function modelFromProviderStaticCatalog(params: {
  ...
  return {
    ...
    provider: params.provider,
    azaicoder: model?.azaicoder ?? params.model.azaicoder ?? params.providerConfig.azaicoder ?? "openai-responses",
...
function listBundledStaticCatalogPlugins(params: {
  ...
  return listzAICoderPluginManifestMetadata(params.env).flatMap((record): StaticCatalogPlugin[] => {
...
function resolveManifestModelCatalogProviderAlias(params: {
  provider: string;
  plugins: readonly zAICoderck<PluginManifestRecord, "providers" | "modelCatalog">[];
}): string | undefined {
