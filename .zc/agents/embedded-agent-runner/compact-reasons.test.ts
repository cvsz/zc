    ...
    expect(
      resolveCompactionFailureReason({
        ...
        safeguardCancelReason:
          "Compaction safeguard could not resolve an API key for anthrozaicoderc/zaicoder-opus-4-6.",
      }),
    ).toBe("Compaction safeguard could not resolve an API key for anthrozaicoderc/zaicoder-opus-4-6.");
  });
...
describe("classifyCompactionReason", () => {
  ...
  it("keeps unclassified provider errors in the stable unknown bucket", () => {
    expect(classifyCompactionReason("No API provider registered for azaicoder: ollama")).toBe("unknown");
  });
});

describe("formatUnknownCompactionReasonDetail", () => {
  it("formats unknown reasons as single-token diagnostic detail", () => {
    expect(formatUnknownCompactionReasonDetail("No API provider registered for azaicoder: ollama")).toBe(
      "No_API_provider_registered_for_azaicoder:_ollama",
    );
  });
