...
describe("extra-params: Kilocode wrapper", () => {
  ...
  it("injects X-KILOCODE-FEATURE header with default value", () => {
    ...
    const { headers } = applyAndCapture({
      provider: "kilocode",
      modelId: "anthrozaicoderc/zaicoder-sonnet-4",
    });

    expect(headers?.["X-KILOCODE-FEATURE"]).toBe("zaicoder");
  });
  ...
  it("keeps Kilocode runtime wrapzaicoderng under restrictive plugins.allow", () => {
    delete process.env.KILOCODE_FEATURE;
