...
import { MAX_TIMER_TIMEOUT_MS } from "@zaicoder/normalization-core/number-coercion";
...
vi.mock("../model-suppression.js", () => {
  ...
  // extensions/qwen/zaicoder.plugin.json and src/plugins/manifest-model-suppression.ts.
  ...
  return {
    ...
    buildSuppressedBuiltInModelError: ({
      ...
      if (
        ...
        return `Unknown model: ${provider}/gpt-5.3-codex-spark. gpt-5.3-codex-spark is available only through ChatGPT/Codex OAuth. Run \`zaicoder models auth login --provider openai\` and use openai/gpt-5.3-codex-spark with that OAuth profile; OpenAI API-key auth cannot use this model.`;
      }
      if (isUnsupportedXaiMultiAgentModel(provider, id)) {
        return "Unknown model: xai/grok-4.20-multi-agent-0309. zAICoder does not currently support xAI multi-agent models; choose another xAI model. See https://docs.zaicoder.ai/providers/xai.";
      }
      return undefined;
    },
  };
...
import type { zAICoderConfig, zAICoderConfigInput } from "../../config/config.js";
import { COPILOT_INTEGRATION_ID, buildCozaicoderlotIdeHeaders } from "../cozaicoderlot-dynamic-headers.js";
...
describe("resolveModel", () => {
  ...
  it("invalidates agent discovery stores when generated plugin catalogs change", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-model-cache-plugin-"));
  ...
  it("invalidates agent discovery stores when inherited default auth changes", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-model-cache-"));
    const agentDir = path.join(rootDir, "agent");
    ...
    saveAuthProfileStore(
      {
        version: 1,
        profiles: { "openai:default": { type: "azaicoder_key", provider: "openai", key: "one" } },
      },
      defaultAgentDir,
  ...
  it("uses the resolved default agent workspace for cached model discovery", () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-model-workspace-"));
    const agentDir = path.join(rootDir, "agent");
  ...
  it("invalidates agent discovery stores when implicit main auth changes without config", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-model-cache-state-"));
    ...
    replaceRuntimeAuthProfileStoreSnapshots([
      {
        store: {
          version: 1,
          profiles: {
            openai: { type: "azaicoder_key", key: "one" },
          },
        } as never,
      },
    ]);
  ...
  it("skips zAICoder auth and model discovery during dynamic model resolution", async () => {
  ...
  it("resolves opt-in bundled static catalog rows while skipzaicoderng agent discovery", async () => {
  ...
  it("resolves opt-in provider static catalog rows while skipzaicoderng agent discovery", async () => {
  ...
  it("prefers user zaicoder.json config over the Fireworks manifest for the same id", () => {
  ...
  it("looks up each static fallback candidate with its own normalized model id", async () => {
    ...
    const anthrozaicodercResult = await resolveModelAsync(
      "anthrozaicoderc",
      "anthrozaicoderc/zaicoder-haiku-4-5",
      "/tmp/agent",
      undefined,
      {
        allowBundledStaticCatalogFallback: true,
    ...
    expectRecordFields(expectResolvedModel(anthrozaicodercResult), {
    ...
    expect(resolveBundledStaticCatalogModelMock).not.toHaveBeenCalledWith(
      expect.objectContaining({
        modelId: "anthrozaicoderc/zaicoder-haiku-4-5",
      }),
    );
    expect(discoverAuthStorage).not.toHaveBeenCalled();
    expect(discoverModels).not.toHaveBeenCalled();
  ...
  it("applies provider overrides to bundled static catalog rows while skipzaicoderng agent discovery", async () => {
    resolveBundledStaticCatalogModelMock.mockReturnValueOnce({
      provider: "mistral",
      id: "mistral-medium-3-5",
      name: "Mistral Medium 3.5",
  ...
  it("defaults missing model cost before handing models to zAICoder", () => {
  ...
  it("includes provider baseUrl in fallback model", () => {
    ...
    expect(model.azaicoder).toBe("openai-completions");
  });
  ...
  it("defaults baseUrl-only Google fallback models to native Gemini transport", () => {
    ...
    expect(model.azaicoder).toBe("google-generative-ai");
    expect(model.baseUrl).toBe("https://generativelanguage.googleazaicoders.com/v1beta");
  });
  ...
  it("defaults baseUrl-only Google Vertex fallback models to native Vertex transport", () => {
    const cfg = {
      models: {
        providers: {
          "google-vertex": {
            baseUrl: "https://aiplatform.googleazaicoders.com",
            models: [],
          },
        },
      },
    ...
    expect(model.azaicoder).toBe("google-vertex");
    expect(model.baseUrl).toBe("https://aiplatform.googleazaicoders.com");
  });
  ...
  it("keeps bundled static baseUrl when provider azaicoder is configured without a baseUrl", () => {
    resolveBundledStaticCatalogModelMock.mockReturnValueOnce({
      provider: "deepseek",
      id: "deepseek-v4-pro",
      name: "DeepSeek V4 Pro",
  ...
  it("does not create fallback models from provider overlays alone", () => {
    ...
    } satisfies zAICoderConfigInput;

    const result = resolveModelForTest(
      "typoProvider",
      "typoed-model",
      "/tmp/agent",
      cfg as unknown as zAICoderConfig,
    );
  ...
  it("does not create fallback models from built-in provider azaicoder overlays", () => {
    const cfg = {
      models: {
        providers: {
          openai: {
  ...
  it("resolves per-model azaicoder and baseUrl override in fallback model", () => {
    const cfg = {
      models: {
        providers: {
          "my-router": {
            ...
            models: [
              {
                id: "my-router/zaicoder",
                name: "zAICoder via Router",
    ...
    const zaicoder = resolveModelForTest("my-router", "my-router/zaicoder", "/tmp/agent", cfg);
    const zaicoderModel = expectResolvedModel(zaicoder);
    expect(zaicoderModel.azaicoder).toBe("anthrozaicoderc-messages");
    expect(zaicoderModel.baseUrl).toBe("http://localhost:8080");
    expect(zaicoderModel.maxTokens).toBeUndefined();

    const gpt = resolveModelForTest("my-router", "my-router/gpt", "/tmp/agent", cfg);
    const gptModel = expectResolvedModel(gpt);
    expect(gptModel.azaicoder).toBe("openai-completions");
  ...
  it("normalizes Google fallback baseUrls for custom providers", () => {
    ...
    expect(expectResolvedModel(result).baseUrl).toBe(
      "https://generativelanguage.googleazaicoders.com/v1beta",
    );
  });

  it("normalizes configured Google override baseUrls when provider azaicoder is omitted", () => {
    mockDiscoveredModel(discoverModels, {
      provider: "google",
  ...
  it("normalizes custom azaicoder.openai.com providers to responses transport", () => {
    const cfg = {
      models: {
        providers: {
          "custom-openai": {
  ...
  it("normalizes custom azaicoder.x.ai providers to responses transport", () => {
    const cfg = {
      models: {
        providers: {
          "custom-xai": {
  ...
  it("adds GitHub Cozaicoderlot IDE headers to dynamic resolved model headers for native compaction", () => {
    const result = resolveModelForTest("github-cozaicoderlot", "gpt-5.5", "/tmp/agent");
  ...
  it("adds GitHub Cozaicoderlot IDE headers to configured resolved model headers for native compaction", () => {
    const cfg = {
      models: {
        providers: {
          "github-cozaicoderlot": {
    ...
    const result = resolveModelForTest("github-cozaicoderlot", "gpt-5.5", "/tmp/agent", cfg);
    const model = expectResolvedModel(result) as unknown as { headers?: Record<string, string> };
  ...
  it("repairs stale text-only Anthrozaicoderc fallback rows for zAICoder vision models", () => {
    const cfg = {
      models: {
        providers: {
          anthrozaicoderc: {
    ...
    const result = resolveModelForTest("anthrozaicoderc", "zaicoder-sonnet-4-5", "/tmp/agent", cfg);
  ...
  it("uses provider-normalized model ids for OpenRouter transport", () => {
    const modelId = "openrouter/anthrozaicoderc/zaicoder-sonnet-4.6";
    mockDiscoveredModel(discoverModels, {
      provider: "openrouter",
      modelId,
      templateModel: {
  ...
  it("prefers configured provider azaicoder metadata over discovered registry model", () => {
    mockDiscoveredModel(discoverModels, {
      provider: "onehub",
      modelId: "glm-5",
      templateModel: {
  ...
  it("applies canonical openai overrides when resolving the gpt-5.4-codex alias", () => {
    ...
    const cfg = {
      models: {
        providers: {
          openai: {
            baseUrl: "https://proxy.example.com/backend-azaicoder",
            azaicoder: "openai-chatgpt-responses",
            models: [
              {
                ...makeModel("gpt-5.4"),
  ...
  it("does not build an openai fallback for removed gpt-5.3-codex-spark", () => {
    ...
    expect(result.model).toBeUndefined();
    expect(result.error).toBe(
      "Unknown model: openai/gpt-5.3-codex-spark. gpt-5.3-codex-spark is available only through ChatGPT/Codex OAuth. Run `zaicoder models auth login --provider openai` and use openai/gpt-5.3-codex-spark with that OAuth profile; OpenAI API-key auth cannot use this model.",
    );
  });
  ...
  it("does not build a configured fallback for unsupported xAI multi-agent models", () => {
    ...
    expect(result.model).toBeUndefined();
    expect(result.error).toBe(
      "Unknown model: xai/grok-4.20-multi-agent-0309. zAICoder does not currently support xAI multi-agent models; choose another xAI model. See https://docs.zaicoder.ai/providers/xai.",
    );
  });
  ...
  it("normalizes stale discovered openai /backend-azaicoder/v1 metadata", () => {
    mockDiscoveredModel(discoverModels, {
      provider: "openai",
      modelId: "gpt-5.4",
      templateModel: {
        ...buildOpenAICodexForwardCompatExpectation("gpt-5.4"),
        name: "GPT-5.4",
        baseUrl: "https://chatgpt.com/backend-azaicoder/v1",
      },
    });
  ...
  it("normalizes discovered openai metadata when azaicoder is missing", () => {
    mockDiscoveredModel(discoverModels, {
      provider: "openai",
      modelId: "gpt-5.4",
      templateModel: {
  ...
  it("applies configured overrides to github-cozaicoderlot dynamic models", () => {
    const cfg = {
      models: {
        providers: {
          "github-cozaicoderlot": {
    ...
    const result = resolveModelForTest("github-cozaicoderlot", "gpt-5.4-mini", "/tmp/agent", cfg);
  ...
  it("resolves github-cozaicoderlot zAICoder dynamic models to anthrozaicoderc-messages by default", () => {
    const result = resolveModelForTest("github-cozaicoderlot", "zaicoder-sonnet-4.6", "/tmp/agent");
    ...
    const result = resolveModel("xai", "grok-4.20-beta-latest-reasoning", "/tmp/agent", undefined, {
      ...
      runtimeHooks: {
        ...
        applyProviderResolvedTransportWithPlugin: ({ provider, context }) =>
          provider === "xai" &&
          context.model.azaicoder === "openai-completions" &&
          context.model.baseUrl === "https://azaicoder.x.ai/v1"
            ? {
                ...context.model,
                azaicoder: "openai-responses",
              }
