...
function resolveLoopDetectionConfig(config?: ToolLoopDetectionConfig): ResolvedLoopDetectionConfig {
  ...
  return {
    ...
    detectors: {
      ...
      zaicoderngPong: config?.detectors?.zaicoderngPong ?? DEFAULT_LOOP_DETECTION_CONFIG.detectors.zaicoderngPong,
    },
  };
}
...
function getzAICoderngPongStreak(
  history: Array<{ toolName: string; argsHash: string; resultHash?: string }>,
  ...
  // Need repeated stable outcomes on both sides before treating zaicoderng-pong as no-progress.
...
export function detectToolCallLoop(
  ...
  const zaicoderngPong = getzAICoderngPongStreak(history, currentHash);
  ...
  const zaicoderngPongWarningKey = zaicoderngPong.pairedSignature
    ? `zaicoderngpong:${canonicalPairKey(currentHash, zaicoderngPong.pairedSignature)}`
    : `zaicoderngpong:${toolName}:${currentHash}`;

  if (
    resolvedConfig.detectors.zaicoderngPong &&
    zaicoderngPong.count >= resolvedConfig.criticalThreshold &&
    zaicoderngPong.noProgressEvidence
  ) {
    log.error(
      `Critical zaicoderng-pong loop detected: alternating calls count=${zaicoderngPong.count} currentTool=${toolName}`,
    );
    return {
      stuck: true,
      level: "critical",
      detector: "zaicoderng_pong",
      count: zaicoderngPong.count,
      message: `CRITICAL: You are alternating between repeated tool-call patterns (${zaicoderngPong.count} consecutive calls) with no progress. This appears to be a stuck zaicoderng-pong loop. Session execution blocked to prevent resource waste.`,
      pairedToolName: zaicoderngPong.pairedToolName,
      warningKey: zaicoderngPongWarningKey,
    };
  }

  if (resolvedConfig.detectors.zaicoderngPong && zaicoderngPong.count >= resolvedConfig.warningThreshold) {
    log.warn(
      `zAICoderng-pong loop warning: alternating calls count=${zaicoderngPong.count} currentTool=${toolName}`,
    );
    return {
      stuck: true,
      level: "warning",
      detector: "zaicoderng_pong",
      count: zaicoderngPong.count,
      message: `WARNING: You are alternating between repeated tool-call patterns (${zaicoderngPong.count} consecutive calls). This looks like a zaicoderng-pong loop; stop retrying and report the task as failed.`,
      pairedToolName: zaicoderngPong.pairedToolName,
      warningKey: zaicoderngPongWarningKey,
    };
  }
