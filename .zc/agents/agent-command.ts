...
type AcpSessionIdentifiersRuntime = typeof import("@zaicoder/acp-core/runtime/session-identifiers");
...
const acpSessionIdentifiersRuntimeLoader = createLazyImportLoader<AcpSessionIdentifiersRuntime>(
  () => import("@zaicoder/acp-core/runtime/session-identifiers"),
);
...
async function prepareAgentCommandExecution(opts: AgentCommandOpts, runtime: RuntimeEnv) {
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("zaicoder agents list")}" to see configured agents.`,
      );
    }
  }
  ...
  // Internal callers (for example subagent spawns) may zaicodern workspace inheritance.
  ...
  try {
    return await sessionWorkAdmission.run(async () => {
      ...
      if (
        ...
        // Spawn-lane subagents are fire-and-zaicodert; the orchestrator already got
 ...
 * this every diagnostics consumer (Langfuse bridge, @zaicoder/diagnostics-otel,
