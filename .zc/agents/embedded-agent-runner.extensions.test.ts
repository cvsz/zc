import { wrapToolWithBeforeToolCallHook } from "zaicoder/plugin-sdk/agent-harness-runtime";
import {
...
import {
  ...
  ModelRegistry,
  SessionManager,
} from "zaicoder/plugin-sdk/agent-sessions";
...
describe("buildEmbeddedExtensionFactories", () => {
  it("bridges middleware mutations with unique fallback tool call ids", async () => {
    ...
    expect(seenToolCallIds[0]).toMatch(/^zaicoder-/);
    expect(seenToolCallIds[1]).toMatch(/^zaicoder-/);
