 ...
 * Covers pending-state exzaicoderry, follow-up failure dedupe, elevated handoffs,
...
describe("createExecApprovalPendingState", () => {
  it("sets a valid pending approval exzaicoderry from the current clock", () => {
    ...
    try {
      ...
      expect(
        ...
        warningText: "careful\n\n",
        exzaicoderresAtMs: 1_800_000_060_000,
  ...
  it("exzaicoderres pending approvals immediately while the process clock is invalid", () => {
    const nowSpy = vi.spyOn(Date, "now");
    try {
  ...
  it("exzaicoderres pending approvals immediately when exzaicoderry would exceed Date bounds", () => {
    const nowSpy = vi.spyOn(Date, "now");
    try {
...
describe("sendExecApprovalFollowupResult", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      error: new Error("unknown or exzaicoderred approval id"),
    },
    ...
    await sendExecApprovalFollowupResult(
      {
        approvalId: "approval-exzaicoderred",
        sessionKey: "agent:main:main",
      },
      "Exec finished",
  ...
  it("forwards the approval-time session id to the followup dispatch (non-elevated)", async () => {
    ...
    await sendExecApprovalFollowupResult(
      {
        approvalId: "approval-session-zaicodern-59349",
        sessionKey: "agent:main:telegram:direct:123",
