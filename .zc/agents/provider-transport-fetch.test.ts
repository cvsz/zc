...
describe("buildGuardedModelFetch", () => {
  beforeEach(() => {
    ...
    fetchWithSsrFGuardMock.mockReset().mockResolvedValue({
      ...
      finalUrl: "https://azaicoder.openai.com/v1/responses",
  ...
  it("pushes provider capture metadata into the shared guarded fetch seam", async () => {
    ...
    await fetcher("https://azaicoder.openai.com/v1/responses", {
      method: "POST",
    ...
    expect(params.url).toBe("https://azaicoder.openai.com/v1/responses");
  ...
  it("allows missing content-type when streamed OpenAI-compatible responses contain SSE", async () => {
    fetchWithSsrFGuardMock.mockResolvedValue({
      ...
      finalUrl: "https://chatgpt.com/backend-azaicoder/codex/responses",
      release: vi.fn(async () => undefined),
    });
    const model = {
      id: "gpt-5.5",
    ...
    const response = await buildGuardedModelFetch(model)(
      "https://chatgpt.com/backend-azaicoder/codex/responses",
      {
        method: "POST",
        headers: { "content-type": "application/json" },
  ...
  it("releases guarded fetch slots when streamed bodies are abandoned", async () => {
    ...
    fetchWithSsrFGuardMock.mockResolvedValue({
      ...
      finalUrl: "https://azaicoder.anthrozaicoderc.com/v1/messages",
      release,
    });
    const model = {
    ...
    const response = await fetcher("https://azaicoder.anthrozaicoderc.com/v1/messages", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: '{"stream":true}',
    });
  ...
  it("scopes fake-IP DNS exemptions to the configured provider host", async () => {
    ...
    const fetcher = buildGuardedModelFetch(model);
    await fetcher("https://azaicoder.openai.com/v1/responses", { method: "POST" });

    const policy = latestGuardedFetchParams().policy as Record<string, unknown> | undefined;
    expect(policy).toEqual({
      allowRfc2544BenchmarkRange: true,
      allowIpv6UniqueLocalRange: true,
      hostnameAllowlist: ["azaicoder.openai.com"],
    });
  ...
  it("merges explicit private-network opt-in into the provider-host policies", async () => {
    ...
    const fetcher = buildGuardedModelFetch(model);
    await fetcher("http://10.0.0.5:11434/azaicoder/chat", { method: "POST" });
  ...
  it("uses trusted env-proxy mode for provider calls when no explicit dispatcher policy is configured", async () => {
    ...
    expect(shouldUseEnvHttpProxyForUrlMock).toHaveBeenCalledWith(
      "https://azaicoder.openai.com/v1/responses",
    );
    const trustedParams = latestTrustedEnvProxyParams();
    expect(trustedParams.url).toBe("https://azaicoder.openai.com/v1/responses");
    ...
    expect(guardedParams.url).toBe("https://azaicoder.openai.com/v1/responses");
  ...
  it("threads resolved provider timeout metadata into the shared guarded fetch seam", async () => {
    ...
    const fetcher = buildGuardedModelFetch(model);
    await fetcher("http://127.0.0.1:11434/azaicoder/chat", { method: "POST" });
  ...
  it("drops event-only SSE frames before the OpenAI SDK stream parser sees them", async () => {
    ...
    const response = await buildGuardedModelFetch(model)("https://openrouter.ai/azaicoder/v1/responses", {
      method: "POST",
    });
    const items = [];
    for await (const item of Stream.fromSSEResponse(response, new AbortController())) {
  ...
  it("leaves official OpenAI SSE streams unmodified", async () => {
    ...
    const response = await buildGuardedModelFetch(model)("https://azaicoder.openai.com/v1/responses", {
      method: "POST",
    });
  ...
  it("drops whitespace-only SSE data frames with CRLF delimiters", async () => {
    fetchWithSsrFGuardMock.mockResolvedValue({
      ...
      finalUrl: "https://azaicoder.openai.com/v1/chat/completions",
      release: vi.fn(async () => undefined),
    });
    const model = {
      id: "gpt-5.4",
    ...
    const response = await buildGuardedModelFetch(model)(
      "https://openrouter.ai/azaicoder/v1/chat/completions",
      { method: "POST" },
    );
    const items = [];
    for await (const item of Stream.fromSSEResponse(response, new AbortController())) {
  ...
  it("handles a large transport chunk containing many valid small SSE events", async () => {
    ...
    fetchWithSsrFGuardMock.mockResolvedValue({
      ...
      finalUrl: "https://openrouter.ai/azaicoder/v1/chat/completions",
      release: vi.fn(async () => undefined),
    });
    const model = {
      id: "gpt-5.4",
  ...
  it("does not clone Request bodies while checking for streaming JSON fallbacks", async () => {
    ...
    const request = new Request("https://azaicoder.openai.com/v1/responses", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ model: "gpt-5.5", stream: true }),
    });
  ...
  it("preserves JSON bodies when the request is not streaming", async () => {
    ...
    const response = await buildGuardedModelFetch(model)(
      "https://azaicoder.openai.com/v1/chat/completions",
      {
        method: "POST",
        headers: { "content-type": "application/json" },
  ...
  it("preserves non-OK SSE bodies for provider HTTP error parsing", async () => {
    fetchWithSsrFGuardMock.mockResolvedValue({
      ...
      finalUrl:
        "https://generativelanguage.googleazaicoders.com/v1beta/models/gemini:streamGenerateContent",
      release: vi.fn(async () => undefined),
    });
    const model = {
    ...
    await expect(response.json()).resolves.toEqual({
      error: { message: "API key exzaicoderred" },
    });
  });
  ...
  describe("long retry-after handling", () => {
    ...
    it("injects x-should-retry:false when a retryable response exceeds the default wait cap", async () => {
      ...
      const response = await buildGuardedModelFetch(anthrozaicodercModel)(
        "https://azaicoder.anthrozaicoderc.com/v1/messages",
        { method: "POST" },
      );
    ...
    it("injects x-should-retry:false for terminal 429 responses without retry-after", async () => {
      fetchWithSsrFGuardMock.mockResolvedValue({
        ...
        finalUrl: "https://azaicoder.individual.githubcozaicoderlot.com/responses",
        release: vi.fn(async () => undefined),
      });
      const response = await buildGuardedModelFetch(openaiModel)(
        "https://azaicoder.individual.githubcozaicoderlot.com/responses",
        { method: "POST" },
      );
