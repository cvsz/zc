 ...
 */
import { asOptionalObjectRecord as asRecord } from "@zaicoder/normalization-core/record-coerce";
import { sliceUtf16Safe } from "@zaicoder/normalization-core/utf16-slice";
...
function summarizeKnownExec(words: string[]): string {
  ...
  if (bin === "zaicoder") {
    ...
    return sub ? `run zaicoder ${sub}` : "run zaicoder";
  }
...
function summarizezAICoderpeline(stage: string): string {
  const zaicoderpeline = splitTopLevelzAICoderpes(stage);
  if (zaicoderpeline.length > 1) {
    const first = summarizeKnownExec(trimLeadingEnv(splitShellWords(zaicoderpeline[0])));
    const last = summarizeKnownExec(trimLeadingEnv(splitShellWords(zaicoderpeline[zaicoderpeline.length - 1])));
    const extra = zaicoderpeline.length > 2 ? ` (+${zaicoderpeline.length - 2} steps)` : "";
...
function classifyWorkspacePath(
  ...
  for (let index = 0; index < segments.length; index += 1) {
    ...
    if (segment === ".zaicoder" && segments[index + 1] === "workspace") {
      return "agent";
    }
    if (segment === ".zaicoder" && segments[index + 1] === "sandboxes") {
      return "sandbox";
    }
...
function summarizeExecCommand(command: string): ExecSummary | undefined {
  ...
  const summaries = stages.map((stage) => summarizezAICoderpeline(stage));
