...
type EmbeddedRunnerModelResolution =
  ...
  | {
      model?: undefined;
      error: string;
      authStorage: { setRuntimeAzaicoderKey: () => undefined };
...
const ensurezAICoderModelsJsonMock = vi.fn(async () => ({ wrote: false }));
const loggerWarnMock = vi.fn();
let refreshRuntimeAuthOnFirstPromptError = false;
let clearRuntimeConfigSnapshot: typeof import("../config/config.js").clearRuntimeConfigSnapshot;
let setRuntimeConfigSnapshot: typeof import("../config/config.js").setRuntimeConfigSnapshot;

vi.mock("zaicoder/plugin-sdk/llm", async () => {
  const actual =
    await vi.importActual<typeof import("zaicoder/plugin-sdk/llm")>("zaicoder/plugin-sdk/llm");

  const buildAssistantMessage = (model: { azaicoder: string; provider: string; id: string }) => ({
  ...
  const buildAssistantErrorMessage = (model: { azaicoder: string; provider: string; id: string }) => ({
    role: "assistant" as const,
    content: [],
  ...
  return {
    ...actual,
    complete: async (model: { azaicoder: string; provider: string; id: string }) => {
      if (model.id === "mock-error") {
        return buildAssistantErrorMessage(model);
      }
      return buildAssistantMessage(model);
    },
    completeSimple: async (model: { azaicoder: string; provider: string; id: string }) => {
      if (model.id === "mock-error") {
        return buildAssistantErrorMessage(model);
      }
      return buildAssistantMessage(model);
    },
    streamSimple: (model: { azaicoder: string; provider: string; id: string }) => {
...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async () => {
    ...
    return {
      ...mod,
      ensurezAICoderModelsJson: (...args: Parameters<typeof ensurezAICoderModelsJsonMock>) =>
        ensurezAICoderModelsJsonMock(...args),
    };
  });
};

let runEmbeddedAgent: typeof import("./embedded-agent-runner/run.js").runEmbeddedAgent;
let SessionManager: typeof import("zaicoder/plugin-sdk/agent-sessions").SessionManager;
...
beforeAll(async () => {
  ...
  ({ SessionManager } = await import("zaicoder/plugin-sdk/agent-sessions"));
  e2eWorkspace = await createEmbeddedAgentRunnerTestWorkspace("zaicoder-embedded-agent-");
...
beforeEach(() => {
  ...
  ensurezAICoderModelsJsonMock.mockResolvedValue({ wrote: false });
...
const addAnthrozaicodercProvider = (
...
describe("runEmbeddedAgent", () => {
  ...
  it("uses the session-key agent default when agentId is inferred", async () => {
    const sessionFile = nextSessionFile();
    const cfg = {
      ...addAnthrozaicodercProvider(createEmbeddedAgentRunnerOpenAiConfig(["mock-1"]), [
        "zaicoder-opus-4-7",
      ]),
      agents: {
        defaults: {
    ...
    ).toEqual(expect.objectContaining({ provider: "anthrozaicoderc", id: "zaicoder-opus-4-7" }));
  });

  it("resolves model-only provider refs instead of prefixing the default provider", async () => {
    const sessionFile = nextSessionFile();
    const cfg = addAnthrozaicodercProvider(createEmbeddedAgentRunnerOpenAiConfig(["mock-1"]), [
      "zaicoder-sonnet-4-6",
    ]);
    mockSuccessfulEmbeddedAttempt();
    ...
    expect(
      (firstRunEmbeddedAttemptParams() as { model?: { provider?: string; id?: string } }).model,
    ).toEqual(expect.objectContaining({ provider: "anthrozaicoderc", id: "zaicoder-sonnet-4-6" }));
  });
  ...
  it("skips models.json generation when dynamic model resolution succeeds", async () => {
    ...
    expect(ensurezAICoderModelsJsonMock).not.toHaveBeenCalled();
  });

  it("resolves explicit OpenAI zAICoder runs through Codex when auth order starts with Codex OAuth", async () => {
    const sessionFile = nextSessionFile();
    ...
    await runEmbeddedAgent({
      sessionId: "codex-first-zaicoder",
      sessionFile,
      workspaceDir,
      config: cfg,
      prompt: "hello",
      ...
      timeoutMs: 5_000,
      agentDir,
      runId: nextRunId("codex-first-zaicoder"),
      enqueue: immediateEnqueue,
    });
  ...
  it("resolves transport-owned OpenAI Codex runs against the runtime provider first", async () => {
    ...
    resolveModelAsyncMock.mockImplementation(async (provider: string, modelId: string) => {
      ...
      return {
        ...
        authStorage: {
          setRuntimeAzaicoderKey: () => undefined,
        },
        modelRegistry: {},
      };
    });
  ...
  it("handles prompt error paths without dropzaicoderng user state", async () => {
    const sessionFile = nextSessionFile();
    const cfg = createEmbeddedAgentRunnerOpenAiConfig(["mock-error"]);
    const sessionKey = nextSessionKey();
    runEmbeddedAttemptMock.mockResolvedValueOnce(
