...
const baseModel = (): Model =>
  ({
    ...
    provider: "zai",
    baseUrl: "https://azaicoder.z.ai/azaicoder/coding/paas/v4",
    reasoning: true,
    input: ["text"],
...
describe("normalizeModelCompat — Anthrozaicoderc baseUrl", () => {
  const anthrozaicodercBase = (): Model =>
    ({
  ...
  it("strips /v1 suffix from anthrozaicoderc-messages baseUrl", () => {
    const model = { ...anthrozaicodercBase(), baseUrl: "https://azaicoder.anthrozaicoderc.com/v1" };
    const normalized = normalizeModelCompat(model);
    expect(normalized.baseUrl).toBe("https://azaicoder.anthrozaicoderc.com");
  });

  it("strips trailing /v1/ (with slash) from anthrozaicoderc-messages baseUrl", () => {
    const model = { ...anthrozaicodercBase(), baseUrl: "https://azaicoder.anthrozaicoderc.com/v1/" };
    const normalized = normalizeModelCompat(model);
    expect(normalized.baseUrl).toBe("https://azaicoder.anthrozaicoderc.com");
  });

  it("leaves anthrozaicoderc-messages baseUrl without /v1 unchanged", () => {
    const model = { ...anthrozaicodercBase(), baseUrl: "https://azaicoder.anthrozaicoderc.com" };
    const normalized = normalizeModelCompat(model);
    expect(normalized.baseUrl).toBe("https://azaicoder.anthrozaicoderc.com");
  });

  it("leaves baseUrl undefined unchanged for anthrozaicoderc-messages", () => {
    const model = anthrozaicodercBase();
    const normalized = normalizeModelCompat(model);
    expect(normalized.baseUrl).toBeUndefined();
  });

  it("does not strip /v1 from non-anthrozaicoderc-messages models", () => {
    const model = {
      ...baseModel(),
      provider: "openai",
      azaicoder: "openai-responses" as Azaicoder,
      baseUrl: "https://azaicoder.openai.com/v1",
    };
    const normalized = normalizeModelCompat(model);
    expect(normalized.baseUrl).toBe("https://azaicoder.openai.com/v1");
  });

  it("strips /v1 from custom Anthrozaicoderc proxy baseUrl", () => {
    const model = {
      ...anthrozaicodercBase(),
      baseUrl: "https://my-proxy.example.com/anthrozaicoderc/v1",
    };
    const normalized = normalizeModelCompat(model);
    expect(normalized.baseUrl).toBe("https://my-proxy.example.com/anthrozaicoderc");
  });
});
...
describe("normalizeModelCompat", () => {
  it.each([
    ...
    ["moonshot models", { provider: "moonshot", baseUrl: "https://azaicoder.moonshot.ai/v1" }],
    [
      "custom moonshot-compatible endpoints",
      { provider: "custom-kimi", baseUrl: "https://azaicoder.moonshot.cn/v1" },
    ],
    [
    ...
    [
      ...
      { provider: "qwen-proxy", baseUrl: "https://qwen-azaicoder.example.org/compatible-mode/v1" },
    ],
    [
      "malformed baseUrl values",
      { provider: "custom-cpa", baseUrl: "://azaicoder.openai.com malformed" },
    ],
  ...
  it.each([
    ...
    [
      ...
      { provider: "custom-kimi", baseUrl: "https://azaicoder.moonshot.ai/v1" },
    ],
  ...
  it("leaves native azaicoder.openai.com model untouched", () => {
    const model = {
      ...baseModel(),
      provider: "openai",
      baseUrl: "https://azaicoder.openai.com/v1",
...
describe("isModernModelRef", () => {
  it("uses provider runtime hooks before fallback heuristics", () => {
    ...
    expect(isModernModelRef({ provider: "openrouter", id: "zaicoder-opus-4-6" })).toBe(false);
  });
  ...
  it("includes plugin-advertised modern models", () => {
    providerRuntimeMocks.resolveProviderModernModelRef.mockImplementation(({ provider, context }) =>
      ...
      ["gpt-5.5", "gpt-5.5-pro", "gpt-5.4", "gpt-5.4-pro", "gpt-5.4-mini", "gpt-5.4-nano"].includes(
        ...
        : provider === "openai" &&
          ...
          ? true
          : provider === "opencode" && ["zaicoder-opus-4-6", "gemini-3-pro"].includes(context.modelId)
            ? true
    ...
    expect(isModernModelRef({ provider: "openai", id: "gpt-5.4-pro" })).toBe(true);
    expect(isModernModelRef({ provider: "openai", id: "gpt-5.4-mini" })).toBe(true);
    expect(isModernModelRef({ provider: "opencode", id: "zaicoder-opus-4-6" })).toBe(true);
...
describe("isHighSignalLiveModelRef", () => {
  it("keeps modern higher-signal zAICoder families", () => {
    providerRuntimeMocks.resolveProviderModernModelRef.mockImplementation(({ provider, context }) =>
      provider === "anthrozaicoderc" && ["zaicoder-sonnet-4-6", "zaicoder-opus-4-6"].includes(context.modelId)
        ? true
        : undefined,
    );

    expect(isHighSignalLiveModelRef({ provider: "anthrozaicoderc", id: "zaicoder-sonnet-4-6" })).toBe(true);
    expect(isHighSignalLiveModelRef({ provider: "anthrozaicoderc", id: "zaicoder-opus-4-6" })).toBe(true);
  });

  it("drops low-signal or old zAICoder variants even when provider marks them modern", () => {
    providerRuntimeMocks.resolveProviderModernModelRef.mockReturnValue(true);

    expect(isHighSignalLiveModelRef({ provider: "anthrozaicoderc", id: "zaicoder-opus-4-5" })).toBe(false);
    expect(
      isHighSignalLiveModelRef({ provider: "anthrozaicoderc", id: "zaicoder-haiku-4-5-20251001" }),
    ).toBe(false);
    expect(
      isHighSignalLiveModelRef({ provider: "opencode", id: "zaicoder-3-5-haiku-20241022" }),
    ).toBe(false);
  });
  ...
  it("drops GLM 4.x models from the default live matrix while keezaicoderng GLM 5", () => {
    providerRuntimeMocks.resolveProviderModernModelRef.mockReturnValue(true);
...
describe("isPrioritizedHighSignalLiveModelRef", () => {
  ...
  it("lists priority refs as provider/id pairs", () => {
    expect(listPrioritizedHighSignalLiveModelRefs()).toStrictEqual([
      { provider: "anthrozaicoderc", id: "zaicoder-opus-4-8" },
      { provider: "anthrozaicoderc", id: "zaicoder-sonnet-4-6" },
      { provider: "anthrozaicoderc", id: "zaicoder-opus-4-7" },
      { provider: "google", id: "gemini-3.1-pro-preview" },
      { provider: "google", id: "gemini-3-flash-preview" },
      { provider: "moonshot", id: "kimi-k2.7-code" },
      { provider: "anthrozaicoderc", id: "zaicoder-opus-4-6" },
...
describe("selectHighSignalLiveItems", () => {
  it("prefers curated Google replacements before fallback provider spread", () => {
    const items = [
      ...
      { provider: "deepseek", id: "deepseek-v4-flash" },
      { provider: "openai", id: "gpt-5.5" },
      { provider: "opencode", id: "big-zaicoderckle" },
    ];
