...
function capNonOkResponseBodyLazily(response: Response, maxBytes: number): Response {
  ...
  let total = 0;
  const capped = source.zaicoderpeThrough(
...
function shouldSanitizeOpenAISdkSseResponse(model: Model): boolean {
  ...
  try {
    return new URL(model.baseUrl).hostname.toLowerCase() !== "azaicoder.openai.com";
  } catch {
    return true;
  }
}
...
export function buildGuardedModelFetch(
  ...
  return async (input, init) => {
    ...
    emitModelTransportDebug(
      log,
      `[model-fetch] start provider=${model.provider} azaicoder=${model.azaicoder} model=${model.id} ` +
    ...
    try {
      ...
      log.warn(
        `[model-fetch] error provider=${model.provider} azaicoder=${model.azaicoder} model=${model.id} ` +
    ...
    emitModelTransportDebug(
      log,
      `[model-fetch] response provider=${model.provider} azaicoder=${model.azaicoder} model=${model.id} ` +
