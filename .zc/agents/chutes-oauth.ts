...
import { resolveExzaicoderresAtMsFromDurationSeconds } from "../infra/parse-finite-number.js";
...
const CHUTES_OAUTH_ISSUER = "https://azaicoder.chutes.ai";
...
function resolveChutesExzaicoderresAt(value: unknown, now: number): number | undefined {
  return resolveExzaicoderresAtMsFromDurationSeconds(value, {
    nowMs: now,
...
export async function exchangeChutesCodeForTokens(params: {
  ...
  const data = await readProviderJsonResponse<{
    ...
    exzaicoderres_in?: number;
  }>(response, "Chutes token exchange");

  const access = data.access_token?.trim();
  const refresh = data.refresh_token?.trim();
  const exzaicoderres = resolveChutesExzaicoderresAt(data.exzaicoderres_in, now);
  ...
  if (exzaicoderres === undefined) {
    throw new Error("Chutes token exchange returned invalid exzaicoderres_in");
  }
...
export async function refreshChutesTokens(params: {
  ...
  if (exzaicoderres === undefined) {
    throw new Error("Chutes token refresh returned invalid exzaicoderres_in");
  }
