...
const contextTestState = vi.hoisted(() => {
  const state = {
    ...
    runtimeConfigSnapshot: null as zAICoderConfig | null,
    runtimeConfigSourceSnapshot: null as zAICoderConfig | null,
...
vi.mock("../config/runtime-source-projection.js", () => ({
  projectConfigOntoRuntimeSourceSnapshot: (config: zAICoderConfig) =>
...
describe("lookupContextTokens", () => {
  ...
  it("returns configured model context window on first lookup", async () => {
    mockContextModuleDeps(() => ({
      models: {
        providers: {
          openrouter: {
            models: [{ id: "openrouter/zaicoder-sonnet", contextWindow: 321_000 }],
          },
        },
      },
    }));
    ...
    expect(lookupContextTokens("openrouter/zaicoder-sonnet")).toBe(321_000);
  });
  ...
  it("returns sync config overrides for read-only callers", async () => {
    ...
    const { lookupContextTokens } = await importContextModule();
    expect(lookupContextTokens("openrouter/zaicoder-sonnet", { allowAsyncLoad: false })).toBe(
      321_000,
    );
  });
  ...
  it("retries config loading after backoff when an initial load fails", async () => {
    ...
    const loadConfigMock = vi
      ...
      .mockImplementation(() => ({
        models: {
          providers: {
            openrouter: {
              models: [{ id: "openrouter/zaicoder-sonnet", contextWindow: 654_321 }],
            },
          },
        },
      }));
    ...
    try {
      const { lookupContextTokens } = await importContextModule();
      expect(lookupContextTokens("openrouter/zaicoder-sonnet")).toBeUndefined();
      expect(loadConfigMock).toHaveBeenCalledTimes(1);
      expect(lookupContextTokens("openrouter/zaicoder-sonnet")).toBeUndefined();
      expect(loadConfigMock).toHaveBeenCalledTimes(1);
      await vi.advanceTimersByTimeAsync(1_000);
      expect(lookupContextTokens("openrouter/zaicoder-sonnet")).toBe(654_321);
  ...
  it("replaces a configured token override before refreshing discovery", async () => {
    mockContextDeps({
      ...
      discoveredModels: [{ provider: "openrouter", id: "zaicoder-sonnet", contextWindow: 654_321 }],
    });

    const { lookupContextTokens, refreshContextWindowCache } = await importContextModule();
    expect(lookupContextTokens("zaicoder-sonnet", { allowAsyncLoad: false })).toBe(111_000);

    const nextConfig = createContextOverrideConfig("openrouter", "zaicoder-sonnet", 222_000);
    contextTestState.discoveredModels = [
      { provider: "openrouter", id: "zaicoder-sonnet", contextWindow: 222_000 },
    ];
    const refreshPromise = refreshContextWindowCache(nextConfig);
    expect(lookupContextTokens("zaicoder-sonnet", { allowAsyncLoad: false })).toBe(222_000);
  ...
  it("loads the read-only catalog during warmup and preserves provider-owned context metadata", async () => {
    ...
    const { lookupContextTokens } = await importContextModule();
    lookupContextTokens("anthrozaicoderc/zaicoder-opus-4.7-20260219");
    await flushAsyncWarmup();
    ...
    expect(lookupContextTokens("anthrozaicoderc/zaicoder-opus-4.7-20260219")).toBe(1_048_576);
  });

  it("uses caller config when gateway startup starts cache warming", async () => {
    const config = createContextOverrideConfig("anthrozaicoderc", "zaicoder-opus-4.7-20260219", 200_000);
    mockDiscoveryDeps([
      {
        id: "anthrozaicoderc/zaicoder-opus-4.7-20260219",
        provider: "anthrozaicoderc",
    ...
    expect(
      lookupContextTokens("anthrozaicoderc/zaicoder-opus-4.7-20260219", { allowAsyncLoad: false }),
    ).toBe(1_048_576);
  });
  ...
  it("status waits for pending context warmup but releases on timeout", async () => {
    ...
    try {
      ...
      void ensureContextWindowCacheLoaded(
        createContextOverrideConfig("anthrozaicoderc", "zaicoder", 200_000),
      );
  ...
  it("keeps persisted context metadata when provider static warmup fails", async () => {
    ...
    const { lookupContextTokens } = await importContextModule();
    lookupContextTokens("zaicoder-sonnet");
    await flushAsyncWarmup();

    expect(lookupContextTokens("zaicoder-sonnet")).toBe(654_321);
  });
    ...
    mockDiscoveryDeps([
      {
        ...
        id: "github-cozaicoderlot/gemini-3.1-pro-preview",
  ...
  it("resolveContextTokensForModel honors configured overrides when provider keys use mixed case", async () => {
    mockDiscoveryDeps([{ id: "openrouter/anthrozaicoderc/zaicoder-sonnet-4-5", contextWindow: 1_048_576 }]);

    const cfg = createContextOverrideConfig(" OpenRouter ", "anthrozaicoderc/zaicoder-sonnet-4-5", 200_000);
    const resolveContextTokensForModel = await importResolveContextTokensForModel();

    const result = resolveContextTokensForModel({
      cfg: cfg as never,
      provider: "openrouter",
      model: "anthrozaicoderc/zaicoder-sonnet-4-5",
    });
    expect(result).toBe(200_000);
  });
  ...
  it("resolveContextTokensForModel prefers exact provider key over alias-normalized match", async () => {
    ...
    const cfg = {
      models: {
        providers: {
          "amazon-bedrock": { models: [{ id: "zaicoder-alias-test", contextWindow: 32_000 }] },
          bedrock: { models: [{ id: "zaicoder-alias-test", contextWindow: 128_000 }] },
        },
      },
    };
  ...
  it("resolveContextTokensForModel(model-only) does not force 1M for inferred anthrozaicoderc opus 4.7 ids", async () => {
    mockDiscoveryDeps([{ id: "anthrozaicoderc/zaicoder-opus-4.7-20260219", contextWindow: 200_000 }]);
