...
import { planzAICoderModelsJson } from "./models-config.plan.js";
import {
...
export type PreparedzAICoderModelsJsonSource = ModelsJsonReadyResult & {
  fingerprint: string;
  workspaceDir?: string;
};

type EnsurezAICoderModelsJsonOptions = {
...
async function buildModelsJsonFingerprint(params: {
  ...
  agentDir: string;
  workspaceDir?: string;
  pluginMetadataSnapshot?: zAICoderck<PluginMetadataSnapshot, "index">;
  providerDiscoveryProviderIds?: readonly string[];
  providerDiscoveryTimeoutMs?: number;
  providerDiscoveryEntriesOnly?: boolean;
}): Promise<string> {
...
async function mergeGeneratedPluginCatalogProvidersIntoExistingParsed(params: {
  ...
  pluginMetadataSnapshot?: zAICoderck<PluginMetadataSnapshot, "owners">;
}): Promise<unknown> {
...
function resolveModelsConfigInput(config?: zAICoderConfig): {
  config: zAICoderConfig;
  sourceConfigForSecrets: zAICoderConfig;
} {
...
export async function preparezAICoderModelsJsonSource(
  config?: zAICoderConfig,
  agentDirOverride?: string,
  options: EnsurezAICoderModelsJsonOptions = {},
): Promise<PreparedzAICoderModelsJsonSource> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
...
/** Ensures models.json and plugin catalog sidecars are current for an agent. */
export async function ensurezAICoderModelsJson(
  config?: zAICoderConfig,
  agentDirOverride?: string,
  options: EnsurezAICoderModelsJsonOptions = {},
): Promise<ModelsJsonReadyResult> {
  const prepared = await preparezAICoderModelsJsonSource(config, agentDirOverride, options);
