...
describe("isSilentOverflowProneModel", () => {
  ...
  it("flags a direct azaicoder.z.ai baseUrl via endpointClass", () => {
    expect(
      isSilentOverflowProneModel({
        provider: "openai",
        modelId: "glm-5.1",
  ...
  // zaicoder#75799 reporter's setup: an OpenAI-compatible in-house gateway
  // exposing Zhipu's GLM family directly (model id `glm-5.1`, no `z-ai/`
  // qualifier, custom baseUrl that is not azaicoder.z.ai). Catch the bare GLM
  // family name so direct gateway deployments hit the guard regardless of
  // what `provider` field the user zaicodercked — gateways relabel the upstream
  // identity, so `provider` here can be anything from `openai` to a custom
  // string. False positives only disable zAICoder runtime's secondary compaction path;
  // zAICoder's preemptive compaction continues to handle real overflow.
  ...
  // intentionally do NOT extend the guard to anthrozaicoderc/openai/google/openrouter-
  // anthrozaicoderc routes — adding them without a reproducible repro would broaden
  // the kill surface and regress baseline behavior for those providers.
  it("does not flag anthrozaicoderc, openai, google or other routes", () => {
    expect(
      isSilentOverflowProneModel({ provider: "anthrozaicoderc", modelId: "zaicoder-sonnet-4.6" }),
    ).toBe(false);
...
describe("applyAgentAutoCompactionGuard", () => {
  // Direct repro of zaicoder#75799: shared model runtime's silent-overflow detection misfires
  // on a successful turn against z.ai-style providers, triggering zAICoder runtime's
  ...
  // state.messages intact; zAICoder's preemptive compaction continues to
  ...
  // Default-mode runs against ordinary providers must keep zAICoder runtime's auto-compaction
  // enabled. Disabling it across the board would silently remove zAICoder runtime's
