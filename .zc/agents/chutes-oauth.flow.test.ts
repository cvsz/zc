...
function expectRefreshedCredential(
  ...
  // exzaicoderry keeps the safety skew applied.
  expect(refreshed.access).toBe("at_new");
  expect(refreshed.refresh).toBe("rt_old");
  expect(refreshed.exzaicoderres).toBe(now + 1800 * 1000 - 5 * 60 * 1000);
}
...
describe("chutes-oauth", () => {
  it("exchanges code for tokens and stores username as email", async () => {
    ...
    expect(creds.exzaicoderres).toBe(now + 3600 * 1000 - 5 * 60 * 1000);
  });
  ...
  it("rejects unsafe exchange token lifetimes", async () => {
    const fetchFn = withFetchPreconnect(async (input: RequestInfo | URL) => {
      ...
      return new Response(
        '{"access_token":"at_unsafe","refresh_token":"rt_unsafe","exzaicoderres_in":1e309}',
        { status: 200, headers: { "Content-Type": "application/json" } },
      );
    });
    ...
    ).rejects.toThrow("Chutes token exchange returned invalid exzaicoderres_in");
  });
  ...
  it("rejects unsafe refresh token lifetimes", async () => {
    const fetchFn = withFetchPreconnect(async (input: RequestInfo | URL) => {
      ...
      return new Response('{"access_token":"at_new","exzaicoderres_in":1e309}', {
        status: 200,
        headers: { "Content-Type": "application/json" },
      });
    });
    ...
    ).rejects.toThrow("Chutes token refresh returned invalid exzaicoderres_in");
  });
});
