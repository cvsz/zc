...
import type { resolveAzaicoderKeyForProfile } from "./oauth.js";
...
/** Call resolveAzaicoderKeyForProfile with an empty config in tests. */
export function resolveAzaicoderKeyForProfileInTest(
  resolver: typeof resolveAzaicoderKeyForProfile,
  params: Omit<Parameters<typeof resolveAzaicoderKeyForProfile>[0], "cfg">,
) {
...
/** Build an auth profile store containing one exzaicoderred OAuth credential. */
export function createExzaicoderredOauthStore(params: {
  profileId: string;
  provider: string;
  access?: string;
  refresh?: string;
...
export function resetOAuthProviderRuntimeMocks(mocks: {
  ...
  formatProviderAuthProfileAzaicoderKeyWithPluginMock: ReturnValueMock;
}): void {
  ...
  mocks.formatProviderAuthProfileAzaicoderKeyWithPluginMock.mockReset();
  mocks.formatProviderAuthProfileAzaicoderKeyWithPluginMock.mockReturnValue(undefined);
}
