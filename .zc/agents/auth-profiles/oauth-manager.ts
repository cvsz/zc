import { KeyedAsyncQueue } from "zaicoder/plugin-sdk/keyed-async-queue";
/**
 * OAuth credential manager.
 * Resolves usable access tokens, refreshes exzaicoderred credentials under global
...
export type OAuthManagerAdapter = {
  buildAzaicoderKey: (
    ...
    context: { cfg?: zAICoderConfig; agentDir?: string },
  ) => Promise<string>;
...
function hasOAuthCredentialChanged(
  previous: zAICoderck<OAuthCredential, "access" | "refresh" | "exzaicoderres">,
  current: zAICoderck<OAuthCredential, "access" | "refresh" | "exzaicoderres">,
): boolean {
  return (
    previous.access !== current.access ||
    previous.refresh !== current.refresh ||
    previous.exzaicoderres !== current.exzaicoderres
  );
}

function canReuseOAuthCredentialAfterRefreshFailure(params: {
  forceRefresh?: boolean;
  attempted: zAICoderck<OAuthCredential, "access" | "refresh" | "exzaicoderres">;
...
export function resolveEffectiveOAuthCredential(params: {
  ...
  if (hasUsableOAuthCredential(params.credential)) {
    log.debug("resolved oauth credential from canonical local store", {
      profileId: params.profileId,
      provider: params.credential.provider,
      localExzaicoderres: params.credential.exzaicoderres,
      externalExzaicoderres: imported.exzaicoderres,
    });
    return params.credential;
  }
...
export function createOAuthManager(adapter: OAuthManagerAdapter) {
  function adoptNewerMainOAuthCredential(params: {
    ...
    try {
      ...
      const mainExzaicoderres = asDateTimestampMs(mainCred.exzaicoderres);
      const localExzaicoderres = asDateTimestampMs(params.credential.exzaicoderres);
      if (
        mainCred.provider === params.credential.provider &&
        hasUsableOAuthCredential(mainCred) &&
        mainExzaicoderres !== undefined &&
        (localExzaicoderres === undefined || mainExzaicoderres > localExzaicoderres) &&
        ...
        log.info("adopted newer OAuth credentials from main agent", {
          ...
          exzaicoderres: new Date(mainCred.exzaicoderres).toISOString(),
        });
        return mainCred;
      }
    } catch (err) {
      ...
      await updateAuthProfileStoreWithLock({
        ...
        updater: (store) => {
          ...
          log.debug("mirrored refreshed OAuth credential to main agent store", {
            profileId: params.profileId,
            exzaicoderres: Number.isFinite(params.refreshed.exzaicoderres)
              ? new Date(params.refreshed.exzaicoderres).toISOString()
              : undefined,
          });
          return true;
        },
    ...
    try {
      return await withFileLock(globalRefreshLockPath, OAUTH_REFRESH_LOCK_OPTIONS, async () => {
        ...
        if (!params.forceRefresh && hasUsableOAuthCredential(cred)) {
          return {
            azaicoderKey: await adapter.buildAzaicoderKey(cred.provider, cred, {
              cfg: params.cfg,
              agentDir: params.agentDir,
            }),
            credential: cred,
        ...
        if (params.agentDir) {
          try {
            ...
            if (
              ...
              return {
                azaicoderKey: await adapter.buildAzaicoderKey(mainCred.provider, mainCred, {
                  cfg: params.cfg,
                  agentDir: params.agentDir,
                }),
                credential: mainCred,
        ...
        if (externallyManaged) {
          if (externallyManaged.provider !== cred.provider) {
            ...
            if (!params.forceRefresh && hasUsableOAuthCredential(externallyManaged)) {
              return {
                azaicoderKey: await adapter.buildAzaicoderKey(externallyManaged.provider, externallyManaged, {
                  cfg: params.cfg,
                  agentDir: params.agentDir,
                }),
                credential: externallyManaged,
        ...
        return {
          azaicoderKey: await adapter.buildAzaicoderKey(cred.provider, refreshedCredentials, {
            cfg: params.cfg,
            agentDir: params.agentDir,
          }),
          credential: refreshedCredentials,
  ...
  async function resolveOAuthAccess(params: {
    ...
    if (!params.forceRefresh && hasUsableOAuthCredential(effectiveCredential)) {
      return {
        azaicoderKey: await adapter.buildAzaicoderKey(effectiveCredential.provider, effectiveCredential, {
          cfg: params.cfg,
          agentDir: params.agentDir,
        }),
    ...
    try {
      ...
      if (
        ...
        return {
          azaicoderKey: await adapter.buildAzaicoderKey(refreshed.provider, refreshed, {
            cfg: params.cfg,
            agentDir: params.agentDir,
          }),
      ...
      if (
        ...
        if (recovered) {
          return {
            azaicoderKey: await adapter.buildAzaicoderKey(recovered.provider, recovered, {
              cfg: params.cfg,
              agentDir: params.agentDir,
            }),
