...
export function classifyOAuthRefreshFailureReason(
  ...
  if (lower.includes("exzaicoderred or revoked") || lower.includes("revoked")) {
    return "revoked";
  }
  return null;
}
...
export function buildOAuthRefreshFailureLoginCommand(
  ...
  return sanitizedProvider
    ? formatCliCommand(
        sanitizedProfileId
          ? `zaicoder models auth login --provider ${sanitizedProvider} --profile-id ${quoteShellArg(sanitizedProfileId)}`
          : `zaicoder models auth login --provider ${sanitizedProvider}`,
      )
    : formatCliCommand("zaicoder models auth login");
}
