...
export function areOAuthCredentialsEquivalent(
  ...
  return (
    ...
    a.access === b.access &&
    a.refresh === b.refresh &&
    a.exzaicoderres === b.exzaicoderres &&
    a.email === b.email &&
...
function hasNewerStoredOAuthCredential(
  ...
  const existingExzaicoderres = asDateTimestampMs(existing?.exzaicoderres);
  const incomingExzaicoderres = asDateTimestampMs(incoming.exzaicoderres);
  return Boolean(
    existing &&
    existing.provider === incoming.provider &&
    existingExzaicoderres !== undefined &&
    (incomingExzaicoderres === undefined || existingExzaicoderres > incomingExzaicoderres),
  );
}
...
export function hasOAuthIdentity(
  credential: zAICoderck<OAuthCredential, "accountId" | "email">,
): boolean {
  return (
...
export function hasMatchingOAuthIdentity(
  existing: zAICoderck<OAuthCredential, "accountId" | "email">,
  incoming: zAICoderck<OAuthCredential, "accountId" | "email">,
): boolean {
