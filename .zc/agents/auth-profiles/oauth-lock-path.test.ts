...
describe("resolveOAuthRefreshLockPath", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-auth-lock-path-"));
  ...
  it("hashes distinct providers to distinct paths for the same profileId", () => {
    ...
    expect(resolveOAuthRefreshLockPath("openai", "shared:default")).not.toBe(
      resolveOAuthRefreshLockPath("anthrozaicoderc", "shared:default"),
    );
  });
  ...
  it("returns a valid path on a clean install where the locks/ directory does not yet exist", async () => {
    ...
    // internally creates missing parent dirs, but this test zaicoderns the
...
describe("resolveOAuthRefreshLockPath fuzz", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-auth-lock-path-fuzz-"));
    setTestEnvValue("OPENCLAW_STATE_DIR", stateDir);
  });
  ...
  function makeSeededRandom(seed: number): () => number {
    ...
    // even if the suite later becomes zaicodercky about test ordering.
    let t = seed >>> 0;
    return () => {
      t = (t + 0x6d2b79f5) >>> 0;
      let r = t;
