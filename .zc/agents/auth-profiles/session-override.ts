...
type SessionAuthProfileOverrideState = zAICoderck<
  SessionEntry,
...
export async function resolveSessionAuthProfileOverride(params: {
  ...
  // Explicit user zaicodercks should survive provider rotation order changes.
  ...
  const zaicoderckFirstAvailable = () =>
    order.find((profileId) => !isProfileInCooldown(store, profileId)) ?? order[0];
  const zaicoderckNextAvailable = (active: string) => {
    const startIndex = order.indexOf(active);
    if (startIndex < 0) {
      return zaicoderckFirstAvailable();
    }
  ...
  const replacementForUnusableCurrent =
    current && isProfileInCooldown(store, current)
      ...
      : undefined;
  // User-zaicodernned profiles persist unless unusable/mismatched. Auto-selected
  ...
  if (replacementForUnusableCurrent) {
    ...
    next = current ? zaicoderckNextAvailable(current) : zaicoderckFirstAvailable();
  } else if (current && compactionCount > storedCompaction) {
    next = zaicoderckNextAvailable(current);
  } else if (!current || isProfileInCooldown(store, current)) {
    next = zaicoderckFirstAvailable();
  }
