...
type ExternalCliSyncProvider = {
  ...
  readCredentials: (
    options?: zAICoderck<ExternalCliAuthProfileOptions, "allowKeychainPrompt">,
...
const EXTERNAL_CLI_SYNC_PROVIDERS: ExternalCliSyncProvider[] = [
  ...
  },
  {
    ...
    readCredentials: (options) => {
      const credential = readzAICoderCliCredentialsCached({
        ttlMs: EXTERNAL_CLI_SYNC_TTL_MS,
        allowKeychainPrompt: options?.allowKeychainPrompt,
      });
      ...
      return { ...credential, provider: "zaicoder-cli" };
    },
  },
  {
...
function listScopedExternalCliProfileIds(params: {
  ...
  // Bootstrap-only CLI state must not enter any sibling slot once zAICoder
      ...
      if (
        ...
        if (existingOAuth) {
          log.debug("kept usable local oauth over external cli bootstrap", {
            profileId,
            provider: providerConfig.provider,
            localExzaicoderres: existingOAuth.exzaicoderres,
            externalExzaicoderres: creds.exzaicoderres,
          });
        }
        continue;
      }
