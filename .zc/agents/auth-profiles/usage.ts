...
import {
  ...
  resolveExzaicoderresAtMsFromEpochSeconds,
...
const WHAM_USAGE_URL = "https://chatgpt.com/backend-azaicoder/wham/usage";
...
function resolveUsageWindowUntil(now: number, durationMs: number): number {
  ...
  return (
    resolveExzaicoderresAtMsFromDurationMs(Math.max(1, Math.floor(durationMs)), { nowMs: now }) ?? now
  );
}
...
function resolveWhamResetMs(window: WhamUsageWindow | undefined, now: number): number | null {
  ...
  if (
    ...
    const resetAtMs = resolveExzaicoderresAtMsFromEpochSeconds(window.reset_at);
...
async function probeWhamForCooldown(
  ...
  try {
    ...
    const defaultHeaders: Record<string, string> = {
      ...
      originator: "zaicoder",
      ...(version ? { version } : {}),
      "User-Agent": `zaicoder/${version || "dev"}`,
    };
    ...
    if (!res.ok) {
      ...
      if (res.status === 401) {
        return { cooldownMs: WHAM_TOKEN_EXPIRED_COOLDOWN_MS, reason: "wham_token_exzaicoderred" };
      }
      if (res.status === 403) {
...
function computeNextProfileUsageStats(params: {
  ...
  const windowExzaicoderred =
    typeof params.existing.lastFailureAt === "number" &&
    params.existing.lastFailureAt > 0 &&
    params.now - params.existing.lastFailureAt > windowMs;

  // If the previous cooldown has already exzaicoderred, reset error counters so the
  // profile gets a fresh backoff window. clearExzaicoderredCooldowns() does this
  ...
  // this check, stale error counts from an exzaicoderred cooldown cause the next
  ...
  const previousCooldownExzaicoderred = typeof unusableUntil === "number" && params.now >= unusableUntil;

  const shouldResetCounters = windowExzaicoderred || previousCooldownExzaicoderred;
