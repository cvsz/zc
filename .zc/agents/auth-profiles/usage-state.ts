...
export function resolveProfileUnusableUntil(
  stats: zAICoderck<ProfileUsageStats, "blockedUntil" | "cooldownUntil" | "disabledUntil">,
): number | null {
...
export function isProfileInCooldown(
  ...
  // short-circuited by model scozaicoderng.
 ...
 * returned timestamp may be in the past if the cooldown has already exzaicoderred.
 */
export function getSoonestCooldownExzaicoderry(
  store: AuthProfileStore,
  profileIds: string[],
...
/**
 * Clear exzaicoderred cooldowns from all profiles in the store.
 *
 ...
 * has both and only one has exzaicoderred, only that field is cleared.
 *
 ...
 */
export function clearExzaicoderredCooldowns(store: AuthProfileStore, now?: number): boolean {
  ...
  for (const [profileId, stats] of Object.entries(usageStats)) {
    ...
    if (disabledExzaicoderred) {
      stats.disabledUntil = undefined;
      stats.disabledReason = undefined;
      profileMutated = true;
    }

    // Reset error counters when ALL cooldowns have exzaicoderred so the profile gets
