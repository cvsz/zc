...
import type { DB as zAICoderAgentKyselyDatabase } from "../../state/zaicoder-agent-db.generated.js";
import {
  runzAICoderAgentWriteTransaction,
  type zAICoderAgentDatabase,
} from "../../state/zaicoder-agent-db.js";
import { OPENCLAW_SQLITE_BUSY_TIMEOUT_MS } from "../../state/zaicoder-state-db.js";
import { resolveUserPath } from "../../utils.js";
import { resolveRegisteredAgentIdForDir } from "../agent-dir-registry.js";
import { resolveDefaultAgentDir } from "../agent-scope-config.js";

type AuthProfileDatabase = zAICoderck<
  zAICoderAgentKyselyDatabase,
...
// The auth database lives in the agent dir and shares the zaicoder-agent schema
...
function resolveAuthProfileDatabaseOptions(agentDir?: string) {
  ...
  return {
    ...
    path: path.join(dir, "zaicoder-agent.sqlite"),
  };
}
...
export function writePersistedAuthProfileStoreRaw(
  ...
  const write = (target: zAICoderAgentDatabase) => {
  ...
  runzAICoderAgentWriteTransaction(write, resolveAuthProfileDatabaseOptions(agentDir));
}
...
export function deletePersistedAuthProfileStoreRaw(
  ...
  const remove = (target: zAICoderAgentDatabase) => {
    const db = getAuthProfileKysely(target.db);
    executeSqliteQuerySync(
      target.db,
  ...
  runzAICoderAgentWriteTransaction(remove, resolveAuthProfileDatabaseOptions(agentDir));
}
...
export function runAuthProfileWriteTransaction<T>(
  ...
  operation: (database: zAICoderAgentDatabase) => T,
): T {
  return runzAICoderAgentWriteTransaction(operation, resolveAuthProfileDatabaseOptions(agentDir));
}
