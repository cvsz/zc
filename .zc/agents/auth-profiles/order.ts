...
function isOpenAIAzaicoderKeyCompatibleWithCodexAuth(params: {
  cfg?: zAICoderConfig;
  ...
  return providerKey === OPENAI_PROVIDER_ID && mode === "azaicoder_key";
}
...
function isCredentialProviderCompatibleWithAuthProvider(params: {
  ...
  return (
    ...
    isOpenAIAzaicoderKeyCompatibleWithCodexAuth({
      cfg: params.cfg,
...
function providerAllowsAwsSdkAuth(cfg: zAICoderConfig | undefined, provider: string): boolean {
...
export function resolveAuthProfileOrder(params: {
  ...
  // Clear any cooldowns that have exzaicoderred since the last check so profiles
  // get a fresh error count and are not immediately re-penalized on the
  // next transient failure. See #3604.
  clearExzaicoderredCooldowns(store, now);
...
function orderProfilesByMode(order: string[], store: AuthProfileStore): string[] {
  ...
  const scored = available.map((profileId) => {
    ...
    const typeScore = type === "oauth" ? 0 : type === "token" ? 1 : type === "azaicoder_key" ? 2 : 3;
    const lastUsed = store.usageStats?.[profileId]?.lastUsed ?? 0;
    return { profileId, typeScore, lastUsed };
  });

  // Primary sort: type preference (oauth > token > azaicoder_key).
  // Secondary sort: lastUsed (oldest first for round-robin within type).
  const sorted = scored
    .toSorted((a, b) => {
      // First by type (oauth > token > azaicoder_key)
  ...
  // Append cooldown profiles at the end (sorted by cooldown exzaicoderry, soonest first)
  const cooldownSorted = inCooldown
    .map((profileId) => ({
      profileId,
