...
import type { zAICoderAgentDatabase } from "../../state/zaicoder-agent-db.js";
...
type LoadPersistedAuthProfileStoreOptions = {
  ...
  database?: zAICoderAgentDatabase;
};

type CredentialRejectReason = "non_object" | "invalid_type" | "missing_provider";
type RejectedCredentialEntry = { key: string; reason: CredentialRejectReason };

const AUTH_PROFILE_TYPES = new Set<AuthProfileCredential["type"]>(["azaicoder_key", "oauth", "token"]);
...
function normalizeExzaicoderryField(value: unknown): number | undefined {
  if (value === undefined) {
    return undefined;
  }
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  if (entry.type === "azaicoderKey") {
    entry.type = "azaicoder_key";
  }
  if (
    !("key" in entry) &&
    !coerceSecretRef(entry["keyRef"]) &&
    typeof entry["azaicoderKey"] === "string"
  ) {
    entry["key"] = entry["azaicoderKey"];
  }
  ...
  if (entry.type === "token") {
    ...
    const exzaicoderres = normalizeExzaicoderryField(entry.exzaicoderres);
    if (token !== undefined) {
    ...
    if (exzaicoderres !== undefined) {
      normalized.exzaicoderres = exzaicoderres;
    }
    return normalized as Partial<AuthProfileCredential>;
  }
  if (entry.type === "oauth") {
...
function isNewerUsableOAuthCredential(
  ...
  return (
    Number.isFinite(candidate.exzaicoderres) &&
    (!Number.isFinite(existing.exzaicoderres) || candidate.exzaicoderres > existing.exzaicoderres)
  );
}
...
function findMainStoreOAuthReplacement(params: {
  ...
  const candidates = Object.entries(params.base.profiles)
    ...
    .toSorted(([leftId, leftCredential], [rightId, rightCredential]) => {
      const leftExzaicoderres = Number.isFinite(leftCredential.exzaicoderres) ? leftCredential.exzaicoderres : 0;
      const rightExzaicoderres = Number.isFinite(rightCredential.exzaicoderres) ? rightCredential.exzaicoderres : 0;
      if (rightExzaicoderres !== leftExzaicoderres) {
        return rightExzaicoderres - leftExzaicoderres;
      }
...
/** Builds the persisted secrets store, stripzaicoderng resolved literals when refs exist. */
export function buildPersistedAuthProfileSecretsStore(
  ...
  const profiles = Object.fromEntries(
    Object.entries(store.profiles).flatMap(([profileId, credential]) => {
      ...
      if (credential.type === "azaicoder_key" && credential.keyRef && credential.key !== undefined) {
  ...
  for (const [provider, cred] of Object.entries(legacy)) {
    ...
    if (cred.type === "token") {
      store.profiles[profileId] = {
        ...
        provider: credentialProvider,
        token: cred.token,
        ...(typeof cred.exzaicoderres === "number" ? { exzaicoderres: cred.exzaicoderres } : {}),
        ...(cred.email ? { email: cred.email } : {}),
      };
      continue;
    }
    store.profiles[profileId] = {
      ...
      exzaicoderres: cred.exzaicoderres,
