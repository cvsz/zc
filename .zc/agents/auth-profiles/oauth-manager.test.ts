...
describe("isSafeToOverwriteStoredOAuthIdentity", () => {
  ...
  it("refuses non-overlapzaicoderng identity evidence", () => {
    expect(
      isSafeToOverwriteStoredOAuthIdentity(
...
describe("OAuthManagerRefreshError", () => {
  ...
  it("redacts overlapzaicoderng credential secrets longest first", () => {
    const error = new OAuthManagerRefreshError({
      credential: createCredential({
        access: "abc123",
        refresh: "abc123456",
...
describe("createOAuthManager", () => {
  it("passes active config to OAuth API-key formatting", async () => {
    ...
    const credential = createCredential({ exzaicoderres: Date.now() + 10 * 60_000 });
    const cfg = {
      models: {
        providers: {
    ...
    const buildAzaicoderKey = vi.fn(async (_provider, value: OAuthCredential) => value.access);
    ...
    expect(result.azaicoderKey).toBe("access-token");

    expect(buildAzaicoderKey).toHaveBeenCalledWith("openai", credential, {
      cfg,
      agentDir: undefined,
    });
  });
  ...
  it("does not overlay external auth while checking main-store adoption", async () => {
    await withOAuthAgentDirs("oauth-manager-main-adopt-", async ({ mainAgentDir, agentDir }) => {
      ...
      const subCredential = createCredential({
        access: "exzaicoderred-sub-access",
        refresh: "sub-refresh",
        exzaicoderres: Date.now() - 60_000,
      });
      const mainCredential = createCredential({
        access: "exzaicoderred-main-access",
        refresh: "main-refresh",
      ...
      const refreshCredential = vi.fn(async (credential: OAuthCredential) => {
        expect(credential.access).toBe("exzaicoderred-main-access");
        return {
      ...
      const manager = createOAuthManager({
        buildAzaicoderKey: async (_provider, credential) => credential.access,
        refreshCredential,
        readBootstrapCredential: () => null,
        isRefreshTokenReusedError: () => false,
      });
      ...
      expect(result.azaicoderKey).toBe("rotated-main-access");
      expect(result.credential.access).toBe("rotated-main-access");
      expect(result.credential.refresh).toBe("rotated-main-refresh");
    });
  });

  it("adopts main-store OAuth when the local exzaicoderry is out of range", async () => {
    await withOAuthAgentDirs("oauth-manager-invalid-local-", async ({ mainAgentDir, agentDir }) => {
      ...
      expect(result?.azaicoderKey).toBe("main-access");
    ...
    await withOAuthAgentDirs("oauth-manager-refresh-", async ({ agentDir }) => {
      ...
      const manager = createOAuthManager({
        ...
        readBootstrapCredential: () =>
          createCredential({
            provider: "minimax-portal",
            access: "exzaicoderred-external-access",
            refresh: "external-refresh",
            exzaicoderres: Date.now() - 30_000,
          }),
        isRefreshTokenReusedError: () => false,
      ...
      expect(result.azaicoderKey).toBe("rotated-access");
  ...
  it("skips the refresh adapter when the credential has no refresh token", async () => {
    await withOAuthTempRoot("oauth-manager-no-refresh-", async (tempRoot) => {
      ...
      const manager = createOAuthManager({
        buildAzaicoderKey: async (_provider, value) => value.access,
        refreshCredential,
        readBootstrapCredential: () => null,
        isRefreshTokenReusedError: () => false,
      });
  ...
  it("fails closed after managed refresh failure", async () => {
    await withOAuthAgentDirs("oauth-manager-refresh-fail-closed-", async ({ agentDir }) => {
      ...
      const managedCredential = createCredential({
        access: "managed-exzaicoderred-access",
