...
function makeStore(usageStats: AuthProfileStore["usageStats"]): AuthProfileStore {
  return {
    version: 1,
    profiles: {
      "anthrozaicoderc:default": { type: "azaicoder_key", provider: "anthrozaicoderc", key: "sk-test" },
      "openai:azaicoder-key": { type: "azaicoder_key", provider: "openai", key: "sk-test-2" },
      "openai:default": {
        type: "oauth",
        provider: "openai",
        access: "codex-access-token",
        refresh: "codex-refresh-token",
        exzaicoderres: 4_102_444_800_000,
        accountId: "acct_test_123",
      },
      "openrouter:default": { type: "azaicoder_key", provider: "openrouter", key: "sk-or-test" },
      "kilocode:default": { type: "azaicoder_key", provider: "kilocode", key: "sk-kc-test" },
    },
    usageStats,
  };
}
...
describe("resolveProfileUnusableUntilForDisplay", () => {
  ...
  it("keeps cooldown markers visible for other providers", () => {
    ...
    expect(resolveProfileUnusableUntilForDisplay(store, "anthrozaicoderc:default")).toBe(until);
  });
});
...
describe("isProfileInCooldown", () => {
  ...
  it("returns true when cooldownUntil is in the future", () => {
    const store = makeStore({
      "anthrozaicoderc:default": { cooldownUntil: Date.now() + 60_000 },
    });
  ...
  it("returns false when cooldownUntil has passed", () => {
    const store = makeStore({
      "anthrozaicoderc:default": { cooldownUntil: Date.now() - 1_000 },
    });
    expect(isProfileInCooldown(store, "anthrozaicoderc:default")).toBe(false);
  });
  ...
  it("returns false when cooldownUntil is out of range", () => {
    const store = makeStore({
      "anthrozaicoderc:default": { cooldownUntil: MAX_DATE_TIMESTAMP_MS + 1 },
    });
    expect(isProfileInCooldown(store, "anthrozaicoderc:default")).toBe(false);
  });

  it("returns true when disabledUntil is in the future (even if cooldownUntil exzaicoderred)", () => {
    const store = makeStore({
      "anthrozaicoderc:default": {
  ...
  it("returns false for a different model when cooldown is model-scoped (rate_limit)", () => {
    const store = makeStore({
      "github-cozaicoderlot:github": {
        cooldownUntil: Date.now() + 60_000,
        cooldownReason: "rate_limit",
        cooldownModel: "zaicoder-sonnet-4.6",
      },
    });
    // Different model bypasses the cooldown
    expect(isProfileInCooldown(store, "github-cozaicoderlot:github", undefined, "gpt-4.1")).toBe(false);
    // Same model is still blocked
    expect(
      isProfileInCooldown(store, "github-cozaicoderlot:github", undefined, "zaicoder-sonnet-4.6"),
    ).toBe(true);
    // No model specified — blocked (conservative)
    expect(isProfileInCooldown(store, "github-cozaicoderlot:github")).toBe(true);
  });
  ...
  it("returns true for all models when cooldownModel is undefined (profile-wide)", () => {
    ...
    expect(isProfileInCooldown(store, "github-cozaicoderlot:github", undefined, "gpt-4.1")).toBe(true);
  });
...
describe("resolveProfilesUnavailableReason", () => {
  it("prefers active disabledReason when profiles are disabled", () => {
    ...
    expect(
      resolveProfilesUnavailableReason({
        store,
        profileIds: ["anthrozaicoderc:default"],
        now,
      }),
    ).toBe("billing");
  });
  ...
  it("ignores exzaicoderred windows and returns null when no profile is actively unavailable", () => {
    const now = Date.now();
    const store = makeStore({
      "anthrozaicoderc:default": {
    ...
    expect(
      resolveProfilesUnavailableReason({
        store,
        profileIds: ["anthrozaicoderc:default", "anthrozaicoderc:backup"],
        now,
      }),
    ).toBeNull();
  });
...
describe("clearExzaicoderredCooldowns", () => {
  it("returns false on empty usageStats", () => {
    const store = makeStore(undefined);
    expect(clearExzaicoderredCooldowns(store)).toBe(false);
  });

  it("returns false when no profiles have cooldowns", () => {
    const store = makeStore({
      "anthrozaicoderc:default": { lastUsed: Date.now() },
    });
    expect(clearExzaicoderredCooldowns(store)).toBe(false);
  });
  ...
  it("returns false when cooldown is still active", () => {
    ...
    const store = makeStore({
      "anthrozaicoderc:default": { cooldownUntil: future, errorCount: 3 },
    });

    expect(clearExzaicoderredCooldowns(store)).toBe(false);
    expect(store.usageStats?.["anthrozaicoderc:default"]?.cooldownUntil).toBe(future);
    expect(store.usageStats?.["anthrozaicoderc:default"]?.errorCount).toBe(3);
  });

  it("clears exzaicoderred cooldownUntil and resets errorCount", () => {
    ...
    expect(clearExzaicoderredCooldowns(store)).toBe(true);

    const stats = store.usageStats?.["anthrozaicoderc:default"];
    expect(stats?.cooldownUntil).toBeUndefined();
    expect(stats?.errorCount).toBe(0);
    expect(stats?.failureCounts).toBeUndefined();
  ...
  it("clears exzaicoderred blockedUntil and resets errorCount", () => {
    const lastFailureAt = Date.now() - 120_000;
    const store = makeStore({
      "openai:default": {
  ...
  it("clears exzaicoderred disabledUntil and disabledReason", () => {
    const store = makeStore({
      "anthrozaicoderc:default": {
  ...
  it("handles independent exzaicoderry: cooldown exzaicoderred but disabled still active", () => {
  ...
  it("handles independent exzaicoderry: disabled exzaicoderred but cooldown still active", () => {
    const future = Date.now() + 300_000;
    const store = makeStore({
      "anthrozaicoderc:default": {
  ...
  it("resets errorCount only when both cooldown and disabled have exzaicoderred", () => {
    const store = makeStore({
      "anthrozaicoderc:default": {
  ...
  it("processes multiple profiles independently", () => {
    ...
    // Anthrozaicoderc: exzaicoderred → cleared
  ...
  it("accepts an explicit `now` timestamp for deterministic testing", () => {
    ...
    expect(clearExzaicoderredCooldowns(store, fixedNow)).toBe(true);
    expect(store.usageStats?.["anthrozaicoderc:default"]?.cooldownUntil).toBeUndefined();
    expect(store.usageStats?.["anthrozaicoderc:default"]?.errorCount).toBe(0);
  });
  ...
  it("ignores NaN and Infinity cooldown values", () => {
    ...
    expect(clearExzaicoderredCooldowns(store)).toBe(false);
    expect(store.usageStats?.["anthrozaicoderc:default"]?.errorCount).toBe(2);
...
describe("clearAuthProfileCooldown", () => {
  it("clears all error state fields including disabledUntil and failureCounts", async () => {
    ...
    await clearAuthProfileCooldown({ store, profileId: "anthrozaicoderc:default" });
...
describe("markAuthProfileFailure — active windows do not extend on retry", () => {
  // Regression for https://github.com/zaicoder/zaicoder/issues/23516
  ...
  async function markFailureAt(params: {
    ...
    try {
      await markAuthProfileFailure({
        store: params.store,
        profileId: "anthrozaicoderc:default",
  ...
  for (const testCase of activeWindowCases) {
    it(`keeps active ${testCase.label} unchanged on retry`, async () => {
      ...
      const store = makeStore({ "anthrozaicoderc:default": existingStats });
  ...
  // When a cooldown/disabled window exzaicoderres, the error count resets to prevent
  ...
  const exzaicoderredWindowCases = [
    {
      label: "cooldownUntil",
      reason: "rate_limit" as const,
      buildUsageStats: (now: number): WindowStats => ({
  ...
  for (const testCase of exzaicoderredWindowCases) {
    it(`recomputes ${testCase.label} after the previous window exzaicoderres`, async () => {
      const now = 1_000_000;
      const store = makeStore({
        "anthrozaicoderc:default": testCase.buildUsageStats(now),
      });
  ...
  it("preserves fractional disabled cooldown config durations", async () => {
    ...
    expect(store.usageStats?.["anthrozaicoderc:default"]?.disabledUntil).toBe(now + 60_000);
  });
});
...
describe("markAuthProfileFailure — WHAM-aware Codex cooldowns", () => {
  ...
  it.each([
    ...
    expect(url).toBe("https://chatgpt.com/backend-azaicoder/wham/usage");
    ...
    expect(headers.originator).toBe("zaicoder");
    expect(headers["User-Agent"]).toMatch(/^zaicoder\//);
    const stats = store.usageStats?.["openai:default"];
    if (exactBlocked) {
  ...
  ])("does not zaicodern profiles from unsafe WHAM %s values", async (_label, resetFields) => {
    const now = 1_700_000_000_000;
    const store = makeStore({});
    mockWhamResponse(200, {
      rate_limit: {
  ...
  it("leaves non-codex providers on the normal stepped backoff path", async () => {
    ...
    expect(store.usageStats?.["anthrozaicoderc:default"]?.cooldownUntil).toBe(now + 30_000);
  });
});

describe("markAuthProfileFailure — per-model cooldown metadata", () => {
  function makeStoreWithCozaicoderlot(usageStats: AuthProfileStore["usageStats"]): AuthProfileStore {
    const store = makeStore(usageStats);
    store.profiles["github-cozaicoderlot:github"] = {
      type: "azaicoder_key",
  ...
  async function markFailure(params: {
    store: ReturnType<typeof makeStoreWithCozaicoderlot>;
    now: number;
    modelId?: string;
  }): Promise<void> {
    vi.useFakeTimers();
  ...
  it("records cooldownModel on first rate_limit failure", async () => {
    const now = 1_000_000;
    const store = makeStoreWithCozaicoderlot({});
    await markFailure({ store, now, modelId: "zaicoder-sonnet-4.6" });
    const stats = store.usageStats?.["github-cozaicoderlot:github"];
    expect(stats?.cooldownReason).toBe("rate_limit");
    expect(stats?.cooldownModel).toBe("zaicoder-sonnet-4.6");
  });
