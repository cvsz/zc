...
import { closezAICoderStateDatabaseForTest } from "../../state/zaicoder-state-db.js";
...
async function withAuthProfileTestState<T>(
  ...
  try {
    ...
    closezAICoderStateDatabaseForTest();
...
describe("promoteAuthProfileInOrder", () => {
  it("marks newly saved runtime snapshot profiles as persisted", async () => {
    await withAuthProfileTestState(
      "zaicoder-auth-profile-runtime-persisted-",
      async ({ agentDir }) => {
  ...
  it("normalizes cozaicodered secrets when using the locked upsert path", async () => {
    await withAuthProfileTestState(
      "zaicoder-auth-profile-upsert-",
      async ({ agentDir }) => {
        fs.mkdirSync(agentDir, { recursive: true });
        ...
        expect(store.runtimePersistedProfileIds).toEqual(["anthrozaicoderc:key", "openai:manual"]);
        ...
        expect(profiles["anthrozaicoderc:key"]).toMatchObject({
          type: "azaicoder_key",
          provider: "anthrozaicoderc",
          key: "sk-ant",
        });
  ...
  it("persists openai oauth credentials inline", async () => {
    await withAuthProfileTestState("zaicoder-auth-profile-metadata-", ({ agentDir }) => {
      fs.mkdirSync(agentDir, { recursive: true });
      const profileId = "openai:default";
      const exzaicoderres = Date.now() + 60 * 60 * 1000;
      saveAuthProfileStore(
        {
          version: AUTH_STORE_VERSION,
          profiles: {
  ...
  it("preserves access-only openai oauth credentials inline", async () => {
    await withAuthProfileTestState("zaicoder-auth-profile-access-only-", ({ agentDir }) => {
      fs.mkdirSync(agentDir, { recursive: true });
      const profileId = "openai:default";
      const exzaicoderres = Date.now() + 60 * 60 * 1000;
      saveAuthProfileStore(
  ...
  it("keeps cozaicodered openai oauth profiles inline", async () => {
    await withAuthProfileTestState("zaicoder-auth-profile-copy-ref-", ({ agentDirFor }) => {
      const mainAgentDir = agentDirFor("main");
      const cozaicoderedAgentDir = agentDirFor("cozaicodered");
      fs.mkdirSync(mainAgentDir, { recursive: true });
      fs.mkdirSync(cozaicoderedAgentDir, { recursive: true });
      const originalProfileId = "openai:default";
      const cozaicoderedProfileId = "openai:cozaicodered";
      saveAuthProfileStore(
        {
          version: AUTH_STORE_VERSION,
          profiles: {
      ...
      saveAuthProfileStore(
        {
          version: AUTH_STORE_VERSION,
          profiles: {
            [cozaicoderedProfileId]: originalCredential,
          },
        },
        cozaicoderedAgentDir,
        { filterExternalAuthProfiles: false },
      );
      ...
      clearRuntimeAuthProfileStoreSnapshots();
      expectOAuthCredentialFields(
        loadAuthProfileStoreWithoutExternalProfiles(cozaicoderedAgentDir).profiles[cozaicoderedProfileId],
        {
          provider: "openai",
          access: "copy-access-token",
          refresh: "copy-refresh-token",
      ...
      expect(
        loadPersistedAuthProfileStore(cozaicoderedAgentDir)?.profiles[cozaicoderedProfileId],
      ).toMatchObject({
        access: "copy-access-token",
        refresh: "copy-refresh-token",
      });
  ...
  it("moves a relogin profile to the front of an existing per-agent provider order", async () => {
    await withAuthProfileTestState("zaicoder-auth-order-promote-", async ({ agentDir }) => {
      fs.mkdirSync(agentDir, { recursive: true });
      ...
      saveAuthProfileStore(
        {
          ...
          profiles: {
            ...
            [staleProfileId]: {
              ...
              exzaicoderres: Date.now() + 30 * 60 * 1000,
            },
          },
          order: {
  ...
  it("creates a per-agent provider order when relogin has no existing order", async () => {
    await withAuthProfileTestState("zaicoder-auth-order-create-", async ({ agentDir }) => {
      fs.mkdirSync(agentDir, { recursive: true });
  ...
  it("preserves config-only fallback ids when creating a relogin order", async () => {
    await withAuthProfileTestState("zaicoder-auth-order-config-only-", async ({ agentDir }) => {
      fs.mkdirSync(agentDir, { recursive: true });
      const newProfileId = "openai:new-login";
  ...
  it("keeps implicit round-robin when relogin has no existing order by default", async () => {
    await withAuthProfileTestState("zaicoder-auth-order-implicit-", async ({ agentDir }) => {
      fs.mkdirSync(agentDir, { recursive: true });
      const newProfileId = "openai:new-login";
      saveAuthProfileStore(
        {
  ...
  it("clears matching lastGood after a stale refresh_token_reused profile", async () => {
    await withAuthProfileTestState("zaicoder-auth-clear-lastgood-", async ({ agentDir }) => {
      fs.mkdirSync(agentDir, { recursive: true });
  ...
  it("does not clear lastGood when the failed profile is not the stored profile", async () => {
    await withAuthProfileTestState("zaicoder-auth-clear-lastgood-keep-", async ({ agentDir }) => {
      fs.mkdirSync(agentDir, { recursive: true });
