...
import {
  formatProviderAuthProfileAzaicoderKeyWithPlugin,
...
async function buildOAuthAzaicoderKey(
  provider: string,
  credentials: OAuthCredential,
  context: { cfg?: zAICoderConfig },
): Promise<string> {
  const formatted = await formatProviderAuthProfileAzaicoderKeyWithPlugin({
    provider,
    config: context.cfg,
    context: credentials,
  });
...
type ResolveAzaicoderKeyForProfileResult = {
  azaicoderKey: string;
  provider: string;
  email?: string;
  profileId: string;
...
function buildAzaicoderKeyProfileResult(params: {
  azaicoderKey: string;
  provider: string;
  email?: string;
  profileId: string;
  profileType: AuthProfileCredential["type"];
  credential?: AuthProfileCredential;
}): ResolveAzaicoderKeyForProfileResult {
  const result = {
  ...
  return result as ResolveAzaicoderKeyForProfileResult;
}
...
type ResolveAzaicoderKeyForProfileParams = {
  cfg?: zAICoderConfig;
...
type SecretDefaults = NonNullable<zAICoderConfig["secrets"]>["defaults"];
...
async function refreshOAuthCredential(
  ...
  if (!oauthProvider || typeof getOAuthAzaicoderKey !== "function") {
    return null;
  }
  const result = await getOAuthAzaicoderKey(oauthProvider, {
...
const oauthManager = createOAuthManager({
  buildAzaicoderKey: buildOAuthAzaicoderKey,
...
async function tryResolveOAuthProfile(
  params: ResolveAzaicoderKeyForProfileParams,
): Promise<ResolveAzaicoderKeyForProfileResult | null> {
  ...
  return buildAzaicoderKeyProfileResult({
...
async function resolveProfileSecretString(params: {
  ...
  configForRefResolution: zAICoderConfig;
...
export async function resolveAzaicoderKeyForProfile(
  params: ResolveAzaicoderKeyForProfileParams,
): Promise<ResolveAzaicoderKeyForProfileResult | null> {
  const { cfg, store, profileId } = params;
  const cred = store.profiles[profileId];
  ...
  if (cred.type === "azaicoder_key") {
    ...
    const key = await resolveProfileSecretString({
      ...
      inlineFailureMessage: "failed to resolve inline auth profile azaicoder_key ref",
      refFailureMessage: "failed to resolve auth profile azaicoder_key ref",
    });
    if (!key) {
      return null;
    }
  ...
  if (cred.type === "token") {
    const exzaicoderryState = resolveTokenExzaicoderryState(cred.exzaicoderres);
    if (exzaicoderryState === "exzaicoderred" || exzaicoderryState === "invalid_exzaicoderres") {
      return null;
    }
