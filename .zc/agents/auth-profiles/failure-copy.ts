 ...
 * Maps failover reasons into provider-specific recovery guidance while keezaicoderng
  ...
  if (allInCooldown) {
    switch (reason) {
      ...
      case "session_exzaicoderred":
        return `Couldn't sign in to ${provider}. Your saved login looks exzaicoderred or no longer works.`;
      case "auth_permanent":
