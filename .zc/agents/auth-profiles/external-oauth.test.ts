...
function createUsableOAuthExzaicoderry(): number {
  // Keep fixtures comfortably outside the shared near-exzaicoderry refresh margin.
...
describe("auth external oauth helpers", () => {
  ...
  it("keeps explicit local non-oauth auth over external cli oauth overlays", () => {
    ...
    const profile = requireProfile(overlaid, "openai:default");
    expect(profile.type).toBe("azaicoder_key");
    expect(profile.provider).toBe("openai");
    expect(profile.key).toBe("sk-local");
  });

  it("keeps exzaicoderred local oauth when external cli belongs to a different account", () => {
    readCodexCliCredentialsCachedMock.mockReturnValue(
      createCredential({
        access: "fresh-cli-access-token",
        refresh: "fresh-cli-refresh-token",
    ...
    const overlaid = overlayExternalAuthProfiles(
      createStore({
        "openai:default": createCredential({
          ...
          refresh: "exzaicoderred-local-refresh-token",
          exzaicoderres: Date.now() - 60_000,
    ...
    const profile = requireProfile(overlaid, "openai:default");
    expect(profile.access).toBe("exzaicoderred-local-access-token");
    expect(profile.refresh).toBe("exzaicoderred-local-refresh-token");
