...
/** Static token credential that zAICoder does not refresh. */
export type TokenCredential = {
  /**
   * Static bearer-style token (often OAuth access token / PAT).
   * Not refreshable by zAICoder (unlike `type: "oauth"`).
   */
  type: "token";
  provider: string;
  token?: string;
  ...
  /** Explicit opt-out for copying this profile when creating another agent. */
  copyToAgents?: boolean;
  /** Optional exzaicoderry timestamp (ms since epoch). */
  exzaicoderres?: number;
  email?: string;
  displayName?: string;
};
...
export type AuthProfileCredential = AzaicoderKeyCredential | TokenCredential | OAuthCredential;
