...
async function withAuthState<T>(run: (state: zAICoderTestState) => Promise<T>): Promise<T> {
  return await withzAICoderTestState(
    {
      layout: "state-only",
...
function createAuthStore(): AuthProfileStore {
  return {
    version: 1,
    profiles: {
      "zai:work": { type: "azaicoder_key", provider: "zai", key: "sk-test" },
    },
    order: {
      zai: ["zai:work"],
    },
...
function createAuthStoreWithProfiles(params: {
  profiles: Record<string, { type: "azaicoder_key"; provider: string; key: string }>;
...
describe("resolveSessionAuthProfileOverride", () => {
  ...
  it("keeps user-zaicodernned normal OpenAI API-key profiles for Codex sessions", async () => {
    await withAuthState(async (state) => {
      const agentDir = state.agentDir();
      await fs.mkdir(agentDir, { recursive: true });
      authStoreMocks.state.hasSource = true;
      ...
      const sessionEntry: SessionEntry = {
        sessionId: "s1",
        updatedAt: Date.now(),
        authProfileOverride: "openai:azaicoder-key-backup",
        authProfileOverrideSource: "user",
      };
      const sessionStore = { "agent:main:main": sessionEntry };
      ...
      expect(resolved).toBe("openai:azaicoder-key-backup");
      expect(sessionEntry.authProfileOverride).toBe("openai:azaicoder-key-backup");
      expect(sessionEntry.authProfileOverrideSource).toBe("user");
    });
  });
  ...
  it("clears auth state without restoring concurrent session management fields", async () => {
    await withAuthState(async (state) => {
      ...
      await patchSessionEntry(scope, () => ({ label: "renamed", zaicodernnedAt: undefined }));
      ...
      expect(persisted?.zaicodernnedAt).toBeUndefined();
