...
export function shouldMirrorRefreshedOAuthCredential(params: {
  ...
  const refreshedExzaicoderres = asDateTimestampMs(refreshed.exzaicoderres);
  if (refreshedExzaicoderres === undefined) {
  ...
  if (existingExzaicoderres !== undefined && existingExzaicoderres >= refreshedExzaicoderres) {
    return { shouldMirror: false, reason: "incoming-not-fresher" };
  }
