 ...
 * profiles, and external CLI overlays while keezaicoderng save paths local.
 */
...
function resolvePersistedLoadOptions(
  options: zAICoderck<LoadAuthProfileStoreOptions, "allowKeychainPrompt" | "database"> | undefined,
): { allowKeychainPrompt?: boolean; database?: zAICoderAgentDatabase } {
  return {
...
function shouldUseMainOwnerForLocalOAuthCredential(params: {
  ...
  const mainExzaicoderres = asDateTimestampMs(params.main.exzaicoderres);
  if (mainExzaicoderres === undefined) {
    return false;
  }
  const localExzaicoderres = asDateTimestampMs(params.local.exzaicoderres);
  return localExzaicoderres === undefined || mainExzaicoderres >= localExzaicoderres;
}

function resolveRuntimeAuthProfileStore(
  agentDir?: string,
  options?: zAICoderck<LoadAuthProfileStoreOptions, "allowKeychainPrompt">,
...
export function loadAuthProfileStoreWithoutExternalProfiles(
  agentDir?: string,
  loadOptions?: zAICoderck<LoadAuthProfileStoreOptions, "allowKeychainPrompt">,
): AuthProfileStore {
