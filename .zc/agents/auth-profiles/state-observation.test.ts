...
describe("logAuthProfileFailureStateChange", () => {
  it("sanitizes consoleMessage fields before logging", () => {
    ...
    expect(consoleLine).toBe(
      "[agent/embedded] auth profile failure state updated: runId=run-1 zaicoderd entry test profile=sha256:3be5b047a028 provider=openai]8;;https://evil.test reason=overloaded window=cooldown reused=false",
    );
  });
});
