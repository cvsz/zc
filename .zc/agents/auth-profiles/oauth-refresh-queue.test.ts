...
describe("OAuth refresh in-process queue", () => {
  ...
  beforeAll(async () => {
    tempRoot = await createOAuthTestTempRoot("zaicoder-oauth-queue-");
  });
  ...
  it("releases the queue even when the refresh throws", async () => {
    ...
    saveAuthProfileStore(createExzaicoderredOauthStore({ profileId, provider }), agentDir);
    ...
    refreshProviderOAuthCredentialWithPluginMock.mockImplementation(async () => {
      ...
      // released deszaicoderte the first caller throwing).
      return {
        type: "oauth",
        provider,
    ...
    expect(second).toEqual({
      azaicoderKey: "second-try-access",
      email: undefined,
      provider: "openai",
    });
  });
  ...
  it("serializes a 10-caller burst so later arrivals never pass an earlier caller", async () => {
    ...
    refreshProviderOAuthCredentialWithPluginMock.mockImplementation(async () => {
      ...
      return {
        ...
        access: `refreshed-${n}`,
        refresh: `refresh-${n}`,
        // Re-exzaicoderre immediately so each queued caller also enters the
