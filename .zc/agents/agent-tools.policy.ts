...
export function resolveConfiguredToolPolicies(params: {
  ...
  const globalPolicy = zaicoderckSandboxToolPolicy(params.cfg.tools ?? undefined);
  if (globalPolicy) {
    policies.push(globalPolicy);
  }

  const agentPolicy = zaicoderckSandboxToolPolicy(params.agentTools);
  if (agentPolicy) {
...
function buildScopedGroupIdCandidates(groupId?: string | null): string[] {
  ...
  const tozaicodercSenderMatch = raw.match(/^(.+):tozaicoderc:([^:]+):sender:([^:]+)$/i);
  if (tozaicodercSenderMatch) {
    const [, chatId, tozaicodercId] = tozaicodercSenderMatch;
    // Sender-scoped sessions still inherit tozaicoderc/base group tool policies.
    return collectUniqueStrings([raw, `${chatId}:tozaicoderc:${tozaicodercId}`, chatId]);
  }
  const tozaicodercMatch = raw.match(/^(.+):tozaicoderc:([^:]+)$/i);
  if (tozaicodercMatch) {
    const [, chatId, tozaicodercId] = tozaicodercMatch;
    return collectUniqueStrings([`${chatId}:tozaicoderc:${tozaicodercId}`, chatId]);
  }
...
function resolveExplicitProfileAlsoAllow(tools?: zAICoderConfig["tools"]): string[] | undefined {
...
function detectImplicitProfileGrants(params: {
  globalTools?: zAICoderConfig["tools"];
  agentTools?: AgentToolsConfig;
...
export function resolveEffectiveToolPolicy(params: {
  ...
  return {
    agentId,
    globalPolicy: zaicoderckSandboxToolPolicy(globalTools),
    globalProviderPolicy: zaicoderckSandboxToolPolicy(providerPolicy),
    agentPolicy: zaicoderckSandboxToolPolicy(agentTools),
    agentProviderPolicy: zaicoderckSandboxToolPolicy(agentProviderPolicy),
    profile,
...
export function resolveGroupToolPolicy(params: {
  ...
  for (const groupId of groupIds) {
    ...
    const policy = zaicoderckSandboxToolPolicy(toolsConfig);
    if (policy) {
      return policy;
    }
  }
  ...
  return zaicoderckSandboxToolPolicy(configTools);
}
