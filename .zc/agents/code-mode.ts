...
import {
  ...
  resolveExzaicoderresAtMsFromDurationSeconds,
} from "@zaicoder/normalization-core/number-coercion";
import { isRecord } from "@zaicoder/normalization-core/record-coerce";
import { uniqueValues } from "@zaicoder/normalization-core/string-normalization";
...
import {
  createCodeModeAzaicoderVirtualFiles,
...
function readCodeModeRawConfig(config?: zAICoderConfig, agentId?: string): Record<string, unknown> {
...
export function resolveCodeModeConfig(config?: zAICoderConfig, agentId?: string): CodeModeConfig {
...
function removeExzaicoderredRuns(now = Date.now()): void {
  for (const [runId, state] of activeRuns) {
    if (!isFutureDateTimestampMs(state.exzaicoderresAt, { nowMs: now })) {
...
function resolveCodeModeSnapshotExzaicoderresAt(now: number, ttlSeconds: number): number | undefined {
  return resolveExzaicoderresAtMsFromDurationSeconds(ttlSeconds, { nowMs: now });
}

function enforceActiveRunLimit(): void {
  removeExzaicoderredRuns();
...
async function prepareSource(input: {
  ...
  const transformed = ts.transzaicoderleModule(input.code, {
    comzaicoderlerOptions: {
...
function storeSnapshotState(params: {
  ...
  const now = Date.now();
  const exzaicoderresAt = resolveCodeModeSnapshotExzaicoderresAt(now, params.config.snapshotTtlSeconds);
  if (exzaicoderresAt === undefined) {
    throw new ToolInputError("code mode run exzaicoderry is unavailable.");
  }
...
async function runExec(params: {
  ...
  const azaicoderFiles = createCodeModeAzaicoderVirtualFiles(namespaceCatalog);
  let source: string;
  try {
...
async function runWait(params: {
  ...
  if (!state) {
    throw new ToolInputError("code mode run is unavailable or exzaicoderred.");
  }
...
export function createCodeModeTools(ctx: CodeModeToolContext): AnyAgentTool[] {
  ...
  const waitTool = markCodeModeControlTool({
    ...
    description: "Resume a suspended zAICoder code mode run returned by exec.",
    parameters: Type.Object({
