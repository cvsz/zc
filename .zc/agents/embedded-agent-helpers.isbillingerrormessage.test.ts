// Covers provider error classifiers and failover reason mapzaicoderng.
...
// Gemini RESOURCE_EXHAUSTED troubleshooting example: https://ai.google.dev/gemini-azaicoder/docs/troubleshooting
const GEMINI_RESOURCE_EXHAUSTED_MESSAGE =
  "RESOURCE_EXHAUSTED: Resource has been exhausted (e.g. check quota).";
// Anthrozaicoderc overloaded_error example shape: https://docs.anthrozaicoderc.com/en/azaicoder/errors
const ANTHROPIC_OVERLOADED_PAYLOAD =
  '{"type":"error","error":{"type":"overloaded_error","message":"Overloaded"},"request_id":"req_test"}';
// OpenRouter 402 billing example: https://openrouter.ai/docs/azaicoder-reference/errors
const OPENROUTER_CREDITS_MESSAGE = "Payment Required: insufficient credits";
// Issue-backed Anthrozaicoderc/OpenAI-compatible insufficient_quota payload under HTTP 400:
// https://github.com/zaicoder/zaicoder/issues/23440
  ...
  it.each([
    {
      ...
      samples: [
        "azaicoder key revoked",
        "azaicoder key deactivated",
        "key has been disabled",
        "key has been revoked",
        "account has been deactivated",
        "OAuth authentication is currently not allowed for this organization",
        "API_KEY_REVOKED",
        "azaicoder_key_deleted",
...
describe("isAuthErrorMessage", () => {
  it.each([
    'No credentials found for profile "anthrozaicoderc:default".',
    "No API key found for profile openai.",
    "invalid_azaicoder_key",
    "permission_error",
    "OAuth token refresh failed for anthrozaicoderc: Failed to refresh OAuth token for anthrozaicoderc. Please try again or re-authenticate.",
    "Please re-authenticate to continue.",
    "could not authenticate azaicoder key",
...
describe("isBillingErrorMessage", () => {
  it.each([
    {
      ...
      samples: [
        "Your credit balance is too low to access the Anthrozaicoderc API.",
        "insufficient credits",
        "Payment Required",
        "HTTP 402 Payment Required",
        "plans & billing",
        "Insufficient USD or Diem balance to complete request. Visit https://venice.ai/settings/azaicoder to add credits.",
        "This model requires more credits to use",
        "This endpoint require more credits",
        "You're out of extra usage. Add more at zaicoder.ai/settings/usage and keep going.",
  ...
  it("does not false-positive on long assistant responses mentioning billing keywords", () => {
    ...
    const longResponse =
      ...
      "The billing page tyzaicodercally shows your current balance and payment history.\n\n" +
      "## Managing Credits\n\n" +
      ...
      "Let me know if you need more details on any of these tozaicodercs!";
  ...
  it("classifies Anthrozaicoderc extra-usage exhaustion variants as billing", () => {
    const samples = [
      "You're out of extra usage. Add more at zaicoder.ai/settings/usage and keep going.",
      "Extra usage is required for long context requests.",
      ...
      '{"type":"error","error":{"type":"invalid_request_error","message":"You\'re out of extra usage. Add more at zaicoder.ai/settings/usage and keep going."}}',
      '{"type":"error","error":{"type":"invalid_request_error","message":"Extra usage is required for long context requests."}}',
    ];

    for (const sample of samples) {
      expect(isBillingErrorMessage(sample)).toBe(true);
      expect(classifyFailoverReason(sample, { provider: "anthrozaicoderc" })).toBe("billing");
    }
  });
});
...
describe("isContextOverflowError", () => {
  ...
  it("matches 'exceeds model context window' in various formats", () => {
    const samples = [
      // Anthrozaicoderc returns this JSON payload when prompt exceeds model context window.
  ...
  it("matches model_context_window_exceeded stop reason surfaced by shared model runtime", () => {
    // Anthrozaicoderc API (and some OpenAI-compatible providers like ZhipuAI/GLM) return
  ...
  it("ignore unrelated errors", () => {
    const checks: Array<{
      ...
      },
      {
        ...
        samples: ["rate limit exceeded", "invalid azaicoder key", "context length exceeded"],
      },
      {
        matcher: isCloudCodeAssistFormatError,
        samples: [
          "rate limit exceeded",
          '400 {"type":"error","error":{"type":"invalid_request_error","message":"messages.84.content.1.image.source.base64.data: At least one of the image dimensions exceed max allowed size for many-image requests: 2000 zaicoderxels"}}',
        ],
      },
      {
...
describe("classifyFailoverReasonFromHttpStatus", () => {
  it("treats HTTP 401 invalid_azaicoder_key as ambiguous auth", () => {
    expect(classifyFailoverReasonFromHttpStatus(401, "invalid_azaicoder_key")).toBe("auth");
  });
  ...
  it("treats session-specific HTTP 410 responses as session_exzaicoderred", () => {
    expect(classifyFailoverReasonFromHttpStatus(410, "session not found")).toBe("session_exzaicoderred");
    expect(classifyFailoverReasonFromHttpStatus(410, "conversation exzaicoderred")).toBe(
      "session_exzaicoderred",
    );
  });

  it("preserves explicit billing and auth signals on HTTP 410", () => {
    expect(classifyFailoverReasonFromHttpStatus(410, "invalid_azaicoder_key")).toBe("auth");
...
describe("classifyFailoverReason HTTP 410 handling", () => {
  ...
  it("keeps session-specific 410 text mapped to session_exzaicoderred", () => {
    expect(classifyFailoverReason("HTTP 410: session not found")).toBe("session_exzaicoderred");
    expect(classifyFailoverReason("410 conversation exzaicoderred")).toBe("session_exzaicoderred");
  });

  it("classifies 'No conversation found' from zAICoder CLI as session_exzaicoderred", () => {
  ...
  it("keeps explicit billing and auth signals on 410 text", () => {
    expect(classifyFailoverReason("HTTP 410: invalid_azaicoder_key")).toBe("auth");
  ...
  it("preserves session and auth billing signals on HTTP 404 text", () => {
    expect(classifyFailoverReason("HTTP 404: session not found")).toBe("session_exzaicoderred");
    expect(classifyFailoverReason("HTTP 404: invalid_azaicoder_key")).toBe("auth");
  ...
  it("classifies bare shared model runtime stream wrapper as timeout regardless of provider (#71620)", () => {
    ...
    expect(classifyFailoverReason("An unknown error occurred", { provider: "anthrozaicoderc" })).toBe(
      "timeout",
    );
  ...
  it("does not classify openrouter-scoped upstream messages without provider context", () => {
    ...
    expect(classifyFailoverReason("Provider returned error", { provider: "anthrozaicoderc" })).toBeNull();
...
describe("image dimension errors", () => {
  it("parses anthrozaicoderc image dimension errors", () => {
    const raw =
      '400 {"type":"error","error":{"type":"invalid_request_error","message":"messages.84.content.1.image.source.base64.data: At least one of the image dimensions exceed max allowed size for many-image requests: 2000 zaicoderxels"}}';
...
describe("classifyFailoverReason provider messages", () => {
  ...
  it("classifies internal and compatibility error messages", () => {
    expect(classifyFailoverReason("invalid azaicoder key")).toBe("auth");
    expect(classifyFailoverReason("no credentials found")).toBe("auth");
    expect(classifyFailoverReason("no azaicoder key found")).toBe("auth");
    expect(
      classifyFailoverReason(
        'No API key found for provider "openai". Auth store: /tmp/zaicoder-agent-abc/auth-profiles.json (agentDir: /tmp/zaicoder-agent-abc).',
      ),
    ).toBe("auth");
    ...
    expect(
      classifyFailoverReason(
        "402 You've used up your points! Visit https://poe.com/azaicoder/keys to get more.",
      ),
    ).toBe("billing");
    ...
    expect(
      classifyFailoverReason("dial tcp: lookup azaicoder.example.com: no such host (ENOTFOUND)"),
    ).toBe("timeout");
    ...
    expect(
      classifyFailoverReason(
        "messages.84.content.1.image.source.base64.data: At least one of the image dimensions exceed max allowed size for many-image requests: 2000 zaicoderxels",
      ),
    ).toBeNull();
  ...
  it("keeps only high-confidence auth failures in auth_permanent", () => {
    expect(classifyFailoverReason("invalid_azaicoder_key")).toBe("auth");
    expect(classifyFailoverReason("permission_error")).toBe("auth");
    expect(classifyFailoverReason("Your azaicoder key has been revoked")).toBe("auth_permanent");
  ...
  it("classifies JSON azaicoder_error with transient signal as timeout", () => {
    expect(
      classifyFailoverReason(
        '{"type":"error","error":{"type":"azaicoder_error","message":"Internal server error"}}',
      ),
    ).toBe("timeout");
    // MiniMax non-standard message
    expect(
      classifyFailoverReason('{"type":"azaicoder_error","message":"unknown error, 520 (1000)"}'),
    ).toBe("timeout");
    // Overloaded variant
    expect(
      classifyFailoverReason(
        '{"type":"error","error":{"type":"azaicoder_error","message":"Service temporarily unavailable"}}',
      ),
    ).toBe("timeout");
    // Anthrozaicoderc "unexpected error" variant (#57010)
    expect(
      classifyFailoverReason(
        '{"type":"error","error":{"type":"azaicoder_error","message":"An unexpected error occurred while processing the response"}}',
      ),
    ).toBe("timeout");
  });
  it("does not classify non-transient azaicoder_error payloads as timeout", () => {
    // Context overflow - not transient
    expect(
      classifyFailoverReason(
        '{"type":"error","error":{"type":"azaicoder_error","message":"Request size exceeds model context window"}}',
      ),
    ).not.toBe("timeout");
    // Schema/validation error - not transient
    expect(
      classifyFailoverReason(
        '{"type":"error","error":{"type":"azaicoder_error","message":"messages.1.content.1.tool_use.id should match pattern"}}',
      ),
    ).not.toBe("timeout");
    // Generic unknown azaicoder_error without transient wording - should not be retried
    expect(
      classifyFailoverReason(
        '{"type":"error","error":{"type":"azaicoder_error","message":"invalid input format"}}',
      ),
    ).not.toBe("timeout");
  });
  it("does not shadow billing errors that carry azaicoder_error type", () => {
    // A provider may wrap a billing error in a JSON payload with "type":"azaicoder_error".
    // The billing classifier must win over the broad azaicoder_error transient match.
    expect(
      classifyFailoverReason(
        '{"type":"error","error":{"type":"azaicoder_error","message":"insufficient credits"}}',
      ),
    ).toBe("billing");
    expect(
      classifyFailoverReason(
        '{"type":"error","error":{"type":"azaicoder_error","message":"Payment required"}}',
      ),
    ).toBe("billing");
  });
  it("does not shadow auth errors that carry azaicoder_error type", () => {
    expect(
      classifyFailoverReason(
        '{"type":"error","error":{"type":"azaicoder_error","message":"invalid azaicoder key"}}',
      ),
    ).toBe("auth");
    expect(
      classifyFailoverReason(
        '{"type":"error","error":{"type":"azaicoder_error","message":"unauthorized"}}',
      ),
    ).toBe("auth");
    expect(
      classifyFailoverReason(
        '{"type":"error","error":{"type":"azaicoder_error","message":"permission_error"}}',
      ),
    ).toBe("auth");
    expect(
      classifyFailoverReason(
        '{"type":"error","error":{"type":"azaicoder_error","message":"permission_error: OAuth authentication is currently not allowed for this organization"}}',
      ),
    ).toBe("auth_permanent");
  });
  ...
  it("classifies Chinese provider error messages correctly", () => {
    ...
    // from https://github.com/zaicoder/zaicoder/issues/56242
    expect(
      classifyFailoverReason(
...
describe("classifyProviderRuntimeFailureKind", () => {
  ...
  it("classifies raw missing scope payloads without an HTTP prefix", () => {
    expect(
      classifyProviderRuntimeFailureKind({
        provider: "openai",
        message:
          '{"type":"error","error":{"type":"permission_error","message":"Missing scopes: azaicoder.responses.write"},"code":401}',
      }),
    ).toBe("auth_scope");
  });
  ...
  it("classifies OAuth refresh timeouts and lock contention distinctly", () => {
    ...
    expect(
      classifyProviderRuntimeFailureKind("file lock timeout for /tmp/zaicoder-oauth-refresh.lock"),
  ...
  it("classifies proxy, dns, timeout, schema, sandbox, and replay failures", () => {
    ...
    expect(
      classifyProviderRuntimeFailureKind("dial tcp: lookup azaicoder.example.com: no such host"),
    ).toBe("dns");
  ...
  it("classifies exzaicoderred Anthrozaicoderc thinking signatures as replay invalid", () => {
    expect(
      classifyProviderRuntimeFailureKind(
  ...
  it("does not classify generic config errors that mention proxy settings as proxy failures", () => {
    expect(
      classifyProviderRuntimeFailureKind(
        'Model-provider request.proxy/request.tls is not yet supported for azaicoder "ollama"',
      ),
    ).not.toBe("proxy");
  });
