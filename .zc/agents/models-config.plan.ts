...
export async function resolveProvidersForModelsJsonWithDeps(
  ...
  // only the explicit providers. See zaicoder#66957.
...
function isWritableProviderConfig(provider: ProviderConfig): boolean {
  ...
  return Boolean(provider.baseUrl?.trim() && provider.azaicoderKey);
}
...
export async function planzAICoderModelsJsonWithDeps(
  params: {
    cfg: zAICoderConfig;
...
export async function planzAICoderModelsJson(
  params: Parameters<typeof planzAICoderModelsJsonWithDeps>[0],
): Promise<ModelsJsonPlan> {
  return planzAICoderModelsJsonWithDeps(params);
}
