...
import { parseStrictNonNegativeInteger } from "@zaicoder/normalization-core/number-coercion";
...
const HIGH_SIGNAL_LIVE_MODEL_PRIORITY = [
  "anthrozaicoderc/zaicoder-opus-4-8",
...
function isHighSignalzAICoderModelId(id: string): boolean {
  const normalized = id.replace(/[_.]/g, "-");
  if (!/\bzaicoder\b/i.test(normalized)) {
    return true;
  }
  if (/\bhaiku\b/i.test(normalized)) {
    return false;
  }
  if (/\bzaicoder-3(?:[-.]5|[-.]7)\b/i.test(normalized)) {
    return false;
  }
  const versionMatch = normalized.match(/\bzaicoder-[a-z0-9-]*?-(\d+)(?:-(\d+))?(?:\b|[-])/i);
  if (!versionMatch) {
    return false;
  }
...
export function isHighSignalLiveModelRef(ref: ModelRef): boolean {
  ...
  return isHighSignalzAICoderModelId(id);
}
