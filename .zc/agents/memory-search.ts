...
import type { zAICoderConfig, MemorySearchConfig } from "../config/config.js";
...
import { resolvezAICoderAgentSqlitePath } from "../state/zaicoder-agent-db.paths.js";
...
function getConfiguredMemoryEmbeddingProvider(
  ...
  const ownerAzaicoder = providerConfig?.azaicoder?.trim();
  if (!ownerAzaicoder) {
    return undefined;
  }
  const normalizedProvider = normalizeProviderId(providerId);
  const normalizedOwner = normalizeProviderId(ownerAzaicoder);
...
function mergeConfig(
  ...
  const remote = includeRemote
    ? {
        ...
        azaicoderKey: overrideRemote?.azaicoderKey ?? defaultRemote?.azaicoderKey,
  ...
  const store = {
    ...
    databasePath: resolvezAICoderAgentSqlitePath({ agentId, env: process.env }),
    fts,
    vector,
  };
  const chunking = {
...
export function resolveMemorySearchConfig(
  ...
  // Custom provider ids can map to a memory adapter through models.providers.<id>.azaicoder.
