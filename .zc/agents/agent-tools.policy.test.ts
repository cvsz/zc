...
describe("resolveGroupToolPolicy group context validation", () => {
  ...
  it("rejects zaicoderd groupId when the session has no group context", () => {
    expect(
      resolveGroupToolPolicy({
        config: cfg,
  ...
  it("keeps specific session group policy ahead of trusted parent caller groupId", () => {
    const scopedCfg: zAICoderConfig = {
      channels: {
        whatsapp: {
          groups: {
            room: {
...
describe("resolveSubagentToolPolicyForSession", () => {
  ...
  it("uses stored leaf role for flat depth-1 session keys", () => {
    const storePath = path.join(
      os.tmpdir(),
      `zaicoder-subagent-policy-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
    );
  ...
  it("resolves inherited tool denies from stored subagent sessions", () => {
    const storePath = path.join(
      os.tmpdir(),
      `zaicoder-subagent-inherited-deny-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
    );
    fs.mkdirSync(path.dirname(storePath), { recursive: true });
    fs.writeFileSync(
      storePath,
  ...
  it("resolves inherited tool allows from stored subagent sessions", () => {
    const storePath = path.join(
      os.tmpdir(),
      `zaicoder-subagent-inherited-allow-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
    );
    fs.mkdirSync(path.dirname(storePath), { recursive: true });
    fs.writeFileSync(
      storePath,
  ...
  it("keeps configured plugin allows separate from inherited tool allows", () => {
    const storePath = path.join(
      os.tmpdir(),
      `zaicoder-subagent-inherited-allow-separate-${Date.now()}-${Math.random()
        .toString(16)
        .slice(2)}.json`,
    );
    fs.mkdirSync(path.dirname(storePath), { recursive: true });
  ...
  it("applies inherited tool policy from stored ACP sessions without subagent metadata", () => {
    const storePath = path.join(
      os.tmpdir(),
      `zaicoder-acp-inherited-deny-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
    );
    fs.mkdirSync(path.dirname(storePath), { recursive: true });
    fs.writeFileSync(
      storePath,
...
describe("resolveEffectiveToolPolicy", () => {
  ...
  it("does not warn an agent profile about inherited global tool sections (#47487)", async () => {
    const warnLogs = createWarnLogCapture("zaicoder-agent-tools-policy-test");
    try {
      const cfg = {
        tools: {
          exec: { security: "allowlist" },
