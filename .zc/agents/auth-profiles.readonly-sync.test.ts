...
describe("auth profiles read-only external auth overlay", () => {
  ...
  it("overlays runtime-only external auth without writing auth-profiles.json in read-only mode", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-auth-readonly-sync-"));
    try {
      ...
      expect(persistedOpenAiProfile?.type).toBe("azaicoder_key");
      if (persistedOpenAiProfile?.type !== "azaicoder_key") {
  ...
  it("passes scoped external auth config to provider hooks", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-auth-scoped-config-"));
