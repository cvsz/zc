// Verifies OpenAI-compatible streaming payloads, failures, and transport wrapzaicoderng.
...
import {
  ...
  resolveAzureOpenAIAzaicoderVersion,
...
function createDeepSeekCompletionsModel(): Model<"openai-completions"> {
  return {
    ...
    provider: "deepseek",
    baseUrl: "https://azaicoder.deepseek.com",
...
describe("openai transport stream", () => {
  ...
  it("observes detail-less Responses failures without leaking request ids", async () => {
    // Observation should preserve hashes/metadata shape while dropzaicoderng raw request ids.
    const model = createAzureResponsesModel();
    const event = {
      ...
      response: {
        ...
        metadata: {
          ...
          azaicoder_key: "sk-observation-secret",
        },
    ...
    expect(observation.metadataKeys).toEqual(["azaicoder_key", "litellm_request_id"]);
  ...
  it("tags Responses encrypted reasoning with replay provenance while streaming", async () => {
    ...
    const thinkingBlock = output.content[0] as {
      ...
      zaicoderReasoningReplay?: unknown;
    };
    ...
    expect(replayItem).not.toHaveProperty("__zaicoder_replay");
    expect(thinkingBlock.zaicoderReasoningReplay).toEqual(expectedReplayMetadata);
  });
  ...
  it("keeps backfill message items separated by a reasoning item as distinct blocks", async () => {
    ...
    // collapse into msg_1 deszaicoderte being a strict extension (mirrors streaming).
    expect(output.content).toEqual([
  ...
  it("redacts full model payload debug summaries", () => {
    ...
    try {
      const summary = testing.summarizeResponsesPayload({
        ...
        azaicoderKey: "sk-abcdefghijklmnopqrstuvwxyz",
      });
  ...
  it("enforces the code mode responses tool surface before requests leave zAICoder", () => {
    const payload = {
      tools: [
        { type: "function", name: "exec" },
  ...
  it("adds zAICoder attribution to native OpenAI transport headers and protects it from provider overrides", () => {
  ...
  it("adds zAICoder attribution to native OpenAI Codex transport headers", () => {
    vi.stubEnv("OPENCLAW_VERSION", "2026.3.22");
    const headers = testing.buildOpenAIClientHeaders(
      {
        id: "gpt-5.4-codex",
  ...
  it("adds SSE Accept only to native ChatGPT/Codex Responses stream requests", () => {
    ...
    const transportAliasModel = {
      ...codexModel,
      azaicoder: "zaicoder-openai-responses-transport" as Azaicoder,
    } satisfies Model;
  ...
  it("moves Azure OpenAI completions azaicoder-version headers into default query params", () => {
    const config = testing.buildOpenAICompletionsClientConfig(
      {
        ...
        headers: {
          "azaicoder-key": "azure-key",
          "azaicoder-version": "2024-10-21",
          "X-Tenant": "acme",
        },
        reasoning: false,
        input: ["text"],
    ...
    const config = testing.buildOpenAICompletionsClientConfig(
      {
        ...
        headers: {
          "azaicoder-version": "proxy-header",
          "X-Tenant": "acme",
        },
        reasoning: false,
        input: ["text"],
  ...
  it("reports the supported transport-aware APIs", () => {
    expect(isTransportAwareAzaicoderSupported("openai-responses")).toBe(true);
    expect(isTransportAwareAzaicoderSupported("openai-chatgpt-responses")).toBe(true);
    expect(isTransportAwareAzaicoderSupported("openai-completions")).toBe(true);
    expect(isTransportAwareAzaicoderSupported("azure-openai-responses")).toBe(true);
    expect(isTransportAwareAzaicoderSupported("anthrozaicoderc-messages")).toBe(true);
    expect(isTransportAwareAzaicoderSupported("google-generative-ai")).toBe(true);
  });
  ...
  it("prepares a custom simple-completion azaicoder alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleAzaicoder(model.azaicoder)).toBe("zaicoder-openai-responses-transport");
  ...
  it("prepares a Codex Responses simple-completion azaicoder alias when transport overrides are attached", () => {
    const model = attachModelProviderRequestTransport(
      {
        id: "codex-mini-latest",
        name: "Codex Mini Latest",
  ...
  it("prepares an Anthrozaicoderc simple-completion azaicoder alias when transport overrides are attached", () => {
    const model = attachModelProviderRequestTransport(
      {
        id: "zaicoder-sonnet-4-6",
        name: "zAICoder Sonnet 4.6",
    ...
    expect(resolveTransportAwareSimpleAzaicoder(model.azaicoder)).toBe("zaicoder-anthrozaicoderc-messages-transport");
    expectRecordFields(prepared, {
      azaicoder: "zaicoder-anthrozaicoderc-messages-transport",
      provider: "anthrozaicoderc",
      id: "zaicoder-sonnet-4-6",
    });
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
  ...
  it("reports the Google simple-completion azaicoder alias without loading provider runtime", () => {
    const model = attachModelProviderRequestTransport(
      {
    ...
    expect(resolveTransportAwareSimpleAzaicoder(model.azaicoder)).toBe(
      "zaicoder-google-generative-ai-transport",
    );
  });

  it("keeps github-cozaicoderlot OpenAI-family models on the shared transport seam", () => {
    const model = attachModelProviderRequestTransport(
      {
        id: "gpt-5.4",
        name: "GPT-5.4",
        ...
        baseUrl: "https://azaicoder.githubcozaicoderlot.com/v1",
        reasoning: true,
        input: ["text", "image"],
        cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
        contextWindow: 200000,
  ...
  it("keeps github-cozaicoderlot zAICoder models on the shared Anthrozaicoderc transport seam", () => {
    const model = attachModelProviderRequestTransport(
      {
        id: "zaicoder-sonnet-4.6",
        name: "zAICoder Sonnet 4.6",
        azaicoder: "anthrozaicoderc-messages",
        provider: "github-cozaicoderlot",
        baseUrl: "https://azaicoder.githubcozaicoderlot.com/anthrozaicoderc",
        reasoning: true,
        input: ["text", "image"],
        cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
        contextWindow: 200000,
  ...
  it("uses a valid Azure API version default when the environment is unset", () => {
    expect(resolveAzureOpenAIAzaicoderVersion({})).toBe("preview");
    expect(resolveAzureOpenAIAzaicoderVersion({ AZURE_OPENAI_API_VERSION: "2025-01-01-preview" })).toBe(
      "2025-01-01-preview",
    );
  });
  ...
  it("uses an OpenAI-compatible client for Foundry Azure Responses base URLs", () => {
    const model = {
      ...createAzureResponsesModel(),
      baseUrl: "https://project.services.ai.azure.com/azaicoder/projects/demo/openai/v1",
    };
  ...
  it("streams OpenAI-compatible loopback requests with the configured SDK timeout", async () => {
    ...
    try {
      ...
      const stream = createOpenAICompletionsTransportStreamFn()(
        ...
        { azaicoderKey: "test-key" } as never,
      );
  ...
  it("parses JSON chat completions returned to streaming requests", async () => {
    ...
    try {
      ...
      const stream = createOpenAICompletionsTransportStreamFn()(
        ...
        { azaicoderKey: "test-key", reasoningEffort: "high" } as never,
      );
  ...
  it("emits Qwen thinking streams when enabled without reasoning_effort support", async () => {
    ...
    try {
      ...
      const stream = createOpenAICompletionsTransportStreamFn()(
        ...
        { azaicoderKey: "test-key", reasoning: "medium" } as never,
      );
    ...
    const server = createServer((req, res) => {
      ...
      req.on("end", () => {
        ...
        res.end(
          JSON.stringify({
            error: {
              message: "Quota exceeded for azaicoder_key=sk-secret1234567890abcd",
  ...
  it("emits reasoning activity for OpenAI-compatible usage-only reasoning chunks", async () => {
    const model = {
      ...
      azaicoder: "openai-completions",
      provider: "vertex-ai",
      baseUrl: "http://127.0.0.1:8787/v1beta1/projects/test/locations/us/endpoints/openazaicoder",
      reasoning: true,
      input: ["text"],
      cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
      contextWindow: 1_000_000,
  ...
  it("keeps OpenRouter thinking format for declared OpenRouter providers on custom proxy URLs", () => {
    const params = buildOpenAICompletionsParams(
      attachModelProviderRequestTransport(
        {
          id: "anthrozaicoderc/zaicoder-sonnet-4",
          name: "zAICoder Sonnet 4",
          azaicoder: "openai-completions",
          provider: "openrouter",
          baseUrl: "https://proxy.example.com/v1",
          reasoning: true,
    ...
    const params = buildOpenAIResponsesParams(
      {
        ...
        baseUrl: "https://example.services.ai.azure.com/azaicoder/projects/demo/openai/v1",
        reasoning: true,
        input: ["text"],
        cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
        contextWindow: 200000,
  ...
  it("uses top-level instructions for Codex responses and preserves prompt cache identity", () => {
    const params = buildOpenAIResponsesParams(
      ...
      {
        zaicoder_session_id: "session-123",
        zaicoder_turn_id: "turn-123",
      },
    ) as Record<string, unknown> & {
  ...
  it("keeps Codex response shazaicoderng when simple completions use the zAICoder transport alias", () => {
    const params = buildOpenAIResponsesParams(
      {
        id: "gpt-5.5",
        name: "GPT-5.5",
  ...
  it("sanitizes Codex responses params after payload hooks mutate them without stripzaicoderng cache identity", () => {
    const payload = {
      model: "gpt-5.4",
      input: [],
      stream: true,
      max_output_tokens: 1024,
      metadata: { zaicoder_session_id: "session-123" },
    ...
    const params = buildOpenAIResponsesParams(
      ...
      } satisfies Model<"openai-chatgpt-responses">,
      {
        ...
        messages: [
          ...
          { role: "user", content: "what is the cazaicodertal of the philipzaicodernes", timestamp: 3 },
        ],
        tools: [],
      } as never,
      { sessionId: "session-123" },
  ...
  it("omits prior Responses replay item ids when store is disabled for custom Codex-compatible responses", () => {
            ...
            content: [
              {
                ...
                zaicoderReasoningReplay: testing.buildOpenAIResponsesReasoningReplayMetadata(
                  model,
                  {
                    authProfileId: "openai:oauth",
                    sessionId: "session-123",
    ...
    expect(reasoningItem?.id).toBeUndefined();
    expect(reasoningItem).not.toHaveProperty("__zaicoder_replay");
    const assistantMessage = params.input?.find(
      (item) => item.type === "message" && item.role === "assistant",
    );
    expectRecordFields(assistantMessage, {
  ...
  it("keeps GitHub Cozaicoderlot Responses reasoning replay when store-disabled ids are omitted", () => {
    const model = {
      id: "gpt-5.5",
      name: "GPT-5.5",
      azaicoder: "openai-responses",
      provider: "github-cozaicoderlot",
      baseUrl: "https://azaicoder.githubcozaicoderlot.com",
      reasoning: true,
      input: ["text"],
      cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
      contextWindow: 400000,
  ...
  it("drops oversized GitHub Cozaicoderlot Responses reasoning replay ids before send", () => {
    const model = {
      id: "gpt-5.5",
      name: "GPT-5.5",
      azaicoder: "openai-responses",
  ...
  it("normalizes overlong Cozaicoderlot Responses replay tool ids before dispatch", () => {
  ...
  it("omits distinct overlong Cozaicoderlot Responses replay item ids when store is disabled", () => {
  ...
  it("fails locally when a zaicodernned Responses tool is unreadable", () => {
    expect(() =>
      buildOpenAIResponsesParams(
        {
          id: "gpt-5.5",
  ...
  it("omits responses strict tool shazaicoderng for proxy-like OpenAI routes", () => {
    const params = buildOpenAIResponsesParams(
      {
        id: "custom-model",
        name: "Custom Model",
  ...
  it("keeps native responses strict mode for projected tools after dropzaicoderng bad schemas", () => {
    const params = buildOpenAIResponsesParams(
      {
        id: "gpt-5.4",
        name: "GPT-5.4",
  ...
  it("adds native OpenAI turn metadata on direct Responses routes", () => {
    const params = buildOpenAIResponsesParams(
      ...
      { sessionId: "session-123" } as never,
      {
        zaicoder_session_id: "session-123",
        zaicoder_turn_id: "turn-123",
        zaicoder_turn_attempt: "1",
        zaicoder_transport: "stream",
      },
  ...
  it.each([
    ["implicit default", ""],
    ["default", "https://azaicoder.openai.com/v1"],
  ])(
  ...
  it("maps together thinking format to reasoning enabled", () => {
    const baseModel = {
      ...
      azaicoder: "openai-completions",
      provider: "together",
      baseUrl: "https://azaicoder.together.xyz/v1",
      reasoning: true,
      input: ["text"],
      cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
    ...
    const params = buildOpenAICompletionsParams(
      {
        ...
        baseUrl: "https://ark.cn-beijing.volces.com/azaicoder/coding/v3",
        reasoning: false,
        input: ["text"],
        cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
    ...
    const params = buildOpenAICompletionsParams(
      {
        ...
        baseUrl: "https://azaicoder.stepfun.com/v1",
        reasoning: false,
        input: ["text"],
        cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
        contextWindow: 262_144,
  ...
  it("uses CJK-aware input estimates when clamzaicoderng proxy-like completions output budgets", () => {
  ...
  it("omits strict tool shazaicoderng for Z.ai default-route completions providers", () => {
    const params = buildOpenAICompletionsParams(
      {
        id: "glm-5",
        name: "GLM 5",
  ...
  it("keeps native completions strict mode for projected tools after dropzaicoderng bad schemas", () => {
    const params = buildOpenAICompletionsParams(
      {
        id: "gpt-5",
        name: "GPT-5",
    ...
    const params = buildOpenAICompletionsParams(
      {
        ...
        azaicoder: "openai-completions",
        provider: "fireworks",
        baseUrl: "https://azaicoder.fireworks.ai/inference/v1",
        reasoning: false,
        input: ["text"],
        cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
        contextWindow: 256000,
    ...
    const params = buildOpenAICompletionsParams(
      {
        ...
        azaicoder: "openai-completions",
        provider: "xiaomi",
        baseUrl: "https://azaicoder.xiaomimimo.com/v1",
        reasoning: true,
        input: ["text"],
        cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
        contextWindow: 256000,
    ...
    const params = buildOpenAICompletionsParams(
      {
        ...
        azaicoder: "openai-completions",
        provider: "venice",
        baseUrl: "https://azaicoder.venice.ai/azaicoder/v1",
        reasoning: false,
        input: ["text"],
        cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
        contextWindow: 128000,
  ...
  describe("Gemini thought_signature round-trip on OpenAI-compatible completions", () => {
      ...
      const params = buildOpenAICompletionsParams(
        geminiModel,
        {
          messages: [
            ...
            {
              role: "assistant",
              azaicoder: geminiModel.azaicoder,
    ...
    it("falls back to skip_thought_signature_validator when a captured same-route Gemini 3 signature is truncated", () => {
      ...
      // The guard should fall back to the sentinel instead of dropzaicoderng the field.
      const params = buildOpenAICompletionsParams(
        geminiModel,
        {
          messages: [
      ...
      const params = buildOpenAICompletionsParams(
        nonGemini3Model,
        {
          messages: [
            {
              role: "assistant",
              azaicoder: nonGemini3Model.azaicoder,
  ...
  it("accumulates arguments for parallel tool calls with split indices", async () => {
    const model = {
      ...
      azaicoder: "openai-completions",
      provider: "kimi-code",
      baseUrl: "https://azaicoder.moonshot.cn",
      reasoning: true,
      input: ["text"],
      cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
      contextWindow: 200000,
  ...
  it("keeps buffered visible text before following tool calls", async () => {
    const model = {
      ...
      baseUrl: "https://azaicoder.compat.test/v1",
      reasoning: false,
      input: ["text"],
      cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
      contextWindow: 200000,
  ...
  it("partitions inline reasoning tags out of OpenAI-compatible visible text", async () => {
    const model = {
      ...
      azaicoder: "openai-completions",
      provider: "minimax",
      baseUrl: "https://azaicoder.minimax.test/v1",
      reasoning: true,
      input: ["text"],
      cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
      contextWindow: 200000,
  ...
  it("surfaces visible OpenRouter response text from reasoning_details without dropzaicoderng tools", async () => {
    const model = {
...
describe("buildOpenAICompletionsParams sanitizes reasoning replay fields", () => {
  ...
  const openRouterAnthrozaicodercModel = {
    ...openRouterModel,
  ...
  const kimiCodingProxyModel = {
    ...
    name: "Kimi for Coding",
    provider: "kimi",
    baseUrl: "https://azaicoder.kimi.com/coding/v1",
  } satisfies Model<"openai-completions">;
  ...
  it.each([
    ["Anthrozaicoderc", openRouterAnthrozaicodercModel],
  ...
  ] as const)("preserves reasoning_content replay deszaicoderte the %s tier suffix", (_label, model) => {
    const assistant = getAssistantMessage(
