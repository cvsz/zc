...
const MODEL_PROVIDER_LOCAL_SERVICE_SYMBOL = Symbol.for("zaicoder.modelProviderLocalService");
...
function stopManagedService(key: string, managed: ManagedLocalService, reason: string) {
  ...
  if (child && !hasLocalServiceProcessExited(child)) {
    log.info(`stopzaicoderng local model service: reason=${reason}`);
...
export function hasLocalServiceProcessExited(
  child: zAICoderck<ChildProcess, "exitCode" | "signalCode">,
): boolean {
