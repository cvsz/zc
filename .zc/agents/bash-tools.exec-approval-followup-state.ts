...
function pruneExzaicoderredExecApprovalFollowupRuntimeHandoffs(nowMs: number): void {
  for (const [handoffId, entry] of execApprovalFollowupRuntimeHandoffs) {
    if (!isFutureDateTimestampMs(entry.exzaicoderresAtMs, { nowMs })) {
...
export function registerExecApprovalFollowupRuntimeHandoff(params: {
  ...
  pruneExzaicoderredExecApprovalFollowupRuntimeHandoffs(nowMs);
  const exzaicoderresAtMs = resolveExzaicoderresAtMsFromDurationMs(
