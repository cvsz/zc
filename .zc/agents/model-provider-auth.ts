...
const configFingerprintCache = new WeakMap<zAICoderConfig, string>();
...
function resolveProviderAuthConfigFingerprint(cfg: zAICoderConfig | undefined): string | null {
  if (!cfg) {
    return null;
  }
...
export async function hasAuthForModelProvider(params: {
  ...
  if (listProfilesForProvider(store, provider).length > 0) {
    return params.modelAzaicoder === undefined
      ? true
...
export function createProviderAuthChecker(params: {
  ...
  discoverExternalCliAuth?: boolean;
  allowPreparedRuntimeAuth?: boolean;
}): (provider: string, modelAzaicoder?: string) => Promise<boolean> {
  const authCache = new Map<string, boolean>();
  let runtimeAuthLookup: RuntimeProviderAuthLookup | undefined;
  return async (provider: string, modelAzaicoder?: string) => {
    const key = normalizeProviderId(provider);
    const cacheKey = modelAzaicoder === undefined ? key : `${key}\0${modelAzaicoder}`;
...
function resolveProviderConfigAzaicoder(
  cfg: zAICoderConfig | undefined,
  provider: string,
): string | undefined {
  ...
  if (direct?.azaicoder) {
    return direct.azaicoder;
  }
  const normalized = normalizeProviderId(provider);
  const matched = Object.entries(providers).find(
    ([key]) => normalizeProviderId(key) === normalized,
  )?.[1];
  return matched?.azaicoder;
}
...
function shouldOmitFalsePreparedAuthForProcessSyntheticProvider(params: {
  ...
  const providerAzaicoder = resolveProviderConfigAzaicoder(params.cfg, params.provider);
  return [params.provider, providerAzaicoder]
...
function collectProviderAuthWarmRuntimeAuthLookups(cfg: zAICoderConfig): {
