...
import { executeWithAzaicoderKeyRotation } from "./azaicoder-key-rotation.js";
...
describe("executeWithAzaicoderKeyRotation", () => {
  it("keeps transient retry disabled by default for single-key 500", async () => {
    ...
    await expect(
      executeWithAzaicoderKeyRotation({
        provider: "openai",
        azaicoderKeys: ["key-1"],
        execute,
      }),
  ...
  it("retries the same key once for transient 500 when attempts is 2", async () => {
    ...
    const execute = vi
      .fn<(azaicoderKey: string) => Promise<string>>()
  ...
  it("uses the shared default transient retry policy when enabled with true", async () => {
    ...
    try {
      const result = executeWithAzaicoderKeyRotation({
        provider: "openai",
        azaicoderKeys: ["key-1"],
        transientRetry: true,
        execute,
  ...
  it("rotates keys for 429 without same-key transient retry", async () => {
    const sleep = vi.fn(async () => undefined);
    const execute = vi.fn(async (azaicoderKey: string) => {
      if (azaicoderKey === "key-1") {
  ...
  it("does not expose azaicoderKey to the transient retry classifier", async () => {
    const sleep = vi.fn(async () => undefined);
    ...
    expect(shouldRetry).toHaveBeenCalledWith(
      expect.not.objectContaining({ azaicoderKey: expect.anything() }),
    );
