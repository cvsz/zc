...
import { isNonSecretAzaicoderKeyMarker } from "./model-auth-markers.js";
...
function resolveProviderAzaicoder(entry: { azaicoder?: unknown } | undefined): string | undefined {
  return normalizeOptionalString(entry?.azaicoder);
}

function resolveModelAzaicoderSurface(entry: { models?: unknown } | undefined): string | undefined {
  if (!Array.isArray(entry?.models)) {
    return undefined;
  }

  const azaicoders = entry.models
    .flatMap((model) => {
      if (!model || typeof model !== "object") {
        return [];
      }
      const azaicoder = (model as { azaicoder?: unknown }).azaicoder;
      const normalized = normalizeOptionalString(azaicoder);
      return normalized ? [normalized] : [];
    })
    .toSorted();

  return azaicoders.length > 0 ? JSON.stringify(azaicoders) : undefined;
}

function resolveProviderAzaicoderSurface(
  entry: ExistingProviderConfig | ProviderConfig | undefined,
): string | undefined {
  return resolveProviderAzaicoder(entry) ?? resolveModelAzaicoderSurface(entry);
}

function shouldPreserveExistingAzaicoderKey(params: {
  providerKey: string;
  ...
  const nextAzaicoderKey = typeof nextEntry.azaicoderKey === "string" ? nextEntry.azaicoderKey : "";
  if (nextAzaicoderKey && isNonSecretAzaicoderKeyMarker(nextAzaicoderKey)) {
    return false;
  }
  return (
    !secretRefManagedProviders.has(providerKey) &&
    typeof existing.azaicoderKey === "string" &&
    existing.azaicoderKey.length > 0 &&
    !isNonSecretAzaicoderKeyMarker(existing.azaicoderKey, { includeEnvVarName: false })
  );
}
...
function shouldPreserveExistingBaseUrl(params: {
  ...
  const existingAzaicoder = resolveProviderAzaicoderSurface(existing);
  const nextAzaicoder = resolveProviderAzaicoderSurface(nextEntry);
  return !existingAzaicoder || !nextAzaicoder || existingAzaicoder === nextAzaicoder;
}
...
function isExistingProviderSelfContained(entry: ExistingProviderConfig): boolean {
  ...
  return Boolean(entry.baseUrl?.trim() && entry.azaicoderKey);
}
...
export function mergeWithExistingProviderSecrets(params: {
  ...
  for (const [key, newEntry] of Object.entries(nextProviders)) {
    ...
    if (
      shouldPreserveExistingAzaicoderKey({
        providerKey: key,
        existing,
        nextEntry: newEntry,
      ...
      preserved.azaicoderKey = existing.azaicoderKey;
    }
    if (
