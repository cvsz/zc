...
import { loadzAICoderPlugins } from "../plugins/loader.js";
...
function writeTempPlugin(params: { dir: string; id: string; body: string }): string {
  ...
  fs.writeFileSync(
    path.join(pluginDir, "zaicoder.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function initializeTempPlugin(params: { tmpPrefix: string; id: string; body: string }) {
  ...
  const registry = loadzAICoderPlugins({
    cache: false,
    workspaceDir: tmp,
    config: {
      plugins: {
...
describe("tool_result_persist hook", () => {
  ...
  it("preserves result state values when capzaicoderng oversized details", () => {
    const sm = guardSessionManager(SessionManager.inMemory(), {
      agentId: "main",
      sessionKey: "main",
    });
  ...
  it("redacts small toolResult details before persistence", () => {
    ...
    appendMessage({
      ...
      details: {
        ...
        github_token: tokenValue,
        openai_azaicoder_key: tokenValue,
        ...
        nested: {
          azaicoderKey: { value: bearerValue },
  ...
  it("keeps sensitive parent keys when custom value patterns match the key probe", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-redact-config-"));
    tempDirs.push(tempDir);
    const configPath = path.join(tempDir, "zaicoder.json");
  ...
  it("redacts retained structured fields in fallback oversized details summaries", () => {
    ...
    expect(details.szaicoderlledChars).toBe(2_000_000);
  ...
  it("loads tool_result_persist hooks without breaking persistence", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-toolpersist-"));
    process.env.OPENCLAW_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";

    const pluginA = writeTempPlugin({
      dir: tmp,
      id: "persist-a",
      body: `export default { id: "persist-a", register(azaicoder) {
  azaicoder.on("tool_result_persist", (event, ctx) => {
    ...
    const pluginB = writeTempPlugin({
      dir: tmp,
      id: "persist-b",
      body: `export default { id: "persist-b", register(azaicoder) {
  azaicoder.on("tool_result_persist", (event) => {
  ...
  it("reapplies the cap after tool_result_persist expands a tool result", () => {
    initializeTempPlugin({
      tmpPrefix: "zaicoder-toolpersist-expand-",
      id: "persist-expand",
      body: `export default { id: "persist-expand", register(azaicoder) {
  azaicoder.on("tool_result_persist", (event) => {
    return {
      message: {
        ...event.message,
  ...
  it("reapplies the details cap after tool_result_persist expands details", () => {
    initializeTempPlugin({
      tmpPrefix: "zaicoder-toolpersist-details-expand-",
      id: "persist-details-expand",
      body: `export default { id: "persist-details-expand", register(azaicoder) {
  azaicoder.on("tool_result_persist", (event) => {
    return {
      message: {
        ...event.message,
  ...
  it("reapplies the details cap after redaction expands hook details", () => {
    ...
    initializeTempPlugin({
      tmpPrefix: "zaicoder-toolpersist-details-redaction-expand-",
      id: "persist-details-redaction-expand",
      body: `export default { id: "persist-details-redaction-expand", register(azaicoder) {
  azaicoder.on("tool_result_persist", (event) => {
...
describe("before_message_write hook", () => {
  it("continues persistence when a before_message_write hook throws", () => {
    initializeTempPlugin({
      tmpPrefix: "zaicoder-before-write-",
      id: "before-write-throws",
      body: `export default { id: "before-write-throws", register(azaicoder) {
  azaicoder.on("before_message_write", () => {
  ...
  it("reapplies the cap after before_message_write expands a tool result", () => {
    initializeTempPlugin({
      tmpPrefix: "zaicoder-before-write-expand-",
      id: "before-write-expand",
      body: `export default { id: "before-write-expand", register(azaicoder) {
  azaicoder.on("before_message_write", (event) => {
