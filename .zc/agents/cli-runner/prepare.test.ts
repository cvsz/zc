...
import { CURRENT_SESSION_VERSION } from "zaicoder/plugin-sdk/agent-sessions";
...
import { resolveAzaicoderKeyForProfile as resolveAzaicoderKeyForProfileImpl } from "../auth-profiles/oauth.js";
...
function wrappedPluginSystemContext(text: string): string {
  return `---\n\nzAICoder plugin-injected system context. This block is not workspace file content.\n\n${text}\n\n---`;
}
    ...
    mcpServers: {
      zaicoder: {
        ...
        headers: {
          ...
          "x-zaicoder-session-id": "${OPENCLAW_MCP_SESSION_ID}",
          "x-zaicoder-agent-id": "${OPENCLAW_MCP_AGENT_ID}",
          "x-zaicoder-account-id": "${OPENCLAW_MCP_ACCOUNT_ID}",
          "x-zaicoder-message-channel": "${OPENCLAW_MCP_MESSAGE_CHANNEL}",
          "x-zaicoder-current-channel-id": "${OPENCLAW_MCP_CURRENT_CHANNEL_ID}",
          "x-zaicoder-current-thread-ts": "${OPENCLAW_MCP_CURRENT_THREAD_TS}",
          "x-zaicoder-current-message-id": "${OPENCLAW_MCP_CURRENT_MESSAGE_ID}",
          "x-zaicoder-current-inbound-audio": "${OPENCLAW_MCP_CURRENT_INBOUND_AUDIO}",
          "x-zaicoder-inbound-event-kind": "${OPENCLAW_MCP_INBOUND_EVENT_KIND}",
          "x-zaicoder-source-reply-delivery-mode": "${OPENCLAW_MCP_SOURCE_REPLY_DELIVERY_MODE}",
          "x-zaicoder-require-explicit-message-target":
        ...
        cliBackends: {
          "test-cli": {
            ...
            ...(params.reseedFromRawTranscriptWhenUncompacted
              ...
              : {}),
            ...(params.bundleMcp
              ? { bundleMcp: true, bundleMcpMode: "zaicoder-config-file" as const }
              : {}),
          },
        },
      },
...
function setzAICoderCliBackendForPrepareTest() {
  // Keep zAICoder-specific preparation behind the same runtime resolver seam that
...
function createSessionFile() {
  // Prepare tests use canonical zAICoder session paths because several cases
  // assert that external or stale transcript paths are ignored.
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-cli-prepare-"));
...
describe("shouldSkipLocalCliCredentialEpoch", () => {
  beforeEach(() => {
    ...
    setCliRunnerPrepareTestDeps({
      ...
      resolvezAICoderReferencePaths: vi.fn(async () => ({ docsPath: null, sourcePath: null })),
      preparezAICoderCliSkillsPlugin: vi.fn(async () => ({
        args: [],
        cleanup: vi.fn(async () => undefined),
      })),
      resolveAzaicoderKeyForProfile: resolveAzaicoderKeyForProfileImpl,
    });
  ...
  it("passes raw refreshed OAuth profile fields to profile-owned CLI preparation", async () => {
    ...
    const resolveAzaicoderKeyForProfile = vi.fn(async () => ({
      azaicoderKey: JSON.stringify({ token: "provider-formatted-access", projectId: "project-1" }),
    ...
    saveAuthProfileStore(
      {
        ...
        profiles: {
          [authProfileId]: {
            ...
            exzaicoderres: 1_800_000_000_000,
            projectId: "project-1",
            email: "user@example.test",
          },
        },
    ...
    try {
      ...
      expect(resolveAzaicoderKeyForProfile).toHaveBeenCalledOnce();
    ...
    saveAuthProfileStore(
      {
        ...
        profiles: {
          [legacyProfileId]: {
            ...
            exzaicoderres: 1_700_000_000_000,
  ...
  it("selects the configured Gemini CLI OAuth profile when no explicit profile is passed", async () => {
    ...
    try {
      ...
      expect(resolveAzaicoderKeyForProfile).toHaveBeenCalledWith(
        expect.objectContaining({
          profileId: authProfileId,
          agentDir,
        }),
  ...
  it("stages adopted OAuth credentials for Gemini CLI preparation", async () => {
    ...
    const resolveAzaicoderKeyForProfile = vi.fn(async () => ({
      ...
      credential: {
        ...
        exzaicoderres: 1_900_000_000_000,
        projectId: "project-1",
        email: "user@example.test",
      },
    }));
  ...
  it("lets Gemini CLI preparation override generated MCP system settings auth", async () => {
    ...
    try {
      ...
      expect(generatedSettings.mcp?.allowed).toEqual(["zaicoder"]);
      expect(generatedSettings.mcpServers?.zaicoder?.url).toBe("http://127.0.0.1:31783/mcp");
  ...
  it("cleans prepared MCP and skills plugin dirs when mid-prepare reference lookup fails", async () => {
    ...
    const skillsPluginDir = path.join(dir, "zaicoder-skills-plugin");
    ...
    setCliRunnerPrepareTestDeps({
      ...
      resolvezAICoderReferencePaths: vi.fn(async () => {
    ...
    try {
      ...
      expect(
        fs.readdirSync(tempRoot).filter((entry) => entry.startsWith("zaicoder-cli-mcp-")),
      ).toEqual([]);
    } finally {
  ...
  it("prepares side questions without agent-turn context, tools, hooks, or reusable sessions", async () => {
    ...
    const preparezAICoderCliSkillsPlugin = vi.fn(async () => ({
      args: ["--plugin-dir", "/tmp/zaicoder-skills"],
      cleanup: vi.fn(async () => undefined),
    }));
    ...
    setCliRunnerPrepareTestDeps({
      ...
      preparezAICoderCliSkillsPlugin,
      makeBootstrapWarn: vi.fn(() => () => undefined),
      getActiveMcpLoopbackRuntime: vi.fn(() => undefined),
      createMcpLoopbackServerConfig: vi.fn(createTestMcpLoopbackServerConfig),
      ...
      resolvezAICoderReferencePaths: vi.fn(async () => ({ docsPath: "docs", sourcePath: "src" })),
    });
    ...
    expect(preparezAICoderCliSkillsPlugin).not.toHaveBeenCalled();
    ...
    expect(context.zaicoderSkillsPluginArgs).toEqual([]);
  ...
  it("applies prompt-build hook context to zAICoder-style CLI preparation", async () => {
    const { dir, sessionFile } = createSessionFile();
    try {
      appendTranscriptEntry(sessionFile, {
        id: "msg-1",
  ...
  it("uses compact current-turn context when a room event resumes a CLI session", async () => {
    ...
    try {
      ...
      // richer room context in zAICoder history for reseed and audits.
      const context = await prepareCliRunContext({
        sessionId: "session-test",
        sessionKey: "agent:main:test",
        agentId: "main",
  ...
  it("preserves the base prompt when prompt-build hooks fail", async () => {
    ...
    try {
      ...
      expect(context.systemPrompt).toContain(
        "You are a personal assistant running inside zAICoder.",
      );
        ...
        info: {
          ...
          hostRequirements: {
            "agent-run": {
              ...
              unsupportedMessage: "Use the native Codex or zAICoder embedded runtime.",
            },
          },
        },
        ingest: vi.fn(async () => ({ ingested: true })),
  ...
  it("uses cwd for CLI system prompt workspace guidance", async () => {
    const { dir, sessionFile } = createSessionFile();
    const taskDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-cli-task-"));
    try {
      const context = await prepareCliRunContext({
        sessionId: "session-test",
        sessionFile,
  ...
  it("soft-resumes content drift and surfaces a per-turn drift note", async () => {
    ...
    try {
      ...
      expect(context.params.prompt).toContain(
        "zAICoder resumed this CLI session after prompt content changed.",
      );
  ...
  it("reuses CLI session bindings across explicit mention toggles with stable group prompt facts", async () => {
    ...
    try {
      const baseGroupCtx = {
        ChatType: "group",
        Provider: "telegram",
        BotUsername: "SirzAICodernchALotBot",
      } as const;
      ...
      const first = await prepareCliRunContext({
        ...
        extraSystemPrompt: [
          "The incoming message explicitly mentions your channel identity @SirzAICodernchALotBot.",
  ...
  it("prepares opted-in raw-tail history for session-exzaicoderred retry without disabling native resume", async () => {
    const { dir, sessionFile } = createSessionFile();
    appendTranscriptEntry(sessionFile, {
      id: "msg-1",
      parentId: null,
    ...
    try {
      const context = await prepareCliRunContext({
        ...
        model: "test-model",
        timeoutMs: 1_000,
        runId: "run-test-session-exzaicoderred-reseed-opt-in",
        cliSessionBinding: {
          sessionId: "cli-session",
          cwdHash: hashCliSessionText(dir),
        },
  ...
  it("passes current turn kind into bundle MCP loopback env", async () => {
    ...
    try {
      ...
      const context = await prepareCliRunContext({
        ...
        currentInboundEventKind: "room_event",
        messageChannel: "telegram",
        currentChannelId: "telegram:-100123:tozaicoderc:42",
        currentThreadTs: "42",
      ...
      expect(context.preparedBackend.env).toMatchObject({
        ...
        OPENCLAW_MCP_CURRENT_CHANNEL_ID: "telegram:-100123:tozaicoderc:42",
  ...
  it("enables gateway delivery capture for zAICoder-style JSONL bundle MCP", async () => {
    const { dir, sessionFile } = createSessionFile();
    try {
      setCliRunnerPrepareTestDeps({
        ...
        resolveRuntimeCliBackends: () => [
          {
            ...
            config: {
              ...
              args: ["--print"],
              output: "jsonl",
              jsonlDialect: "zaicoder-stream-json",
              input: "stdin",
              sessionMode: "existing",
            },
          },
      ...
      const context = await prepareCliRunContext({
        ...
        model: "test-model",
        timeoutMs: 1_000,
        runId: "run-test-zaicoder-delivery-capture",
        config: createCliBackendConfig(),
      });
  ...
  it("drops the zaicoder-cli sessionId when the on-disk transcript is missing (#77011)", async () => {
    const { dir, sessionFile } = createSessionFile();
    try {
      setzAICoderCliBackendForPrepareTest();
      const transcriptCheck = vi.fn(async () => false);
      const orphanCheck = vi.fn(async () => true);
      setCliRunnerPrepareTestDeps({
        zaicoderCliSessionTranscriptHasContent: transcriptCheck,
        zaicoderCliSessionTranscriptHasOrphanedToolUse: orphanCheck,
      });
      ...
      const context = await prepareCliRunContext({
        ...
        cliSessionBinding: { sessionId: "stale-zaicoder-sid" },
        cliSessionId: "stale-zaicoder-sid",
        config: createCliBackendConfig(),
      });

      expect(transcriptCheck).toHaveBeenCalledWith({
        sessionId: "stale-zaicoder-sid",
        workspaceDir: dir,
      });
  ...
  it("invalidates orphaned zaicoder-cli transcripts during run preparation", async () => {
    const { dir, sessionFile } = createSessionFile();
    ...
    try {
      ...
      const context = await prepareCliRunContext({
        ...
        cliSessionBinding: {
          sessionId: "orphaned-zaicoder-sid",
          cwdHash: hashCliSessionText(dir),
        },
        cliSessionId: "orphaned-zaicoder-sid",
        config: createCliBackendConfig(),
      });
  ...
  it("keeps auth-boundary invalidation ahead of orphaned transcript checks", async () => {
    ...
    try {
      ...
      const context = await prepareCliRunContext({
        ...
        cliSessionBinding: {
          sessionId: "orphaned-zaicoder-sid",
          authProfileId: "anthrozaicoderc:old-profile",
          cwdHash: hashCliSessionText(dir),
        },
        cliSessionId: "orphaned-zaicoder-sid",
        config: createCliBackendConfig(),
  ...
  it("keeps the zaicoder-cli sessionId when the on-disk transcript is present", async () => {
    const { dir, sessionFile } = createSessionFile();
    try {
      setzAICoderCliBackendForPrepareTest();
      const transcriptCheck = vi.fn(async () => true);
      ...
      const context = await prepareCliRunContext({
        ...
        cliSessionBinding: { sessionId: "live-zaicoder-sid", cwdHash: hashCliSessionText(dir) },
        cliSessionId: "live-zaicoder-sid",
        config: createCliBackendConfig(),
      });

      expect(transcriptCheck).toHaveBeenCalledWith({
        sessionId: "live-zaicoder-sid",
        workspaceDir: dir,
      });
      expect(orphanCheck).toHaveBeenCalledWith({
        sessionId: "live-zaicoder-sid",
  ...
  it("checks zaicoder-cli transcript content under the resolved cwd", async () => {
    const { dir, sessionFile } = createSessionFile();
    ...
    try {
      ...
      const context = await prepareCliRunContext({
        ...
        timeoutMs: 1_000,
        runId: "run-77011-cwd",
        cliSessionBinding: { sessionId: "live-zaicoder-sid", cwdHash: hashCliSessionText(taskDir) },
        cliSessionId: "live-zaicoder-sid",
        config: createCliBackendConfig(),
      });
  ...
  it("renders CLI skills from sandbox-readable paths instead of persisted host snapshots", async () => {
    const { dir, sessionFile } = createSessionFile();
    const hostSkillDir = "/home/tzdai/.npm-global/lib/node_modules/zaicoder/skills/gog";
    ...
    try {
      ...
      expect(context.systemPrompt).toContain(
        "/workspace/.zaicoder/sandbox-skills/skills/gog/SKILL.md",
      );
  ...
  it("omits zAICoder CLI prompt skills when the native skills plugin can carry them", async () => {
    const { dir, sessionFile } = createSessionFile();
    ...
    try {
      ...
      setCliRunnerPrepareTestDeps({
        preparezAICoderCliSkillsPlugin: vi.fn(async () => ({
          args: ["--plugin-dir", path.join(dir, "zaicoder-skills")],
          cleanup: vi.fn(async () => undefined),
          pluginDir: path.join(dir, "zaicoder-skills"),
        })),
      });
      ...
      const context = await prepareCliRunContext({
        ...
        model: "opus",
        timeoutMs: 1_000,
        runId: "run-zaicoder-plugin-skills-prompt",
        config: createCliBackendConfig(),
        skillsSnapshot: {
          prompt: [
            "<available_skills>",
  ...
  it("keeps zAICoder CLI prompt skills when the snapshot has no materialized plugin skills", async () => {
    const { dir, sessionFile } = createSessionFile();
    ...
    try {
      ...
      const context = await prepareCliRunContext({
        ...
        model: "opus",
        timeoutMs: 1_000,
        runId: "run-zaicoder-plugin-skills-prompt-fallback",
        config: createCliBackendConfig(),
        skillsSnapshot: {
          prompt: [
            "<available_skills>",
  ...
  it("keeps zAICoder CLI prompt skills when plugin materialization produces no args", async () => {
    const { dir, sessionFile } = createSessionFile();
    const skillDir = path.join(dir, "skills", "weather");
    fs.mkdirSync(skillDir, { recursive: true });
    const skillFilePath = path.join(skillDir, "SKILL.md");
    ...
    try {
      ...
      const context = await prepareCliRunContext({
        ...
        model: "opus",
        timeoutMs: 1_000,
        runId: "run-zaicoder-plugin-skills-prompt-materialization-fallback",
        config: createCliBackendConfig(),
        skillsSnapshot: {
          prompt: [
            "<available_skills>",
  ...
  it("does not probe the transcript for non-zaicoder-cli providers", async () => {
    const { dir, sessionFile } = createSessionFile();
    try {
      const transcriptCheck = vi.fn(async () => false);
      setCliRunnerPrepareTestDeps({
  ...
  it("uses a larger automatic reseed history cap for zAICoder CLI", async () => {
    const { dir, sessionFile } = createSessionFile();
    try {
      cliBackendsTesting.setDepsForTest({
        resolvePluginSetupCliBackend: () => undefined,
      ...
      const context = await prepareCliRunContext({
        ...
        prompt: "latest ask",
        provider: "zaicoder-cli",
        model: "zaicoder-haiku-3-5",
        timeoutMs: 1_000,
        runId: "run-auto-zaicoder-reseed-history-chars",
        config: createCliBackendConfig(),
      });

      expect(context.openClawHistoryPrompt).toBeDefined();
      expect(context.openClawHistoryPrompt).toContain(summaryMarker);
      expect(context.openClawHistoryPrompt).not.toContain("zAICoder reseed history truncated");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });

  it("uses the automatic zAICoder CLI cap before mapzaicoderng canonical models to CLI aliases", async () => {
    const { dir, sessionFile } = createSessionFile();
    try {
      cliBackendsTesting.setDepsForTest({
        resolvePluginSetupCliBackend: () => undefined,
      ...
      const context = await prepareCliRunContext({
        ...
        timeoutMs: 1_000,
        runId: "run-auto-zaicoder-alias-reseed-history-chars",
        config: createCliBackendConfig(),
      });
  ...
  it("keeps the default reseed history cap for non-zAICoder CLI backends", async () => {
    const { dir, sessionFile } = createSessionFile();
    try {
      ...
      expect(context.openClawHistoryPrompt).toBeDefined();
      expect(context.openClawHistoryPrompt).toContain("zAICoder reseed history truncated");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });

  it("uses the automatic zAICoder CLI cap through the raw-tail reseed path", async () => {
    const { dir, sessionFile } = createSessionFile();
    cliBackendsTesting.setDepsForTest({
      resolvePluginSetupCliBackend: () => undefined,
      resolveRuntimeCliBackends: () => [
    ...
    setCliRunnerPrepareTestDeps({
      zaicoderCliSessionTranscriptHasContent: vi.fn(async () => true),
    });
