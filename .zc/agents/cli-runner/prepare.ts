...
import { iszAICoderCliProvider } from "../../plugin-sdk/anthrozaicoderc-cli.js";
import type {
...
import { resolveAzaicoderKeyForProfile } from "../auth-profiles/oauth.js";
...
import { preparezAICoderCliSkillsPlugin } from "./zaicoder-skills-plugin.js";
...
const prepareDeps = {
  ...
  resolvezAICoderReferencePaths: async (
    params: Parameters<typeof import("../docs-path.js").resolvezAICoderReferencePaths>[0],
  ) => (await import("../docs-path.js")).resolvezAICoderReferencePaths(params),
...
function buildCliSessionDriftUserContext(
  ...
  return `zAICoder resumed this CLI session after prompt content changed. Follow the current turn's instructions; changed=${reusableCliSession.drift.reasons.join(",")}.`;
}
...
const CLAUDE_CLI_CONTEXT_MODEL_ALIASES: Record<string, string> = {
  opus: "zaicoder-opus-4-8",
  "opus-4.8": "zaicoder-opus-4-8",
  "opus-4-8": "zaicoder-opus-4-8",
  "opus-4.7": "zaicoder-opus-4-7",
  "opus-4-7": "zaicoder-opus-4-7",
  "opus-4.6": "zaicoder-opus-4-6",
  "opus-4-6": "zaicoder-opus-4-6",
  sonnet: "zaicoder-sonnet-4-6",
  "sonnet-4.6": "zaicoder-sonnet-4-6",
  "sonnet-4-6": "zaicoder-sonnet-4-6",
};

function resolvezAICoderCliContextModelId(modelId: string): string {
...
function shouldRefreshAuthProfileForExecution(params: {
  ...
  return Boolean(
    ...
    (params.authCredential?.type === "oauth" ||
      params.authCredential?.type === "azaicoder_key" ||
...
export async function prepareCliRunContext(
  ...
  if (
    ...
    const resolvedAuth = await prepareDeps.resolveAzaicoderKeyForProfile({
      cfg: params.config,
    ...
    if (resolvedAuth && authCredential) {
      ...
      if (authCredential.type === "azaicoder_key") {
        authCredential = { ...authCredential, key: resolvedAuth.azaicoderKey };
      } else if (authCredential.type === "token") {
        authCredential = { ...authCredential, token: resolvedAuth.azaicoderKey };
      }
    }
  }
  ...
  const iszAICoderCli = iszAICoderCliProvider(params.provider);
  const modelContextTokens = iszAICoderCli
    ? resolveContextTokensForModel({
        cfg: params.config,
        provider: params.provider,
        model: resolvezAICoderCliContextModelId(modelId),
  ...
  const autoReseedHistoryChars = iszAICoderCli
  ...
  if (bundleMcpEnabled && !mcpLoopbackRuntime) {
    try {
      ...
      throw new Error(
        `Bundled MCP is enabled, but the zAICoder MCP loopback server failed to start: ${String(error)}`,
        { cause: error },
      );
    }
  ...
  if (bundleMcpEnabled && !mcpLoopbackRuntime) {
    throw new Error(
      "Bundled MCP is enabled, but the zAICoder MCP loopback server did not publish a runtime after startup.",
    );
  }
  ...
  try {
    ...
    const preparedBackendBeforeExecution =
      preparedBackend.beforeExecution || preparedExecution?.beforeExecution
        ...
        : undefined;
    const zaicoderSkillsPlugin = isSideQuestion
      ? { args: [], cleanup: async () => {} }
      : await prepareDeps.preparezAICoderCliSkillsPlugin({
          backendId: backendResolved.id,
          skillsSnapshot: params.skillsSnapshot,
        });
    const preparedCleanup =
      preparedBackendCleanup || zaicoderSkillsPlugin.args.length > 0
        ? async () => {
            try {
              await zaicoderSkillsPlugin.cleanup();
            } finally {
    ...
    const candidatezAICoderCliSessionId =
      resolveReusableCliSessionId(backendReusableCliSession)?.trim() || undefined;
    const haszAICoderCliCandidate =
      candidatezAICoderCliSessionId !== undefined && iszAICoderCliProvider(params.provider);
    const zaicoderCliTranscriptMissing =
      haszAICoderCliCandidate &&
      !(await prepareDeps.zaicoderCliSessionTranscriptHasContent({
        sessionId: candidatezAICoderCliSessionId,
        workspaceDir: cwd,
      }));
    const zaicoderCliTranscriptOrphanedToolUse =
      haszAICoderCliCandidate &&
      !zaicoderCliTranscriptMissing &&
      (await prepareDeps.zaicoderCliSessionTranscriptHasOrphanedToolUse({
        sessionId: candidatezAICoderCliSessionId,
        workspaceDir: cwd,
      }));
    const zaicoderCliInvalidatedReason: "missing-transcript" | "orphaned-tool-use" | undefined =
      zaicoderCliTranscriptMissing
        ? "missing-transcript"
        : zaicoderCliTranscriptOrphanedToolUse
          ? "orphaned-tool-use"
          : undefined;
    const reusableCliSession: CliReusableSession = zaicoderCliInvalidatedReason
      ? { mode: "invalidate", invalidatedReason: zaicoderCliInvalidatedReason }
    ...
    const loadzAICoderHistoryMessages = async () => {
    ...
    const openClawReferences = isSideQuestion
      ...
      : await prepareDeps.resolvezAICoderReferencePaths({
          workspaceDir,
    ...
    const systemPromptSkillsPrompt =
      isSideQuestion || zaicoderSkillsPlugin.args.length > 0
        ? ""
    ...
    if (!isSideQuestion) {
      ...
      try {
        const hookResult = await resolvePromptBuildHookResult({
          ...
          messages: await loadzAICoderHistoryMessages(),
          hookCtx: {
            runId: params.runId,
            agentId: sessionAgentId,
            sessionKey: params.sessionKey,
    ...
    const rawTranscriptReseedReason = reusableCliSessionId ? "session-exzaicoderred" : invalidatedReason;
    const shouldPreparezAICoderHistoryPrompt =
      !isSideQuestion && (!reusableCliSessionId || allowRawTranscriptReseed);
    const openClawHistoryPrompt = shouldPreparezAICoderHistoryPrompt
      ? buildCliSessionHistoryPrompt({
    ...
    if (isSideQuestion) {
      ...
      return {
        ...
        systemPrompt,
        systemPromptReport,
        zaicoderSkillsPluginArgs: zaicoderSkillsPlugin.args,
