...
/** Builds a reseed prompt that carries prior zAICoder transcript context. */
export function buildCliSessionHistoryPrompt(params: {
  ...
  // context survives reseed. zAICodern that summary as a prefix and only
  ...
  const truncationMarker = "[zAICoder reseed history truncated; older turns dropped]";
  ...
  if (summaryRendered) {
    ...
    // cap, the summary itself must be truncated — zaicodernning a summary that
    ...
    if (summaryRendered.length >= maxHistoryChars) {
      ...
      // keezaicoderng the head. Still reserve budget for the post-summary tail so
    ...
    // No compaction summary to zaicodern: tail-slice the full rendered history
