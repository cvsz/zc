/** Tests projecting zAICoder user MCP servers into Codex app-server config. */
...
describe("buildCodexUserMcpServersThreadConfigPatch", () => {
  it("returns undefined when cfg has no mcp.servers (regression: #80814)", () => {
    ...
    expect(buildCodexUserMcpServersThreadConfigPatch({} as zAICoderConfig)).toBeUndefined();
    expect(
      buildCodexUserMcpServersThreadConfigPatch({ mcp: {} } as zAICoderConfig),
    ).toBeUndefined();
    expect(
      buildCodexUserMcpServersThreadConfigPatch({ mcp: { servers: {} } } as zAICoderConfig),
    ).toBeUndefined();
  });
  ...
  it("filters Codex-scoped user MCP servers by zAICoder agent id", () => {
    // Agent-scoped MCP servers should follow the active zAICoder agent, while
  ...
  it("omits scoped Codex MCP servers when no zAICoder agent id is available", () => {
    const patch = buildCodexUserMcpServersThreadConfigPatch({
      mcp: {
        servers: {
          atlas: {
