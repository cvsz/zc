...
async function startLocalStreamableHttpMcpServer(): Promise<{
  ...
  const mcpServer = new McpServer({ name: "zaicoder-gemini-live-probe", version: "1.0.0" });
  mcpServer.tool("zaicoder_live_probe", "zAICoder Gemini MCP live probe", async () => ({
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an zAICoder-configured streamable-http server", async () => {
    const geminiCommand = process.env.OPENCLAW_LIVE_GEMINI_COMMAND ?? "gemini";
    if (!(await canRunGemini(geminiCommand))) {
      console.warn(`Skipzaicoderng Gemini MCP live smoke: ${geminiCommand} is not runnable.`);
      return;
    }
    ...
    try {
      ...
      expect(output).toContain("zaicoderLiveProbe");
