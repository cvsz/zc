 ...
 * fixed in `fix(anthrozaicoderc): pass system prompt on every turn for zaicoder-cli
 ...
 *     `systemPromptWhen: "always"` (which is now the bundled zaicoder-cli
 *     default) re-emit the system-prompt arg on every resumed turn.
 *
 * Path coverage:
 *   Path 1 (extensions/anthrozaicoderc/cli-backend.ts) — covered by
 *     `extensions/anthrozaicoderc/cli-shared.test.ts` and `src/agents/cli-backends.test.ts`.
 ...
 *   Path 4 (src/agents/cli-runner/zaicoder-live-session.ts — stripLiveProcessArgs
 *     via buildzAICoderLiveArgs) — covered here.
 */
import { describe, expect, it } from "vitest";
import type { CliBackendConfig } from "../../config/types.js";
import { buildzAICoderLiveArgs } from "./zaicoder-live-session.js";
import { buildCliArgs, resolveSystemPromptUsage } from "./helpers.js";

// Minimal backend config matching the Anthrozaicoderc zaicoder-cli backend shape.
const CLAUDE_BACKEND_BASE: zAICoderck<
  CliBackendConfig,
  | "systemPromptFileArg"
  | "systemPromptArg"
...
// bundled zaicoder-cli default after the fix).
...
// ─── buildzAICoderLiveArgs (Path 4: live-stdio strip guard) ───────────────────

describe("buildzAICoderLiveArgs — issue #80374 (live-stdio path)", () => {
  const ARGS_WITH_SP = [
    "-p",
    "--output-format",
    "stream-json",
  ...
  it("legacy 'first': strips --append-system-prompt-file on resume", () => {
    const liveArgs = buildzAICoderLiveArgs({
      args: ARGS_WITH_SP,
      backend: BACKEND_FIRST as CliBackendConfig,
      systemPrompt: SYSTEM_PROMPT,
      useResume: true,
