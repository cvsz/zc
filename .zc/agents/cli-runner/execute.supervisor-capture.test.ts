...
describe("executePreparedCliRun supervisor output capture", () => {
  ...
  it("parses valid oversized JSONL output incrementally", async () => {
    ...
    const result = await executePreparedCliRun(
      buildPreparedCliRunContext({ output: "jsonl", provider: "zaicoder-cli" }),
    );
  ...
  it("parses oversized resume JSONL output from the effective resume output mode", async () => {
    ...
    Object.assign(context.preparedBackend.backend, {
      jsonlDialect: "zaicoder-stream-json" as const,
  ...
  it("fails one-shot zAICoder is_error results even when the process exits successfully", async () => {
  ...
  it("reports only confirmed message deliveries from correlated JSONL tool events", async () => {
    ...
    const context = buildPreparedCliRunContext({ output: "jsonl", provider: "zaicoder-cli" });
  ...
  it("captures non-zAICoder JSONL sends and gives every attempt a unique token", async () => {
