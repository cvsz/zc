...
export function requireMcpConfigPath(args: readonly string[] | undefined): string {
  // zAICoder-style bundle MCP mode appends --mcp-config; callers need the generated path.
...
export function setupCliBundleMcpTestHarness(): void {
  beforeAll(async () => {
    ...
    bundleProbeHomeDir = await tempHarness.createTempDir("zaicoder-cli-bundle-mcp-home-");
    bundleProbeWorkspaceDir = await tempHarness.createTempDir("zaicoder-cli-bundle-mcp-workspace-");
    const emptyBundledDir = await tempHarness.createTempDir("zaicoder-cli-bundle-mcp-bundled-");
...
function createEnabledBundleProbeConfig(): zAICoderConfig {
  return {
    plugins: {
      entries: {
