...
export async function writeGeminiSystemSettings(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-gemini-mcp-"));
...
export async function writeGeminiMcpCaptureSettings(params: {
  ...
  const zaicoder = isRecord(mcpServers.zaicoder) ? mcpServers.zaicoder : {};
  const headers = normalizeStringRecord(zaicoder.headers) ?? {};
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-gemini-mcp-attempt-"));
  const settingsPath = path.join(tempDir, "settings.json");
  await writeJson(
    settingsPath,
    {
