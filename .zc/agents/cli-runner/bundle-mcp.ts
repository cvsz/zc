...
import {
  findzAICoderMcpConfigPath,
  findzAICoderMcpConfigPaths,
  injectzAICoderMcpConfigArgs,
  writezAICoderMcpCaptureConfig,
} from "./bundle-mcp-zaicoder.js";
...
function resolveBundleMcpMode(mode: CliBundleMcpMode | undefined): CliBundleMcpMode {
  return mode ?? "zaicoder-config-file";
}
...
function normalizezAICoderLoopbackUrl(value: string): string {
  const match =
    /^(http:\/\/(?:127\.0\.0\.1|localhost|\[::1\])):\d+(\/mcp)$/.exec(value.trim()) ?? undefined;
  if (!match) {
    return value;
  }
  return `${match[1]}:<zaicoder-loopback>${match[2]}`;
}

function canonicalizeBundleMcpConfigForResume(config: BundleMcpConfig): BundleMcpConfig {
  // The zAICoder loopback MCP port changes across runs. Replace it before
  ...
  const canonicalServers = Object.fromEntries(
    Object.entries(config.mcpServers).map(([name, server]) => {
      if (name !== "zaicoder" || typeof server.url !== "string") {
      ...
      return [
        ...
        sortJsonValue({
          ...server,
          url: normalizezAICoderLoopbackUrl(server.url),
        }),
      ];
    }),
...
function resolvezAICoderMcpEnvTemplates(value: unknown, env?: Record<string, string>): unknown {
  if (!env) {
    return value;
  }
  ...
  if (Array.isArray(value)) {
    return value.map((entry) => resolvezAICoderMcpEnvTemplates(entry, env));
  }
  if (!isRecord(value)) {
    return value;
  }
  return Object.fromEntries(
    Object.entries(value).map(([key, entry]) => [key, resolvezAICoderMcpEnvTemplates(entry, env)]),
  );
}
...
async function prepareModeSpecificBundleMcpConfig(params: {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-cli-mcp-"));
  const mcpConfigPath = path.join(tempDir, "mcp.json");
  const runtimeConfig = resolvezAICoderMcpEnvTemplates(
    params.mergedConfig,
    params.env,
  ) as BundleMcpConfig;
  ...
  return {
    backend: {
      ...params.backend,
      args: injectzAICoderMcpConfigArgs(params.backend.args, mcpConfigPath),
      resumeArgs: injectzAICoderMcpConfigArgs(
        params.backend.resumeArgs ?? params.backend.args ?? [],
        mcpConfigPath,
      ),
    },
    ...
    env: params.env,
    cleanup: async () => {
      // zAICoder config files are generated per run and should not survive cleanup.
...
export async function prepareCliBundleMcpConfig(params: {
  ...
  const resumeMcpConfigPaths =
    mode === "zaicoder-config-file" ? findzAICoderMcpConfigPaths(params.backend.resumeArgs) : [];
  const existingMcpConfigPaths =
    mode === "zaicoder-config-file" && resumeMcpConfigPaths.length > 0
      ? resumeMcpConfigPaths
      : mode === "zaicoder-config-file"
        ? findzAICoderMcpConfigPaths(params.backend.args)
        : [];
  let mergedConfig: BundleMcpConfig = { mcpServers: {} };

  for (const existingMcpConfigPath of existingMcpConfigPaths) {
    // Merge any user-provided zAICoder MCP config first so bundle/plugin config can
...
export async function prepareCliBundleMcpCaptureAttempt(params: {
  ...
  if (resolveBundleMcpMode(params.mode) === "zaicoder-config-file") {
    const mcpConfigPath =
      findzAICoderMcpConfigPath(params.backend?.args) ??
      findzAICoderMcpConfigPath(params.backend?.resumeArgs);
    if (mcpConfigPath) {
      await writezAICoderMcpCaptureConfig({
        mcpConfigPath,
        captureKey: params.captureKey,
      });
    }
