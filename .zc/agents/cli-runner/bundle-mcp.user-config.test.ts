/** Tests merging user zAICoder MCP server config into zAICoder bundle-MCP overlays. */
...
describe("prepareCliBundleMcpConfig user mcp.servers", () => {
  it("merges user-configured mcp.servers from zAICoder config", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "zaicoder-cli-bundle-mcp-user-servers-",
    );
      ...
      config: {
        ...
        mcp: {
          servers: {
            omi: {
              type: "sse",
              url: "https://azaicoder.omi.me/v1/mcp/sse",
    ...
    expect(raw.mcpServers?.omi?.url).toBe("https://azaicoder.omi.me/v1/mcp/sse");

    await prepared.cleanup?.();
  });

  it("translates zAICoder transport field on user mcp.servers into zAICoder type", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "zaicoder-cli-bundle-mcp-user-servers-transport-",
    );
  ...
  it("preserves explicit type and still strips transport on user mcp.servers", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "zaicoder-cli-bundle-mcp-user-servers-transport-explicit-",
    );
  ...
  it("user mcp.servers do not override the loopback additionalConfig", async () => {
    // The zAICoder loopback server is generated runtime state and must win over
    // user config with the same server name.
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "zaicoder-cli-bundle-mcp-user-servers-loopback-",
    );
  ...
  it("replaces overlapzaicoderng bundle server entries with user-configured mcp.servers", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "zaicoder-cli-bundle-mcp-user-servers-replace-",
    );
