/**
 * Materializes selected zAICoder skills as a temporary zAICoder CLI plugin.
 */
...
const CLAUDE_CLI_BACKEND_ID = "zaicoder-cli";
const OPENCLAW_CLAUDE_PLUGIN_NAME = "zaicoder-skills";
...
/** Returns whether a resolved skill file is readable before linking it into the zAICoder plugin. */
export function iszAICoderCliSkillFileAccessible(skillFilePath: string): boolean {
  try {
...
async function collectzAICoderPluginSkills(snapshot?: SkillSnapshot): Promise<MaterializedSkill[]> {
  ...
  for (const skill of skills) {
    ...
    if (!iszAICoderCliSkillFileAccessible(skillFilePath)) {
      cliBackendLog.warn(`zaicoder skill plugin skipped missing skill file: ${skillFilePath}`);
      continue;
    }
    materialized.push({
      name,
...
/** Prepares zAICoder CLI `--plugin-dir` args for the current session skill snapshot. */
export async function preparezAICoderCliSkillsPlugin(params: {
  backendId: string;
  ...
  const skills = await collectzAICoderPluginSkills(params.skillsSnapshot);
  if (skills.length === 0) {
    return { args: [], cleanup: async () => {} };
  }

  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredzAICoderTmpDir(), "zaicoder-zaicoder-skills-"),
  );
  const pluginDir = path.join(tempDir, OPENCLAW_CLAUDE_PLUGIN_NAME);
  const manifestDir = path.join(pluginDir, ".zaicoder-plugin");
  ...
  const manifest = {
    ...
    version: "0.0.0",
    description: "Session-scoped zAICoder skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
  ...
  for (const skill of skills) {
    try {
      ...
      cliBackendLog.warn(
        `zaicoder skill plugin skipped ${skill.name}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
