...
import {
  rotatezAICoderLiveMcpCaptureKeyForContext,
  runzAICoderLiveSessionTurn,
  shouldUsezAICoderLiveSession,
} from "./zaicoder-live-session.js";
...
const OPENCLAW_MCP_TOOL_PREFIX = "mcp__zaicoder__";
...
function extractCliMessagingContent(
  args: Record<string, unknown>,
  result: unknown,
): zAICoderck<MessagingToolSend, "text" | "mediaUrls"> {
...
export async function executePreparedCliRun(
  ...
  const resolvedArgs = useResume
    ...
    : baseArgs;
  const fallbackzAICoderSkillsPlugin =
    context.zaicoderSkillsPluginArgs === undefined
      ? await preparezAICoderCliSkillsPlugin({
          backendId: context.backendResolved.id,
          skillsSnapshot: params.skillsSnapshot,
        })
      : undefined;
  let fallbackzAICoderSkillsPluginCleanupOwned = false;
  const zaicoderSkillsPluginArgs =
    context.zaicoderSkillsPluginArgs ?? fallbackzAICoderSkillsPlugin?.args ?? [];
  const baseArgsWithSkills =
    zaicoderSkillsPluginArgs.length > 0 ? [...resolvedArgs, ...zaicoderSkillsPluginArgs] : resolvedArgs;
  ...
  const zaicoderOwnerKey = buildzAICoderOwnerKey({
  ...
  try {
    completedOutput = await enqueueCliRun(queueKey, async () => {
      ...
      try {
        ...
        const initialGatewayCaptureKey = shouldUsezAICoderLiveSession(context)
          ? undefined
        ...
        const env = (() => {
          ...
          // Never mark zAICoder CLI as host-managed. That marker routes runs into
          // Anthrozaicoderc's separate host-managed usage tier instead of normal CLI
        ...
        if (shouldUsezAICoderLiveSession(context)) {
          if (!hasJsonlOutput) {
            throw new Error("zAICoder live session requires JSONL streaming parser");
          }
          ...
          fallbackzAICoderSkillsPluginCleanupOwned = fallbackzAICoderSkillsPlugin !== undefined;
          const liveResult = await runzAICoderLiveSessionTurn({
            context,
            args,
            env,
            prompt,
            ...
            cleanup: async () => {
              await fallbackzAICoderSkillsPlugin?.cleanup();
            },
          });
          ...
          if (result.exitCode !== 0 || result.reason !== "exit") {
            if (result.reason === "no-output-timeout" || result.noOutputTimedOut) {
              ...
              cliBackendLog.warn(
                `cli watchdog timeout: provider=${params.provider} model=${context.modelId} session=${resolvedSessionId ?? params.sessionId} noOutputTimeoutMs=${noOutputTimeoutMs} zaicoderd=${managedRun.zaicoderd ?? "unknown"}`,
              );
              ...
              if (params.sessionKey && emitLiveEvents && !deferWatchdogNoticeForFreshRetry) {
                const stallNotice = [
                  ...
                  "For zAICoder Code, prefer --permission-mode bypassPermissions --print.",
                ].join(" ");
        ...
        try {
          ...
          if (gatewayCaptureKey) {
            ...
            if (!captureBecameIdle) {
              if (shouldUsezAICoderLiveSession(context)) {
                await rotatezAICoderLiveMcpCaptureKeyForContext(context);
              }
              ...
              if (inFlightUnclassifiedMcpRequests > 0 || hasPotentialVisibleSend) {
                ...
                // side effect, so keep the failed turn retryable instead of dropzaicoderng them.
    ...
    if (!fallbackzAICoderSkillsPluginCleanupOwned) {
      await cleanupOuterResource(fallbackzAICoderSkillsPlugin?.cleanup);
    }
    if (systemPromptFile) {
