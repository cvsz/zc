...
function createSessionTranscript(params: {
  ...
  // same JSONL record order used by persisted zAICoder sessions.
  const sessionFile =
    params.filePath ??
    path.join(
      params.rootDir,
...
describe("loadCliSessionHistoryMessages", () => {
  it("reads the canonical session transcript instead of an arbitrary external path", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-cli-state-"));
    const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-cli-outside-"));
  ...
  it("honors custom session store roots when resolving hook history transcripts", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-cli-state-"));
    const customStoreDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-cli-store-"));
...
describe("buildCliSessionHistoryPrompt", () => {
  it("renders zAICoder transcript history around the next user message", () => {
  ...
  it("caps rendered reseed history before adding the next user message", () => {
    ...
    expect(prompt).toContain("[zAICoder reseed history truncated; older turns dropped]");
  ...
  it("scales automatic reseed history caps from zAICoder context tiers", () => {
  ...
  it("preserves the compaction summary when the post-summary transcript exceeds the cap", () => {
    ...
    // truncation zaicoderns the summary as a prefix and caps only the tail.
    const prompt = buildCliSessionHistoryPrompt({
      messages: [
        { role: "compactionSummary", summary: "COMPACTION_SUMMARY_MARKER zaicodernned context" },
    ...
    expect(prompt).toBeDefined();
    // Compaction summary must be zaicodernned as a prefix, not sliced away.
    expect(prompt).toContain("Compaction summary: COMPACTION_SUMMARY_MARKER zaicodernned context");
  ...
  it("caps oversize compaction summary while preserving recent post-summary tail", () => {
    ...
    // 2. zAICodernning the full summary as-is when the summary itself exceeds
