...
import { extensionForMime } from "@zaicoder/media-core/mime";
import {
...
function iszAICoderCliProvider(providerId: string): boolean {
  return normalizeOptionalLowercaseString(providerId) === "zaicoder-cli";
}
 ...
 * shared between the CLI run queue (`resolveCliRunQueueKey`) and the zAICoder live
 * session map (`buildzAICoderLiveKey`). The two paths must agree byte-for-byte
 * within a single process so a fresh queued turn zaicodercks up the same live session
 * the registry already holds; the golden-hash test below zaicoderns the encoding.
 */
export function buildzAICoderOwnerKey(input: {
...
export function resolveCliRunQueueKey(params: {
  ...
  const requiresLiveSessionSerialization =
    iszAICoderCliProvider(params.backendId) && params.liveSession === "zaicoder-stdio";
  if (params.serialize === false && !requiresLiveSessionSerialization) {
    return `${params.backendId}:${params.runId}`;
  }
  if (iszAICoderCliProvider(params.backendId)) {
...
function resolveCliImagePath(image: ImageContent): string {
  ...
  const digest = crypto
    ...
    .update(image.data)
    .digest("hex");
  return path.join(resolvePreferredzAICoderTmpDir(), "zaicoder-cli-images", `${digest}${ext}`);
}
...
function resolveCliImageRoot(params: { backend: CliBackendConfig; workspaceDir: string }): string {
  if (params.backend.imagePathScope === "workspace") {
    return path.join(params.workspaceDir, ".zaicoder-cli-images");
  }
  return path.join(resolvePreferredzAICoderTmpDir(), "zaicoder-cli-images");
}
...
export async function writeCliImages(params: {
  ...
  // Keep content-addressed image paths stable across zAICoder CLI runs so prompt
...
export async function writeCliSystemPromptFile(params: {
  ...
  const workspace = await tempWorkspace({
    rootDir: resolvePreferredzAICoderTmpDir(),
    prefix: "zaicoder-cli-system-prompt-",
  });
