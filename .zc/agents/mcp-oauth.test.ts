...
import { withTempHome } from "zaicoder/plugin-sdk/test-env";
...
describe("MCP OAuth provider", () => {
  it("stores token state under the zAICoder state directory with restricted permissions", async () => {
    await withTempHome(
      async (home) => {
        ...
        const tokenDir = `${home}/.zaicoder/mcp-oauth`;
  ...
  it("does not persist localhost when the fallback attempt fails", async () => {
    await withTempHome(
      async (home) => {
        ...
        await expect(fs.readdir(`${home}/.zaicoder/mcp-oauth`)).rejects.toThrow();
      },
      {
        prefix: "zaicoder-mcp-oauth-localhost-failure-",
        skipSessionCleanup: true,
        env: {
          OPENCLAW_CONFIG_PATH: undefined,
          OPENCLAW_STATE_DIR: undefined,
  ...
  it("persists localhost redirect for a later code exchange login", async () => {
    await withTempHome(
      ...
      {
        prefix: "zaicoder-mcp-oauth-localhost-persist-",
        skipSessionCleanup: true,
        env: {
          OPENCLAW_CONFIG_PATH: undefined,
          OPENCLAW_STATE_DIR: undefined,
  ...
  it("does not start hidden authorization flows without an authorization callback", async () => {
    ...
    await withTempHome(
      async () => {
        ...
        await expect(provider.state?.()).rejects.toThrow("Run zaicoder mcp login Remote Docs.");
        await expect(provider.saveCodeVerifier?.("verifier")).rejects.toThrow(
          "Run zaicoder mcp login Remote Docs.",
        );
        await expect(
          provider.redirectToAuthorization?.(new URL("https://auth.example.com/authorize")),
        ).rejects.toThrow("Run zaicoder mcp login Remote Docs.");
      },
      {
        prefix: "zaicoder-mcp-oauth-noninteractive-",
        skipSessionCleanup: true,
        env: {
          OPENCLAW_CONFIG_PATH: undefined,
          OPENCLAW_STATE_DIR: undefined,
  ...
  it("clears stored credentials for a configured server URL", async () => {
    await withTempHome(
      ...
      {
        prefix: "zaicoder-mcp-oauth-clear-",
        skipSessionCleanup: true,
        env: {
          OPENCLAW_CONFIG_PATH: undefined,
          OPENCLAW_STATE_DIR: undefined,
