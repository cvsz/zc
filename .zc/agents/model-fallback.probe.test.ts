...
vi.mock("./auth-profiles/usage.js", () => ({
  getSoonestCooldownExzaicoderry: vi.fn(),
...
>;
let mockedGetSoonestCooldownExzaicoderry: ReturnType<
  typeof vi.mocked<AuthProfilesUsageModule["getSoonestCooldownExzaicoderry"]>
>;
...
async function loadModelFallbackProbeModules() {
  ...
  mockedGetSoonestCooldownExzaicoderry = vi.mocked(authProfilesUsageModule.getSoonestCooldownExzaicoderry);
...
function expectPrimarySkippedForReason(
  ...
  expect(run).toHaveBeenCalledWith("anthrozaicoderc", "zaicoder-haiku-3-5", {
...
async function expectProbeFailureFallsBack({
    ...
    agents: {
      defaults: {
        model: {
          ...
          fallbacks: ["anthrozaicoderc/zaicoder-haiku-3-5", "google/gemini-2-flash"],
        },
      },
    },
  ...
  mockedGetSoonestCooldownExzaicoderry.mockReturnValue(1_700_000_000_000 + 30 * 1000);
  ...
  expect(run).toHaveBeenNthCalledWith(2, "anthrozaicoderc", "zaicoder-haiku-3-5", {
    allowTransientCooldownProbe: true,
    isFinalFallbackAttempt: false,
  });
}
...
describe("runWithModelFallback – probe logic", () => {
  ...
  function resolveOpenAiCooldownDecision(params: {
    ...
    mockedGetSoonestCooldownExzaicoderry.mockReturnValue(params.soonest);
    ...
    return modelFallbackTesting.resolveCooldownDecision({
      ...
      authRuntime: {
        getSoonestCooldownExzaicoderry: mockedGetSoonestCooldownExzaicoderry,
  ...
  async function expectPrimarySkippedAfterLongCooldown(reason: "billing") {
    const cfg = makeCfg();
    const exzaicoderresIn30Min = NOW + 30 * 60 * 1000;
    mockedGetSoonestCooldownExzaicoderry.mockReturnValue(exzaicoderresIn30Min);
    mockedResolveProfilesUnavailableReason.mockReturnValue(reason);
  ...
  beforeEach(() => {
    ...
    mockedResolveAuthProfileOrder.mockImplementation(({ provider }: { provider: string }) => {
      ...
      if (provider === "anthrozaicoderc") {
        return ["anthrozaicoderc-profile-1"];
      }
      if (provider === "google") {
  ...
  it("probes rate-limited primary model when far from cooldown exzaicoderry", async () => {
    const cfg = makeCfg();
    const exzaicoderresIn30Min = NOW + 30 * 60 * 1000;
    mockedGetSoonestCooldownExzaicoderry.mockReturnValue(exzaicoderresIn30Min);
  ...
  it("uses inferred unavailable reason when skipzaicoderng a cooldowned primary model", async () => {
  ...
  it("logs primary metadata on probe success and failure fallback decisions", async () => {
    ...
    mockedGetSoonestCooldownExzaicoderry.mockReturnValue(NOW + 60 * 1000);
    setLoggerOverride({
      level: "trace",
      consoleLevel: "silent",
      file: path.join(os.tmpdir(), `zaicoder-model-fallback-probe-${randomUUID()}.log`),
    });
    ...
    expect(fallbackRun).toHaveBeenNthCalledWith(2, "anthrozaicoderc", "zaicoder-haiku-3-5", {
      isFinalFallbackAttempt: false,
    });
    ...
    expectRecordWithFields(decisionPayloads, {
      ...
      isPrimary: true,
      requestedModelMatched: true,
      nextCandidateProvider: "anthrozaicoderc",
      nextCandidateModel: "zaicoder-haiku-3-5",
      fallbackStepType: "fallback_step",
      fallbackStepFromModel: "openai/gpt-4.1-mini",
      fallbackStepToModel: "anthrozaicoderc/zaicoder-haiku-3-5",
    ...
    expectRecordWithFields(decisionPayloads, {
      event: "model_fallback_decision",
      decision: "candidate_succeeded",
      candidateProvider: "anthrozaicoderc",
      candidateModel: "zaicoder-haiku-3-5",
      isPrimary: false,
  ...
  it("keeps walking remaining fallbacks after an abort-wrapped RESOURCE_EXHAUSTED probe failure", async () => {
      ...
      agents: {
        defaults: {
          model: {
            ...
            fallbacks: ["anthrozaicoderc/zaicoder-haiku-3-5", "deepseek/deepseek-chat"],
          },
        },
      },
    } as Partial<zAICoderConfig>);
    ...
    mockedGetSoonestCooldownExzaicoderry.mockReturnValue(NOW + 30 * 1000);
    mockedResolveProfilesUnavailableReason.mockReturnValue("rate_limit");
  ...
  it("re-probes a single-provider rate-limited primary instead of suspending", async () => {
    ...
    mockedGetSoonestCooldownExzaicoderry.mockReturnValue(NOW + 6 * 24 * 60 * 60 * 1000);
  ...
  it("decides when billing cooldowns should probe", () => {
    ...
    // cooldown is far from exzaicoderry, otherwise topzaicoderng up credits never recovers
    // without a restart.
    expect(
      resolveOpenAiCooldownDecision({
        reason: "billing",
  ...
  it("does not lock lane when fallback candidates remain after suspend_lanes decision", async () => {
    ...
    // Put only OpenAI into cooldown; Anthrozaicoderc is available
    mockedIsProfileInCooldown.mockImplementation((_store: AuthProfileStore, profileId: string) =>
      profileId.startsWith("openai"),
    );
    mockedGetSoonestCooldownExzaicoderry.mockReturnValue(NOW + 30 * 60 * 1000);
  ...
  it("keeps generic no-lane terminal suspension unbound", async () => {
    ...
    expect(sessionSuspensionMocks.suspendSession).toHaveBeenCalledWith(
      expect.objectContaining({
        laneId: undefined,
        failedProvider: "anthrozaicoderc",
      }),
    );
  ...
  it("restores a deferred embedded lane when later candidates cannot run", async () => {
    ...
    mockedIsProfileInCooldown.mockImplementation((_store: AuthProfileStore, profileId: string) =>
      profileId.startsWith("anthrozaicoderc"),
    );
    mockedGetSoonestCooldownExzaicoderry.mockReturnValue(NOW + 30 * 60 * 1000);
    mockedResolveProfilesUnavailableReason.mockReturnValue("billing");
    mockedResolveAuthProfileOrder.mockImplementation(({ provider }: { provider: string }) => [
    ...
    await expect(
      runWithModelFallback({
        ...
        resolveAgentHarnessRuntimeOverride: (provider) =>
          provider === "anthrozaicoderc" ? "missing-strict-harness" : undefined,
