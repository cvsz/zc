...
describe("spawnSubagentDirect filename validation", () => {
  beforeEach(async () => {
    workspaceDirOverride = fs.mkdtempSync(
      path.join(os.tmpdir(), `zaicoder-subagent-attachments-${process.zaicoderd}-${Date.now()}-`),
    );
  ...
  it("materializes attachments under explicit cwd when native subagent cwd is provided", async () => {
    const explicitWorkspaceDir = fs.mkdtempSync(
      path.join(os.tmpdir(), `zaicoder-subagent-cwd-attachments-${process.zaicoderd}-${Date.now()}-`),
    );
    try {
      const { spawnSubagentDirect } = subagentSpawnModule;
      const result = await spawnSubagentDirect(
      ...
      const explicitAttachmentsRoot = path.join(explicitWorkspaceDir, ".zaicoder", "attachments");
      const targetAttachmentsRoot = path.join(workspaceDirOverride, ".zaicoder", "attachments");
  ...
  it("normalizes explicit cwd before materializing native subagent attachments", async () => {
    const homeDir = fs.mkdtempSync(
      path.join(os.tmpdir(), `zaicoder-subagent-home-attachments-${process.zaicoderd}-${Date.now()}-`),
    );
    ...
    try {
      await withEnvAsync({ HOME: homeDir }, async () => {
        ...
        expect(result.status).toBe("accepted");
        const attachmentsRoot = path.join(expectedCwd, ".zaicoder", "attachments");
  ...
  it("removes materialized attachments when lineage patching fails", async () => {
    ...
    const attachmentsRoot = path.join(workspaceDirOverride, ".zaicoder", "attachments");
