...
describe("applyExtraParamsToAgent Moonshot", () => {
  ...
  it("disables thinking instead of broadening zaicodernned Moonshot tool_choice", () => {
    // A zaicodernned tool choice is stricter than thinking. Disable thinking instead
  ...
  it("drops thinking.keep on kimi-k2.6 when thinking is forced off by zaicodernned tool_choice", () => {
    const payload = runExtraParamsPayloadCase({
      provider: "moonshot",
      modelId: "kimi-k2.6",
      thinkingLevel: "low",
  ...
  it("omits thinking controls and broadens zaicodernned tool choice for kimi-k2.7-code", () => {
    const payload = runExtraParamsPayloadCase({
      provider: "moonshot",
