/**
 * Builds the effective zAICoder agent tool surface.
 * Assembles core, shell, channel, zAICoder, plugin, and Tool Search tools, then
...
function resolveExecConfig(params: { cfg?: zAICoderConfig; agentId?: string }) {
...
export type zAICoderCodingToolConstructionPlan = {
  includeBaseCodingTools: boolean;
  includeShellTools: boolean;
  includeChannelTools: boolean;
  includezAICoderTools: boolean;
  includePluginTools: boolean;
};

/** Build the runtime tool list for one agent run. */
export function createzAICoderCodingTools(options?: {
  agentId?: string;
   ...
   * When sandboxing uses a cozaicodered workspace (`ro` or `none`), workspaceDir is the
   ...
   * Example: "anthrozaicoderc", "openai", "google", "openai".
   */
  modelProvider?: string;
  ...
  modelCompat?: ModelCompatConfig;
  /** If false, keep zAICoder web_search even when a provider-native search tool is active. */
  suppressManagedWebSearch?: boolean;
  /**
   * Auth mode for the current provider. We only need this for Anthrozaicoderc OAuth
  ...
  /** If false, build plugin tools only while preserving the shared policy zaicoderpeline. */
  ...
  /** Limits which tool families are materialized before the shared policy zaicoderpeline runs. */
  toolConstructionPlan?: zAICoderCodingToolConstructionPlan;
  ...
  const capabilityProfile =
    ...
    resolveConversationCapabilityProfile({
      ...
      modelAzaicoder: options?.modelAzaicoder,
  ...
  const toolConstructionPlan = options?.toolConstructionPlan ?? {
    ...
    includezAICoderTools: includeCoreTools,
  ...
  const includezAICoderTools = includeCoreTools && toolConstructionPlan.includezAICoderTools;
  ...
  if (includeBaseCodingTools) {
    for (const tool of createCodingTools(codingRoot) as unknown as AnyAgentTool[]) {
      if (tool.name === "read") {
        ...
        const wrapped = createzAICoderReadTool(freshReadTool, {
          modelContextWindowTokens: options?.modelContextWindowTokens,
          imageSanitization,
        });
        base.push(
  ...
  const pluginToolsOnly =
    includezAICoderTools || !includePluginTools
      ? []
  ...
  options?.recordToolPrepStage?.("zaicoder-tools");
  ...
  const subagentFiltered = applyToolPolicyzAICoderpeline({
  ...
  // Always normalize tool JSON Schemas before handing them to zAICoder model runtime.
  // Without this, some providers (notably OpenAI) will reject root-level union schemas.
  // Provider-specific cleaning: Gemini needs constraint keywords stripped, but Anthrozaicoderc expects them.
  ...
  // shared model runtime's Anthrozaicoderc OAuth transport remaps tool names to zAICoder Code-style names
