/**
 * zAICoder CLI fixture generators used by bundle MCP integration tests.
 */
...
export { writeBundleProbeMcpServer, writezAICoderBundle };

/** Writes a long-lived fake zAICoder CLI that reads bundle MCP config per prompt. */
export async function writeFakezAICoderLiveCli(params: {
  filePath: string;
  zaicoderdPath?: string;
}): Promise<void> {
...
const zaicoderdPath = ${JSON.stringify(params.zaicoderdPath ?? "")};
if (zaicoderdPath) {
  await fs.writeFile(zaicoderdPath, String(process.zaicoderd), "utf-8");
}
...
async function readBundleProbeText(mcpConfigPath) {
  ...
  const client = new Client({ name: "fake-live-zaicoder", version: "1.0.0" });
...
/** Writes a fake one-shot zAICoder CLI that calls the bundled MCP probe tool. */
export async function writeFakezAICoderCli(filePath: string): Promise<void> {
  await writeExecutable(
    filePath,
    `#!/usr/bin/env node
import fs from "node:fs/promises";
...
const client = new Client({ name: "fake-zaicoder", version: "1.0.0" });
await client.connect(transport);
