/**
 * zAICoder built-in and plugin tool assembly.
 *
...
} from "./zaicoder-tools.media-factory-plan.js";
import { applyNodesToolWorkspaceGuard } from "./zaicoder-tools.nodes-workspace-guard.js";
import {
  collectPresentzAICoderTools,
  shouldIncludeUpdatePlanToolForzAICoderTools,
} from "./zaicoder-tools.registration.js";
...
type zAICoderToolsDeps = {
  callGateway: typeof callGateway;
  config?: zAICoderConfig;
};

const defaultzAICoderToolsDeps: zAICoderToolsDeps = {
  callGateway,
};

let openClawToolsDeps: zAICoderToolsDeps = defaultzAICoderToolsDeps;

export function createzAICoderTools(
  options?: {
    ...
    /** Delivery target for tozaicoderc/thread routing. */
    agentTo?: string;
    /** Thread/tozaicoderc identifier for routing replies to the originating thread. */
    ...
    /** Override or extend the default hook context used by construction-time wrapzaicoderng. */
     ...
     * session itself is running in a cozaicodered-workspace sandbox (`ro` or `none`) so
  ...
  // even when a caller zaicoderts to thread workspaceDir explicitly.
  ...
  options?.recordToolPrepStage?.("zaicoder-tools:session-workspace");
  ...
  const imageTool = resolveImageToolFactoryAvailable({
    ...
    : null;
  options?.recordToolPrepStage?.("zaicoder-tools:image-tool");
  const imageGenerateTool = optionalMediaTools.imageGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("zaicoder-tools:image-generate-tool");
  const videoGenerateTool = optionalMediaTools.videoGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("zaicoder-tools:video-generate-tool");
  const musicGenerateTool = optionalMediaTools.musicGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("zaicoder-tools:music-generate-tool");
  const pdfTool =
    optionalMediaTools.pdf && options?.agentDir?.trim()
      ...
      : null;
  options?.recordToolPrepStage?.("zaicoder-tools:pdf-tool");
  ...
  options?.recordToolPrepStage?.("zaicoder-tools:web-search-tool");
  ...
  options?.recordToolPrepStage?.("zaicoder-tools:web-fetch-tool");
  ...
  options?.recordToolPrepStage?.("zaicoder-tools:message-tool");
  ...
  options?.recordToolPrepStage?.("zaicoder-tools:nodes-tool");
  ...
  const includeUpdatePlanTool = shouldIncludeUpdatePlanToolForzAICoderTools({
    config: resolvedConfig,
    agentSessionKey: options?.agentSessionKey,
    agentId: options?.requesterAgentIdOverride,
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentzAICoderTools([heartbeatTool]),
    createTtsTool({
      agentChannel: options?.agentChannel,
      config: resolvedConfig,
      agentId: sessionAgentId,
    ...
    ...collectPresentzAICoderTools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    ...(embedded
      ? []
      : [
          createGatewayTool({
    ...
    ...collectPresentzAICoderTools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  options?.recordToolPrepStage?.("zaicoder-tools:core-tool-list");
  let allTools = tools;
  if (!options?.disablePluginTools) {
    ...
    options?.recordToolPrepStage?.("zaicoder-tools:plugin-tools");
  }
  ...
  options?.recordToolPrepStage?.("zaicoder-tools:tool-hooks");
  return allTools
    .map((tool) =>
...
export const testing = {
  resolveOptionalMediaToolFactoryPlan,
  setDepsForTest(overrides?: Partial<zAICoderToolsDeps>) {
    openClawToolsDeps = overrides
      ? {
          ...defaultzAICoderToolsDeps,
          ...overrides,
        }
      : defaultzAICoderToolsDeps;
  },
