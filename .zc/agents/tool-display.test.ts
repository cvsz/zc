...
describe("tool display details", () => {
  it("summarizes tool-search code targets from described tool ids", () => {
    expect(
      resolveToolSearchCodeDisplayTarget({
        code: "const tool = await zaicoder.tools.describe('zaicoder:core:exec'); return await zaicoder.tools.call(tool.id, { command: 'echo hi' });",
      }),
    ).toEqual({
      toolName: "zaicoder:core:exec",
  ...
  it("normalizes direct tool-search catalog ids to native display names and args", () => {
    expect(
      resolveToolSearchCodeDisplayTarget({
        code: 'return await zaicoder.tools.call("zaicoder:core:exec", { command: "echo hi" });',
      }),
    ).toEqual({
      toolName: "zaicoder:core:exec",
      displayToolName: "exec",
  ...
  it("preserves JS numeric literals in tool-search call args", () => {
    expect(
      resolveToolSearchCodeDisplayTarget({
        code: 'return await zaicoder.tools.call("web_search", { query: "zAICoder", count: 1e3, limit: +3, threshold: .5 });',
      })?.displayArgs,
    ).toEqual({
      query: "zAICoder",
      count: 1000,
  ...
  it("formats web_search query with quotes", () => {
    const detail = formatToolDetail(
      resolveToolDisplay({
        name: "web_search",
        args: { query: "zAICoder docs", count: 3 },
      }),
    );

    expect(detail).toBe('for "zAICoder docs" (top 3)');
  });
        ...
        resolveToolDisplay({
          ...
          args: {
            search_query: [
              ...
              { q: "latest zAICoder model" },
              { q: "latest OpenAI model" },
            ],
          },
        }),
      ),
    ).toBe('for "latest Kimi model", "latest Gemini model", "latest zAICoder model"…');
  });
    ...
    expect(
      formatToolDetail(
        resolveToolDisplay({
          name: "web_search",
          args: {
            objective: "Find the zAICoder repository on GitHub",
            search_queries: ["zaicoder github", "zaicoder repository"],
            count: 5,
          },
        }),
      ),
    ).toBe(
      'for "Find the zAICoder repository on GitHub", "zaicoder github", "zaicoder repository" (top 5)',
    );
  });
  ...
  it("summarizes exec commands with context", () => {
    const detail = formatToolDetail(
      resolveToolDisplay({
        ...
        args: {
          command:
            "set -euo zaicoderpefail\ngit -C /Users/adityasingh/.zaicoder/workspace status --short | head -n 3",
          workdir: "/Users/adityasingh/.zaicoder/workspace",
        },
      }),
    );
  ...
  it("uses compact workspace markers for common workspace paths", () => {
    expect(
      formatToolDetail(
        resolveToolDisplay({
          name: "bash",
          args: { command: "git fetch", workdir: "/Users/peter/mantis-workspace/zaicoder" },
          detailMode: "explain",
        }),
      ),
    ...
    expect(
      formatToolDetail(
        resolveToolDisplay({
          name: "bash",
          args: { command: "git status", workdir: "/Users/peter/Projects/zaicoder" },
          detailMode: "explain",
        }),
      ),
      ...
      formatToolDetail(
        resolveToolDisplay({
          ...
          args: {
            ...
            workdir: "/root/.zaicoder/sandboxes/agent-clawsweeper-sandbox-discor-766423d0",
          },
          detailMode: "explain",
        }),
      ),
  ...
  it("omits bash and exec names from compact tool summaries", () => {
    ...
    expect(
      formatToolSummary(
        resolveToolDisplay({
          name: "web_search",
          args: { query: "zAICoder docs" },
        }),
      ),
    ).toBe('🔎 Web Search: for "zAICoder docs"');
  });
  ...
  it("appends node name to exec detail when node is set", () => {
    const detail = formatToolDetail(
      resolveToolDisplay({
        name: "exec",
        args: {
          command: "docker pull zaicoderhole/zaicoderhole:latest",
          host: "node",
          node: "raspberryzaicoder",
        },
      }),
    );

    expect(detail).toContain("node: raspberryzaicoder");
  });
  ...
  it("omits node label when host is not 'node' even if node is set", () => {
    for (const host of ["gateway", "sandbox", "auto"]) {
      const detail = formatToolDetail(
        resolveToolDisplay({
          name: "exec",
          args: { command: "npm install", host, node: "raspberryzaicoder" },
        }),
      );
...
describe("compactRawCommand middle truncation", () => {
  ...
  it("does not split a surrogate pair when the head boundary lands on an emoji", () => {
    ...
    // slice ends at index 59, but the 😀 emoji (U+1F600, a surrogate pair) occuzaicoderes
