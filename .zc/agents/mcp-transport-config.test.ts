...
describe("resolveMcpTransportConfig", () => {
  ...
  it("drops dangerous env overrides from stdio config", () => {
    ...
    // variables and child-process config zaicodervots while preserving explicit MCP
  ...
  it("sanitizes config-controlled names in stdio env warnings", () => {
    resolveMcpTransportConfig("probe\nWARN zaicoderd\u001b[31m", {
      command: "node",
      env: {
        "LD_PRELOAD\nWARN zaicoderd\u001b[31m": "/tmp/pwn.so",
      },
    });

    expect(logWarn).toHaveBeenCalledWith(
      'bundle-mcp: server "probeWARN zaicoderd": env "LD_PRELOADWARN zaicoderd" is blocked for stdio startup safety and was ignored.',
    );
  });
