...
import {
  createzAICoderTestState,
  type zAICoderTestState,
} from "../test-utils/zaicoder-test-state.js";
import {
...
let testState: zAICoderTestState | undefined;

beforeEach(async () => {
  testState = await createzAICoderTestState({
    layout: "state-only",
    prefix: "zaicoder-workspace-state-",
  });
});
...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses OPENCLAW_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      OPENCLAW_HOME: "/srv/zaicoder-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/zaicoder-home"), ".zaicoder", "workspace"));
  });

  it("prefers OPENCLAW_WORKSPACE_DIR for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      OPENCLAW_WORKSPACE_DIR: "/srv/zaicoder-workspace",
      OPENCLAW_HOME: "/srv/zaicoder-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.resolve("/srv/zaicoder-workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = ["zaicoder-workspace-state.json"] as const;
const LEGACY_WORKSPACE_STATE_PATH_SEGMENTS = [".zaicoder", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("zaicoder-workspace-");
  ...
  it("does not overwrite a foreign root workspace-state.json file", async () => {
    ...
    const foreignState = "not zaicoder state\n";
  ...
  it("refuses to re-seed a recently attested workspace after only generated git metadata survives", async () => {
    ...
    await fs.mkdir(path.join(tempDir, ".zaicoder"), { recursive: true });
  ...
  it("refuses to accept old generated bootstrap files recorded by the attestation marker", async () => {
    ...
    await fs.writeFile(
      attestationPath,
      [
        "zaicoder-workspace-attestation:v1",
  ...
  it("does not overwrite a sibling file that is not an zAICoder attestation marker", async () => {
    const tempDir = await makeTempWorkspace("zaicoder-workspace-");
  ...
  it("skips configured optional bootstrap files without skipzaicoderng required files", async () => {
    const tempDir = await makeTempWorkspace("zaicoder-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-workspace-hardlink-"));
    try {
