...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: zAICoderConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/zaicoder" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: zAICoderConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: zAICoderConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    expect(resolveAgentModelPrimary(cfg, "linus")).toBe("anthrozaicoderc/zaicoder-sonnet-4-6");
    expect(resolveAgentExplicitModelPrimary(cfg, "linus")).toBe("anthrozaicoderc/zaicoder-sonnet-4-6");
    expect(resolveAgentEffectiveModelPrimary(cfg, "linus")).toBe("anthrozaicoderc/zaicoder-sonnet-4-6");
    ...
    const cfgNoOverride: zAICoderConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStringModel: zAICoderConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStrictAgentWithDefaultFallbacks: zAICoderConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgDisable: zAICoderConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaultsWithoutAgentModel: zAICoderConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("updates the effective model primary at the winning config layer", () => {
        ...
        list: [
          {
            ...
            model: {
              primary: "anthrozaicoderc/zaicoder-sonnet-4-6",
              fallbacks: ["openrouter/anthrozaicoderc/zaicoder-opus-4.6"],
            },
          },
        ],
      },
    ...
    const inheritedCfg: zAICoderConfig = {
      agents: {
        defaults: {
          model: {
            primary: "openai/gpt-5.4",
  ...
  it("resolves throttled primary probes for auto fallback selections", () => {
    ...
    const entry: SessionEntry = {
      ...
      modelOverrideSource: "auto",
      modelOverrideFallbackOriginProvider: "anthrozaicoderc",
      modelOverrideFallbackOriginModel: "zaicoder-sonnet-4-6",
    ...
    expect(
      resolveAutoFallbackPrimaryProbe({
        entry,
        sessionKey: "agent:main:session",
        primaryProvider: "anthrozaicoderc",
        primaryModel: "zaicoder-sonnet-4-6",
        now: 1_000,
    ...
    ).toMatchObject({ provider: "anthrozaicoderc", model: "zaicoder-sonnet-4-6" });
  });
    ...
    expect(
      resolveAutoFallbackPrimaryProbe({
        entry: {
          ...
          modelOverride: "zaicoder-sonnet-4-6",
        },
        primaryProvider: "anthrozaicoderc",
        primaryModel: "zaicoder-sonnet-4-6",
        probeState: new Map(),
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: zAICoderConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: zAICoderConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/zaicoder");
  });

  it("uses OPENCLAW_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "zaicoder-home");
    withEnv({ OPENCLAW_HOME: home }, () => {
      const workspace = resolveAgentWorkspaceDir({} as zAICoderConfig, "main");
      expect(workspace).toBe(path.join(path.resolve(home), ".zaicoder", "workspace"));
    });
  });

  it("uses OPENCLAW_WORKSPACE_DIR for default agent workspace", () => {
    const workspaceDir = path.join(path.sep, "srv", "zaicoder-workspace");
    withEnv(
      {
        OPENCLAW_WORKSPACE_DIR: workspaceDir,
        OPENCLAW_HOME: path.join(path.sep, "srv", "zaicoder-home"),
      },
      () => {
        const workspace = resolveAgentWorkspaceDir({} as zAICoderConfig, "main");
  ...
  it("uses OPENCLAW_HOME for default agentDir", () => {
    ...
    withEnv({ OPENCLAW_HOME: home, OPENCLAW_STATE_DIR: "" }, () => {
      const agentDir = resolveAgentDir({} as zAICoderConfig, "main");
      expect(agentDir).toBe(path.join(path.resolve(home), ".zaicoder", "agents", "main", "agent"));
    });
  });
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/zaicoder-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/zaicoder-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-agent-scope-"));
