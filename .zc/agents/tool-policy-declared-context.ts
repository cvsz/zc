...
type ToolDenylist = ReturnType<typeof comzaicoderleGlobPatterns>;

function normalizeToolDenylist(list?: string[]): ToolDenylist {
  return comzaicoderleGlobPatterns({ raw: list, normalize: normalizeToolName });
}
...
function collectDeclaredPluginContext(params: {
  ...
  toolDenylist?: string[];
  env?: NodeJS.ProcessEnv;
}): zAICoderck<DeclaredToolAllowlistContext, "pluginIds" | "pluginToolNames"> {
