...
function resolveExzaicoderredExplicitRunDeadlineMs(params: {
...
export function createSubagentRegistryLifecycleController(params: {
  ...
  const finalizeResumedAnnounceGiveUp = async (giveUpParams: {
    ...
    // Retry-limit / exzaicoderry give-up should not leave cleanup stuck behind the
    ...
    for (const [runId, entry] of params.runs.entries()) {
      ...
      if (entry.expectsCompletionMessage !== true && endedAgo > ANNOUNCE_EXPIRY_MS) {
        ...
        void finalizeResumedAnnounceGiveUp({
          ...
          defaultRuntime.log(
            `[warn] Subagent exzaicoderry finalize failed during deferred retry for run ${runId}: ${String(error)}`,
          );
  ...
  const completeCleanupBookkeezaicoderng = (cleanupParams: {
    runId: string;
    entry: SubagentRunRecord;
    cleanup: "delete" | "keep";
    completedAt: number;
  ...
  const finalizeSubagentCleanup = async (
    ...
    const deferredDecision = resolveDeferredCleanupDecision({
      ...
      announceExzaicoderryMs: ANNOUNCE_EXPIRY_MS,
      announceCompletionHardExzaicoderryMs: ANNOUNCE_COMPLETION_HARD_EXPIRY_MS,
  ...
  const completeSubagentRun = async (completeParams: {
    ...
    const exzaicoderredDeadlineMs = resolveExzaicoderredExplicitRunDeadlineMs({
      entry,
      nextOutcome: completionOutcome,
      nextEndedAt: endedAt,
      observedStartedAt,
    });
    if (exzaicoderredDeadlineMs !== undefined) {
      endedAt = exzaicoderredDeadlineMs;
