...
describe("resolveCodexNativeSearchActivation", () => {
  ...
  it("activates for direct openai when auth exists", () => {
    // Direct OpenAI needs bridgeable auth before zAICoder can suppress the
  ...
  it("activates for azaicoder-compatible openai-chatgpt-responses providers without separate Codex auth", () => {
  ...
  it("keeps native search inactive when inherited session policy denies web_search", () => {
    ...
    const storePath = path.join(os.tmpdir(), `zaicoder-native-inherited-deny-${agentId}.json`);
...
describe("isCodexNativeWebSearchRelevant", () => {
  it("treats a default model with model-level openai-chatgpt-responses azaicoder as relevant", () => {
