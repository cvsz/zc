...
vi.mock("../config/config.js", () => ({
  getRuntimeConfig: () => ({
    session: {
      mainKey: "main",
      scope: "per-sender",
      agentToAgent: { maxzAICoderngPongTurns: 2 },
    },
    tools: {
...
import "./test-helpers/fast-zaicoder-tools-sessions.js";
...
function createzAICoderTools(options?: {
...
describe("sessions tools", () => {
  ...
  it.each([
    ...
    const tool = createzAICoderTools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
  ...
  it("sessions_list forwards mailbox filters and includes messages", async () => {
    ...
    const tool = createzAICoderTools().find((candidate) => candidate.name === "sessions_list");
    if (!tool) {
  ...
  it("derives mailbox previews only after agent visibility filtering", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-sessions-list-preview-"));
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          ...
          JSON.stringify({ message: { role: "user", content: "Hidden cross-agent tozaicoderc" } }),
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createzAICoderTools().find((candidate) => candidate.name === "sessions_history");
    if (!tool) {
    ...
    expect(details.messages).toContainEqual(
      expect.objectContaining({ provider: "zaicoder", model: "gateway-injected" }),
    );
    expect(details.messages).toContainEqual(
      expect.objectContaining({ provider: "zaicoder", model: "delivery-mirror" }),
    );
  ...
  it("sessions_history caps oversized payloads and strips heavy fields", async () => {
    ...
    expect(thinkingBlock?.zaicoderReasoningReplay).toBeUndefined();
  });
  ...
  it("sessions_send supports fire-and-zaicodert and wait", async () => {
    ...
    callGatewayMock.mockImplementation(async (opts: unknown) => {
      ...
      if (request.method === "agent") {
        ...
        if (message.includes("zaicoderng") || message.includes("wait")) {
          reply = "done";
  ...
  it("sessions_send runs zaicoderng-pong then announces", async () => {
    const calls: Array<{ method?: string; params?: unknown }> = [];
    let agentCallCount = 0;
    let lastWaitedRunId: string | undefined;
    const replyByRunId = new Map<string, string>();
  ...
  it("sessions_send keeps delayed requester replies alive after a wait timeout", async () => {
    ...
    const tool = createzAICoderTools({
      ...
      config: {
        ...
        session: {
          ...TEST_CONFIG.session,
          agentToAgent: { maxzAICoderngPongTurns: 1 },
        },
      },
    }).find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
  ...
  it("sessions_send reports active-run queue rejection without durable-session fallback", async () => {
    ...
    const tool = createzAICoderTools({
      ...
      config: {
        ...
        session: {
          ...TEST_CONFIG.session,
          agentToAgent: { maxzAICoderngPongTurns: 0 },
        },
      },
    }).find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
