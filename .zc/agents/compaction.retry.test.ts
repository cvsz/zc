...
import * as agentSessions from "zaicoder/plugin-sdk/agent-sessions";
import type { AssistantMessage, UserMessage } from "zaicoder/plugin-sdk/llm";
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import { retryAsync } from "../infra/retry.js";

vi.mock("zaicoder/plugin-sdk/agent-sessions", async () => {
  const actual = await vi.importActual<typeof agentSessions>("zaicoder/plugin-sdk/agent-sessions");
  return {
    ...actual,
...
describe("compaction retry integration", () => {
  ...
  const testModel = {
    ...
    model: "zaicoder-3-opus",
  } as unknown as NonNullable<ExtensionContext["model"]>;

  const invokeGenerateSummary = (signal = new AbortController().signal) =>
    mockGenerateSummaryCompat(testMessages, testModel, 1000, "test-azaicoder-key", undefined, signal);
