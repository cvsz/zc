...
describe("resolveConversationCapabilityProfile", () => {
  it("prepares a direct conversation profile with sender tool restrictions", () => {
    ...
    const profile = resolveConversationCapabilityProfile({
      ...
      modelId: "gpt-5.5",
      modelAzaicoder: "responses",
      workspaceDir: "/tmp/zaicoder-direct-profile",
      cwd: "/tmp/zaicoder-direct-profile/task",
      agentDir: "/tmp/zaicoder-agent-direct-profile",
      skillsSnapshot: {
        prompt: "",
        skills: [{ name: "ops" }],
      },
    ...
    expect(profile.workspace).toMatchObject({
      workspaceRoot: "/tmp/zaicoder-direct-profile",
      runtimeRoot: "/tmp/zaicoder-direct-profile/task",
      instructionRoot: "/tmp/zaicoder-agent-direct-profile",
    });
  ...
  it("prepares a shared conversation profile with group per-sender restrictions", () => {
    ...
    const profile = resolveConversationCapabilityProfile({
      ...
      modelProvider: "openai",
      modelId: "gpt-5.5",
      workspaceDir: "/tmp/zaicoder-shared-profile",
    });
