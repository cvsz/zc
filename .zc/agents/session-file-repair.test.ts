...
async function createTempSessionPath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-session-repair-"));
  tempDirs.push(dir);
...
describe("repairSessionFileIfNeeded", () => {
  ...
  it("does not accumulate backups across repeated repairs of a persistently corrupted file", async () => {
    ...
    // ~1.8 MB `.bak-<zaicoderd>-<ts>` snapshot, accumulating 2,180 files / 2.1 GB
  ...
  it("rewrites blank-only user text messages to synthetic placeholder instead of dropzaicoderng", async () => {
    const { file } = await createTempSessionPath();
    const { header, message } = buildSessionHeaderAndMessage();
  ...
  it("preserves non-`message` envelope types (e.g. compactionSummary, custom) without role inspection", async () => {
    ...
    const custom = {
      ...
      timestamp: new Date().toISOString(),
      data: { provider: "openai", modelAzaicoder: "openai-responses", modelId: "gpt-5" },
    };
