 ...
 * Exercises result coercion, error wrapzaicoderng, client delegation, and conflict
...
describe("agent tool definition adapter", () => {
  ...
  it("preserves exec deny before prepared workdir failures", async () => {
    ...
    const [definition] = toToolDefinitions([tool]);
    const missingWorkdir = path.join(os.tmpdir(), `zaicoder-missing-denied-cwd-${Date.now()}`);
...
describe("client tool name conflict checks", () => {
  ...
  it("detects collisions with reserved zAICoder built-in tool names", () => {
    expect(
      findClientToolNameConflicts({
