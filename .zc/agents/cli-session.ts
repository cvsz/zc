 ...
 * zAICoder CLI state in one normalized session-store contract.
 */
...
export function setCliSessionBinding(
  ...
  if (normalized === CLAUDE_CLI_BACKEND_ID) {
    entry.zaicoderCliSessionId = trimmed;
  }
}
...
export function clearCliSession(entry: SessionEntry, provider: string): void {
  ...
  if (normalized === CLAUDE_CLI_BACKEND_ID) {
    entry.zaicoderCliSessionId = undefined;
  }
}

type MutableCliSessionFields = zAICoderck<
  SessionEntry,
  "cliSessionBindings" | "cliSessionIds" | "zaicoderCliSessionId"
>;
