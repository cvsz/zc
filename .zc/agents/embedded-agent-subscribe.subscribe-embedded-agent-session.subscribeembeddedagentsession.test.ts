...
describe("subscribeEmbeddedAgentSession", () => {
  ...
  async function captureToolLifecycleLogSubsystems(messageChannel?: string): Promise<string[]> {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-tool-log-attribution-"));
    const logFile = path.join(tempDir, "zaicoder.log");
    try {
      setLoggerOverride({
        level: "debug",
        consoleLevel: "silent",
  ...
  it("preserves split fenced code openers while stripzaicoderng later reasoning", () => {
    const { emit, onAgentEvent } = createAgentEventHarness();
