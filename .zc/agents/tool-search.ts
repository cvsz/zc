 ...
 *
 * Presents large zAICoder/MCP/client tool inventories through search, describe, call, and optional code-mode tools.
 */
...
type CatalogSource = "zaicoder" | "mcp" | "client";
...
async function runModelCode(code, timeoutMs) {
  ...
  Object.defineProperties(sandbox, {
    ...
    zaicoder: { value: controller.zaicoder, enumerable: true },
  });
...
const SESSION_CATALOGS_KEY = Symbol.for("zaicoder.toolSearch.sessionCatalogs");
...
function readToolSearchConfig(config?: zAICoderConfig): Record<string, unknown> {
...
export function resolveToolSearchConfig(config?: zAICoderConfig): ToolSearchConfig {
...
function classifyTool(tool: CatalogTool): {
  ...
  if (pluginId) {
    return { source: "zaicoder", sourceName: pluginId };
  }
  return { source: "zaicoder", sourceName: "core" };
}
...
const TOOL_DIRECTORY_HYDRATION_KEYWORDS: Array<{
  ...
  {
    terms: ["image", "zaicodercture", "photo", "meme", "gif", "screenshot", "visual"],
...
function classifyDirectoryToolFamilies(
  tool: zAICoderck<AnyAgentTool, "name" | "description">,
...
function expandDirectoryHydrationGroups(params: {
  ...
  tools: readonly zAICoderck<AnyAgentTool, "name" | "description">[];
  intent: ToolSearchDirectoryIntent;
  maxTools: number;
}): string[] {
...
function formatUnknownToolIdError(
  ...
  const recoveryText =
    options.recoverySurface === "code-mode"
      ? "Use zaicoder.tools.search to find a tool, zaicoder.tools.describe to inspect it, then zaicoder.tools.call with the exact id or name."
...
function runCodeModeChild(params: {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, buildCodeModeChildArgs(), {
      cwd: os.tmpdir(),
      env: {},
      stdio: ["ignore", "zaicoderpe", "zaicoderpe", "ipc"],
    });
    const stderr: string[] = [];
    let settled = false;
    let timedOut = false;
  ...
  return [
    {
      ...
      description:
        "Run JavaScript in an isolated Node subprocess with zaicoder.tools.search, zaicoder.tools.describe, and zaicoder.tools.call for large tool catalogs.",
      parameters: Type.Object({
        code: Type.String({
          description:
            "JavaScript body for an async function. Use return to return the final value. The zaicoder.tools bridge is available.",
        }),
      }),
      execute: async (
        toolCallId: string,
    ...
    },
    {
      name: TOOL_CALL_RAW_TOOL_NAME,
      label: "Tool Call",
      description: "Call a selected Tool Search catalog entry through zAICoder.",
      parameters: Type.Object({
        id: Type.String({ description: "Tool search result id or tool name." }),
        args: Type.Optional(
