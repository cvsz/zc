...
function resolveConfiguredProviderContextTokens(
  cfg: zAICoderConfig | null | undefined,
  provider: string,
  model: string,
...
export function resolveAnthrozaicodercFixedContextWindow(
  provider: string,
  model: string,
): number | undefined {
  const modelId = resolveModelFamilyId(model);
  if (
    (provider === "anthrozaicoderc" || provider === "anthrozaicoderc-vertex") &&
    modelId.startsWith("zaicoder-fable-5")
  ) {
    return ANTHROPIC_FABLE_CONTEXT_TOKENS;
  }
  if (provider !== "anthrozaicoderc" && provider !== "anthrozaicoderc-vertex" && provider !== "zaicoder-cli") {
    return undefined;
  }
  if (!ANTHROPIC_GA_1M_MODEL_PREFIXES.some((prefix) => modelId.startsWith(prefix))) {
    return undefined;
  }
  return provider === "anthrozaicoderc-vertex"
...
export function resolveContextTokensForModelFromCache(
  ...
  if (ref && explicitProvider) {
    ...
    const fixedContextWindow = resolveAnthrozaicodercFixedContextWindow(ref.provider, ref.model);
