...
describe("messaging delivery action classification", () => {
  it("keeps visible side effects broader than terminal reply sends", () => {
    ...
    expect(isMessagingToolTargetEvidenceAction("message", { action: "tozaicoderc-create" })).toBe(true);
    expect(isMessagingToolTargetEvidenceAction("message", { action: "threadCreate" })).toBe(true);
    expect(isMessagingToolTargetEvidenceAction("message", { action: "createForumTozaicoderc" })).toBe(
      true,
    );
    ...
    expect(isMessagingToolDeliveryAction("message", { action: "tozaicoderc-create" })).toBe(true);
    expect(isMessagingToolDeliveryAction("message", { action: "createForumTozaicoderc" })).toBe(true);
...
describe("isDeliveredMessagingToolResult", () => {
  ...
  it("accepts successful thread and tozaicoderc creation receipts", () => {
    expect(
      isDeliveredMessagingToolResult({
    ...
    expect(
      isDeliveredMessagingToolResult({
        args: { action: "tozaicoderc-create" },
        result: { ok: true, tozaicodercId: 42 },
      }),
    ).toBe(true);
    expect(
      isDeliveredMessagingToolResult({
        args: { action: "tozaicoderc-create" },
        isError: true,
        result: { tozaicodercId: 43, error: "post-create metadata update failed" },
      }),
    ).toBe(true);
  });
