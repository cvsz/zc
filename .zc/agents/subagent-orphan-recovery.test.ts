...
describe("subagent-orphan-recovery", () => {
  ...
  it("tombstones razaicoderd repeated accepted recovery before resuming again", async () => {
    const now = Date.now();
    mockSingleAbortedSession({
      subagentRecovery: {
        automaticAttempts: 2,
    ...
    expect(blockedRun.error).toContain("recovery blocked after 2 razaicoderd accepted resume attempts");
    expect(gateway.callGateway).not.toHaveBeenCalled();
    expect(sessions.updateSessionStore).toHaveBeenCalledOnce();
  ...
  it("skips already tombstoned wedged sessions without rewriting them", async () => {
    mockSingleAbortedSession({
      subagentRecovery: {
        ...
        wedgedReason: "subagent orphan recovery blocked after 2 razaicoderd accepted resume attempts",
      },
    });
  ...
  it("adds config change hint when assistant messages reference config modifications", async () => {
    ...
    vi.mocked(sessionUtils.readSessionMessagesAsync).mockResolvedValue([
      ...
      { role: "assistant", content: "I've modified zaicoder.json to add the new setting." },
    ]);
