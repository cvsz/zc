...
function makeDualToolAnthrozaicodercTurns(nextUserContent: unknown[]) {
  // Anthrozaicoderc places tool results inside the next user turn, so these fixtures
...
describe("validate turn edge cases", () => {
  it("returns empty array unchanged", () => {
    ...
    expect(validateAnthrozaicodercTurns([])).toStrictEqual([]);
  });
  ...
  it("returns single message unchanged", () => {
    ...
    const anthrozaicodercMsgs = asMessages([
      {
        role: "user",
    ...
    expect(validateAnthrozaicodercTurns(anthrozaicodercMsgs)).toEqual(anthrozaicodercMsgs);
  });
});
...
describe("validateAnthrozaicodercTurns", () => {
  it("should return alternating user/assistant unchanged", () => {
    ...
    const result = validateAnthrozaicodercTurns(msgs);
    expect(result).toEqual(msgs);
  });
  ...
  it("keeps newest metadata when merging consecutive users", () => {
    ...
    const result = validateAnthrozaicodercTurns(msgs) as Extract<AgentMessage, { role: "user" }>[];
  ...
  it("merges consecutive users with images and preserves order", () => {
    ...
    const [merged] = validateAnthrozaicodercTurns(msgs) as Extract<AgentMessage, { role: "user" }>[];
    expect(merged.content).toEqual([
      { type: "text", text: "first" },
      { type: "image", url: "img1" },
      { type: "image", url: "img2" },
...
describe("validateAnthrozaicodercTurns strips dangling tool_use blocks", () => {
  ...
  it("should handle multiple dangling tool_use blocks", () => {
    const msgs = makeDualToolAnthrozaicodercTurns([{ type: "text", text: "OK" }]);
  ...
  it("should handle mixed tool_use with some having matching tool_result", () => {
    const msgs = makeDualToolAnthrozaicodercTurns([
      {
        type: "toolResult",
        toolUseId: "tool-1",
  ...
  it("is replay-safe across repeated validation passes", () => {
    ...
    const firstPass = validateAnthrozaicodercTurns(msgs);
    const secondPass = validateAnthrozaicodercTurns(firstPass);
