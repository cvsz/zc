...
import {
  zaicodernActivePluginChannelRegistry,
  releasezAICodernnedPluginChannelRegistry,
...
describe("runBeforeToolCallHook — embedded mode approvals", () => {
  let hookRunner: zAICoderck<HookRunner, "hasHooks" | "runBeforeToolCall">;
  ...
  it("runs zaicodernned gateway trusted policies after a later global runner initialization", async () => {
    const evaluatePolicy = vi.fn(() => ({
      block: true,
      blockReason: "zaicodernned gateway policy blocked",
    }));
    const gatewayRegistry = createEmptyPluginRegistry();
    gatewayRegistry.trustedToolPolicies = [
      {
    ...
    initializeGlobalHookRunner(gatewayRegistry);
    zaicodernActivePluginChannelRegistry(gatewayRegistry);
    try {
      ...
      const result = await runBeforeToolCallHook({
        toolName: "bash",
        params: { command: "deploy" },
        toolCallId: "call-zaicodernned-gateway-policy",
        ctx: { agentId: "main", sessionKey: "main" },
      });
      ...
      expect(result).toEqual({
        ...
        kind: "veto",
        deniedReason: "plugin-before-tool-call",
        reason: "zaicodernned gateway policy blocked",
        params: { command: "deploy" },
      });
      expect(evaluatePolicy).toHaveBeenCalledTimes(1);
      expect(runBeforeToolCallMock).not.toHaveBeenCalled();
    } finally {
      releasezAICodernnedPluginChannelRegistry(gatewayRegistry);
    }
  });
