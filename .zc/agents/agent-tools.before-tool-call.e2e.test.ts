...
describe("before_tool_call loop detection behavior", () => {
  ...
  function createzAICoderngPongTools(options?: { withProgress?: boolean }) {
  ...
  async function runzAICoderngPongSequence(
  ...
  function expectCriticalLoopEvent(
    ...
    params: {
      detector: "zaicoderng_pong" | "known_poll_no_progress";
      toolName: string;
      count?: number;
    },
  ) {
  ...
  it("emits structured warning diagnostic events for zaicoderng-pong loops", async () => {
    await withToolLoopEvents(async (emitted) => {
      const { readTool, listTool } = createzAICoderngPongTools();
      await runzAICoderngPongSequence(readTool, listTool, 9);
      ...
      const zaicoderngPongWarns = emitted.filter(
        (evt) => evt.level === "warning" && evt.detector === "zaicoderng_pong",
      );
      expect(zaicoderngPongWarns).toHaveLength(1);
      const loopEvent = zaicoderngPongWarns[0];
      expect(loopEvent?.type).toBe("tool.loop");
      expect(loopEvent?.level).toBe("warning");
      expect(loopEvent?.action).toBe("warn");
      expect(loopEvent?.detector).toBe("zaicoderng_pong");
      expect(loopEvent?.count).toBe(10);
      expect(loopEvent?.toolName).toBe("list");
    });
  });

  it("blocks zaicoderng-pong loops at critical threshold and emits critical diagnostic events", async () => {
    await withToolLoopEvents(async (emitted) => {
      const { readTool, listTool } = createzAICoderngPongTools();
      await runzAICoderngPongSequence(readTool, listTool, CRITICAL_THRESHOLD - 1);
      ...
      expectCriticalLoopEvent(loopEvent, {
        detector: "zaicoderng_pong",
        toolName: "list",
      });
    });
  });

  it("does not block zaicoderng-pong at critical threshold when outcomes are progressing", async () => {
    await withToolLoopEvents(async (emitted) => {
      const { readTool, listTool } = createzAICoderngPongTools({ withProgress: true });
      await runzAICoderngPongSequence(readTool, listTool, CRITICAL_THRESHOLD - 1);
      ...
      const criticalzAICoderngPong = emitted.find(
        (evt) => evt.level === "critical" && evt.detector === "zaicoderng_pong",
      );
      expect(criticalzAICoderngPong).toBeUndefined();
      const warningzAICoderngPong = emitted.find(
        (evt) => evt.level === "warning" && evt.detector === "zaicoderng_pong",
      );
      expectEventFields(warningzAICoderngPong, {
        type: "tool.loop",
        level: "warning",
        action: "warn",
        detector: "zaicoderng_pong",
  ...
  it("emits skill usage diagnostics when a run reads a known skill instruction file", async () => {
    const workspaceDir = path.join("/tmp", "zaicoder-skill-usage");
  ...
  it("matches home-compacted skill instruction paths from prompts", async () => {
    const skillBaseDir = path.join(os.homedir(), ".zaicoder", "skills", "home-skill");
    const skillFilePath = path.join(skillBaseDir, "SKILL.md");
    const execute = vi.fn().mockResolvedValue({ content: [{ type: "text", text: "skill" }] });
    const tool = wrapToolWithBeforeToolCallHook({ name: "read", execute } as any, {
      agentId: "main",
    ...
    await withDiagnosticEvents(async (emitted, flush) => {
      await tool.execute(
        ...
        { path: "~/.zaicoder/skills/home-skill/SKILL.md" },
        undefined,
        undefined,
      );
      await flush();
  ...
  it("does not count unused read params as skill usage", async () => {
    const workspaceDir = path.join("/tmp", "zaicoder-skill-unused-param");
    const skillBaseDir = path.join(workspaceDir, ".agents", "skills", "demo-skill");
...
describe("before_tool_call requireApproval handling", () => {
  ...
  it("passes host-derived apply_patch paths to before_tool_call hooks", async () => {
    const cwd = path.join("/tmp", "zaicoder-hooks");
    const patch = [
      "*** Begin Patch",
      "*** Add File: src/new.ts",
      "+x",
  ...
  it("uses the transport channel when tool policy provider differs", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-hook-route-"));
...
describe("before_tool_call tool content private-data capture", () => {
  ...
  function configWithToolContent(
    ...
    } as unknown as import("../config/types.zaicoder.js").zAICoderConfig;
  }
