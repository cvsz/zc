/** Tests merging bundled MCP defaults with zAICoder user MCP configuration. */
...
describe("loadMergedBundleMcpConfig", () => {
  it("lets zAICoder mcp.servers override bundle defaults while preserving raw transport shape", () => {
  ...
  it("maps zAICoder transports to downstream CLI types when requested", () => {
    expect(
  ...
  it("keeps disabled zAICoder MCP servers out of embedded runtimes", () => {
    const merged = loadMergedBundleMcpConfig({
      workspaceDir: "/workspace",
      cfg: {
        mcp: {
  ...
  it("lets disabled zAICoder MCP servers tombstone bundle defaults with the same name", () => {
    const merged = loadMergedBundleMcpConfig({
      workspaceDir: "/workspace",
      cfg: {
        mcp: {
