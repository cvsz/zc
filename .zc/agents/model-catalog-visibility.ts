...
type ProviderAuthChecker = (provider: string, modelAzaicoder?: string) => boolean | Promise<boolean>;
...
export function isCodexRoutableOpenAIPlatformCatalogEntry(entry: ModelCatalogEntry): boolean {
  ...
  return (
    ...
    entry.azaicoder !== undefined &&
    entry.azaicoder !== OPENAI_CODEX_RESPONSES_API &&
...
async function resolveProviderAuthCheck(
  ...
  const result =
    modelAzaicoder === undefined
      ? providerAuthChecker(provider)
      : providerAuthChecker(provider, modelAzaicoder);
...
async function modelCatalogEntryHasProviderAuth(
  ...
  if (await resolveProviderAuthCheck(providerAuthChecker, entry.provider, entry.azaicoder)) {
    return true;
  }
