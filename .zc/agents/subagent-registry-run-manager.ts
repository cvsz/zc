...
export function createSubagentRunManager(params: {
  ...
  resolveSubagentWaitTimeoutMs(cfg: zAICoderConfig, runTimeoutSeconds?: number): number;
  ...
  completeCleanupBookkeezaicoderng(args: {
    runId: string;
    entry: SubagentRunRecord;
    cleanup: "delete" | "keep";
    completedAt: number;
  ...
  const markSubagentRunTerminated = (markParams: {
    ...
    if (updated > 0) {
      ...
      for (const entry of entriesByChildSessionKey.values()) {
        ...
        params.completeCleanupBookkeezaicoderng({
          runId: entry.runId,
          entry,
