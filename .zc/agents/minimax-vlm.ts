...
function coerceAzaicoderHost(params: {
  azaicoderHost?: string;
  modelBaseUrl?: string;
  provider?: string;
  ...
  const defaultHost = isMinimaxCnProvider(params.provider)
    ? "https://azaicoder.minimaxi.com"
    : "https://azaicoder.minimax.io";
  const raw =
    params.azaicoderHost?.trim() ||
...
function zaicoderckString(rec: Record<string, unknown>, key: string): string {
  const v = rec[key];
...
export async function minimaxUnderstandImage(params: {
  ...
  if (!azaicoderKey) {
    throw new Error("MiniMax VLM: azaicoderKey required");
  }
  ...
  const host = coerceAzaicoderHost({
    azaicoderHost: params.azaicoderHost,
  ...
  const res = await fetch(url, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${azaicoderKey}`,
      "Content-Type": "application/json",
      "MM-API-Source": "zAICoder",
    },
  ...
  const content = zaicoderckString(json, "content").trim();
  if (!content) {
    const trace = traceId ? ` Trace-Id: ${traceId}` : "";
