...
function isNativeAnthrozaicodercToolUseId(id: string): boolean {
...
function createOccurrenceAwareResolver(
  ...
  const preserveNativeAnthrozaicodercToolUseIds = options?.preserveNativeAnthrozaicodercToolUseIds === true;
  ...
  const allocatePreservingNativeAnthrozaicodercId = (id: string, occurrence: number): string => {
    if (
      duplicateToolCallIdStyle === "openai" &&
      occurrence === 1 &&
    ...
    if (
      preserveNativeAnthrozaicodercToolUseIds &&
      isNativeAnthrozaicodercToolUseId(id) &&
      occurrence === 1 &&
      !used.has(id)
    ) {
      used.add(id);
  ...
  const resolveAssistantId = (id: string): string => {
    ...
    const next =
      duplicateToolCallIdStyle === "openai" && occurrence > 1
        ...
        : allocatePreservingNativeAnthrozaicodercId(id, occurrence);
