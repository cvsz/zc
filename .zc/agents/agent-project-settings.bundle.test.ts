...
const bundleTestDeps = await vi.hoisted(async () => {
  ...
  const loadBundleRegistry = (params: { workspaceDir?: string }) => {
    ...
    if (!fsSync.existsSync(pathModule.join(rootDir, ".zaicoder-plugin", "plugin.json"))) {
...
async function createWorkspaceBundle(params: {
  ...
  const pluginId = params.pluginId ?? "zaicoder-bundle";
  const pluginRoot = path.join(params.workspaceDir, ".zaicoder", "extensions", pluginId);
  await fs.mkdir(path.join(pluginRoot, ".zaicoder-plugin"), { recursive: true });
  await fs.writeFile(
    path.join(pluginRoot, ".zaicoder-plugin", "plugin.json"),
    JSON.stringify({
      name: pluginId,
    }),
    "utf-8",
...
describe("loadEnabledBundleAgentSettingsSnapshot", () => {
  it("reuses a compatible plugin metadata snapshot without loading a fresh one", async () => {
    const workspaceDir = await tempDirs.make("zaicoder-workspace-");
