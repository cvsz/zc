...
export async function resolveAgentRuntimeConfig(
  ...
  loadedRaw: zAICoderConfig;
  sourceConfig: zAICoderConfig;
  cfg: zAICoderConfig;
}> {
...
function hasAgentRuntimeSecretRefs(params: {
  ...
  if (hasNestedSecretRef(config.agents?.defaults?.memorySearch?.remote?.azaicoderKey)) {
    return true;
  }
  if (
    Array.isArray(config.agents?.list) &&
    config.agents.list.some((agent) => hasNestedSecretRef(agent?.memorySearch?.remote?.azaicoderKey))
  ) {
    return true;
  }
