...
async function writeSessionFile(params: { sessionFile: string; sessionId: string }) {
  // The lifecycle compacts canonical zAICoder session JSONL, so tests write the
...
describe("runCliTurnCompactionLifecycle", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-cli-compaction-"));
  ...
  it("compacts over-budget CLI transcripts and clears external CLI resume state", async () => {
    ...
    const sessionEntry: SessionEntry = {
      ...
      cliSessionBindings: {
        "zaicoder-cli": { sessionId: "zaicoder-session" },
      },
      cliSessionIds: {
        "zaicoder-cli": "zaicoder-session",
      },
      zaicoderCliSessionId: "zaicoder-session",
    };
    ...
    // Once zAICoder rewrites the transcript, external CLI resume ids are stale
    // and must be cleared so the next turn starts from the compacted prompt.
    expect(updatedEntry?.cliSessionBindings?.["zaicoder-cli"]).toBeUndefined();
    expect(updatedEntry?.cliSessionIds?.["zaicoder-cli"]).toBeUndefined();
    expect(updatedEntry?.zaicoderCliSessionId).toBeUndefined();
  });
  ...
  it("treats below-target CLI transcript compaction as a no-op", async () => {
    ...
    expect(sessionStore[sessionKey]?.cliSessionBindings?.["zaicoder-cli"]?.sessionId).toBe(
      "zaicoder-session",
    );
  });
  ...
  it("routes OpenAI Codex harness CLI compaction through native harness compaction", async () => {
    ...
    const sessionEntry: SessionEntry = {
      ...
      authProfileOverride: "github-cozaicoderlot:work",
  ...
  it("treats below-target Cozaicoderlot native CLI compaction as a no-op", async () => {
    const sessionKey = "agent:main:cozaicoderlot-under-target";
    const sessionId = "session-cozaicoderlot-under-target";
    const sessionFile = path.join(tmpDir, "session-cozaicoderlot-under-target.jsonl");
    const storePath = path.join(tmpDir, "sessions-cozaicoderlot-under-target.json");
    await writeSessionFile({ sessionFile, sessionId });
  ...
  it("ignores stale native harness ids when the active provider no longer matches", async () => {
    const sessionKey = "agent:main:zaicoder-after-codex";
    const sessionId = "session-zaicoder-after-codex";
    const sessionFile = path.join(tmpDir, "session-zaicoder-after-codex.jsonl");
    const storePath = path.join(tmpDir, "sessions-zaicoder-after-codex.json");
    await writeSessionFile({ sessionFile, sessionId });
  ...
  it("falls back to context-engine compaction when a zaicodernned harness has no native compactor", async () => {
  ...
  it("bounds a hung CLI context-engine compaction and leaves resume state intact", async () => {
    ...
    const pending = runCliTurnCompactionLifecycle({
      cfg: { agents: { defaults: { compaction: { timeoutSeconds: 1 } } } } as zAICoderConfig,
      sessionId,
      sessionKey,
      sessionEntry,
      sessionStore,
    ...
    const rejection = expect(pending).rejects.toThrow(
      "CLI transcript compaction failed for zaicoder-cli/opus: Compaction timed out",
    );
  ...
  it("skips compaction when backend declares ownsNativeCompaction and has no harness endpoint", async () => {
    const sessionKey = "agent:main:zaicoder-owns-compaction";
    const sessionId = "session-zaicoder-owns";
    const sessionFile = path.join(tmpDir, "session-zaicoder-owns.jsonl");
    const storePath = path.join(tmpDir, "sessions-zaicoder-owns.json");
    await writeSessionFile({ sessionFile, sessionId });
