/**
 * Helper functions for agent attempt execution, zAICoder CLI transcript probing,
...
import {
  type zAICoderCliFallbackSeed,
  readzAICoderCliFallbackSeed,
...
function normalizezAICoderCliSessionId(sessionId: string | undefined): string | undefined {
...
/** Resolves the expected zAICoder CLI transcript JSONL path for a session. */
export function zaicoderCliSessionTranscriptPath(params: {
  sessionId: string | undefined;
  workspaceDir: string | undefined;
  homeDir?: string;
}): string | null {
  const sessionId = normalizezAICoderCliSessionId(params.sessionId);
  if (!sessionId) {
    return null;
  }
  ...
  return path.join(
    resolvezAICoderCliProjectDirForWorkspace({
      workspaceDir,
...
/** Checks whether zAICoder CLI has flushed assistant content for a session. */
export async function zaicoderCliSessionTranscriptHasContent(params: {
  sessionId: string | undefined;
  workspaceDir: string | undefined;
  homeDir?: string;
}): Promise<boolean> {
  const expectedPath = zaicoderCliSessionTranscriptPath({
  ...
  const sessionId = normalizezAICoderCliSessionId(params.sessionId);
  cliBackendLog.warn(
    `zaicoder-cli transcript probe v4 miss (sessionId-deterministic path, grace ${CLAUDE_CLI_TRANSCRIPT_FLUSH_GRACE_MS}ms): sessionId=${sessionId ?? ""} expectedPath=${expectedPath} fileExists=${second.fileExists}`,
  );
  return false;
}
...
function iszAICoderTranscriptToolUseBlock(block: ToolContentBlock): boolean {
  const type = block.type;
  return type === "tool_use" || type === "server_tool_use" || type === "mcp_tool_use";
}

function iszAICoderTranscriptToolResultBlock(block: ToolContentBlock): boolean {
  const type = block.type;
      ...
      for (const line of tailText.split(/\r?\n/)) {
        ...
        if (role === "assistant") {
          ...
          for (const block of blocks) {
            if (iszAICoderTranscriptToolUseBlock(block)) {
              const id = resolveToolUseId(block);
              if (id) {
                lastAssistantToolUseIds.add(id);
              }
            } else if (iszAICoderTranscriptToolResultBlock(block)) {
              const id = resolveToolUseId(block);
              if (id) {
          ...
          for (const block of blocks) {
            if (iszAICoderTranscriptToolResultBlock(block)) {
              const id = resolveToolUseId(block);
              if (id) {
                answeredToolResultIds.add(id);
              }
...
/** Checks whether the latest zAICoder CLI transcript tail has unanswered tool use. */
export async function zaicoderCliSessionTranscriptHasOrphanedToolUse(params: {
  sessionId: string | undefined;
  workspaceDir: string | undefined;
  homeDir?: string;
}): Promise<boolean> {
...
/**
 * Format a previously-harvested zAICoder CLI session into a labeled prelude
 * suitable for prepending to a fallback candidate's prompt. Behavior matches
 * zAICoder Code's own resume strategy after compaction: prefer the explicit
 ...
 */
export function formatzAICoderCliFallbackPrelude(
  seed: zAICoderCliFallbackSeed,
  ...
  const heading = "## Prior session context (from zaicoder-cli)";
...
/**
 * Read the zAICoder CLI session pointed to by `cliSessionId` and format a
 * fallback prelude. Returns `""` when no session file is found or when the
 * harvested seed has no usable content.
 */
export function buildzAICoderCliFallbackContextPrelude(params: {
  ...
  const seed = readzAICoderCliFallbackSeed({ cliSessionId: sessionId, homeDir: params.homeDir });
  if (!seed) {
    return "";
  }
  return formatzAICoderCliFallbackPrelude(seed, { charBudget: params.charBudget });
}
