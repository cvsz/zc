 ...
 */
import type { AcpRuntimeEvent } from "@zaicoder/acp-core/runtime/types";
import type { FastMode } from "@zaicoder/normalization-core/string-coerce";
...
function resolveCliExecutionAuthProfileId(params: {
  ...
  return resolveAuthProfileOrder({
    ...
    return credential?.provider === GOOGLE_PROVIDER_ID && credential.type === "azaicoder_key";
  });
}
  ...
  if (replyText) {
    messages.push({
      message: {
        ...
        azaicoder: params.assistant.azaicoder,
...
function iszAICoderCliProvider(provider: string): boolean {
  return provider.trim().toLowerCase() === "zaicoder-cli";
}
...
export function runAgentAttempt(params: {
  ...
  const zaicoderCliFallbackPrelude =
    !isRawModelRun &&
    params.isFallbackRetry &&
    iszAICoderCliProvider(params.originalProvider) &&
    !iszAICoderCliProvider(params.providerOverride)
      ? buildzAICoderCliFallbackContextPrelude({
          cliSessionId: getCliSessionBinding(params.sessionEntry, "zaicoder-cli")?.sessionId,
        })
      : "";
  const resolvedPrompt = resolveFallbackRetryPrompt({
    ...
    priorContextPrelude: zaicoderCliFallbackPrelude,
  });
  ...
  const requestedAgentHarnessId = isRawModelRun ? "zaicoder" : undefined;
  ...
  const agentHarnessPolicy = isRawModelRun
    ? ({ runtime: "zaicoder", runtimeSource: "model" } as const)
  ...
  const embeddedAgentHarnessOverride =
    ...
    (agentHarnessPolicy.runtime === "zaicoder" && agentHarnessPolicy.runtimeSource !== "implicit"
      ? "zaicoder"
      : undefined);
  if (!isRawModelRun && isCliExecutionProvider) {
    ...
    const resolveReusableCliSessionBinding = async () => {
      if (
        !iszAICoderCliProvider(cliExecutionProvider) ||
