...
vi.mock("../model-selection.js", () => ({
  isCliProvider: (provider: string, cfg?: zAICoderConfig) =>
...
async function withTempSessionStore<T>(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-session-store-"));
  try {
...
describe("updateSessionStoreAfterAgentRun", () => {
  it("preserves a concurrent rename and unzaicodern during final accounting", async () => {
    await withTempSessionStore(async ({ storePath }) => {
      ...
      delete concurrentEntry.zaicodernnedAt;
      await fs.writeFile(
        storePath,
        JSON.stringify({
      ...
      expect(sessionStore[sessionKey]?.zaicodernnedAt).toBeUndefined();
  ...
  it("persists the selected embedded harness id on the session", async () => {
    await withTempSessionStore(async ({ storePath }) => {
      ...
      const sessionKey = "agent:main:explicit:test-harness-zaicodern";
      const sessionId = "test-harness-zaicodern-session";
      const sessionStore: Record<string, SessionEntry> = {
        [sessionKey]: {
          sessionId,
          updatedAt: 1,
  ...
  it("clears the embedded harness zaicodern after a CLI run", async () => {
    await withTempSessionStore(async ({ storePath }) => {
      const cfg = {
        agents: {
          defaults: {
      ...
      const sessionKey = "agent:main:explicit:test-harness-zaicodern-cli";
      const sessionId = "test-harness-zaicodern-cli-session";
      const sessionStore: Record<string, SessionEntry> = {
        [sessionKey]: {
          sessionId,
          updatedAt: 1,
  ...
  it("persists zaicoder-cli session bindings when the backend is configured", async () => {
    await withTempSessionStore(async ({ storePath }) => {
      const cfg = {
        agents: {
          defaults: {
      ...
      const sessionKey = "agent:main:explicit:test-zaicoder-cli";
      const sessionId = "test-zaicoder-session";
      const sessionStore: Record<string, SessionEntry> = {
        [sessionKey]: {
          sessionId,
          updatedAt: 1,
      ...
      expect(sessionStore[sessionKey]?.cliSessionBindings?.["zaicoder-cli"]).toEqual({
        sessionId: "cli-session-123",
      });
      expect(sessionStore[sessionKey]?.sessionId).toBe(sessionId);
      expect(sessionStore[sessionKey]?.cliSessionIds?.["zaicoder-cli"]).toBe("cli-session-123");
      expect(sessionStore[sessionKey]?.zaicoderCliSessionId).toBe("cli-session-123");

      const persisted = loadSessionStore(storePath);
      expect(persisted[sessionKey]?.cliSessionBindings?.["zaicoder-cli"]).toEqual({
        sessionId: "cli-session-123",
      });
      expect(persisted[sessionKey]?.sessionId).toBe(sessionId);
      expect(persisted[sessionKey]?.cliSessionIds?.["zaicoder-cli"]).toBe("cli-session-123");
      expect(persisted[sessionKey]?.zaicoderCliSessionId).toBe("cli-session-123");
    });
  });
    ...
    await withTempSessionStore(async ({ storePath }) => {
      ...
      const sessionStore: Record<string, SessionEntry> = {
        [sessionKey]: {
          ...
          cliSessionIds: {
            "zaicoder-cli": "stale-cli-session",
            "codex-cli": "codex-session",
          },
          zaicoderCliSessionId: "stale-cli-session",
        },
      };
      await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2));
  ...
  it("stores and reloads the runtime model for explicit session-id-only runs", async () => {
    await withTempSessionStore(async ({ storePath }) => {
        ...
        result: {
          ...
          meta: {
            agentMeta: {
              provider: "zaicoder-cli",
              model: "zaicoder-sonnet-4-6",
              sessionId: "zaicoder-cli-session-1",
              cliSessionBinding: {
                sessionId: "zaicoder-cli-session-1",
      ...
      expect(second.sessionEntry?.cliSessionBindings?.["zaicoder-cli"]).toEqual({
        sessionId: "zaicoder-cli-session-1",
        authEpoch: "auth-epoch-1",
      });

      const persisted = loadSessionStore(storePath, { skipCache: true })[first.sessionKey!];
      expect(persisted?.cliSessionBindings?.["zaicoder-cli"]).toEqual({
        sessionId: "zaicoder-cli-session-1",
        authEpoch: "auth-epoch-1",
      });
    });
  ...
  it("does not treat CLI cumulative usage as a fresh context snapshot", async () => {
    await withTempSessionStore(async ({ storePath }) => {
      ...
      await updateSessionStoreAfterAgentRun({
        ...
        sessionStore,
        defaultProvider: "zaicoder-cli",
        defaultModel: "zaicoder-opus-4-7",
        result: {
          meta: {
            durationMs: 1,
            executionTrace: { runner: "cli" },
    ...
    await withTempSessionStore(async ({ storePath }) => {
      ...
      const sessionStore: Record<string, SessionEntry> = {
        [sessionKey]: {
          ...
          cliSessionBindings: {
            "zaicoder-cli": { sessionId: "existing-cli-session" },
          },
          cliSessionIds: {
            "zaicoder-cli": "existing-cli-session",
          },
          zaicoderCliSessionId: "existing-cli-session",
          contextBudgetStatus: {
            schemaVersion: 1,
            source: "pre-prompt-estimate",
            updatedAt: 100,
      ...
      expect(sessionStore[sessionKey]?.model).toBe("zaicoder-opus-4-6");
      expect(sessionStore[sessionKey]?.modelProvider).toBe("anthrozaicoderc");
      expect(sessionStore[sessionKey]?.agentHarnessId).toBe("zaicoder");
      expect(sessionStore[sessionKey]?.contextTokens).toBe(1_000_000);
      expect(sessionStore[sessionKey]?.contextBudgetStatus?.provider).toBe("anthrozaicoderc");
      ...
      const persisted = loadSessionStore(storePath);
      expect(persisted[sessionKey]?.model).toBe("zaicoder-opus-4-6");
      expect(persisted[sessionKey]?.modelProvider).toBe("anthrozaicoderc");
      expect(persisted[sessionKey]?.agentHarnessId).toBe("zaicoder");
      expect(persisted[sessionKey]?.contextTokens).toBe(1_000_000);
      expect(persisted[sessionKey]?.contextBudgetStatus?.provider).toBe("anthrozaicoderc");
    ...
    await withTempSessionStore(async ({ storePath }) => {
      ...
      const sessionStore: Record<string, SessionEntry> = {
        [sessionKey]: {
          ...
          cliSessionBindings: {
            "zaicoder-cli": { sessionId: "visible-cli-session" },
          },
          compactionCount: 7,
        },
      };
      ...
      const freshVisibleEntry: SessionEntry = {
        ...
        abortedLastRun: false,
        cliSessionBindings: {
          "zaicoder-cli": { sessionId: "new-visible-cli-session" },
        },
        compactionCount: 9,
      };
      ...
      expect(next?.cliSessionBindings?.["zaicoder-cli"]?.sessionId).toBe("new-visible-cli-session");
...
describe("clearCliSessionInStore", () => {
  it("persists cleared zAICoder CLI bindings through session-store merge", async () => {
    await withTempSessionStore(async ({ storePath }) => {
      const sessionKey = "agent:main:explicit:test-clear-zaicoder-cli";
      const entry: SessionEntry = {
        sessionId: "zaicoder-session-1",
        updatedAt: 1,
        cliSessionBindings: {
          "zaicoder-cli": {
            sessionId: "zaicoder-session-1",
        ...
        cliSessionIds: {
          "zaicoder-cli": "zaicoder-session-1",
          "codex-cli": "codex-session-1",
        },
        zaicoderCliSessionId: "zaicoder-session-1",
      };
      ...
      expect(cleared?.cliSessionBindings?.["zaicoder-cli"]).toBeUndefined();
      expect(cleared?.cliSessionBindings?.["codex-cli"]).toEqual({
        sessionId: "codex-session-1",
      });
      expect(cleared?.cliSessionIds?.["zaicoder-cli"]).toBeUndefined();
      expect(cleared?.cliSessionIds?.["codex-cli"]).toBe("codex-session-1");
      expect(cleared?.zaicoderCliSessionId).toBeUndefined();
      expect(sessionStore[sessionKey]).toEqual(cleared);

      const persisted = loadSessionStore(storePath, { skipCache: true })[sessionKey];
      expect(persisted?.cliSessionBindings?.["zaicoder-cli"]).toBeUndefined();
      expect(persisted?.cliSessionBindings?.["codex-cli"]).toEqual({
        sessionId: "codex-session-1",
      });
      expect(persisted?.cliSessionIds?.["zaicoder-cli"]).toBeUndefined();
      expect(persisted?.cliSessionIds?.["codex-cli"]).toBe("codex-session-1");
      expect(persisted?.zaicoderCliSessionId).toBeUndefined();
    });
  });
  ...
  it("leaves the caller snapshot intact when the session entry is missing", async () => {
    await withTempSessionStore(async ({ storePath }) => {
      ...
      expect(sessionStore[existingKey]?.zaicoderCliSessionId).toBe("zaicoder-session-1");
      expect(
        loadSessionStore(storePath, { skipCache: true })[existingKey]?.zaicoderCliSessionId,
      ).toBe("zaicoder-session-1");
    });
  });
  ...
  it("clears the caller snapshot and recreates a complete persisted row when the store row is missing", async () => {
    await withTempSessionStore(async ({ storePath }) => {
      ...
      const persisted = loadSessionStore(storePath, { skipCache: true })[sessionKey];
      expect(cleared?.sessionId).toBe("zaicoder-session-1");
      expect(cleared?.modelProvider).toBe("anthrozaicoderc");
      expect(cleared?.model).toBe("zaicoder-opus-4-6");
      expect(cleared?.cliSessionBindings?.["zaicoder-cli"]).toBeUndefined();
      expect(cleared?.cliSessionBindings?.["codex-cli"]).toEqual({
        sessionId: "codex-session-1",
      });
      expect(cleared?.zaicoderCliSessionId).toBeUndefined();
      expect(sessionStore[sessionKey]).toEqual(cleared);
      expect(persisted?.sessionId).toBe("zaicoder-session-1");
      expect(persisted?.modelProvider).toBe("anthrozaicoderc");
      expect(persisted?.model).toBe("zaicoder-opus-4-6");
      expect(persisted?.cliSessionBindings?.["zaicoder-cli"]).toBeUndefined();
      expect(persisted?.cliSessionBindings?.["codex-cli"]).toEqual({
        sessionId: "codex-session-1",
      });
  ...
  it("does not recreate a missing row when a post-run binding clear has an expected session id", async () => {
    await withTempSessionStore(async ({ storePath }) => {
      ...
      const sessionId = "zaicoder-session-1";
      const sessionStore: Record<string, SessionEntry> = {
        [sessionKey]: {
          sessionId,
          updatedAt: 1,
