...
function makeTaskCompletionEvents(): NonNullable<AgentCommandOpts["internalEvents"]> {
  ...
  // cannot spoof zAICoder runtime-context envelopes.
  return [
    {
...
describe("attempt execution prompt materialization", () => {
  it("materializes ACP internal events without zAICoder internal runtime markers", () => {
...
describe("persistSessionEntry", () => {
  it("clears stale local entries when guarded persistence sees no persisted entry", async () => {
    const dir = tempDirs.make("zaicoder-session-store-");
    try {
  ...
  it.each([
    {
      name: "rename and unzaicodern",
      current: { label: "Renamed", zaicodernnedAt: undefined },
      expected: { label: "Renamed", zaicodernnedAt: undefined },
    },
    {
      name: "label clear and zaicodern",
      current: { label: undefined, zaicodernnedAt: 300 },
      expected: { label: undefined, zaicodernnedAt: 300 },
    },
    ...
    try {
      ...
      if (current.zaicodernnedAt === undefined) {
        delete currentEntry.zaicodernnedAt;
      }
      ...
      expect(persisted?.zaicodernnedAt).toBe(expected.zaicodernnedAt);
