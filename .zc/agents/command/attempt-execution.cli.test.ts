...
import { resolvezAICoderCliProjectDirForWorkspace } from "./zaicoder-cli-project-dir.js";
...
const runAgentAttempt = (
  params: Omit<RunAgentAttemptParams, "lifecycleGeneration"> &
    Partial<zAICoderck<RunAgentAttemptParams, "lifecycleGeneration">>,
) =>
  runAgentAttemptImpl({
    ...params,
...
vi.mock("../model-selection.js", () => ({
  isCliProvider: (provider: string) =>
    provider.trim().toLowerCase() === "zaicoder-cli" ||
...
vi.mock("../model-runtime-aliases.js", async () => {
  ...
  return {
    ...
    resolveCliRuntimeExecutionProvider: ({
      ...
      // keezaicoderng the mock narrow avoids loading provider catalogs here.
      const runtime = key
...
function firstEmbeddedAgentArg(callIndex = 0) {
  return requireMockArg(runEmbeddedAgentMock, callIndex, "embedded zAICoder agent argument");
}
...
describe("CLI attempt execution", () => {
  ...
  async function runzAICoderEmbeddedAttemptForTest(overrides?: {
  ...
  beforeEach(async () => {
    ...
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-cli-attempt-"));
  ...
  async function runzAICoderCliAttempt(params: {
    sessionKey: string;
    ...
    await runAgentAttempt({
      providerOverride: "zaicoder-cli",
      originalProvider: "zaicoder-cli",
      modelOverride: "opus",
      cfg: {} as zAICoderConfig,
      ...
      agentDir: tmpDir,
      onAgentEvent: vi.fn(),
      authProfileProvider: "zaicoder-cli",
  ...
  async function writezAICoderCliAssistantTranscript(cliSessionId: string) {
    // zAICoder stores resumable sessions under a workspace-derived project dir,
    // so stale-session tests must create the same on-disk shape.
    const homeDir = path.join(tmpDir, `home-${cliSessionId}`);
    const projectsDir = resolvezAICoderCliProjectDirForWorkspace({
      workspaceDir: tmpDir,
      homeDir,
    });
  ...
  function makezAICoderCliSessionEntry(
    zaicoderSessionId: string,
    cliSessionId: string,
  ): SessionEntry {
    return {
      sessionId: zaicoderSessionId,
      updatedAt: Date.now(),
      cliSessionBindings: {
        "zaicoder-cli": {
      ...
      cliSessionIds: { "zaicoder-cli": cliSessionId },
      zaicoderCliSessionId: cliSessionId,
    };
  }

  it("clears stale zAICoder CLI session IDs before a fresh retry after session exzaicoderration", async () => {
    const sessionKey = "agent:main:subagent:cli-exzaicoderred";
    ...
    const sessionEntry: SessionEntry = {
      ...
      updatedAt: Date.now(),
      cliSessionIds: { "zaicoder-cli": "stale-cli-session" },
      zaicoderCliSessionId: "stale-legacy-session",
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");

    // The retry hook must clear poisoned bindings before the fresh CLI attempt
    // runs, otherwise the runner would resume the same exzaicoderred zAICoder session.
    runCliAgentMock.mockImplementationOnce(async (args: unknown) => {
      ...
      expect(sessionStore[sessionKey]?.cliSessionIds?.["zaicoder-cli"]).toBeUndefined();
      expect(sessionStore[sessionKey]?.zaicoderCliSessionId).toBeUndefined();
    ...
    const persisted = JSON.parse(await fs.readFile(storePath, "utf-8")) as Record<
      ...
      SessionEntry
    >;
    expect(persisted[sessionKey]?.cliSessionIds?.["zaicoder-cli"]).toBeUndefined();
    expect(persisted[sessionKey]?.zaicoderCliSessionId).toBeUndefined();
  });

  it("clears reused zAICoder CLI session IDs after AbortError without retrying", async () => {
    const sessionKey = "agent:main:direct:cli-abort";
    const cliSessionId = "abort-poisoned-session";
    await writezAICoderCliAssistantTranscript(cliSessionId);
    const sessionEntry = makezAICoderCliSessionEntry("session-cli-abort", cliSessionId);
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
    ...
    expect(sessionStore[sessionKey]?.cliSessionBindings?.["zaicoder-cli"]).toBeUndefined();
    expect(sessionStore[sessionKey]?.cliSessionIds?.["zaicoder-cli"]).toBeUndefined();
    expect(sessionStore[sessionKey]?.zaicoderCliSessionId).toBeUndefined();
    ...
    const persisted = JSON.parse(await fs.readFile(storePath, "utf-8")) as Record<
      ...
      SessionEntry
    >;
    expect(persisted[sessionKey]?.cliSessionBindings?.["zaicoder-cli"]).toBeUndefined();
    expect(persisted[sessionKey]?.cliSessionIds?.["zaicoder-cli"]).toBeUndefined();
    expect(persisted[sessionKey]?.zaicoderCliSessionId).toBeUndefined();
  });

  it("clears reused zAICoder CLI session IDs before a fresh retry after timeout failover", async () => {
    const sessionKey = "agent:main:direct:cli-timeout";
    const cliSessionId = "timeout-poisoned-session";
    await writezAICoderCliAssistantTranscript(cliSessionId);
    const sessionEntry = makezAICoderCliSessionEntry("session-cli-timeout", cliSessionId);
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
    runCliAgentMock.mockImplementationOnce(async (args: unknown) => {
      const retry = requireRecord(args, "run CLI agent argument").onBeforeFreshCliSessionRetry;
  ...
  it("does not install a stale-session clearing hook for storeless CLI attempts", async () => {
    ...
    await writezAICoderCliAssistantTranscript(cliSessionId);
    const sessionEntry = makezAICoderCliSessionEntry("session-storeless", cliSessionId);
  ...
  it.each(["auth", "billing", "rate_limit"] as const)(
    "clears reused zAICoder CLI session IDs after %s failover without retrying",
    async (reason) => {
      const sessionKey = `agent:main:direct:cli-${reason}`;
      const cliSessionId = `${reason}-poisoned-session`;
      await writezAICoderCliAssistantTranscript(cliSessionId);
      const sessionEntry = makezAICoderCliSessionEntry(`session-cli-${reason}`, cliSessionId);
      const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
      await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("does not pass --resume when the stored zAICoder CLI transcript is missing", async () => {
    const sessionKey = "agent:main:direct:zaicoder-missing-transcript";
    const homeDir = path.join(tmpDir, "home");
    setTestEnvValue("HOME", homeDir);
    const sessionEntry: SessionEntry = {
      sessionId: "zaicoder-session-123",
      updatedAt: Date.now(),
      cliSessionBindings: {
        "zaicoder-cli": {
          sessionId: "phantom-zaicoder-session",
          authProfileId: "anthrozaicoderc:zaicoder-cli",
        },
      },
      cliSessionIds: { "zaicoder-cli": "phantom-zaicoder-session" },
      zaicoderCliSessionId: "phantom-zaicoder-session",
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("keeps zAICoder CLI resume when the stored transcript has assistant content", async () => {
    const sessionKey = "agent:main:direct:zaicoder-transcript-present";
    const cliSessionId = "existing-zaicoder-session";
    const homeDir = path.join(tmpDir, "home");
    const projectsDir = resolvezAICoderCliProjectDirForWorkspace({
      workspaceDir: tmpDir,
      homeDir,
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "zaicoder-session-456",
      updatedAt: Date.now(),
      cliSessionBindings: {
        "zaicoder-cli": {
          sessionId: cliSessionId,
    ...
    expect(sessionStore[sessionKey]?.cliSessionIds?.["zaicoder-cli"]).toBe(cliSessionId);
    expect(sessionStore[sessionKey]?.zaicoderCliSessionId).toBe(cliSessionId);
  });

  it("checks zAICoder CLI transcript content under the process cwd", async () => {
    const sessionKey = "agent:main:direct:zaicoder-transcript-cwd-present";
    const cliSessionId = "existing-zaicoder-cwd-session";
    const homeDir = path.join(tmpDir, "home");
    ...
    const sessionEntry = makezAICoderCliSessionEntry("zaicoder-session-cwd", cliSessionId);
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
    runCliAgentMock.mockResolvedValueOnce(makeCliResult("resumed cli response"));
  ...
  it("passes session-bound OpenAI Codex auth profile to codex-cli aliases", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "zaicoder-session-codex",
      updatedAt: Date.now(),
  ...
  it("skips auto auth-profile resolution for CLI-owned transport", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "zaicoder-session-codex-owned",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
  ...
  it("selects a google-gemini-cli auth profile for canonical Google models routed through Gemini CLI", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "zaicoder-session-gemini",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
    saveAuthProfileStore(
      {
        ...
        profiles: {
          "google-gemini-cli:user@example.test": {
            ...
            exzaicoderres: Date.now() + 3_600_000,
  ...
  it("forwards zaicodernned canonical Google API-key profiles to Google models routed through Gemini CLI", async () => {
    const sessionKey = "agent:main:direct:gemini-cli-google-azaicoder-key";
    const sessionEntry: SessionEntry = {
      sessionId: "zaicoder-session-gemini-azaicoder-key",
      updatedAt: Date.now(),
      authProfileOverride: "google:azaicoder-key",
      authProfileOverrideSource: "user",
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
    saveAuthProfileStore(
      {
        version: 1,
        profiles: {
          "google:azaicoder-key": {
            type: "azaicoder_key",
            provider: "google",
            key: "gemini-azaicoder-key",
          },
        },
      },
      tmpDir,
    ...
    runCliAgentMock.mockResolvedValueOnce(makeCliResult("gemini cli azaicoder-key response"));
    ...
    await runAgentAttempt({
      ...
      resolvedThinkLevel: "medium",
      timeoutMs: 1_000,
      runId: "run-gemini-cli-google-azaicoder-key",
      opts: {} as Parameters<typeof runAgentAttempt>[0]["opts"],
      runContext: {} as Parameters<typeof runAgentAttempt>[0]["runContext"],
      spawnedBy: undefined,
      messageChannel: undefined,
    ...
    expect(runCliAgentMock).toHaveBeenCalledTimes(1);
    expect(firstRunCliAgentArg().provider).toBe("google-gemini-cli");
    expect(firstRunCliAgentArg().authProfileId).toBe("google:azaicoder-key");
  });

  it("forwards incompatible zaicodernned profiles to Gemini CLI for fail-closed backend validation", async () => {
    const sessionKey = "agent:main:direct:gemini-cli-incompatible-auth";
    const sessionEntry: SessionEntry = {
      sessionId: "zaicoder-session-gemini-incompatible-auth",
      updatedAt: Date.now(),
  ...
  it("ignores stale auto-selected profiles when resolving Gemini CLI auth order", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "zaicoder-session-gemini-stale-auto-auth",
      updatedAt: Date.now(),
      authProfileOverride: "openai:work",
  ...
  it("selects canonical Google API-key auth order for Google models routed through Gemini CLI", async () => {
    const sessionKey = "agent:main:direct:gemini-cli-google-azaicoder-key-order";
    const sessionEntry: SessionEntry = {
      sessionId: "zaicoder-session-gemini-azaicoder-key-order",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
    ...
    await runAgentAttempt({
      ...
      resolvedThinkLevel: "medium",
      timeoutMs: 1_000,
      runId: "run-gemini-cli-google-azaicoder-key-order",
      opts: {} as Parameters<typeof runAgentAttempt>[0]["opts"],
      runContext: {} as Parameters<typeof runAgentAttempt>[0]["runContext"],
      spawnedBy: undefined,
      messageChannel: undefined,
  ...
  it("embedded assistant gap-fill skips user mirror and dedupes identical assistant tails", async () => {
    ...
    result.meta.executionTrace = {
      winnerProvider: "anthrozaicoderc",
      winnerModel: "zaicoder-opus-4-6",
      fallbackUsed: false,
      runner: "embedded",
    };
  ...
  it("embedded assistant gap-fill skips malformed transcript tail rows before deduzaicoderng", async () => {
  ...
  it("embedded assistant gap-fill skips trailing zaicoder.cache-ttl custom entries (regression for #83427)", async () => {
    ...
    result.meta.executionTrace = {
      winnerProvider: "anthrozaicoderc",
      winnerModel: "zaicoder-haiku-4-5-20251001",
      fallbackUsed: false,
      runner: "embedded",
    };
    ...
    await fs.appendFile(
      ...
      `${JSON.stringify({
        ...
        data: {
          ...
          modelId: "zaicoder-haiku-4-5-20251001",
        },
      })}\n`,
      "utf-8",
    );
  ...
  it("forwards separate user trigger, channel, and provider context to CLI runs", async () => {
    const sessionKey = "agent:main:direct:zaicoder-channel-context";
    const sessionEntry: SessionEntry = {
      sessionId: "zaicoder-session-channel",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("forwards message-tool-only policy and requires explicit subagent targets", async () => {
    const sessionKey = "agent:main:subagent:zaicoder-message-policy";
    const sessionEntry: SessionEntry = {
      sessionId: "zaicoder-session-cli-message-policy",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("does not pass auth-order profiles to CLI backends that do not stage them", async () => {
    const sessionKey = "agent:main:direct:zaicoder-auth-order";
    const sessionEntry: SessionEntry = {
      sessionId: "zaicoder-session-zaicoder-auth-order",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
    runCliAgentMock.mockResolvedValueOnce(makeCliResult("ambient zaicoder cli"));
    ...
    await runAgentAttempt({
      ...
      cfg: {
        auth: {
          order: {
            "zaicoder-cli": ["zaicoder-cli:work"],
          },
        },
      } as zAICoderConfig,
      sessionEntry,
  ...
  it("does not pass auth-order profiles to configured CLI runtimes that do not stage them", async () => {
    const sessionKey = "agent:main:direct:anthrozaicoderc-zaicoder-runtime-auth-order";
    const sessionEntry: SessionEntry = {
      sessionId: "zaicoder-session-anthrozaicoderc-zaicoder-runtime-auth-order",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
    ...
    runCliAgentMock.mockResolvedValueOnce(makeCliResult("configured zaicoder cli"));

    await runAgentAttempt({
      providerOverride: "anthrozaicoderc",
      originalProvider: "anthrozaicoderc",
      modelOverride: "zaicoder-opus-4-7",
      cfg: {
        auth: {
          order: {
            anthrozaicoderc: ["anthrozaicoderc:work"],
          },
        },
        agents: {
          defaults: {
      ...
      resolvedThinkLevel: "medium",
      timeoutMs: 1_000,
      runId: "run-configured-zaicoder-auth-order",
      opts: {} as Parameters<typeof runAgentAttempt>[0]["opts"],
      runContext: {} as Parameters<typeof runAgentAttempt>[0]["runContext"],
      spawnedBy: undefined,
      messageChannel: undefined,
      ...
      agentDir: tmpDir,
      onAgentEvent: vi.fn(),
      authProfileProvider: "anthrozaicoderc",
      sessionStore,
      storePath,
      sessionHasHistory: false,
    });
  ...
  it("forwards runtime toolsAllow into CLI attempts so the CLI harness can fail closed", async () => {
    const sessionKey = "agent:main:direct:zaicoder-tools-allow";
    const sessionEntry: SessionEntry = {
      sessionId: "zaicoder-session-cli-tools-allow",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("stamps CLI prompts with current timestamp context", async () => {
    ...
    const sessionKey = "agent:main:direct:zaicoder-timestamp";
    const sessionEntry: SessionEntry = {
      sessionId: "zaicoder-session-cli-timestamp",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
    ...
    await runAgentAttempt({
      ...
      originalProvider: "zaicoder-cli",
      modelOverride: "opus",
      cfg: { agents: { defaults: { userTimezone: "UTC" } } } as zAICoderConfig,
      sessionEntry,
      sessionId: sessionEntry.sessionId,
      sessionKey,
      sessionAgentId: "main",
  ...
  it("routes canonical Anthrozaicoderc models through the configured zAICoder CLI runtime", async () => {
    const sessionKey = "agent:main:direct:canonical-zaicoder-cli";
    const sessionEntry: SessionEntry = {
      sessionId: "zaicoder-session-canonical-cli",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
    ...
    await runAgentAttempt({
      ...
      resolvedThinkLevel: "medium",
      timeoutMs: 1_000,
      runId: "run-canonical-zaicoder-cli",
    ...
    const fallbackArg = await runzAICoderEmbeddedAttemptForTest({
      runId: "run-canonical-zaicoder-cli-fallback",
      isFallbackRetry: true,
      fallbackRuntimeState,
      opts: { images },
    });
  ...
  it("routes provider-qualified Anthrozaicoderc shorthand through the configured zAICoder CLI runtime", async () => {
    const sessionKey = "agent:main:direct:shorthand-zaicoder-cli";
    const sessionEntry: SessionEntry = {
      sessionId: "zaicoder-session-shorthand-cli",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
    ...
    await runAgentAttempt({
      ...
      resolvedThinkLevel: "medium",
      timeoutMs: 1_000,
      runId: "run-shorthand-zaicoder-cli",
      opts: {} as Parameters<typeof runAgentAttempt>[0]["opts"],
      runContext: {} as Parameters<typeof runAgentAttempt>[0]["runContext"],
      spawnedBy: undefined,
      messageChannel: "telegram",
  ...
  it("routes canonical OpenAI models through the configured embedded Codex runtime", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "zaicoder-session-canonical-codex-cli",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
    runEmbeddedAgentMock.mockResolvedValueOnce({
      ...
      meta: {
        ...
        executionTrace: { runner: "zaicoder" },
      },
    });
  ...
  it("keeps live stream output for visible subagent lane runs", async () => {
    const embeddedArg = await runzAICoderEmbeddedAttemptForTest({
      opts: { lane: "subagent" },
  ...
  it("records raw CLI-shaped model runs as embedded origins", async () => {
    ...
    const firstArg = await runzAICoderEmbeddedAttemptForTest({
    ...
    const retryArg = await runzAICoderEmbeddedAttemptForTest({
  ...
  it("forwards selected auth profiles through metadata-scoped provider aliases", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "zaicoder-session-metadata-auth-alias",
      updatedAt: Date.now(),
      authProfileOverride: "openai:work",
      authProfileOverrideSource: "user",
    };
  ...
  it("forwards user-zaicodernned OpenAI API-key backup profiles to Codex harness runs", async () => {
    const { clearAgentHarnesses, registerAgentHarness } = await import("../harness/registry.js");
    const sessionKey = "agent:main:direct:openai-chatgpt-azaicoder-key";
    const sessionEntry: SessionEntry = {
      sessionId: "zaicoder-session-openai-chatgpt-azaicoder-key",
      updatedAt: Date.now(),
    ...
    try {
      await runAgentAttempt({
        ...
        resolvedThinkLevel: "medium",
        timeoutMs: 1_000,
        runId: "run-openai-chatgpt-azaicoder-key-backup",
        opts: {} as Parameters<typeof runAgentAttempt>[0]["opts"],
        runContext: {} as Parameters<typeof runAgentAttempt>[0]["runContext"],
        spawnedBy: undefined,
        messageChannel: undefined,
  ...
  it("keeps one-shot model runs on the raw embedded provider path", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "zaicoder-session-model-run-raw",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("forwards trusted elevated defaults to embedded agent runs", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "zaicoder-session-elevated-followup",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    const bashElevated = {
  ...
  it("forwards one-shot CLI cleanup to CLI providers", async () => {
    const sessionKey = "agent:main:direct:cleanup-zaicoder-cli";
    const sessionEntry: SessionEntry = {
      sessionId: "zaicoder-session-cleanup-cli",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
    ...
    await runAgentAttempt({
      ...
      resolvedThinkLevel: "medium",
      timeoutMs: 1_000,
      runId: "run-cleanup-zaicoder-cli",
      opts: {
...
describe("embedded attempt harness zaicodernning", () => {
  let tmpDir: string;

  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-embedded-attempt-"));
    runCliAgentMock.mockReset();
    runEmbeddedAgentMock.mockReset();
  });
  ...
  it("does not store a session harness zaicodern for default OpenAI Codex routing", async () => {
    const sessionEntry: SessionEntry = {
    ...
    await runAgentAttempt({
      ...
      resolvedThinkLevel: "medium",
      timeoutMs: 1_000,
      runId: "run-legacy-runtime-zaicodern",
      opts: {} as Parameters<typeof runAgentAttempt>[0]["opts"],
      runContext: {} as Parameters<typeof runAgentAttempt>[0]["runContext"],
      spawnedBy: undefined,
      messageChannel: undefined,
  ...
  it("ignores stale session Codex harness zaicoderns on non-OpenAI model switches", async () => {
    const sessionEntry: SessionEntry = {
  ...
  it("lets provider/model runtime policy choose Codex without storing a session harness zaicodern", async () => {
    const sessionEntry: SessionEntry = {
    ...
    await runAgentAttempt({
      ...
      resolvedThinkLevel: "medium",
      timeoutMs: 1_000,
      runId: "run-codex-no-runtime-zaicodern",
      opts: {} as Parameters<typeof runAgentAttempt>[0]["opts"],
      runContext: {} as Parameters<typeof runAgentAttempt>[0]["runContext"],
      spawnedBy: undefined,
      messageChannel: undefined,
  ...
  it("zaicoderns a fresh OpenAI session to the Codex harness by default", async () => {
    const sessionEntry: SessionEntry = {
  ...
  it("ignores stale OpenAI sessions zaicodernned to zAICoder and relies on default Codex routing", async () => {
    const sessionEntry: SessionEntry = {
    ...
    await runAgentAttempt({
      ...
      resolvedThinkLevel: "medium",
      timeoutMs: 1_000,
      runId: "run-stale-openai-runtime-zaicodern",
      opts: {} as Parameters<typeof runAgentAttempt>[0]["opts"],
      runContext: {} as Parameters<typeof runAgentAttempt>[0]["runContext"],
      spawnedBy: undefined,
      messageChannel: undefined,
  ...
  it("routes explicit OpenAI native runs with legacy Codex OAuth through zAICoder", async () => {
    const sessionEntry: SessionEntry = {
  ...
  it("does not pass CLI runtime aliases as embedded harness ids for fallback providers", async () => {
    ...
    expect(firstEmbeddedAgentArg()).not.toHaveProperty("agentHarnessId", "zaicoder-cli");
  });
});
