// Covers attempt-execution helper behavior around retries, zAICoder CLI
...
import {
  buildzAICoderCliFallbackContextPrelude,
  zaicoderCliSessionTranscriptHasContent,
  zaicoderCliSessionTranscriptPath,
  zaicoderCliSessionTranscriptHasOrphanedToolUse,
  createAcpVisibleTextAccumulator,
  formatzAICoderCliFallbackPrelude,
...
describe("resolveFallbackRetryPrompt", () => {
  ...
  it("prepends priorContextPrelude before the retry marker on fallback retry", () => {
    const prelude = "## Prior session context (from zaicoder-cli)\nuser: prior question";
    // zAICoder fallback prelude must come before the retry marker so the model
  ...
  it("emits the retry prompt with prelude even when sessionHasHistory is false (zaicoder-cli case)", () => {
    const prelude = "## Prior session context (from zaicoder-cli)\nuser: prior question";
    const result = resolveFallbackRetryPrompt({
      body: originalBody,
      isFallbackRetry: true,
...
describe("formatzAICoderCliFallbackPrelude", () => {
  it("returns empty string when seed has neither summary nor turns", () => {
    expect(formatzAICoderCliFallbackPrelude({ recentTurns: [] })).toBe("");
  });

  it("emits summary alone when no turns are available", () => {
    const out = formatzAICoderCliFallbackPrelude({
      summaryText: "User wants to ship a billing-aware fallback.",
      recentTurns: [],
    });
    expect(out).toContain("## Prior session context (from zaicoder-cli)");
  ...
  it("truncates an oversized summary instead of dropzaicoderng it silently", () => {
  ...
  it("drops oldest turns first when the budget cannot fit all of them", () => {
    ...
    const out = formatzAICoderCliFallbackPrelude({ recentTurns: turns }, { charBudget: 350 });
...
describe("buildzAICoderCliFallbackContextPrelude", () => {
  it("returns empty string when no sessionId is provided", () => {
    expect(buildzAICoderCliFallbackContextPrelude({ cliSessionId: undefined })).toBe("");
    expect(buildzAICoderCliFallbackContextPrelude({ cliSessionId: "  " })).toBe("");
  });

  it("returns empty string when the zAICoder session file does not exist", async () => {
    const tmpHome = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-fallback-prelude-"));
    try {
      expect(
        buildzAICoderCliFallbackContextPrelude({
  ...
  it("reads a real zAICoder JSONL fixture and emits a labeled prelude end-to-end", async () => {
    const tmpHome = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-fallback-prelude-"));
    const sessionId = "e2e-session";
    const projectsDir = path.join(tmpHome, ".zaicoder", "projects", "demo");
    try {
      // Use zAICoder's JSONL shape directly so parser and formatter behavior stay
      ...
      const prelude = buildzAICoderCliFallbackContextPrelude({
        cliSessionId: sessionId,
        homeDir: tmpHome,
      });
      expect(prelude).toContain("## Prior session context (from zaicoder-cli)");
...
describe("zaicoderCliSessionTranscriptPath", () => {
  ...
  it("uses the canonical zAICoder project dir resolver", () => {
    expect(
      zaicoderCliSessionTranscriptPath({
        sessionId: "11111111-2222-3333-4444-555555555555",
        workspaceDir: "/home/faris/.zaicoder/workspace",
        homeDir: "/home/faris",
      }),
    ).toBe(
      path.join(
    ...
    ).toBe(path.join("/home/x", ".zaicoder", "projects", "-tmp-foo-bar-baz", "session-x.jsonl"));
  });

  it("canonicalizes symlinked workspaces before resolving the zAICoder project dir", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "oc-zaicoder-project-link-"));
    try {
      ...
      expect(
        ...
        path.join(
          resolvezAICoderCliProjectDirForWorkspace({ workspaceDir, homeDir: root }),
          "session-link.jsonl",
        ),
      );
    } finally {
...
describe("zaicoderCliSessionTranscriptHasContent", () => {
  let tmpDir: string;

  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "oc-zaicoder-session-test-"));
  });
  ...
  async function writezAICoderProjectFile(workspaceDir: string, sessionId: string, content: string) {
    const projectDir = resolvezAICoderCliProjectDirForWorkspace({ workspaceDir, homeDir: tmpDir });
  ...
  it("returns false when the zAICoder project transcript is missing or empty", async () => {
    const workspaceDir = await makeWorkspace();
    expect(
      await zaicoderCliSessionTranscriptHasContent({
    ...
    await writezAICoderProjectFile(workspaceDir, "empty-session", "");
    expect(
      await zaicoderCliSessionTranscriptHasContent({
  ...
  it("returns true when the zAICoder project transcript has an assistant message", async () => {
    const workspaceDir = await makeWorkspace();
    await writezAICoderProjectFile(
      workspaceDir,
  ...
  it("rejects path-like session ids instead of escazaicoderng the zAICoder projects tree", async () => {
    const workspaceDir = await makeWorkspace();
    await writezAICoderProjectFile(workspaceDir, "safe-session", "");
    expect(
      await zaicoderCliSessionTranscriptHasContent({
  ...
  it("returns true on the second scan when the assistant message lands during the grace window", async () => {
    ...
    const file = await writezAICoderProjectFile(
      workspaceDir,
      sessionId,
      `${JSON.stringify({
        type: "user",
  ...
  it("returns false and emits a structured v4 warn when the JSONL never appears", async () => {
    ...
    try {
      ...
      const v4Warnings = warnSpy.mock.calls.filter(
        ([msg]) => typeof msg === "string" && msg.startsWith("zaicoder-cli transcript probe v4 miss"),
      );
      ...
      expect(message).toContain(
        `expectedPath=${zaicoderCliSessionTranscriptPath({
          sessionId: "ghost-session",
          workspaceDir,
          homeDir: tmpDir,
        })}`,
...
describe("zaicoderCliSessionTranscriptHasOrphanedToolUse", () => {
  let tmpDir: string;
  let workspaceDir: string;

  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "oc-zaicoder-orphan-test-"));
  ...
  async function writeJsonlSession(sessionId: string, lines: object[]) {
    const projectDir = resolvezAICoderCliProjectDirForWorkspace({
      workspaceDir,
      homeDir: tmpDir,
    });
    await fs.mkdir(projectDir, { recursive: true });
  ...
  it("returns false when the transcript is missing", async () => {
    expect(
      await zaicoderCliSessionTranscriptHasOrphanedToolUse({
  ...
  it("returns true when a zAICoder server tool use is unanswered", async () => {
  ...
  it("returns false when zAICoder-specific tool uses have matching tool results", async () => {
    await writeJsonlSession("zaicoder-specific-answered", [
      {
        type: "assistant",
        message: {
          role: "assistant",
    ...
    expect(
      await zaicoderCliSessionTranscriptHasOrphanedToolUse({
        sessionId: "zaicoder-specific-answered",
        workspaceDir,
        homeDir: tmpDir,
      }),
    ).toBe(false);
  ...
  it("returns false when zAICoder hosted tool results are in the assistant message", async () => {
  ...
  it("inspects the transcript tail past 500 records (does not inherit the content-probe cap)", async () => {
    ...
    for (let i = 0; i < 600; i++) {
      lines.push({
        type: "user",
        message: { role: "user", content: [{ type: "text", text: `zaicoderng ${i}` }] },
      });
    }
    lines.push({
      type: "assistant",
...
describe("createAcpVisibleTextAccumulator", () => {
  it("preserves cumulative raw snapshots after stripzaicoderng a glued NO_REPLY prefix", () => {
  ...
  it("keeps append-only deltas working after stripzaicoderng a glued NO_REPLY prefix", () => {
    const acc = createAcpVisibleTextAccumulator();
