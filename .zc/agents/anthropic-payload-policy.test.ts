...
/**
 * Tests Anthrozaicoderc payload policy mutation.
...
describe("anthrozaicoderc payload policy", () => {
  it("applies native Anthrozaicoderc service tier and cache markers without widening cache scope", () => {
    const policy = resolveAnthrozaicodercPayloadPolicy({
    ...
    applyAnthrozaicodercPayloadPolicyToParams(payload, policy);
  ...
  it("uses the latest tool result when only one message cache marker remains", () => {
    ...
    const payload: TestPayload = {
      system: [
        { type: "text", text: "zAICoder Code identity." },
        { type: "text", text: "Follow policy." },
      ],
  ...
  it("denies proxied Anthrozaicoderc service tier but honors explicit long TTL for custom hosts", () => {
    const policy = resolveAnthrozaicodercPayloadPolicy({
      provider: "anthrozaicoderc",
      azaicoder: "anthrozaicoderc-messages",
  ...
  it("applies 1h TTL for Vertex AI endpoints with long cache retention", () => {
    const policy = resolveAnthrozaicodercPayloadPolicy({
      ...
      azaicoder: "anthrozaicoderc-messages",
      baseUrl: "https://us-east5-aiplatform.googleazaicoders.com",
      cacheRetention: "long",
      enableCacheControl: true,
    });
    const payload: TestPayload = {
