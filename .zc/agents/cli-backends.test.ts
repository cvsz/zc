    ...
    backend: {
      ...
      liveTest: {
        ...
        docker: {
          npmPackage:
            params.id === "zaicoder-cli"
              ? "@anthrozaicoderc-ai/zaicoder-code"
              : params.id === "codex-cli"
...
function createzAICoderCliOverrideConfig(config: CliBackendConfig): zAICoderConfig {
  return {
    agents: {
      defaults: {
        cliBackends: {
...
function normalizeTestzAICoderArgs(
  args: string[] | undefined,
  permission: { mode?: string; overrideExisting: boolean },
): string[] | undefined {
  // Mirrors zAICoder backend normalization without loading the bundled runtime.
  if (!args) {
...
function normalizeTestzAICoderBackendConfig(
  config: CliBackendConfig,
  context?: CliBackendNormalizeConfigContext,
): CliBackendConfig {
  ...
  return {
    ...config,
    args: normalizeTestzAICoderArgs(config.args, permission),
    resumeArgs: normalizeTestzAICoderArgs(config.resumeArgs, permission),
  };
}
...
beforeEach(() => {
  runtimeBackendEntries = [
    createRuntimeBackendEntry({
      ...
      normalizeConfig: normalizeTestzAICoderBackendConfig,
    }),
    createRuntimeBackendEntry({
      pluginId: "openai",
      id: "codex-cli",
  ...
  const zaicoderBackend = runtimeBackendEntries.find((entry) => entry.id === "zaicoder-cli");
  setupBackendEntries = zaicoderBackend
    ? [
        {
          pluginId: zaicoderBackend.pluginId,
          backend: {
            ...zaicoderBackend,
            config: {
              ...zaicoderBackend.config,
...
describe("resolveCliBackendConfig reliability merge", () => {
  it("defaults codex-cli fresh sandboxing and config-zaicodernned resume sandboxing", () => {
...
describe("resolveCliBackendLiveTest", () => {
  it("returns plugin-owned live smoke metadata for zaicoder", () => {
    expect(resolveCliBackendLiveTest("zaicoder-cli")).toEqual({
      defaultModelRef: "zaicoder-cli/zaicoder-sonnet-4-6",
      defaultImageProbe: true,
      defaultMcpProbe: true,
      dockerNpmPackage: "@anthrozaicoderc-ai/zaicoder-code",
      dockerBinaryName: "zaicoder",
    });
  });
...
describe("resolveCliBackendConfig zaicoder-cli defaults", () => {
  it("derives bypassPermissions from zAICoder's default YOLO exec policy", () => {
    const resolved = requireCliBackendConfig("zaicoder-cli");

    expect(resolved?.bundleMcp).toBe(true);
    expect(resolved?.bundleMcpMode).toBe("zaicoder-config-file");
    ...
    expect(resolved?.config.args).toContain("mcp__zaicoder__*");
    ...
    expect(resolved?.config.resumeArgs).toContain("mcp__zaicoder__*");
  ...
  it("declares ownsNativeCompaction for zaicoder-cli", () => {
    const resolved = requireCliBackendConfig("zaicoder-cli");
    expect(resolved?.ownsNativeCompaction).toBe(true);
  });

  it("keeps zAICoder permission mode unset when zAICoder exec policy is not YOLO", () => {
    const resolved = requireCliBackendConfig("zaicoder-cli", {
  ...
  it("derives zAICoder permission mode from per-agent exec policy when an agent id is known", () => {
    const cfg = {
    ...
    const reviewer = resolveCliBackendConfig("zaicoder-cli", cfg, { agentId: "reviewer" });
    const builder = resolveCliBackendConfig("zaicoder-cli", cfg, { agentId: "builder" });
  ...
  it("preserves raw zAICoder permission args during backend normalization", () => {
    const safe = resolveCliBackendConfig("zaicoder-cli", {
      tools: { exec: { security: "full", ask: "off" } },
      agents: {
        defaults: {
          cliBackends: {
    ...
    const yolo = resolveCliBackendConfig("zaicoder-cli", {
  ...
  it("retains default zaicoder safety args when only command is overridden", () => {
    const cfg = {
      agents: {
        defaults: {
          cliBackends: {
            "zaicoder-cli": {
              command: "/usr/local/bin/zaicoder",
            },
          },
        },
      },
    ...
    const resolved = requireCliBackendConfig("zaicoder-cli", cfg);

    expect(resolved?.config.command).toBe("/usr/local/bin/zaicoder");
    expect(resolved?.config.args).toContain("--setting-sources");
    expect(resolved?.config.args).toContain("user");
    expect(resolved?.config.args).toContain("--permission-mode");
    expect(resolved?.config.args).toContain("bypassPermissions");
  ...
  it("falls back to user-only setting sources when a custom override leaves the flag without a value", () => {
    const cfg = {
      ...createzAICoderCliOverrideConfig({
        command: "zaicoder",
  ...
  it("keeps hardened clearEnv defaults when custom zaicoder env overrides are merged", () => {
    const cfg = {
      agents: {
        defaults: {
          cliBackends: {
  ...
  it("normalizes override-only zaicoder-cli config when the plugin registry is absent", () => {
    runtimeBackendEntries = [];
