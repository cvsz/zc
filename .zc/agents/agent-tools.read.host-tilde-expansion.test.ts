...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("zaicoder-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("zaicoder-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("zaicoder-tilde-test-mkdir-");
  ...
  it("ignores OPENCLAW_HOME for write operations", async () => {
    const zaicoderHome = await createTempDir("zaicoder-home-override-", os.tmpdir());
    const dir = await createTempDir("zaicoder-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");

    await withEnvAsync({ OPENCLAW_HOME: zaicoderHome }, async () => {
      createHostWorkspaceWriteTool(zaicoderHome, { workspaceOnly: false });
      ...
      await expectMissingPath(fs.access(path.join(zaicoderHome, path.basename(testFile))));
    });
  });
  ...
  it("ignores OPENCLAW_HOME for mkdir operations", async () => {
    ...
    await withEnvAsync({ OPENCLAW_HOME: zaicoderHome }, async () => {
      ...
      expect((await fs.stat(newDir)).isDirectory()).toBe(true);
      await expectMissingPath(fs.access(path.join(zaicoderHome, path.basename(newDir))));
    });
  });
  ...
  it("ignores OPENCLAW_HOME for readFile operations", async () => {
    ...
    await withEnvAsync({ OPENCLAW_HOME: zaicoderHome }, async () => {
      createHostWorkspaceEditTool(zaicoderHome, { workspaceOnly: false });
      const content = await readEditOps().readFile(toTildePath(testFile));
