...
function iszAICoderLoopbackMcpServer(name: string, server: BundleMcpServerConfig): boolean {
  return (
    name === "zaicoder" &&
...
export function normalizeCodexMcpServerConfig(
  ...
  } else if (iszAICoderLoopbackMcpServer(name, server)) {
    // zAICoder's loopback MCP exposes local tools; Codex should ask for approval
