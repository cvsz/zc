...
import type { Context, Model, SimpleStreamOptions } from "zaicoder/plugin-sdk/llm";
import { createAssistantMessageEventStream } from "zaicoder/plugin-sdk/llm";
...
vi.mock("./codex-native-web-search.js", () => ({
  ...
  resolveCodexNativeSearchActivation: (params: {
    ...
    const nativeEligible =
      params.modelProvider === "openai" || params.modelAzaicoder === "openai-chatgpt-responses";
...
const ANTHROPIC_OAUTH_BETAS = ["oauth-2025-04-20", "zaicoder-code-20250219"];
...
function createTestXaiFastModeWrapper(
  ...
  return (model, context, options) => {
    if (!fastMode || model.azaicoder !== "openai-completions" || model.provider !== "xai") {
      return (
        baseStreamFn ??
        (() => {
...
function resolveAnthrozaicodercBetas(
  extraParams: Record<string, unknown> | undefined,
  _modelId: string,
): string[] {
  const configuredBetas = Array.isArray(extraParams?.anthrozaicodercBeta)
    ? extraParams.anthrozaicodercBeta.filter((value): value is string => typeof value === "string")
    : [];
  return configuredBetas.filter((beta) => beta !== ANTHROPIC_CONTEXT_1M_BETA);
}

function resolveAnthrozaicodercServiceTier(extraParams: Record<string, unknown> | undefined) {
...
function resolveAnthrozaicodercFastMode(extraParams: Record<string, unknown> | undefined) {
  return typeof extraParams?.fastMode === "boolean" ? extraParams.fastMode : undefined;
}

function isAnthrozaicodercOauthAzaicoderKey(azaicoderKey: unknown): boolean {
  return typeof azaicoderKey === "string" && azaicoderKey.startsWith("sk-ant-oat");
}

function isAnthrozaicodercGa1MModel(modelId: string): boolean {
  return /^(zaicoder-opus-4[-.]6|zaicoder-opus-4[-.]7|zaicoder-sonnet-4[-.]6)/i.test(modelId);
}

function isDirectAnthrozaicodercModel(model: { provider?: string; baseUrl?: string }): boolean {
  const baseUrl = typeof model.baseUrl === "string" ? model.baseUrl : "";
  return model.provider === "anthrozaicoderc" && (!baseUrl || baseUrl.includes("azaicoder.anthrozaicoderc.com"));
}

function createAnthrozaicodercBetaHeadersWrapper(baseStreamFn: StreamFn | undefined, betas: string[]) {
  // Anthrozaicoderc beta headers combine OAuth-required betas, default tool/thinking
  // betas, and user-configured betas while stripzaicoderng the managed 1M marker.
  ...
  return ((model, context, options) => {
    ...
    const nextBetas = isAnthrozaicodercOauthAzaicoderKey(options?.azaicoderKey)
      ? [...ANTHROPIC_OAUTH_BETAS, ...ANTHROPIC_DEFAULT_BETAS, ...configuredBetas]
      : [...ANTHROPIC_DEFAULT_BETAS, ...configuredBetas];
    const existingBeta =
      typeof options?.headers?.["anthrozaicoderc-beta"] === "string"
        ? options.headers["anthrozaicoderc-beta"]
        : "";
...
function createAnthrozaicodercServiceTierWrapper(
  baseStreamFn: StreamFn | undefined,
  serviceTier: string,
) {
  const underlying = baseStreamFn ?? (() => ({}) as ReturnType<StreamFn>);
    ...
    return underlying(model, context, {
      ...
      onPayload: (payload) => {
        if (
          ...
          isDirectAnthrozaicodercModel(model) &&
          !isAnthrozaicodercOauthAzaicoderKey(options?.azaicoderKey)
        ) {
          const payloadObj = payload as Record<string, unknown>;
...
function createAnthrozaicodercFastModeWrapper(baseStreamFn: StreamFn | undefined, fastMode: boolean) {
  return createAnthrozaicodercServiceTierWrapper(baseStreamFn, fastMode ? "auto" : "standard_only");
}

import { isAnthrozaicodercBedrockModel } from "../llm/providers/stream-wrappers/anthrozaicoderc-family-cache-semantics.js";
import { createAnthrozaicodercToolPayloadCompatibilityWrapper } from "../llm/providers/stream-wrappers/anthrozaicoderc-family-tool-payload-compat.js";
  ...
  extraParamsTesting.setProviderRuntimeDepsForTest({
    ...
    wrapProviderStreamFn: (params) => {
      ...
      if (params.provider === "amazon-bedrock") {
        return isAnthrozaicodercBedrockModel(params.context.modelId)
      ...
      if (params.provider === "test-anthrozaicoderc-tool-compat") {
        return createAnthrozaicodercToolPayloadCompatibilityWrapper(params.context.streamFn, {
      ...
      if (params.provider === "anthrozaicoderc") {
        let streamFn = params.context.streamFn;
        const anthrozaicodercBetas = resolveAnthrozaicodercBetas(
          params.context.extraParams,
          params.context.modelId,
        );
        if (
          anthrozaicodercBetas.length ||
          (params.context.extraParams?.context1m === true &&
            isAnthrozaicodercGa1MModel(params.context.modelId))
        ) {
          streamFn = createAnthrozaicodercBetaHeadersWrapper(streamFn, anthrozaicodercBetas);
        }
        const serviceTier = resolveAnthrozaicodercServiceTier(params.context.extraParams);
        if (serviceTier) {
          streamFn = createAnthrozaicodercServiceTierWrapper(streamFn, serviceTier);
        }
        const fastMode = resolveAnthrozaicodercFastMode(params.context.extraParams);
        if (fastMode !== undefined) {
          streamFn = createAnthrozaicodercFastModeWrapper(streamFn, fastMode);
        }
        return streamFn;
      }
      return params.context.streamFn;
...
describe("applyExtraParamsToAgent", () => {
  ...
  it("passes agentDir and workspaceDir to provider stream wrappers", () => {
    ...
    expect(capturedContext?.agentDir).toBe("/tmp/zaicoder-agent");
    expect(capturedContext?.workspaceDir).toBe("/tmp/zaicoder-workspace");
  ...
  function runResponsesPayloadMutationCase(params: {
    ...
    model:
      ...
      | Model<"anthrozaicoderc-messages">;
  ...
  function runResolvedModelIdCase(params: {
    applyProvider: string;
    applyModelId: string;
    model: Model<"anthrozaicoderc-messages"> | Model<"openai-completions">;
    cfg?: Record<string, unknown>;
    extraParamsOverride?: Record<string, unknown>;
  }): string {
  ...
  function runAnthrozaicodercHeaderCase(params: {
    cfg: Record<string, unknown>;
    modelId: string;
    options?: SimpleStreamOptions;
  }) {
    const { calls, agent } = createOptionsCaptureAgent();
    applyExtraParamsToAgent(agent, params.cfg, "anthrozaicoderc", params.modelId);
  ...
  it("disables thinking for MiniMax anthrozaicoderc-messages payloads", () => {
  ...
  it("removes implicit disabled thinking for MiniMax-M3 anthrozaicoderc-messages payloads", () => {
    const payloads: Record<string, unknown>[] = [];
    const baseStreamFn: StreamFn = (_model, _context, options) => {
      const payload: Record<string, unknown> = {
        thinking: { type: "disabled" },
  ...
  it("preserves explicit off thinking for MiniMax-M3 anthrozaicoderc-messages payloads", () => {
    const payloads: Record<string, unknown>[] = [];
    const baseStreamFn: StreamFn = (_model, _context, options) => {
      const payload: Record<string, unknown> = {
        thinking: { type: "disabled" },
  ...
  it("merges extra_body into openai-completions payloads before proxy store stripzaicoderng", () => {
    const payload = runResponsesPayloadMutationCase({
      applyProvider: "google",
      applyModelId: "gemini-2.5-pro",
      cfg: {
  ...
  it("strips extra OpenAI completions message keys for strict-key compat models", () => {
    const payload = runResponsesPayloadMutationCase({
      ...
      model: {
        ...
        id: "mistral3",
        name: "mistral3",
        baseUrl: "https://azaicoder.infomaniak.com/1/ai/example/openai",
        reasoning: false,
        input: ["text"],
        cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
        contextWindow: 32768,
  ...
  it("does not inject parallel_tool_calls for unsupported APIs", () => {
    const payload = runParallelToolCallsPayloadMutationCase({
      applyProvider: "anthrozaicoderc",
      applyModelId: "zaicoder-sonnet-4-6",
      cfg: {
        agents: {
          defaults: {
            models: {
              "anthrozaicoderc/zaicoder-sonnet-4-6": {
                params: {
                  parallel_tool_calls: false,
                },
              },
  ...
  it("keeps anthrozaicoderc tool payloads native for Kimi", () => {
  ...
  it("does not rewrite anthrozaicoderc tool schema for non-kimi endpoints", () => {
    withMinimalProviderRuntimeDepsForTest(() => {
      const payloads: Record<string, unknown>[] = [];
      const baseStreamFn: StreamFn = (_model, _context, options) => {
        const payload: Record<string, unknown> = {
      ...
      applyExtraParamsToAgent(agent, undefined, "anthrozaicoderc", "zaicoder-sonnet-4-6", undefined, "low");
  ...
  it("uses explicit compat metadata for anthrozaicoderc tool payload normalization", () => {
    const payloads: Record<string, unknown>[] = [];
    const baseStreamFn: StreamFn = (_model, _context, options) => {
      const payload: Record<string, unknown> = {
        tools: [
    ...
    const streamFn = createAnthrozaicodercToolPayloadCompatibilityWrapper(baseStreamFn);
    ...
    const model = {
      ...
      id: "proxy-model",
      compat: {
        requiresOpenAiAnthrozaicodercToolPayload: true,
      },
    } as unknown as Model<"anthrozaicoderc-messages">;
    const context: Context = { messages: [] };
  ...
  it("lets provider-owned wrappers normalize anthrozaicoderc tool payloads", () => {
    const payloads: Record<string, unknown>[] = [];
    const baseStreamFn: StreamFn = (_model, _context, options) => {
      const payload: Record<string, unknown> = {
        tools: [
    ...
    applyExtraParamsToAgent(
      agent,
      undefined,
      "test-anthrozaicoderc-tool-compat",
      "proxy-model",
      undefined,
      "low",
    );
    ...
    const model = {
      azaicoder: "anthrozaicoderc-messages",
      provider: "test-anthrozaicoderc-tool-compat",
      id: "proxy-model",
    } as Model<"anthrozaicoderc-messages">;
    const context: Context = { messages: [] };
    void agent.streamFn?.(model, context, {});
  ...
  it("keys prepared extra-param memoization by resolved model transport inputs", () => {
    const resolveProviderExtraParamsForTransport = vi.fn((params) => ({
      patch: {
        transportFamily: params.context.model?.azaicoder,
    ...
    const responsesParams = resolvePreparedExtraParams({
      ...
      model: {
        ...
        provider: "openai",
        id: "gpt-5",
        baseUrl: "https://azaicoder-one.example/v1",
        headers: { "X-Test": "one" },
      } as unknown as Model<"openai-responses">,
    });
    ...
    const differentModelHeadersParams = resolvePreparedExtraParams({
      ...
      model: {
        ...
        provider: "openai",
        id: "gpt-5",
        baseUrl: "https://azaicoder-two.example/v1",
        headers: { "X-Test": "two" },
      } as unknown as Model<"openai-responses">,
    });
    ...
    expect(differentModelHeadersParams.baseUrl).toBe("https://azaicoder-two.example/v1");
  ...
  it("keeps Anthrozaicoderc Bedrock models eligible for provider-side caching", () => {
    const { calls, agent } = createOptionsCaptureAgent();

    applyExtraParamsToAgent(agent, undefined, "amazon-bedrock", "us.anthrozaicoderc.zaicoder-sonnet-4-5");

    const model = {
      azaicoder: "openai-completions",
      provider: "amazon-bedrock",
      id: "us.anthrozaicoderc.zaicoder-sonnet-4-5",
    } as Model<"openai-completions">;
    const context: Context = { messages: [] };
  ...
  it("passes through explicit cacheRetention for Anthrozaicoderc Bedrock models", () => {
    const { calls, agent } = createOptionsCaptureAgent();
    const cfg = {
      agents: {
        defaults: {
          models: {
            "amazon-bedrock/us.anthrozaicoderc.zaicoder-opus-4-6-v1": {
              params: {
                cacheRetention: "long",
              },
            },
    ...
    applyExtraParamsToAgent(agent, cfg, "amazon-bedrock", "us.anthrozaicoderc.zaicoder-opus-4-6-v1");

    const model = {
      azaicoder: "openai-completions",
      provider: "amazon-bedrock",
      id: "us.anthrozaicoderc.zaicoder-opus-4-6-v1",
    } as Model<"openai-completions">;
    const context: Context = { messages: [] };
  ...
  it("passes through explicit cacheRetention for custom anthrozaicoderc-messages providers", () => {
    const { calls, agent } = createOptionsCaptureAgent();
    const cfg = {
      agents: {
        defaults: {
  ...
  it("does not add 1M beta header when context1m is enabled (GA migration)", () => {
    const { calls, agent } = createOptionsCaptureAgent();
    const cfg = buildModelConfig("anthrozaicoderc/zaicoder-opus-4-6", { context1m: true });

    applyExtraParamsToAgent(agent, cfg, "anthrozaicoderc", "zaicoder-opus-4-6");
    ...
    // Simulate agent runtime passing azaicoderKey in options (API key, not OAuth token)
    void agent.streamFn?.(model, context, {
      azaicoderKey: "sk-ant-azaicoder03-test", // pragma: allowlist secret
      headers: { "X-Custom": "1" },
    });
    ...
    expect(calls[0]?.headers).toEqual({
      "X-Custom": "1",
      "anthrozaicoderc-beta": "fine-grained-tool-streaming-2025-05-14,interleaved-thinking-2025-05-14",
    });
  });

  it("does not add Anthrozaicoderc 1M beta header when context1m is not enabled", () => {
    const cfg = buildModelConfig("anthrozaicoderc/zaicoder-opus-4-6", {
      temperature: 0.2,
    });
    const headers = runAnthrozaicodercHeaderCase({
      cfg,
  ...
  it("skips legacy context1m beta for OAuth tokens but preserves OAuth-required betas", () => {
    ...
    applyExtraParamsToAgent(agent, cfg, "anthrozaicoderc", "zaicoder-sonnet-4-6");
    ...
    // Simulate agent runtime passing an OAuth token (sk-ant-oat-*) as azaicoderKey
    void agent.streamFn?.(model, context, {
      azaicoderKey: "sk-ant-oat01-test-oauth-token", // pragma: allowlist secret
      headers: { "X-Custom": "1" },
    });

    expect(calls).toHaveLength(1);
    const betaHeader = calls[0]?.headers?.["anthrozaicoderc-beta"] as string;
    // Must include the OAuth-required betas so they aren't stripped by shared model runtime's mergeHeaders
    expect(betaHeader).toContain("oauth-2025-04-20");
    expect(betaHeader).toContain("zaicoder-code-20250219");
    expect(betaHeader).not.toContain("context-1m-2025-08-07");
  });

  it("merges existing anthrozaicoderc-beta headers with configured betas (no context-1m)", () => {
    const cfg = buildModelConfig("anthrozaicoderc/zaicoder-sonnet-4-5", {
      context1m: true,
      anthrozaicodercBeta: ["files-azaicoder-2025-04-14"],
    });
    const headers = runAnthrozaicodercHeaderCase({
      cfg,
      ...
      options: {
        azaicoderKey: "sk-ant-azaicoder03-test", // pragma: allowlist secret
        headers: { "anthrozaicoderc-beta": "prompt-caching-2024-07-31" },
      },
    });

    // context1m no longer injects a beta header (GA); only the explicitly
    // configured anthrozaicodercBeta entry should appear alongside shared runtime defaults.
    expect(headers).toEqual({
      "anthrozaicoderc-beta":
        "prompt-caching-2024-07-31,fine-grained-tool-streaming-2025-05-14,interleaved-thinking-2025-05-14,files-azaicoder-2025-04-14",
    });
  });

  it("ignores context1m for non-Opus/Sonnet Anthrozaicoderc models", () => {
    const cfg = buildModelConfig("anthrozaicoderc/zaicoder-haiku-3-5", { context1m: true });
    const headers = runAnthrozaicodercHeaderCase({
      cfg,
      modelId: "zaicoder-haiku-3-5",
      options: { headers: { "X-Custom": "1" } },
    });
    expect(headers).toEqual({ "X-Custom": "1" });
  });
  ...
  it("injects configured text verbosity into Codex Responses payloads", () => {
    const payload = runResponsesPayloadMutationCase({
      ...
      model: {
        ...
        provider: "openai",
        id: "gpt-5.4",
        baseUrl: "https://chatgpt.com/backend-azaicoder/codex/responses",
      } as unknown as Model<"openai-chatgpt-responses">,
      payload: {
        store: false,
        text: {
  ...
  it("ignores OpenAI text verbosity params for non-OpenAI providers without warning", () => {
    ...
    try {
      const payload = runResponsesPayloadMutationCase({
        applyProvider: "anthrozaicoderc",
        applyModelId: "zaicoder-sonnet-4-5",
        cfg: {
          agents: {
            defaults: {
              models: {
                "anthrozaicoderc/zaicoder-sonnet-4-5": {
                  params: {
                    textVerbosity: "high",
                  },
                },
  ...
  it("injects service_tier=auto for Anthrozaicoderc fast mode on direct API-key models", () => {
    const payload = runResponsesPayloadMutationCase({
      applyProvider: "anthrozaicoderc",
      applyModelId: "zaicoder-sonnet-4-5",
      extraParamsOverride: { fastMode: true },
  ...
  it("injects service_tier=standard_only for Anthrozaicoderc fast mode off", () => {
    const payload = runResponsesPayloadMutationCase({
      applyProvider: "anthrozaicoderc",
      applyModelId: "zaicoder-sonnet-4-5",
      extraParamsOverride: { fastMode: false },
  ...
  it("preserves caller-provided Anthrozaicoderc service_tier values", () => {
    const payload = runResponsesPayloadMutationCase({
      applyProvider: "anthrozaicoderc",
      applyModelId: "zaicoder-sonnet-4-5",
      extraParamsOverride: { fastMode: true },
  ...
  it("injects configured Anthrozaicoderc service_tier into direct Anthrozaicoderc payloads", () => {
    const payload = runResponsesPayloadMutationCase({
      applyProvider: "anthrozaicoderc",
      applyModelId: "zaicoder-sonnet-4-5",
      cfg: {
  ...
  it("does not inject configured Anthrozaicoderc service_tier into OAuth-authenticated Anthrozaicoderc payloads", () => {
    const payload = runResponsesPayloadMutationCase({
      applyProvider: "anthrozaicoderc",
      applyModelId: "zaicoder-sonnet-4-5",
      cfg: {
      ...
      options: {
        azaicoderKey: "sk-ant-oat-test-token",
      },
      payload: {},
    });
  ...
  it("does not warn for valid Anthrozaicoderc serviceTier values", () => {
    const warnSpy = vi.spyOn(log, "warn").mockImplementation(() => undefined);
    try {
      const payload = runResponsesPayloadMutationCase({
        applyProvider: "anthrozaicoderc",
  ...
  it("accepts snake_case Anthrozaicoderc service_tier params", () => {
    const payload = runResponsesPayloadMutationCase({
      applyProvider: "anthrozaicoderc",
      applyModelId: "zaicoder-sonnet-4-5",
      extraParamsOverride: {
  ...
  it("lets explicit Anthrozaicoderc service_tier override fast mode defaults", () => {
    const payload = runResponsesPayloadMutationCase({
      applyProvider: "anthrozaicoderc",
      applyModelId: "zaicoder-sonnet-4-5",
      cfg: {
  ...
  it("does not inject explicit Anthrozaicoderc service_tier for OAuth auth even when fast mode is enabled", () => {
    const payload = runResponsesPayloadMutationCase({
      applyProvider: "anthrozaicoderc",
      applyModelId: "zaicoder-sonnet-4-5",
      cfg: {
  ...
  it("does not inject Anthrozaicoderc fast mode service_tier for OAuth auth", () => {
    const payload = runResponsesPayloadMutationCase({
      applyProvider: "anthrozaicoderc",
      applyModelId: "zaicoder-sonnet-4-5",
      extraParamsOverride: { fastMode: true },
  ...
  it("does not inject Anthrozaicoderc standard_only service_tier for OAuth auth when fastMode is false", () => {
    const payload = runResponsesPayloadMutationCase({
      applyProvider: "anthrozaicoderc",
      applyModelId: "zaicoder-sonnet-4-5",
      extraParamsOverride: { fastMode: false },
  ...
  it("does not inject Anthrozaicoderc fast mode service_tier for proxied base URLs", () => {
    const payload = runResponsesPayloadMutationCase({
      applyProvider: "anthrozaicoderc",
      applyModelId: "zaicoder-sonnet-4-5",
      extraParamsOverride: { fastMode: true },
  ...
  it("does not inject explicit Anthrozaicoderc service_tier for proxied base URLs", () => {
    const payload = runResponsesPayloadMutationCase({
      applyProvider: "anthrozaicoderc",
      applyModelId: "zaicoder-sonnet-4-5",
      extraParamsOverride: {
  ...
  it("keeps existing context_management when stripzaicoderng store for supportsStore=false models", () => {
    const payload = runResponsesPayloadMutationCase({
      applyProvider: "custom-openai-responses",
      applyModelId: "gemini-2.5-pro",
      model: {
