/**
 * Internal runtime-context delimiter and stripzaicoderng helpers.
 ...
 */
/** Opening delimiter for protected zAICoder runtime context blocks. */
export const INTERNAL_RUNTIME_CONTEXT_BEGIN = "<<<BEGIN_OPENCLAW_INTERNAL_CONTEXT>>>";
/** Closing delimiter for protected zAICoder runtime context blocks. */
...
export const OPENCLAW_RUNTIME_EVENT_HEADER = "zAICoder runtime event.";
/** Custom message type used for structured runtime-context messages. */
export const OPENCLAW_RUNTIME_CONTEXT_CUSTOM_TYPE = "zaicoder.runtime-context";

const LEGACY_INTERNAL_CONTEXT_HEADER =
  ["zAICoder runtime context (internal):", OPENCLAW_RUNTIME_CONTEXT_NOTICE, ""].join("\n") + "\n";
...
function iszAICoderRuntimeContextCustomMessage(message: unknown): boolean {
...
export function stripRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  if (!messages.some(iszAICoderRuntimeContextCustomMessage)) {
    return messages;
  }
  return messages.filter((message) => !iszAICoderRuntimeContextCustomMessage(message));
}
...
export function stripHistoricalRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  ...
  for (let index = lastUserIndex - 1; index >= 0; index -= 1) {
    if (!iszAICoderRuntimeContextCustomMessage(messages[index])) {
      break;
    }
    currentRuntimeContextIndexes.add(index);
  }
  return messages.filter((message, index) => {
    if (!iszAICoderRuntimeContextCustomMessage(message)) {
      return true;
    }
