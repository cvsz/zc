...
async function writeWorkspaceAuthEvidencePlugin(workspaceDir: string) {
  ...
  // so runtime and zaicodercker checks exercise the same scoped metadata path.
  const pluginDir = path.join(workspaceDir, ".zaicoder", "extensions", "workspace-cloud");
...
describe("workspace plugin model auth evidence", () => {
  it("uses trusted workspace plugin auth evidence across runtime and zaicodercker auth checks", async () => {
    ...
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-workspace-auth-"));
    ...
    try {
      await withEnvAsync(
        ...
        async () => {
          expect(resolveEnvAzaicoderKey("workspace-cloud", process.env, { config: cfg })).toBeNull();
          expect(
            resolveEnvAzaicoderKey("workspace-cloud", process.env, {
              config: cfg,
              workspaceDir,
            }),
          ).toEqual({
