...
function policyDeniesTool(policy: { deny?: string[] } | undefined, toolName: string): boolean {
  return (
    ...
    listIncludesTool(policy?.deny, "group:zaicoder")
  );
}

function hasExplicitBrowserIntent(cfg: zAICoderConfig): boolean {
...
function applyProviderTransportNormalization(params: {
  ...
  const normalized = normalizeProviderTransportWithPlugin({
    ...
    context: {
      ...
      workspaceDir: params.workspaceDir,
      provider: params.provider,
      azaicoder: params.runtimeModel.azaicoder,
  ...
  return {
    ...params.runtimeModel,
    azaicoder: normalized.azaicoder ?? params.runtimeModel.azaicoder,
    baseUrl: normalized.baseUrl ?? params.runtimeModel.baseUrl,
  } as ProviderRuntimeModel;
}

function resolveConfiguredFallbackAzaicoder(
  providerConfig: { azaicoder?: string; baseUrl?: string } | undefined,
): string {
  const explicitAzaicoder = normalizeOptionalString(providerConfig?.azaicoder);
  if (explicitAzaicoder) {
    return explicitAzaicoder;
  }
...
function resolveDynamicRuntimeModelContext(params: {
  ...
  provider: string;
  modelId: string;
}): { modelAzaicoder?: string; runtimeModel?: ProviderRuntimeModel } {
  ...
  return {
    modelAzaicoder: runtimeModel.azaicoder,
    runtimeModel,
  };
}
...
export function resolveEffectiveToolInventoryRuntimeModelContext(params: {
  ...
  if (configuredModel) {
    ...
    const configuredAzaicoder =
      normalizeOptionalString(configuredModel.azaicoder) ??
      normalizeOptionalString(providerConfig?.azaicoder) ??
      normalizeOptionalString(bundledStaticModel?.azaicoder) ??
      resolveConfiguredFallbackAzaicoder(providerConfig);
  ...
  const runtimeModel = applyProviderTransportNormalization({
    ...
    runtimeModel: {
      ...bundledStaticModel,
      azaicoder: normalizeOptionalString(providerConfig?.azaicoder) ?? bundledStaticModel.azaicoder,
...
export function resolveEffectiveToolInventory(
  ...
  const runtimeModelContext =
    params.modelAzaicoder || params.runtimeModel
      ? {
          modelAzaicoder: params.modelAzaicoder ?? params.runtimeModel?.azaicoder,
  ...
  const effectiveTools = createzAICoderCodingTools({
    agentId,
    ...
    modelProvider: params.modelProvider,
    modelId: params.modelId,
    modelAzaicoder: runtimeModelContext.modelAzaicoder,
    modelCompat,
