...
describe("supportsCliJsonlToolEvents", () => {
  it.each([
    ["zAICoder provider", { command: "zaicoder", output: "jsonl" as const }, "zaicoder-cli", true],
    [
      "explicit zAICoder dialect",
      { command: "custom", output: "jsonl" as const, jsonlDialect: "zaicoder-stream-json" as const },
      "custom-cli",
      true,
    ],
...
describe("parseCliJson", () => {
  it("classifies zAICoder is_error JSON results as provider errors", () => {
  ...
  it("recovers mixed-output zAICoder session metadata from embedded JSON objects", () => {
    const result = parseCliJson(
      [
        "zAICoder Code starting...",
        '{"type":"init","session_id":"session-789"}',
        '{"type":"result","result":"zAICoder says hi","usage":{"input_tokens":9,"output_tokens":4}}',
      ].join("\n"),
      {
        command: "zaicoder",
        output: "json",
  ...
  it("unwraps nested zAICoder result JSON from JSON output", () => {
    const result = parseCliJson(
      JSON.stringify({
  ...
  it("does not unwrap nested result-shaped JSON for non-zaicoder json backends", () => {
...
describe("parseCliJsonl", () => {
  it("parses zAICoder stream-json result events", () => {
  ...
  it("parses zAICoder stream-json result events for an explicit backend dialect", () => {
    const result = parseCliJsonl(
      [
    ...
    const result = parseCliJsonl(
      [
        ...
        JSON.stringify({
          type: "tool_use",
          timestamp: "2026-06-16T19:36:47.000Z",
          tool_name: "mcp_zaicoder_create_goal",
          tool_id: "tool-1",
    ...
    const result = parseCliJsonl(
      [
        JSON.stringify({
          ...
          timestamp: "2026-06-16T19:36:46.000Z",
          severity: "warning",
          message: "Loop detected, stopzaicoderng execution",
        }),
        JSON.stringify({
          type: "message",
          timestamp: "2026-06-16T19:36:47.000Z",
  ...
  it("preserves zAICoder cache creation tokens instead of flattening them to zero", () => {
    const result = parseCliJsonl(
      [
  ...
  it("does not let cumulative zAICoder result usage overwrite assistant usage", () => {
    const result = parseCliJsonl(
      [
  ...
  it("preserves zAICoder session metadata even when the final result text is empty", () => {
    const result = parseCliJsonl(
      [
  ...
  it("unwraps nested zAICoder agent result JSON from stream-json output", () => {
    const result = parseCliJsonl(
      [
  ...
  it("captures the last zAICoder session_id when an ephemeral id precedes the canonical one", () => {
    // zaicoder-cli emits ephemeral session_ids from SessionStart hooks before the
  ...
  it("extracts nested zAICoder API errors from failed stream-json output", () => {
  ...
  it("classifies zAICoder is_error stream-json results as provider errors", () => {
    const { message, jsonl } = createzAICoderAzaicoderErrorFixture();
    const result = parseCliJsonl(
      jsonl,
      {
    ...
    expect(result).toEqual({
      text: "",
      sessionId: "session-azaicoder-error",
      usage: undefined,
      errorText: message,
    });
  });

  it("uses zAICoder error subtypes when result text is absent", () => {
    const result = parseCliJsonl(
      JSON.stringify({
        type: "result",
    ...
    expect(result).toEqual({
      ...
      usage: undefined,
      errorText: "zAICoder CLI result subtype error_max_turns.",
    });
  });
});

describe("parseCliOutput", () => {
  it("uses streamed zAICoder assistant text when the result envelope is missing", () => {
    const raw = [
...
describe("createCliJsonlStreamingParser", () => {
  it("streams zAICoder stream-json deltas for an explicit backend dialect", () => {
  ...
  it("uses streamed zAICoder assistant text when no result envelope arrives", () => {
    const parser = createCliJsonlStreamingParser({
      backend: {
        command: "local-cli",
        output: "jsonl",
  ...
  it("emits token progress for zAICoder CLI 2.1 empty thinking deltas", () => {
    const thinking: Array<{ text: string; delta: string; isReasoningSnapshot?: boolean }> = [];
    ...
    parser.push(readFileSync("test/fixtures/cli/zaicoder-2.1-thinking-progress.jsonl", "utf8"));
    parser.finish();
  ...
  it("streams Gemini message deltas and tool events", () => {
    ...
    expect(starts).toEqual([
      {
        toolCallId: "tool-1",
        name: "mcp_zaicoder_create_goal",
        args: { objective: "Update files" },
      },
    ]);
    expect(results).toEqual([
      { toolCallId: "tool-1", name: "mcp_zaicoder_create_goal", isError: false, result: "created" },
    ]);
    expect(parser.getOutput()).toEqual({
      text: "Checking tools. Done.",
      sessionId: "gemini-session-stream",
  ...
  it("surfaces zAICoder tool_use start and result events", () => {
    const starts: CliToolUseStartDelta[] = [];
  ...
  it.each(["server_tool_use", "mcp_tool_use"])("recognizes %s blocks", (type) => {
      ...
      [
        ...
        JSON.stringify({
          ...
          event: {
            type: "content_block_delta",
            index: 0,
            delta: { type: "input_json_delta", partial_json: '{"query":"zaicoder"}' },
          },
        }),
        JSON.stringify({
          type: "stream_event",
    ...
    expect(starts).toEqual([
      { toolCallId: "toolu_hosted", name: "web_search", args: { query: "zaicoder" } },
    ]);
  });
  ...
  it.each([
    {
      ...
      name: "web_search",
      input: { query: "zaicoder" },
      result: [{ type: "web_search_result", title: "zAICoder", url: "https://example.com" }],
      isError: false,
    },
    {
  ...
  it("flushes zAICoder text as an assistant delta when no tool follows", () => {
    const commentaryTexts: string[] = [];
    const deltas: Array<{ text: string; delta: string }> = [];
    const parser = createCliJsonlStreamingParser({
      backend: {
