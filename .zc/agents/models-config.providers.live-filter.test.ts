...
function resolveOwners(provider: string): readonly string[] | undefined {
  return provider === "zaicoder-cli" ? ["anthrozaicoderc"] : undefined;
}
...
describe("resolveProviderDiscoveryFilterForTest", () => {
    ...
    expect(
      resolveProviderDiscoveryFilterForTest({
        env: liveFilterEnv({
          ...
          OPENCLAW_LIVE_PROVIDERS: "zaicoder-cli",
        }),
        resolveOwners,
      }),
    ).toEqual(["anthrozaicoderc"]);
  });
  ...
  it("honors gateway live provider filters too", () => {
    expect(
      resolveProviderDiscoveryFilterForTest({
        env: liveFilterEnv({
          OPENCLAW_LIVE_TEST: "1",
          OPENCLAW_LIVE_GATEWAY_PROVIDERS: "zaicoder-cli",
        }),
        resolveOwners,
      }),
    ).toEqual(["anthrozaicoderc"]);
  ...
  it("maps live provider backend ids through plugin metadata cli backend owners", () => {
    const snapshot = {
      owners: metadataOwners({
        cliBackends: new Map([["zaicoder-cli", ["anthrozaicoderc"]]]),
      }),
    };
    ...
    expect(resolveMetadataOwners("zaicoder-cli")).toEqual(["anthrozaicoderc"]);
    expect(
      resolveProviderDiscoveryFilterForTest({
        env: liveFilterEnv({
          OPENCLAW_LIVE_TEST: "1",
  ...
  it("normalizes mixed-case backend ids through plugin metadata owners", () => {
    ...
    expect(resolvePluginMetadataProviderOwnersForTest(snapshot, "zAICoder-CLI")).toEqual([
      "anthrozaicoderc",
    ]);
    expect(
      resolveProviderDiscoveryFilterForTest({
  ...
  it("scopes normal startup discovery to requested provider owners", () => {
    const snapshot = {
      owners: metadataOwners({
        providers: new Map([
          ["openai", ["openai"]],
          ["anthrozaicoderc", ["anthrozaicoderc"]],
        ]),
      }),
    };
