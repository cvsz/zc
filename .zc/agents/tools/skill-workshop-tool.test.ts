...
let testState: zAICoderTestState;
let stateDir = "";

beforeEach(async () => {
  testState = await createzAICoderTestState({
    layout: "state-only",
    prefix: "zaicoder-skill-workshop-state-",
  });
...
describe("skill_workshop tool", () => {
  it("is exposed in the zAICoder tool set", async () => {
    const workspaceDir = await tempDirs.make("zaicoder-skill-workshop-tool-");
  ...
  it("is not exposed from sandboxed zAICoder tool sets", async () => {
    const workspaceDir = await tempDirs.make("zaicoder-skill-workshop-tool-");
    const tools = createzAICoderTools({
      workspaceDir,
      config: {},
  ...
  it("scopes proposal discovery to the tool workspace", async () => {
    const firstWorkspaceDir = await tempDirs.make("zaicoder-skill-workshop-tool-first-");
    const secondWorkspaceDir = await tempDirs.make("zaicoder-skill-workshop-tool-second-");
