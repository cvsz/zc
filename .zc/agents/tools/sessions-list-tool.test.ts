...
describe("sessions-list-tool", () => {
  ...
  it("keeps deliveryContext.threadId in sessions_list results", async () => {
    // Thread/tozaicoderc ids are required for channel-specific follow-up routing, so
      ...
      if (request.method === "sessions.list") {
        return {
          ...
          sessions: [
            ...
            {
              key: "agent:main:telegram:tozaicoderc",
              kind: "direct",
              sessionId: "sess-telegram-tozaicoderc",
              deliveryContext: {
                channel: "telegram",
                to: "telegram:tozaicoderc",
                accountId: "acct-2",
                threadId: 271,
              },
            },
    ...
    mocks.gatewayCall.mockImplementation(async (opts: unknown) => {
      ...
      if (request.method === "sessions.list") {
        return {
          ...
          sessions: [
            {
              key: "agent:main:telegram:group:-100123:tozaicoderc:99",
              kind: "group",
              sessionId: "sess-telegram-tozaicoderc",
              deliveryContext: {
                channel: "telegram",
