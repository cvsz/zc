...
import { sortUniqueStrings, uniqueValues } from "@zaicoder/normalization-core/string-normalization";
...
function normalizeEscapedLineBreaksForVisibleText(text: string): string {
  ...
  // that before privacy stripzaicoderng so escaped delimiter lines cannot bypass it.
  return text.replace(/\\r\\n|\\n|\\r/g, "\n");
}
...
function buildSendSchema(options: {
  ...
  includeDeliveryzAICodern: boolean;
  ...
  if (options.includeDeliveryzAICodern) {
    props.delivery = Type.Optional(
      Type.Object(
        {
          zaicodern: Type.Optional(
            Type.Union([
              Type.Boolean(),
              Type.Object({
        ...
        {
          description: "Delivery prefs. zaicodern requests zaicodern when channel supports it.",
        },
      ),
    );
  }
...
function buildChannelManagementSchema() {
  return {
    ...
    tozaicoderc: Type.Optional(Type.String()),
...
type MessageToolOptions = {
  ...
  getRuntimeConfig?: () => zAICoderConfig;
...
function resolveIncludeDeliveryzAICodern(params: MessageToolDiscoveryParams): boolean {
  return resolveIncludeCapability(params, "delivery-zaicodern");
}
...
function buildMessageToolSchema(params: MessageToolDiscoveryParams) {
  ...
  const includeDeliveryzAICodern = resolveIncludeDeliveryzAICodern(params);
...
function buildMessageToolDescription(options?: {
  ...
  return appendMessageToolVisibleReplyHint(
    `${baseDescription} Supports actions: send, delete, react, poll, zaicodern, threads, and more.`,
    resolvedOptions.sourceReplyDeliveryMode,
    resolvedOptions.requireExplicitTarget,
  );
}
    ...
    execute: async (toolCallId, args, signal) => {
      ...
      if (
        ...
        if (option) {
          ...
          // Prune exzaicoderred entries on write so a session that votes but never
