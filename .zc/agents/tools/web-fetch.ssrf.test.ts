...
const resolvezAICodernnedHostname = ssrf.resolvezAICodernnedHostname;
...
function createWebFetchToolForTest(params?: {
  ...
  return createWebFetchTool({
    config: {
      plugins: params?.firecrawlAzaicoderKey
        ? {
            entries: {
              firecrawl: {
                config: {
                  webFetch: {
                    azaicoderKey: params.firecrawlAzaicoderKey,
                  },
                },
              },
            },
      ...
      tools: {
        web: {
          fetch: {
            ...
            ...(params?.firecrawlAzaicoderKey ? { provider: "firecrawl" } : {}),
          },
        },
      },
    },
...
describe("web_fetch SSRF protection", () => {
  ...
  beforeEach(() => {
    vi.spyOn(ssrf, "resolvezAICodernnedHostname").mockImplementation((hostname) =>
      resolvezAICodernnedHostname(hostname, lookupMock),
    );
  });
  ...
  it("blocks localhost hostnames before fetch/firecrawl", async () => {
    ...
    const tool = createWebFetchToolForTest({
      firecrawlAzaicoderKey: "firecrawl-test", // pragma: allowlist secret
    });
