...
import { isRecord as isPlainObject } from "@zaicoder/normalization-core/record-coerce";
import {
...
const ALLOWED_GATEWAY_CONFIG_PATHS = [
  ...
  // adapters use today — Telegram tozaicoderc overrides live at
  // `channels.telegram.groups.<group>.tozaicodercs.<tozaicoderc>.requireMention`.
...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `zaicoder security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as zAICoderConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as zAICoderConfig);
...
// because zAICoder API on Vertex AI rejects nested anyOf schemas as invalid JSON Schema.
...
// - zAICoder/Vertex has other JSON Schema quirks.
