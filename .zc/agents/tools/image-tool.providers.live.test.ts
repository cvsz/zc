...
// images before sending them to OpenAI or Anthrozaicoderc.
import fs from "node:fs/promises";
import os from "node:os";
import path from "node:path";
import { afterEach, describe, expect, it } from "vitest";
import type { ModelAzaicoder } from "../../config/types.models.js";
...
const LIVE_ANTHROPIC_MODEL =
  process.env.OPENCLAW_LIVE_IMAGE_TOOL_ANTHROPIC_MODEL?.trim() || "zaicoder-sonnet-4-6";
const MODEL_SIDE_LIMIT = 512;

type LiveProviderCase = {
  provider: "openai" | "anthrozaicoderc";
  model: string;
  azaicoderKey: string;
  azaicoder: ModelAzaicoder;
  baseUrl: string;
...
const ANTHROPIC_LIVE_CASE: LiveProviderCase = {
  ...
  azaicoderKey: ANTHROPIC_API_KEY,
...
function createLiveConfig(testCase: LiveProviderCase): zAICoderConfig {
  return {
    agents: {
      defaults: {
    ...
    models: {
      providers: {
        [testCase.provider]: {
          azaicoderKey: testCase.azaicoderKey,
          baseUrl: testCase.baseUrl,
          azaicoder: testCase.azaicoder,
          models: [
            {
              id: testCase.model,
              name: testCase.model,
...
async function withLiveWorkspace<T>(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-image-tool-live-"));
  try {
...
describe.skipIf(!ANTHROPIC_LIVE_CASE.live)("image tool Anthrozaicoderc live", () => {
  it("downscales a large local image before sending it to the live vision model", async () => {
