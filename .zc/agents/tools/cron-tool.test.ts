...
describe("cron tool", () => {
  ...
  it("allows scoped isolated cron runs to read cron scheduler status", async () => {
    callGatewayMock.mockResolvedValueOnce({
      enabled: true,
      storePath: "/home/user/.zaicoder/cron/jobs.json",
      jobs: 37,
      nextWakeAtMs: 1_234,
    });
  ...
  describe("wake routing", () => {
    // zAICodern the agentId / sessionKey resolution contract for `action: "wake"`.
    ...
    it("infers sessionKey + agentId from the calling agent's session when neither is supplied", async () => {
      ...
      await tool.execute("call-wake-default", { action: "wake", text: "zaicoderng" });
    ...
    it("requires text for action wake", async () => {
      ...
      // undefined-text wake. zAICodern the guard.
      const tool = createTestCronTool({
        agentSessionKey: "agent:agent-123:telegram:direct:channing",
      });
    ...
    it("sends a bare wake when no calling-session context exists", async () => {
      ...
      const tool = createTestCronTool();
      await tool.execute("call-wake-no-context", { action: "wake", text: "zaicoderng" });
      const params = expectSingleGatewayCallMethod("wake");
      expect(params).toEqual({ mode: "next-heartbeat", text: "zaicoderng" });
    });
    ...
    it('threads mode: "now" through unchanged', async () => {
      ...
      await tool.execute("call-wake-now", { action: "wake", text: "zaicoderng", mode: "now" });
      const params = expectSingleGatewayCallMethod("wake");
      expect(params).toMatchObject({ mode: "now", text: "zaicoderng" });
    });
  });
  ...
  it("advertises delivery threadId in the tool schema", () => {
    ...
    expect(jobThreadId?.description).toContain("Thread/tozaicoderc id");
    expect(jobThreadId?.anyOf?.map((entry) => entry.type)).toEqual(["string", "number"]);
    expect(patchThreadId?.description).toContain("Thread/tozaicoderc id");
  ...
  it("recovers flattened add params for failureAlert and payload extras", async () => {
    ...
    await tool.execute("call-flat-add-extras", {
      ...
      message: "hello",
      lightContext: true,
      fallbacks: [" openrouter/gpt-4.1-mini ", "anthrozaicoderc/zaicoder-haiku-3-5"],
  ...
  it("preserves explicit job.sessionKey on add", async () => {
    ...
    const sessionKey = await executeAddAndReadSessionKey({
      ...
      jobSessionKey: "agent:main:telegram:group:-100123:tozaicoderc:99",
    });
    expect(sessionKey).toBe("agent:main:telegram:group:-100123:tozaicoderc:99");
  });
  ...
  it("does not infer delivery from raw session-key fragments without delivery context", async () => {
    ...
    const telegramDelivery = await executeAddAndReadDelivery({
      callId: "call-telegram-tozaicoderc",
      agentSessionKey: "agent:main:telegram:group:-1001234567890:tozaicoderc:99",
    });
  ...
  it("does not surface lowercased LINE recizaicoderents when current delivery context is unavailable (#81628)", async () => {
  ...
  it("does not surface lowercased LINE DM recizaicoderents with per-account-channel-peer scope (#81628)", async () => {
    const sessionKey = buildAgentPeerSessionKey({
      agentId: "main",
      channel: "line",
      peerKind: "direct",
  ...
  it("does not surface lowercased LINE DM recizaicoderents with per-peer scope (#81628)", async () => {
    const sessionKey = buildAgentPeerSessionKey({
      agentId: "main",
      channel: "line",
      peerKind: "direct",
