...
const ANTHROPIC_PDF_MODEL = "anthrozaicoderc/zaicoder-opus-4-8";
const TEST_AGENT_DIR = "/tmp/zaicoder-pdf-model-config";
...
vi.mock("./model-config.helpers.js", () => ({
  ...
  hasProviderAuthForTool: ({ provider, cfg }: { provider: string; cfg?: zAICoderConfig }) => {
    const providerCfg = cfg?.models?.providers?.[provider] as { azaicoderKey?: string } | undefined;
    if (providerCfg?.azaicoderKey?.trim()) {
      return true;
    }
  ...
  resolveDefaultModelRef: (cfg?: zAICoderConfig) => {
    const modelCfg = cfg?.agents?.defaults?.model;
    const primary =
      (typeof modelCfg === "string"
        ? modelCfg
        : (modelCfg as { primary?: string } | undefined)?.primary) ?? "anthrozaicoderc/zaicoder-sonnet-4-5";
    const [provider = "anthrozaicoderc", model = "zaicoder-sonnet-4-5"] = primary.split("/", 2);
...
describe("resolvePdfModelConfigForTool", () => {
  ...
  it("prefers anthrozaicoderc when available for native PDF support", () => {
  ...
  it("uses anthrozaicoderc primary when provider is anthrozaicoderc", () => {
    vi.stubEnv("ANTHROPIC_API_KEY", "anthrozaicoderc-test");
