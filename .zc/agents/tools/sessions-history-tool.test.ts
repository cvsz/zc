...
function readMessageSeq(message: unknown): number | undefined {
  ...
  const meta = (message as Record<string, unknown>)["__zaicoder"];
...
describe("sessions_history redaction", () => {
  beforeAll(async () => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zaicoder-sessions-history-redact-"));
  ...
  it("uses the oldest visible message for pagination after tool messages are filtered", async () => {
    const tool = createSessionsHistoryTool({
      ...
      callGateway: async <T = Record<string, unknown>>(): Promise<T> =>
        ({
          messages: [
            { role: "tool", content: "hidden", __zaicoder: { seq: 6 } },
            { role: "assistant", content: "visible", __zaicoder: { seq: 7 } },
            { role: "assistant", content: "latest", __zaicoder: { seq: 8 } },
          ],
          offset: 0,
          nextOffset: 5,
          hasMore: true,
