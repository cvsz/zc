// Gateway call helper tests zaicodern URL override, token, and RPC scope behavior for
...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    zaicoder: process.env.OPENCLAW_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.zaicoder === undefined) {
      delete process.env.OPENCLAW_GATEWAY_TOKEN;
    } else {
      process.env.OPENCLAW_GATEWAY_TOKEN = envSnapshot.zaicoder;
    }
  ...
  it("marks local wake calls from trusted tool context with agent runtime identity", async () => {
    ...
    await withGatewayToolCallerIdentity(
      { agentId: "ops", sessionKey: "agent:ops:telegram:direct:alice" },
      async () => {
        await callGatewayTool("wake", {}, { mode: "now", text: "zaicoderng" });
      },
    );
  ...
  it("explains stale gateway cron connection metadata rejections", async () => {
    ...
    await expect(
      ...
      "The running Gateway is from an older zAICoder build and rejected current agent cron connection metadata. Restart the Gateway with `zaicoder gateway restart`, then retry.",
    );
