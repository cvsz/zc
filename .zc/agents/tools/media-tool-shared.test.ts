...
describe("resolveMediaToolLocalRoots", () => {
  it("does not widen default local roots from media sources", () => {
    const stateDir = path.join("/tmp", "zaicoder-media-tool-roots-state");
    const zaicodercturesDir =
      process.platform === "win32" ? "C:\\Users\\peter\\zAICoderctures" : "/Users/peter/zAICoderctures";
    const moviesDir =
    ...
    const roots = withEnv({ OPENCLAW_STATE_DIR: stateDir }, () =>
      resolveMediaToolLocalRoots(path.join(stateDir, "workspace-agent"), undefined, [
        path.join(zaicodercturesDir, "photo.png"),
    ...
    expect(normalizedRoots).not.toContain(normalizeHostPath(zaicodercturesDir));
  ...
  it("keeps channel inbound attachment roots separate from local roots", () => {
    ...
    const accountRoot = path.join("/tmp", "zaicoder-imessage-work");
    const sharedRoot = path.join("/tmp", "zaicoder-imessage-shared");
    const cfg = {
      channels: {
        imessage: {
...
describe("resolveModelFromRegistry", () => {
  ...
  it("falls back to provider-prefixed custom model IDs", () => {
    ...
    const foundModel = { provider: "kimchi", id: "kimchi/zaicoder-opus-4-6" };
    const { calls, registry } = createModelRegistryStub((_, modelId) =>
      modelId === "kimchi/zaicoder-opus-4-6" ? foundModel : null,
    );
    ...
    expect(calls).toEqual([
      ["kimchi", "zaicoder-opus-4-6"],
      ["kimchi", "kimchi/zaicoder-opus-4-6"],
    ]);
    expect(result).toBe(foundModel);
  }, 180_000);
});
