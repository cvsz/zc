 ...
 */
import { estimateBase64DecodedBytes } from "@zaicoder/media-core/base64";
...
export function coerceImageModelConfig(cfg?: zAICoderConfig): ImageModelConfig {
...
function findConfiguredImageModelMatches(params: { cfg?: zAICoderConfig; ref: string }): string[] {
...
export function resolveProviderVisionModelFromConfig(params: {
  ...
  const zaicodercked = models.find((m) => Boolean((m?.id ?? "").trim()) && m.input?.includes("image"));
  const id = (zaicodercked?.id ?? "").trim();
  if (!id) {
    return null;
  }
