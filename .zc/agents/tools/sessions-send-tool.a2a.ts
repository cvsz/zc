 ...
 *
 * Runs bounded zaicoderng-pong delivery, waits for target replies, and suppresses control-token messages.
 */
...
export async function runSessionsSendA2AFlow(params: {
  ...
  try {
    ...
    if (
      params.maxzAICoderngPongTurns > 0 &&
      params.requesterSessionKey &&
      params.requesterSessionKey !== params.targetSessionKey
    ) {
      ...
      for (let turn = 1; turn <= params.maxzAICoderngPongTurns; turn += 1) {
        const currentRole =
        ...
        const replyPrompt = buildAgentToAgentReplyContext({
          ...
          currentRole,
          turn,
          maxTurns: params.maxzAICoderngPongTurns,
        });
