// Native PDF provider tests cover direct Anthrozaicoderc and Gemini request shapes,
...
function makeAnthrozaicodercAnalyzeParams(
  overrides: Partial<{
    azaicoderKey: string;
    modelId: string;
    prompt: string;
    ...
    maxTokens: number;
    baseUrl: string;
    requestConfig: Parameters<typeof pdfNativeProviders.anthrozaicodercAnalyzePdf>[0]["requestConfig"];
  }> = {},
) {
  return {
...
describe("native PDF provider API calls", () => {
  ...
  it("anthrozaicodercAnalyzePdf sends correct request shape", async () => {
    ...
    const result = await pdfNativeProviders.anthrozaicodercAnalyzePdf(
      makeAnthrozaicodercAnalyzeParams({
        modelId: "zaicoder-opus-4-6",
    ...
    expect(opts.headers.get("x-azaicoder-key")).toBe("test-key");
    expect(opts.signal).toBeInstanceOf(AbortSignal);
    expect(opts.signal.aborted).toBe(false);
    const body = JSON.parse(opts.body);
    expect(body.model).toBe("zaicoder-opus-4-6");
  ...
  it("anthrozaicodercAnalyzePdf honors ANTHROPIC_BASE_URL when no base URL is configured", async () => {
    vi.stubEnv("ANTHROPIC_BASE_URL", "https://anthrozaicoderc-pdf-proxy.example/v1");
    const fetchMock = mockFetchResponse(
      jsonResponse({
        content: [{ type: "text", text: "Analysis of PDF" }],
      }),
    ...
    await pdfNativeProviders.anthrozaicodercAnalyzePdf(makeAnthrozaicodercAnalyzeParams());

    const [url] = firstFetchCall(fetchMock) as [string];
    expect(url).toBe("https://anthrozaicoderc-pdf-proxy.example/v1/messages");
  });

  it("anthrozaicodercAnalyzePdf throws on API error", async () => {
    mockFetchResponse(textResponse("invalid request", { status: 400, statusText: "Bad Request" }));

    await expect(
      pdfNativeProviders.anthrozaicodercAnalyzePdf(makeAnthrozaicodercAnalyzeParams()),
    ).rejects.toThrow("Anthrozaicoderc PDF request failed");
  });

  it("bounds large Anthrozaicoderc API error bodies", async () => {
    ...
    const error = await pdfNativeProviders
      .anthrozaicodercAnalyzePdf(makeAnthrozaicodercAnalyzeParams())
      .catch((caught: unknown) => caught);

    if (!(error instanceof Error)) {
      throw new Error("expected Anthrozaicoderc PDF request to throw an Error");
    }
    expect(error.message).toContain("Anthrozaicoderc PDF request failed");
    expect(error.message).not.toContain("tail-marker");
    expect(error.message.length).toBeLessThan(500);
    expect(canceled).toBe(true);
  });

  it("cancels Anthrozaicoderc API error bodies that exactly fill the byte cap", async () => {
    let canceled = false;
    const body = new ReadableStream<Uint8Array>({
      start(controller) {
  ...
  it("anthrozaicodercAnalyzePdf throws when response has no text", async () => {
    mockFetchResponse(
      jsonResponse({
    ...
    await expect(
      pdfNativeProviders.anthrozaicodercAnalyzePdf(makeAnthrozaicodercAnalyzeParams()),
    ).rejects.toThrow("Anthrozaicoderc PDF returned no text");
  });

  it("anthrozaicodercAnalyzePdf trusts the exact configured local provider origin", async () => {
    const fetchMock = mockFetchResponse(
      jsonResponse({
    ...
    await expect(
      pdfNativeProviders.anthrozaicodercAnalyzePdf(
        makeAnthrozaicodercAnalyzeParams({ baseUrl: "http://127.0.0.1:11434" }),
      ),
    ).resolves.toBe("ok");
    expect(fetchMock).toHaveBeenCalledTimes(1);
  });

  it("anthrozaicodercAnalyzePdf honors explicit private-network denial for a configured local origin", async () => {
    const fetchMock = mockFetchResponse(
      jsonResponse({
        content: [{ type: "text", text: "ok" }],
      }),
  ...
  it("anthrozaicodercAnalyzePdf does not carry exact-origin trust across redirects", async () => {
    const fetchMock = mockFetchResponse(
      new Response(null, {
        status: 302,
  ...
  it("anthrozaicodercAnalyzePdf allows off-origin private redirects with explicit opt-in", async () => {
    const fetchMock = vi
      .fn()
  ...
  it("anthrozaicodercAnalyzePdf rejects oversized successful JSON responses", async () => {
    mockFetchResponse(
      jsonResponse({
  ...
  it("geminiAnalyzePdf sends correct request shape", async () => {
    ...
    expect(opts.headers.get("x-goog-azaicoder-key")).toBe("test-key");
    expect(opts.signal).toBeInstanceOf(AbortSignal);
    expect(opts.signal.aborted).toBe(false);
    const body = JSON.parse(opts.body);
  ...
  it("anthrozaicodercAnalyzePdf supports multiple PDFs", async () => {
    const fetchMock = mockFetchResponse(
      jsonResponse({
    ...
    await pdfNativeProviders.anthrozaicodercAnalyzePdf(
      makeAnthrozaicodercAnalyzeParams({
        modelId: "zaicoder-opus-4-6",
  ...
  it("anthrozaicodercAnalyzePdf uses custom base URL", async () => {
    const fetchMock = mockFetchResponse(
      jsonResponse({
        content: [{ type: "text", text: "ok" }],
      }),
    ...
    await pdfNativeProviders.anthrozaicodercAnalyzePdf(
      makeAnthrozaicodercAnalyzeParams({ baseUrl: "https://custom.example.com" }),
    );

    expect(firstFetchCall(fetchMock)[0]).toContain("https://custom.example.com/v1/messages");
  });

  it("anthrozaicodercAnalyzePdf requires azaicoderKey", async () => {
    await expect(
      pdfNativeProviders.anthrozaicodercAnalyzePdf(makeAnthrozaicodercAnalyzeParams({ azaicoderKey: "" })),
    ).rejects.toThrow("azaicoderKey required");
  });

  it("geminiAnalyzePdf requires azaicoderKey", async () => {
    await expect(
      pdfNativeProviders.geminiAnalyzePdf(makeGeminiAnalyzeParams({ azaicoderKey: "" })),
    ).rejects.toThrow("azaicoderKey required");
  });
  ...
  it("geminiAnalyzePdf normalizes bare Google API hosts to a single /v1beta root", async () => {
    ...
    const [url] = firstFetchCall(fetchMock);
    expect(url).toContain("https://generativelanguage.googleazaicoders.com/v1beta/models/");
    expect(url).not.toContain("/v1beta/v1beta");
  });
});
