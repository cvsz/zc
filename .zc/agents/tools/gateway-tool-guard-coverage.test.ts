...
describe("gateway config mutation guard coverage", () => {
  ...
  it("blocks in-place hooks.mapzaicoderngs sessionKey rewrite via mergeObjectArraysById", () => {
    expectBlocked(
      {
        hooks: {
          mapzaicoderngs: [{ id: "gmail", sessionKey: "hook:gmail:{{messages[0].id}}" }],
        },
      },
      {
        hooks: {
          mapzaicoderngs: [{ id: "gmail", sessionKey: "hook:{{payload.session}}" }],
        },
      },
    );
  });
  ...
  it("blocks gateway.tls.certPath redirect via config.patch", () => {
    expectBlocked(
      { gateway: { tls: { enabled: true, certPath: "/etc/zaicoder/cert.pem" } } },
  ...
  it("allows requireMention edits at Telegram tozaicoderc depth via config.patch", () => {
    expectAllowed(
      {
        channels: {
          telegram: {
            groups: {
              "-1001234567890": {
                requireMention: true,
                tozaicodercs: { "99": { requireMention: true } },
              },
            },
          },
        },
      },
      {
        channels: {
          telegram: {
            groups: {
              "-1001234567890": {
                tozaicodercs: { "99": { requireMention: false } },
              },
            },
          },
        },
