...
const detachedTaskRuntimeMocks = vi.hoisted(() => ({
  ...
  createRunningTaskRun: vi.fn(() => ({ taskId: "task-zaicodernned-route" })),
...
describe("createMediaGenerationTaskLifecycle", () => {
  it("zaicoderns a missing requester target from session state when the task starts", async () => {
  ...
  it("does not zaicodern a session target from another account", () => {
    subagentAnnounceDeliveryMocks.loadRequesterSessionEntry.mockReturnValue({
      entry: {
        lastChannel: "telegram",
        lastTo: "peer-b",
  ...
  it("does not zaicodern a stored thread from a different requester target", () => {
    subagentAnnounceDeliveryMocks.loadRequesterSessionEntry.mockReturnValue({
      entry: {
        lastChannel: "telegram",
        lastTo: "room-b",
  ...
  it("zaicoderns the external session route for an internal requester origin", () => {
    subagentAnnounceDeliveryMocks.loadRequesterSessionEntry.mockReturnValue({
      entry: {
        lastChannel: "telegram",
        lastTo: "room-a",
