...
function hasExplicitDefaultPrimaryModel(cfg?: zAICoderConfig): boolean {
 ...
 *   - fall back to OpenAI/Anthrozaicoderc when available
 */
...
function zaicoderckMaxBytes(cfg?: zAICoderConfig, maxBytesMb?: number): number | undefined {
...
function imageCompressionPolicyHasDimensionLimit(policy: ImageCompressionModelPolicy): boolean {
  return typeof policy.maxSidePx === "number" || typeof policy.maxzAICoderxels === "number";
}
...
async function runImagePrompt(params: {
  ...
  const providerCfg: zAICoderConfig = effectiveCfg ?? {};
...
export function createImageTool(options?: {
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      const maxBytes = zaicoderckMaxBytes(options?.config, maxBytesMb);
