...
describe("sessions_spawn tool", () => {
  ...
  it("uses subagent runtime by default", async () => {
    ...
    expect(spawnArgs.model).toBe("anthrozaicoderc/zaicoder-sonnet-4-6");
  ...
  it("accepts ACP spawns when inherited allows include zAICoder command tools", async () => {
    registerAcpBackendForTest();
    const tool = createSessionsSpawnTool({
      agentSessionKey: "agent:main:main",
  ...
  it("forwards model override to ACP runtime spawns", async () => {
    ...
    await tool.execute("call-2-model", {
      ...
      task: "investigate the failing CI run",
      agentId: "codex",
      model: "github-cozaicoderlot/zaicoder-sonnet-4.6",
    });

    const spawnArgs = mockCallArg(hoisted.spawnAcpDirectMock, 0, 0, "spawnAcpDirect");
    expect(spawnArgs.task).toBe("investigate the failing CI run");
    expect(spawnArgs.agentId).toBe("codex");
    expect(spawnArgs.model).toBe("github-cozaicoderlot/zaicoder-sonnet-4.6");
  });
