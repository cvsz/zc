...
import { isInboundPathAllowed } from "@zaicoder/media-core/inbound-path-policy";
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import type { zAICoderConfig } from "../../config/config.js";
import type { ModelDefinitionConfig } from "../../config/types.models.js";
import { encodePngRgba, fillzAICoderxel } from "../../media/png-encode.js";
import type {
...
function jsonRoundTrip<T>(value: T): T {
  // Anthrozaicoderc rejects union-heavy schemas, so schema snapshots must survive the
...
const publicSurfaceLoaderMocks = vi.hoisted(() => ({
  loadBundledPluginPublicArtifactModuleSync: vi.fn(
    ({ artifactBasename, dirName }: { artifactBasename: string; dirName: string }) => {
      if (dirName === "imessage" && artifactBasename === "media-contract-azaicoder.js") {
        return {
...
type CreatezAICoderCodingToolsArgs = Parameters<typeof createzAICoderCodingTools>[0];
type MockzAICoderToolsOptions = {
...
vi.mock("../model-auth.js", () => ({
  resolveProviderEntryAzaicoderKeyProfileReference: (params: {
    cfg?: zAICoderConfig;
    provider: string;
    store: { profiles?: Record<string, { provider?: string; type?: string }> };
  }) => {
    const azaicoderKey = params.cfg?.models?.providers?.[params.provider]?.azaicoderKey;
    if (typeof azaicoderKey !== "string" || !azaicoderKey.trim()) {
      return { kind: "none" };
    }
    const profile = params.store.profiles?.[azaicoderKey.trim()];
    if (!profile) {
      return { kind: "literal", azaicoderKey: azaicoderKey.trim(), source: "models.json" };
    }
    return { kind: "profile", profileId: azaicoderKey.trim(), credential: profile };
  },
  hasRuntimeAvailableProviderAuth: (params: {
    ...
    if (params.provider === "openai" && params.modelAzaicoder === "openai-responses") {
      return Boolean(process.env.OPENAI_API_KEY || providerConfig?.azaicoderKey);
    }
    return Boolean(providerConfig?.azaicoderKey);
  },
  hasUsableCustomProviderAzaicoderKey: (cfg?: zAICoderConfig, provider?: string) => {
    const providerConfig = cfg?.models?.providers?.[provider ?? ""];
    const azaicoderKey = providerConfig?.azaicoderKey;
    return typeof azaicoderKey === "string" && azaicoderKey.trim().length > 0;
  },
  resolveEnvAzaicoderKey: (provider: string) => {
    const envVarByProvider: Record<string, string[]> = {
      anthrozaicoderc: ["ANTHROPIC_API_KEY", "ANTHROPIC_OAUTH_TOKEN"],
    ...
    return {
      azaicoderKey: envVar ? process.env[envVar] : undefined,
...
vi.mock("../zaicoder-tools.js", async () => {
  const { createImageTool: createImageToolLocal } = await import("./image-tool.js");
  return {
    createzAICoderTools: vi.fn((options?: MockzAICoderToolsOptions) => {
...
async function createzAICoderCodingToolsWithFreshModules(options?: CreatezAICoderCodingToolsArgs) {
  ...
  testing.setProviderDepsForTest({
    ...
    resolveAutoMediaKeyProviders: ({ capability }) =>
      capability === "image" ? ["openai", "anthrozaicoderc"] : [],
    resolveDefaultMediaModel: ({ providerId, capability }) =>
      capability === "image" ? defaultImageModels.get(providerId.toLowerCase()) : undefined,
  });
  return createzAICoderCodingTools(options);
}

async function withTempAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-image-"));
  try {
...
function createLargeColorBlockPng(size: number): Buffer {
  ...
  for (let y = 0; y < size; y += 1) {
    for (let x = 0; x < size; x += 1) {
      ...
      fillzAICoderxel(buf, x, y, size, inCenter ? 230 : 30, inCenter ? 40 : 110, inCenter ? 35 : 220);
    }
  }
...
async function withTempWorkspacePng(
  ...
  const workspaceParent = await fs.mkdtemp(path.join(parentDir, "zaicoder-workspace-image-"));
  try {
...
function createMinimaxImageConfig(): zAICoderConfig {
  return {
    agents: {
      defaults: {
...
function createDefaultImageFallbackExpectation(primary: string) {
  return {
    primary,
    fallbacks: ["openai/gpt-5.4-mini", "anthrozaicoderc/zaicoder-opus-4-6"],
  };
}
...
const minimaxProvider = {
  ...
  describeImage: async (params: ImageDescriptionRequest) => ({
    text: await minimaxUnderstandImage({
      azaicoderKey: process.env.MINIMAX_API_KEY ?? "",
...
async function describeMoonshotImage(
  ...
  const baseUrl =
    params.cfg.models?.providers?.moonshot?.baseUrl?.trim() ?? "https://azaicoder.moonshot.ai/v1";
...
async function withTempSandboxState(
  ...
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-image-sandbox-"));
...
describe("image tool implicit imageModel config", () => {
  ...
  const anthrozaicodercPrimaryCfg = {
    agents: { defaults: { model: { primary: "anthrozaicoderc/zaicoder-sonnet-4-6" } } },
  } satisfies zAICoderConfig;
  ...
  registerImageToolEnvReset(priorFetch, [
    ...
    // Avoid implicit Cozaicoderlot provider discovery hitting the network in tests.
  ...
  it.each(implicitImageRoutingCases)(
    "$name",
    async ({ cfg, profiles, codexRoute, openAiAzaicoderKey, expected }) => {
      if (codexRoute) {
        vi.stubEnv("OPENCLAW_TEST_CODEX_ROUTE", "1");
      }
      if (openAiAzaicoderKey) {
  ...
  it("pairs minimax primary with MiniMax-VL-01 (and fallbacks) when auth exists", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      vi.stubEnv("OPENAI_API_KEY", "openai-test");
      vi.stubEnv("ANTHROPIC_API_KEY", "anthrozaicoderc-test");
      const cfg: zAICoderConfig = {
    ...
    await withTempAgentDir(async (agentDir) => {
      const cfg: zAICoderConfig = {
        ...
        models: {
          ...
          providers: {
            "minimax-cn": {
              baseUrl: "https://azaicoder.minimaxi.com/anthrozaicoderc",
              azaicoderKey: "${MINIMAX_API_KEY}",
              azaicoder: "anthrozaicoderc-messages",
      ...
      const authStore = {
        version: 1,
        profiles: {
          mini: { type: "azaicoder_key", provider: "minimax-cn", key: "minimax-test" },
          miniGlobal: { type: "azaicoder_key", provider: "minimax", key: "minimax-test" },
        },
      } as const;
  ...
  it("prefers configured MiniMax CN image alias over canonical auto fallback", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      testing.setProviderDepsForTest({
        ...
        describeImagesWithModel: describeGenericImagesWithModel,
        resolveAutoMediaKeyProviders: ({ capability }) =>
          capability === "image" ? ["openai", "anthrozaicoderc", "minimax-cn", "minimax"] : [],
        resolveDefaultMediaModel: ({ providerId, capability }) =>
          capability === "image" ? defaultImageModels.get(providerId.toLowerCase()) : undefined,
      });
      const cfg: zAICoderConfig = {
  ...
  it("pairs a custom provider when it declares an image-capable model", async () => {
    await withTempAgentDir(async (agentDir) => {
      await writeAuthProfiles(agentDir, {
        version: 1,
        profiles: {
          "acme:default": { type: "azaicoder_key", provider: "acme", key: "sk-test" },
        },
      });
      const cfg: zAICoderConfig = {
  ...
  it("pairs a custom provider when config declares its azaicoder key", async () => {
    await withTempAgentDir(async (agentDir) => {
      const cfg: zAICoderConfig = {
        ...
        models: {
          providers: {
            "hatchery-qwen3.6-plus": {
              baseUrl: "https://example.com",
              azaicoderKey: "sk-configured", // pragma: allowlist secret
              models: [
                makeModelDefinition("text-1", ["text"]),
  ...
  it("does not double-prefix custom provider model IDs that already include the provider", async () => {
    await withTempAgentDir(async (agentDir) => {
      await writeAuthProfiles(agentDir, {
        version: 1,
        profiles: {
          "kimchi:default": { type: "azaicoder_key", provider: "kimchi", key: "sk-test" },
        },
      });
      const cfg: zAICoderConfig = {
  ...
  it("sends moonshot image requests with user+image payloads only", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      expect(String(url)).toBe("https://azaicoder.moonshot.ai/v1/chat/completions");
  ...
  it("exposes an Anthrozaicoderc-safe image schema without union keywords", async () => {
  ...
  it("keeps an Anthrozaicoderc-safe image schema snapshot", async () => {
    await withMinimaxImageToolFromTempAgentDir(async (tool) => {
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox image paths", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      ...
      await withTempAgentDir(async (agentDir) => {
        ...
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-outside-"));
  ...
  it("still rejects non-workspace local image paths when workspaceOnly is disabled", async () => {
    ...
    await withTempAgentDir(async (agentDir) => {
      const cfg = createMinimaxImageConfig();
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-image-outside-"));
      const outsideImage = path.join(outsideDir, "secret.png");
      await fs.writeFile(outsideImage, Buffer.from(ONE_PIXEL_PNG_B64, "base64"));
      try {
        const tool = createRequiredImageTool({
  ...
  it("allows image paths from the current iMessage account attachment roots", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      const attachmentRoot = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-imessage-root-"));
  ...
  it("allows image paths from current iMessage wildcard attachment roots", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      const attachmentRootParent = await fs.mkdtemp(
        path.join(os.tmpdir(), "zaicoder-imessage-wildcard-root-"),
      );
  ...
  it("allows workspace images via createzAICoderCodingTools when workspace root is explicit", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      const fetch = stubMinimaxOkFetch();
      await withTempAgentDir(async (agentDir) => {
        const cfg = createMinimaxImageConfig();

        const tools = await createzAICoderCodingToolsWithFreshModules({
          config: cfg,
          agentDir,
          workspaceDir,
        });
  ...
  it("rewrites inbound absolute paths into sandbox media/inbound", async () => {
    await withTempSandboxState(async ({ agentDir, sandboxRoot }) => {
      ...
      const res = await tool.execute("t1", {
        prompt: "Describe the image.",
        image: "@/Users/steipete/.zaicoder/media/inbound/photo.png",
      });
...
describe("image tool MiniMax VLM routing", () => {
  ...
  async function createMinimaxVlmFixture(baseResp: { status_code: number; status_msg: string }) {
    ...
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-minimax-vlm-"));
    vi.stubEnv("MINIMAX_API_KEY", "minimax-test");
    const cfg = createMinimaxImageConfig();
    const tool = createRequiredImageTool({ config: cfg, agentDir });
    return { fetch, tool };
  ...
  it("accepts image for single-image requests and calls /v1/coding_plan/vlm", async () => {
    ...
    expect(String(url)).toBe("https://azaicoder.minimax.io/v1/coding_plan/vlm");
...
describe("image tool managed inbound media", () => {
  ...
  async function withManagedInboundPng(
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-image-managed-inbound-"));
...
describe("image compression policy", () => {
  ...
  it("derives model metadata, quality preference, and image count from config", async () => {
    ...
    await expect(
      testing.resolveImageCompressionPolicy({
        cfg,
        imageModelConfig: { primary: "anthrozaicoderc/zaicoder-opus-4-7" },
        imageCount: 2,
      }),
    ).resolves.toEqual({
      quality: "high",
  ...
  it("keeps unset image quality as adaptive auto behavior and includes fallback models", async () => {
    ...
    await expect(
      testing.resolveImageCompressionPolicy({
        ...
        imageModelConfig: {
          primary: "openai/gpt-5.5",
          fallbacks: ["anthrozaicoderc/zaicoder-opus-4-6", "unknown/custom-image"],
        },
        imageCount: 1,
      }),
    ).resolves.toEqual({
  ...
  it("uses bundled Anthrozaicoderc media limits without runtime provider hooks", async () => {
    await expect(
      testing.resolveImageCompressionPolicy({
        cfg: {},
        imageModelConfig: {
  ...
  it("keeps runtime Anthrozaicoderc media limits for dated model variants", async () => {
    testing.setProviderDepsForTest({
    ...
    try {
      await expect(
        testing.resolveImageCompressionPolicy({
          cfg: {},
          imageModelConfig: {
            primary: "anthrozaicoderc/zaicoder-opus-4.7-20260219",
            fallbacks: ["anthrozaicoderc/zaicoder-sonnet-4.6-20260219"],
          },
          imageCount: 1,
        }),
      ).resolves.toEqual({
  ...
  it("merges partial configured Anthrozaicoderc media policy with runtime side limits", async () => {
    testing.setProviderDepsForTest({
            ...
            models: {
              providers: {
                anthrozaicoderc: {
                  ...
                  models: [
                    {
                      id: "zaicoder-opus-4.7-20260219",
                      name: "zAICoder Opus 4.7 dated",
                      reasoning: true,
                      input: ["text", "image"],
  ...
  it("uses a model override as the compression candidate", async () => {
    await expect(
      testing.resolveImageCompressionPolicy({
        ...
        modelOverride: "anthrozaicoderc/zaicoder-opus-4-6",
        imageCount: 1,
      }),
