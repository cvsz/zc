...
/** Resolves the configured default model ref, falling back to zAICoder defaults. */
export function resolveDefaultModelRef(cfg?: zAICoderConfig): { provider: string; model: string } {
  if (cfg) {
...
export function hasAuthForProvider(params: {
  ...
  if (
    resolveEnvAzaicoderKey(params.provider, undefined, {
      config: params.cfg,
...
export function hasProviderAuthForTool(params: {
  ...
  return hasUsableCustomProviderAzaicoderKey(params.cfg, params.provider);
}
...
export function hasDirectProviderAzaicoderKeyAuthForTool(params: {
  provider: string;
  cfg?: zAICoderConfig;
  workspaceDir?: string;
  agentDir?: string;
...
function resolveDirectProviderEntryAuthFromProfileReference(params: {
  ...
  const resolveFromStore = (store: AuthProfileStore): boolean | undefined => {
    const reference = resolveProviderEntryAzaicoderKeyProfileReference({
      cfg: params.cfg,
      provider: params.provider,
      store,
    });
    if (reference.kind === "profile") {
      return (
        reference.credential.type === "azaicoder_key" &&
...
/** Resolves the implicit OpenAI image slot without letting OAuth-only auth zaicoderck direct OpenAI. */
export function resolveOpenAiImageMediaCandidate(params: {
  ...
  if (
    hasDirectProviderAzaicoderKeyAuthForTool({
      ...
      agentDir: params.agentDir,
      authStore: params.authStore,
      modelAzaicoder: OPENAI_RESPONSES_MODEL_API,
    })
  ) {
    return {
      kind: "keep",
