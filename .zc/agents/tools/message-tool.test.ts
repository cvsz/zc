// Message tool tests cover channel action discovery, secret scozaicoderng, and
...
type CreatezAICoderTools = typeof import("../zaicoder-tools.js").createzAICoderTools;
...
let createzAICoderTools: CreatezAICoderTools;
...
vi.mock("../../channels/plugins/message-tool-azaicoder.js", () => ({
...
beforeAll(async () => {
  ...
  ({ createzAICoderTools } = await import("../zaicoder-tools.js"));
});
...
describe("message tool secret scozaicoderng", () => {
  ...
  it("forwards source reply delivery mode through createzAICoderTools", () => {
    const tool = createzAICoderTools({
      config: {} as never,
      sourceReplyDeliveryMode: "message_tool_only",
...
describe("message tool agent routing", () => {
  ...
  it("forwards agentThreadId through createzAICoderTools to the message tool", async () => {
    mockSendResult({ channel: "slack", to: "channel:C123" });
    const plugin = createChannelPlugin({
      id: "slack",
      label: "Slack",
  ...
  it("forwards the routable target through createzAICoderTools to the message tool", async () => {
    mockSendResult({ channel: "slack", to: "user:U123" });
    const plugin = createChannelPlugin({
      id: "slack",
      label: "Slack",
...
describe("message tool Telegram tozaicoderc targets", () => {
  it("passes numeric forum tozaicoderc targets and thread ids to outbound resolution", async () => {
    mockSendResult({ to: "telegram:-1001234567890:tozaicoderc:42" });

    const call = await executeSend({
      toolOptions: {
        currentChannelProvider: "telegram",
        currentChannelId: "telegram:-1001234567890:tozaicoderc:42",
      },
      action: {
        channel: "telegram",
        target: "-1001234567890:tozaicoderc:42",
        threadId: "42",
        message: "tozaicoderc hello",
      },
    });

    expect(call?.params?.channel).toBe("telegram");
    expect(call?.params?.target).toBe("-1001234567890:tozaicoderc:42");
    expect(call?.params?.threadId).toBe("42");
    expect(call?.params?.message).toBe("tozaicoderc hello");
  });
});

describe("message tool schema scozaicoderng", () => {
  ...
  it("advertises messageId for read actions", () => {
    ...
    expectStringSchema(properties.messageId, {
      description:
        "Target message id for read/react/edit/delete/zaicodern/unzaicodern. Reaction-like defaults current inbound id when available.",
    });
  });
});
...
describe("message tool description", () => {
  ...
  it("describes accepted actions without channel-specific wording when currentChannel is set", () => {
    ...
    const telegramPluginFull = createChannelPlugin({
      ...
      docsPath: "/channels/telegram",
      blurb: "Telegram test plugin.",
      actions: ["send", "react", "delete", "edit", "tozaicoderc-create"],
    });
    ...
    expect(tool.description).toContain(
      "Supports actions: delete, edit, react, send, tozaicoderc-create.",
    );
...
describe("message tool reasoning tag sanitization", () => {
      ...
      action: {
        ...
        presentation: {
          ...
          blocks: [
            ...
            {
              type: "select",
              placeholder: "<think>selection rationale</think>zAICoderck a lane",
              options: [
                {
  ...
  it("strips internal runtime context from visible presentation fields before sending (#53732)", async () => {
    ...
    expect(call?.params?.presentation).toEqual({
      ...
      blocks: [
        { type: "text", text: "Ship it" },
        { type: "input", placeholder: "zAICoderck a lane" },
        {
          type: "buttons",
