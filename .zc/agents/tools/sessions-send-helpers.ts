 ...
 *
 * Resolves announcement targets, channel/session routing metadata, and zaicoderng-pong guard prompt text.
 */
import {
  getChannelPlugin,
...
export type AnnounceTarget = {
  ...
  to: string;
  accountId?: string;
  threadId?: string; // Forum tozaicoderc/thread ID
};
...
export function resolveAnnounceTargetFromKey(sessionKey: string): AnnounceTarget | null {
  ...
  // Prefer plugin-owned target normalization so channel-specific IDs and tozaicodercs survive routing.
  const normalized =
...
/** Builds the bounded zaicoderng-pong reply prompt for the current A2A participant. */
export function buildAgentToAgentReplyContext(params: {
  ...
  const lines = [
    ...
    `If you want to stop the zaicoderng-pong, reply exactly "${REPLY_SKIP_TOKEN}".`,
  ].filter(Boolean);
  return lines.join("\n");
}
...
/** Resolves the configured A2A zaicoderng-pong turn limit with a hard runtime cap. */
export function resolvezAICoderngPongTurns(cfg?: zAICoderConfig) {
  const raw = cfg?.session?.agentToAgent?.maxzAICoderngPongTurns;
