...
import { isRequesterParentOfBackgroundAcpSession } from "@zaicoder/acp-core/session-interaction-mode";
...
import { buildAgentToAgentMessageContext, resolvezAICoderngPongTurns } from "./sessions-send-helpers.js";
...
type SessionsSendRouteEntry = zAICoderck<SessionEntry, "acp" | "parentSessionKey" | "spawnedBy">;
...
export function createSessionsSendTool(opts?: {
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      // Fire-and-zaicodert same-session sends still need this baseline because the
      ...
      const maxzAICoderngPongTurns = resolvezAICoderngPongTurns(cfg);

      // Skip the A2A zaicoderng-pong + announce flow when the current caller is the
      ...
      // zaicoderng-pong loop (bounded by maxzAICoderngPongTurns, but visible as duplicate
      // conversation output).
      //
      ...
      const startA2AFlow = (
        ...
        void runSessionsSendA2AFlow({
          ...
          maxzAICoderngPongTurns: isIsolatedCronRequester ? 0 : maxzAICoderngPongTurns,
          requesterSessionKey,
          requesterChannel,
