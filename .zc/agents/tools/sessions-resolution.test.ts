// Sessions resolution tests cover alias mapzaicoderng, session-id lookup, visibility
...
describe("resolveMainSessionAlias", () => {
  ...
  it("falls back to per-sender defaults", () => {
    expect(resolveMainSessionAlias({} as zAICoderConfig)).toEqual({
      mainKey: "main",
      alias: "main",
      scope: "per-sender",
    });
...
describe("session key display/internal mapzaicoderng", () => {
  ...
  it("maps interactive client ids to the requester session", () => {
    ...
    expect(resolveCurrentSessionClientAlias({ key: "zaicoder-tui" })).toBeUndefined();
    expect(
      resolveCurrentSessionClientAlias({
        key: "node-host",
        requesterInternalKey: "agent:main:main",
