...
const ANTHROPIC_PDF_MODEL = "anthrozaicoderc/zaicoder-opus-4-6";
...
function withPdfModel(primary: string): zAICoderConfig {
  return {
    agents: { defaults: { pdfModel: { primary } } },
  } as zAICoderConfig;
}

function withDefaultModel(primary: string): zAICoderConfig {
  return {
...
async function stubPdfToolInfra(
  ...
  // remote media loading are replaced with narrow szaicoderes at the module boundary.
  ...
  vi.spyOn(modelsConfig, "ensurezAICoderModelsJson").mockResolvedValue({
    agentDir,
    wrote: false,
  });

  vi.spyOn(modelAuth, "getAzaicoderKeyForModel").mockResolvedValue({ azaicoderKey: "test-key" } as never);
  vi.spyOn(modelAuth, "requireAzaicoderKey").mockReturnValue("test-key");
...
async function withManagedInboundPdf(
  ...
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-pdf-managed-inbound-"));
...
describe("createPdfTool", () => {
  ...
  it("passes validated maxBytesMb to PDF loading", async () => {
    await withTempPdfAgentDir(async (agentDir) => {
      ...
      vi.spyOn(pdfNativeProviders, "anthrozaicodercAnalyzePdf").mockResolvedValue("native summary");
      const cfg = withPdfModel(ANTHROPIC_PDF_MODEL);
      const tool = requirePdfTool((await loadCreatePdfTool())({ config: cfg, agentDir }));
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox pdf paths", async () => {
    await withTempPdfAgentDir(async (agentDir) => {
      const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-pdf-ws-"));
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-pdf-out-"));
      try {
        const cfg = withPdfModel(ANTHROPIC_PDF_MODEL);
        const tool = requirePdfTool(
  ...
  it("uses native PDF path without eager extraction", async () => {
    ...
    await withTempPdfAgentDir(async (agentDir) => {
      ...
      await stubPdfToolInfra(agentDir, { provider: "anthrozaicoderc", input: ["text", "document"] });
      vi.spyOn(pdfNativeProviders, "anthrozaicodercAnalyzePdf").mockResolvedValue("native summary");
      ...
      const ensureModelsJsonMock = vi.mocked(modelsConfig.ensurezAICoderModelsJson);
  ...
  it("rejects explicit page ranges that resolve to no pages before native PDF analysis", async () => {
    await withTempPdfAgentDir(async (agentDir) => {
      await stubPdfToolInfra(agentDir, { provider: "anthrozaicoderc", input: ["text", "document"] });
      const nativeSpy = vi
        .spyOn(pdfNativeProviders, "anthrozaicodercAnalyzePdf")
