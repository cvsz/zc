...
import {
  ...
  resolveModelRuntimeAzaicoderKey,
  resolvePromptAndModelOverride,
  resolveRemoteMediaSsrfPolicy,
} from "./media-tool-shared.js";
import { hasToolModelConfig } from "./model-config.helpers.js";
import { anthrozaicodercAnalyzePdf, geminiAnalyzePdf } from "./pdf-native-providers.js";
import {
...
function hasExplicitPdfToolModelConfig(config?: zAICoderConfig): boolean {
  return (
...
function buildPdfExtractionContext(
  ...
  const systemPrompt =
    model?.azaicoder === "openai-chatgpt-responses" ? CODEX_PDF_INSTRUCTIONS : undefined;
...
async function runPdfPrompt(params: {
  ...
  await ensurezAICoderModelsJson(effectiveCfg, params.agentDir, modelsOptions);
  ...
  const result = await runWithImageModelFallback({
    ...
    run: async (provider, modelId) => {
      ...
      const azaicoderKey = await resolveModelRuntimeAzaicoderKey({
        model,
        cfg: effectiveCfg,
      ...
      if (providerSupportsNativePdf(provider)) {
        ...
        if (provider === "anthrozaicoderc") {
          const text = await anthrozaicodercAnalyzePdf({
            azaicoderKey,
            modelId,
...
export function createPdfTool(options?: {
  ...
  const description =
    "Analyze PDFs with model. Anthrozaicoderc/Google native PDF when supported; else text/image extraction. Use pdf for one, pdfs for max 10; prompt says what to inspect.";
      ...
      const getExtractions = async (): Promise<PdfExtractedContent[]> => {
        ...
        for (const pdf of loadedPdfs) {
          const extracted = await extractPdfContent({
            ...
            maxzAICoderxels: PDF_MAX_PIXELS,
