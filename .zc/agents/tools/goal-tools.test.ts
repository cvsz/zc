...
async function createStoreConfig(): Promise<{ config: zAICoderConfig; template: string }> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-goal-tools-"));
  const template = path.join(dir, "{agentId}", "sessions.json");
  return {
    config: { session: { store: template } } as zAICoderConfig,
    template,
  };
}
