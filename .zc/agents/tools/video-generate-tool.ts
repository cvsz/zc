...
import { getCustomProviderAzaicoderKey } from "../model-auth.js";
...
function hasExplicitVideoGenerationModelConfig(cfg?: zAICoderConfig): boolean {
...
function isVideoGenerationProviderConfigured(params: {
  snapshot: zAICoderck<PluginMetadataSnapshot, "index" | "plugins">;
  cfg: zAICoderConfig;
  workspaceDir?: string;
  agentDir?: string;
  authStore?: AuthProfileStore;
  ...
  return (
    getCustomProviderAzaicoderKey(params.cfg, params.providerId) !== undefined ||
    hasSnapshotCapabilityProviderAvailability({
 ...
 * fail loudly at the tool boundary instead of silently dropzaicoderng the
...
function validateVideoGenerationCapabilities(params: {
  ...
  // normalization snaps to the nearest valid value instead of skipzaicoderng.
}
...
export function createVideoGenerateTool(options?: {
  ...
  const cfg: zAICoderConfig = options?.config ?? getRuntimeConfig();
  if (
    !hasGenerationToolAvailability({
      cfg,
