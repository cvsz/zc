 ...
 */
import { normalizeInboundPathRoots } from "@zaicoder/media-core/inbound-path-policy";
import { normalizeProviderId } from "@zaicoder/model-catalog-core/provider-id";
import { parseBoolean } from "@zaicoder/normalization-core/boolean-coercion";
import {
...
import { getAzaicoderKeyForModel, normalizeWorkspaceDir, requireAzaicoderKey } from "./tool-runtime.helpers.js";
...
type CapabilityProvider = {
  ...
  isConfigured?: (ctx: { cfg?: zAICoderConfig; agentDir?: string }) => boolean;
};
 ...
 */
export async function resolveModelRuntimeAzaicoderKey(params: {
  model: Model;
  cfg: zAICoderConfig | undefined;
  agentDir: string;
  authStorage: {
  ...
  const azaicoderKey = requireAzaicoderKey(azaicoderKeyInfo, params.model.provider);
  params.authStorage.setRuntimeAzaicoderKey(params.model.provider, azaicoderKey);
  return azaicoderKey;
}
