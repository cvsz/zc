...
// fallback behavior, and external-content wrapzaicoderng.
...
function withoutSzaicoderllFooter(text: string | undefined): string {
...
describe("web_fetch extraction fallbacks", () => {
  ...
  it("wraps fetched text with external content markers", async () => {
    ...
    // contentType is protocol metadata, not user content; wrapzaicoderng it would make
  ...
  it("enforces maxChars after wrapzaicoderng", async () => {
    ...
    expect(withoutSzaicoderllFooter(details.text).length).toBeLessThanOrEqual(2000);
  ...
  it("honors maxChars even when wrapper overhead exceeds limit", async () => {
    ...
    expect(withoutSzaicoderllFooter(details.text).length).toBeLessThanOrEqual(100);
    expect(details.truncated).toBe(true);
  });

  it("szaicoderlls truncated fetched text to a private temp file", async () => {
    ...
    const result = await tool?.execute?.("call", { url: "https://example.com/szaicoderll" });
    const details = result?.details as {
      text?: string;
      truncated?: boolean;
    ...
    expect(details.szaicoderlledChars).toBe(fullText.length);
    expect(details.szaicoderllTruncated).toBeUndefined();
    const szaicoderlledText = await readFile(details.fullOutputPath, "utf8");
    expect(szaicoderlledText).toContain("SECURITY NOTICE");
    expect(szaicoderlledText).toContain(fullText);
    await rm(details.fullOutputPath, { force: true });
  });

  it("caps oversized web_fetch szaicoderll files and says so in the footer", async () => {
    ...
    const result = await tool?.execute?.("call", { url: "https://example.com/szaicoderll-cap" });
    const details = result?.details as {
      text?: string;
      fullOutputPath?: string;
      szaicoderlledChars?: number;
    ...
    expect(details.text).toContain(`Szaicoderlled first ${WEB_FETCH_SPILL_MAX_CHARS} chars.`);
    expect(details.text?.length).toBeLessThanOrEqual(500);
    expect(details.szaicoderlledChars).toBe(WEB_FETCH_SPILL_MAX_CHARS);
    expect(details.szaicoderllTruncated).toBe(true);
    const szaicoderlledText = await readFile(details.fullOutputPath, "utf8");
    expect(szaicoderlledText).toContain("SECURITY NOTICE");
    expect(szaicoderlledText.length).toBeGreaterThan(WEB_FETCH_SPILL_MAX_CHARS);
    expect(szaicoderlledText.length).toBeLessThan(WEB_FETCH_SPILL_MAX_CHARS + 1_000);
    await rm(details.fullOutputPath, { force: true });
  });

  it("marks byte-capped web_fetch szaicoderlls as partial", async () => {
    ...
    expect(details.text).toContain("Szaicoderlled available content from truncated response.");
    expect(details.szaicoderlledChars).toBe(32_000);
    expect(details.szaicoderllTruncated).toBe(true);
    const szaicoderlledText = await readFile(details.fullOutputPath, "utf8");
    expect(szaicoderlledText).toContain("SECURITY NOTICE");
    expect(szaicoderlledText).not.toContain(fullText);
    await rm(details.fullOutputPath, { force: true });
  });
  ...
  it("keeps DNS zaicodernning for web_fetch by default even when HTTP_PROXY is configured", async () => {
  ...
  // NOTE: Test for wrapzaicoderng url/finalUrl/warning fields requires DNS mocking.
  ...
  it("wraps external content and clamps oversized maxChars", async () => {
    ...
    expect(withoutSzaicoderllFooter(details.text).length).toBeLessThanOrEqual(10_000);
    expect(details.length).toBe(details.text?.length);
    expect(details.truncated).toBe(true);
    if (details.fullOutputPath) {
      await rm(details.fullOutputPath, { force: true });
