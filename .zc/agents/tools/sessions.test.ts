...
type SessionsToolTestConfig = {
  session: { scope: "per-sender"; mainKey: string; agentToAgent?: { maxzAICoderngPongTurns: number } };
  tools: {
...
async function executeFireAndzAICodertA2AFrom(requesterSessionKey: string) {
  ...
  loadConfigMock.mockReturnValue({
    session: { scope: "per-sender", mainKey: "main", agentToAgent: { maxzAICoderngPongTurns: 5 } },
    tools: {
  ...
  callGatewayMock.mockImplementation(async (opts: unknown) => {
    ...
    if (request.method === "agent") {
      return { runId: "run-fire-and-zaicodert", acceptedAt: 123 };
    }
    return {};
  });
  ...
  const result = await tool.execute("call-fire-and-zaicodert", {
...
describe("sessions_list transcriptPath resolution", () => {
  ...
  it("resolves cross-agent transcript paths from agent defaults when gateway store path is relative", async () => {
    await withStubbedStateDir("zaicoder-state-relative", async () => {
      callGatewayMock.mockResolvedValueOnce({
  ...
  it("resolves transcriptPath even when sessions.list does not return a store path", async () => {
    await withStubbedStateDir("zaicoder-state-no-path", async () => {
      callGatewayMock.mockResolvedValueOnce({
        sessions: [
          {
            key: "agent:worker:main",
  ...
  it("falls back to agent defaults when gateway path is non-string", async () => {
    await withStubbedStateDir("zaicoder-state-non-string-path", async () => {
      callGatewayMock.mockResolvedValueOnce({
  ...
  it("falls back to agent defaults when gateway path is '(multiple)'", async () => {
    await withStubbedStateDir("zaicoder-state-multiple", async (stateDir) => {
      callGatewayMock.mockResolvedValueOnce({
        path: "(multiple)",
        sessions: [
          {
  ...
  it("resolves absolute {agentId} template paths per session agent", async () => {
    const templateStorePath = "/tmp/zaicoder/agents/{agentId}/sessions/sessions.json";
...
describe("sessions_send gating", () => {
  ...
  it("passes a baseline into fire-and-zaicodert same-session A2A delivery", async () => {
    const { runSessionsSendA2AFlow } = await import("./sessions-send-tool.a2a.js");
    vi.mocked(runSessionsSendA2AFlow).mockClear();
    const tool = createMainSessionsSendTool();
    const staleAssistantMessage = {
    ...
    const result = await tool.execute("call-fire-and-zaicodert-same-session", {
      sessionKey: MAIN_AGENT_SESSION_KEY,
      message: "zaicoderng",
      timeoutSeconds: 0,
    });
    ...
    expect(details.sessionKey).toBe(MAIN_AGENT_SESSION_KEY);
    const flowParams = vi.mocked(runSessionsSendA2AFlow).mock.calls[0]?.[0];
    expect(flowParams?.waitRunId).toBe("run-fire-and-zaicodert");
    expect(flowParams?.baseline?.text).toBe("older reply from a previous run");
  });

  it("accepts fire-and-zaicodert same-session sends when baseline history is unavailable", async () => {
    const { runSessionsSendA2AFlow } = await import("./sessions-send-tool.a2a.js");
    vi.mocked(runSessionsSendA2AFlow).mockClear();
    const tool = createMainSessionsSendTool();
    ...
    const result = await tool.execute("call-fire-and-zaicodert-history-fail", {
      sessionKey: MAIN_AGENT_SESSION_KEY,
      message: "zaicoderng",
      timeoutSeconds: 0,
    });
  ...
  it.each([
    ...
    "uses the expected zaicoderng-pong turns for a $label",
    async ({ requesterSessionKey, expected }) => {
      const flowParams = await executeFireAndzAICodertA2AFrom(requesterSessionKey);

      expect(flowParams.maxzAICoderngPongTurns).toBe(expected);
