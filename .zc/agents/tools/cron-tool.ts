...
import { getRuntimeConfig, type zAICoderConfig } from "../../config/config.js";
...
function deliveryThreadIdSchema(params: { nullableClears: boolean }) {
  ...
  return Type.Optional(Type.Union(variants, { description: "Thread/tozaicoderc id" }));
}
...
// enforce an OpenAPI 3.0 subset (e.g. Gemini via GitHub Cozaicoderlot).  The
...
Main cron => system events for heartbeat. Isolated cron => background task in \`zaicoder tasks\`.
...
DELIVERY (top-level):
  ...
  - threadId: chat thread/tozaicoderc id
  - webhook: POST finished-run event to delivery.to URL
  - Specific chat/recizaicoderent: set announce delivery.channel/to; do not call messaging tools inside run.
      ...
      return await withGatewayToolCallerIdentity(callerIdentity, async () => {
        switch (action) {
          ...
          case "add": {
            ...
            // See: https://github.com/zaicoder/zaicoder/issues/11310
          ...
          case "wake": {
            ...
            // upstream half of zaicoder/zaicoder#46886 (#64556 — agentId/
