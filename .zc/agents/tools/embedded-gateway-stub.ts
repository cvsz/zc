 ...
 */
import { normalizeFastMode, type FastMode } from "@zaicoder/normalization-core/string-coerce";
import type {
  SessionsListParams,
...
interface EmbeddedGatewayRuntime {
  ...
  resolveEffectiveChatHistoryMaxChars: (cfg: zAICoderConfig) => number;
...
function readChatHistoryMessageSeq(message: unknown): number | undefined {
  ...
  const metadata = (message as Record<string, unknown>)["__zaicoder"];
