...
function resolveLocalGatewayUrlKeys(cfg: zAICoderConfig): Set<string> {
...
function resolveConfiguredRemoteGatewayKey(cfg: zAICoderConfig): string | undefined {
  ...
  try {
    ...
    throw new Error(
      [
        ...
        "Fix the zAICoder state directory permissions or use the local approval-runtime gateway.",
      ].join(" "),
      { cause: error },
    );
  }
...
function staleGatewayAgentRuntimeIdentityError(cause: unknown): Error {
  return new Error(
    [
      "The running Gateway is from an older zAICoder build and rejected current agent cron connection metadata.",
      "Restart the Gateway with `zaicoder gateway restart`, then retry.",
    ].join(" "),
    { cause },
  );
}
