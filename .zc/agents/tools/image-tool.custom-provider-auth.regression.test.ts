...
import { getAzaicoderKeyForModel, hasUsableCustomProviderAzaicoderKey } from "../model-auth.js";
import { resolveImageToolFactoryAvailable } from "../zaicoder-tools.media-factory-plan.js";
...
function createUserReportedConfig(params?: { includeAzaicoderKey?: boolean }): zAICoderConfig {
  const includeAzaicoderKey = params?.includeAzaicoderKey ?? true;
  return {
    agents: {
      defaults: {
    ...
    models: {
      providers: {
        [USER_PROVIDER]: {
          ...
          ...(includeAzaicoderKey ? { azaicoderKey: CONFIG_API_KEY } : {}),
...
async function withEmptyAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-image-auth-regression-"));
  try {
...
describe("image custom provider auth regression", () => {
  ...
  it("uses real model-auth to accept config-only custom provider credentials", async () => {
    ...
    expect(hasUsableCustomProviderAzaicoderKey(cfg, USER_PROVIDER)).toBe(true);
  ...
  it("executes deferred image tool discovery with config-backed auth and runtime key resolution", async () => {
    ...
    await withEmptyAgentDir(async (agentDir) => {
      ...
      expect(auth.azaicoderKey).toBe(CONFIG_API_KEY);
  ...
  it("still rejects the same config when azaicoderKey is missing", async () => {
    await withEmptyAgentDir(async (agentDir) => {
      const cfg = createUserReportedConfig({ includeAzaicoderKey: false });
      expect(hasUsableCustomProviderAzaicoderKey(cfg, USER_PROVIDER)).toBe(false);
