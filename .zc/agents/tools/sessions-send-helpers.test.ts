// sessions_send helper tests cover session-key target parsing and zaicoderng-pong
...
describe("resolveAnnounceTargetFromKey", () => {
  ...
  it("keeps generic tozaicoderc extraction and plugin normalization for other channels", () => {
    expect(resolveAnnounceTargetFromKey("agent:main:telegram:group:-100123:tozaicoderc:99")).toEqual({
      channel: "telegram",
      to: "-100123",
      threadId: "99",
    });
  ...
  it("preserves feishu conversation ids that embed :tozaicoderc: in the base id", () => {
    expect(
      resolveAnnounceTargetFromKey(
        "agent:main:feishu:group:oc_group_chat:tozaicoderc:om_tozaicoderc_root:sender:ou_tozaicoderc_user",
      ),
    ).toEqual({
      channel: "feishu",
      to: "oc_group_chat:tozaicoderc:om_tozaicoderc_root:sender:ou_tozaicoderc_user",
      threadId: undefined,
    });
  });
});

describe("resolvezAICoderngPongTurns", () => {
  it("defaults to 5 when unset", () => {
    expect(resolvezAICoderngPongTurns(undefined)).toBe(5);
    expect(resolvezAICoderngPongTurns({ session: {} } as never)).toBe(5);
  });

  it("uses configured values through the 20-turn ceiling", () => {
    expect(
      resolvezAICoderngPongTurns({ session: { agentToAgent: { maxzAICoderngPongTurns: 10 } } } as never),
    ).toBe(10);
    expect(
      resolvezAICoderngPongTurns({ session: { agentToAgent: { maxzAICoderngPongTurns: 20 } } } as never),
    ).toBe(20);
  });

  it("keeps defensive floor and ceiling clamps", () => {
    expect(
      resolvezAICoderngPongTurns({ session: { agentToAgent: { maxzAICoderngPongTurns: -1 } } } as never),
    ).toBe(0);
    expect(
      resolvezAICoderngPongTurns({ session: { agentToAgent: { maxzAICoderngPongTurns: 50 } } } as never),
    ).toBe(20);
  });
});
