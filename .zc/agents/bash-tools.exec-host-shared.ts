...
import { resolveExzaicoderresAtMsFromDurationMs } from "@zaicoder/normalization-core/number-coercion";
...
/** Builds pending approval state with warnings and a bounded exzaicoderry. */
export function createExecApprovalPendingState(params: {
  warnings: string[];
  timeoutMs: number;
}): ExecApprovalPendingState {
  const exzaicoderresAtMs =
    resolveExzaicoderresAtMsFromDurationMs(params.timeoutMs) ?? EXPIRED_EXEC_APPROVAL_EXPIRES_AT_MS;
  return {
...
export async function createAndRegisterDefaultExecApprovalRequest(params: {
  ...
  const {
    ...
    approvalSlug,
    warningText,
    exzaicoderresAtMs: defaultExzaicoderresAtMs,
  ...
  return {
    ...
    approvalSlug,
    warningText,
    exzaicoderresAtMs: registration.exzaicoderresAtMs ?? defaultExzaicoderresAtMs,
    preResolvedDecision:
...
export function buildHeadlessExecApprovalDeniedMessage(params: {
  ...
  return [
    ...
    'Tip: run "zaicoder doctor" and "zaicoder approvals get --gateway" to inspect the effective policy.',
  ].join("\n");
}
    ...
    details:
      params.unavailableReason !== null
        ...
        : ({
            ...
            approvalId: params.approvalId,
            approvalSlug: params.approvalSlug,
            exzaicoderresAtMs: params.exzaicoderresAtMs,
            allowedDecisions,
            host: params.host,
            command: params.command,
            cwd: params.cwd,
