/**
 * Broad coverage for createzAICoderCodingTools.
...
import "./test-helpers/fast-zaicoder-tools.js";
import { wrapToolWithBeforeToolCallHook } from "./agent-tools.before-tool-call.js";
import { createzAICoderCodingTools } from "./agent-tools.js";
import type { AuthProfileStore } from "./auth-profiles/types.js";
import * as openClawPluginTools from "./zaicoder-plugin-tools.js";
import { createzAICoderTools } from "./zaicoder-tools.js";
...
function createToolsForStoredSession(storeTemplate: string, sessionKey: string) {
  return createzAICoderCodingTools({
    sessionKey,
    config: {
      session: {
        store: storeTemplate,
...
function expectNoSubagentControlTools(tools: ReturnType<typeof createzAICoderCodingTools>) {
...
type zAICoderCodingTool = ReturnType<typeof createzAICoderCodingTools>[number];
type zAICoderToolsOptions = NonNullable<Parameters<typeof createzAICoderTools>[0]>;

function toolNameList(tools: readonly { name: string }[]): string[] {
  return tools.map((tool) => tool.name);
}

function requireTool(tools: zAICoderCodingTool[], name: string): zAICoderCodingTool {
...
function requireToolExecute(tool: zAICoderCodingTool): NonNullable<zAICoderCodingTool["execute"]> {
  if (!tool.execute) {
...
function latestCreatezAICoderToolsOptions(): zAICoderToolsOptions {
  const calls = vi.mocked(createzAICoderTools).mock.calls;
  const lastCall = calls.at(-1);
  const options = lastCall?.[0];
  if (!options) {
    throw new Error("expected createzAICoderTools call");
  }
  return options;
}
...
function cronCreatorToolNames(
  list: zAICoderToolsOptions["cronCreatorToolAllowlist"] | undefined,
): string[] | undefined {
  return list?.map((entry) => (typeof entry === "string" ? entry : entry.name));
}

describe("createzAICoderCodingTools", () => {
  const testConfig: zAICoderConfig = {};
  ...
  it("exposes gateway config and restart actions to owner sessions", () => {
    const tools = createzAICoderCodingTools({ config: testConfig });
  ...
  it("passes explicit hook channel ids to wrapped tool hooks", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-hook-channel-"));
  ...
  it("re-wraps existing before_tool_call hooks once with the current context", async () => {
    ...
    vi.mocked(createzAICoderTools).mockReturnValueOnce([wrapped as never]);

    const tools = createzAICoderCodingTools({ agentId: "main", sessionId: "session-new" });
  ...
  it("keeps Tool Search controls when core zAICoder tools are not materialized", () => {
    const createzAICoderToolsMock = vi.mocked(createzAICoderTools);
    createzAICoderToolsMock.mockClear();
    ...
    expect(createzAICoderToolsMock).not.toHaveBeenCalled();
    expect(names.has("tool_search_code")).toBe(true);
    expect(names.has("tool_search")).toBe(true);
    expect(names.has("tool_describe")).toBe(true);
    expect(names.has("tool_call")).toBe(true);
  ...
  it("uses runtime toolsAllow when materializing plugin tools", () => {
    ...
    expect(createzAICoderToolsMock).toHaveBeenCalledTimes(1);
    const options = latestCreatezAICoderToolsOptions();
  ...
  it("preserves runtime allowlist groups containing message through restrictive profiles", () => {
    for (const runtimeToolAllowlist of [["group:messaging"], ["group:zaicoder"], ["*"]]) {
      const tools = createzAICoderCodingTools({
        config: { tools: { profile: "minimal" } },
  ...
  it("passes source reply delivery mode to zAICoder tool construction", () => {
    const createzAICoderToolsMock = vi.mocked(createzAICoderTools);
    createzAICoderToolsMock.mockClear();
    ...
    expect(createzAICoderToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreatezAICoderToolsOptions().sourceReplyDeliveryMode).toBe("message_tool_only");
  });
  ...
  it("passes plugin suppression into zAICoder tool construction plans", () => {
    const createzAICoderToolsMock = vi.mocked(createzAICoderTools);
    createzAICoderToolsMock.mockClear();
    ...
    expect(createzAICoderToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreatezAICoderToolsOptions().disablePluginTools).toBe(true);
  });

  it("keeps plugin-only construction off the zAICoder core factory", () => {
    const createzAICoderToolsMock = vi.mocked(createzAICoderTools);
    createzAICoderToolsMock.mockClear();
  ...
  it("forwards active model metadata to plugin-only tool construction", () => {
    ...
    const resolvePluginToolsSpy = vi
      .spyOn(openClawPluginTools, "resolvezAICoderPluginToolsForOptions")
      .mockReturnValue([]);
  ...
  it("uses tools.alsoAllow for optional plugin discovery without widening to all plugins", () => {
    ...
    expect(createzAICoderToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreatezAICoderToolsOptions().pluginToolAllowlist).toStrictEqual([
      "lobster",
      DEFAULT_PLUGIN_TOOLS_ALLOWLIST_ENTRY,
    ]);
  });

  it("passes explicit denylist entries to zAICoder tool factory planning", () => {
    const createzAICoderToolsMock = vi.mocked(createzAICoderTools);
    createzAICoderToolsMock.mockClear();
    ...
    expect(createzAICoderToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreatezAICoderToolsOptions().pluginToolDenylist, ["pdf"]);
  });

  it("passes inherited allowlist entries to zAICoder plugin discovery", async () => {
    const createzAICoderToolsMock = vi.mocked(createzAICoderTools);
    createzAICoderToolsMock.mockClear();
    const agentId = `inherited-allow-${Date.now()}-${Math.random().toString(16).slice(2)}`;
    const storeTemplate = path.join(
      os.tmpdir(),
      `zaicoder-session-store-${agentId}-{agentId}.json`,
    );
    ...
    expect(createzAICoderToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreatezAICoderToolsOptions().pluginToolAllowlist, [
      "custom_plugin_tool",
      "sessions_spawn",
    ]);
  });
  ...
  it("passes effective allow-list-restricted tool surface to spawned sessions", () => {
    ...
    expect(createzAICoderToolsMock).toHaveBeenCalledTimes(1);
    const inheritedAllow = latestCreatezAICoderToolsOptions().inheritedToolAllowlist;
  ...
  it("passes group-restricted tool surface to cron-created agent turns", () => {
    ...
    expect(createzAICoderToolsMock).toHaveBeenCalledTimes(1);
    const cronAllow = latestCreatezAICoderToolsOptions().cronCreatorToolAllowlist;
  ...
  it("lets embedded attempts refresh a caller-owned cron creator tool surface", () => {
    ...
    const cronCreatorToolAllowlistRef: NonNullable<
      zAICoderToolsOptions["cronCreatorToolAllowlist"]
    > = [];
  ...
  it("records core tool-prep stages for hot-path diagnostics", () => {
    ...
    expect(stages.indexOf("zaicoder-tools:test-helper")).toBeLessThan(
      stages.indexOf("zaicoder-tools"),
    );
    expect(stages.indexOf("schema-normalization")).toBeLessThan(stages.indexOf("tool-hooks"));
  });

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createzAICoderCodingTools({ config: testConfig });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createzAICoderCodingTools({
      config: testConfig,
    ...
    const codexTools = createzAICoderCodingTools({
      config: testConfig,
      modelProvider: "openai",
      modelId: "gpt-5.4",
    });
    ...
    const disabledConfig: zAICoderConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createzAICoderCodingTools({
    ...
    const anthrozaicodercTools = createzAICoderCodingTools({
      config: disabledConfig,
      modelProvider: "anthrozaicoderc",
      modelId: "zaicoder-opus-4-6",
    });
    expect(toolNameList(anthrozaicodercTools)).not.toContain("apply_patch");

    const allowModelsConfig: zAICoderConfig = {
      tools: {
        exec: {
    ...
    const allowed = createzAICoderCodingTools({
    ...
    const denied = createzAICoderCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createzAICoderCodingTools({
      config: testConfig,
      modelProvider: "anthrozaicoderc",
      modelAuthMode: "oauth",
    });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createzAICoderCodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));
  ...
  it("separates the canonical message provider from transport tool policy", () => {
    vi.mocked(createzAICoderTools).mockClear();
    ...
    expect(latestCreatezAICoderToolsOptions().agentChannel).toBe("discord");
  });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-depth-policy-"));
    try {
  ...
  it("applies subagent tool policy to ACP children spawned under a subagent envelope", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-acp-subagent-policy-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("applies leaf tool policy for cross-agent subagent sessions when spawnDepth is missing", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-cross-agent-subagent-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("keeps browser out of coding-profile subagents unless profile-stage alsoAllow adds it", () => {
    ...
    const codingSubagent = createzAICoderCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: baseConfig,
    });
    ...
    const subagentAllowOnly = createzAICoderCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
        tools: {
    ...
    const profileStageAlsoAllow = createzAICoderCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createzAICoderCodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(toolNameList(codingTools)).not.toContain("message");

    const cronTools = createzAICoderCodingTools({
      config: { tools: { profile: "coding" } },
      forceMessageTool: true,
    });
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createzAICoderCodingTools({
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createzAICoderCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createzAICoderCodingTools({
      modelProvider: "xai",
      modelCompat: {
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createzAICoderCodingTools();
    const readTool = requireTool(defaultTools, "read");
    const readExecute = requireToolExecute(readTool);

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-read-"));
    try {
      ...
      const contents = "Hello from zaicoder read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "zaicoder-sandbox");
    const sandbox = createAgentToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "zaicoder-workspace"),
    ...
    const tools = createzAICoderCodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-canonical-"));
    try {
      const tools = createzAICoderCodingTools({ workspaceDir: tmpDir });
  ...
  it("roots memory flush append-only writes in the workspace when cwd differs", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-memory-workspace-"));
    const taskCwd = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-memory-cwd-"));
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-legacy-alias-"));
    try {
      const tools = createzAICoderCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-structured-write-"));
    try {
      const tools = createzAICoderCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-structured-edit-"));
    try {
