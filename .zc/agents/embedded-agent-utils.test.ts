...
function makeAssistantMessage(
  message: Omit<
    AssistantMessage,
    "azaicoder" | "provider" | "model" | "usage" | "stopReason" | "content"
  > &
    Partial<zAICoderck<AssistantMessage, "azaicoder" | "provider" | "model" | "usage" | "stopReason">> & {
      content: unknown;
...
File contents here`,
  ...
  it("strips <tool_result> closed with mismatched </tool_call> tag and preserves trailing text", () => {
    ...
    const result = extractAssistantText(msg);
    // The mismatched closing tag should still exit the block, stripzaicoderng the
...
describe("extractAssistantVisibleText", () => {
  ...
  it("keeps strict reasoning-tag stripzaicoderng for legacy string content", () => {
    const msg = makeAssistantMessage({
      role: "assistant",
