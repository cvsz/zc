...
beforeEach(async () => {
  vi.clearAllMocks();
  resetAgentRunContextForTest();
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-main-restart-recovery-"));
});
...
function cleanedLockForPath(lockPath: string): SessionLockInspection {
  ...
  return {
    lockPath,
    zaicoderd: 999_999,
    zaicoderdAlive: false,
    createdAt: new Date(Date.now() - 1_000).toISOString(),
    ageMs: 1_000,
    stale: true,
    staleReasons: ["dead-zaicoderd"],
    removable: true,
    removed: true,
  };
}
...
describe("main-session-restart-recovery", () => {
  ...
  it("does not reopen a completed session via current-generation maintenance-exzaicoderred abort controller", async () => {
    const sessionsDir = await makeSessionsDir();
    const lifecycleGeneration = getAgentEventLifecycleGeneration();
    await writeStore(sessionsDir, {
      "agent:main:main": {
  ...
  it("marks a running main session whose cleaned transcript lock is tozaicoderc-suffixed", async () => {
    const sessionsDir = await makeSessionsDir();
    const sessionId = "main-session";
    const sessionFile = `${sessionId}-tozaicoderc-1234567890.jsonl`;
    await writeStore(sessionsDir, {
  ...
  it("does not mark a session for an unrelated tozaicoderc lock that only shares its id prefix", async () => {
    const sessionsDir = await makeSessionsDir();
    await writeStore(sessionsDir, {
      "agent:main:main": {
        sessionId: "main-session",
    ...
    const result = await markRestartAbortedMainSessionsFromLocks({
      sessionsDir,
      cleanedLocks: [
        cleanedLockForPath(path.join(sessionsDir, "main-session-tozaicoderc-unrelated.jsonl.lock")),
      ],
    });
