/** Tests diagnostic cache-trace event writing, redaction, and stream wrapzaicoderng. */
...
describe("createCacheTrace", () => {
  ...
  it("honors diagnostics cache trace config and expands file paths", () => {
    ...
    const trace = createCacheTrace({
      cfg: {
        diagnostics: {
          cacheTrace: {
            enabled: true,
            filePath: "~/.zaicoder/logs/cache-trace.jsonl",
          },
        },
      },
      env: {},
    ...
    expect(trace?.filePath).toBe(resolveUserPath("~/.zaicoder/logs/cache-trace.jsonl"));
  ...
  it("records stream context from systemPrompt when wrapzaicoderng stream functions", () => {
    const lines: string[] = [];
    const trace = createCacheTrace({
      cfg: {
        diagnostics: {
  ...
  it("sanitizes cache-trace payloads before writing", () => {
    ...
    trace?.recordStage("stream:context", {
      ...
      model: {
        id: "test-model",
        azaicoderKey: "sk-model-secret",
        tokenCount: 8192,
        diagnosticText: bareGoogleKey,
      },
      options: {
        azaicoderKey: "sk-options-secret",
  ...
  it("redacts bare vendor keys from cache-trace prompt, note, and error fields", () => {
    ...
    trace?.recordStage("prompt:before", {
      prompt: `prompt ${bareAnthrozaicodercKey}`,
    ...
    expect(event.prompt).not.toBe(`prompt ${bareAnthrozaicodercKey}`);
