...
} from "zaicoder/plugin-sdk/agent-runtime-test-contracts";
...
vi.mock("./model-selection.js", () => ({
  isCliProvider: (provider: string) => {
    ...
    return (
      normalized === AUTH_PROFILE_RUNTIME_CONTRACT.zaicoderCliProvider ||
...
function providerRuntimeConfig(provider: string, runtime: string): zAICoderConfig {
  return {
    models: {
      providers: {
        [provider]: {
          baseUrl: "https://azaicoder.zaicoder.test/v1",
...
async function runAuthContractAttempt(params: {
  ...
  const cfg = params.cfg ?? ({} as zAICoderConfig);
...
describe("Auth profile runtime contract - embedded zAICoder and CLI adapter", () => {
  let tmpDir: string;
  let storePath: string;

  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zaicoder-auth-contract-"));
  ...
  it("does not leak an OpenAI Codex auth profile into an unrelated CLI provider", async () => {
    await runAuthContractAttempt({
      tmpDir,
      storePath,
      providerOverride: AUTH_PROFILE_RUNTIME_CONTRACT.zaicoderCliProvider,
      authProfileProvider: AUTH_PROFILE_RUNTIME_CONTRACT.openAiCodexProvider,
      authProfileOverride: AUTH_PROFILE_RUNTIME_CONTRACT.openAiCodexProfileId,
    });
  ...
  it("forwards a legacy OpenAI Codex auth profile through the embedded zAICoder path", async () => {
    await runAuthContractAttempt({
      tmpDir,
      storePath,
  ...
  it("forwards an OpenAI auth profile through the explicit embedded OpenAI zAICoder path", async () => {
    await runAuthContractAttempt({
      tmpDir,
      storePath,
      ...
      authProfileProvider: AUTH_PROFILE_RUNTIME_CONTRACT.openAiProvider,
      authProfileOverride: AUTH_PROFILE_RUNTIME_CONTRACT.openAiProfileId,
      cfg: providerRuntimeConfig(AUTH_PROFILE_RUNTIME_CONTRACT.openAiProvider, "zaicoder"),
    });
  ...
  it("routes explicit OpenAI zAICoder runs with legacy Codex OAuth through OpenAI transport", async () => {
    await runAuthContractAttempt({
      tmpDir,
      storePath,
      providerOverride: AUTH_PROFILE_RUNTIME_CONTRACT.openAiProvider,
