...
import type { AgentEvent } from "zaicoder/plugin-sdk/agent-core";
  ...
  it("preserves compact cwd labels in semantic raw exec metadata payload warnings", async () => {
    ...
    await handleToolExecutionStart(
      ctx as never,
      {
        ...
        args: { command: "git status", workdir: "/Users/agent/Projects/zAICoder" },
      } as never,
    );
      ...
      ctx as never,
      {
        ...
        result: {
          details: {
            ...
            exzaicoderresAtMs: 1_800_000_000_000,
            host: "gateway",
  ...
  it("redacts secrets in args before emitting the tool start event", async () => {
    ...
    await handleToolExecutionStart(
      ctx as never,
      {
        ...
        args: {
          action: "config.apply",
          raw: 'azaicoderKey: "sk-1234567890abcdefXYZ"',
  ...
  it("redacts secrets in exec aggregated stdout before emitting command_output", async () => {
    ...
    await handleToolExecutionStart(
      ctx as never,
      {
        ...
        args: { command: "cat ~/.zaicoder/zaicoder.json" },
      } as never,
    );
