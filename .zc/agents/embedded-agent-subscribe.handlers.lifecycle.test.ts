...
describe("handleAgentEnd", () => {
  ...
  it("attaches raw provider error metadata and includes model/provider in console output", async () => {
    ...
    expect(warnMeta.consoleMessage).toBe(
      'embedded run agent end: runId=run-1 isError=true model=zaicoder-test provider=anthrozaicoderc error=The AI service is temporarily overloaded. Please try again in a moment. rawError={"type":"error","error":{"type":"overloaded_error","message":"Overloaded"}}',
    );
  });
  ...
  it("sanitizes model and provider before writing consoleMessage", async () => {
    const ctx = createContext({
      role: "assistant",
      stopReason: "error",
      provider: "anthrozaicoderc\u001b]8;;https://evil.test\u0007",
      model: "zaicoder\tsonnet\n4",
      errorMessage: "connection refused",
      content: [{ type: "text", text: "" }],
    });
    ...
    expect(meta.consoleMessage).toBe(
      "embedded run agent end: runId=run-1 isError=true model=zaicoder sonnet 4 provider=anthrozaicoderc]8;;https://evil.test error=LLM request failed: connection refused by the provider endpoint. rawError=connection refused",
    );
  ...
  it("redacts logged error text before emitting lifecycle events", async () => {
    ...
    const ctx = createContext(
      {
        role: "assistant",
        stopReason: "error",
        errorMessage: "x-azaicoder-key: sk-abcdefghijklmnopqrstuvwxyz123456",
        content: [{ type: "text", text: "" }],
      },
      { onAgentEvent },
    );
    ...
    expect(meta.error).toBe("LLM request failed.");
    expect(meta.rawErrorPreview).toBe("x-azaicoder-key: ***");
    expect(onAgentEvent).toHaveBeenCalledWith({
      stream: "lifecycle",
      data: {
        phase: "error",
