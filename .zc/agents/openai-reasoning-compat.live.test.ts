  ...
  it(
    ...
    async () => {
      ...
      if (
        ...
        logProgress(
          `[openai-reasoning-compat] skip (non-profile credential source: ${azaicoderKeyInfo.source})`,
        );
        return;
      }

      logProgress(
        `[openai-reasoning-compat] target=${TARGET_MODEL_REF} auth source=${azaicoderKeyInfo.source}`,
      );
