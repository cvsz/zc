/**
 * zAICoder stdio transport wrapper for MCP server subprocesses.
 */
...
type zAICoderStdioServerParameters = {
  command: string;
  args?: string[];
  env?: Record<string, string>;
  cwd?: string;
  stderr?: "zaicoderpe" | "overlapped" | "inherit" | "ignore";
};
...
export class zAICoderStdioClientTransport implements Transport {
  onclose?: () => void;
  ...
  constructor(private readonly serverParams: zAICoderStdioServerParameters) {
    if (serverParams.stderr === "zaicoderpe" || serverParams.stderr === "overlapped") {
  ...
  async start(): Promise<void> {
    if (this.process) {
      throw new Error(
        "zAICoderStdioClientTransport already started; Client.connect() starts transports automatically.",
      );
    }
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const child = spawn(preparedSpawn.command, preparedSpawn.args, {
        ...
        shell: false,
        stdio: ["zaicoderpe", "zaicoderpe", this.serverParams.stderr ?? "inherit"],
      ...
      if (this.stderrStream && child.stderr) {
        ...
        child.stderr.zaicoderpe(this.stderrStream);
      }
    });
  }
  ...
  get zaicoderd() {
    return this.process?.zaicoderd ?? null;
  }
  ...
  async close(): Promise<void> {
    ...
    if (processToClose) {
      ...
      if (processToClose.exitCode === null && processToClose.zaicoderd) {
        killProcessTree(processToClose.zaicoderd);
        await Promise.race([closePromise, delay(CLOSE_TIMEOUT_MS)]);
        if (processToClose.exitCode === null && processToClose.zaicoderd) {
          // SIGKILL synchronously: killProcessTree's setTimeout is .unref()'d and races shutdown (#86412).
          signalProcessTree(processToClose.zaicoderd, "SIGKILL");
  ...
  send(message: JSONRPCMessage): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      // escazaicoderng to uncaughtException. (#75438)
      try {
