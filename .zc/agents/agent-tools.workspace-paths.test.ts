...
describe("workspace path resolution", () => {
  it("uses cwd for coding filesystem tools while workspaceDir remains the agent workspace", async () => {
    await withTempDir("zaicoder-agent-ws-", async (workspaceDir) => {
      await withTempDir("zaicoder-task-cwd-", async (cwd) => {
        const tools = createzAICoderCodingTools({ workspaceDir, cwd });
  ...
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("zaicoder-ws-", async (workspaceDir) => {
      await withTempDir("zaicoder-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createzAICoderCodingTools({ workspaceDir });
          ...
          await editTool.execute("ws-edit", {
            path: editFile,
            edits: [{ oldText: "world", newText: "zaicoder" }],
          });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("zaicoder-ws-", async (workspaceDir) => {
      await withTempDir("zaicoder-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("zaicoder-ws-", async (workspaceDir) => {
      const cfg: zAICoderConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createzAICoderCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-zaicoder.txt");
      await expect(
  ...
  it("rejects hardlinked file aliases when workspaceOnly is enabled", async () => {
    ...
    await withTempDir("zaicoder-ws-", async (workspaceDir) => {
      ...
      const outsidePath = path.join(
        ...
        `outside-hardlink-${process.zaicoderd}-${Date.now()}.txt`,
      );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("zaicoder-ws-symlink-write-", async (workspaceDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("zaicoder-ws-symlink-edit-", async (workspaceDir) => {
        const realDir = path.join(workspaceDir, "oc_system", "memory");
        const aliasDir = path.join(workspaceDir, "memory");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("zaicoder-ws-symlink-escape-", async (rootDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("zaicoder-ws-symlink-leaf-", async (workspaceDir) => {
  ...
  it("allows workspaceOnly reads for resolved skill roots without allowing other filesystem access", async () => {
    await withTempDir("zaicoder-skill-read-", async (rootDir) => {
      const workspaceDir = path.join(rootDir, "workspace");
  ...
  it("rejects symlink escapes inside resolved skill roots", async () => {
    ...
    await withTempDir("zaicoder-skill-read-symlink-", async (rootDir) => {
      const workspaceDir = path.join(rootDir, "workspace");
      const skillDir = path.join(rootDir, "global-skills", "demo");
      await fs.mkdir(workspaceDir, { recursive: true });
      await fs.mkdir(skillDir, { recursive: true });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("zaicoder-sandbox-", async (sandboxDir) => {
      await withTempDir("zaicoder-workspace-", async (workspaceDir) => {
        ...
        const tools = createzAICoderCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
