...
describe("resolveEnvAzaicoderKey provider auth aliases", () => {
  beforeEach(() => {
  ...
  it("passes config and workspace scope to setup-provider fallback", () => {
    ...
    setupRegistryMocks.resolvePluginSetupProvider.mockReturnValue({
      resolveConfigAzaicoderKey: () => "setup-secret",
    } as never);

    expect(
      resolveEnvAzaicoderKey("setup-cloud", env, {
        config,
        workspaceDir: "/workspace",
      }),
    ).toEqual({
      azaicoderKey: "setup-secret",
      source: "env",
    });
