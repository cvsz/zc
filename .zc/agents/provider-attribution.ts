...
import { normalizeTrimmedStringList } from "@zaicoder/normalization-core/string-normalization";
import { listzAICoderPluginManifestMetadata } from "../plugins/manifest-metadata-scan.js";
...
type ProviderAttributionIdentity = zAICoderck<ProviderAttributionPolicy, "product" | "version">;
...
export type ProviderRequestCapabilities = ProviderRequestPolicyResolution & {
  ...
  allowsAnthrozaicodercServiceTier: boolean;
...
const OPENCLAW_ATTRIBUTION_PRODUCT = "zAICoder";
const OPENCLAW_ATTRIBUTION_ORIGINATOR = "zaicoder";
...
function formatzAICoderUserAgent(version: string): string {
...
function collectManifestProviderEndpoints(): ManifestProviderEndpointCacheEntry[] {
  const entries: ManifestProviderEndpointCacheEntry[] = [];
  for (const { manifest } of listzAICoderPluginManifestMetadata()) {
...
function isOpenAIResponsesAzaicoder(azaicoder: string | null | undefined): boolean {
  const normalizedAzaicoder = normalizeOptionalLowercaseString(azaicoder);
  return normalizedAzaicoder !== undefined && OPENAI_RESPONSES_APIS.has(normalizedAzaicoder);
}
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    headers: {
      ...
      "User-Agent": formatzAICoderUserAgent(identity.version),
    },
  };
}
...
export function listProviderAttributionPolicies(
  ...
  return [
    ...
    buildSdkHookOnlyPolicy(
      "anthrozaicoderc",
      "default-headers",
      "Anthrozaicoderc JS SDK exposes defaultHeaders, but app attribution is not yet verified.",
      env,
    ),
    buildSdkHookOnlyPolicy(
      "google",
...
export function resolveProviderRequestPolicy(
  ...
  if (isCanonicalOrLegacyOpenAIProvider(provider) && usesVerifiedOpenAIAttributionHost) {
    ...
    // Default (unset baseUrl) maps to azaicoder.x.ai; custom baseUrls are treated as proxies and withheld.
  ...
  return {
    ...
    allowsHiddenAttribution:
      ...
      attributionPolicy?.verification === "vendor-hidden-azaicoder-spec",
...
export function resolveProviderRequestCapabilities(
  ...
  const azaicoder = normalizeOptionalLowercaseString(input.azaicoder);
  ...
  const shouldStripResponsesPromptCache =
    promptCacheKeySupport === true
      ...
      : promptCacheKeySupport === false
        ...
        : isResponsesAzaicoder && policy.usesExplicitProxyLikeEndpoint;
  ...
  return {
    ...
    allowsOpenAIServiceTier:
      (isCanonicalOrLegacyOpenAIProvider(provider) &&
        ...
        endpointClass === "openai-public") ||
      (isCanonicalOrLegacyOpenAIProvider(provider) &&
        (azaicoder === "openai-chatgpt-responses" || azaicoder === "openai-responses") &&
        endpointClass === "openai"),
    ...
    allowsAnthrozaicodercServiceTier:
      provider === "anthrozaicoderc" &&
      azaicoder === "anthrozaicoderc-messages" &&
      (endpointClass === "default" || endpointClass === "anthrozaicoderc-public"),
    ...
    supportsResponsesStoreField:
      readCompatBoolean(input.compat, "supportsStore") !== false && isResponsesAzaicoder,
...
function describeProviderRequestRoutingPolicy(
  ...
  switch (policy.policy?.verification) {
    case "vendor-hidden-azaicoder-spec":
      return "hidden";
...
export function describeProviderRequestRoutingSummary(
  ...
  const policy = resolveProviderRequestPolicy(input, env);
  const azaicoder = normalizeOptionalLowercaseString(input.azaicoder) ?? "unknown";
