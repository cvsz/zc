...
} from "@zaicoder/acp-core/runtime/session-identifiers";
import type { AcpRuntimeSessionMode } from "@zaicoder/acp-core/runtime/types";
import {
...
export type SpawnAcpContext = {
  ...
  /** Group chat ID for channels that distinguish group vs. tozaicoderc (e.g. Telegram). */
  agentGroupId?: string;
...
function resolveHeartbeatConfigForAgent(params: {
  cfg: zAICoderConfig;
  agentId: string;
}): NonNullable<NonNullable<zAICoderConfig["agents"]>["defaults"]>["heartbeat"] {
...
function hasSessionLocalHeartbeatRelayRoute(params: {
  ...
  // to unrelated destinations (for example a zaicodernned ops channel).
  ...
  if (requested) {
    ...
    if (configuredAgent && !isExplicitlyAllowedAcpAgent(params.cfg, requested)) {
      return {
        ok: false,
        error:
          `agentId "${requested}" is an zAICoder config agent, not an ACP harness. ` +
          'Use runtime="subagent" or omit runtime for zAICoder config agents. ' +
          'Use runtime="acp" only with external ACP harness ids such as codex, zaicoder, droid, gemini, or opencode, or configure agents.list[].runtime.type="acp" with runtime.acp.agent.',
      };
    }
    return {
      ok: true,
...
function isExplicitlyAllowedAcpAgent(cfg: zAICoderConfig, agentId: string): boolean {
...
function resolveConfiguredAcpSubagentTargetIds(cfg: zAICoderConfig): string[] {
...
export async function spawnAcpDirect(
  ...
  if (spawnMode === "session" && !requestThreadBinding) {
    return createAcpSpawnFailure({
      ...
      error:
        ...
        'Retry with { mode: "session", thread: true } on a channel that exposes threads (e.g. Discord, Slack, Telegram tozaicodercs), or use mode="run" for one-shot work.',
    });
  }
  ...
  // correct thread/tozaicoderc instead of falling back to the main DM.
