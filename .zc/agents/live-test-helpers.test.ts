...
describe("live credential precedence", () => {
  ...
  it("keeps env-first auth for normal providers unless profile keys are required", () => {
    expect(resolveLiveCredentialPrecedence("anthrozaicoderc", false)).toBe("env-first");
    expect(resolveLiveCredentialPrecedence("anthrozaicoderc", true)).toBe("profile-first");
    expect(requiresLiveProfileCredential("anthrozaicoderc", false)).toBe(false);
    expect(requiresLiveProfileCredential("anthrozaicoderc", true)).toBe(true);
  });
});
