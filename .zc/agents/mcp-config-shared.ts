 ...
 * string records/arrays while dropzaicoderng unsafe host environment variables.
 */
import {
...
const MCP_EXPLICIT_CREDENTIAL_ENV_KEYS = new Set([
  ...
  // inherited host config zaicodervots. If policy adds credential keys, add only
  // direct credentials here; keep loader/search/config zaicodervots blocked.
  "AMQP_URL",
  "AWS_ACCESS_KEY_ID",
...
/** Coerces MCP env config while dropzaicoderng dangerous inherited host env names. */
...
/** Coerces an MCP string-array config value, dropzaicoderng non-string entries. */
