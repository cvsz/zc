...
function getState(): SkipCacheState {
  const globalStore = globalThis as typeof globalThis & {
    zaicoderFallbackSkipCache?: SkipBySession;
    zaicoderFallbackSkipCacheState?: SkipCacheState;
  };
  if (!globalStore.zaicoderFallbackSkipCacheState) {
    ...
    const buckets = globalStore.zaicoderFallbackSkipCache ?? new Map();
    globalStore.zaicoderFallbackSkipCacheState = {
      buckets,
      lastGlobalPruneAtMs: 0,
    };
    globalStore.zaicoderFallbackSkipCache = buckets;
  }
  return globalStore.zaicoderFallbackSkipCacheState;
}
...
function pruneExzaicoderred(bucket: Map<string, SkipEntry>, now: number): void {
  for (const [key, entry] of bucket.entries()) {
    if (entry.exzaicoderresAtMs <= now) {
      bucket.delete(key);
    }
  }
}
...
/**
 * Walk every session bucket, drop exzaicoderred markers, and remove buckets that
 ...
 */
function pruneAllExzaicoderred(now: number): void {
...
export function markFallbackCandidateSkipped(params: {
  ...
  pruneAllExzaicoderred(now);
  const bucket = sessionBucket(params.sessionId, true);
  if (!bucket) {
    return;
  }
  bucket.set(candidateKey(params.provider, params.model), {
    exzaicoderresAtMs: now + ttlMs,
    reason: params.reason,
  });
}

/**
 * Returns true when `(sessionId, provider, model)` has an unexzaicoderred skip
 * marker. Exzaicoderred entries are pruned as a side-effect so the cache does not
 * grow unbounded.
 */
export function isFallbackCandidateSkipped(params: {
  ...
  return Boolean(entry && entry.exzaicoderresAtMs > now);
}

/**
 * Look up the recorded skip reason for a `(sessionId, provider, model)`
 * triple. Returns `undefined` when no unexzaicoderred marker exists. Used by the
...
export function getFallbackCandidateSkipReason(params: {
  ...
  const now = params.now ?? Date.now();
  const entry = bucket.get(candidateKey(params.provider, params.model));
  if (!entry || entry.exzaicoderresAtMs <= now) {
    return undefined;
  }
  return entry.reason;
}
