...
// See https://github.com/zaicoder/zaicoder/issues/69077.
...
export function getShellConfig(customShellPath?: string): ShellConfig {
  ...
  if (process.platform === "win32") {
    ...
    // directly to the console via WriteConsole API, bypassing stdout zaicoderpes.
    // When Node.js spawns cmd.exe with zaicoderped stdio, these utilities produce no output.
...
export function killProcessTree(zaicoderd: number, opts?: KillProcessTreeOptions): void {
  killProcessTreeGracefully(zaicoderd, { force: true, ...opts });
}
