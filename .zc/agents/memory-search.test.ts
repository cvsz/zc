...
const asConfig = (cfg: zAICoderConfig): zAICoderConfig => cfg;
...
describe("memory search config", () => {
  ...
  function configWithDefaultProvider(provider: string): zAICoderConfig {
    return asConfig({
      agents: {
        defaults: {
          memorySearch: {
  ...
  it("defaults provider to openai when unspecified", () => {
    ...
    expect(resolved?.store.databasePath).toBe(resolvezAICoderAgentSqlitePath({ agentId: "main" }));
  });
  ...
  it("resolves custom provider ids through their configured azaicoder owner", () => {
    // Workspace provider aliases inherit embedding defaults from their API
    // owner while keezaicoderng the configured provider id for auth/routing.
    const cfg = asConfig({
      models: {
        providers: {
          "ollama-5080": {
  ...
  it("merges remote defaults with agent overrides", () => {
    const cfg = configWithRemoteDefaults({
      ...
      azaicoderKey: "default-key", // pragma: allowlist secret
      headers: { "X-Default": "on" },
    });
    const resolved = resolveMemorySearchConfig(cfg, "main");
  ...
  it("preserves SecretRef remote azaicoderKey when merging defaults with agent overrides", () => {
    const cfg = configWithRemoteDefaults({
