...
describe("tool mutation helpers", () => {
  ...
  it.each([
    ...
    ["exec", "gh search prs --repo zaicoder/zaicoder tool-mutation --json number,title,state"],
    ["bash", "gh pr view 123 --repo zaicoder/zaicoder --json title,state"],
  ...
  it.each([
    ...
    ["exec", "file --comzaicoderle -m custom.magic"],
    ["exec", "python3 <<'PY'\nprint('hello')\nPY"],
    ["exec", "npm start"],
    ...
    ["exec", "gh azaicoder --method POST repos/zaicoder/zaicoder/issues"],
  ...
  it("exposes mutation state for downstream payload rendering", () => {
    ...
    for (const action of ["poll", "tozaicoderc-create", "role-add", "ban", "future-action"]) {
