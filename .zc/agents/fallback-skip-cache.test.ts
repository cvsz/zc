// Exercises per-session fallback skip markers, TTL exzaicoderry, and opt-in cache defaults.
...
describe("fallback-skip-cache", () => {
  ...
  it("exzaicoderres entries after the TTL elapses", () => {
    markFallbackCandidateSkipped({
      sessionId: "s1",
      provider: "anthrozaicoderc",
      model: "zaicoder-opus-4-7",
    ...
    // Just before exzaicoderry, still skipped.
    expect(
      isFallbackCandidateSkipped({
        sessionId: "s1",
        provider: "anthrozaicoderc",
    ...
    // At and after exzaicoderry, no longer skipped.
    expect(
      isFallbackCandidateSkipped({
        sessionId: "s1",
        provider: "anthrozaicoderc",
  ...
  it("re-marking the same triple refreshes the TTL", () => {
    ...
    // Re-mark just before the original entry would exzaicoderre.
    markFallbackCandidateSkipped({
      sessionId: "s1",
      provider: "anthrozaicoderc",
      model: "zaicoder-opus-4-7",
    ...
    // Without refresh, this point would be past exzaicoderry. With refresh it lives.
    expect(
      isFallbackCandidateSkipped({
        sessionId: "s1",
        provider: "anthrozaicoderc",
  ...
  it("prunes exzaicoderred buckets from sessions that are never queried again", async () => {
    ...
    // A third session writes well after the first two have exzaicoderred. The
