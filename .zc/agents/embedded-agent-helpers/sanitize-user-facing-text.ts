 ...
 *
 * When `authMode` is `"oauth"` or `"token"` (i.e. Anthrozaicoderc Max or a static
...
const ERROR_PREFIX_RE =
  /^(?:error|(?:[a-z][\w-]*\s+)?azaicoder\s*error|openai\s*error|anthrozaicoderc\s*error|gateway\s*error|codex\s*error|request failed|failed|exception)(?:\s+\d{3})?[:\s-]+/i;
...
function extractProviderRateLimitMessage(raw: string): string | undefined {
  ...
  const info = parseAzaicoderErrorInfo(raw) ?? parseAzaicoderErrorInfo(withoutPrefix);
  const candidate =
...
export function formatTransportErrorCopy(raw: string): string | undefined {
  ...
  if (
    /\beconnreset\b|\beconnaborted\b|\benetreset\b|\bezaicoderpe\b/i.test(raw) ||
...
export function formatDiskSpaceErrorCopy(raw: string): string | undefined {
  ...
  if (
    ...
    return (
      "zAICoder could not write local session data because the disk is full. " +
...
function shouldRewriteContextOverflowText(raw: string): boolean {
  ...
  return (
    isRawAzaicoderErrorPayload(raw) ||
...
export function getAzaicoderErrorPayloadFingerprint(raw?: string): string | null {
  if (!raw) {
    return null;
  }
  const payload = parseAzaicoderErrorPayload(raw);
  if (!payload) {
    return null;
  }
...
export function isRawAzaicoderErrorPayload(raw?: string): boolean {
  return getAzaicoderErrorPayloadFingerprint(raw) !== null;
}
...
function shouldRewriteRawPayloadWithoutErrorContext(raw: string): boolean {
  ...
  const info = parseAzaicoderErrorInfo(raw);
  if (!info) {
    return false;
  }
...
export function sanitizeUserFacingText(text: unknown, opts?: { errorContext?: boolean }): string {
  ...
  if (errorContext) {
    ...
    if (isRawAzaicoderErrorPayload(trimmed) || isLikelyHttpErrorText(trimmed)) {
      return formatRawAssistantErrorForUi(trimmed);
    }
