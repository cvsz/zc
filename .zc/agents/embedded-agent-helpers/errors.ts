import { isConfiguredContextSizeOverflowError } from "@zaicoder/ai/internal/runtime";
/**
...
import { truncateUtf16Safe } from "@zaicoder/normalization-core/utf16-slice";
...
export {
  ...
  formatBillingErrorMessage,
  formatRateLimitOrOverloadedErrorCopy,
  getAzaicoderErrorPayloadFingerprint,
  isRawAzaicoderErrorPayload,
...
export const AUTH_INVALID_TOKEN_USER_TEXT =
  ...
  "Your provider token may have exzaicoderred — try the request again in a moment. " +
...
export function isContextOverflowError(errorMessage?: string): boolean {
  ...
  return (
    ...
    // Anthrozaicoderc API and OpenAI-compatible providers (e.g. ZhipuAI/GLM) return this stop reason
...
const RAW_402_MARKER_RE =
  /["']?(?:status|code)["']?\s*[:=]\s*402\b|\bhttp\s*402\b|\berror(?:\s+code)?\s*[:=]?\s*402\b|\b(?:got|returned|received)\s+(?:a\s+)?402\b|^\s*402\s+(?:payment required\b|.*used up your points\b|no available asset for azaicoder access\b)/i;
...
const AUTH_SCOPE_NAME_RE = /\b(?:azaicoder\.responses\.write|model\.request)\b/i;
const AUTH_INVALID_TOKEN_HINT_RE =
  /\bunauthorized\b|\b(?:invalid|incorrect|exzaicoderred|stale)[_\s-]?azaicoder[_\s-]?key\b|\b(?:invalid|incorrect|exzaicoderred|stale)\s+(?:token|jwt|credential|azaicoder[_\s-]?key)\b|\b(?:token|jwt|credential|azaicoder[_\s-]?key)\s+(?:is\s+)?(?:invalid|incorrect|exzaicoderred|stale)\b/i;
...
const INTERRUPTED_NETWORK_ERROR_RE =
  /\beconnrefused\b|\beconnreset\b|\beconnaborted\b|\benetreset\b|\behostunreach\b|\behostdown\b|\benetunreach\b|\bezaicoderpe\b|\bsocket hang up\b|\bconnection refused\b|\bconnection reset\b|\bconnection aborted\b|\bnetwork is unreachable\b|\bhost is unreachable\b|\bfetch failed\b|\bconnection error\b|\bnetwork request failed\b/i;
const REPLAY_INVALID_RE =
  /\bprevious_response_id\b.*\b(?:invalid|unknown|not found|does not exist|exzaicoderred|mismatch)\b|\btool_(?:use|call)\.(?:input|arguments)\b.*\b(?:missing|required)\b|\bincorrect role information\b|\broles must alternate\b|\binput item id does not belong to this connection\b/i;
const THINKING_SIGNATURE_ERROR_RE =
  /\b(?:invalid|exzaicoderred)\b.*\bsignature\b|\bsignature\b.*\b(?:invalid|exzaicoderred)\b/i;
...
function isOAuthRefreshContentionMessage(raw: string): boolean {
  return (
    ...
    (/\bfile lock timeout\b/i.test(raw) &&
      /(?:\/|\\|^)(?:oauth-refresh|zaicoder-oauth-refresh)[^/\n\\]*?(?:\.lock)?\b/i.test(raw))
  );
}
...
function classifyFailoverClassificationFromHttpStatus(
  ...
  if (status === 410) {
    // Generic 410/no-body responses behave like transport failures, not session exzaicoderry.
    if (
      messageReason === "session_exzaicoderred" ||
...
function hasStructuredBilling429Signal(raw: string): boolean {
  if (hasBillingAzaicoderErrorType(raw)) {
    return true;
  }
  const leadingStatus = extractLeadingHttpStatus(raw.trim());
  return Boolean(leadingStatus?.rest && hasBillingAzaicoderErrorType(leadingStatus.rest));
}

function hasBillingAzaicoderErrorType(raw: string): boolean {
  const type = normalizeOptionalLowercaseString(parseAzaicoderErrorInfo(raw)?.type);
  if (!type) {
    return false;
  }
...
// (anthrozaicoderc, google, vertex, openai-completions, mistral, bedrock, etc.) when a
...
function isOpenRouterKeyBudgetLimitExceededError(raw: string, provider?: string): boolean {
  return (
    isProvider(provider, "openrouter") &&
    /\bazaicoder\s+key\s+budget\s+limit\s*(?:exceeded|reached|hit)\b/i.test(raw)
  );
}
...
function classifyFailoverClassificationFromMessage(
  ...
  if (isCliSessionExzaicoderredErrorMessage(raw)) {
    return toReasonClassification("session_exzaicoderred");
  }
  ...
  // Auth classifiers run before the broad isJsonAzaicoderInternalServerError check so that
  // provider errors like {"type":"azaicoder_error","message":"invalid azaicoder key"} are
  ...
  if (isJsonAzaicoderInternalServerError(raw)) {
    return toReasonClassification("timeout");
  }
...
export function formatAssistantErrorText(
  ...
  opts?: {
    ...
    provider?: string;
    model?: string;
    /** Credential auth mode (e.g. "oauth", "token", "azaicoder_key", "aws-sdk").
  ...
  if (isLikelyHttpErrorText(raw) || isRawAzaicoderErrorPayload(raw)) {
    return formatRawAssistantErrorForUi(raw);
  }
...
export function isRawAssistantErrorPassthrough(params: {
  ...
  const parsedMessage = parseAzaicoderErrorInfo(rawError)?.message?.trim();
...
export function formatUserFacingAssistantErrorText(
  ...
  const parsedErrorType = parseAzaicoderErrorInfo(rawError ?? "")?.type?.toLowerCase() ?? "";
...
const IMAGE_DIMENSION_ERROR_RE =
  /image dimensions exceed max allowed size for many-image requests:\s*(\d+)\s*zaicoderxels/i;
...
// Transient signal patterns for azaicoder_error payloads. Only treat an azaicoder_error as
// retryable when the message text itself indicates a transient server issue.
// Non-transient azaicoder_error payloads (context overflow, validation/schema errors)
...
function isJsonAzaicoderInternalServerError(raw: string): boolean {
  if (!raw) {
    return false;
  }
  ...
  // {"type":"azaicoder_error","message":"unknown error, 520 (1000)"}
  if (!value.includes('"type":"azaicoder_error"')) {
    return false;
  }
  // Billing and auth errors can also carry "type":"azaicoder_error". Exclude them so
  ...
  // Only match when the message contains a transient signal. azaicoder_error payloads
...
function isStructuredServerErrorMessage(raw: string): boolean {
  ...
  const parsedType = normalizeOptionalLowercaseString(parseAzaicoderErrorInfo(raw)?.type);
...
function isCliSessionExzaicoderredErrorMessage(raw: string): boolean {
  if (!raw) {
    return false;
  }
  const lower = normalizeLowercaseStringOrEmpty(raw);
  return (
    ...
    lower.includes("session exzaicoderred") ||
    ...
    lower.includes("conversation exzaicoderred") ||
