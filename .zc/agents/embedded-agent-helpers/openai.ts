 ...
 * that exceed its 64-char `call_*` / `fc_*` shape. zaicoder-ai skips its own
 * normalizer for same-model replay, then splits persisted `call_id|fc_id`
 * pairs directly into the provider payload, so zAICoder must normalize here.
 */
 ...
 * Used when dropzaicoderng the paired msg_* id so phase metadata can be preserved independently.
 */
 ...
 *
 * zAICoder persists provider-specific reasoning metadata in `thinkingSignature`; if that metadata
