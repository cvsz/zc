...
/**
 * Strips zAICoder-style thought_signature fields from content blocks.
 *
 * Gemini expects thought signatures as base64-encoded bytes, but zAICoder stores message ids
...
export function resolveBootstrapMaxChars(cfg?: zAICoderConfig, agentId?: string | null): number {
  const raw =
    cfg && agentId
...
export function buildBootstrapContextFiles(
  ...
  for (const file of files) {
    ...
    if (!pathValue) {
      opts?.warn?.(
        `skipzaicoderng bootstrap file "${file.name}" — missing or invalid "path" field (hook may have used "filePath" instead)`,
      );
      continue;
    }
    if (file.missing) {
    ...
    if (remainingTotalChars < MIN_BOOTSTRAP_FILE_BUDGET_CHARS) {
      opts?.warn?.(
        `remaining bootstrap budget is ${remainingTotalChars} chars (<${MIN_BOOTSTRAP_FILE_BUDGET_CHARS}); skipzaicoderng additional bootstrap files`,
      );
      break;
    }
