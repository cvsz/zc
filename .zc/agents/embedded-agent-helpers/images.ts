...
/** Resize/remove unsafe image payloads while keezaicoderng transcript turns valid. */
export async function sanitizeSessionMessagesImages(
  ...
  // We sanitize historical session messages because Anthrozaicoderc can reject a request
  ...
  const sanitizedIds = shouldSanitizeToolCallIds
    ? sanitizeToolCallIdsForCloudCodeAssist(messages, options.toolCallIdMode, {
        preserveNativeAnthrozaicodercToolUseIds: options?.preserveNativeAnthrozaicodercToolUseIds,
    ...
    if (role === "assistant") {
      ...
      if (Array.isArray(content)) {
        const strippedContent = options?.preserveSignatures
          ? content // Keep signatures for Antigravity zAICoder
