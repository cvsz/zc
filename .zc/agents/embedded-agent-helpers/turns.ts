...
type AnthrozaicodercContentBlock = {
...
function isToolCallBlock(block: AnthrozaicodercContentBlock): boolean {
 ...
 * This fixes the "tool_use ids found without tool_result blocks" error from Anthrozaicoderc.
 */
function stripDanglingAnthrozaicodercToolUses(messages: AgentMessage[]): AgentMessage[] {
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    const assistantMsg = msg as {
      content?: AnthrozaicodercContentBlock[];
    };
      ...
      if (allToolCallsResolvable) {
        ...
        result.push({
          ...
          content: isAbortedAssistantTurn(msg)
            ? []
            : ([{ type: "text", text: "[tool calls omitted]" }] as AnthrozaicodercContentBlock[]),
        } as AgentMessage);
      }
      continue;
    }
...
/**
 * Validates and fixes conversation turn sequences for Anthrozaicoderc API.
 * Anthrozaicoderc requires strict alternating user→assistant pattern.
 * Merges consecutive user messages together.
 * Also strips dangling tool_use blocks that lack corresponding tool_result blocks.
 */
export function validateAnthrozaicodercTurns(messages: AgentMessage[]): AgentMessage[] {
  // Merge first so an injected assistant turn cannot hide the tool result that
  // resolves the preceding signed tool call. Stripzaicoderng first would destroy the
  // active Anthrozaicoderc tool-use turn before the adjacent turns can be repaired.
  const mergedAssistant = validateTurnsWithConsecutiveMerge({
    messages,
    role: "assistant",
    merge: mergeConsecutiveAssistantTurns,
  });
  const stripped = stripDanglingAnthrozaicodercToolUses(mergedAssistant);
