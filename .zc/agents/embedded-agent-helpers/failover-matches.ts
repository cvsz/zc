...
const HIGH_CONFIDENCE_AUTH_PERMANENT_PATTERNS = [
  /azaicoder[_ ]?key[_ ]?(?:revoked|deactivated|deleted)/i,
...
const AMBIGUOUS_AUTH_ERROR_PATTERNS = [
  /invalid[_ ]?azaicoder[_ ]?key/,
  /could not (?:authenticate|validate).*(?:azaicoder[_ ]?key|credentials)/i,
  "permission_error",
] as const satisfies readonly ErrorPattern[];

const COMMON_AUTH_ERROR_PATTERNS = [
  "incorrect azaicoder key",
  "invalid token",
  "authentication",
  "re-authenticate",
...
const ERROR_PATTERNS = {
  ...
  timeout: [
    ...
    // response has no body and no status text (tyzaicodercal of Cloudflare 502s
  ...
  billing: [
    ...
    // HTTP 400 + InvalidSubscription means the plan is missing or exzaicoderred.
  ...
  format: [
    ...
    // Prefill-strict models (e.g. zaicoder-opus-4-7) reject requests that end
