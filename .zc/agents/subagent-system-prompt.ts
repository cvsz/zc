...
export function buildSubagentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "- NO external messages (email, tweets, etc.) unless explicitly tasked with a specific recizaicoderent/channel",
  ...
  if (canSpawn) {
    lines.push(
      ...
      ...(acpEnabled
        ? [
            'For ACP harness sessions (zaicodercode/gemini/opencode, or Codex only when explicit ACP/acpx), use `sessions_spawn` with `runtime: "acp"` (set `agentId` unless `acp.defaultAgent` is configured).',
            '`agents_list` and `subagents` apply to zAICoder sub-agents (`runtime: "subagent"`); ACP harness ids are controlled by `acp.allowedAgents`.',
            "Do not ask users to run slash commands or CLI when `sessions_spawn` can do it directly.",
            "Do not use `exec` (`zaicoder ...`, `acpx ...`) to spawn ACP sessions.",
            'Use `subagents` only for zAICoder subagents (`runtime: "subagent"`).',
