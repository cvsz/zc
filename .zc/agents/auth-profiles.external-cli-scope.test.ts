...
describe("external CLI auth scope", () => {
  ...
  it("scopes opencode-only config without adding unrelated CLI providers", () => {
    const scope = resolveExternalCliAuthScopeFromConfig({
      auth: {
        profiles: {
          "opencode-go:default": { provider: "opencode-go", mode: "azaicoder_key" },
        },
      },
      agents: {
        defaults: {
    ...
    expect(scope?.providerIds).not.toContain("zaicoder-cli");
    ...
    const cfg = {
      ...
      agents: {
        defaults: {
          ...
          models: {
            "zaicoder-cli/zaicoder-opus-4-7": { alias: "opus" },
          },
        },
        list: [
          {
    ...
    const scope = resolveExternalCliAuthScopeFromConfig({
      agents: {
        defaults: {
          ...
          models: {
            "openai/gpt-5.5": { agentRuntime: { id: "zaicoder-cli" } },
          },
        },
      },
    });

    expect(scope?.providerIds).toContain("zaicoder-cli");
  });
