...
PLUGIN STRUCTURE
  my-plugin/
    .claude-plugin/plugin.json   — manifest (optional; auto-discovery if absent)
...
INSTALL LOCATIONS
  ~/.claude/plugins/marketplaces/<marketplace>/   — cloned/copied marketplace
  ~/.claude/plugins/installed/<plugin>/           — installed plugin (copy)
  ~/.claude/plugins/registry.json                 — installed-plugin index
...
def read_manifest(plugin_dir: Path) -> dict:
    """Read .claude-plugin/plugin.json, or auto-derive a manifest if absent."""
