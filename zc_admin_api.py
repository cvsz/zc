"""
zc_admin_api.py — Admin API: Usage & Cost, API keys, Spend Limits, Rate Limits, ZaiCoder Code Analytics
...
CLI flags:
  ...
  --zc-code-usage-report      Print daily per-user ZaiCoder Code productivity metrics (v1.24.0)
  --zc-code-usage-report-start DATE  Date (YYYY-MM-DD) for --zc-code-usage-report, default: yesterday
"""
...
class AdminApiClient:
    ...
    # ── ZaiCoder Code Analytics API (v1.24.0) ──────────────────────────────
    ...
    def get_zc_code_usage_report(self, starting_at: str, limit: int = 20,
        ...
        commits/PRs created through ZaiCoder Code, per-editing-tool
...
def cmd_zc_code_usage_report(admin_api_key: str, starting_at: str, limit: int = 20):
    """--zc-code-usage-report: daily, per-user ZaiCoder Code productivity
    ...
    if "error" in data:
        print(f"\033[91m✗ ZaiCoder Code usage report failed: {data['error']}\033[0m")
        if data.get("status") in (401, 403):
            print("\033[93m  This endpoint requires an Admin API key (sk-ant-admin...), "
                 "not a regular API key.\033[0m")
        return None

    print(f"\n\033[94mZaiCoder Code usage report — {starting_at}\033[0m\n")
    rows = data.get("data", [])
    if not rows:
        print("  (no ZaiCoder Code activity for this date)")
    for row in rows:
