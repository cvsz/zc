...
SUBAGENTS
  • Spawn from .claude/agents/<name>.md definitions
  • Custom subagent YAML frontmatter: name, description, tools, disallowedTools
  • builtin agent types

SLASH COMMANDS
  • /clear /compact /help /model /status /cost /memory /vim
  • Custom slash commands from .claude/commands/ or .claude/skills/<n>/SKILL.md

AGENT SKILLS
  • Load from .claude/skills/<name>/SKILL.md
...
class CodeAgent:
    """
    Full Claude Code / Agent SDK implementation using the Messages API.
...
def cmd_code_slash(command: str, api_key: str, model: str,
    ...
    if cmd == "compact":
        ...
        # see claude_tools.build_context_management) against an actual
        ...
        # in claude_tools.py for the pause_after_compaction variant).
        session.turns = [{
            "role": "assistant",
