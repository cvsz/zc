"""
claude_word.py — Conversational Word-document assistant (Skills API only)
AI Model Coder CLI v1.33.0

A chat loop for .docx files, same shape as --excel-native/--pptx-native in
claude_excel.py / claude_powerpoint.py: each turn is a Messages API call
carrying the docx Skill (claude_skills_api.py) in a code-execution
...
def cmd_docx_chat(api_key, model, input_path=None, output_path=None, max_tokens=4096):
    """Mirrors claude_powerpoint.py's _cmd_pptx_chat_native /
