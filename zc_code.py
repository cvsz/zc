"""
zc_code.py — ZaiCoder Code / Agent SDK (all features)
AI Model Coder CLI v1.8.0

Implements every ZaiCoder Code / Agent SDK feature from docs.zaicoder.com:
...
MEMORY (CLAUDE.md)
  • Project-level: .zc/CLAUDE.md or ./CLAUDE.md
  • User-level: ~/.zc/CLAUDE.md
  • Inject / read / append
...
MEMORY_FILE   = Path(".zc/CLAUDE.md")
USER_MEMORY   = Path(os.path.expanduser("~/.zc/CLAUDE.md"))
...
class HooksEngine:
    ...
    Hooks receive JSON via stdin and return:
      ...
      exit 2  → block operation (error message in stdout fed back to Claude)
    ...
    def with_plugins(cls, base: "HooksEngine") -> "HooksEngine":
        ...
        try:
            from zc_plugins import load_plugin_hooks
...
class McpConnector:
    ...
    def from_json_file(cls, path: Path = MCP_JSON) -> "McpConnector":
        ...
        try:
            from zc_plugins import load_plugin_mcp_servers
...
class SubagentRegistry:
    ...
    def load(self):
        ...
        try:
            from zc_plugins import load_plugin_agents
...
class SkillsRegistry:
    ...
    def load(self):
        ...
        try:
            from zc_plugins import load_plugin_skills
...
class MemoryManager:
    ...
    def read_project(self) -> str:
        for p in (Path(".zc/CLAUDE.md"), Path("CLAUDE.md")):
            if p.exists():
                return p.read_text()
        return ""
...
class CodeAgent:
    """
    Full ZaiCoder Code / Agent SDK implementation using the Messages API.
        ...
        try:
            if name == "Read":
                ...
                if os.environ.get("AI_CODER_SANDBOX") == "1":
                    try:
                        from zc_sandbox import enforce, SandboxViolation
    ...
    def query(
        ...
        while turn < max_turns:
            ...
            if context_management is not None:
                ...
                from zc_tools import CONTEXT_MANAGEMENT_BETA
...
def cmd_code_agent(
    ...
    """
    if not headless:
        print(f"\033[94mℹ ZaiCoder Code Agent | tools={tools} | permission={permission}\033[0m")
    ...
    if output_style:
        try:
            from zc_output_styles import system_prompt_fragment
    ...
    try:
        from zc_plugins import plugin_bin_paths
...
def cmd_code_slash(command: str, api_key: str, model: str,
                   cwd: str = ".", prompt: str = "", session_id: str = None):
    ...
    if cmd == "compact":
        ...
        from zc_tools import build_context_management, COMPACTION_BETA
        messages = session.messages()
        if not messages:
    ...
    try:
        from zc_plugins import load_plugin_commands
...
def _run_doctor():
    """Diagnostics for ZaiCoder Code environment."""
    print("\n\033[94mℹ ZaiCoder Code Diagnostics\033[0m")
    checks = [
        ...
        ("~/.zc/CLAUDE.md",    USER_MEMORY.exists()),
        ("Sessions dir",           SESSIONS_DIR.exists()),
    ]
    try:
        from zc_plugins import plugin_list, marketplace_list
