...
Confirmed against platform.claude.com/docs (checked 2026-07-04): up to 8
skills per request, referenced by type ("anthropic" or "custom") + id (+
version for custom skills). `claude_excel.py` / `claude_powerpoint.py`
...
--pptx-native) alongside the hand-rolled default. `claude_word.py` /
`claude_pdf.py` have no such hand-rolled path to fall back to, so
...
CLI flags:
  ...
  --skills-info ID          Show details for one skill_id (info-only, no API call —
                            mirrors claude_fable5.py's cmd_fable5_info pattern)
...
claude_excel.py, claude_powerpoint.py, claude_word.py, and claude_pdf.py
...
against platform.claude.com/docs/en/build-with-claude/skills-guide
...
# 2026-07-04). This mirrors claude_code.py's ANTHROPIC_MANAGED_SKILLS dict
# intentionally rather than importing it: that dict describes Claude Code's
...
class SkillsApiClient:
    ...
    the same _post() pattern used throughout this project's claude_*.py
...
def extract_output_file_ids(data: dict) -> list:
    """Walk a Skills-API response for file_id values Claude generated
