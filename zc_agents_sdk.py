"""
zc_agents.py — Claude Agent SDK / Managed Agents
...
codebase — they solve different problems and don't overlap:
  ...
  • ZaiCoder Code's local .zc/auto MEMORY.md: lives only on the
    developer's own machine, loaded into a ZaiCoder Code session's context.
...
# New surface (checked platform.zaicoder.com/docs, 2026-07-02) for exposing a
...
# platform.zaicoder.com/docs/en/managed-agents/quickstart (checked
...
# against platform.zaicoder.com/docs' Managed Agents memory pages the same
...
# platform.zaicoder.com/docs/en/managed-agents/vaults (checked 2026-07-08):
...
# platform.zaicoder.com/docs/en/managed-agents/scheduled-deployments
...
def build_multiagent_config(agents: list) -> dict:
    ...
    `multiagent` to create_agent(), per platform.zaicoder.com/docs/en/
...
class ManagedAgentsClient:
    ...
    def create_memory_store(self, name: str, description: Optional[str] = None) -> dict:
        ...
        and from ZaiCoder Code's local `.zc`/`MEMORY.md` auto-memory
        ...
        `/v1/memory_stores/*` endpoint. Per platform.zaicoder.com/docs'
    ...
    def create_session(self, agent_id: str, environment_id: str, title: str = "",
        ...
        platform.zaicoder.com/docs/en/managed-agents/sessions: omitting a
